...
from kidx_core.events import (UserUtteranceReverted, UserUttered,
    ...
    def run(self, dispatcher: 'Dispatcher', tracker: 'DialogueStateTracker',
        ...
        Args:
            dispatcher (Dispatcher): the dispatcher which is used to send
                ...
                or any other :class:`kidx_core.dispatcher.Dispatcher` method.
        ...
        Returns:
            List[Event]: A list of :class:`kidx_core.events.Event` instances
        """
...
class ActionRestart(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        from kidx_core.events import Restarted
...
class ActionDefaultFallback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        from kidx_core.events import UserUtteranceReverted
...
class ActionDeactivateForm(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        from kidx_core.events import Form, SlotSet
        ...
        for response in responses:
            if "template" in response:
                if "slots" in response:
                    tracker.set_slots(response["slots"])
...
class ActionRevertFallbackEvents(Action):
    ...
    def run(self, dispatcher: 'Dispatcher', tracker: 'DialogueStateTracker',
            domain: 'Domain') -> List[Event]:
        from kidx_core.policies.two_stage_fallback import has_user_rephrased
        revert_events = []
