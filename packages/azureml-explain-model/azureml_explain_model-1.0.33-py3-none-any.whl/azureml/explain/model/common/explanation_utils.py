...
def _get_raw_feature_importances(importance_values, raw_feat_indices):
    ...
    sets = [set(i) for i in raw_feat_indices]
    # check whether any pair shares any items
    for i in range(len(sets) - 1):
        for j in range(i + 1, len(sets)):
            if len(sets[i].intersection(sets[j])):
                raise ValueError('raw features cannot share feature indices')

    def f(x):
        raw_feat_importances = []
        for feature_indices in raw_feat_indices:
            raw_feat_importances.append(x[feature_indices].sum())
...
def _scale_tree_shap(shap_values, expected_values, prediction):
    ...
    for idx, shap_values in enumerate(values_to_convert):
        sum_feature_importances = np.sum(shap_values, axis=1)
        distance_base_pred = prediction[:, idx] - expected_values[idx]
        linear_scaler = sum_feature_importances / distance_base_pred
        values_to_convert[idx] = shap_values / linear_scaler[:, None]
    if is_binary:
        return values_to_convert[0]
