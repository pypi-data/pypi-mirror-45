...
class DatasetWrapper(object):
    ...
    def apply_indexer(self, column_indexer):
        """Indexes categorical string features on the dataset.

        :param indexer: The transformation steps to index the given dataset.
        :type indexer: ColumnTransformer
        """
        ...
        def convert_cols(category_to_index, value):
        ...
        for idx, i in enumerate(cols):
            ...
            self._dataset[:, i] = list(map(lambda x: convert_cols(category_to_index, x), self._dataset[:, i]))
