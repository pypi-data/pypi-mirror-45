...
def run_init(need_env):
    ...
    man_env = {}
    if need_env:
        man_env_str = input('Please input env: [key1=val1 key2=val2 ...] ')
        for seg in man_env_str.split():  # 为了兼容py2才这么实现
            key, val = seg.split('=', 1)
            man_env[key] = val
...
def entrypoint():
    lesscli.run(callback=run, single='hve')
