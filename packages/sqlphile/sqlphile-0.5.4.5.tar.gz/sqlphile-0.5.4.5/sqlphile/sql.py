...
class SQL:
    ...
    def execute (self):
        if self._explicit_empty:
            return self
        if self._check_filter and not self._filters:
            raise ValueError ("No filter for modification, if you want to modify anyway use .all ()")
    ...
    def into (self, table_spec):
        self._insert_into = "INSERT INTO " + table_spec
        return self
    ...
    def with_ (self, sql, alias):
        self._cte = "WITH {} AS ({})".format (alias, str (sql))
        return self
