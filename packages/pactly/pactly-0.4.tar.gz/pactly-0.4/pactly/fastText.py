...
class Text2Dataset:
    ...
    def load_dataset(self, dataset):
        ...
        """
        words_list, label_list = self.text2List(dataset)
...
class FastText:
    ...
    def train(
        ...
        train_X, train_y = self.text2Dataset.load_dataset(dataset)
    ...
    def save_model_bin(self, model_path):
        model_bin = "{0}.bin.gz".format(model_path)

        with gzip.open(model_bin, "wb") as f:
            pickle.dump(
                (
                    self.text2Dataset.word_ngrams,
                    self.text2Dataset.min_count,
    ...
    def save_ngram_vectorizer(self, model_path):
        with open("{0}_wordvec.pkl".format(model_path), "wb") as file:
            pickle.dump(self.text2Dataset, file)
        return

    def save_model(self, label_name, test_results, export_dir):
        """
            Save the model in binary and json + h5 formats.
        """
        filename = "{0}_{1:3f}".format(label_name, test_results["f1_score"])
        model_path = os.path.join(export_dir, filename)

        self.save_model_bin(model_path)
        self.save_model_json(model_path)
        self.save_ngram_vectorizer(model_path)
        print("Model files are saved to {}.".format(export_dir))
        return
    ...
    def calculate_accuracy_metrics(self, pred_val, true_val, show_results=True):
        ...
        model_metrics["true_pos"] = numpy.sum(
            numpy.logical_and(pred_val == 1, true_val == 1)
        )
        model_metrics["true_neg"] = numpy.sum(
            numpy.logical_and(pred_val == 0, true_val == 0)
        )
        model_metrics["false_pos"] = numpy.sum(
            numpy.logical_and(pred_val == 1, true_val == 0)
        )
        model_metrics["false_neg"] = numpy.sum(
