...
from promium.core.support.settings import (
    BASE_CAPABILITIES,
    get_download_path,
    SettingsDrivers,
    SettingsChrome,
...
@pytest.mark.unit
def test_set_device():
    Base.set_device(666, 13, 'user agent xxx')
    assert Base.device == Device(666, 13, 'user agent xxx')


@pytest.mark.unit
def test_set_device_without_user_agent():
    Base.set_device(123, 321)
    assert Base.device == Device(123, 321, '')
...
def test_set_windows_size():
    Base.device = Device(123, 321, 'xxx')
