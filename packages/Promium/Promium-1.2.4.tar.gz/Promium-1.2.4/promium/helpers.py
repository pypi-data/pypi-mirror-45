...
class ParseUrl(object):
    ...
    # def subdomain(self):
    #     """
    #     :return: subdomain, for example: my
    #     """
    #     if not re.match(PROJECT_HOST, self.hostname):
    #         host_list = self.hostname.split(u'.')
    #         return host_list[0]
    ...
    def port(self):
        ...
        """
        parse_url = urlparse(self.url)
        parse_netloc_split = parse_url.netloc.split(u':')
    ...
    def query(self):
        ...
        if parse_url.query:
            return dict(
                [query.split(u'=') for query in parse_url.query.split(u'&')]
            )
