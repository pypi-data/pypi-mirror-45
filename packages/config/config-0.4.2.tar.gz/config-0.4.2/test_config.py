# Copyright 2004-2019 by Vinay Sajip. All Rights Reserved.
#
...
class TestConfig(unittest.TestCase):
    ...
    def testSave(self):
        ...
        out = OutStream()
        self.cfg.save(out)
        self.assertEqual("message : 'Goodbye, cruel world!'" + config.os.linesep,
           out.value)
    ...
    def testEval(self):
        ...
        f = logging.Logger.debug
        if PY2:
            f = f.im_func
        self.assertEqual(f, self.cfg.debug)
        self.assertRaises(ConfigResolutionError, lambda x: x.DEBUG, self.cfg)
    ...
    def testJSON(self):
        with open('styles.json') as f:
            data = StringIO('dummy: ' + f.read())
        self.cfg.load(data)

    def testFileStream(self):
        self.cfg.load(config.defaultStreamOpener('file.cfg'))
        self.assertEqual(123,self.cfg.key1)
        self.assertEqual(KeyError,self.cfg.map1.key2)

    def testFileStream(self):
        self.cfg.load(config.defaultStreamOpener('file.cfg'))
        self.assertEqual(123,self.cfg.key1)
        self.assertEqual(KeyError,self.cfg.map1.key2)
        self.assertTrue('apple' in self.cfg.fruits)
        self.assertTrue('pomegranate' not in self.cfg.fruits)
        self.assertTrue('orange' in self.cfg.fruits)
        self.assertFalse('kiwi' in self.cfg.fruits)
        self.assertTrue('banana' in self.cfg.fruits)
