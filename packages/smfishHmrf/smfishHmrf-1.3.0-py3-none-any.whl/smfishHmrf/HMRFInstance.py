...
nstart <- commandArgs(trailingOnly = TRUE)[2]
mem_file <- commandArgs(trailingOnly = TRUE)[3]
centroid_file <- commandArgs(trailingOnly = TRUE)[4]
kmeans_file <- commandArgs(trailingOnly = TRUE)[5]
...
for(bx in 1:beta_num_iter){
	...
	def __init__(self, prefix, dest, dataset, *args, **kwargs):
		...
		use_existing_init = kwargs.get("use_existing_unit", False)
		...
		if use_existing_init:
			...
			for t_k in self.k:
				...
				if os.path.isfile(centroid_file) and os.path.isfile(kmeans_file):
					...
					missing = True
					break
			assert missing, "Use_existing_init=True, but some centroid and kmeans are missing..."
	...
	def init(self, nstart=1000):
		...
		for t_k in self.k:
			...
			#kmeans_file))
			subprocess.call("Rscript generateCentroids.multi.R %d %d %s %s %s" % (t_k, \
