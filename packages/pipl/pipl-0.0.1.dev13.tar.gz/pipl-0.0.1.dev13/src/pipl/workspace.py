...
def _create_workspace(path, name=None):
    ...
    # dot_devpi = os.path.join(path, '.devpi_clientdir')
    # print 'Creating devpi clientdir', dot_devpi 
    # os.mkdir(dot_devpi)
    ...
    virtualenv.create_environment(
        home_dir=venv_path,
        site_packages=True,     # this is needed to use pypipipe from the packages
        prompt=venv_prompt,
        #search_dirs=[], # I feel like we gonna use this at some point.
    )
...
def shell(workspace_path):
    extra_env = {
        'PYTHONPATH': str(get_workspace_src_path(workspace_path)),
    }
    ...
    command_args = [activate_path]

    command_args = ['start', 'cmd', '/K'] + command_args # use /K instead of /C to keep shell open ?
    shell = True

    print '>>>', subprocess.list2cmdline(command_args)
    subprocess.Popen(command_args, shell=True, env=shell_env)
...
def python(workspace_path):
    ...
    python_path = os.path.join(
        workspace_path, '.VENV', 'Scripts', 'python.exe'
    )
    command_args = [python_path, '-i', '-c', 'import sys; sys.ps1 = %r'%ps1]

    command_args = ['start', 'cmd', '/C'] + command_args # use /K instead of /C to keep shell open ?
    shell = True
