import sys, os, logging, traceback, time, datetime, subprocess, socket, json, uuid, hashlib, copy, unicodedata, thread, urllib, base64, StringIO, gzip, binascii
import requests
...
FILMHUB_CLOUD_REGISTRY_HOSTNAME="filmhub.cloud"
FILMHUB_PORT=443
...
class Session(object):
	...
	def __init__(self, domain=None, username=None, api_key=None, pwd=None, hostname=None, port=None, proxy=None, verbose=False, dev=False):
		...
		self.verbose("Creating FilmHUB Python API session (v%s)"%__version__)
		if domain is None:
			assert ('FILMHUB_DOMAIN' in os.environ or 'FILMHUB_ORG' in os.environ),("Please supply your FilmHUB domain/organization or set FILMHUB_DOMAIN environment!")
		self._domain = domain or (os.environ['FILMHUB_DOMAIN'] if 'FILMHUB_DOMAIN' in os.environ else os.environ['FILMHUB_ORG'])
		if username is None:
			assert ('FILMHUB_API_USER' in os.environ),("Please supply your FilmHUB user name (E-mail) or set FILMHUB_API_USER environment!")
		self._username = username or os.environ['FILMHUB_API_USER']
		if api_key:
			self._api_key = api_key
		else:
			self._api_key = os.environ.get('FILMHUB_API_KEY')
		if len(self._api_key or "") == 0:
			if 0<len(pwd or ""):
				# Store it temporarily
				self._pwd = pwd
			else:
				raise Exception("Please supply your FilmHUB API KEY or set FILMHUB_API_KEY environment!")
		self._hostname = hostname
		self._port = port or FILMHUB_PORT
		if self._hostname is None:
			if self._dev:
				self._hostname = "172.16.178.157"
			else:
				# Get domain
				result = self.rest("PUT", FILMHUB_CLOUD_REGISTRY_HOSTNAME, "registry/organization/domain", {'organization':self._domain})
				# Store hostname
				assert ('domain' in result),("No domain were provided for us!")
				self._hostname = "%s.%s"%(result['domain'], FILMHUB_CLOUD_REGISTRY_HOSTNAME) 
	...
	def login(self, revive_session_key=None):
		...
		if self._api_key:
			headers = {
				"Authorization":"FHCredentials %s"%base64.b64encode('{"domain":"%s","username":"%s","api_key":"%s"}'%(self._domain, self._username, self._api_key))
			}
		else:
			headers = {
				"Authorization":"FHCredentials %s"%base64.b64encode('{"domain":"%s","username":"%s","pwd":"%s"}'%(self._domain, self._username, base64.b64encode(self._pwd)))
			}
	...
	def event(self, method, uri, data, query=None, entityid=None, timeout=None, ssl=True, quiet=False):
		''' Generate an Event and send using REST to FilmHUB cloud '''
	...
	def rest(self, method, hostname, uri, data, timeout=None, ssl=True, port=None, quiet=False, headers=None):
		...
		if port is None:
			port = self._port or FILMHUB_PORT
		if hostname is None:
			hostname = "%s.%s"%(self._domain, FILMHUB_CLOUD_REGISTRY_HOSTNAME)
		# Proxy set?
		proxy = self._proxy or os.environ.get('FILMHUB_PROXY')
		if 0<len(proxy or ""):
			if 0<proxy.find(":"):
				parts = proxy.split(":")
				hostname = parts[0]
		...
		r = None
		#data = FilmHUB.prepare_rest_serialize(data)
		initial_timeout = timeout
		if headers is None:
			if uri.find("registry/")!=0:
				assert (not self._session_key is None),("Need to be authenticated when communicating with FilmHUB!")
				headers = {'Authorization':"FHSession %s"%(base64.b64encode('{"domain":"%s","username":"%s","session_key":"%s"}'%(self._domain, self._username, self._session_key)))}
			else:
				headers = {}
		headers['FHDevice'] = "PythonAPI v%s @ %s %s"%(__version__, sys.platform, os.name)
		for iteration in range(0,2):
			while True:
				...
				try:
					...
					if method.lower() == "get":
						...
						r = requests.delete(url, params = urllib.quote(Session.safe_dumps(data)), timeout=(CONNECT_TO, READ_TO), verify=False, headers=headers)
			...
			try:
				#retval = FilmHUB.prepare_rest_deserialize(r.json())
		...
		if 'exception' in retval:
			...
			Session.warning(retval['message'])
	...
	def update_one(self, entitytype, entityid, data):
		''' Update an entity '''
		assert (0<len(entitytype or "") and (isinstance(entitytype, str) or isinstance(entitytype, unicode))),("Invalid entity type supplied, must be of string type!")
