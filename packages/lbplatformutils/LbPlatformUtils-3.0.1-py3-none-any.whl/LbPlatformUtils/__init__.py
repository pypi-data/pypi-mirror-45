...
def dirac_platform(force_os=None):
    ...
    from LbPlatformUtils.inspect import architecture, os_id
    arch = architecture()
    short_id = normal_name(force_os or os_id(), OS_ALIASES, 'unknown')
...
def check_compatibility(a, b, compatibility_map):
    ...
    if a != b:
        ...
        if '!' + b in allowed:  # explicitely disallowed
            return False
        if b not in allowed:  # not explicitely allowed, recurse
...
def compatible_platforms(required):
    ...
    all_OSs = sorted(all_OSs,key=lambda s: list(map(int, findall(r'\d+', s))), reverse=True)
