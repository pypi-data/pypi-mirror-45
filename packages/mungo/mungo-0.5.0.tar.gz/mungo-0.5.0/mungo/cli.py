...
from mungo.common import vparse, print_dag, mungoversion, NodeId
from mungo.repositorydata import RepositoryData
from mungo.solver import create_dag, nodes_to_install


def _install_prompt(to_install: Dict[NodeId, Dict], installed: dict = None, ask_for_confirmation: bool = True) -> bool:
    if len(to_install) == 0:
        print("Nothing to do.")
        exit(0)
...
def main():
    ...
    if command == "create":
        create_environment(name, channels, packages, pip, jobs, dag, ask_for_confirmation, offline, force_download)
    elif command == "install":
        install_packages(name, channels, packages, pip, jobs, dag, ask_for_confirmation, offline, force_download)


def create_environment(name: str, channels: list, packages: set, pip: list, jobs: int = 1, dag: bool = False,
                       ask_for_confirmation: bool = False, offline: bool = False, force_download: bool = False):
    G = create_dag(channels, packages, [], jobs, offline, force_download)
    to_install = nodes_to_install(G, channels)
    if dag:
        print_dag(G, packages, to_install)
    else:
...
def install_packages(name: str, channels: list, packages: set, pip: list, jobs: int = 1, dag: bool = False,
                     ask_for_confirmation: bool = False, offline: bool = False, force_download: bool = False):
    local_repodata = RepositoryData.from_environment(name)
    G = create_dag(channels, packages, [local_repodata], jobs, offline, force_download)
    to_install = nodes_to_install(G, channels)
    if dag:
        print_dag(G, packages, to_install, installed=local_repodata.data)
    else:
        to_install = [n for n in to_install if "installed" not in n.p]
        installed = local_repodata.data
