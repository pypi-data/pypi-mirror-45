...
from mungo.common import vparse, print_dag, mungoversion
from mungo.repositorydata import RepositoryData
from mungo.solver import create_dag, nodes_to_install


def _install_prompt(to_install: list, installed: dict = None, ask_for_confirmation: bool = True) -> bool:
    if len(to_install) == 0:
        print("Nothing to do.")
        exit(0)
...
def create_environment(name: str, channels: list, packages: set, pip: list, jobs: int = 1, dag: bool = False,
    ...
    if dag:
        print_dag(packages, all_nodes, to_install)
    else:
...
def install_packages(name: str, channels: list, packages: set, pip: list, jobs: int = 1, dag: bool = False,
    ...
    to_install = nodes_to_install(all_nodes, root, channels)
    if dag:
        print_dag(packages, all_nodes, to_install, installed=local_repodata.d)
    else:
