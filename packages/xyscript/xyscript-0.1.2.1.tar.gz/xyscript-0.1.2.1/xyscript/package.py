...
class Package:
    ...
    def change_default_net_env(self,env):
        ...
        filepath = self._search_xcodeproj()
    ...
    def package_to_platform(self,platform=None):
        """
        ...
        if Cert().fastlane_is_in_gem() or Cert().fastlane_is_in_brew() :
            ...
            self.package_to_platform(platform)

    def auto_package(self, project_address=None, branch_name=None, platform=None, net_env=None):
        if branch_name == None:
            branch_name = "Develop"
        if platform == None:
            platform = "pgyer"

        print("自动打包 项目：",project_address," 分支为：",branch_name," 发布平台为：",platform,"网络环境：",net_env)
