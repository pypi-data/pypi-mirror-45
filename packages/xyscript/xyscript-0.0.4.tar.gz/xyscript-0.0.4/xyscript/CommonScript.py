...
class GitLabTool:
    ...
    def change_dev_branch(self):
        ...
        # 查看当前分支
        # print(git)
        # print("返回所有本地分支列表",git.refs)#返回所有本地分支列表
        # print("获取当前head指向",git.head.reference)#获取当前head指向
        # print("返回Head列表",git.heads)#返回Head列表
        # print("返回分支",git.active_branch)#
        # branch_name = 'Develop'
    ...
    def change_branch_g(self, branch_name, git=None):
        if git is None:
            repo = Repo(os.getcwd())
            # print(repo.head.reference)#获取当前head指向
            # print(repo.heads)
            # print(repo.remotes)
            # print(repo.refs)
            ...
            if branch_name == current_branch:
                ...
                if branch_name in local_branch_names:
                    ...
                    if 'origin/' + branch_name in remote_branch_names:
                        print("远端存在目标分支同名分支，checkout")
                    else:
                        print("远端不存在目标分支同名分支，无法checkout,请先创建远端仓库分支再checkout")
