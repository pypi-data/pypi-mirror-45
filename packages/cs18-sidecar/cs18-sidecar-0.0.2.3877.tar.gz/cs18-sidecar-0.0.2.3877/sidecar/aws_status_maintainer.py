...
class AWSStatusMaintainer:
    ...
    class IngressRoute:
        def __init__(self, listener_port: int, path: str, host: str, app_port: int, app_name: str):
    ...
    def get_ingress_routes(self) -> List[IngressRoute]:
        items = []
        for ingress_route in self.table_data.get('ingress_routes', []):
            items.append(AWSStatusMaintainer.IngressRoute(
