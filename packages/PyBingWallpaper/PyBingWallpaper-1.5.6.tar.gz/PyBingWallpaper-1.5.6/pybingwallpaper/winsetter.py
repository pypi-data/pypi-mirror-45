...
if sys.platform.startswith('win32'):
    ...
    class Win32WallpaperSetter(setter.WallpaperSetter):
        ...
        def set(self, path, args):
            k = None
            ret = False
            in_path = path.replace('/', '\\')
            path = "{}\\wallpaper.bmp".format(dirname(in_path))
            # windows only supports BMP, convert before setting
            try:
                convert_photo_to_bmp(in_path, path)
            ...
            try:
                ...
                last_value = self._read_value(k)
                if last_value and self.BACKUP:
                    ret = self._set_value(k, last_value[0], self.VALUE_NAME + 'Backup')
