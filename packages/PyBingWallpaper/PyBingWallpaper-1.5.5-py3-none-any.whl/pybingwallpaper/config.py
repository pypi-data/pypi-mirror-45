...
def str_to_bool(x):
    return True if bool(x) and x.lower() != 'false' else False
...
class ConfigFileLoader(ConfigLoader):
    ...
    def load_value(self, param, parser, _, generate_default):
    ...
    def load(self, db, data, generate_default=False):
        ...
        for param in db.parameters:
            if not param.is_loader_supported(self.OPT_KEY):
                continue
            loaded, key, value = \
...
class ConfigFileDumper(ConfigDumper):
    ...
    def get_param_by_name(self, db, name):
        by_key = list(filter(
            lambda param: name == param.get_option(self.OPT_KEY, 'key', None),
            db.parameters
        ))
        if len(by_key) > 1:
        ...
        by_name = list(filter(lambda param: name == param.name, db.parameters))
        if len(by_name) > 1:
...
class CommandLineArgumentsLoader(ConfigLoader):
    ...
    def assemble_parser(db, generate_default):
        ...
        for param in db.parameters:
            if not param.is_loader_supported(CommandLineArgumentsLoader.OPT_KEY):
                continue
