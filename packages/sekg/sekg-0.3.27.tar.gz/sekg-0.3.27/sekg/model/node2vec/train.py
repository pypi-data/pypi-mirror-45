...
class GraphNode2VecTrainer:
    def __init__(self, graph_data, directed=False, weight="weight"):
        self.node_num = graph_data.get_node_num()
        self.nx_G_instance = None
        if directed:
            self.init_weight_graph(graph_data, weight=weight)
        else:
    ...
    def init_weight_graph(self, graph_data, weight="weight"):
        '''
        INIT WEIGHT GRAPH: init weight graph by unweight graph and edge weight list
        :param weight_list: [(id1, id2, weight)]
        :return:
        '''
        # First Step: Compute the weight of a
        node_ids = graph_data.get_node_ids()
        relation_pairs_with_type = graph_data.get_relation_pairs_with_type()
        print("node num=%d" % self.node_num)
        print("relation num=%d" % len(relation_pairs_with_type))

        print(" pre-compute weight start")

        self.graph_data = WeightGraphData()
        self.graph_data.init_from_directed_graph(graph_data=graph_data, weight=weight)
        print("pre-compute weight end")

        weight_relation_tuples = []
        for (start_id, relation_type, end_id) in relation_pairs_with_type:
            weight = self.graph_data.get_relation_tuple_weight(start_node_id=start_id, relation_name=relation_type,
                                                               end_node_id=end_id)
            print(weight)
