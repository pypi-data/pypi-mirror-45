...
class GraphData(gensim.utils.SaveLoad):
    ...
    def print_relation_info(self):
        ...
        print("Relation Num=%d" % len(relation_type_to_relation.keys()))
    ...
    def subgraph(self, node_ids):
        ...
        graph_data.graph = graph_data.graph.subgraph(node_ids)
        for label in graph_data.label_to_ids_map.keys():
            graph_data.label_to_ids_map[label] = graph_data.label_to_ids_map[label] & node_ids

        remove_nodes = set(self.get_node_ids()) - node_ids
        for node_id in remove_nodes:
            graph_data.index_collection.remove_node(node_id)
