...
class WeightGraphData(SaveLoad):
    ...
    '''
    def init_from_directed_graph(self, graph_data):
        ...
        multigraph = computer.generate_weight_graph()
        self.precompute_weight(graph_data=graph_data)
        for (start_id, end_id, relation_type, weight) in multigraph.edges.data("weight", default=1, keys=True):
            if start_id in self.relation_type_weight_dict.keys():
                weight = (1-weight) * self.relation_type_weight_dict[start_id]
            else:
                weight = (1 - weight) * self.relation_type_weight_dict[end_id]
            self.graph.add_edge(start_id, end_id, key=relation_type, weight=weight)

    def precompute_weight(self, graph_data):
        relation_type_map = {}

        for node_id in graph_data.get_node_ids():
            # get the out degree of a node
            out_relation_list = graph_data.get_all_out_relations(node_id=node_id)

            for start_id, relation_type, end_id in out_relation_list:
                if node_id not in relation_type_map:
                    relation_type_map[node_id] = [relation_type]
                elif relation_type not in relation_type_map[node_id]:
                    relation_type_map[node_id].append(relation_type)
        for node_id in relation_type_map.keys():
                self.relation_type_weight_dict[node_id] = 1/len(relation_type_map[node_id])
...
class WeightGraphDataComputer:
    ...
    def generate_weight_graph(self):
        ...
        for start_id, end_id, relation_type in undirected_graph.edges(keys=True):
            # print(start_id, end_id, relation_type)
        ...
        # print(undirected_graph.edges.data("weight", default=1, keys=True))
    ...
    def compute_r_weight(graph_data):
        ...
        for (start_id, end_id), relation_types in r_to_relation_types_map.items():
            ...
            for r in relation_types:
                w = node_id_to_relation_type_count[start_id][r] + node_id_to_relation_type_count[end_id][r]
