                ...
                async with aiohttp.ClientSession() as session:
                    with async_timeout.timeout(10):
                        async with session.get(self.API_URL, params=params, proxy=self.proxy_server) as response:
                            ...
                            result = [{"pageid": item["pageid"], "title": item["title"], "snippet": item.get("snippet", "")} for item in json_data["query"]["search"]]
            ...
            async with self.semaphore:
                async with aiohttp.ClientSession() as session:
                    with async_timeout.timeout(10):
                        async with session.get(self.API_URL, params=params, proxy=self.proxy_server) as response:
                            json_data = await response.json()
                            self.item_cache[title] = WikiDataItem(title, init_at_once=False).init_wikidata_item_from_json(json_data)
...
if __name__ == '__main__':
    ...
    ids = searcher.search_title(["apple", "orange"])
    ...
    # print(ids)
    # searcher.save(title_save_path="titles.bin")
    ids = {item["title"] for v in ids.values() for item in v}
    result = searcher.fetch_item(ids)
    # searcher.save(item_save_path="items.bin")
    print(result["Q982388"].data_dict)
    print(result["Q982388"].get_en_name())
    # import sys
    # print(result["Q982388"].source_wd_dict_json)
