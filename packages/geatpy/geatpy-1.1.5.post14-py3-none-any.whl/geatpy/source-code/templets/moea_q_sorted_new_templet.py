...
def moea_q_sorted_new_templet(AIM_M, AIM_F, PUN_M, PUN_F, FieldDR, problem, maxormin, MAXGEN, MAXSIZE, NIND, SUBPOP, GGAP, selectStyle, recombinStyle, recopt, pm, distribute, drawing = 1):
    
    """
moea_q_sorted_new_templet.py - 基于改进的快速非支配排序法求解多目标优化问题的进化算法模板

语法：
    该函数除参数drawing外，不设置可缺省参数。当某个参数需要缺省时，在调用函数时传入None即可。
    比如当没有罚函数时，则在调用编程模板时将第3、4个参数设置为None即可，如：
    moea_q_sorted_new_templet(AIM_M, 'aimfuc', None, None, ..., maxormin,...)
...
"""
    ...
    [FitnV, frontIdx] = ga.ndominfast(maxormin * ObjV, LegV) # 快速非支配排序，一定要传入FitnV从而对排除考虑不满足约束条件的解
    if PUN_F is not None:
    ...
    for gen in range(MAXGEN):
        ...
        [ObjVSel, LegVSel] =  aimfuc(SelCh, LegV) # 求目标函数值
        # 父子合并
        Chrom = np.vstack([Chrom, SelCh])
        ObjV = np.vstack([ObjV, ObjVSel])
        LegV = np.vstack([LegV, LegVSel])
        [FitnV, frontIdx] = ga.ndominfast(maxormin * ObjV, LegV) # 快速非支配排序
        if PUN_F is not None:
            FitnV = punishing(LegV, FitnV) # 调用罚函数
        # 更新帕累托最优集以及种群非支配个体的适应度
        [FitnV, NDSet, NDSetObjV, repnum] = ga.upNDSet(Chrom, maxormin * ObjV, FitnV, NDSet, maxormin * NDSetObjV, frontIdx, LegV)
