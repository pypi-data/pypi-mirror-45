...
def moea_rwGA_templet(AIM_M, AIM_F, PUN_M, PUN_F, FieldDR, problem, maxormin, MAXGEN, MAXSIZE, NIND, SUBPOP, GGAP, selectStyle, recombinStyle, recopt, pm, distribute, drawing = 1):
    
    """
moea_rwGA_templet.py - 基于随机权重法(rwGA)的多目标优化的进化算法模板

语法：
    该函数除了参数drawing外，不设置可缺省参数。
    当某个参数需要缺省时，在调用函数时传入None即可。
    比如当没有罚函数时，则在调用编程模板时将第3、4个参数设置为None即可，如：
    moea_rwGA_templet(AIM_M, 'aimfuc', None, None, ..., maxormin,...)
