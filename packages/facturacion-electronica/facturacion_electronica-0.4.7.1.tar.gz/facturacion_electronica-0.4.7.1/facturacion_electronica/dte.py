...
class DTE(object):
    ...
    def setFirma(self, doc, tag='</DTE>', firma=None):
        result = doc.replace(tag, firma.decode() + '\n' + tag)
        return result
    ...
    def firmar(self, message, uri, type='doc'):
        ...
        firma = b''
        if self.firma.firma:
            sig_root = self.firma.firmar(string, uri, type)
        ...
        if type in ['doc', 'bol']:
            ...
            tag = '</RespuestaDTE>'
        fulldoc = self.setFirma(message, tag, firma)
        if self.verify:
            fulldoc = fulldoc if util.validar_xml(fulldoc, type) else ''
        return fulldoc
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX</RSR><MNT>10000</MNT><IT1>IIIIIII\
        ...
        ddxml = etree.tostring(result.find('DD')).decode('ISO-8859-1')\
            ...
            .replace('</item>', '')
        frmt = self.firma.signmessage(ddxml, keypriv)
...
folio: %s" % (
 ...
 deben incluir una referencia por obligación" % (self.TipoDTE, self.Folio))
    ...
    def timbrar(self):
        ...
        envelope_efact = self.crear_DTE(xml_pret)
        type = 'doc'
        if self.es_boleta():
            type = 'bol'
        einvoice = self.firmar(envelope_efact, doc_id_number, type)
