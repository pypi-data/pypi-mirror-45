...
xsi:schemaLocation="http://www.sii.cl/SiiDte RespuestaEnvioDTE_v10.xsd" >
    ...
    def set_firma(self, tag='</EnvioDTE>', firma=None):
        result = self.sii_xml_request.replace(tag, firma.decode() + '\n' + tag)
        return result

    def firmar(self, uri, type='env'):
        doc = etree.fromstring(self.sii_xml_request)
        string = etree.tostring(doc[0])
        firma = b''
        if self.firma_electronica.firma:
            sig_root = self.firma_electronica.firmar(string, uri, type)
            firma = etree.tostring(sig_root)
            firma = firma if util.validar_xml(firma, 'sig') else ''
        tag = '</EnvioDTE>'
        if type in ['doc']:
            tag = '</Documento>'
        elif type in ['consu']:
            tag = '</ConsumoFolios>'
        elif type in ['env_boleta']:
            tag = '</EnvioBoleta>'
        elif type in ['libro', 'libro_s']:
            tag = '</LibroCompraVenta>'
        elif type in ['libro_boleta']:
            tag = '</LibroBoleta>'
        elif type in ['libro_guia']:
            tag = '</LibroGuia>'
        elif type in ['env_recep']:
            tag = '</EnvioRecibos>'
        elif type in ['env_resp']:
            tag = '</RespuestaDTE>'
        elif type in ['recep']:
            tag = '</Recibo>'
        result = self.set_firma(tag, firma)
        if self.verify:
            self.sii_xml_request = result\
                if util.validar_xml(result, type) else ''
        else:
            self.sii_xml_request = result
