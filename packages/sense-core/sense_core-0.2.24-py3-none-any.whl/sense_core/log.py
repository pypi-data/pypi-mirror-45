...
def log_fatal(msg, module_type=''):
    global __logger
    if __logger is None:
        return
    __logger.error(str(msg))
    _send_fatal_message(msg, module_type)


def _send_fatal_message(message, module_type=''):
    try:
        global __module
        module = __module
        if not module:
        ...
        if producer is None:
            return
        json = {'module': module, 'message': message, 'time': get_current_second(), 'type': module_type}
