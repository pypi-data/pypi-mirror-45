...
def solve_and_derivative(A, b, c, cone_dict, **kwargs):
    ...
    Args:
      ...
      cone_dict: A dictionary with keys corresponding to cones, values
          ...
          'f', 'l', 'q', 's', 'ep', and 'ed' (for the zero cone, positive
          orthant, second-order cone, PSD cone, exponential cone, and dual
          exponential cone, respectively). A k-dimensional PSD cone
          corresponds to a k-by-k matrix variable; a value of k for `ep`
          (or `ed`) corresponds to k / 3 exponential cones.
      kwargs: (optional) Keyword arguments to supply to SCS.
