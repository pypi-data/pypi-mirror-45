...
`diffcp` exposes a function

```python
solve_and_derivative(A, b, c, cone_dict).
```

This function returns a primal-dual solution `x`, `y`, and `s`, along with functions for evaluating the derivative and its adjoint (transpose). These functions respectively compute right and left multiplication of the derivative of the solution map at `A`, `b`, and `c` by a vector.
...
```
@article{diffcp2019,
    author       = {Agrawal, A. and Barrat, S. and Boyd, S. and Busseti, E. and Moursi, W.},
