...
def clusterSM(folder,modelname,score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None,ch=None,condition=None):
    print('# clusterSM')
    ...
    if BuildModel : fig.savefig(os.path.join(desktop,"eigen-shape dendrogram with ID number_"+ch+".png"), dpi=1200)
    ...
    for kss in range(int(max(IDX))+1):
        ...
        plt.axis('off')
        if BuildModel : figeach.savefig(os.path.join(desktop,"eigen-shape "+dendidx[kss]+".png"), dpi=1200)
        plt.clf()
        plt.figure(289,figsize=(6,3))
    ...
    plt.axis('equal')
    plt.axis('off')
    if BuildModel : fig922.savefig(os.path.join(desktop,"eigen-shapes_"+ch+".png"), dpi=1200)
    fig289=plt.figure(289,figsize=(6,3))
    plt.axis('equal')
    plt.axis('off')
    if BuildModel : fig289.savefig(os.path.join(desktop,"eigen-shape dendrogram_"+ch+".png"), dpi=1200)
    crap=np.sort(IDX)
    sid=np.argsort(IDX)
    BMsort=cmsn[sid]
    fig20=plt.figure(20)
    ...
    # plt.ylabel('%')
    # fig22.savefig(os.path.join(desktop,'eigen-shape distribution_'+ch+'.png'))
    # plt.close('all')
    plt.close()
    fig22 = plt.figure(22,figsize = (10,5))
    ...
    if BuildModel:
        fig22.savefig(os.path.join(desktop,'eigen-shape distribution_'+ch+'_model_'+'.png'), dpi=1200)
    else:
        fig22.savefig(os.path.join(desktop,'eigen-shape distribution_'+ch+'_'+condition+'.png'), dpi=1200)
    plt.close('all')

    return IDX,bdsubtype,C,VamModel

def cluster_main(folder,modelname,score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None,cellornuc=None,condition=None):
    print('## clusterSM.py')
    start = time.time()
    IDX,bdsubtype,C,VamModel=clusterSM(folder,modelname,score,pc,bdpc,clnum,pcnum,VamModel,BuildModel,cellornuc,condition)
    end = time.time()
