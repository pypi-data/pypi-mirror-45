...
class LanguageChangeTestCase(LiveServerTestCase):
    ...
    def setUp(self):
        ...
        if settings.HIDE_FIREFOX:
            ...
            self.display.start()
        self.selenium = webdriver.Firefox()
        super(LanguageChangeTestCase, self).setUp()
    ...
    def tearDown(self):
        ...
        if settings.HIDE_FIREFOX:
            self.display.stop()
        self.selenium.quit()
        super(LanguageChangeTestCase, self).tearDown()
    ...
    def testing_search_ukrainian_button(self):
        ...
        ul_el = self.selenium.find_element_by_xpath('/html/body/div/div/div[2]/div/nav/div[2]/ul[1]')
        li_texts = set(ul_el.text.split('\n'))
        set_uk = {"Домашня сторінка", "Блог", "Портфоліо", "Відгуки", "Зв'яжіться з нами"}
