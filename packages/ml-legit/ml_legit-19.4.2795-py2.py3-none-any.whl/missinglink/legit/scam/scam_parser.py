...
def __conditions_expr_to_sql_where(field_name, item):
    sql = expr_to_sql_where(field_name, item)
...
def __condition_group_expr_to_sql_where_handle_range(field_name, item):
    ...
    if sql_format is not None:
        return sql_format.format(field_name=field_name, value1=quotes_if_needed(item[0]), value2=quotes_if_needed(item[1]))
...
def __condition_group_expr_to_sql_where(field_name, item):
    for handlers in (
        ...
        if result:
            return _wrap_in_parentheses(result)
...
class SQLQueryBuilder(MLQueryVisitor, MLQueryMixin):
    ...
    def __check_operator_dependencies(self):
        ...
        if self.__has_sample and self.__has_group_by and (self.random_function_level in self.__where):
