...
class DatabaseDictSerializer(object):
    ...
    def dump(self, obj, data=None, many=False, value_serializer=None, ignore_tables=None, follow_rels=True):
        ...
        if follow_rels and (isinstance(follow_rels, (list, tuple)) or hasattr(obj, '__export_follow_rels__')):
            ...
            for relattr in (follow_rels if isinstance(follow_rels, (list, tuple)) else obj.__export_follow_rels__):
                ...
                elif attr.target.name not in ignore_tables:
                    self.dump(getattr(obj, relattr), data, attr.uselist, value_serializer, ignore_tables)
        return data
