...
class BpApi(object):
    ...
    def create_project(self, uid, data):
    ...
    def update_project(self, uid, data):
    ...
    def print_data(self, data_type='', uid=None, json=False):
        ...
        if data_type not in self.LIST_TYPES:
            print(
                ('--list must be one of the following: ' +
                 '{}').format(','.join(self.LIST_TYPES)))
            return
        ...
        if data_type == 'genomes':
            ...
            if uid is None:
                ...
                return
            if len(uid) != 1:
                print('Supply only 1 uid: {}'.format(uid))
                return

            data = self.get_analysis(uid[0])
        ...
        if uid is not None and data_type != 'analysis':
            data = list(filter(lambda x: x['id'] in uid, data))

            if len(data) == 0:
                print('No items left after filtering for your uid(s): ', uid)
                return
            if 'id' not in data:
                print('Could not find item with id: {}'.format(id))
        ...
        if json:
            ...
            if data_type == 'genomes':
                ...
                print('completed_on: {}'.format(data['completed_on']))
                print('sample ids: {}'.format(
                    ', '.join([i.split('/')[-1] for i in data['samples']])))
                print()
                print('Files')

                # convert only filesizes that are not None
                for i, filedata in enumerate(data['files']):
                    if filedata['filesize'] is not None:
                        data['files'][i]['filesize'] /= (1024.**3)
                    else:
                        data['files'][i]['filesize'] = 'NA'
                ...
                tmp = [[
                    ...
                    i['tags']] for i in data['files']]
                [i['filesize'] for i in data['files']]
                print(
                    tabulate(
                        tmp,
                        headers=[
