...
from kidx_core_sdk.cli.arguments import add_endpoint_arguments
from kidx_core_sdk.constants import DEFAULT_SERVER_PORT
from kidx_core_sdk.executor import ActionExecutor
from kidx_core_sdk import ActionExecutionRejection
import kidx_core_sdk

from kidx_core_sdk import utils
...
def check_version_compatibility(core_version):
    """Check if the version of kidx_core and kidx_core_sdk are compatible.
    ...
    kidx_core and kidx_core_sdk.
    Args:
        core_version - A string containing the version of kidx_core that
        is making the call to the action server.
    Raises:
        Warning - The version of kidx_core version unkown or not compatible with
        this version of kidx_core_sdk.
    """
    # Check for versions of core that are too old to report their version number
    if core_version is None:
        logger.warning(
            "You are using an old version of kidx_core which might "
            "not be compatible with this version of kidx_core_sdk "
            "({}).\n"
            ...
            "both kidx_core and kidx_core_sdk."
            "".format(kidx_core_sdk.__version__)
        )
        return

    core = core_version.split(".")[:-1]
    sdk = kidx_core_sdk.__version__.split(".")[:-1]

    if core != sdk:
        logger.warning(
            "Your versions of kidx_core and "
            "kidx_core_sdk might not be compatible. You "
            "are currently running kidx_core version {} "
            "and kidx_core_sdk version {}.\n"
            ...
            "both kidx_core and kidx_core_sdk."
            "".format(core_version, kidx_core_sdk.__version__)
        )
...
def run(actions, port=DEFAULT_SERVER_PORT, cors="*"):
    ...
    http_server.start()
    logger.info("Action endpoint is up and "
                "running. on {}".format(http_server.address))
