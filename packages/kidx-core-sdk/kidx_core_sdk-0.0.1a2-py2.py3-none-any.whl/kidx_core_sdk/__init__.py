...
import kidx_core_sdk.version
import kidx_core_sdk.cli

logger = logging.getLogger(__name__)

__version__ = kidx_core_sdk.version.__version__

if typing.TYPE_CHECKING:
    from kidx_core_sdk.executor import CollectingDispatcher
...
class Tracker(object):
    ...
    def set_slot(self, key, value):
        """Temporary set the extractor entity to s slot"""

        if key in self.slots:
            self.slots[key] = value  # should check the validation of the value
            return True
        else:
            logger.info("Tried to access non existent slot '{}'".format(key))
            return False
    ...
    def __eq__(self, other):
        if isinstance(self, type(other)):
            return other.events == self.events and self.sender_id \
                   == other.sender_id
        else:
            return False
    ...
    def run(
        ...
        Args:
            dispatcher (CollectingDispatcher): the dispatcher which is used to
                ...
                ``kidx_core_sdk.executor.CollectingDispatcher``
                method.
            tracker (Tracker): the state tracker for the current
                ...
                ``kidx_core_sdk.Tracker`` property.
            domain (Dict[Text, Any]): the bot's domain
        Returns:
            List[Dict[Text, Any]]: A dictionary of
                ``kidx_core_sdk.events.Event`` instances that is
...
class ActionExecutionRejection(Exception):
    ...
    def __init__(self, action_name, message=None):
        ...
        self.message = message or "Custom action '{}' rejected " \
                                  "execution of".format(action_name)
