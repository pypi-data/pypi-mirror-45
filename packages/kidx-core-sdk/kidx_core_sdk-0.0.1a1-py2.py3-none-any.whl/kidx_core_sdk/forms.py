...
from kidx_core_sdk import Action, ActionExecutionRejection
from kidx_core_sdk.events import SlotSet, Form

logger = logging.getLogger(__name__)

if typing.TYPE_CHECKING:
    from kidx_core_sdk import Tracker
...
class FormAction(Action):
    ...
    def get_mappings_for_slot(self, slot_to_fill):
        ...
        for requested_slot_mapping in requested_slot_mappings:
            if (
                not isinstance(requested_slot_mapping, dict) or
        ...
        for requested_slot_mapping in requested_slot_mappings:
            ...
            if self.intent_is_desired(requested_slot_mapping, tracker):
                ...
                if mapping_type == "from_entity":
                    ...
                    raise ValueError("Provided slot mapping type "
                                     "is not supported")
    ...
    def validate(self, dispatcher, tracker, domain):
        ...
        for slot, value in slot_values.items():
            validate_func = getattr(self, "validate_{}".format(slot),
                                    lambda *x: value)
            slot_values[slot] = validate_func(value, dispatcher,
                                              tracker, domain)
