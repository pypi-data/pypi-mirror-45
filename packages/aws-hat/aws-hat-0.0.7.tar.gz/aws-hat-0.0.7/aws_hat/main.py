...
AWS_CREDENTIALS_FILE = '~/.aws/config'
...
def default(**kwargs):
    ...
    if kwargs['output'] == "env":
        ...
        print("AWS_SECRET_ACCESS_KEY={}".format(session['aws_secret_access_key']))
        ...
        if 'expiration' not in target:
            if len(target.keys()) > 0 and kwargs['force'] == False:
                print("Profile {} already used. Press enter to proceed or CTRL+C to cancel. ".format(kwargs['profile']))
                input()
