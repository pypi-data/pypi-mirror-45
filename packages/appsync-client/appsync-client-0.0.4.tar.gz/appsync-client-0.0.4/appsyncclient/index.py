...
apiIdDefault = os.environ.get("appsync_api_id")
API_KEY = "API_KEY"
AWS_IAM = "AWS_IAM"
...
class AppSyncClient():
    ...
    def execute(self,**kargs):
        ...
        apiId = kargs.get("api_id") if kargs.get("api_id") else apiIdDefault

        if authenticationType == API_KEY:
            headers = self.getHeaders(region,apiId)
        if authenticationType == AWS_IAM:
            headers = self.getSignedHeaders(url,method,region,data)
    ...
    def getApiKey(self,region,apiId):
        ...
        for keyItem in response.get("apiKeys",[]):
            if int(keyItem.get("expires")) > int(time.time())*1000:
                return keyItem.get("id")
        return None
    ...
    def getHeaders(self,region,apiId):
