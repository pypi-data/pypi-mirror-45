...
t_nocor = ('Uncorrected %1.3f %1.3f' %
           roc(grid_model_counts(X<alpha, nl, sl)))
fig_nocor.axes[0].set_title(t_nocor)

fig_sidak = plot_grid_model(Y_sidak, nl, sl)
t_sidak = ('Sidak %1.3f %1.3f' %
           roc(grid_model_counts(Y_sidak, nl, sl)))
fig_sidak.axes[0].set_title(t_sidak)

fig_fdr = plot_grid_model(Y_fdr, nl, sl)
t_fdr = ('FDR %1.3f %1.3f' %
         roc(grid_model_counts(Y_fdr, nl, sl)))
fig_fdr.axes[0].set_title(t_fdr)

fig_qvalue = plot_grid_model(Y_qvalue, nl, sl)
t_qvalue = ('Q-value %1.3f %1.3f' %
            roc(grid_model_counts(Y_qvalue, nl, sl)))
...
t_tst = ('Two-stage procedure %1.3f %1.3f' %
         roc(grid_model_counts(Y_tst, nl, sl)))
fig_tst.axes[0].set_title(t_tst)

fig_permutation = plot_grid_model(Y_permutation, nl, sl)
t_permutation = ('Permutation %1.3f %1.3f' %
                 roc(grid_model_counts(Y_permutation, nl, sl)))
fig_permutation.axes[0].set_title(t_permutation)

fig_holm = plot_grid_model(Y_holm, nl, sl)
t_holm = ('Holm-Bonferroni %1.3f %1.3f' %
          roc(grid_model_counts(Y_holm, nl, sl)))
fig_holm.axes[0].set_title(t_holm)

fig_hochberg = plot_grid_model(Y_hochberg, nl, sl)
t_hochberg = ('Hochberg %1.3f %1.3f' %
              roc(grid_model_counts(Y_hochberg, nl, sl)))
...
output_path = ('/home/local/puolival/multipy-simulation-results' +
               '/two-group-model-%1.1f' % delta)
...
tp_nocor, fp_nocor, tn_nocor, fn_nocor = grid_model_counts(X<alpha, nl, sl)

plt.figure()
plt.bar(np.arange(0, 4), [tp_nocor, fp_nocor, tn_nocor, fn_nocor])
plt.show()
