...
class MotorExt(Motor):
    ...
    Further documentation for the Motor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.Motor'
    ...
    def __init__(self, port, direction=Direction.CLOCKWISE, gears=None):
        ...
        """
        super(MotorExt, self).__init__(port, direction, gears)

    '''

    def dc(self, duty):
        super(MotorExt, self).dc(duty)

    def angle(self):
        return super(MotorExt, self).angle()

    def reset_angle(self, angle):
        super(MotorExt, self).reset_angle(angle)

    def speed(self):
        return super(MotorExt, self).speed()

    def stop(self, stop_type=Stop.COAST):
        super(MotorExt, self).stop(stop_type=Stop.COAST)
    ...
    def run_angle(self, speed, angle, stop_type=Stop.COAST, wait=True):
        super(MotorExt, self).run_angle(speed, angle, stop_type=stop_type, wait=wait)
    ...
    def track_target(self, target_angle):
        super(MotorExt, self).track_target(target_angle)

    def stalled(self):
        return super(MotorExt, self).stalled()

    def run_until_stalled(self, speed, stop_type=Stop.COAST, duty_limit=100):
        super(MotorExt, self).run_until_stalled(speed, stop_type=stop_type, duty_limit=duty_limit)
    ...
    def output_run_time(self, speed, time, stop_type=Stop.COAST, wait=True, depth=None):
        ...
        :type time: int
        :param stop_type: Whether to coast, brake or hold after coming to a stand still, defaults to Stop.COAST
        :type stop_type: Stop, optional
        :param wait: Whether to wait for the maneuver to complete before continuing with the rest of the program, defaults to True
        ...
        if depth is None:
            super(MotorExt, self).run_time(speed, time, stop_type=stop_type, wait=wait)
        super(MotorExt, self).run_time(speed / get_ratio(self.gears, depth=depth), time, stop_type=stop_type, wait=wait)

    def output_percent_run_time(self, speed, time, stop_type=Stop.COAST, wait=True, rpm=240, depth=None):
        """Keep the motor or linked gears running at a constant speed (percentage) for a specified amount of time

        Medium Motor    - 240 RPM
        Large Motor     - 160 RPM1
        
        :param speed: Speed of the Motor or Gear (percentage)
        :type speed: int, float
        :param time: Duration (milliseconds) of the maneuver
        :type time: int
        :param stop_type: Whether to coast, brake or hold after coming to a standstill, defaults to Stop.COAST
        :type stop_type: Stop, optional
        :param wait: Whether to wait for the maneuver to complete before continuing with the rest of the program, defaults to True
        :type wait: bool, optional
        :param rpm: RPM of the motor, defaults to 240
        ...
        """
        if depth is None:
            percent_run_time(speed, time, stop_type=stop_type, wait=wait, rpm=rpm)
        super(MotorExt, self).run_time(speed_deg(speed, rpm=rpm) / get_ratio(self.gears, depth=depth), time, stop_type=stop_type, wait=wait)
    ...
    def percent_run_time(self, speed, time, stop_type=Stop.COAST, wait=True, rpm=240):
        ...
        :type rpm: int, optional
        """
        super(MotorExt, self).run_time(speed_deg(speed, rpm=rpm), time, stop_type=stop_type, wait=wait)

    def output_run_angle(self, speed, rotation_angle, stop_type=Stop.COAST, wait=True, depth=None):
        """Keep the motor or linked gears running at a constant speed for a speicified amount of degrees
        ...
        super(MotorExt, self).run_angle(speed / ratio, rotation_angle / ratio, stop_type=stop_type, wait=wait)

    def output_percent_run_angle(self, speed, rotation_angle, stop_type=Stop.COAST, wait=True, rpm=240, depth=None):
        """Keep the motor or linked gears running at a constant speed (percentage) for a speicified amount of degrees
        ...
        :param rotation_angle: Angle (degrees) by which the Motor should run
        :type rotation_angle: int
        :param stop_type: Whether to coast, brake, or hold after coming to a tandstill, defaults to Stop.COAST
        :type stop_type: Stop, optional
        :param wait: Whether to wait for the maneuver to complete before continuing with the rest of the program, defaults to True
        :type wait: bool, optional
        :param rpm: RPM of the motor, defaults to 240
        ...
        """
        if depth is None:
            percent_run_angle(speed, rotation_angle, stop_type=stop_type, wait=wait, rpm=rpm)
        ratio = get_ratio(self.gears, depth=depth)
        super(MotorExt, self).run_angle(speed_deg(speed, rpm=rpm) / ratio, rotation_angle / ratio, stop_type=stop_type, wait=wait)

    def percent_run_angle(self, speed, rotation_angle, stop_type=Stop.COAST, wait=True, rpm=240):
        """Keep the motor running at a constant speed (percentage) for a speicified amount of degrees
        ...
        :type rpm: int, optional
        """
        super(MotorExt, self).run_angle(speed_deg(speed, rpm=rpm), rotation_angle, stop_type=stop_type, wait=wait)

    def output_run_target(self, speed, target_angle, stop_type=Stop.COAST, wait=True, depth=None):
        """Keep the motor or linked gears running at a constant speed towards a speicified target degree
        ...
        if depth is None:
            super(MotorExt, self).run_target(speed, target_angle, stop_type=stop_type, wait=wait)
        ratio = get_ratio(self.gears, depth=depth)
        super(MotorExt, self).run_target(speed / ratio, target_angle / ratio, stop_type=stop_type, wait=wait)

    def output_percent_run_target(self, speed, target_angle, stop_type=Stop.COAST, wait=True, rpm=240, depth=None):
        """Keep the motor or linked gears running at a constant speed (percentage) towards a speicified target degree
        ...
        """
        if depth is None:
            percent_run_target(speed, target_angle, stop_type=stop_type, wait=wait, rpm=rpm)
        ratio = get_ratio(self.gears, depth=depth)
        super(MotorExt, self).run_target(speed_deg(speed, rpm=rpm) / ratio, target_angle / ratio, stop_type=stop_type, wait=wait)

    def percent_run_target(self, speed, target_angle, stop_type=Stop.COAST, wait=True, rpm=240):
        """Keep the motor running at a constant speed (percentage) towards a speicified target degree
        ...
        :type rpm: int, optional
        """
        super(MotorExt, self).run_target(speed_deg(speed, rpm=rpm), target_angle, stop_type=stop_type, wait=wait)
    ...
    def output_run_until_stalled(self, speed, stop_type=Stop.COAST, duty_limit=100, depth=None):
        ...
        """
        if depth is None:
            super(MotorExt, self).run_until_stalled(speed, stop_type=stop_type, wait=wait)
        super(MotorExt, self).run_until_stalled(speed / get_ratio(self.gears, depth=depth), stop_type=stop_type, duty_limit=duty_limit)

    def output_percent_run_until_stalled(self, speed, stop_type=Stop.COAST, duty_limit=100, rpm=240, depth=None):
        ...
        """
        if depth is None:
            percent_run_until_stalled(speed, stop_type=stop_type, wait=wait, rpm=rpm)
        super(MotorExt, self).run_until_stalled(speed_deg(speed, rpm=rpm) * get_ratio(self.gears, depth=depth), stop_type=stop_type, duty_limit=duty_limit)
    ...
    def percent_run_until_stalled(self, speed, stop_type=Stop.COAST, duty_limit=100, rpm=240):
        ...
        :type rpm: int, optional
        """
        super(MotorExt, self).run_until_stalled(speed_deg(speed, rpm=rpm), stop_type=stop_type, duty_limit=duty_limit)
    ...
    # UNSURE IF set_run_settings TAKES INTO ACCOUNT THE GEAR TRAIN
    # Marked as deprecated until functionality is tested

    def output_set_run_settings(self, max_speed, acceleration, depth=None):
        ratio = get_ratio(self.gears, depth=depth)
        super(MotorExt, self).set_run_settings(max_speed * ratio, acceleration * ratio)

    def output_percent_set_run_settings(self, max_speed, acceleration, rpm=240, depth=None):
        ratio = get_ratio(self.gears, depth=depth)
        super(MotorExt, self).set_run_settings(speed_deg(max_speed) * ratio, speed_deg(acceleration) * ratio)

    def percent_set_run_settings(self, max_speed, acceleration, rpm=240):
        super(MotorExt, self).set_run_settings(speed_deg(max_speed), speed_deg(acceleration))
...
class TouchSensorExt(TouchSensor):
    ...
    Further documentation for the TouchSensor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.TouchSensor'
    ...
    def __init__(self, port):
        """
        Initiate the TouchSensorExt Object
        """
        super(TouchSensorExt, self).__init__(port)
    ...
    def wait_until_bumped(self, wait_timer=500):
        ...
        :param wait_timer: Time to wait (milliseconds) to consider a press and release a bump, defaults to 500
...
class ColorSensorExt(ColorSensor):
    ...
    Further documentation for the ColorSensor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.ColorSensor'
    ...
    def __init__(self, port):
        """
        Initiate the ColorSensorExt Object
        """
        super(ColorSensorExt, self).__init__(port)
...
class InfraredSensorExt(InfraredSensor):
    ...
    Further documentation for the InfraredSensor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.InfraredSensor'
    ...
    def __init__(self, port):
        """
        Initiate the InfraredSensorExt Object
        """
        super(InfraredSensorExt, self).__init__(port)
    ...
    def wait_until_beacon_distance(self, operator, beacon_distance):
        """Waits until the beacon distance matches certain conditions
        
        :param operator: Operator to be used for the calculation (>, <, <=, >=, ==, !=)
        :type operator: str
        :param beacon_distance: Beacon distance value to calculate against (InfraredSensorExt.beacon_distance <OP> beacon_distance)
        :type beacon_distance: int, float
        """
        while not _operator_calc(self.beacon_distance(), beacon_distance, operator):
            wait(10)
        return

    def wait_until_beacon_angle(self, operator, beacon_angle):
        """Waits until the beacon angle matches certain conditions
        
        :param operator: Operator to be used for the calculation (>, <, <=, >=, ==, !=)
        :type operator: str
        :param beacon_angle: Beacon angle value to calculate against (InfraredSensorExt.beacon_angle <OP> beacon_angle)
        :type beacon_angle: int, float
        """
        while not _operator_calc(self.beacon_angle(), beacon_angle, operator):
            wait(10)
        return
    ...
    def wait_until_button_bumped(self, button, channel, wait_timer=500):
        ...
        ''NOTE: using a list, tuple or dict for buttons will make this function act the same as wait_until_button_pressed if all of the buttons listed are pressed at once''
...
class UltrasonicSensorExt(UltrasonicSensor):
    ...
    Further documentation for the UltrasonicSensor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.UltrasonicSensor'
    ...
    def __init__(self, port):
        """
        Initiate the UltrasonicSensorExt Object
        """
        super(UltrasonicSensorExt, self).__init__(port)
...
class GyroSensorExt(GyroSensor):
    ...
    Further documentation for the GyroSensor class can be found 'https://klutzybubbles.github.io/lego-micropython-docs/ev3devices.html#ev3devices.GyroSensor'
