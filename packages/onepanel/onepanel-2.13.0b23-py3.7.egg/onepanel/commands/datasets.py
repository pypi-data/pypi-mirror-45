...
@datasets.command('pull', help='Pull down dataset and any changes')
@click.option(
    '-y', '--yes',
    is_flag=True,
    default=False,
...
def _datasets_pull(ctx, yes, threads):
...
def datasets_clone(ctx, path, directory, suppress_output=False, run_as_background=False, delete=False):
    ...
    exit_code = aws_util.download_all(home, s3_from_dir, delete)
    if exit_code != 0:
...
def _watch_sync(ctx, threads=10):
    ...
    synchronizer = ThreadedFileSynchronizer(synchronizer, threads, [print_status(cwd_length), update_api_on_success])
