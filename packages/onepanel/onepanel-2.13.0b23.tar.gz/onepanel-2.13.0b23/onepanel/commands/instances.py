...
@click.group(cls=AliasedGroup, help='Workspace commands group')
@click.pass_context
def workspaces(ctx):
    ...
    if id_only and not project_specific:
        ...
        vc.print_items(formatted_items,
                       fields=['uid', 'state', 'age', 'duration_ready', 'duration_paused', 'cpu', 'gpu', 'ram',
                               'hdd', ],
                       field_names=['ID', 'STATE', 'AGE', 'RUNNING', 'PAUSED', 'CPU', 'GPU', 'RAM', 'HDD'])
    else:
        vc.print_items(formatted_items,
                       fields=['account_uid', 'project_uid', 'uid', 'state', 'age', 'duration_ready', 'duration_paused',
                               'cpu', 'gpu', 'ram', 'hdd', ],
                       field_names=['ACCOUNT', 'PROJECT', 'ID', 'STATE', 'AGE', 'RUNNING', 'PAUSED', 'CPU', 'GPU',
                                    'RAM', 'HDD'])
...
def monitor_activity(ctx, identifier, cpu=None, gpu=None, user_duration=None, duration=None, time_repeat=None, verbose=None):
    ...
    def pause_workspace():
        vc.pause(account_uid=account_uid, project_uid=project_uid, instance_uid=instance_uid)
