...
class DatasetDownloader(Thread):
    ...
    def finished_downloading_hook(self):
        ...
        cfg['account_uid'] = self.owner_uid
        cfg.write()
    ...
    def start_downloading_dataset(self, connection, download_to_path, account_uid, dataset_uid, dataset_version):
        creds = CredsUtility.get_credentials(connection, account_uid, 'datasets', dataset_uid)
        ...
        if cloud_provider == 'GCP':
            logging.getLogger(LOGGER).info('starting GCP download')
            return util.download_all_background(download_to_path, cloud_storage_from_dir, account_uid, dataset_uid, 'datasets')
        elif cloud_provider == "AWS":
            logging.getLogger(LOGGER).info('starting AWS download')
            return util.download_all_background(download_to_path, cloud_storage_from_dir)
...
class DatasetDownloadListener(Thread):
    ...
    def __init__(self, connection, download_path, account_uid, project_uid,
                 resource_type, resource_uid, continue_listening=True, verbose=False):
        Thread.__init__(self)
