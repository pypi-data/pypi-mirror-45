...
from gmdata.model.fundamental import FundamentalApi
from gmdata.model.history import HistoryApi
from gmdata.retrying import retry
from gmdata.utils import load_to_datetime_str, standard_fields, gmdatalogger
        ...
        try:
            ...
            if callable(details_fun) and callable(code_fun):
                ...
                if code is grpc.StatusCode.RESOURCE_EXHAUSTED and 'extract' in detailsstr:
                    ...
                    gmdatalogger.warn("你今天的数据限额已用完,请联系商务提高数据限额.返回空数据给你. api[%s]", func_name)
                    # 这个会记录到日志文件里
                    gmdatalogger.debug("你今天的数据限额已用完,请联系商务提高数据限额.返回空数据给你. api[%s] 返回的详细信息:%s\n 执行堆栈信息:%s",
                                       func_name, e, exc_msg)
                elif 'header' in detailsstr and 'http2' in detailsstr and '429' in detailsstr:
                    exc_msg = traceback.format_exc()
                    gmdatalogger.warn("你调用api[%s]的速度太快了, 服务器对你做限流了.返回空数据给你", func_name)
                    # 这个会记录到日志文件里
                    gmdatalogger.debug("你调用api[%s]的速度太快了. 返回空数据 返回的详细信息:%s\n 执行堆栈信息:%s", func_name, e, exc_msg)
                else:
                    gmdatalogger.exception(e)
