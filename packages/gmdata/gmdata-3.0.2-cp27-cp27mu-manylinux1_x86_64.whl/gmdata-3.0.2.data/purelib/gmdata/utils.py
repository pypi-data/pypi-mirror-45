...
gmdatalogger = logging.getLogger('gmdatalogger')
gmdatalogger.setLevel(logging.INFO)
...
gmdatalogger.addHandler(console_handler)
...
def gmsdklog2file():
    ...
    gmdatalogger.addHandler(file_handler)
    __add_logfile = True
...
def str2datetime(d):
    ...
    if len(d) == 19:  # 类于 2017-07-01 08:50:00
        return datetime.strptime(d, '%Y-%m-%d %H:%M:%S')
    gmdatalogger.error("字符串 %s 不能格式化成datetime类型, 请使用类似于 2017-07-01 08:50:00 这种格式", d)
    return None
