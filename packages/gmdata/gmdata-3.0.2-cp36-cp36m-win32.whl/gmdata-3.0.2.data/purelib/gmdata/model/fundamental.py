...
from gmdata import utils
from gmdata.constant import FUNDAMENTAL_ADDR
from gmdata.csdk.c_sdk import py_gmi_get_serv_addr
from gmdata.model.storage import ctx
from gmdata.pb.data_pb2 import Instrument, InstrumentInfo, ContinuousContract, Dividend
from gmdata.pb.fundamental_pb2_grpc import FundamentalServiceStub
from gmdata.pb.fundamental_pb2 import GetFundamentalsReq, GetInstrumentsReq, GetHistoryInstrumentsReq, \
...
from gmdata.utils import str_lowerstrip, load_to_list
...
class FundamentalApi(object):
    ...
    def get_fundamentals(self, table, symbols, start_date, end_date, fields=None, filter=None, order_by=None,
        ...
        resp = self.stub.GetFundamentals(req, metadata=[
            (str('authorization'), ctx.token),
            (str('sdk-lang'), ctx.sdk_lang),
            (str('sdk-version'), ctx.sdk_version),
            (str('sdk-arch'), ctx.sdk_arch),
            (str('sdk-os'), ctx.sdk_os),
        ])
