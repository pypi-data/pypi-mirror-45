...
from ...featurization import CatImputer
    ...
    def _convert_automl_catimputer(scope, operator, container):
        ...
        if type(input_var.type) == FloatTensorType:
            ...
            for i in range(1, len(mask_var_names_for_int)):
                ...
                attrs = {'name': scope.get_unique_operator_name(op_type)}
                container.add_node(op_type, [prev_mask_val, cur_mask_val], mask_variable_name, op_version=7, **attrs)
        ...
        if type(input_var.type) == Int64TensorType:
            ...
            apply_cast(scope, output_name, operator.outputs[0].full_name, container, to=onnx_proto.TensorProto.INT64)
