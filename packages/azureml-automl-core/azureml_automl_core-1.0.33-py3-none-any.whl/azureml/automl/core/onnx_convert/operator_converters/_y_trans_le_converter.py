...
class YTransformerLabelEncoderConverter(_AbstractOperatorConverter):
    ...
    def _convert_automl_y_trans_labelencoder(scope, operator, container):
        ...
        need_cast = False
        if isinstance(operator.outputs[0].type, StringTensorType):
            lbl_out = operator.output_full_names
        else:
        ...
        container.add_node(op_type, operator.input_full_names,
                           ...
                           **attrs)
        if need_cast:
            apply_cast(scope, lbl_out, operator.output_full_names, container, to=onnx_proto.TensorProto.FLOAT)
