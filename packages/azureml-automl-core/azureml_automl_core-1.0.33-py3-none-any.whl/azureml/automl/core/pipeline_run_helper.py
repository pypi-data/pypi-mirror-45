...
def run_pipeline(automl_settings: AutoMLBaseSettings,
    ...
    with logging_utilities.log_activity(logger, activity_name=constants.TelemetryConstants.RUN_PIPELINE_NAME,
        ...
        metrics_to_calculate = metrics.get_default_metrics(automl_settings.task_type)
            ...
            if results is not None and len(results) > 2:
                if training_type in \
                    ...
                    if result_full is None or len(result_full) <= 2:
                        raise RuntimeError("Failed while training full result. Status: {}".format(status))
        ...
        if automl_settings.task_type == constants.Tasks.REGRESSION and not automl_settings.is_timeseries:
            # add mean, and stddev to regression pipelines for error bars on predictions
            mean = score_valid[constants.Metric.Residuals]['data'].get(metrics.ResidualsMetric.MEAN)
            stddev = score_valid[constants.Metric.Residuals]['data'].get(metrics.ResidualsMetric.STDDEV)
            fitted_pipeline = RegressionPipeline(fitted_pipeline, mean, stddev)
        ...
        if automl_settings.is_timeseries:
            # Use ForecastMetric in future.
            augmented_pipeline = ForecastingPipelineWrapper(augmented_pipeline, mean=None, stddev=None)
