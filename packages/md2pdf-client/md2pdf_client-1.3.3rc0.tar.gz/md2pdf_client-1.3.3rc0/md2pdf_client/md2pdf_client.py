...
# If running locally, use 'git describe' from setup.py to get version tag
try:
    from setup import describe_write_version
    version = describe_write_version()
# If using packaged distribution, setup.py won't exist, use __version__.py
except ImportError:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='md2pdf client v' + version + ' - connect to an md2pdf server and create a PDF file')
    ...
    if is_windows:
        parser.add_argument('--create-shortcut', action="store_true", help="Create a shortcut on the Desktop")
    ...
    ## Create desktop shortcut, if command is given
    if is_windows and args.create_shortcut:
        from .shortcut_win32 import create_md2pdf_startmenu
        create_md2pdf_startmenu()
        logging.info("Created shortcut")
        return 0
