...
ci_data_model = 'parallel_x2' # Shows the data was creating using a parallel CTI model with one species.
...
def mask_function_for_bug(shape, ci_frame):
    ...
    mask_bug = msk.Mask.from_masked_regions(shape=shape, masked_regions=[(0, 330, 0, shape[1])])
    fpr_rows = ci_frame.ci_pattern.regions[0].total_rows
    mask_fpr = ci_mask.CIMask.masked_parallel_front_edge_from_ci_frame(
        shape=shape, ci_frame=ci_frame, rows=(0, fpr_rows))

    return mask_bug + mask_fpr
...
pipeline = parallel_x2.make_pipeline(phase_folders=[ci_data_type, ci_data_model, ci_data_resolution],
                                      mask_function=mask_function_for_bug)

pipeline.run(ci_datas=datas, cti_settings=cti_settings, pool=Pool(processes=2), assert_optimizer_pickle_matches=False)