...
def make_pipeline(phase_folders=None, tag_phases=True, mask_function=msk.Mask.empty_for_shape,
    ...
    pipeline_name = 'pipeline_hyper__parallel_x3'
    ...
    class ParallelHyperModelFixedPhase(ph.ParallelHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.parallel_species = results.from_phase('phase_2_parallel_x3').constant.parallel_species
            self.parallel_ccd = results.from_phase('phase_2_parallel_x3').constant.parallel_ccd

    phase1 = ParallelHyperModelFixedPhase(phase_name='phase_1_noise_scaling', phase_folders=phase_folders,
    ...
    class ParallelHyperFixedPhase(ph.ParallelHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.hyper_noise_scalars = results.from_phase('phase_1_noise_scaling').constant.hyper_noise_scalars
            self.parallel_species = results.from_phase('phase_2_parallel_x3').variable.parallel_species
            self.parallel_ccd = results.from_phase('phase_2_parallel_x3').variable.parallel_ccd
            self.parallel_ccd.well_fill_alpha = 1.0
            self.parallel_ccd.well_fill_gamma = 0.0

    phase2 = ParallelHyperFixedPhase(phase_name='phase_2_parallel_x3', phase_folders=phase_folders,
                                     tag_phases=tag_phases,
                                     parallel_species=[prior_model.PriorModel(arctic_params.Species),
                                                       prior_model.PriorModel(arctic_params.Species),
                                                       prior_model.PriorModel(arctic_params.Species)],
