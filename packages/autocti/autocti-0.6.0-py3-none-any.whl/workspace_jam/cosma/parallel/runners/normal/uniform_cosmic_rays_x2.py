...
ci_data_type = 'ci_images_uniform_cosmic_rays' # Charge injection data consisting of 2 images with uniform injections.
...
# This tools loads the correct shape of the image and frame geometry, for the ci_data_resoultion string.
shape, frame_geometry = \
    tools.parallel_shape_and_frame_geometry_from_ci_data_resolution(ci_data_resolution=ci_data_resolution)
...
for index, normalization in enumerate(normalizations):
    ...
    datas.append(ci_data.ci_data_from_fits(
        ...
        cosmic_ray_image_path=ci_data_path + 'cosmic_ray_image_' + str(int(normalization)) + '.fits',
        cosmic_ray_image_hdu=0, noise_map_from_single_value=4.0))
...
def mask_function_for_bug(shape, frame_geometry):
    return msk.Mask.from_masked_regions(shape=shape,masked_regions=[(0, 330, 0, shape[1])])


from workspace_jam.pipelines.initialize import parallel_x2

pipeline_init = parallel_x2.make_pipeline(phase_folders=[ci_data_type, ci_data_model, ci_data_resolution],
                                     mask_function=mask_function_for_bug, cosmic_ray_parallel_buffer=10,
                                     cosmic_ray_serial_buffer=10, cosmic_ray_diagonal_buffer=3)

from workspace_jam.pipelines.normal.from_initialize import parallel_x2

pipeline_normal = parallel_x2.make_pipeline(phase_folders=[ci_data_type, ci_data_model, ci_data_resolution],
                                     mask_function=mask_function_for_bug, cosmic_ray_parallel_buffer=10,
                                     cosmic_ray_serial_buffer=10, cosmic_ray_diagonal_buffer=3)

pipeline = pipeline_init + pipeline_normal

pipeline.run(ci_datas=datas, cti_settings=cti_settings, pool=Pool(processes=8))
