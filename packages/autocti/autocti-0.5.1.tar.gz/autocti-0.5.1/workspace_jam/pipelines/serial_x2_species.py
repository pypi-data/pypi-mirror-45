...
def make_pipeline(phase_folders=None, tag_phases=True, rows=None, mask_function=msk.Mask.empty_for_shape,
                  cosmic_ray_parallel_buffer=None, cosmic_ray_serial_buffer=None, cosmic_ray_diagonal_buffer=None):

    pipeline_name = 'pipeline_serial_x2_species'
    ...
    # (and therefore least affected by serial CTI).
    ...
    phase1 = SerialPhase(phase_name='phase_1_serial_x1_species_initialize', phase_folders=phase_folders,
    ...
    # In phase 2, we will fit the data with a two species serial CTI model and serial CCD filling model. In this
    # phase we will:
    ...
    class SerialPhase(ph.SerialPhase):
        ...
        def pass_priors(self, results):
            ...
            self.serial_species[0].trap_density = prior.UniformPrior(lower_limit=0.0,
                                                                       upper_limit=previous_total_density)
            self.serial_species[1].trap_density = prior.UniformPrior(lower_limit=0.0,
                                                                       upper_limit=previous_total_density)
            ...
            self.serial_ccd.well_notch_depth = \
                results.from_phase('phase_1_serial_x1_species_initialize').variable.serial_ccd.well_notch_depth
            self.serial_ccd.well_fill_beta = \
                results.from_phase('phase_1_serial_x1_species_initialize').variable.serial_ccd.well_fill_beta
            self.serial_ccd.well_fill_alpha = 1.0
            self.serial_ccd.well_fill_gamma = 0.0

    phase2 = SerialPhase(phase_name='phase_2_serial_x2_species_initialize', phase_folders=phase_folders,
                         tag_phases=tag_phases,
                         serial_species=[prior_model.PriorModel(arctic_params.Species),
    ...
    class SerialHyperModelFixedPhase(ph.SerialHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.serial_species = results.from_phase('phase_2_serial_x2_species_initialize').constant.serial_species
            self.serial_ccd = results.from_phase('phase_2_serial_x2_species_initialize').constant.serial_ccd
    ...
    class SerialHyperFixedPhase(ph.SerialHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.serial_species = results.from_phase('phase_2_serial_x2_species_initialize').variable.serial_species
            self.serial_ccd = results.from_phase('phase_2_serial_x2_species_initialize').variable.serial_ccd
            self.serial_ccd.well_fill_alpha = 1.0
            self.serial_ccd.well_fill_gamma = 0.0

    phase4 = SerialHyperFixedPhase(phase_name='phase_4_serial_x2_species', phase_folders=phase_folders,
                                   tag_phases=tag_phases,
                                   serial_species=[prior_model.PriorModel(arctic_params.Species),
                                                   prior_model.PriorModel(arctic_params.Species)],
                                   serial_ccd=arctic_params.CCD,
