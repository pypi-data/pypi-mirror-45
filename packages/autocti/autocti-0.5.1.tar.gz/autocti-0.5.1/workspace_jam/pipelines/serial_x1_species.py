...
def make_pipeline(phase_folders=None, tag_phases=True, rows=None, mask_function=msk.Mask.empty_for_shape,
    ...
    class SerialHyperModelFixedPhase(ph.SerialHyperPhase):
        ...
        def pass_priors(self, results):
            self.serial_species = results.from_phase('phase_1_serial_x1_species_initialize').constant.serial_species
            self.serial_ccd = results.from_phase('phase_1_serial_x1_species_initialize').constant.serial_ccd
    ...
    class SerialHyperFixedPhase(ph.SerialHyperPhase):
        ...
        def pass_priors(self, results):
            ...
            self.serial_species = results.from_phase('phase_1_serial_x1_species_initialize').variable.serial_species
            self.serial_ccd = results.from_phase('phase_1_serial_x1_species_initialize').variable.serial_ccd
            self.serial_ccd.well_fill_alpha = 1.0
            self.serial_ccd.well_fill_gamma = 0.0

    phase3 = SerialHyperFixedPhase(phase_name='phase_3_serial_x1_species', phase_folders=phase_folders,
                                   tag_phases=tag_phases,
                                   serial_species=[prior_model.PriorModel(arctic_params.Species)],
                                   serial_ccd=arctic_params.CCD,
                                   optimizer_class=nl.MultiNest, mask_function=mask_function,
