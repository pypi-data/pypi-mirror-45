...
class PyTexpr1:
    ...
    def __init__(self, expr: Union[PyLinexpr1, POINTER(Texpr1)]):
        ...
        elif isinstance(expr, Texpr1):
            self.texpr1 = byref(expr)
    ...
    def cst(cls, environment: PyEnvironment, cst: PyCoeff):
        texpr1 = libapron.ap_texpr1_cst(environment, cst)
        return cls(texpr1.contents)

    @classmethod
    def var(cls, environment: PyEnvironment, var: PyVar):
        texpr1 = libapron.ap_texpr1_var(environment, var)
        return cls(texpr1.contents)

    @classmethod
    def unop(cls, op: TexprOp, expr, typ: TexprRtype, dir: TexprRdir):
        texpr1 = libapron.ap_texpr1_unop(op, expr, typ, dir)
        return cls(texpr1.contents)

    @classmethod
    def binop(cls, op: TexprOp, expr_a, expr_b, typ: TexprRtype, dir: TexprRdir):
        texpr1 = libapron.ap_texpr1_binop(op, expr_a, expr_b, typ, dir)
        return cls(texpr1.contents)
