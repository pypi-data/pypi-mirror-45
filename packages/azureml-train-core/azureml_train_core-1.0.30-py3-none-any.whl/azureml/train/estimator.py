...
you choose (such as scikit-learn). You can submit your run on any compute target, whether it's your local machine,
a single VM in Azure, or a GPU cluster in Azure. For PyTorch, TensorFlow and Chainer tasks, Azure Machine Learning
also provides :class:`azureml.train.dnn.PyTorch`, :class:`azureml.train.dnn.TensorFlow` and
:class:`azureml.train.dnn.Chainer` estimators respectively to simplify using these frameworks.
...
class MMLBaseEstimator(ABC):
    ...
    @staticmethod
    def _init_run_config(compute_target, vm_size=None, vm_priority=None, entry_script=None, script_params=None,
                         node_count=1, process_count_per_node=1, distributed_backend=None,
        ...
        MMLBaseEstimator._validate_compute_target(compute_target)

        arguments = MMLBaseEstimator._get_arguments(script_params)
        data_inputs = MMLBaseEstimator._get_data_inputs(script_params)
        data_references = MMLBaseEstimator._get_data_references(inputs, data_inputs, source_directory_data_store)
        ...
        conda_dependencies = CondaDependencies(conda_dependencies_file_path)
        MMLBaseEstimator._update_conda_dependencies(conda_dependencies, conda_packages, pip_packages)
        ...
        estimator_config.auto_prepare_environment = True
        ...
        if environment_definition:
            MMLBaseEstimator._update_environment_definition(conda_packages, custom_docker_image,
                                                            ...
                                                            pip_packages, estimator_config, use_gpu)
        else:
            ...
            MMLBaseEstimator._update_docker_config(run_config=estimator_config,
    ...
    def _update_config_for_notebook_run(run_config, use_gpu, custom_docker_image):
        module_logger.warning("Note: A jupyter notebook(*.ipynb) file for entry_script is in preview mode.")
        ...
        if not MMLBaseEstimator._is_user_managed_environment(run_config.environment) and custom_docker_image is None:
            if use_gpu:
    ...
    def _get_script_run_config(self, activity_logger=None, telemetry_values=None):
        ...
        if MMLBaseEstimator._is_notebook_run(script):
    ...
    def _override_params(self, script_params=None, inputs=None, source_directory_data_store=None):
        ...
        if script_params:
            ...
            self._estimator_config.arguments = MMLBaseEstimator._get_arguments(merged_script_params)
            data_inputs = MMLBaseEstimator._get_data_inputs(script_params)
    ...
    def _get_telemetry_values(self, func):
        ...
        telemetry_values['useCustomDockerImage'] = not self.run_config.environment.docker.base_image. \
            startswith('mcr.microsoft.com/azureml/base')
...
class Estimator(MMLBaseEstimator):
    ...
    :type distributed_backend: str
    :param distributed_training: Distributed training parameters.
    ...
    :param pip_packages: List of strings representing pip packages to be added to the Python environment
        for the experiment.
    :type pip_packages: list
    :param conda_dependencies_file_path: A string representing the full path to the conda dependencies yaml file. This
        can be provided in combination with the conda_packages parameter.
    ...
    def __init__(self,
        ...
        :param pip_packages: List of strings representing pip packages to be added to the Python environment
            for the experiment.
        :type pip_packages: list
        :param conda_dependencies_file_path: A string representing the full path to the conda dependencies yaml file.
        ...
        if MMLBaseEstimator._is_notebook_run(estimator_config.script):
            MMLBaseEstimator._update_config_for_notebook_run(estimator_config,
                                                             use_gpu,
                                                             custom_docker_image)
...
class FrameworkBaseEstimator(MMLBaseEstimator):
    """A FrameworkBaseEstimator can be fit to data to produce a model using scenario files or framework images.

    This estimator supports single-node as well as multi-node execution. Execution of the estimator will result
    in a model being produced which should be placed in the ScriptParams.OUTPUT_PATH folder.

    FrameworkBaseEstimator is decoupled from a single default base image and provides multi-version support for
    for several common machine learning frameworks.

    This estimator supports two modes:

        Optimized Mode: In this mode, the estimator uses a pre-built framework image to run the AmlCompute jobs
        directly or uses it as an intermediate image build the final image. This mode is currently restricted to
        internal users.

        Legacy Mode: In this mode, images are built when the estimator is initialized. This is the default mode.
    ...
    :param framework_name: Name of the framework to be used for the estimator.
        ...
        Supported values:'Chainer', 'TensorFlow', and 'PyTorch'
    ...
    _UNSUPPORTED_FRAMEWORK_VERSION_ERROR = \
        ...
        'experiment.'
    _SCENARIO_FILE_NOT_FOUND_ERROR = 'Scenario file for {}:{} not found.'
    _SCENARIO_FILE_PARSE_ERROR = 'There was an issue parsing the scenario file {}.'
    _EMPTY_FRAMEWORK_VERSION_WARNING = 'framework_version is not specified, defaulting to version {}.'
    _OPTIMIZED_MODE_PREVIEW_NOTICE = 'Note: Optimized Mode has been enabled which will use pre-built ' \
                                     'framework images to run AMLCompute jobs or as intermediate images ' \
                                     'to make constructing an estimator more efficient. This feature is ' \
    ...
    def __init__(self,
        ...
        self._estimator_config = MMLBaseEstimator. \
            _init_run_config(compute_target=compute_target, vm_size=vm_size, vm_priority=vm_priority,
        ...
        if not MMLBaseEstimator._is_user_managed_environment(environment_definition):
            self._optimized_mode = _use_framework_image
            ...
            # If framework image is used with no extra packages, bypass the image building.
            if self._optimized_mode \
                    and self.conda_dependencies.serialize_to_string() == CondaDependencies().serialize_to_string():
        ...
        if MMLBaseEstimator._is_notebook_run(entry_script):
            MMLBaseEstimator._update_config_for_notebook_run(self._estimator_config, use_gpu,
                                                             custom_docker_image)
    ...
    def _load_from_scenario_file(self):
        ...
        try:
            ...
            raise TrainingException((self._SCENARIO_FILE_PARSE_ERROR).format(scenario_path))

        return base_image, dependencies

    def _setup_environment(self, custom_docker_image, compute_target, entry_script, environment_definition):
        if self._framework_version not in self.__class__.get_supported_versions():
            raise TrainingException((self._UNSUPPORTED_FRAMEWORK_VERSION_ERROR).
                                    format(name=self._framework_name, version=self._framework_version))

        if self._optimized_mode:
            if not (isinstance(compute_target, _BatchAITarget) or
                    isinstance(compute_target, AmlCompute) or
                    self._estimator_config.amlcompute.vm_size is not None):
                module_logger.warning("Prebuilt-framework images cannot be used for non-AMLCompute "
                                      "targets. Legacy mode will be used unless AmlCompute target "
                                      "is chosen.")
                self._optimized_mode = False
            if environment_definition is not None:
                module_logger.warning("Prebuilt-framework images cannot be used when environment_definition "
                                      "is provided. Legacy mode will be used unless environment_definition "
                                      "is set to None.")
                self._optimized_mode = False
            if custom_docker_image is not None:
                module_logger.warning("Prebuilt-framework images cannot be used when custom_docker_image "
                                      "is provided. Legacy mode will be used unless custom_docker_image "
                                      "is set to None.")
                self._optimized_mode = False
            # Notebook runs are supported only in openMPI base image.
            # Optimized mode uses IntelMPI. All notebook runs are forced to use legacy mode.
            # Notebook runs install papermill dependencies on top the base image.
            # In case of notebook run even if user didn't specify extra dependencies we still want image build step.
            if MMLBaseEstimator._is_notebook_run(entry_script):
                module_logger.warning("Prebuilt-framework images cannot be used when entry_script is a notebook "
                                      "file(.ipynb). Legacy mode will be used.")
                self._optimized_mode = False

        if self._optimized_mode:
            base_image = '{}:{}-{}'.format(self._framework_name.lower(),
                                           self._framework_version,
            ...
            base_image, dependencies = self._load_from_scenario_file()
            self.conda_dependencies._merge_dependencies(CondaDependencies(_underlying_structure=dependencies))

        if custom_docker_image is None and environment_definition is None:
            self._estimator_config.environment.docker.base_image = base_image
