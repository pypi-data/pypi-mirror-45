...
def _create_experiment_dto(hyperdrive_run_config, workspace, experiment_name,
    ...
    if hyperdrive_run_config.source_directory is not None:
        snapshot_client = SnapshotsClient(workspace.service_context)
        snapshot_id = snapshot_client.create_snapshot(hyperdrive_run_config.source_directory)
    ...
    generator_config = CreateExperimentDtoGeneratorConfig.from_dict(hyperdrive_run_config._generator_config)
    policy_config = CreateExperimentDtoPolicyConfig.from_dict(hyperdrive_run_config._policy_config)
    primary_metric_config = CreateExperimentDtoPrimaryMetricConfig.from_dict(hyperdrive_run_config.
...
def search(hyperdrive_run_config, workspace, experiment_name):
    ...
    """
    compute_target = None
    if hyperdrive_run_config.estimator is not None:
        compute_target = hyperdrive_run_config.estimator._compute_target
    if hyperdrive_run_config.run_config is not None:
        compute_target = ComputeTarget(workspace=workspace, name=hyperdrive_run_config.run_config.run_config.target)
    ...
    if compute_target is not None \
        ...
        raise UserErrorException(("Automated hyperparameter tuning is only supported for the compute target "
                                  ...
                                  " type was provided: {}.").format(type(compute_target)))
    ...
    with _TelemetryLogger.log_activity(logger,
        ...
        host_url = hyperdrive_run_config._get_host_url(workspace=workspace, run_name=experiment_name)
        ...
        try:
            with tempfile.TemporaryDirectory() as temporary_config:
                ...
                run_config = hyperdrive_run_config.estimator.run_config if hyperdrive_run_config.estimator else None
...
def _get_telemetry_values(config, workspace):
    ...
    if config.estimator is not None:
        compute_target = config.estimator._compute_target
        telemetry_values['computeTarget'] = compute_target if isinstance(compute_target, str) else \
            compute_target.type if compute_target else "amlcompute"
    elif config.run_config is not None:
        telemetry_values['computeTarget'] = config.run_config.run_config.target
