import unittest
from ..preprocess.parser import AtmanParser, UnderlyingModel, Language
...
class TestPreprocessMethods(unittest.TestCase):
    ...
    def test_benepar_doc_parsing(self):
        en_parser = AtmanParser(UnderlyingModel.BENEPAR, Language.EN)
        ...
        for text, sents in zip(texts, targets):
            doc = en_parser.parse(text)
            self._test_single_doc_parsing(doc, sents)

    def _test_single_doc_phrase_extraction(self, parser, text, target):
        doc = parser.parse(text)
        phrases = [x.text for x in doc.noun_adjective_phrases]
        for t in target:
            self.assertIn(t, phrases,
                          msg='model:{0} language:{1}'.format(parser.model,
                                                              parser.language))

    def test_corenlp_phrase_extraction(self):

        en_parser = AtmanParser(UnderlyingModel.CORE_NLP, Language.EN)
        ...
        for text, target in zip(en_texts, en_targets):
            self._test_single_doc_phrase_extraction(en_parser, text, target)

        zh_parser = AtmanParser(UnderlyingModel.CORE_NLP, Language.ZH)
        ...
        for text, target in zip(zh_texts, zh_targets):
            self._test_single_doc_phrase_extraction(zh_parser, text, target)
    ...
    def test_benepar_phrase_extraction(self):
        ...
        zh_parser = AtmanParser(UnderlyingModel.BENEPAR, Language.ZH)
        zh_texts = ['新藤黄酸诱导LP-1人多发性骨髓瘤细胞凋亡及抑制血管生成作用的实验研究。'
                    '\n\n'
                    '目的:研究新藤黄酸对于多发性骨髓瘤LP-1细胞凋亡的诱导作用及血管生成抑'
                    '制作用。方法:通过采用CCK-8细胞活力检测法研究了新藤黄酸对LP-1多发性'
