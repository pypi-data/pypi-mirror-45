...
from .stanford_nlp.client import LanguageCoreNLPClient
from benepar import Parser as BeneparParser
import jieba
import re
from .benepar.preprocess import *
...
class AtmanParser(object):
    ...
    def parse(self, text):
        ...
        if self.model == CORE_NLP:
            response = self.corenlp_client.annotate(text, annotators=['parse'],
                                                    language=self.language)
            return Document(text, corenlp_resp=response)
        elif self.model == BENEPAR and self.language == 'en':
            spacy_doc = self.benepar_model(text)
            return Document(text, spacy_doc=spacy_doc)
        elif self.model == BENEPAR and self.language == 'zh':
