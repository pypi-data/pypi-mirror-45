...
PredictionEventBundle = namedtuple('PredictionEventBundle', [
...
class Fiddler:
    ...
    def __init__(self, token, organization, project, model,
    ...
    def publish_event(self, event, json_encoder=None):
        """
        ...
        self._sender.send(self._api_endpoint(),
                          self.token, event, json_encoder)

    def publish_event_bundles(self,
                              prediction_event_bundles,
                              json_encoder=None):
        """
        Publishes a list of prediction bundles.
        An PredictionEventBundle represents a batch of inferences and their
        input features. All of these share schema, latency, and success status.
        A bundle can  can consist just one event as well.
        :param prediction_event_bundles: list of PredictionEventBundle objects.
        :param json_encoder: Optional encoder for JSON encoding.
        :raises: raises FiddlerException in case of a failure.
        """
        if not prediction_event_bundles:
            return
        if not isinstance(prediction_event_bundles, list) \
            or not isinstance(prediction_event_bundles[0],
                              PredictionEventBundle):
            raise TypeError('Prediction event bundles is not of type '
                            'list[fiddler.PredictionEventBundle]')

        payload = {
            'protocol_version': _protocol_version,
            'prediction_bundles': [
                b._asdict() for b in prediction_event_bundles
            ]
        }
        self._sender.send(self._api_endpoint(), self.token,
                          payload, json_encoder=json_encoder)
