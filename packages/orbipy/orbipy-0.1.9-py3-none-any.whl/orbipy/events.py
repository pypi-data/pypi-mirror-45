...
#                 value=0, 
    ...
    find_index = \
    compiler.compile_isolated(find_index, 
...
#        if self.jc >= self.selected_jc
        idx = eventSPL.find_index(sp[:,1], self.jc)
#        mask = (sp[:,1] <= self.jc)
        if idx < 0: #not np.any(mask):
            raise RuntimeError("Unable to find suitable orbit for Jacobi constant %f"%self.jc)
#        idx = np.argmax(mask)
#        print('selected orbit:', idx, '/', sp.shape[0])
