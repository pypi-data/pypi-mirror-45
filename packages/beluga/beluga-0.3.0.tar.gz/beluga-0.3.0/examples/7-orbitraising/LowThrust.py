...
plt.plot(np.cos(sol_c.y[:,1])*sol_c.y[:,0], np.sin(sol_c.y[:,1])*sol_c.y[:,0], color='r', linestyle='--', marker='.',
         label='Collocation')
plt.plot(np.cos(anom)*sol_c.y[0,0], np.sin(anom)*sol_c.y[0,0], color='k', linestyle='--')
plt.plot(np.cos(anom)*sol_c.y[-1,0], np.sin(anom)*sol_c.y[-1,0], color='k', linestyle='--')
plt.legend()
plt.axis('equal')
plt.grid('on')
