...
def init_client(
        ...
        queue_maxsize=int(os.environ.get('MONA_QUEUE_SIZE') or 20),
        queue_circular_str=os.environ.get('MONA_QUEUE_CIRCULAR', 'True')):
    """
    Inits the client. Must be called once before starting to create contexts and
    exporting data.
    ...
    if _GLOBAL_SENDER:
        LOGGER.warning(
            'Tried initing an already inited client. Doing nothing.')
        return

    global MONA_USER_ID
    MONA_USER_ID = user_id

    queue_circular = {'True': True, 'False': False}.get(queue_circular_str,
                                                        True)

    LOGGER.debug("Starting Mona client with host {} and port {}".format(
        mona_host, mona_port))
    sender.setup(
        'mona.client',
        host=mona_host,
        ...
        ssl_server_hostname="raw.monalabs.io")
...
def _export_special_message(field_name, message, export_timestamp=None):
    if message:
        ...
        if not _check_timestamp(export_timestamp):
            special_message[INVALID_EXPORT_TIMESTAMP_FIELD_NAME] \
                = export_timestamp
...
def init_sub_context(arc_class=None, sub_context_id=''):
    ...
    LOGGER.info('Mona context after initing sub context: %s',
                get_full_context_id())
...
def close_sub_context(sub_context_id=''):
    ...
    LOGGER.info('Mona context after closing sub context: %s',
                get_full_context_id())
...
def _check_timestamp(timestamp):
    if not isinstance(timestamp, numbers.Number):
        LOGGER.error('Tried to send non-number'
                     ' as timestamp to mona: %s' % str(timestamp))
        return False
    try:
        datetime.datetime.fromtimestamp(timestamp)
    except:
        LOGGER.error("Tried so send a number that is not"
...
def _close_client_if_necessary():
    """
    Close FluentD async sender (if not None)
    """
    if sender.get_global_sender() is not None:
        sender.close()


atexit.register(_close_client_if_necessary)
