...
from kidx_nlu import config, utils
from kidx_nlu.components import ComponentBuilder
from kidx_nlu.config import KidxNLUModelConfig
from kidx_nlu.emulators import NoEmulator
from kidx_nlu.test import run_evaluation
from kidx_nlu.model import InvalidProjectError
from kidx_nlu.project import (
    Project, STATUS_FAILED, STATUS_READY, STATUS_TRAINING, load_from_server)
from kidx_nlu.train import do_train_in_worker
...
class DataRouter(object):
    ...
    def _create_query_logger(response_log):
        ...
        if response_log:
            ...
            log_file_name = "kidx_nlu_log-{}-{}.log".format(timestamp,
                                                            os.getpid())
    ...
    def _create_project_store(self,
        ...
        default_project = KidxNLUModelConfig.DEFAULT_PROJECT_NAME
    ...
    def _list_projects_in_cloud(self) -> List[Text]:
        ...
        try:
            from kidx_nlu.persistor import get_persistor
    ...
    def _create_emulator(mode: Optional[Text]) -> NoEmulator:
        ...
        If no emulator is specified, we will use the Kidx NLU format."""

        if mode is None:
            return NoEmulator()
        elif mode.lower() == 'wit':
            from kidx_nlu.emulators.wit import WitEmulator
            return WitEmulator()
        elif mode.lower() == 'luis':
            from kidx_nlu.emulators.luis import LUISEmulator
            return LUISEmulator()
        elif mode.lower() == 'dialogflow':
            from kidx_nlu.emulators.dialogflow import DialogflowEmulator
            return DialogflowEmulator()
        else:
            raise ValueError("unknown mode : {0}".format(mode))

    @staticmethod
    def _tf_in_pipeline(model_config: KidxNLUModelConfig) -> bool:
    ...
    def parse(self, data: Dict[Text, Any]) -> Dict[Text, Any]:
        project = data.get("project", KidxNLUModelConfig.DEFAULT_PROJECT_NAME)
    ...
    def start_train_process(self,
                            data_file: Text,
                            project: Text,
                            train_config: KidxNLUModelConfig,
    ...
    def evaluate(self,
        ...
        project = project or KidxNLUModelConfig.DEFAULT_PROJECT_NAME
        model = model or None
