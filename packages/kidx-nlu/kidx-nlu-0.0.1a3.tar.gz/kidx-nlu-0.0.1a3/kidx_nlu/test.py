...
def create_argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description='evaluate a Kidx NLU pipeline with cross '
...
def evaluate_intents(intent_results,
    ...
    """Creates a confusion matrix and summary statistics for intent
    predictions. Log samples which could not be classified correctly
    and save them to file.
...
def cross_validate(data: TrainingData, n_folds: int,
                   nlu_config: Union[KidxNLUModelConfig, Text]
                   ) -> CVEvaluationResult:
