...
from kidx_nlu import config
from kidx_nlu.data_router import DataRouter
from kidx_nlu.model import Trainer
...
def app(component_builder):
    ...
    train_models(component_builder,
                 os.path.join(root_dir, "data/examples/kidx/demo-kidx.json"))

    router = DataRouter(os.path.join(root_dir, "test_projects"))
    kidx = KidxNLU(router, logfile=nlu_log_file, testing=True)
...
#     assert rjs.get("error").startswith(
#         response_test.expected_response["error"])
...
#         from kidx_nlu import training_data
#
