...
from kidx_nlu import registry, training_data
from kidx_nlu.model import Interpreter
...
def test_interpreter(pipeline_template, component_builder, tmpdir):
    test_data = "data/examples/kidx/demo-kidx.json"
...
@pytest.mark.parametrize("metadata",
                         [{"kidx_nlu_version": "0.0.0"},
                          {"kidx_nlu_version": "0.0.0a1"}])
def test_model_not_compatible(metadata):
    with pytest.raises(kidx_nlu.model.UnsupportedModelError):
        Interpreter.ensure_model_compatibility(metadata)


@pytest.mark.parametrize("metadata",
                         [{"kidx_nlu_version": "0.0.1"},
                          {"kidx_nlu_version": "0.0.1a1"},
                          {"kidx_nlu_version": "0.0.1a2"}])
