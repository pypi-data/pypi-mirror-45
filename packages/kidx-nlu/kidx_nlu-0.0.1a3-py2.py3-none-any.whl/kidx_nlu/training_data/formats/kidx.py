...
from kidx_nlu.training_data.formats.readerwriter import (
    JsonTrainingDataReader,
    TrainingDataWriter)
from kidx_nlu.training_data.util import transform_entity_synonyms
from kidx_nlu.utils import json_to_string
...
class KidxReader(JsonTrainingDataReader):
    def read_from_json(self, js, **kwargs):
        """Loads training data stored in the kidx NLU data format."""
        from kidx_nlu.training_data import Message, TrainingData

        validate_kidx_nlu_data(js)

        data = js['kidx_nlu_data']
        ...
        if intent_examples or entity_examples:
            logger.warning("DEPRECATION warning: your kidx data "
...
class KidxWriter(TrainingDataWriter):
...
def validate_kidx_nlu_data(data: Dict[Text, Any]) -> None:
    """Validate kidx training data format to ensure proper training.
