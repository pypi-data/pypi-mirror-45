...
from kidx_nlu.training_data.util import check_duplicate_synonym
from kidx_nlu.utils import lazyproperty, list_to_str, write_to_file
...
class TrainingData(object):
    ...
    def as_json(self, **kwargs: Any) -> Text:
        ...
        from kidx_nlu.training_data.formats import KidxWriter
        return KidxWriter().dumps(self)

    def as_markdown(self) -> Text:
        """Generates the markdown representation of the TrainingData."""
        from kidx_nlu.training_data.formats import MarkdownWriter
    ...
    def train_test_split(self,
        ...
        for intent, count in self.examples_per_intent.items():
            ex = [
                e for e in self.intent_examples if e.data["intent"] == intent
            ]
            random.shuffle(ex)
