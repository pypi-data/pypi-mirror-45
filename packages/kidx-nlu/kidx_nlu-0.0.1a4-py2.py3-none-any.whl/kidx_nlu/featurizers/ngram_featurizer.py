...
class NGramFeaturizer(Featurizer):
    ...
    def train(self, training_data: TrainingData, cfg: KidxNLUModelConfig,
              **kwargs: Any):
    ...
    def _ngrams_in_sentences(self, examples, ngrams):
        ...
        returns a feature vector for each sentence.

        The first $k$ elements are from the `intent_features`,
        the rest are {1,0} elements denoting
        whether an ngram is in sentence."""
    ...
    def _generate_all_ngrams(self, list_of_strings, ngram_min_length):
        ...
        return [
            item for sublist in list(features.values()) for item in sublist
        ]
