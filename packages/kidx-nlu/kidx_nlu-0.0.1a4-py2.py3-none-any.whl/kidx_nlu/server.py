...
import kidx_nlu.cli.server as cli
from kidx_nlu.config import KidxNLUModelConfig
from kidx_nlu.data_router import (
    DataRouter, InvalidProjectError, MaxTrainingError)
from kidx_nlu.model import MINIMUM_COMPATIBLE_VERSION
from kidx_nlu.train import TrainingException
from kidx_nlu.utils import json_to_string, read_endpoints
from kidx_nlu.version import __version__
...
class KidxNLU(object):
    """Class representing Kidx NLU http server"""
    ...
    def hello(self, request):
        """Main Kidx route to check if the server is online"""
        return "hello from Kidx NLU: " + __version__
    ...
    def version(self, request):
        """Returns the Kidx server's version"""
    ...
    def train(self, request):
        ...
        try:
            ...
            path_to_model = yield self.data_router.start_train_process(
                data_file, project,
                KidxNLUModelConfig(model_config), model_name)
    ...
    def unload_model(self, request):
        ...
        try:
            ...
            response = self.data_router.unload_model(
                params.get('project', KidxNLUModelConfig.DEFAULT_PROJECT_NAME),
                params.get('model')
            )
...
def get_token(_clitoken: str) -> str:
    _envtoken = os.environ.get("KIDX_NLU_TOKEN")

    if _clitoken and _envtoken:
        raise Exception(
            "KIDX_NLU_TOKEN is set both with the -t option,"
...
def main(args):
    ...
    kidx.app.run('0.0.0.0', args.port)
