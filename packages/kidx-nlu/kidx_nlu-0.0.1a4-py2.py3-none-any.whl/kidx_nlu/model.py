...
import kidx_nlu
from kidx_nlu import components, utils
from kidx_nlu.components import Component, ComponentBuilder
from kidx_nlu.config import (KidxNLUModelConfig,
                             component_config_from_pipeline,
                             make_path_absolute)
from kidx_nlu.persistor import Persistor
from kidx_nlu.training_data import TrainingData, Message
from kidx_nlu.utils import create_dir, write_json_to_file
from kidx_nlu.utils.bilstm_utils import result_to_json
from kidx_nlu.utils.bilstm_utils import iobes_iob, iob_iobes

logger = logging.getLogger(__name__)

MINIMUM_COMPATIBLE_VERSION = '0.0.1a1'
...
class Metadata(object):
    ...
    def persist(self, model_dir: Text):
        ...
        metadata.update({
            ...
            "kidx_nlu_version": kidx_nlu.__version__,
        })
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        if component_builder is None:
            # If no builder is passed,
            # every interpreter creation will result in
    ...
    @staticmethod
    def _build_pipeline(cfg: KidxNLUModelConfig,
...
class Interpreter(object):
    ...
    def ensure_model_compatibility(metadata: Metadata,
        ...
        model_version = metadata.get("kidx_nlu_version", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedModelError(
                "The model version is to old to be "
                "loaded by this Kidx NLU instance. "
                "Either retrain the model, or run with "
                "an older version. "
                "Model version: {} Instance version: {}"
                "".format(model_version, kidx_nlu.__version__))
