...
from kidx_nlu.utils.bilstm_utils import result_to_json
from kidx_nlu.utils.bilstm_utils import iobes_iob, iob_iobes
...
class Model(object):
    ...
    def __init__(self, config):
        ...
        # 此方法会返回一个用于初始化权重的初始化程序 “Xavier”，
        # 确保每一层初始化的权重不大不小，方差尽量相等
        ...
        with tf.variable_scope("optimizer"):
            ...
            capped_grads_vars = [[tf.clip_by_value(g, -self.config["clip"],
                                                   self.config["clip"]), v]
    ...
    def embedding_layer(self, char_inputs, seg_inputs, config, name=None):
        ...
        此处只嵌入了两个特征，不同场景下可以嵌入不同特征，如果嵌入拼音特征、符号特征,
        应该可以用来检测错别字吧 0.0
        """
        embedding = []
        with tf.variable_scope("char_embedding" if not name else name), tf.device('/cpu:0'):
            ...
            # self.char_lookup=[2677*100]的向量，
            # char_inputs字对应在字典的索引/编号/key=[1]
            embedding.append(tf.nn.embedding_lookup(
                self.char_lookup, char_inputs))
            # self.embedding1.append(tf.nn.embedding_lookup(self.char_lookup,
            # char_inputs))
            if config["seg_dim"]:
    ...
    def IDCNN_layer(self, model_inputs,
        ...
        # tf.expand_dims会向tensor中插入一个维度，
        # 插入位置就是参数代表的位置（维度从0开始）。
        ...
        with tf.variable_scope("idcnn" if not name else name):
            ...
            shape of filter = [filter_height, filter_width, in_channels,
             out_channels]
            """
            ...
            # 从tensor中删除所有大小是1的维度
            # Given a tensor input, this operation returns a tensor of
            # the same type with all dimensions of size 1 removed.
            # If you don’t want to remove all size 1 dimensions,
            # you can remove specific size 1
            # dimensions by specifying squeeze_dims.
            # 给定张量输入，此操作返回相同类型的张量，并删除所有尺寸为1的尺寸。
            # 如果不想删除所有尺寸1尺寸，可以通过指定squeeze_dims来删除特定位置的1尺寸。
            # shape(?, ?, ?, 400) ——> shape(?, ?, 400)
    ...
    def project_layer_bilstm(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("hidden"):
                ...
                b = tf.get_variable("b",
                                    shape=[self.lstm_dim], dtype=tf.float32,
    ...
    def loss_layer(self, project_logits, lengths, name=None):
        ...
        with tf.variable_scope("crf_loss" if not name else name):
            ...
            # tag_indices: 一个形状为[batch_size, max_seq_len] 的矩阵,
            # 其实就是真实标签.
    ...
    def evaluate(self, sess, data_manager, id_to_tag):
        ...
        for batch in data_manager.iter_batch():
            ...
            for i in range(len(strings)):
                ...
                # gold = iob_iobes([id_to_tag[int(x)] for x in
                # tags[i][:lengths[i]]])
                # pred = iob_iobes([id_to_tag[int(x)] for x in batch_
                # paths[i][:lengths[i]]])
