...
from kidx_nlu.data_router import DataRouter
from kidx_nlu.server import KidxNLU
...
def app(tmpdir_factory):
    """Use IResource interface of Klein to mock Kidx HTTP server.

    :param component_builder:
    :return:
    """

    _, nlu_log_file = tempfile.mkstemp(suffix="_kidx_nlu_logs.json")

    router = DataRouter(tmpdir_factory.mktemp("projects").strpath)
    kidx = KidxNLU(router,
                   logfile=nlu_log_file,
                   testing=True)
    return StubTreq(kidx.app.resource())


@pytest.fixture
def kidx_default_train_data():
    with io.open('data/examples/kidx/demo-kidx.json',
...
@utilities.slowtest
@pytest.inlineCallbacks
def test_post_train(app, kidx_default_train_data):
    response = app.post("http://dummy-uri/train", json=kidx_default_train_data)
    time.sleep(3)
    app.flush()
...
@utilities.slowtest
@pytest.inlineCallbacks
def test_post_train_success(app, kidx_default_train_data):
    import zipfile
    model_config = {"pipeline": "keyword", "data": kidx_default_train_data}
...
@utilities.slowtest
@pytest.inlineCallbacks
def test_post_train_internal_error(app, kidx_default_train_data):
...
@pytest.inlineCallbacks
def test_model_hot_reloading(app, kidx_default_train_data):
    ...
    response = yield app.get(query)
    assert response.code == 200, "Project should exist after it got trained"


@pytest.inlineCallbacks
def test_evaluate_invalid_project_error(app, kidx_default_train_data):
    response = app.post("http://dummy-uri/evaluate",
                        json=kidx_default_train_data,
...
@pytest.inlineCallbacks
def test_evaluate_internal_error(app, kidx_default_train_data):
    response = app.post("http://dummy-uri/evaluate",
                        json={"data": "dummy_data_for_triggering_an_error"})
    time.sleep(3)
    app.flush()
...
@pytest.inlineCallbacks
def test_evaluate(app, kidx_default_train_data):
    response = app.post("http://dummy-uri/evaluate",
                        json=kidx_default_train_data)
    time.sleep(3)
    app.flush()
