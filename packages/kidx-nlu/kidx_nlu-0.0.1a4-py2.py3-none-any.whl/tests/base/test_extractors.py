...
# from kidx_nlu.extractors.PyLTP_entity_extractor import PyLTPEntityExtractor
...
def test_crf_extractor(spacy_nlp, ner_crf_pos_feature_config):
    ...
    ext.train(TrainingData(training_examples=examples), KidxNLUModelConfig())
...
def test_spacy_ner_extractor(component_builder, spacy_nlp):
    _config = KidxNLUModelConfig({"pipeline":
                                 [{"name": "SpacyEntityExtractor"}]})
    ext = component_builder.create_component(_config.for_component(0), _config)
    example = Message("anywhere in the West", {
        "intent": "restaurant_search",
        "entities": [{'start': 16, 'extractor': 'SpacyEntityExtractor',
                      'end': 20, 'value': 'West', 'entity': 'LOC',
                      'confidence': None}],
        "spacy_doc": spacy_nlp("anywhere in the west")})
...
def test_PyLTP_extractor(pyltp_feature_config):

    from kidx_nlu.extractors.PyLTP_extractor import PyLTPEntityExtractor
    ext = PyLTPEntityExtractor(component_config=pyltp_feature_config)

    example = Message("我的名字叫芳芳", {
            "entities": [{
                'start': 5,
                'extractor': 'PyLTPEntityExtractor',
                'end': 7,
                'value': '芳芳',
                'entity': 'nh'}]
        })
