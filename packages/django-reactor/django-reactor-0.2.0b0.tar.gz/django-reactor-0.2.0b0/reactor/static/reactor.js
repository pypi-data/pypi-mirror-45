var Component,FOCUSABLE_INPUTS,ReactorChannel,component,i,len,merge_objects,origin,reactor_channel,send,indexOf=[].indexOf;origin=new Date();FOCUSABLE_INPUTS=['text','textarea','number','email','password','search','tel','url'];ReactorChannel=class ReactorChannel{constructor(url='/reactor',retry_interval=100){this.on=this.on.bind(this);this.url=url;this.retry_interval=retry_interval;this.online=false;this.callbacks={};}
...
this.websocket=new WebSocket(`${protocol}${window.location.host}${this.url}`);this.websocket.onopen=(e)=>{this.online=true;this.trigger('open');return this.retry_interval=1000;};this.websocket.onclose=(e)=>{this.online=false;this.trigger('close');return setTimeout((()=>{return this.open();}),this.retry_interval);};return this.websocket.onmessage=(e)=>{var data;data=JSON.parse(e.data);return this.trigger('message',data);};}else{return setTimeout((()=>{return this.open();}),this.retry_interval);}}
send(command,payload){var data;data={command:command,payload:payload};if(this.online){try{return this.websocket.send(JSON.stringify(data));}catch(error){return console.log('Failed sending');}}}};reactor_channel=new ReactorChannel();reactor_channel.open();reactor_channel.on('open',function(){var el,i,len,ref,results;console.log('ON-LINE');ref=document.querySelectorAll(reactor_components.join(','));results=[];for(i=0,len=ref.length;i<len;i++){el=ref[i];el.classList.remove('reactor-disconnected');results.push(el.connect());}
return results;});reactor_channel.on('close',function(){var el,i,len,ref,results;ref=document.querySelectorAll(reactor_components.join(','));results=[];for(i=0,len=ref.length;i<len;i++){el=ref[i];results.push(el.classList.add('reactor-disconnected'));}
return results;});reactor_channel.on('message',function({type,id,html_diff}){var el;console.log('<<<',type.toUpperCase(),id);el=document.getElementById(id);if(el!=null){if(type==='render'){return el.apply_diff(html_diff);}else if(type==='remove'){return window.requestAnimationFrame(function(){return el.remove();});}}});for(i=0,len=reactor_components.length;i<len;i++){component=reactor_components[i];Component=class Component extends HTMLElement{constructor(){super();this.tag_name=this.tagName.toLowerCase();this._last_received_html='';}
...
html=html.join('');if(this._last_received_html!==html){this._last_received_html=html;return window.requestAnimationFrame(()=>{var ref;morphdom(this,html,{onBeforeElUpdated:function(from_el,to_el){var ref;if((ref=from_el.type,indexOf.call(FOCUSABLE_INPUTS,ref)>=0)&&from_el===document.activeElement&&indexOf.call(to_el.getAttributeNames(),'reactor-override-value')<0){to_el.getAttributeNames().forEach(function(name){return from_el.setAttribute(name,to_el.getAttribute(name));});from_el.readOnly=to_el.readOnly;return false;}
return true;}});return(ref=this.querySelector('[reactor-focus]'))!=null?ref.focus():void 0;});}}
dispatch(name,args){var k,state,v;state=this.serialize();for(k in args){v=args[k];state[k]=v;}
console.log('>>> USER_EVENT',this.tag_name,name,state);origin=new Date();return reactor_channel.send('user_event',{name:name,state:state});}
