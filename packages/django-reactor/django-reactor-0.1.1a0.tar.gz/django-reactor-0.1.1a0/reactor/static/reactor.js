...
return results;});channel.on('message',function({type,id,html}){var el;console.log('<<<',type.toUpperCase(),id);el=document.getElementById(id);if(el!=null){console.log(new Date()-origin);if(type==='render'){return window.requestAnimationFrame(function(){var ref;morphdom(el,html);return(ref=el.querySelector('[focus]'))!=null?ref.focus():void 0;});}else if(type==='remove'){return window.requestAnimationFrame(function(){return el.remove();});}}});for(i=0,len=reactor_components.length;i<len;i++){component=reactor_components[i];Component=class Component extends HTMLElement{constructor(){super();this.tag_name=this.tagName.toLowerCase();}
