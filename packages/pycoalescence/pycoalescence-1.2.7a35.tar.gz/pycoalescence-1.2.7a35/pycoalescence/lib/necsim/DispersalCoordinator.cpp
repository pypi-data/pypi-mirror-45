...
#endif // DEBUG
                long end_x = this_step.oldx;
                long end_y = this_step.oldy;
                long end_x_wrap = this_step.oldxwrap;
                long end_y_wrap = this_step.oldywrap;
                landscape->findNearestHabitatCells(this_step.oldx, this_step.oldy, this_step.oldxwrap,
                        this_step.oldywrap, end_x, end_y, *generation);
                double end_x_d = end_x;
                double end_y_d = end_y;
                landscape->fixGridCoordinates(end_x_d, end_y_d, end_x_wrap, end_y_wrap);
                end_x = static_cast<long>(end_x_d);
                end_y = static_cast<long>(end_y_d);
                if(!landscape->checkMap(end_x, end_y, end_x_wrap, end_y_wrap, *generation))
                {
                    throw FatalException("Attempted nearest habitat cell is not habitat!");
                }
                fail = false;
                this_step.oldx = end_x;
                this_step.oldy = end_y;
