...
class semantic_map(object):
    def __init__(self, name=None, default=None, label_map=None, color_map=None):
        if label_map is None:
            raise MissingArgumentError('label_map')
        if type(label_map) is not dict or len(label_map.keys()) == 0:
            msg = 'label_map argument has invalid type'.format(default)
            raise InvalidArgumentError(msg)
        if default is not None and default not in label_map.values():
            msg = 'default label {} is not in label map'.format(default)
            raise InvalidArgumentError(msg)
        ...
        self.default = default or list(self.label_map.values())[0]
