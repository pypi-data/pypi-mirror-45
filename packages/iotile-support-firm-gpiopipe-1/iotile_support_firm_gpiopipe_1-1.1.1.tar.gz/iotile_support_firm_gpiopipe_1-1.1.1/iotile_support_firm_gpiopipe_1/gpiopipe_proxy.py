...
class GPIOPipelineProxy(TileBusProxyObject):
    ...
    def last_value(self, pipeline):
        ...
        value, = self.rpc(0x80, pipeline, result_format="L")
        return value
    ...
    def inspect_stage(self, pipeline, stage):
        ...
        value, = self.rpc(0x80, 0x10, pipeline, stage, arg_format="BB", result_format="L")
        return value

    @param("pipeline", "integer", "nonnegative", desc='The pipeline to query')
    @param("stage", "integer", "nonnegative", desc='The stage to inspect')
    def watch_stage(self, pipeline, stage):
        ...
        try:
            ...
            while True:
                ...
                value = self.inspect_stage(pipeline, stage)
...
class PipelineManager(TileBusProxyPlugin):
    ...
    def add_stage_lut(self, pipeline, stage, itbl, otbl,
        ...
        config = struct.pack("<8L", *itbl) + struct.pack("<8L", *otbl) + struct.pack("<BBBBBBBB",0,0,0, input_is_signed, output_is_signed, extrapolate, interpolate, is_valid)
    ...
    def add_stage_pulse_characterize(self, pipeline, stage, active_high=1):
        ...
        """
        self._begin_pipeline_stage(self.HANDLERS['pulse_characterize'], 1)
    ...
    def add_stage_pulse_qualify(self, pipeline, stage, on_cond, on_threshold, off_cond, off_threshold, operation, \
        ...
        self._begin_pipeline_stage(self.HANDLERS['pulse_qualify'], 3)
