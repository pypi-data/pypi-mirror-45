...
def load_state_file(file_path: Union[str, Path], device: str = 'cuda') -> StateFile:
    checkpoint = torch.load(str(file_path), map_location=device)
...
def from_state(state: StateFile, model: nn.Module = None, optimizers: List[Optimizer] = (),
    ...
    if model is None:
        model: nn.Module = torch_model_loader.factory(state.config).to(device)
...
def load_model_from_state(file_path: Union[str, Path], device: str = 'cuda') -> nn.Module:
    state = load_state_file(file_path=file_path, device=device)
    return from_state(state=state, device=device)
