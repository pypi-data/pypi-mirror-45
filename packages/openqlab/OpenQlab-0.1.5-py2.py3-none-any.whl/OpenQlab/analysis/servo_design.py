...
class Filter:
    ...
    def __init_subclass__(cls):
        assert 'calculate' in cls.__dict__
    ...
    def calculate(self):
        ...
        The second task is to update the `description`.

        Returns
        -------
        :code:`z, p, k`
        """
        raise NotImplementedError('This method should be implemented by the Filter implementation.')
...
# TODO: How to write a new Filter class
...
class Integrator(Filter):
    ...
    def __init__(self, cF, sF=None, enabled=True):
        self._description_long = 'The Integrator filter takes a corner frequency and an optional ~1/f slope frequency.'
        self._cF = cF
        self._second_parameter = sF
    ...
    def calculate(self):
        ...
        self._description = 'Int {0}'.format(human_readable(self.cF, 'Hz'))
...
class Differentiator(Filter):
    ...
    def __init__(self, cF, sF=None, enabled=True):
        self._description_long = 'The Differentiator filter takes a corner frequency and an optional ~1/f slope frequency.'
        self._cF = cF
        self._second_parameter = sF
        super().__init__(enabled)
    ...
    def calculate(self):
        ...
        self._description = 'Diff {0}'.format(human_readable(self.cF, 'Hz'))
...
class Lowpass(Filter):
    ...
    def __init__(self, cF, Q=0.707, enabled=True):
        self._cF = cF
        self._second_parameter = Q
        self._description_long = 'The Lowpass filter takes a corner frequency and an optional quality factor which defaults to 0.707.'
    ...
    def calculate(self):
        ...
        self._description = 'LP2 {0}, Q={1:.4g}'.format(human_readable(self.cF, 'Hz'), self.Q)
...
class Notch(Filter):
    ...
    def __init__(self, cF, Q=1, enabled=True):
        self._cF = cF
        self._second_parameter = Q
        self._description_long = 'The Notch filter takes a corner frequency and an optional quality factor.'
    ...
    def calculate(self):
        ...
        self._description = 'Notch {0}, Q={1:.4g}'.format(human_readable(cF, 'Hz'), Q)
...
class ServoDesign:
    ...
    def _add_filter_on_index(self, filter, index):
    ...
    def add(self, filter, index=None):
        """
    ...
    def plant(self, value):
        ...
        value = self._handle_keysight_files(value)

        self._plant = value

    def _handle_keysight_files(self, df):
        value = df.copy()
        # Extra handling for Keysight files
    ...
    def _apply(self, freq, ampl=None, phase=None):
        x, a, p = signal.bode(self.zpk(), freq)
