...
from nav_dataverk import DataverkContext
from nav_dataverk.connectors import KafkaConnector
from nav_dataverk.elastic_search_updater import ElasticSearchUpdater
from nav_dataverk.connectors.elasticsearch import ElasticsearchConnector
from nav_dataverk.package_publisher import PackagePublisher
from nav_dataverk.utils import db_utils
...
class Dataverk:
    ...
    def read_sql(self, source, sql, connector='Oracle') -> pd.DataFrame:
        ...
        """
        conn = db_utils.get_db_connector(settings_store=self.context.settings, connector=connector, source=source)
    ...
    def to_sql(self, df, table, sink=None, schema=None, connector='Oracle', if_exists: str = 'replace'):
        ...
        """
        conn = db_utils.get_db_connector(settings_store=self._context.settings, connector=connector, source=sink)
