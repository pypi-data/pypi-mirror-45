...
from probator.constants import DEFAULT_CONFIG_OPTIONS
...
from probator.utils import get_hash, diff
...
def _import_templates(force=False):
    ...
    Returns:
        `None`
    """
    tmplpath = os.path.join(resource_filename('probator', 'data'), 'templates')
...
def before_request():
    ...
    session['accounts'] = [acct.account_id for acct in accounts]
