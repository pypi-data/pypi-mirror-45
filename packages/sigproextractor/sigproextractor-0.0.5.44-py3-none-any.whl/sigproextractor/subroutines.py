...
def cluster_converge_outerloop(Wall, Hall, totalprocess):
    ...
    for i in range(10):  # using 10 iterations to get the best clustering 
...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    minProcessStabity= round(np.min(process_stabililities), 2) 
    ...
    print ('The reconstruction error is {} and the minimum process stability is {} for {} signatures\n\n'.format(reconstruction_error, round(minProcessStabity,2), i))
    fh.write('{}, {}, {}\n'.format(i, round(minProcessStabity, 2), round(reconstruction_error, 2)))
    fh.close()
...
def stabVsRError(csvfile, output, title, all_similarities_list, mtype= ""):
    ...
    median_l1 = []; maximum_l1 = []; median_l2 = []; maximum_l2 = []; median_kl = []; maximum_kl = []; wilcoxontest=[]; all_med_l2=[];
    ...
    pre_l2 = np.zeros([all_similarities_list[0].shape[0]])
    pre_mean = 100
    pre_med_l2 = 100
    ...
    for values in range(len(all_similarities_list)):
      ...
      cur_l2 = all_similarities["L2_Norm_%"]
      cur_mean = all_similarities["L2_Norm_%"].mean()
      wiltest = wilcoxon(np.array(cur_l2), np.array(pre_l2))[1]
      med_l2= all_similarities["L2_Norm_%"].median()
      if (wiltest<0.05) and (pre_mean-cur_mean>0):
          wilcoxontest.append(True)
          all_med_l2.append(med_l2)
          pre_med_l2 = med_l2
      else:
          wilcoxontest.append(False)
          if wiltest<0.05:
              all_med_l2.append(pre_med_l2)
              # Median L2 value doesn't changes if the there is no significance 
          else:
              all_med_l2.append(med_l2)
              pre_med_l2 = med_l2
      ...
      pre_l2 = all_similarities["L2_Norm_%"]
      
    
    all_med_l2 = np.array(all_med_l2)/100
    all_med_l2 = np.round(all_med_l2, 3)
...
# =============================================================================
    ...
    for i in range(len(x[:,0])):
        #print(x[i,0])
        if x[i,0]>=0.7:
            scaledStability[i] = change_scale(x[i,0], cul = 1, cll = 0.7, nul = 0.5, nll = 0.4)
        elif x[i,0]<0.7:
            scaledStability[i] = change_scale(x[i,0], cul = 0.7, cll = -1.0, nul = 0.05, nll = 0.001)
...
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    data = data.assign(**{'Median Sample L1%': median_l1, 'Maximum Sample L1%': maximum_l1, 'Median Sample L2%': median_l2, 'Maximum Sample L2%': maximum_l2, 'Significant Decrease of L2':wilcoxontest, 'Median Sample KL': median_kl, 'Maximum Sample KL': maximum_kl})  
    data=data.rename(columns = {'Stability':'Minimum Stability (min Silhouette)'})
