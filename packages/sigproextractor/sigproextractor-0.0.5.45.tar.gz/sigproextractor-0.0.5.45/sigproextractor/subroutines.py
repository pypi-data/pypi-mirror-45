...
def export_information(loopResults, mutation_context, output, index, colnames):
    ...
    meanProcessStabity= round(np.mean(process_stabililities), 2) 
...
# =============================================================================
    ...
    for i in range(len(x[:,0])):
        #print(x[i,0])
        if x[i,0]>=0.8:
            scaledStability[i] = change_scale(x[i,0], cul = 1, cll = 0.8, nul = 0.5, nll = 0.4)
        elif x[i,0]<0.8:
...
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    ax1.set_ylabel('Median L2 %', color=color)
    ax1.set_title(title)
    ...
    color = 'tab:blue'
    ax2.set_ylabel('Stability', color=color)  # we already handled the x-label with ax1
