...
from .utils import (
    ...
    replace_none, identity, get_first_elem,
)
...
class StructuredNestedDict(col.OrderedDict):
    ...
    def __init__(self, *args, **kwargs):
        """Extension of dict/OrderedDict that exposes advanced operations on nested
    ...
    def stratify(self, levels=None, stratified_names=None):
        ...
        """
        try:
            levels = replace_none(levels, len(get_first_elem(self.keys())))
        except IndexError:
            raise LevelError("Cannot infer stratify-levels")

        # stratify 0 = nothing, stratify 1 = 1
        new_dict = col.OrderedDict()
        for key_tup, val in self.items():
    ...
    def swap_levels(self, level_a, level_b):
        ...
        new_level_ls = list(range(required_levels))
