...
class PredictionModelHandler:
    ...
    def __generate_prediction_result(self, job_id, prediction_result):
        prediction_result = {
            self.JOB_ID: job_id,
            self.JOB_PREDICTION_RESULT: {
                self.RESULT: prediction_result
            },
            self.CLASSIFIER_ID: self.model_loader.get_model_identifier()
        }
        return prediction_result

    def collect_prediction_result_metrics(self, prediction_result):
        # Count number of predictions provided by the current model
        classifier_id_str = str(prediction_result[self.CLASSIFIER_ID])
        app_globals.metrics_registrar.counter(name='predictions_count',
                                              documentation='number of predictions provided by the current model',
                                              labels=['classifier_id']).labels(classifier_id=classifier_id_str).inc()

        prediction_result_str = str(prediction_result[self.JOB_PREDICTION_RESULT][self.RESULT])
        app_globals.metrics_registrar.counter(name='prediction_result_counter',
