...
def main():
    ...
    user_credentials_add_login = user_credentials_subparsers.add_parser(
        'add_login', help='add user_credential from login information')
    user_credentials_add_login.add_argument('--user', dest='user', action='store', required=True)
    user_credentials_add_login.add_argument(
        '--description', dest='description', action='store', required=False, default='CLI Credentials')
    ...
    _main = Main(raw=parsed_args.raw)
    ...
    if parsed_args.method == 'locks':
        ...
        if parsed_args.sub_method == 'add':
            _main.user_credentials_add(
                user_id=parsed_args.user,
                description=parsed_args.description
            )
        elif parsed_args.sub_method == 'add_login':
            _main.user_credentials_add_login(
                user_id=parsed_args.user,
                description=parsed_args.description
            )
        elif parsed_args.sub_method == 'delete':
            _main.user_credentials_delete(
                _id=parsed_args.id,
                user_id=parsed_args.user
            )
        elif parsed_args.sub_method == 'get':
            _main.user_credentials_get(
                _id=parsed_args.id,
                user_id=parsed_args.user
            )
        elif parsed_args.sub_method == 'update':
            _main.user_credentials_update(
                _id=parsed_args.id,
                user_id=parsed_args.user,
                description=parsed_args.description
            )
        elif parsed_args.sub_method == 'list':
            _main.user_credentials_list(
                user_id=parsed_args.user
            )
...
class Main(object):
    def __init__(self, raw):
        ...
        try:
            ...
            sys.exit(1)
        self.ca = self._config.get('main', 'ca', fallback=None)
    ...
    def users_add(self, _id, admin, email, name, method='post'):
    ...
    def user_credentials_add_login(self, user_id, description):
        password = getpass.getpass()
        login = requests.post(
        ...
        if 'errors' in login.json():
            print(json.dumps(login.json(), indent=4, sort_keys=True))
            sys.exit(1)
        secret = requests.post(
            url="{0}users/_self/credentials".format(self.endpoint),
            json={
                "data": {
                    "description": description,
                }
        ...
        print("secret_id is: {0}".format(secret.json()['data']['id']))
        print("secret is: {0}".format(secret.json()['data']['secret']))
