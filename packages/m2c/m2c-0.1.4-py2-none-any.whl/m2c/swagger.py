...
class SwaggerParameter(object):
    ...
    def todict(self):
        ...
        if self.in_ == 'body' and self.schema:
            ...
            if len(self.items) > 0:
                d['items'] = map(lambda x: x.todict(), self.items)
        if self.default:
...
def _getGoStruct(lines):
    ...
    for line in lines:
        ...
        typObj = ''
        if typ not in ['string', 'int64', 'bool']: # not in base type
...
def genSwaggerContent(url, title, description, version):
    ...
    for group in apiModel.groups:
        ...
        for api in group.apis:
            ...
            # api req model
            propItem = SwaggerPropertie(api.reqModelName(), 'object', '', api.reqModelName(), 'object', name)
            for pa in api.apiParams:
                if pa.required:
                    propItem.addRequiredParam(pa.name)

                typ = sg.goType2SwaggerType(pa.typ)
                ref = ''
                paramProp = SwaggerPropertie(pa.name, typ, ref, pa.name, typ, pa.comment)
                propItem.addPropertie(pa.name, paramProp)
            ...
            if paramIn == 'body':
                swpa = SwaggerParameter(paramIn, 'body', pa.comment, pa.required)
            ...
            for rp in api.apiResps():
                name, isArray, typ, typObj, comment = rp # TODO handler array
