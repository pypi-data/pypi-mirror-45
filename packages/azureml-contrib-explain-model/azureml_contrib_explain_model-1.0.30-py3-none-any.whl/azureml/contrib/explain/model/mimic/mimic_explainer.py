...
from ..scoring.scoring_model import TreeScoringModel, LinearScoringModel
...
class MimicExplainer(mimic_explainer.MimicExplainer):
    ...
    def create_scoring_model(self):
        """Create the scoring model for the mimic explainer.

        :return: The scoring model based on the model being explained.
        :rtype: ScoringModel
        """
        try:
            return TreeScoringModel(self.surrogate_model.model)
        except:
            return LinearScoringModel(self.surrogate_model.model,
