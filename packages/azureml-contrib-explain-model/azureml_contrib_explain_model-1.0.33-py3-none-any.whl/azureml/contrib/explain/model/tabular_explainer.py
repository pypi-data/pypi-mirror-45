...
class TabularExplainer(tabular_explainer.TabularExplainer):
    ...
    def create_scoring_model(self, evaluation_examples=None):
        """Create the scoring model for the tabular explainer.
        ...
        if isinstance(self.explainer, TreeExplainer):
            return self.explainer.create_scoring_model()
        else:
            if evaluation_examples is None:
                raise Exception('For this type of model, please provide evaluation examples to create_scoring_model.')
            return self.explainer.create_scoring_model(evaluation_examples)
