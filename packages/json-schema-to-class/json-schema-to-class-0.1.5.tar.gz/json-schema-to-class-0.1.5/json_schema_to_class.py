...
class Enum(Basic):
    ...
    def to_class_code(self, level: int = 0) -> str:
        result = [
            ...
            f'{spaces(1)}enum = {repr(self.enum)}',
            f'',
            f'{spaces(1)}def __init__(self, values=None):',
            f'{spaces(2)}self.value = values if values is not None else {repr(self.default)}',
            f'{spaces(2)}assert self.value in self.enum or self.value is None',
            '',
            f'{spaces(1)}def __eq__(self, other):',
            f'{spaces(2)}assert isinstance(other, type(self))',
            f'{spaces(2)}return self.value == other.value'
        ]
