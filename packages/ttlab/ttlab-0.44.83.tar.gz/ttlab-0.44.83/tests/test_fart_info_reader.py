import unittest
from ttlab.video.fart.info_reader import InfoReader


class FARTInfoReaderTestCases(unittest.TestCase):

    class FARTInfoExample:
        filename = 'tests/mock_data/fart_info.txt'
        info = {
            'x pixels': 480,
            'y pixels': 30,
            'exposure time': 0.000499,
            'number of accumulations': 1273,
            'vertical binning': 1,
            'horisontal binning': 5,
    ...
    def test_read_fart_info(self):
        result = InfoReader.read_info_from_file(self.FARTInfoExample.filename)
