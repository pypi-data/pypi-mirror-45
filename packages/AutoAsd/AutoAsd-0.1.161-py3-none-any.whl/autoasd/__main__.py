...
def main():
    ...
    while True:
        ...
        #check for unexpected files in data directory. This could happen if the user starts saving into a directory that isn't empty, or if the user hits spacebar on the spec compy (which saves a spectrum manually.
        file=check_for_unexpected(spec_controller.save_dir, spec_controller.hopefully_saved_files, data_files_to_ignore)

        while file !=None:
            data_files_to_ignore.append(spec_controller.save_dir+'\\'+file)
            ...
            for file in cmdfiles:
                if file not in cmdfiles0:
                    ...
                    if 'checkwriteable' in cmd: #Check whether you can write to a given directory
                        try:
                            print('?')
                            if os.path.exists(params[0]+'\\autospec_temp'):
                        ...
                        while file !=None:
                            ...
                            data_files_to_ignore.append(save_path+'\\'+file)
                            with open(write_command_loc+'\\unexpectedfile'+str(cmdnum)+'&'+file,'w+') as f:
                                    pass
                            cmdnum+=1
                            file=check_for_unexpected(save_path, spec_controller.hopefully_saved_files, data_files_to_ignore)
                        ...
                        #Makes sure that there was always a save configuration set before optimizing. Data files don't get saved during optimization, but this needs to happen anyway because we need to know where to put the log file when we record that we optimized.
                        if spec_controller.save_dir=='':
                            with open(write_command_loc+'\\noconfig'+str(cmdnum),'w+') as f:
                                pass
                            cmdnum+=1
                        ...
                        #If the specified output path is in the C drive, we can write straight to it. Otherwise, we're going to temporarily store the file in C:\\SpecShare\\temp
                            ...
                            try:
                                ...
                                while t-t0<5 and saved==False:
                                    ...
                                    time.sleep(0.2)
                                    t=time.process_time()+0.2 #process_time does not include time elapsed during sleep, so add it manually
                                print('done waiting')
                                if saved:
                                ...
                                try:
                                    ...
                                    print('Error: failed to reset ProcessController')
...
def check_for_unexpected(save_dir, hopefully_saved_files, data_files_to_ignore):
    ...
    for file in data_files:
        if save_dir+'\\'+file not in data_files_to_ignore: #If we aren't ignoring it
            if save_dir+'\\'+file not in hopefully_saved_files: #And we didn't save it
                return file #then it is unexpected
    return None
