...
class Cursor(common.DBAPICursor):
    ...
    def __init__(self, username, password, catalog='default', schema='default', session_props=None):
        super(Cursor, self).__init__()
        self.__host = 'host.doorda.com'
        self.__port = 443
        ...
        self._requests_kwargs = {'auth': HTTPBasicAuth(self.username, self.password), }
    ...
    def description(self):
        ...
        self._fetch_while(
            lambda: self._columns is None and self._state not in (self._STATE_NONE, self._STATE_FINISHED)
        )
    ...
    def execute(self, operation, parameters=None):
        ...
        url = urlparse.urlunparse((
            self.__protocol,
            '{}:{}'.format(self.__host, self.__port), '/v1/statement', None, None, None))
        _logger.info('%s', sql)
    ...
    def show_tables(self, catalog="", schema=""):
        ...
        if catalog and schema:
            ...
            self.execute(query.format(catalog=self.catalog, schema=self.schema))
            return self.fetchall()
        else:
    ...
    def is_connected(self):
        ...
        url = urlparse.urlunparse((
            ...
            '{}:{}'.format(self.__host, self.__port), '/v1/statement', None, None, None))
        response = self._requests_session.post(
            url, data="SELECT 1".encode('utf-8'), headers=headers, **self._requests_kwargs)
        if response.status_code != requests.codes.ok:
