...
def list_all_experiments(root=None, debug=False, not_debug=False, saved=False, not_saved=False):
...
def list_all_rundirs(saved_only=False, not_saved_only=False):
    ...
    datapaths = []
    if not_saved_only:
        datapaths.extend([os.path.join(root, RELATIVE_PATHS.run_data, name) for name in list_all_experiments()])
    if saved_only:
        datapaths.extend(os.path.join(root, RELATIVE_PATHS.saved_data))
    ...
    if not all:
        ...
        if saved:
            selected.difference_update([i for i, path in enumerate(rundirs)
                                        if os.path.join(common.find_xanity_root(), RELATIVE_PATHS.run_data) in path])

        if not_saved:
            selected.difference_update([i for i, path in enumerate(rundirs)
                                        if os.path.join(common.find_xanity_root(), RELATIVE_PATHS.saved_data) in path])
...
def get_rundirs(experiments=None,
    ...
    if include_not_saved:
        datapaths.extend([os.path.join(root, RELATIVE_PATHS.run_data, name) for name in experiments])
    ...
    for run in all_runs:
        ...
        if regex and not re.match(regex, run):
            continue
        if fnmatch_str and not fnmatch(dd,fnmatch_str):
            continue
...
def score_entry(args=None):
    ...
    if dirs:
        scan_data(rundirs=dirs, print_results=True, sort=sort, order=order, top=kn.top, headings=kn.headings, normalize=kn.normalize)
