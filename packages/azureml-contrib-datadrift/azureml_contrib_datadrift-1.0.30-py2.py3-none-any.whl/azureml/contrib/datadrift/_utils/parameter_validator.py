...
class ParameterValidator:
    ...
    def validate_frequency(input, none_ok=True):
        ...
        if not (input is frequencies[0] or input is None):
            module_logger.warning("Only frequency=Day is supported for now, updating..")
        return frequencies[0]  # TODO: frequencies[0] if input is None else input

    @staticmethod
    def validate_interval(input, none_ok=True):
        ...
        if not (input is 1 or input is None):
            module_logger.warning("Only interval=1 is supported for now, updating..")
    ...
    def validate_alert_configuration(input, none_ok=True):
        if not ((none_ok and input is None) or isinstance(input, dict)):
            raise TypeError("alert_config must be a dict")
        return input
