...
from azureml.contrib.datadrift._utils.constants import LOG_FLUSH_LATENCY, RUN_TYPE_ADHOC, RUN_TYPE_SCHEDULE
from azureml.contrib.datadrift._utils import pdutils

from azureml.contrib.datadrift.dataset_purpose import DatasetPurpose
from azureml.contrib.datadrift import _model_extensions
...
def main():
    ...
    try:
        ...
        is_localrun = False
    except AttributeError:  # in local test run returns error
    ...
    module_logger.info("At {} _generate_script.py called with the following argument list: {}".format(
    ...
    try:
        with _TelemetryLogger.log_activity(module_logger, activity_name="gen_script") as logger:
            ...
            logger.info("using Model extension _model_entensions having " + str(dir(_model_extensions)))
            ...
            if args.training_data:
                ...
                trainpd = pd.read_csv(args.training_data)
            else:
                ...
                try:
                    snap = model.get_training_dataset_snapshot()
                    trainpd = snap.to_pandas_dataframe()
                    logger.debug("Successfully read training data from dataset snapshot")
                # fall back to read training data from the dataset
                except Exception as e:
                    logger.error(e, exc_info=True)
                    logger.debug("Fall back to read training data from the dataset.")
                    dataset = model.datasets[DatasetPurpose.TRAINING]
                    trainpd = dataset.to_pandas_dataframe()
                    logger.debug("Successfully read training data from the dataset")
            ...
            if adhoc_run:
                scoringpd = pd.read_csv(args.scoring_data)
            else:
                scoring_data_withdate = os.path.join("{}{}".format(args.scoring_data, target_date.strftime('%Y')),
                                                     target_date.strftime('%m'), target_date.strftime('%d'),
                                                     "data.csv")
                scoringpd = pd.read_csv(scoring_data_withdate)
            ...
            if adhoc_run:
                ...
                metrics_output_path = datadrift.DataDrift._get_metrics_path(args.model_name, args.model_version,
                                                                            args.service, target_date)
            ...
            logger.debug("Metric output file absolute path is:")
            logger.debug("{}".format(os.path.abspath(metrics_output)))
