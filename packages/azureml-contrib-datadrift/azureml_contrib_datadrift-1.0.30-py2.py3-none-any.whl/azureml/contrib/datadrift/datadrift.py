...
from azureml.contrib.datadrift import _generate_script, _generate_script_datacuration_python
...
from azureml.contrib.datadrift._utils.constants import EMAIL_ADDRESSES, SERVICE, COLUMN_NAME
from azureml.contrib.datadrift._utils.constants import MODEL_NAME, MODEL_VERSION, \
    DRIFT_THRESHOLD, DATETIME, SERVICE_NAME, \
    METRIC_COLUMN_METRICS, METRIC_DATASET_METRICS, METRIC_TYPE, HAS_DRIFT, RUN_ID, SCORING_DATE, \
...
class DataDrift:
    """Enables users to identify any drifting between their training and/or scoring datasets for a model."""

    def __init__(self, workspace, model_name, model_version, services, compute_target_name=None,
                 frequency=None, interval=None, feature_list=None, schedule_start=None, alert_config=None):
        r"""Datadrift constructor.

        Enables users to identify any skew or drifting between their training, scoring and/or model serving datasets
        for a model.
        ...
        :type alert_config: dict()
        :return: A datadrift object
        :rtype: DataDrift
        """
        if workspace is ... or model_name is ... or model_version is ... or services is ...:
        ...
        log_context = {'workspace_name': workspace.name, 'model_name': model_name, 'model_version': model_version,
                       'services': ",".join(services), 'subscription_id': workspace.subscription_id}
        module_logger.addFilter(_TelemetryLoggerContextFilter(log_context))
        with _TelemetryLogger.log_activity(module_logger, activity_name="constructor") as logger:
            ...
            try:
                ...
                logger.info("DataDrift object for model_name: {}, model_version: {} doesn't exist. Creating..."
            ...
            dto = DataDriftDto(frequency=frequency,
                               ...
                               if alert_config and EMAIL_ADDRESSES in alert_config else None,
                               id=None,
            ...
            self._initialize(workspace, client_dto.model_name, client_dto.model_version, client_dto.services,
    ...
    def alert_config(self):
        ...
        :return: Dictionary of alert configuration.
        :rtype: dict()
        """
    ...
    def get(workspace, model_name, model_version):
        ...
        logger.addFilter(_TelemetryLoggerContextFilter(log_context))
        ...
        with _TelemetryLogger.log_activity(logger, activity_name="get") as logger:
            ...
            if len(dd_list) > 1:
                ...
                return dd_list[0]
            error_msg = "Could not find DataDrift object for: {} {} {}".format(workspace, model_name, model_version)
            logger.error(error_msg)
            raise KeyError(error_msg)
    ...
    def list(workspace, model_name=None, model_version=None, services=None):
        ...
        with _TelemetryLogger.log_activity(logger, activity_name="list") as logger:
            return DataDrift._get_datadrift_list(workspace, model_name, model_version, services, logger)

    @staticmethod
    def _get_datadrift_list(workspace, model_name, model_version, services=None, logger=None):
        ...
        dto_list = dd_client.list(model_name=model_name, model_version=model_version, services=services,
                                  logger=logger)
        dd_list = []
        for dto in dto_list:
            dd = DataDrift(..., ..., ..., ...)
            _alert = {
                EMAIL_ADDRESSES: dto.alert_configuration.email_addresses} if dto.alert_configuration else None
            dd._initialize(workspace, dto.model_name, dto.model_version, dto.services,
                           compute_target_name=dto.compute_target_name, frequency=dto.frequency, interval=dto.interval,
                           feature_list=dto.features,
                           schedule_start=dto.schedule_start_time if dto.schedule_start_time else None,
                           enabled=dto.enabled, schedule_id=dto.schedule_id, dd_id=dto.id, alert_config=_alert)
    ...
    def _get_or_create_aml_compute(workspace, name, should_create=False):
        ...
        try:
            ...
            module_logger.info("found existing compute target.")
        except ComputeTargetException as e:
            if should_create:
                module_logger.info("creating new compute target")
    ...
    def _generate_script():
        ...
        with open(os.path.join(output_dir, script_name), 'w+') as f:
    ...
    def run(self, target_date, create_compute_target=False):
        ...
        with _TelemetryLogger.log_activity(module_logger, activity_name="run",
    ...
    def enable_schedule(self, create_compute_target=False):
        ...
        with _TelemetryLogger.log_activity(module_logger, activity_name="enable_schedule") as logger:
            if not self.enabled:
    ...
    def disable_schedule(self):
        ...
        with _TelemetryLogger.log_activity(module_logger, activity_name="disable_schedule") as logger:
    ...
    def get_output(self, start_time, end_time, run_id=None):
    ...
    def update(self, services=..., compute_target_name=..., frequency=..., interval=..., feature_list=...,
               schedule_start=..., alert_config=..., create_compute_target=False):
        ...
        :type schedule_start: datetime.datetime
        :param alert_config: Alert configuration parameters
        :type alert_config: dict()
        :param create_compute_target: Whether the DataDrift API should automatically create an AML compute target
        ...
        """
        with _TelemetryLogger.log_activity(module_logger, activity_name="update") as logger:
            if services is not ...:
                services = ParameterValidator.validate_services(services)
                self._services = services
            ...
            if frequency is not ...:
                self._frequency = ParameterValidator.validate_frequency(frequency)
            if interval is not ...:
                self._interval = ParameterValidator.validate_interval(interval)
    ...
    def _update_remote(self, logger):
        ...
        dto = DataDriftDto(frequency=self._frequency,
                           ...
                           if self.alert_config and EMAIL_ADDRESSES in self.alert_config else None,
                           id=self._id,
    ...
    def _create_validate_pipeline(self, script_dir_drift, script_name_drift, script_dir_curation, script_name_curation,
        ...
        for service in self.services:
            ...
            module_logger.debug("Curation step {} created".format(step_curation_name))
            ...
            module_logger.debug("Drift step {} created".format(step_drift_name))

        pipeline1 = Pipeline(workspace=self.workspace, steps=steps)
        module_logger.debug("Pipeline is built")

        pipeline1.validate()
        module_logger.debug("Simple validation complete")
    ...
    @staticmethod
    def _get_valid_dates(input_start, input_end, available_dates):
        ...
        if input_start is not None and input_end is not None and input_start > input_end:
            module_logger.error("Invalid Time Range from {} to {}".format(input_start, input_end))
        ...
        if range_ed < dates[0] or range_st > dates[-1]:
            module_logger.error("No available data from {} to {}".format(range_st, range_ed))
        ...
        module_logger.debug("Valid Time Range is from {} to {}".format(range_st, range_ed))
    ...
    def _generate_plot_figure(environment, ordered_content, with_details):
        ...
        plt.sca(ax1)
        plt.plot(dates, drifts_train, '-g', linewidth=2.0)
        ...
        if with_details is True:
            ...
            plt.yscale('log')
            for c in columns_importance:
                plt.plot(dates, feature_importance[c].values(), c=colors(columns_importance.index(c)), linewidth=2.0)
            plt.xlabel("Date", fontsize=16)
            ...
            for c in columns_distance_e:
                ...
                plt.plot(dates, distance_energy[c].values(), c=colors(columns_importance.index(c)), linewidth=2.0)
            plt.xlabel("Date", fontsize=16)
            ...
            for c in columns_distance_w:
                # keep the column color aligned with feature importance↘
                plt.plot(dates, distnace_wasserstein[c].values(), c=colors(columns_importance.index(c)), linewidth=2.0)
            plt.xlabel("Date", fontsize=16)
            plt.ylabel("Distance", fontsize=16)
            plt.xticks(rotation=15)
    ...
    def show(self, start_time=None, end_time=None, with_details=False):
        ...
        range_st, range_ed = DataDrift._get_valid_dates(start_time, end_time, metrics_dates)
