...
def remote_plugins_info(names: _List[str] = None, v_range: _semver.VersionRange = None, use_cache: bool = True):
    """Get information about remote plugin
    """
    if use_cache:
        try:
            info = _plugman_cache.get_hash('remote_plugins')
        except _cache.error.KeyNotExist:
            info = _plugman_cache.put_hash('remote_plugins', _plugins_api_request('plugins'), _CACHE_TTL)
    else:
        info = _plugins_api_request('plugins')
    ...
    if v_range:
        ...
        for p_name, p_info in info.items():
            n_info[p_name] = {p_ver: p_info[p_ver] for p_ver in p_info.keys() if p_ver in v_range}
...
def remote_plugin_info(plugin_name: str, v_range: _semver.VersionRange = None) -> _Dict[str, dict]:
    """Get information about remote plugin
    """
    p_info = remote_plugins_info([plugin_name], v_range)
    if plugin_name not in p_info:
        raise _error.UnknownPlugin(plugin_name)
    if not p_info[plugin_name]:
        raise _error.UnknownPluginVersion(plugin_name, _semver.VersionRange(v_range))

    return p_info[plugin_name]


def dependencies(plugin_name: str, v_range: _semver.VersionRange = None, _deps: dict = None) -> dict:
    """Get plugin's dependencies tree
    """
    deps = _deps or {
        'pytsite': None,
        'packages': {},
    ...
    p_info = remote_plugin_info(plugin_name, v_range)
    p_version = _semver.last(p_info.keys())

    # Packages
    deps_part = deps['packages']
    for req_name, req_ver_str in p_info[p_version]['requires']['packages'].items():
        req_ver = _semver.VersionRange(req_ver_str)
        if req_name in deps_part:
            print('Duplicate: {} requires {} {}'.format(plugin_name, req_name, req_ver))
        else:
            print('{} requires {} {}'.format(plugin_name, req_name, req_ver))
            deps_part[req_name] = req_ver

    # Plugins
    deps_part = deps['plugins']
    for req_name, req_ver_str in p_info[p_version]['requires']['plugins'].items():
        req_ver = _semver.VersionRange(req_ver_str)
        if req_name == plugin_name:
            raise _error.CircularDependencyError(req_name, plugin_name)
        elif req_name in deps_part:
            if req_ver in deps_part[req_name]:
                deps_part[req_name] = req_ver
        else:
            deps_part[req_name] = req_ver
            dependencies(req_name, req_ver, deps)
...
def load(plugin_name: str, v_range: _semver.VersionRange = None, _required_by: str = None) -> object:
    ...
    """
    if not v_range:
        v_range = _semver.VersionRange()

    # Normalize plugin spec
    plugin_spec = '{} {}'.format(plugin_name, v_range)
    ...
    try:
        ...
        for req_p_name, req_p_ver in p_info['requires']['plugins'].items():
            ...
            if _DEBUG:
                _logger.debug("Plugin '{}-{}' requires '{}{}'".format(plugin_name, v_range,
...
def _install_dependencies(plugin_name: str) -> int:
    ...
    for p_name, p_version in plugin_info['requires']['plugins'].items():
        ...
        installed_plugins_count += install(p_name, _semver.VersionRange(p_version), False)

    return installed_plugins_count


def install(plugin_name: str, v_range: _semver.VersionRange = None, install_dependencies: bool = True) -> int:
    """Install a plugin
    ...
    # Get information about plugin and determine version to install
    r_plugin_info = remote_plugin_info(plugin_name, v_range)
    ver_to_install = _semver.last(r_plugin_info.keys())
