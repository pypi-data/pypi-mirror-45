...
"""
    ...
    def one_species_co_albedo(wavelength, SSA, impurities_content, impurities_type):
        ...
        if impurities_content <= 0:
            ...
            # Eq (9)
            return 12*pi/(wavelength*SSA)*impurities_content/density_impurities*abs_impurities

        elif hasattr(impurities_type, "MAE"):  # added by Ghislain
            mae_impurities = impurities_type.MAE(
                wavelength)  # in m^-1 / (kg m^-3)
...
"""
    #print(refrac_index, type(refrac_index))
...
"""
    ...
    if compute_irradiance_profiles or compute_actinic_profile:
        ...
        nearest_layer = searchsorted(thickness_total, z, side="right")
    ...
    for i in range(0, N):
        ...
        if compute_absorption:
            ...
            energyprofile[i, -1] = dirrad_i * soil_absorption(solutions_dir, kestar_i, dtaustar_i, taustar_i, Gmdir_i, mudir, soilalbedo_i) + \
                diffrad_i * soil_absorption(solutions_diff, kestar_i,
                                          dtaustar_i, taustar_i, Gmdiff_i, mudiff, soilalbedo_i)
