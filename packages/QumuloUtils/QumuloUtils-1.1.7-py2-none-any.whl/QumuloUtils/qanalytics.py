...
#        ForkedPdb().set_trace()
        ...
        # setup to ONLY debug with the logger.
...
#        ForkedPdb().set_trace()
        ...
        while True:
            try:
                pathname = self.dir_q.get (True, 0.05)
                self.count_in_q.decrement ()

                self.list_dir (self.qcluster, self._callback, pathname)
        ...
        while next_page != "":
            ...
            for ent in r["files"]:
                ...
                if ent["type"] == "FS_FILE_TYPE_DIRECTORY" and int(ent["child_count"]) > 0:
                    ...
                    self.dir_q.put ({"path": d["path"] + ent["name"] + "/", "max_depth": d["max_depth"]})
...
#                    if callback is not None:
#                        _id = ent["id"]
#                        cur_dir = rc.fs.read_dir_aggregates (id_ = _id, max_depth = 0)
#                        callback.create_dir_doc (ent, cur_dir)
...
class QAnalytics (QCluster):
    ...
    def __init__ (self, qhost, qlogin = "admin", qpasswd = "admin", qport = 8000, 
        ...
        if self.start_path[-1] != "/":
            self.start_path = self.start_path + "/"

        if logger is not None:
            self.logger.debug ("File analysis on Qumulo cluster %s starting at path %s" % 
                              (qhost, self.start_path))

        # Create Queues and Thread Pool
    ...
    def FullScan (self):
        ...
        root = self.connection.fs.read_dir_aggregates (path = self.start_path, max_depth = 0)
        ...
        self.dir_q.put ({"path": self.start_path, "max_depth": 5})
        self.count_in_q.increment ()
