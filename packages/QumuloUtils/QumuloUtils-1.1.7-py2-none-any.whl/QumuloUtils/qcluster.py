...
class QCluster (object):
    ...
    def ClusterIPs (self):
        ...
        try:
            netinfo = self.connection.network.list_interfaces ()
        except Exception, excpt:
            if self.logger is not None:
                self.logger.error ('Unable to get network status from cluster. Error is %s',
                                   excpt)
        ...
        for info in netinfo[0]:
            netstatus = info['network_statuses']

            if len (netstatus['floating_addresses']) > 0:
                for floats in netstatus['floating_addresses']:
                    ips.append (floats)
            else:
                ips.append (netstatus['address'])
