...
class EventType(Serializable):
    ...
    def deserialize(cls, raw: JSON) -> Any:
        return cls.by_event_type.get(raw, EventType(raw))
...
def _init_types():
    ...
    for t_class, types in standard_types.items():
        for t, name in types.items():
            event_type = EventType(t, t_class)
            EventType.by_event_type[t] = event_type
            setattr(EventType, name, event_type)
