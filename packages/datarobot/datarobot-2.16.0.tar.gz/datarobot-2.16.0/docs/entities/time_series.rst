...
windows_basis_unit
    ...
    rows, rather than time units. If omitted, value is detected time unit,
    one of the ``datarobot.enums.TIME_UNITS``.
feature_settings
...
:py:class:`datarobot.FeatureSettings` constructor receives `feature_name` and settings. For now only
...
You can upload a :py:class:`calendar file <datarobot.CalendarFile>` containing a list of events relevant to your dataset. When provided, DataRobot automatically derives time series features based on the calendar events. DataRobot creates special features from the calendar events (e.g., time until the next event, labeling the most recent event).
...
on models that have not had all backtests completed. Additionally, prediction intervals are not available when the number
of points per forecast distance is less than 10, due to insufficient data.
...
:py:meth:`datarobot.models.DatetimeModel.request_predictions` method and specifying ``include_prediction_intervals = True``.
The size for the prediction interval can be specified with the ``prediction_intervals_size`` parameter for the same function,
...
the retrieved prediction data for that request (see the :py:class:`Predictions <datarobot.models.Predictions>` class for retrieval methods).
