...
from ..enums import MONOTONICITY_FEATURELIST_DEFAULT, DEFAULT_MAX_WAIT, CHART_DATA_SOURCE
...
class Model(APIObject):
    ...
    def request_predictions(self, dataset_id, include_prediction_intervals=None,
        ...
        prediction_intervals_size : int, optional
            ...
            80 if `include_prediction_intervals` is True.
    ...
    def get_feature_impact(self):
        ...
        contribute much in additional, the 'redundantWith' value is the name of feature that has the
        ...
        impact_trafaret = t.Dict({
            t.Key('ranRedundancyDetection'): t.Bool,
            t.Key('featureImpacts'): t.List(t.Dict({
        ...
        data = impact_trafaret.check(data)
        if not data['ranRedundancyDetection']:
    ...
    def get_or_request_feature_impact(self, max_wait=DEFAULT_MAX_WAIT):
        """
        ...
        try:
            ...
            qid = e.json['jobId']
            from .job import Job
            feature_impact_job = Job.get(self.project_id, qid)
        ...
        data_subset : str
            ...
            Choices are:
                ...
                - `dr.enums.DATA_SUBSET.ALL_BACKTESTS` for downloading the predictions for all
                   ...
                   backtests. Datetime partitioned projects only.
...
class DatetimeModel(Model):
    ...
    effective_feature_derivation_window_start : int or None
        ...
        history for at prediction time. This can differ from the `feature_derivation_window_start`
        set on the project due to the differencing method and period selected, or if the model is a
        time series native model such as ARIMA. Will be a negative integer in time series projects
        and `None` otherwise.
    effective_feature_derivation_window_end : int or None
        (New in v2.16) For :ref:`time series <time_series>` projects only.
        How many timeUnits into the past relative to the forecast point the feature derivation
        window should end. Will be a non-positive integer in time series projects and `None`
        otherwise.
    forecast_window_start : int or None
        (New in v2.16) For :ref:`time series <time_series>` projects only.
        How many timeUnits into the future relative to the forecast point the forecast window should
        start. Note that this field will be the same as what is shown in the project settings. Will
        be a non-negative integer in time series projects and `None` otherwise.
    forecast_window_end : int or None
        (New in v2.16) For :ref:`time series <time_series>` projects only.
        How many timeUnits into the future relative to the forecast point the forecast window should
        end. Note that this field will be the same as what is shown in the project settings. Will be
        a non-negative integer in time series projects and `None` otherwise.
    windows_basis_unit : str or None
        (New in v2.16) For :ref:`time series <time_series>` projects only.
        Indicates which unit is the basis for the feature derivation window and the  forecast
        window. Note that this field will be the same as what is shown in the project settings. In
        time series projects, will be either the detected time unit or "ROW", and `None` otherwise.
    """
    ...
    def get_series_accuracy_as_dataframe(self):
        ...
        return pd.DataFrame(unpaginate(url, {}, self._client))

    def download_series_accuracy_as_csv(self, filename, encoding='utf-8'):
