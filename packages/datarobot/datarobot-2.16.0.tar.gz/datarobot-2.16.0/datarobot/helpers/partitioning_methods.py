...
from datarobot.enums import (CV_METHOD, DEFAULT_MAX_WAIT, PERIODICITY_MAX_TIME_STEP,
                             SERIES_AGGREGATION_TYPE,
                             TIME_UNITS, TREAT_AS_EXPONENTIAL, DIFFERENCING_METHOD)
...
class FeatureSettings(object):
    ...
    def __eq__(self, other):
        return (self.feature_name == other.feature_name and
                self.known_in_advance == other.known_in_advance and
                self.do_not_derive == other.do_not_derive)

    def __repr__(self):
        return ('FeatureSettings(feature_name=\'{feature_name}\', '
                'known_in_advance={kia}, do_not_derive={dnd})').format(
...
class DatetimePartitioningSpecification(PartitioningMethod):
    ...
    holdout_start_date : datetime.datetime or None
        ...
        holdout_duration must also be specified.  If disable_holdout is set to True,
        neither holdout_duration nor holdout_start_date must be specified.
    ...
    disable_holdout : bool or None
        ...
        If set to True, holdout_start_date and holdout_duration must not be specified.
    ...
    use_time_series : bool
        (New in version v2.8) Whether to create a time series project (if True) or an OTV project
        which uses datetime partitioning (if False).  The default behaviour is to create an OTV
        project.
    default_to_known_in_advance : bool
        (New in version v2.11) Optional, for time series projects only. Sets whether all features
        default to being treated as known in advance. Known in advance features are expected to be
        known for dates in the future when making predictions, e.g., "is this a holiday?". The
        default is false, all features are not known in advance. Individual features can be set to
        a value different than the default using the featureSettings parameter.
    feature_derivation_window_start : int or None
        ...
        Expressed in terms of the time_unit of the datetime_partition_column and should be negative
        or zero.
    feature_derivation_window_end : int or None
        ...
        in terms of the time_unit of the datetime_partition_column, and should be a positive value.
    feature_settings : list of :py:class:`FeatureSettings <datarobot.FeatureSettings>` objects
    ...
    forecast_window_start : int or None
        ...
        in terms of the time_unit of the datetime_partition_column.
    ...
    periodicities : list of Periodicity, optional
        ...
        should be 'ROW', if windows_basis_unit is 'ROW'
    ...
    def __init__(self, datetime_partition_column, autopilot_data_selection_method=None,
                 ...
                 holdout_start_date=None, holdout_duration=None, disable_holdout=None,
                 gap_duration=None, number_of_backtests=None, backtests=None,
                 use_time_series=False,
                 default_to_known_in_advance=False, default_to_do_not_derive=False,
                 feature_derivation_window_start=None, feature_derivation_window_end=None,
                 ...
                 multiseries_id_columns=None, target=None, use_cross_series_features=None,
                 aggregation_type=None, cross_series_group_by_columns=None, calendar_id=None):
    ...
    def collect_payload(self):
        ...
        ] if self.feature_settings else None
        periodicities = [
            p.collect_payload() for p in self.periodicities
        ] if self.periodicities else None
        payload = {
    ...
    def prep_payload(self, project_id, max_wait=DEFAULT_MAX_WAIT):
        ...
        props = datetime_part.get_multiseries_properties(self.multiseries_id_columns,
                                                         max_wait=max_wait)
        if not props['time_series_eligible']:
            msg = ('The selected datetime partition and multiseries id columns are not eligible for'
        ...
        if self.use_cross_series_features and self.cross_series_group_by_columns:
            ...
            res = multiseries_id.get_cross_series_properties(self.datetime_partition_column,
                                                             self.cross_series_group_by_columns,
                                                             max_wait=max_wait)
            if not res['isEligible']:
                msg = ('The selected cross-series group-by column is not eligible for'
...
class DatetimePartitioning(object):
    ...
    default_to_known_in_advance : bool
        ...
        default to being treated as known in advance.  Known in advance features are expected to be
        known for dates in the future when making predictions, e.g., "is this a holiday?". The
        default is false, all features are not known in advance. Individual features can be set to a
    ...
    feature_derivation_window_start : int or None
        ...
        Expressed in terms of the time_unit of the datetime_partition_column.
    feature_derivation_window_end : int or None
        (New in version v2.8) Only used for time series projects. Offset into the past to define how
        far back relative to the forecast point the feature derivation window should end. Expressed
        in terms of the time_unit of the datetime_partition_column.
    ...
    forecast_window_end : int or None
        ...
        terms of the time_unit of the datetime_partition_column.
    windows_basis_unit : string, optional
        (New in version v2.14) Only used for time series projects. Indicates which unit is
        a basis for feature derivation window and forecast window. Valid options are detected time
        unit (one of the ``datarobot.enums.TIME_UNITS``) or "ROW".
    ...
    treat_as_exponential : string, optional
        (New in version v2.9) defaults to "auto". Used to specify whether to treat data
        as exponential trend and apply transformations like log-transform. Use values from
        ``datarobot.enums.TREAT_AS_EXPONENTIAL`` enum.
    differencing_method : string, optional
        (New in version v2.9) defaults to "auto". Used to specify which differencing method to
        apply of case if data is stationary. Use values from
    ...
    periodicities : list of Periodicity, optional
        ...
        :py:class:`datarobot.Periodicity` Periodicities units should be 'ROW',
        if windows_basis_unit is 'ROW'
    multiseries_id_columns : list of str or null
        (New in version v2.11) a list of the names of multiseries id columns to define series
        within the training data.  Currently only one multiseries id column is supported.
    ...
    _converter = t.Dict({
        ...
        t.Key('windows_basis_unit', optional=True): t.Enum(TIME_UNITS.MILLISECOND,
                                                           TIME_UNITS.SECOND,
                                                           TIME_UNITS.MINUTE,
                                                           TIME_UNITS.HOUR,
                                                           TIME_UNITS.DAY,
        ...
        t.Key('treat_as_exponential', optional=True): t.Enum(
            TREAT_AS_EXPONENTIAL.ALWAYS,
            TREAT_AS_EXPONENTIAL.NEVER,
        ...
        t.Key('aggregation_type', optional=True): t.Enum(
            SERIES_AGGREGATION_TYPE.AVERAGE,
            SERIES_AGGREGATION_TYPE.TOTAL
        ),
    ...
    def __init__(self, project_id=None, datetime_partition_column=None, date_format=None,
                 ...
                 treat_as_exponential=None,
                 differencing_method=None, periodicities=None,
                 multiseries_id_columns=None, number_of_known_in_advance_features=0,
    ...
    def from_server_data(cls, data):
        ...
        if 'feature_settings' in converted_data:
            converted_data['feature_settings'] = [FeatureSettings.from_server_data(**fs) for fs
                                                  in converted_data['feature_settings']] or None
        if 'periodicities' in converted_data:
            converted_data['periodicities'] = [Periodicity(**p) for p
                                               in converted_data['periodicities']]
        return cls(**converted_data)

    @classmethod
    def generate(cls, project_id, spec, max_wait=DEFAULT_MAX_WAIT):
    ...
    def to_dataframe(self):
        ...
        default_to_known_in_advance and defining the feature derivation and forecast windows.
        """
