...
from .rest import RESTClientObject, DataRobotClientConfig, Retry
...
def _is_compatible_client(client):
    ...
    try:
        ...
        w_msg_tmpl = ('Error retrieving a version string from the server. '
                      'Server did not reply with an API version. This may indicate the endpoint '
                      'parameter `{}` is incorrect, or that the server there is incompatible '
                      'with this version of the DataRobot client package. Note that if you access '
                      'the DataRobot webapp at `https://app.datarobot.com`, then the correct '
                      'endpoint to specify would be `https://app.datarobot.com/api/v2`.')
...
def _is_compatible_version(version_response):
    ...
    if server_version_string is None:
        warn_msg = ('Server did not respond with a version string, you may have incompatibilities. '
    ...
    if server_version.version[1] < client_version.version[1]:
        warn_msg = ('Client version is ahead of server version, you may have incompatibilities. '
                    'Server version: {} - Client version: {}').format(server_version.vstring,
                                                                      client_version.vstring)
        warnings.warn(warn_msg)
    return True
