...
@responses.activate
@pytest.mark.usefixtures('client')
def test_use_cross_series_features(time_series_partition_spec, holdout_start_date,
                                   project_with_target_json, project_without_target,
                                   unittest_endpoint, project_id):
    time_series_partition_spec.multiseries_id_columns = ['series_id']
...
def test_use_cross_series_group_by_with_columns_validated(
        time_series_partition_spec, holdout_start_date, project_with_target_json,
        project_without_target, unittest_endpoint, project_id):
    ...
    responses.add(responses.GET,
                  ...
                  json={
                      ...
                      'lowInformation': False, 'uniqueCount': 500
                  })
    ...
    responses.add(responses.GET, ('{}multiseriesIds/{}/crossSeriesProperties/?'
                                  'crossSeriesGroupByColumns={}').format(
        project_url, time_series_partition_spec.multiseries_id_columns[0],
...
def test_use_cross_series_group_by_with_columns_not_validated(
        time_series_partition_spec, project_with_target_json, project_without_target,
        holdout_start_date, unittest_endpoint, project_id):
    time_series_partition_spec.multiseries_id_columns = ['series_id']
    time_series_partition_spec.use_cross_series_features = True
    time_series_partition_spec.aggregation_type = 'total'
...
def test_use_cross_series_group_by_with_invalid_columns(
        time_series_partition_spec, project_with_target_json,
        project_without_target, unittest_endpoint, project_id):
