...
from datarobot import (BacktestSpecification, DatetimeModel, DatetimePartitioning,
                       DatetimePartitioningSpecification, FeatureSettings, Periodicity,
                       Project, enums)
...
def datetime_partition_time_series_seq_based_windows_server_data(
    ...
    settings.update({'windowsBasisUnit': 'ROW',
                     'periodicities': [{'timeSteps': 10, 'timeUnit': 'ROW'}]})
    return settings
...
def datetime_partition_milliseconds_server_data(datetime_partition_time_series_server_data):
    settings = dict(datetime_partition_time_series_server_data)
    settings.update({'windowsBasisUnit': 'MILLISECOND',
...
def datetime_partition_time_series_server_data(datetime_partition_server_data):
    settings = dict(datetime_partition_server_data)
    time_series_settings = {'useTimeSeries': True, 'defaultToKnownInAdvance': True,
                            'defaultToDoNotDerive': True,
                            'featureDerivationWindowStart': -9, 'featureDerivationWindowEnd': -5,
                            'forecastWindowStart': 5, 'forecastWindowEnd': 9}
...
def datetime_partition_spec_bad_feature_settings(
    ...
    return DatetimePartitioningSpecification(
        ...
        feature_settings=[FeatureSettings(fs['featureName'], None)
                          for fs in feature_settings_data]
    )
...
def datetime_partition_spec_extra_time_series_controls(
    ...
    return DatetimePartitioningSpecification(
        ...
        periodicities=[Periodicity(p['timeSteps'], p['timeUnit'])
                       for p in server_data['periodicities']]
    )
...
def time_series_partition_spec(datetime_partition_time_series_server_data, holdout_start_date):
    ...
    return DatetimePartitioningSpecification(
        ...
        default_to_do_not_derive=datetime_partition_time_series_server_data[
