...
class ModelDeleter(object):
    ...
    def execute_delete(self, adapter_cls, adapter_settings):
        """ Deletes ows from database that can be selected (pulled from
        database) using values from `selectors` but excluding those that can
        be selected by `keepers`.
        
        :param adapter_cls: Database adapter class.
        :param adapter_settings: Database adapter class settings.
        """
