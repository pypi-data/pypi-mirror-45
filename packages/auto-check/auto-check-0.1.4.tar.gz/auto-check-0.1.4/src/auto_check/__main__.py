...
def is_holiday():
    ...
    try:
        ...
        msg_list.append(get_log(topic, resp))
    except Exception as err:
        # 请求接口失败，调用本地日历，进行基本判断
        msg_list.append(get_log(topic, '查询API失败，失败原因为：\n' + err + '\n---> 调用本地日历，判断是否为周末'))
...
def get_check_conf(url):
    ...
    try:
        ...
        msg_list.append(get_log(topic, '获取配置信息失败, 失败原因为:\n' + err))
    else:
        return conf
...
def main(**kwargs):
    ...
    if params.url == '' and params.config_dir == '':
        msg_list.append(get_log(topic, 'No conf info defined, please check again or use -h'))
        return
    # 工作日判定
    msg_list.append(get_log(topic,'即将检查是否为节假日'))
    if is_holiday():
        msg_list.append(get_log(topic, '节假日, 马上退出'))
        return
    msg_list.append(get_log(topic, '工作日'))
    # 优先从网络获取配置参数
    ...
    if params.url != '':
        try:
            ...
            print(configs)
            msg_list.append(get_log(topic, '获取配置成功, 总数为：' + str(len(configs))))
        except Exception as err:
            msg_list.append(get_log(topic, '请求配置数据异常，原因为：\n' + err.__str__()))
...
if __name__ == '__main__':
    main()
    # send('管理员', '932100862@qq.com', ''.join(msg_list))