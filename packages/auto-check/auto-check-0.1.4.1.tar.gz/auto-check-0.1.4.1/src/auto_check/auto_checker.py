...
def wait(check_type, delay, delay_min=0):
    ...
    if check_type == CheckType.LOGIN_CHECK or delay:
        ...
        msg_list.append(get_log(topic, '将随机暂停' + str(t) + '秒!'))
        time.sleep(t)
    else:
        msg_list.append(get_log(topic, '将直接跳过暂停'))
...
class AutoChecker:
    ...
    def __check_inout(self, check_type):
        ...
        url = 'https://' + self.__server + '/client.do?method=checkin&type=' + check_type.value[0] + '&latlng=' + self.__latlng + '&addr=' + self.__addr + '&sessionkey=abcTX_cZ9RjYt5MD_CjBw&wifiMac='
        query_url = 'https://' + self.__server + '/client.do?method=checkin&type=getStatus&sessionkey=abcTX_cZ9RjYt5MD_CjBw&wifiMac='
        # 先等待，营造随机效果
        # 如果需要延时
        ...
        try:
            ...
            if self.__notice is not None and self.__notice != '':
                send(self.__username, self.__notice, "".join(msg_list))
                # print(msg)
            else:
                print("".join(msg_list))
            msg_list.clear()
    ...
    def __login(self):
        ...
        url = 'https://' + self.__server + '/client.do?method=login&udid=' + \
              ...
              self.__client_os + '&clientosver=12.1&authcode=&dynapass=&tokenpass=&clientChannelId=&clientuserid=&relogin=';
        data = {
