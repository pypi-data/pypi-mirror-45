...
class script_updater():
    def __init__(self, credentials, custom_script_name=None):
        self.default_directory_name = os.path.basename(os.getcwd())
        self.script_name = custom_script_name or self.default_directory_name
        self.zip_address = self.script_name + '.zip'
    ...
    def update_script(self):
        try:
            self.cloudshell_session.UpdateScript(self.script_name, self.zip_address)
        except Exception as e:
            print("[-] THERE WAS AN ERROR UPDATING SCRIPT\n" + str(e) + "\nPLEASE LOAD SCRIPT MANUALLY THE FIRST TIME")
            exit(1)
        else:
            print("[+] SUCCESFULLY UPDATED IN PORTAL: '{script}'".format(script=self.script_name))
