...
def run_test():
    ...
    gpr_object = gpr1dfusion.GibbsInverseGaussianFGPR1D(raw_x_values,raw_y_values,raw_y_errors,amplitude=5.0e-1,length_scale=1.0e0,peak_width=1.0e-1,peak_location=0.5,nrestarts=5)
    ...
    if plt is not None:
        ...
        ax.set_xlim(0.0,1.0)
        fig.savefig(plot_save_directory+'gig_test.png')
        plt.close(fig)
        ...
        ax.set_xlim(0.0,1.0)
        fig.savefig(plot_save_directory+'gig_dtest.png')
        plt.close(fig)

        print("Results of GIG kernel test plotted in directory %s\n" % (plot_save_directory))
