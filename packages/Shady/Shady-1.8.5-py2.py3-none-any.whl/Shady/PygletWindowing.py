...
class Window( object ):
	...
	def __init__( self, width=None, height=None, left=0, top=0, screen=None, frame=False, fullScreenMode=False, visible=True, openglContextVersion=0, legacy=True ):
		...
		if openglContextVersion >= 330: raise NotImplementedError( 'openglContextVersion >= 330 not yet implemented in PygletWindowing back-end' )
		DPI.SetDPIAwareness() # prevent DPI scaling by Windows
...
class Event( object ):
	def __init__( self, **kwargs ):
		for field in 'key button modifiers x y dx dy text'.split(): kwargs.setdefault( field, None )
