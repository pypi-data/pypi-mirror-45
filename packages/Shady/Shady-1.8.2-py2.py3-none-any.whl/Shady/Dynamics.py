...
def ResetTimeBase( x ):
	...
	by `Integral`, 'Derivative` or `Smoother`.  In any of these
	cases, it erases their memory of previous calls.  They (and
	hence the `Function` as a whole) will consider the next `t`
...
def Transition( start=0.0, end=1.0, duration=1.0, delay=0.0, transform=None, finish=None ):
	...
	if delay:
		shift = delay / float( duration )
		p.Transform( lambda x: max( 0.0, x - shift ) )
	p.Watch( watch )
	...
	def AddState( self, state, duration=None, next=Unspecified, onset=None, ongoing=None, offset=None ):
		...
		method is as a class decorator::
			...
			class Second( StateMachine.State ):
				...
				def onset( self ):
					print( self.name )
					self.__duration = random.uniform( 1, 5 )
				def ongoing( self ):
					if self.elapsed > 3.0:
		...
		class-definition approach, although the syntax is simpler for simple cases
