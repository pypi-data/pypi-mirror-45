...
Read it through again and you'll get it."""  # Douglas Adams (1984): So Long And Thanks For All The Fish.
...
def RenderTextOntoImage( imageNumpy, position, string, border=( 0, 0 ), linespacing=1.1, align='left', anchor='lower left', bg=None, blockbg=None, defer=False, wrapping=None, **kwargs ):
	...
	string = WrapString( string, wrapping, lambda s: draw.textsize( s, font=font )[ 0 ] )
...
def WrapString( s, wrapping, widthFunc=len, justify=True, **kwargs ):
	if not wrapping: return s
	justify = ( wrapping < 0 )
	wrapping = abs( wrapping )
	paragraphSeparationPattern = r'\n\s+'
	paragraphSeparators = re.findall( paragraphSeparationPattern, s ) + [ '' ]
	paragraphs = re.split( paragraphSeparationPattern, s )
	spaceWidth = widthFunc( ' ' )
	if not spaceWidth: spaceWidth = widthFunc( 'i' )
	for iPara, s in enumerate( paragraphs ):
		s = re.sub( r'\s*\n\s*', ' ', s ).replace( '\t', '    ' )
		words = re.split( r'(\s*\S+)', s )[ 1::2 ]
		s, width, extraSpaces = '', 0, []
		for i, word in enumerate( words ):
			wordWidth = widthFunc( word )
			if width == 0 or width + wordWidth < wrapping:
				width += wordWidth
				s += word
			else:
				if justify:
					extraSpaces.append( int( ( wrapping - width ) / spaceWidth ) )
				if i:
					word = word.lstrip()
					wordWidth = widthFunc( word )
				width = wordWidth
		...
		if justify:
			...
			for i, ( nExtra, line ) in enumerate( zip( extraSpaces, lines ) ):
				_, nGaps = re.subn( r'(\S\s+)(\S)', '', line )
				nExtra = list( numpy.diff( numpy.round ( numpy.linspace( 0, nExtra, nGaps + 1, endpoint=1 ) ) ).astype( int ) ) 
				lines[ i ] = re.sub( r'(\S\s+)(\S)', lambda m: m.group( 1 ) + ' ' * nExtra.pop( 0 ) + m.group( 2 ), line )
			s = '\n'.join( lines )
			
		paragraphs[ iPara ] = s
		
	return ''.join( p + sep for p, sep in zip( paragraphs, paragraphSeparators ) )
