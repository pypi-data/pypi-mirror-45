...
if __name__ == '__main__':
	...
	try: import numpy
		...
		NB: creating a gauge requires the third-party package `numpy` to
		    have been installed.
		"""#:
	...
	"""
	From now on, we'll also need the `pillow` package, since it is used
	under the hood whenever we want to load from, or save to, an image file.
	"""#:
	...
	def DebugSmoothing( self, event ):
		if event.type == 'key_press' and event.key in [ 's' ]:
			field.smoothing = not field.smoothing
			print( 'polygon smoothing is now %s' % ( 'enabled' if field.smoothing else 'disabled' ) )
		if event.type == 'key_press' and event.key in [ 'c' ]:
			import os, time
			filename = os.path.realpath( '%s-smoothing=%d.png' % ( time.strftime('%Y%m%d-%H%M%S' ), field.smoothing ) )
			print( 'capturing Shady window to ' + filename )
			world.Capture( saveas=filename )
