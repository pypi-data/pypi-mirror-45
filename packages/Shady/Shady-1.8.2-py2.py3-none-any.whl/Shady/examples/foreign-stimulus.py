...
if __name__ == '__main__':
	import Shady
	OpenGL = Shady.DependencyManagement.Import( 'OpenGL', packageName='pyopengl' )
	if not OpenGL: raise ImportError( OpenGL )
	...
	which means some of them are less easy-to-use---you would have
	to wrap sequences by hand in `ctypes` containers---it's easier
	just to install PyOpenGL).
	"""#:
	...
	`.Draw()` or `.draw()`, taking no arguments.
	"""#.
