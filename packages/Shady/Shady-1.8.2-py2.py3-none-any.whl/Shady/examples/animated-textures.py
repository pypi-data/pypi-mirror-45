...
if __name__ == '__main__':
	...
	do things this way.  So there is a different, slightly more
	complicated way, if you need it...
	"""#:
	...
	loaded one by one. In practice you would probably want to
	create the Stimulus with `visible=False` and only make it
	visible after all the textures are in place.
	
	So now we can use the `.page` property, which also supports
	dynamics in the same way as `.frame`. This time, let's use
	a special callable object from the `Shady.Dynamics` sub-
	module:
	"""#:
