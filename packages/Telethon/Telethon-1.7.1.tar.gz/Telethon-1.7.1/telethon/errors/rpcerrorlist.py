...
class EncryptionOccupyFailedError(ServerError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('TDLib developer claimed it is not an error while accepting secret chats and 500 is used instead of 420' + self._fmt_request(request))


class EntitiesTooLongError(BadRequestError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('It is no longer possible to send such long data inside entity tags (for example inline text URLs)' + self._fmt_request(request))
...
class NeedMemberInvalidError(ServerError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('The provided member is invalid or does not exist (for example a thumb size)' + self._fmt_request(request))
...
class PollUnsupportedError(BadRequestError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('This layer does not support polls in the issued method' + self._fmt_request(request))
...
class UntilDateInvalidError(BadRequestError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('That date cannot be specified in this request (try using None)' + self._fmt_request(request))
