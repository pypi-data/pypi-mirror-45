...
from Homevee.Utils.device_types import MQTT_SENSOR
...
class MQTTSensor(Sensor):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM MQTT_SENSORS WHERE ID == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def save_to_db(self, db=None):
        try:
            ...
            if(self.id is None or self.id == ""):
                Database.insert("INSERT INTO MQTT_SENSORS (ROOM, NAME, ICON, SAVE_DATA,"
                            ...
                            {'location': self.location, 'name': self.name, 'icon': self.icon,
                             ...
                             'mqtt_device_id': self.mqtt_device_id, 'mqtt_value_id': self.mqtt_value_id,}, db)
            #update
            else:
                Database.update("UPDATE MQTT_SENSORS SET ROOM = :location, NAME = :name, ICON = :icon,"
                            ...
                            {'location': self.location, 'name': self.name, 'icon': self.icon,
                             ...
                             'value': self.value, 'id': self.id}, db)
    ...
    def load_all(db=None):
        return MQTTSensor.load_all_from_db('SELECT * FROM MQTT_SENSORS', {}, db)
