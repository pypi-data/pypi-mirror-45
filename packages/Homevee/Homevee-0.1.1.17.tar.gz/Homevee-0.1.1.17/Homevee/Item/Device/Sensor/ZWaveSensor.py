...
class ZWaveSensor(Sensor):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM ZWAVE_SENSOREN WHERE ID == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def save_to_db(self, db=None):
        try:
            ...
            if(self.id is None or self.id == ""):
                Database.insert("INSERT INTO ZWAVE_SENSOREN (RAUM, SHORTFORM, ICON, SAVE_DATA,"
                            "SENSOR_TYPE, VALUE) VALUES (:location, :name, :icon, :save_data,"
                            ":sensor_type)",
                            {'location': self.location, 'name': self.name, 'icon': self.icon,
                             'save_data': self.save_data, 'sensor_type': self.sensor_type}, db)
            #update
            else:
                Database.update("UPDATE ZWAVE_SENSOREN SET RAUM = :location, SHORTFORM = :name, ICON = :icon,"
