...
def rgb_control(user, devicetype, id, mode, brightness, color, db, check_user=True):
    if devicetype == URL_RGB_LIGHT:
        result = Database.select_one("SELECT * FROM URL_RGB_LIGHT WHERE ID = :id", {'id': id}, db)
...
def get_rgb_devices(user, room, db):
    devices = []

    #URL_RGB_LIGHT
    results = Database.select_all("SELECT * FROM URL_RGB_LIGHT WHERE LOCATION = :room", {'room': room}, db)
    for item in results:
    ...
    #PHILIPS_HUE
    results = Database.select_all("SELECT * FROM PHILIPS_HUE_LIGHTS WHERE LOCATION = :room AND TYPE = 'color'", {'room': room}, db)
    for item in results:
...
def get_rgb_device(user, type, id, db):
        ...
        #PHILIPS_HUE
            ...
            item = Database.select_one("SELECT * FROM PHILIPS_HUE_LIGHTS WHERE ID = :id AND TYPE = 'color'", {'id': id}, db)
