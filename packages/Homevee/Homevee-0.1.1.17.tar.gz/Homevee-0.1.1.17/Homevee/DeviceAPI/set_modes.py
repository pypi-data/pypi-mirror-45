...
def set_url_switch_binary(user, id, mode, db, check_user=True):
    data = Database.select_one("SELECT * FROM URL_SWITCH_BINARY WHERE ID == :id",
                {'id': id}, db)
...
def set_url_toggle(user, id, db, check_user=True):
    data = Database.select_one("SELECT * FROM URL_TOGGLE WHERE ID == :id",
                {'id': id}, db)
...
def set_socket(user, id, mode, db, check_user=True):
    try:
        funksteckdose = Device.load_from_db(Funksteckdose, id, db)
...
def set_zwave_switch(user, id, mode, db, check_user=True):
    try:
        ...
        cur = db.cursor()
        data = Database.select_one("SELECT * FROM ZWAVE_SWITCHES WHERE ID == :device",
                    {'device': id}, db)
        ...
        if result['code'] == 200:
            Database.update("UPDATE ZWAVE_SWITCHES SET VALUE = :value WHERE ID = :id",
                        {'value': mode, 'id': id}, db)
