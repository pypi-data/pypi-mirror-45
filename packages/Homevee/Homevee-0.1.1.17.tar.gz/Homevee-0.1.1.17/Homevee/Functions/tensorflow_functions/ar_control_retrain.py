...
from Homevee.Utils.file_utils import get_image_directory_path

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
image_dir = get_image_directory_path("ar_control")
output_graph = os.path.join(BASE_DIR, "tensorflow_data", "ar_control_temp", "output_graph.pb")
intermediate_output_graphs_dir = os.path.join(BASE_DIR, "tensorflow_data", "ar_control_temp", "intermediate_graph")
intermediate_store_frequency = 0
output_labels = os.path.join(BASE_DIR, "tensorflow_data", "ar_control_temp", "output_labels.txt")
summaries_dir = os.path.join(BASE_DIR, "tensorflow_data", "ar_control_temp", "retrain_logs")
...
bottleneck_dir = os.path.join(BASE_DIR, "tensorflow_data", "ar_control_temp", "bottleneck")
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    results = Database.select_all("SELECT DISTINCT(CONTROL_KEY) FROM (SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY IS NOT NULL)", {}, db)

    for data in results:
        Logger.log(data)
        labels.append(data['CONTROL_KEY'])

    Logger.log("Labels: " + str(labels))

    for label in labels:
        results = Database.select_all("SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY = :key", {"key": label}, db)
        ...
        result[label] = {
            'dir': os.path.join(Constants.DATA_DIR, "images", "ar_control"),
...
def ar_training(username):

    db = Database.get_database_con()

    training_steps = Database.get_server_data('AR_TF_TRAINING_STEPS', db)

    try:
        how_many_training_steps = int(training_steps)
    except:
        Logger.log("error reading AR_TF_TRAINING_STEPS, using default: "+str(how_many_training_steps))

    model_size = Database.get_server_data('AR_TF_MODEL_SIZE', db)
    image_size = Database.get_server_data('AR_TF_MODEL_IMAGE_SIZE', db)
    try:
    ...
    NotificationManager().send_notification_to_users(username, "AR-Steuerung", "Das Training der AR-Steuerung ist abgeschlossen.", db)
