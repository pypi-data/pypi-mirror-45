...
def upload_images(user, data, img_class, db):
    ...
    for image in images:
        ...
        Database.insert("INSERT INTO PEOPLE_LEARNING_DATA (PATH, PERSON_ID) VALUES (:path, :person_id)",
...
def save_people_class(user, id, data, classname, db):
    return
...
def classify_person(image_data, db):
    ...
    Database.insert("INSERT INTO PEOPLE_LEARNING_DATA (PATH) VALUES (:path)",
                {'path': image_path})
...
def get_person_class_data(id, db):
        ...
        Database.select_all("SELECT * FROM PEOPLE_DATA WHERE ID = :id;", {'id': id})

        return cur.fetchone()

def start_people_training(user, db):
...
def get_people_classes(db):
        ...
        Database.select_all("SELECT * FROM PEOPLE_DATA")
...
def get_people_class_images(image_class, show, offset, db):
    ...
    if image_class is None or image_class == "-1" or image_class == -1:
        results = Database.select_all("SELECT * FROM 'PEOPLE_LEARNING_DATA' WHERE PERSON_ID IS NULL OR PERSON_ID = -1 LIMIT :limit OFFSET :offset",
                    param_array, db)
    else:
        param_array['key'] = image_class
        results = Database.select_all("SELECT * FROM 'PEOPLE_LEARNING_DATA' WHERE PERSON_ID = :key LIMIT :limit OFFSET :offset",
                    param_array, db)
...
def delete_people_images(ids, db):
    ...
    for id in ids:
        ...
        item = Database.select_one("SELECT * FROM PEOPLE_LEARNING_DATA WHERE ID == :id;",
                    {'id': id}, db)
        ...
        if (item is not None):
            ...
            Database.delete("DELETE FROM PEOPLE_LEARNING_DATA WHERE ID == :id;",
                        {'id': id}, db)
...
def change_people_image_class(ids, newclass, db):
    ...
    for id in ids:
        Database.update("UPDATE 'PEOPLE_LEARNING_DATA' SET PERSON_ID = :key WHERE ID == :id;",
...
def get_people_images(image_class, show, offset, db):
    ...
    if image_class == "-1":
        results = Database.select_all("SELECT * FROM 'PEOPLE_LEARNING_DATA' WHERE PERSON_ID IS NULL LIMIT :limit OFFSET :offset",
                    param_array, db)
    else:
        param_array['key'] = image_class
        results = Database.select_all("SELECT * FROM 'PEOPLE_LEARNING_DATA' WHERE PERSON_ID = :key LIMIT :limit OFFSET :offset",
...
def get_people_image(id, db):
    result = Database.select_all("SELECT PATH FROM 'PEOPLE_LEARNING_DATA' WHERE ID == :id",
                {'id': id}, db)
...
def get_performance_settings(user, db):
    ...
    for index in indices:
        result = Database.select_one("SELECT VALUE FROM SERVER_DATA WHERE KEY = :key",
                    {'key': prefix+index}, db)
...
def set_performance_settings(user, data, db):
    ...
    for index in indices:
        ...
        Database.update("UPDATE OR IGNORE SERVER_DATA SET VALUE = :value WHERE KEY = :key;", param_array)

        Database.insert("INSERT OR IGNORE INTO SERVER_DATA (VALUE, KEY) VALUES (:value, :key);", param_array)
