...
from Homevee.Helper.helper_functions import load_remote_data
from Homevee.Item.Status import *
from Homevee.Utils.Database import Database


def get_nutrition_data(user, keyword, db):
...
def submit_food(user, name, calories, portionsize, portionunit, protein, fat, saturated, unsaturated, carbs, sugar, ean, db):
    remote_data = load_remote_data(user, db)
...
def get_user_day_nutrition_data(user, db):
    return

def get_user_nutrition_overview(user, db):
    user_profile = get_user_fitness_profile(user, db)
    ...
    if (user_profile is False):
        ...
        results = Database.select_all("SELECT * FROM NUTRITION_DATA WHERE USER = :user AND DATE = :date",
                    {'user': user.username, 'date': datetime.datetime.today().strftime("%d.%m.%Y")}, db)
        ...
        nutrition_day_data = {'calories_goal': int(user_profile['caloriesgoal']), 'calories_left': int(user_profile['caloriesgoal']-eaten_calories_today), 'nutrition_distribution': nutrition_distribution}
...
def save_user_settings(user, height, weight, birthdate, mode, activity, db):
    calories_goal = get_calorie_need(birthdate, weight, height, activity, mode)

    protein_goal, carbs_goal, fat_goal = get_macro_distribution(calories_goal)

    water_goal = 2.0

    add_user_weight(user, weight, db)
    result = Database.select_one("SELECT COUNT(*) FROM USER_FITNESS_DATA WHERE USERNAME = :user", {'user': user.username}, db)
    ...
    if(result == 0):
        Database.insert("INSERT INTO USER_FITNESS_DATA (user, CALORIE_GOAL, FAT_GOAL, SATURATED_GOAL, CARBS_GOAL, SUGAR_GOAL, PROTEIN_GOAL, WATER_GOAL, BIRTHDATE, HEIGHT, ACTIVITY_MODE, DIET_MODE) VALUES (:user, :cals, :fat, :saturated, :carbs, :sugar, :protein, :water, :birthdate, :height, :activity, :diet)", param_array, db)
    else:
        Database.update("UPDATE USER_FITNESS_DATA SET CALORIE_GOAL = :cals, FAT_GOAL = :fat, SATURATED_GOAL = :saturated, CARBS_GOAL = :carbs, SUGAR_GOAL = :sugar, PROTEIN_GOAL = :protein, WATER_GOAL = :water, BIRTHDATE = :birthdate, HEIGHT = :height, ACTIVITY_MODE = :activity, DIET_MODE = :diet WHERE USERNAME = :user", param_array, db)

    return {'result': 'ok', 'calories_goal': calories_goal, 'protein_goal': protein_goal, 'carbs_goal': carbs_goal, 'fat_goal': fat_goal}

def get_macro_distribution(calories):
...
def load_user_settings(user, db):
        data = {}
        ...
        result = Database.select_one("SELECT * FROM USER_WEIGHT_DATA WHERE USERNAME = :user ORDER BY DATE DESC", {'user': user.username})
        weight_data = result['WEIGHT']

        result = Database.select_one("SELECT * FROM USER_FITNESS_DATA WHERE USERNAME = :user", {'user': user.username})
...
def add_user_weight(user, weight, db):
        date = datetime.datetime.today().__format__("%Y-%m-%d")

        Database.insert("INSERT INTO USER_WEIGHT_DATA (user, DATE, WEIGHT) VALUES (:user, :date, :weight)",
                   {'user': user.username, 'date': date, 'weight': weight})

def delete_food_item(user, id, db):
        Database.delete("DELETE FROM NUTRITION_DATA WHERE ID = :id AND USER = :user",
                    {'id': id, 'user': user.username})

        return Status(type=STATUS_OK).get_dict()

def get_calorie_need(birthdate, weight, height, movement_mode, diet_mode):
...
def get_user_fitness_profile(user, db):

        cur = db.cursor()

        Database.select_all("SELECT * FROM USER_FITNESS_DATA WHERE USERNAME = :user", {'user': user.username})
...
def get_user_weight_progress(user, start, end, db):
    start = datetime.datetime.strptime(start, "%d.%m.%Y").strftime("%Y-%m-%d")
    end = datetime.datetime.strptime(end, "%d.%m.%Y").strftime("%Y-%m-%d")

    weight_items = []

    results = Database.select_all("SELECT WEIGHT, strftime(\"%d.%m.%Y\", DATE) as DATETIME FROM USER_WEIGHT_DATA WHERE USERNAME = :user AND DATE >= :start AND DATE <= :ende ORDER BY DATE ASC",
                {'user': user.username, 'start': start, 'end': end}, db)

    for item in results:
        weight_items.append({'date': item['DATETIME'], 'weight': item['WEIGHT']})

    return {'progress': weight_items}

def get_user_day_nutrition_items(user, date, db):
    daytime_items = {'morning': [], "noon": [], "evening": [], "snacks": []}

    results = Database.select_all("SELECT * FROM NUTRITION_DATA WHERE USER = :user AND DATE = :date",
                {'user': user.username, 'date': date}, db)
    ...
    calories_goal = get_user_fitness_profile(user, db)['caloriesgoal']

    return {'Item': daytime_items, 'caloriesgoal': calories_goal}

def move_item(user, id, date, daytime, deleteold, db):
    item = Database.select_all("SELECT * FROM NUTRITION_DATA WHERE ID = :id", {'id': id}, db)
    ...
    add_edit_user_day_nutrition_item(user, None, date, daytime, name, eatenportionsize, portionsize, portionunit,
                                     calories, fat, saturated, unsaturated, carbs, sugar, protein, db)

    if deleteold == "true" or deleteold == True or deleteold == 1 or deleteold == "1":
        Database.delete("DELETE FROM NUTRITION_DATA WHERE ID = :id", {'id': id}, db)

    return Status(type=STATUS_OK).get_dict()

def add_edit_user_day_nutrition_item(user, id, date, daytime, name, eatenportionsize, portionsize, portionunit, calories, fat, saturated, unsaturated, carbs, sugar, protein, db):
    ...
    if add_new:
        Database.insert("INSERT INTO NUTRITION_DATA (USER, DATE, DAYTIME, FOOD_NAME, EATEN_PORTION_SIZE, PORTIONSIZE, PORTIONUNIT, CALORIES, FAT, SATURATED, UNSATURATED, CARBS, SUGAR, PROTEIN) VALUES (:user, :date, :daytime, :name, :eatenportionsize, :portionsize, :portionunit, :calories, :fat, :saturated, :unsaturated, :carbs, :sugar, :protein)",
                    param_array, db)
    else:
        param_array['id'] = id

        Database.update("UPDATE NUTRITION_DATA SET USER = :user, DATE = :date, DAYTIME = :daytime, FOOD_NAME = :name, EATEN_PORTION_SIZE = :eatenportionsize, PORTIONSIZE = :portionsize, PORTIONUNIT = :portionunit, CALORIES = :calories, FAT = :fat, SATURATED = :saturated, UNSATURATED = :unsaturated, CARBS = :carbs, SUGAR = :sugar, PROTEIN = :protein WHERE ID = :id",
                    param_array, db)
