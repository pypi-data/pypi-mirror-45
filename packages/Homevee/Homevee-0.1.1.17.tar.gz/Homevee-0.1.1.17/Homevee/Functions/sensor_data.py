...
def get_einheit(type, id, db):
        ...
        result = Database.select_one("SELECT " + db_data[type]['sensor_type_col'] + " FROM " + db_data[type]['table'] + " WHERE " +
...
def get_sensor_value(type, id, show_einheit, db):
    ...
    sensor = Device.load_from_db(module_map[type], id, db)
...
def get_sensor_data(user, room, db):
    ...
    if isinstance(room, Room):
        ...
        rooms = [Item.load_from_db(Room, room, db)]
