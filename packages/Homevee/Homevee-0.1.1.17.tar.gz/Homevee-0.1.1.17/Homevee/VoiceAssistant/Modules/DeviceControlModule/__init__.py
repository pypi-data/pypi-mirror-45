...
class VoiceDeviceControlModule(VoiceModule):
    ...
    def find_room(self, text, db):
        results = Database.select_all("SELECT * FROM ROOMS", {}, db)
    ...
    def find_devices(self, text, device_types, location_key, db):
        devices = []
        ...
        for device_type in device_types:
            ...
            if location_key is None:
                ...
                results = Database.select_all("SELECT * FROM " + item[0] + " WHERE " + item[4] + " = :location", param_array, {}, db)
