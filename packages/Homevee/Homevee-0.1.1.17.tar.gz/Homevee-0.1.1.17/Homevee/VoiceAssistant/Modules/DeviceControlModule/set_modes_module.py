#!/usr/bin/python
# -*- coding: utf-8 -*-
from Homevee.DeviceAPI import philips_hue, blind_control
...
from Homevee.VoiceAssistant.voice_patterns import GET_PATTERN_SET_MODES


class VoiceDeviceSetModesModule(VoiceDeviceControlModule):
    def get_pattern(self, db):
        return GET_PATTERN_SET_MODES(db)
    ...
    def set_modes_voice(self, username, text, context, db):
    ...
    def control_blinds(self, username, text, context, db):
        words = text.split(" ")
        ...
        for data_item in blinds_db_data:
            if room is not None:
                results = Database.select_all('SELECT * FROM ' + data_item['table'] + ' WHERE ' + data_item['room_col'] + ' = :room',
                        {'room': room_key}, db)
            else:
                results = Database.select_all('SELECT * FROM ' + data_item['table'], db)
        ...
        for blinds_item in blinds:
            blind_control.set_blinds(username, blinds_item['type'], blinds_item['id'], value, db)
    ...
    def set_thermostat(self, username, text, context, db):
        words = text.split(" ")
        ...
        for thermostat in thermostats:
            heating_control(username, thermostat['type'], thermostat['id'], value, db)
    ...
    def set_dimmer(self, username, text, context, db):
        room = self.find_room(text, db)
