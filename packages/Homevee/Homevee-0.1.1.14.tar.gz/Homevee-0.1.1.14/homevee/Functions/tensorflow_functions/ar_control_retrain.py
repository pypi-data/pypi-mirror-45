...
from homevee.utils.database import get_database_con, get_server_data
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    with db:
        cur = db.cursor()
        cur.execute("SELECT DISTINCT(CONTROL_KEY) FROM (SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY IS NOT NULL)")
        ...
        for label in labels:
            cur = db.cursor()
            cur.execute("SELECT * FROM AR_CONTROL_LEARNING_DATA WHERE CONTROL_KEY = :key", {"key": label})
...
def ar_training(username):
    ...
    training_steps = get_server_data('AR_TF_TRAINING_STEPS', db)
    ...
    model_size = get_server_data('AR_TF_MODEL_SIZE', db)
    image_size = get_server_data('AR_TF_MODEL_IMAGE_SIZE', db)
    try:
    ...
    send_notification_to_users(username, "AR-Steuerung", "Das Training der AR-Steuerung ist abgeschlossen.", db)
