...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    with db:
        cur = db.cursor()
        cur.execute("SELECT DISTINCT(PERSON_ID) FROM (SELECT * FROM PEOPLE_LEARNING_DATA WHERE PERSON_ID IS NOT NULL)")
        ...
        for label in labels:
            cur = db.cursor()
            cur.execute("SELECT * FROM PEOPLE_LEARNING_DATA WHERE PERSON_ID = :key", {"key": label})
...
def people_training(username):
    ...
    training_steps = get_server_data('PEOPLE_TF_TRAINING_STEPS', db)
    try:
    ...
    model_size = get_server_data('PEOPLE_TF_MODEL_SIZE', db)
    image_size = get_server_data('PEOPLE_TF_MODEL_IMAGE_SIZE', db)
    try:
    ...
    send_notification_to_users(username, "Personenerkennung", "Das Training der Personenerkennung ist abgeschlossen.", db)
