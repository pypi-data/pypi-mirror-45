...
def get_energy_device_data(user, room, db):
    devices = []
    ...
    # Z-Wave Stromzähler
    results = Database.select_all("SELECT * FROM ZWAVE_POWER_METER WHERE ROOM_ID = :room", {'room': room}, db)
    for item in results:
        try:
...
def get_energy_data(username, room, devicetype, deviceid, von, bis, db):
    ...
    if((room is None or room == "")
        ...
        with db:
            ...
            Database.select_all("SELECT SUM(POWER_USAGE) AS POWER_USAGE FROM ENERGY_DATA WHERE DATE >= :von AND DATE <= :bis",
                        {'von': von, 'bis': bis})
            result = cur.fetchone()
            ...
            percentage = []
            Database.select_all("SELECT ROOMS.NAME, SUM(POWER_USAGE) AS USAGE, ROOM_ID FROM ROOM_ENERGY_DATA, ROOMS WHERE ROOM_ID = ROOMS.LOCATION AND DATE >= :von AND DATE <= :bis GROUP BY ROOM_ID",
                        {'von': von, 'bis': bis})
...
def get_room_energy_data(username, room, von, bis, db):
    ...
    result = Database.select_all("SELECT SUM(POWER_USAGE) AS POWER_USAGE FROM ROOM_ENERGY_DATA WHERE ROOM_ID = :room AND DATE >= :von AND DATE <= :bis",
                {'room': room, 'von': von, 'bis': bis}, db)
    ...
    for device_type in device_types:
        results = Database.select_all(device_type, {'room': room, 'von': von, 'bis': bis}, db)
...
def get_device_energy_data(username, room , devicetype, deviceid, von, bis, db):
    data = {}
    result = Database.select_all("SELECT SUM(POWER_USAGE) AS POWER_USAGE FROM DEVICE_ENERGY_DATA WHERE DEVICE_TYPE = :type AND DEVICE_ID = :id AND DATE >= :von AND DATE <= :bis",
                {'type': devicetype, 'id': deviceid, 'von':von, 'bis': bis}, db)
...
def get_device_energy_course(username, room, type, id, von, bis, db):
    ...
    results = Database.select_all("SELECT POWER_USAGE, strftime(\"%d.%m.%Y\", DATE) as DATETIME FROM DEVICE_ENERGY_DATA WHERE DEVICE_TYPE = :type AND DEVICE_ID = :id AND DATE >= :von AND DATE <= :bis ORDER BY DATE ASC",
        {'type': type, 'id': id, 'von': von, 'bis': bis}, db)
...
def get_energy_course(username, room, von, bis, db):
    ...
    if (room is not None and room != ""):
        results = Database.select_all("""SELECT POWER_USAGE, strftime(\"%d.%m.%Y\", DATE) as DATETIME FROM ROOM_ENERGY_DATA 
        WHERE ROOM_ID = :room AND DATE >= :von AND DATE <= :bis ORDER BY DATE ASC""",
                                      {'room': room, 'von': von, 'bis': bis}, db)
    else:
        results = Database.select_all("""SELECT SUM(POWER_USAGE) as POWER_USAGE, strftime(\"%d.%m.%Y\", DATE) as DATETIME 
