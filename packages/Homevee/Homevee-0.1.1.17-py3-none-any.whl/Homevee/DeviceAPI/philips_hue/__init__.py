...
def set_light_mode(user, id, mode, saturation, brightness, color, db, check_user=True):
        ...
        Database.select_all("SELECT LOCATION FROM PHILIPS_HUE_LIGHTS WHERE ID = :id", {'id': id})

        result = cur.fetchone()

        if result is None:
            return Status(type=STATUS_ERROR).get_dict()
        elif check_user and not user.has_permission(result['LOCATION']):
        ...
        Database.select_all("SELECT * FROM PHILIPS_HUE_LIGHTS WHERE ID = :id", {'id': id})
        ...
        response = requests.put("http://" + gateway.ip + "/api/" + gateway.key1 + "/lights/" + str(id) + "/state", data=json.dumps(data))
...
def update_light_info(id, mode, hue, saturation, brightness, db):
        ...
        Database.update("UPDATE PHILIPS_HUE_LIGHTS SET IS_ON = :mode, BRIGHTNESS = :bri, SATURATION = :sat, HUE = :hue WHERE ID = :id",
