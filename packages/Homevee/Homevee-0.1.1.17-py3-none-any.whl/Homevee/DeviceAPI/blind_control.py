...
def set_blinds(user, type, id, new_position, db, check_user=True):
    try:
        if type == RADEMACHER_BLIND_CONTROL:
            data = Database.select_one("SELECT LOCATION FROM HOMEPILOT_BLIND_CONTROL WHERE ID = :id", {'id': id}, db)
...
def set_room_blinds(user, room, new_position, db):

        cur = db.cursor()
        devices = get_blinds(user, room, db)
...
def get_all_blinds(user, db):
        ...
        for room in rooms:
            ...
            room_blinds = {'name': room['NAME'], 'location': room['LOCATION'], 'icon': room['ICON'],
                                'blind_array': get_blinds(user, room['LOCATION'], db)}
            blinds.append(room_blinds)
        return {'blinds': blinds}

def get_blinds(user, location, db):
