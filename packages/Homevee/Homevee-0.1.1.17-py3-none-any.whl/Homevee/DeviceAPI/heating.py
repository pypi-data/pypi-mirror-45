...
def heating_control(user, type, id, value, db, check_user=True):
    if type == MAX_CUBE:
    ...
    thermostat = Device.load_from_db(module, id, db)
...
def control_room_heating(user, room, value, db):
        devices = get_thermostats(user, room, "", "", db)['thermostats'][0]['thermostat_array']
        for device in devices:
            id = device['id']
            type = device['type']
            result = heating_control(user, type, id, value, db)
            if 'result' not in result or result['result'] != "ok":
                return Status(type=STATUS_ERROR).get_dict()
        return Status(type=STATUS_OK).get_dict()

def get_thermostat_info(user, room, type, id, db):
    ...
    thermostat = Device.load_from_db(module_map[type], id, db)

    min, max = thermostat.get_min_max()
    return {'value': thermostat.temp, 'min': min, 'max': max}

def get_thermostats(user, room, db):
    if isinstance(room, Room):
        rooms = [room]
    elif room == "all":
        rooms = Room.load_all(db)
    else:
        rooms = Room.load_all_ids_from_db([room], db)
    ...
    for room in rooms:
        ...
        for module in modules:
            thermostat_items = Device.get_all(module, room, db)
