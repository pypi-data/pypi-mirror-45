...
def save_request_to_db(data, reply, db):
    ...
    if data['action'] == "arcontrol":
        ...
        Database.insert("INSERT INTO REQUEST_DATA (RESPONSE) VALUES (:response)",
            {'response': reply}, db)

        request_id = cur.lastrowid

        for key in list(data.keys()):
            Database.insert("INSERT INTO REQUEST_DATA_PARAMS (REQUEST_ID, PARAM_KEY, PARAM_VALUE) VALUES(:id, :key, :value)",
                {'id': request_id, 'key': key, 'value': data[key]}, db)
...
def remote_control_enabled(db):
    REMOTE_CONTROL_ENABLED = Database.get_server_data("USE_REMOTE_CONTROL", db)
    return REMOTE_CONTROL_ENABLED == "true"

def set_remote_control_enabled(user, enabled, db):
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}
    Database.set_server_data("USE_REMOTE_CONTROL", enabled, db)
    return Status(type=STATUS_OK).get_dict()

def save_remote_data(user, remote_id, linked_account, db):
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}

    Database.set_server_data("REMOTE_ID", remote_id, db)
    Database.set_server_data("REMOTE_LINKED_ACCOUNT", linked_account, db)

    return Status(type=STATUS_OK).get_dict()

def load_remote_data(user, db):
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}

    remote_id = Database.get_server_data("REMOTE_ID", db)
    access_token = Database.get_server_data("REMOTE_ACCESS_TOKEN", db)
    linked_account = Database.get_server_data("REMOTE_LINKED_ACCOUNT", db)
...
def connect_remote_id_with_account(user, account_name, account_secret, db):
    if(not user.has_permission("admin")):
        return {'result': 'noadmin'}
    ...
    if response == "ok":
        Database.set_server_data("REMOTE_LINKED_ACCOUNT", account_name, db)
