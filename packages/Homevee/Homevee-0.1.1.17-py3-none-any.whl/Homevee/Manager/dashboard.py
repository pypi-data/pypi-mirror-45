...
def get_user_dashboard(user, db):
    ...
    if dashboard_data is not None and dashboard_data != "":
        ...
        for item in data['dynamic']:
            ...
            if not found:
                dashboard_data['dynamic'].append(get_device_info(user, item['type'], item['id'], db))

        return dashboard_data

    return {'result': 'unknownuser'}

def get_user_dashboard_items(user, db):
    dashboard_array = []
    ...
    if 'calendar' in static_data:
        if static_data['calendar']:
            calendar_entries = get_calendar_day_items(user, datetime.datetime.now().strftime("%Y-%m-%d"), db)
    ...
    try:
        if 'nutritionmanager' in static_data:
            if static_data['nutritionmanager']:
                data_item = get_user_nutrition_overview(user, db)

                if 'nutrition_day_data' in data_item['data']:
                    calories_left = data_item['data']['nutrition_day_data']['calories_left']
                    if (calories_left >= 0):
...
def edit_user_dashboard(username, dashboard_data, db):
    ...
    Database.update("UPDATE USERDATA SET DASHBOARD_DATA = :data WHERE USERNAME == :username",
                {'data': dashboard_data_json, 'username': username})

    return get_user_dashboard(username, db)

def get_user_favourite_devices(user, db):
    ...
    fav_array = get_dynamic_items(user, dynamic_items, db)

    return fav_array

def get_dynamic_items(user, dynamic_items, db):
    rooms = get_rooms(user, db)['rooms']
    ...
    for type in types:
        ...
        if len(ids) > 0:
            for id in ids:
                item = get_device_info(user, type, id, db)
...
def get_device_info(user, type, id, db):
    Logger.log(type + " - " + id)
    ...
    try:
        if type == FUNKSTECKDOSE:
            item = Database.select_one("SELECT * FROM FUNKSTECKDOSEN WHERE DEVICE == :id", {'id': id}, db)
            ...
            item = Database.select_one("SELECT * FROM PHILIPS_HUE_LIGHTS WHERE ID == :id", {'id': id}, db)
        ...
        #URL-Geräte
            ...
            item = Database.select_one("SELECT * FROM URL_RGB_LIGHT WHERE ID = :id", {'id': id}, db)
            ...
            item = Database.select_one("SELECT * FROM URL_TOGGLE WHERE ID = :id",{'id': id}, db)
            device_data = {'name': item['NAME'], 'room': item['LOCATION'], 'id': item['ID'],
                                'icon': item['ICON'], 'value': '', 'type': 'toggle'}
        #Z-Wave
        elif type == ZWAVE_THERMOSTAT:
            item = Database.select_one("SELECT * FROM ZWAVE_THERMOSTATS WHERE THERMOSTAT_ID == :id", {'id': id}, db)
            data = get_thermostat_info(user, item['RAUM'], type, item['THERMOSTAT_ID'], db)
            ...
            item = Database.select_one("SELECT * FROM ZWAVE_DIMMER WHERE ID == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['THERMOSTAT_ID'],
                    'icon': item['ICON'], 'room': item['LOCATION'], 'type': 'heating',
                    'value': item['VALUE']}
        elif type == ZWAVE_POWER_METER:
            item = Database.select_one("SELECT * FROM ZWAVE_THERMOSTATS WHERE DEVICE_ID == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['DEVICE_ID'],
                    'icon': item['ICON'], 'room': item['LOCATION'], 'type': 'heating',
                    'value': item['VALUE']}
        elif type == ZWAVE_SENSOR:
            item = Database.select_one("SELECT * FROM ZWAVE_SENSOREN WHERE ID == :id", {'id': id}, db)
            device_data = {'name': item['SHORTFORM'], 'id': item['ID'],
                    'icon': item['ICON'], 'room': item['RAUM'], 'type': 'value',
                           'value': get_sensor_data(user, item['RAUM'], type, id, 1, db)}
        elif type == ZWAVE_SWITCH:
            item = Database.select_one("SELECT * FROM ZWAVE_SWITCHES WHERE THERMOSTAT_ID == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['THERMOSTAT_ID'],
                    'icon': item['ICON'], 'room': item['LOCATION'], 'type': 'heating',
                    'value': item['VALUE']}
        #MAX Cube
        elif type == MAX_THERMOSTAT:
            item = Database.select_one("SELECT * FROM MAX_THERMOSTATS WHERE ID == :id", {'id': id}, db)
            data = get_thermostat_info(user, item['RAUM'], type, id, db)
            device_data = {'name': item['NAME'], 'id': item['ID'], 'room': item['RAUM'], 'type': 'heating',
                    'icon': item['ICON'],
                    'value': data['value'], 'min': data['min'], 'max': data['max'], 'step': 0.5}
        #Wake on Lan
        elif type == WAKE_ON_LAN:
            item = Database.select_one("SELECT * FROM WAKE_ON_LAN WHERE DEVICE == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['DEVICE'], 'icon': item['ICON'], 'room': item['LOCATION'],
                           'value': '', 'type': 'wakeonlan'}
        elif type == XBOX_ONE_WOL:
            item = Database.select_one("SELECT * FROM XBOX_ONE_WOL WHERE ID == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['ID'], 'icon': item['ICON'], 'room': item['LOCATION'],
                           'value': '', 'type': 'wakeonlan'}

        #MQTT Sensor
        elif type == MQTT_SENSOR:
            item = Database.select_one("SELECT * FROM MQTT_SENSORS WHERE ID == :id", {'id': id}, db)
            device_data = {'name': item['NAME'], 'id': item['ID'],
                           'icon': item['ICON'], 'room': item['ROOM'], 'type': 'value',
                           'value': get_sensor_data(user, item['ROOM'], type, id, 1, db)}
    except:
    ...
    device_data['roomname'] = Room.get_name_by_id(device_data['room'], db)
