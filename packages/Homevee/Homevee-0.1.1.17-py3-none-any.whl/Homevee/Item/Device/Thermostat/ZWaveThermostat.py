...
class ZWaveThermostat(Thermostat):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM ZWAVE_THERMOSTATS WHERE ID == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def save_to_db(self, db=None):
        try:
            # insert
            Database.insert("""INSERT OR IGNORE INTO ZWAVE_THERMOSTATS (THERMOSTAT_ID, NAME, ICON, VALUE, RAUM) VALUES 
                        (:id, :name, :icon, :last_temp, :room)""",
                        {'id': self.id, 'name': self.name, 'icon': self.icon,
                         'last_temp': self.temp, 'room': self.location})
            # update
            Database.update("""UPDATE OR IGNORE ZWAVE_THERMOSTATS SET NAME = :name, ICON = :icon,
