...
class Funksteckdose(Switch):
    ...
    def delete(self, db=None):
        try:
            Database.delete("DELETE FROM FUNKSTECKDOSEN WHERE DEVICE == :id", {'id': self.id}, db)
            return True
        except:
            return False
    ...
    def save_to_db(self, db=None):
        try:
            ...
            if(self.id is None or self.id == ""):
                Database.insert("INSERT INTO FUNKSTECKDOSEN (ROOM, NAME, ICON, HAUSCODE, STECKDOSENNNUMMER) "
                            "VALUES (:location, :name, :icon, :home_code, :socket_number)",
                            {'location': self.location, 'name': self.name, 'icon': self.icon,
                             'home_code': self.home_code, 'socket_number': self.socket_number}, db)
            #update
            else:
                Database.update("UPDATE FUNKSTECKDOSEN SET ROOM = :location, NAME = :name, ICON = :icon,"
                            "HAUSCODE = :home_code, STECKDOSENNUMMER = :socket_number, ZUSTAND = :mode"
                            "WHERE DEVICE = :id",
                            {'location': self.location, 'name': self.name, 'icon': self.icon,
                             'home_code': self.home_code, 'socket_number': self.socket_number,
                             'mode': self.mode, 'id': self.id}, db)
