...
def set_context(username, context_key, context_data, db):
    ...
    Database.delete("DELETE FROM 'VOICE_COMMAND_USER' WHERE USERNAME == :user",
                {'user': username}, db)

    Database.insert("INSERT INTO VOICE_COMMAND_USER (USERNAME, CONTEXT_TIME, CONTEXT_KEY, CONTEXT_DATA) VALUES (:username, :time, :key, :data)",
                {'username': username, 'time': int(time.time()), 'key': context_key, 'data': context_data}, db)

def get_context(username, db):
    data = Database.select_all("SELECT * FROM 'VOICE_COMMAND_USER' WHERE USERNAME == :user",
                {'user': username}, db)
...
def replace_voice_commands(text, username, db):
        ...
        Database.select_all("SELECT * FROM VOICE_COMMAND_REPLACE WHERE USERNAME = :user",
                    {'user': username})
