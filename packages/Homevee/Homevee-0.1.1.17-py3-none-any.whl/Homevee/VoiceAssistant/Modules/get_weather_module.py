...
from Homevee.VoiceAssistant import helper, context_keys
from Homevee.VoiceAssistant.Modules import VoiceModule
from Homevee.VoiceAssistant.voice_patterns import PATTERN_GET_WEATHER


class VoiceGetWeatherModule(VoiceModule):
    def get_pattern(self, db):
        return PATTERN_GET_WEATHER
    ...
    def get_weather(self, username, text, context, db):
        weather = Database.get_server_data('WEATHER_CACHE', db)
        ...
        for day in weather_list:
            ...
            if (cur_date.date() == search_date.date()):
                ...
                helper.set_context(username, context_keys.CONTEXT_GET_WEATHER, {}, db)
