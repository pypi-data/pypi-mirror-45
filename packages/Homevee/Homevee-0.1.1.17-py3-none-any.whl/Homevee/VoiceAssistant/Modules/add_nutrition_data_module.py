...
from Homevee.Functions.nutrition_data import get_nutrition_data, add_edit_user_day_nutrition_item, \
    get_user_nutrition_overview
...
class VoiceAddNutritionDataModule(VoiceModule):
    def get_pattern(self, db):
        return PATTERN_ADD_NUTRITION_ITEM
    ...
    def voice_add_nutrition_item(self, username, text, context, db):
        ...
        if(item is not None):
            ...
            if(amount is None):
                ...
                set_context(username, CONTEXT_ADD_NUTRITION, context, db)
