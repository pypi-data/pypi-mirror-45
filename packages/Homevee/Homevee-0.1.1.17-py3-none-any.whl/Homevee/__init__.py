...
from Homevee.Cronjob.run_automations import TimedAutomationsCronjob
...
class Homevee():
    ...
    def __init__(self, cloud_connection=True, websocket_server=True, http_server=True):
        ...
        if(homevee_update is not None):
            translate_print('update_available').format(homevee_update)
    ...
    def add_user(self, username, password, is_admin=False):
        ...
        Database.insert("INSERT OR IGNORE INTO 'userdata' (USERNAME, PASSWORD, PW_SALT, IP, PERMISSIONS) VALUES (:username, :password, :salt, :ip, :permissions);",
            param_array)
    ...
    def start(self):
        ...
        if test_speech:
            while (True):
                ...
                print(VoiceAssistant.voice_command("sascha", voice_input, None, None, Database.get_database_con(), translations.LANGUAGE))
        ...
        # Database.set_server_data(ar_control.IS_TRAINING_TAG, "false", db)
        # Database.set_server_data(people_classifier.IS_TRAINING_TAG, "false", db)

        # check if certs exist
        check_cert(db=Database.get_database_con())
