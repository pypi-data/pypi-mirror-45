...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    results = Database.select_all("SELECT DISTINCT(PERSON_ID) FROM (SELECT * FROM PEOPLE_LEARNING_DATA WHERE PERSON_ID IS NOT NULL)", {}, db)
    ...
    for label in labels:
        results = Database.select_all("SELECT * FROM PEOPLE_LEARNING_DATA WHERE PERSON_ID = :key", {"key": label}, db)
...
def people_training(username):
    ...
    training_steps = Database.get_server_data('PEOPLE_TF_TRAINING_STEPS', db)
    try:
    ...
    model_size = Database.get_server_data('PEOPLE_TF_MODEL_SIZE', db)
    image_size = Database.get_server_data('PEOPLE_TF_MODEL_IMAGE_SIZE', db)
    try:
    ...
    NotificationManager().send_notification_to_users(username, "Personenerkennung", "Das Training der Personenerkennung ist abgeschlossen.", db)
