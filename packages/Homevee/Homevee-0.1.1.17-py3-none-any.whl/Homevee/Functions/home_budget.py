...
def get_home_budget_data(user, startdate, db):
    data = {}

    #Find expenditures grouped by date
    results = Database.select_all("SELECT SUM(AMOUNT) as AMOUNT, strftime(\"%d.%m.%Y\", DATE) as DATE FROM HOME_BUDGET_DATA WHERE AMOUNT <= 0 AND DATE >= :date GROUP BY DATE ORDER BY DATE ASC",
                {'date': startdate}, db)
    costs = []
    ...
    item = Database.select_one("SELECT SUM(AMOUNT) as AMOUNT FROM HOME_BUDGET_DATA", {}, db)
    overall_amount = round(item['AMOUNT'], 2)
    data['overall_amount'] = overall_amount

    #Compute date where money reaches 0
    item = Database.select_one("SELECT AVG(AMOUNT) as AMOUNT FROM (SELECT SUM(AMOUNT) as AMOUNT FROM HOME_BUDGET_DATA WHERE AMOUNT < 0 GROUP BY DATE);", {}, db)
    average_spending_per_day = item['AMOUNT']*-1

    #compute timeperiod
    min_max_date = Database.select_one("SELECT MIN(DATE) as mindate, MAX(DATE) as maxdate, COUNT(*) as numdays FROM (SELECT DISTINCT DATE FROM HOME_BUDGET_DATA);", {}, db)
...
def get_home_budget_data_graph(user, startdate, enddate, db):
    item = Database.select_one("SELECT SUM(AMOUNT) as AMOUNT FROM HOME_BUDGET_DATA WHERE DATE < :startdate;",
    ...
    #Get values
    results = Database.select_all("SELECT SUM(AMOUNT) as AMOUNT, strftime(\"%d.%m.%Y\", DATE) as FORMATTED_DATE FROM HOME_BUDGET_DATA WHERE DATE >= :startdate AND DATE <= :enddate GROUP BY DATE ORDER BY DATE ASC",
        {'startdate': startdate, 'enddate': enddate}, db)
...
def get_home_budget_data_day_items(user, date, db):
    data = []

    results = Database.select_all("SELECT *, strftime(\"%d.%m.%Y\", DATE) as FORMATTED_DATE FROM HOME_BUDGET_DATA WHERE DATE = :date ORDER BY DATE ASC",
                {'date': date}, db)
...
def add_edit_home_budget_data(user, id, date, info, amount, db):
    ...
    return home_budget_item.api_save()

def delete_home_budget_data(user, id, db):
    item = Item.load_from_db(HomeBudgetItem, id, db)
