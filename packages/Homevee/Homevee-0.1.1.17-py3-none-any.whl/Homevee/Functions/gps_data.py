...
def update_gps(user, time, lat, lng, db):

        cur = db.cursor()

        on_location_change(user, int(time), float(lat), float(lng), db)

        Database.insert("INSERT INTO GPS_DATA (user, TIMESTAMP, LATITUDE, LONGITUDE) VALUES (:user, :time, :lat, :lng);",
                    {'user': user.username, 'time': time, 'lat': lat, 'lng': lng})

        return Status(type=STATUS_OK).get_dict()

def on_location_change(user, time, lat, lng, db):
    #determine user location (at work, at school, at home, etc.)
    current_user_location = determine_user_location(user, time, lat, lng, db)
    set_user_location(user, current_user_location, db)

    return

def determine_user_location(user, time, lat, lng, db):
    ...
    #check distance to place => less than MAX_DISTANCE => user is there
        ...
        place = get_nearest_place(user, lat, lng, db)

        Database.select_all("SELECT * FROM GPS_DATA WHERE USERNAME = :user ORDER BY TIMESTAMP DESC LIMIT 50", {'user': user.username})
...
def get_nearest_place(user, lat, lng, db):

        cur = db.cursor()

        Database.select_all("SELECT *, ((LATITUDE - :lat)*(LATITUDE - :lat)) + ((LONGITUDE - :lng)*(LONGITUDE - :lng)) as DISTANCE FROM PLACES ORDER BY DISTANCE ASC LIMIT 1",
                    {'lat': lat, 'lng': lng})
...
def set_user_location(user, place, db):
        ...
        Database.update("UPDATE USERDATA SET CURRENT_LOCATION = :loc WHERE USERNAME = :user",
                    {'loc': place_id, 'user': user.username})

def get_gps_locations(user, db):

        cur = db.cursor()

        locations = []

        Database.select_all("SELECT * FROM GPS_DATA WHERE USERNAME = :user", {'user': user.username})
