...
class TekDPO(TekOSC):
    ...
    def get_waveform(self,ch=1,io_timeout=3000,requestSize=4096):
        ...
        h=self.ask(":WFMO?;") # this opeation may take 0.3 sec avg.
        self.write(":curve?;")
...
class waveform(_waveform):
    ...
    def __init__(self,preamble,wfdata):
        ...
        if (len(self.rdata) < 17):
            sys.stderr.write("%s data read:"%(self.rdata[:16], len(self.rdata)))
...
def test_srq(hostname="192.168.2.4", device="inst0"):
    ...
    osc.createSVCThread()
    sys.stdout.write("Thread Created\n")
    osc.Stop()
    ...
    sys.stdout.write("%s"%osc.svc_lock.locked()) #should be True
    osc.write(":ACQ:STATE RUN;*OPC;")
    sys.stdout.write("%s"%osc.svc_lock.locked())
    return osc
