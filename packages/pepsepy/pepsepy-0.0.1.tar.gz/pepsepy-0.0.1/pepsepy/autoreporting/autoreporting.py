...
# content = "Hello, World!" # content could be an object comme from simulation
...
class ModelResults():
    ...
    def get_result_df_as_html(self):
        """
...
def csv_to_df(filepath):
    """
    Open a .csv file and return it into DataFrame format.
...
def common_misidentified(list_model_results):
    """
...
def csv_to_html(filepath):
    """
    Open a .csv file and return it into html format.
    :param filepath: Filepath to a .csv file to be read.
...
def main():
    ...
    sections.append(table_section_template.render(
        ...
        table = vgg19_results.get_result_df_as_html()
        ))
    sections.append(table_section_template.render(
        model = mobilenet_results.model_name,
        dataset = mobilenet_results.dataset,
        table = mobilenet_results.get_result_df_as_html()
        ))
    
    # Produce and write the report to file
    with open("outputs/report.html","w") as f:
        f.write(base_template.render(title = title,
        sections = sections))
