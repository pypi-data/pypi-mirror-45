...
FLAGS.server = 'localhost:9000'
FLAGS.servable_name = 'servex'
FLAGS.problem = 'deepspeech'
FLAGS.data_dir = 'gs://servex-tensor/data'
...
def stt_train():
  try:
    ...
    params = [['audio', "", 'Audio for synthesize.']]
    ...
    if values['audio'] == '':
      raise ValueError('Enter a valid audio')

    inputs = values['audio']
    outputs = serving_utils.predict([inputs], problem, request_fn)
    outputs, = outputs
    output, score = outputs
    data['Response'].append({'synthezise_info':{'Input': str(inputs), 'Output':str(output), 'Score': str(score)}})
    return jsonify(data)
