...
class deepspeech(speech_recognition.SpeechRecognitionProblem):
  ...
  def generator(self, data_dir, tmp_dir, datasets,
    ...
    print(tmp_dir)
    print(data_dir)
    raw_data_dir = os.path.join(tmp_dir, "data")
    data_files = _collect_data(raw_data_dir, "wav", "txt")
...
@registry.register_problem()
class deepspeechClean(deepspeech):
  """Problem to train on full 960h, but evaluate on clean data only."""

  def training_filepaths(self, data_dir, num_shards, shuffled):
    return deepspeech.training_filepaths(self, data_dir, num_shards, shuffled)

  def dev_filepaths(self, data_dir, num_shards, shuffled):
    return deepspeechClean.dev_filepaths(self, data_dir, num_shards, shuffled)

  def test_filepaths(self, data_dir, num_shards, shuffled):
    return deepspeechClean.test_filepaths(self, data_dir, num_shards, shuffled)
...
@registry.register_problem()
class deepspeechCleanSmall(deepspeech):
...
@registry.register_problem()
class deepspeech__Clean(deepspeech):
...
@registry.register_problem()
class deepspeechNoisy(deepspeech):
