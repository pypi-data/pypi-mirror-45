...
class TestOptionParam(unittest.TestCase):
    def test_init(self):
        ...
        self.assertEqual(six.text_type(cm.exception), "Illegal value: 46 "\
                         + b"\xe2\x88\x89 [45, 56]".decode('utf-8'))
        ...
        self.assertEqual(six.text_type(OptionParam(45, [45, 56], "bada")), \
    ...
    def test_assign(self):
        ...
        with self.assertRaises(ValueError) as cm:
            p = OptionParam(45, [45, 56], "jada")
            p.value = 57
        self.assertEqual(six.text_type(cm.exception), "Illegal value 'jada': 57 "\
                         + b"\xe2\x88\x89 [45, 56]".decode('utf-8'))
...
class TestScalarParam(unittest.TestCase):
    def test_init(self):
        ...
        with self.assertRaises(ValueError) as cm:
            ScalarParam(45, 50)
        self.assertEqual(six.text_type(cm.exception), b"Illegal value: 45 \xe2\x88\x89 ".decode('utf-8')\
    ...
    def test_value_assign(self):
        with self.assertRaises(ValueError) as cm:
            ...
            p.value = 56
        self.assertEqual(six.text_type(cm.exception), "Illegal value: 56 "\
...
if np is not None:
    ...
    class TestArrayParam(unittest.TestCase):
        def test_init(self):
            ...
            with self.assertRaises(ValueError) as cm:
                ArrayParam(45, 0)
            self.assertEqual(six.text_type(cm.exception), b"0 \xe2\x88\x89 [1, ".decode('utf-8')\
            ...
            self.assertEqual(six.text_type(cm.exception), "Illegal value: [40, 60, 45] "\
                             + b"\xe2\x88\x89 [50, \xe2\x88\x9e]".decode('utf-8'))

            with self.assertRaises(ValueError) as cm:
                ArrayParam(45, 4, ge=0, lt=10)
            self.assertEqual(six.text_type(cm.exception), "Illegal value: [45, 45, 45, 45]"\
                             + b" \xe2\x88\x89 [0, 10)".decode('utf-8'))
