...
class DataSet:
	...
	@logsup.with_logging(log_level=DEBUG,atomic_print=True)
...
@logsup.with_logging()
def rgb_to_geotiff(tiff_path,*band_paths,no_data_value=-9999,**warp_opts):
	...
	outdata=driver.Create(tiff_path,rows,cols,len(band_paths),gdal.GDT_Float32,options=options)
...
def main():
	...
	from .databases.sqla_orms import Cadastres_Info
	import geojson
