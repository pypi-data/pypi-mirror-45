...
class TheLogger(IndentedLoggerAdapter):
	...
	def __init__(self,name,folder_path: Union[Folder,str,],level=logging.DEBUG,log_format: str = None,to_stream=True,**kwargs):
		self.base_log_folder: Folder=Folder(folder_path) if type(folder_path) is str else folder_path
		...
		self.__log_folder: Folder=self.base_log_folder
	...
	def create(self,*child_folders,**info_kwargs):
		self.__log_folder=self.base_log_folder.create(*[get_name(x) for x in child_folders],**info_kwargs)
		return self
	...
	def get_filepath(self,*name_portions,ext='.log',delim='__',include_depth=2,include_datetime=True,**name_kwargs):
		...
		self.log_path=self.__log_folder.get_filepath(*name_portions,ext=ext,delim=delim,include_depth=include_depth,
	...
	# 	fp=self.__log_folder.get_filepath(*name_portions,ext=ext,delim=delim,include_depth=include_depth,
	# 	                                       datetime_loc_index=datetime_loc_index,**name_kwargs)
...
aligner=lambda n:f'{n}s'
color_log_format=f"{color_info}%(asctime)s pid:%(process){aligner(5)} %(levelname){aligner(8)} {color_info}%(message)s"
# nocolor_log_format=f"%(asctime)s %(levelname){aligner(8)} %(message)s"
nocolor_log_format=f"%(asctime)s pid:%(process) %(levelname){aligner(8)} %(message)s"
