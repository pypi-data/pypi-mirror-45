...
class ExtensionRepositoryApi(object):
    ...
    def get_extension_repo_version_sha256_0(self, group_id, artifact_id, version, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_extension_repo_version_sha256_0(group_id, artifact_id, version, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_extension_repo_version_sha256_0_with_http_info(group_id, artifact_id, version, **kwargs)
        else:
            (data) = self.get_extension_repo_version_sha256_0_with_http_info(group_id, artifact_id, version, **kwargs)
            return data

    def get_extension_repo_version_sha256_0_with_http_info(self, group_id, artifact_id, version, **kwargs):
        """
        Get global extension repo version checksum
        Gets the hex representation of the SHA-256 digest for the binary content with the given bucket, group, artifact, and version. Since the same group-artifact-version can exist in multiple buckets, this will return the checksum of the first one returned. This will be consistent since the checksum must be the same when existing in multiple buckets.   NOTE: This endpoint is subject to change as NiFi Registry and its REST API evolve.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_extension_repo_version_sha256_0_with_http_info(group_id, artifact_id, version, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_extension_repo_version_sha256_0" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'group_id' is set
        if ('group_id' not in params) or (params['group_id'] is None):
            raise ValueError("Missing the required parameter `group_id` when calling `get_extension_repo_version_sha256_0`")
        # verify the required parameter 'artifact_id' is set
        if ('artifact_id' not in params) or (params['artifact_id'] is None):
            raise ValueError("Missing the required parameter `artifact_id` when calling `get_extension_repo_version_sha256_0`")
        # verify the required parameter 'version' is set
        if ('version' not in params) or (params['version'] is None):
            raise ValueError("Missing the required parameter `version` when calling `get_extension_repo_version_sha256_0`")
