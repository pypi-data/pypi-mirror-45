...
class GitBranch(object):
    ...
    *repo_url*: Urls of remote origins of the repository
    ...
    def __init__(self, key_path=False):
        ...
        self.ssh_command = 'ssh -i %s' % (key_path)
            ...
            for element in os.listdir(path):
                ...
                if os.path.isdir(element_path) and not os.path.islink(element_path):
                    if element == '.git':
                        ...
                        _logger.info("Branch collected from %s", element_path)
                    else:
    ...
    def __clone(self, path, branch):
        ...
        _logger.info('Cloning repo %s - branch %s', branch['name'], branch['branch'])
        kwargs = {}
        if self.key_path:
            kwargs.update({'env': {'GIT_SSH_COMMAND': self.ssh_command}})
    ...
    def reset(self, branch, path):
        _logger.info('Reseting repo %s to commit %s', branch['path'], branch['commit'])
    ...
    def __pull(self, branch, path=False):
        ...
        try:
            ...
            if self.key_path:
                ...
                set_known_hosts(git_url=repo.remote().url)
    ...
    def pull(self, branch):
        """ Updates a single branch to a specified commit
            ...
            :return: Message dictionary
        """
        _logger.info('Pulling latest changes for %s', branch['name'])
...
def action_file(tmp_dir, action, path):
    ...
    """
    _logger.info('Creating %s', action)
    b_info = []
    branches = GitBranch()
...
def load(repos, path, tmp, key_path=False):
    ...
    known_hosts = []
    for repo in repos:
        res = utils.re_match_git_url(repo.get('repo_url', {}).get('origin'))
        if not res or res.get('host') in known_hosts:
            continue
        known_hosts.append(res.get('host'))
    if key_path:
        set_known_hosts(known_hosts=known_hosts)
    for paths in path_list:
...
def set_known_hosts(git_url=False, known_hosts=False):
    """Set the new host to the `known_hosts` file.
    """
    git_url = git_url or ''
    known_hosts = known_hosts or []
    host = utils.re_match_git_url(git_url).get('host')
    known_hosts.append(host)
    path_known = os.path.join(os.environ['HOME'], '.ssh/known_hosts')
    if not os.path.exists(path_known):
        os.system('touch %s 2> /dev/null' % (path_known))
    for host in known_hosts:
        os.system('ssh-keyscan %s >> %s 2> /dev/null' % (host, path_known))
