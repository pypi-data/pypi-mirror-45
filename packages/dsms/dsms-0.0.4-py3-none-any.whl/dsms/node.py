...
class NodeUtil:
  ...
  def send_diff_info(project):
    project_info = NodeConfig.projects[project]
    sio.emit('project_info', project_info)
  ...
  def set_default_project_info(project):
    if project not in NodeConfig.projects:
      NodeConfig.projects[project] = {
        'project': project,
        'diff_list': [],
        'last_diff_ts': 0,
        'last_pull_ts': 0,
...
@sio.on('sync_mirror', namespace='/test')
def on_sync_mirror(data):
  logging.debug('on_sync_mirror')
  origin = 'dcs'
  mirror = 'gcs'
  tid = data['tid']
  project = data['project']
  ...
  if remove:
    remove_option = '--remove'
  # _create_folder_if_not_exist(project)
  command = 'mc mirror --quiet --overwrite %s %s/%s %s/%s' % (
    remove_option, origin, project, mirror, project)
  proc = subprocess.Popen([command], stdout=subprocess.PIPE, shell=True)
  (out, err) = proc.communicate()
  ...
  if not success:
    logging.warning(out)
    logging.warning(err)
  log_to_center(tid, title='sync_mirror', msg=msg, success=success)
