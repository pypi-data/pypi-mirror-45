...
class Tree():
    ...
    def get_node_info(self, all_node):
        ...
        self.root.record_count = self.all_node_list[0][4]
        ...
        if root.value == -2:
            if len(self.classes) > 1:
                ...
                if self.classes[0].__class__.__name__ == 'int':
                    prob[self.classes.index(int(root.score))] = float(root.record_count)
                else:
                    prob[self.classes.index(root.score)] = float(root.record_count)
            else:
    ...
    def build_tree(self):
        ...
        for nd in self.all_node_list:
            if nd[2] == cur_node.value:
                if nd[3] == 'lessOrEqual' or nd[3] == 'lessThan':
                    cur_node.left = self.Node()
                    if nd[0] != -2:
                        cur_node.left.score = nd[0]
                        cur_node.left.record_count = nd[4]
                    else:
                    ...
                    if nd[0] != -2:
                        ...
                        cur_node.right.record_count = nd[4]
