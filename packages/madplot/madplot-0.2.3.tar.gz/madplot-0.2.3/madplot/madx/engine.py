...
class MADXSession:
    ...
    def run(self, commands: Sequence[str] = (), results: Sequence[str] = (), buffer_time=0.05, timeout=1):
        ...
        for command in commands:
            ...
            self.process.stdin.flush()
        # Execute sentry command which creates a file on the disk. We then check for existence of that file
        # in order to make sure all previous commands have been executed.
        command = (
        ...
        if not os.path.exists(sentry_name):
            raise FileNotFoundError(f'Not all requested files exist: {paths}')
        os.remove(sentry_name)
        return list(map(lambda x: Convert.by_file_name(x[0], **x[1]), results))
