...
cdef class MdApiWrapper:
  ...
  def Init_Base(self):
    bInit=-1
    try:
      self._api = CreateFtdcMdApi(self.pszFlowPath.encode(), self.bIsUsingUdp, self.bIsMulticast)
      if not self._api:
          raise MemoryError()
  ...
  def Init_Net(self):
    try:
      ...
      if self._api is not NULL and self._spi is not NULL:
        ...
        logDes="step(RegisterSpi) 1(of 3 step)"
        ...
        logDes="step(RegisterFront) 2(of 3 step)"
        myEnv.logger.info(logDes)
      ...
      myEnv.logger.error(funName, exc_info=True)
    ...
    try:      
      ...
      if self.Md_Rule == 1:
        ...
        if len(self.Md_Trading_Day) >0 and len(myDEnv.my_instrument_id) > 0:
          ...
          myEnv.logger.info(l_sql)
          l_MDs = self.PTP_Algos.ExecuteQuery(l_sql);
