...
class BoxCoxScaler(pasoModel, TransformerMixin):
    """
    ...
    def train(self, X,inplace=False):
        ...
        """
        #todo: transform dataframe to numpy and bavk?
        ...
        if not np.asarray(X).dtype in [np.float, np.int]:
            ...
            raise ValueError(
                "BoxCox fit does not work on any X array elements that are a negative value."
            )
        ...
        if len(X.shape) == 1:
            ...
            raise ValueError(
                "Passed X array elements must be  1-d or 2-d list or numpt array or pandas DataFrame."
            )
    ...
    def predict(self, X,inplace=False, **kwargs):
        ...
        """
        #todo: insure train before predict check
...
class LambertScaler(pasoModel, TransformerMixin):
    """
...
class scaler(pasoModel):
    ...
    def train(self, Xarg,inplace=False):
        """
        ...
        if is_DataFrame(Xarg):
            ...
            raise pasoError('scale:predict: Xarg must be if type DataFrame. Was type:{}', format(type(Xarg)))
            # cached . dont'caclulate again
        if self.cache and self.transformed:
            return (self.f_x)
        else:
            pass
        if inplace:
            ...
            Xdf = Xarg.copy()
        _Check_No_NA_Values(Xdf)

        X = Xdf.values   #now numpy array
        logger.debug("FIT paso_Scaler")
        self.model = self.encoder.fit(X)
        self.trained = True
        return self

    def predict(self, Xarg,inplace=False, **kwargs):
        """
        Parameters:
            Xarg:  pandas dataFrame #Todo:Dask numpy
        ...
        if self.trained:
            self.f_x = self.encoder.predict(X)
            self.predicted = True
            self.f_x = toDataFrame().transform(self.f_x,labels=Xarg.columns,inplace=False)
            if self.verbose: logger.info("Scaler:{}".format(self.encoderKey))
            return self.f_x
        else:
            raise pasoError('scale:predict: must call train before predict')
    ...
    def inverse_predict(self, Xarg,inplace=False, **kwargs):
        ...
        X = Xarg.values   #now numpy array

        logger.debug("inwerse TRANSFORM ")
        if trained and predicted:
            ...
            return toDataFrame().transform(self.f_x,labels=Xarg.columns,inplace=False)
        else:
