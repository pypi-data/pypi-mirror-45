...
@click.option('--link-gcp/--no-gcp', required=False, default=True, help='Similar to AWS, GCP also has its default directory and this flag allows the folder to be mounted in your jobs and Resource Manager to call GCP APIs. Defaults to `--link-gcp`.')
@click.option('--link-azure/--no-azure', required=False, default=True, help='Similar to AWS, Azure also has its default directory and this flag allows the folder to be mounted in your jobs and Resource Manager to call Azure APIs. Defaults to `--link-azure`.')
...
def apply_aws_template(
        ...
        rm_config_volume, rm_container_name, ml_backend, cache_path):
    ...
    run_docker(ctx, ml_backend, ml_data, prefix, rm_config_volume, rm_container_name, rm_manager_image,
               rm_socket_server, ssh_key, token, None, cache_path)
...
def apply_azure_template(
    ...
    run_docker(ctx, ml_backend, ml_data, prefix, rm_config_volume, rm_container_name, rm_manager_image,
               rm_socket_server, ssh_key, token, env={'ML_INSTANCE_ROLE': role_id}, cache_path=cache_path)
...
@click.pass_context
def manage_queue(ctx, org=None, create=None, name=None, **kwargs):
    ...
    if create:
        ...
        result = ApiCaller.call(ctx.obj, ctx.obj.session, 'post', url, updates)
    elif updates:
        result = ApiCaller.call(ctx.obj, ctx.obj.session, 'put', url, updates)
    else:
