        ...
        with open(file) as grepfile:
            for line in grepfile:
                if re_pattern.search(line)  is not None:
                    lines.append(line)
    except (IsADirectoryError, FileNotFoundError, UnicodeDecodeError, PermissionError):
        pass
    return lines


def print_result(result, re_pattern, count, color, print_filename):
    file, future = result
    prefix = f'{file}:' if print_filename else ''
    if count:
        matched_lines = len(future.result())
        sys.stdout.write(f'{prefix}{matched_lines}\n')
        return
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    sys.stdout.write(f'mgrep {__version__}\n')
    ctx.exit()
...
@click.option('--color', type=click.Choice(['always', 'never', 'auto']), default="auto")
@click.argument('pattern')
@click.argument('paths', nargs=-1, type=PathlibPath())
def cli(recursive, ignore_case, count, color, pattern, paths):
    ...
    for result in results:
        print_result(result, re_pattern, count, color, print_filename)
