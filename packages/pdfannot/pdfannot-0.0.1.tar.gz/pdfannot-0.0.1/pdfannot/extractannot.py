...
def extract_word_from_highlight(annot, words, debug=False):
    ...
    highlight annotations which contents are named specifically ('same_content'+ -/- 'next_integer'),
    ...
    if len(annot.vertices) > 4:
        ...
        if annot.next and annot.next.info['content']:
            content_next = annot.next.info['content']
            print(annot.next.info) if debug else 0

            while annot.next and content_next[len(content_next) - 1].isdigit() and int(
                    content_next[len(content_next) - 1]) > 1:
                annot = annot.next
                mywords += [w for w in words if
                            annot.rect.intersect(fitz.Rect(w[:4])).getRectArea() / fitz.Rect(w[:4]).getRectArea() > 0.6]
                if annot.next:
...
def pdfannot2df(input_pdf, debug=False):
    ...
    for ixpage, page in enumerate(pdf):
        ...
        while annot:
            ...
            if annot.type[1] == 'Highlight':
                mywords, annot = extract_word_from_highlight(annot, words)
            ...
            annot_text = " ".join(w[4] for y1, gwords in group for w in gwords)
            ...
            tmp.update({'x': int(annot.rect[0]), 'y': int(annot.rect[1]),
                        ...
                        'annot_type': annot.type[1], 'content': annot.info['content'], 'order': order,
    ...
    if adf[adf['annot_type'].isnull()].shape[0]:
        raise Exception(f'Missing {adf[adf["annot_type"].isnull()].shape[0]} type annotation(s) in {input_pdf}')

    final_columns = ['order', 'page', 'x', 'y', 'w', 'h', 'annot_type', 'content', 'page_height', 'page_width',
                     'pdf_path', 'annot_text']
...
if __name__ == '__main__':
    ...
    input_pdf = RESSOURCES_DIR + '/pdf_test_annot_3.pdf-marked'
    pdfannot2df(input_pdf).to_excel(RESSOURCES_DIR + '/pdf_annot_3_output.xlsx')
