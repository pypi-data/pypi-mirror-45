...
def add_annotation(annot_text, content, page, annot_type, debug=False):
    ...
    rl = page.searchFor(annot_text, hit_max=200)
    ...
    if rl:  # If there's a match
        if annot_type == 'Square':
            ...
            annot.setInfo({'content': f'{content}', 'name': '', 'title': '', 'creationDate': '', 'modDate': '',
                           'subject': ''})
            annot.setColors({"stroke": gen_color(content)})  # annot.setColors({"stroke":(0, 0, 1), "fill":(0.75, 0.8, 0.95)}) (Red, Yellow, Blue)
        ...
        print(f'WARNING: NotFound: text to annotate is not found in page {page}')
...
def annotate_pdf(adf, pdf_path, dest_pdf_path, debug=False):
    ...
    for index, row in adf.iterrows():
        ...
        if ('page' in adf.columns) and row['page'] :
            add_annotation(row['annot_text'], row['content'], pdf[row['page'] - 1], row['annot_type'])
            print('test') if debug else 0
        else:
            for page in pdf:
                print(page) if debug else 0
                add_annotation(row['annot_text'], row['content'], page, row['annot_type'])
...
def df_to_adf(df):
    """Transforms a Dataframe so that it match genannot requirements
    ...
    for c in df.columns:
        ...
        adf1 = adf1[['page', c]]
        adf1.rename(columns={c: 'annot_text'}, inplace=True)
        adf1['content'] = c[6:]
        adf1['annot_type'] = "Highlight"

        frames.append(adf1)

    adf = pd.concat(frames)
    adf['annot_text'] = adf['annot_text'].astype(str)
