...
def add_annotation_from_text(annot_text, content, page, annot_type, debug=False):
...
def annotate_pdf(adf, pdf_path, dest_pdf_path, debug=False):
    ...
    for index, row in adf.iterrows():
        ...
        if ('page' in adf.columns) and row['page']:
            if 'annot_text' in adf.columns :
                add_annotation_from_text(row['annot_text'], row['content'], pdf[row['page'] - 1], row['annot_type'])
            ...
            for page in pdf:
                ...
                add_annotation_from_text(row['annot_text'], row['content'], page, row['annot_type'])
...
def df_to_adf(df):
    ...
    adf.drop_duplicates(inplace=True, subset = ['annot_text', 'page'])

    for index,row in adf.iterrows() :
        if ' ;; ' in row['annot_text'] :
            l = row['annot_text'].split(' ;; ')
            for annot in l :
                new_row = row
                new_row['annot_text'] = annot
