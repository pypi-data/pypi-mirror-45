...
def write_para(options,configfile):
    ...
    Config.set('Vis_learn','lr_tsne',str(options.lr_tsne))
    Config.set('Vis_learn','label_tsne',options.label_tsne)
    Config.set('Vis_learn','iter_tsne',str(options.iter_tsne))
    Config.set('Vis_learn','ini_tsne',str(options.ini_tsne))
...
def validation_para(options):
    ...
    if not (options.type_emb in ['raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf']):
        print  ('you entered --type_emb=' + str(options.type_emb))
        print  (utils.textcolor_display('--type_emb') + " only supports 'raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf'")
        valid = 0
...
def read_data (options, suffix = '', included_labels = 'y' , get_names_of_features = 'n', mode='internal') :
    ...
    if mode == 'internal':        
        path_v_set_x = os.path.join(options.original_data_folder , options.data_dir_img + '_' +  'x.csv')   
    elif mode == 'external':        
        if options.ext_data == '':
            #print  ('with the same pattern name with internal set'
            path_v_set_x = os.path.join(options.original_data_folder , options.data_dir_img + str(suffix) + '_' +  'x.csv') 
        else:
    ...
    if included_labels in ['y']:
        if mode == 'internal':            
            path_v_set_y = os.path.join(options.original_data_folder , options.data_dir_img + '_' +  'y.csv')   
        elif mode == 'external':                
            if options.ext_data == '':
                #print  ('with the same pattern name with internal set'
                path_v_set_y = os.path.join(options.original_data_folder , options.data_dir_img + str(suffix) + '_' +  'y.csv') 
            else:
                #print  ('with the different pattern name with internal set'
...
def visual_section (options, path_write_parentfolder_img, seed_value, index,train_x,val_x, train_y, val_y, v_data, v_labels,train_x_original,
    ...
    if options.type_emb in ['fills','zfills'] and options.type_data != 'pr': #fills: use fill-up with bins created from train-set with scale, should be used with type_data='eqw'
        ...
        if (not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png")) or options.recreate_img > 0 :
            ...
            if options.orderf_fill == 'high':  
                ...
                vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = [x for _,x in sorted(zip(mean_spe,mean_spe))], type_data=options.type_data,
                    name_file= path_write+"/global.png", fig_size = fig_v_size, 
                    min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                    size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor, 
                    colormap = options.colormap, cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)
                ...
                for i in range(0, len(train_y)):
                    ...
                    #print (train_high)
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,train_x[i]))]  , type_data=options.type_data,
                ...
                for i in range(0, len(val_y)):
                    ...
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,val_x[i]))], type_data=options.type_data,
            ...
            if (not os.path.exists(path_write+"/testval_" + str(len(v_labels)-1) + ".png")) or options.recreate_img > 0 :
                if options.orderf_fill == 'high':  
                    ...
                    for i in range(0, len(v_labels)):
                        ...
                        vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,v_data[i]))], type_data=options.type_data,
