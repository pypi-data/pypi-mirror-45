...
def embed_image(X_embedded, X, file_name_emb='test', size_p=1, fig_size=4, type_bin='spb', 
    ...
    #binning 
    if setcolor=="gray":
        color_arr = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin) for y in X ]   
    else:
        if colormap in ['','custom']:
            if type_bin=="eqw" and num_bin==10: #if forget to set auto_v = 1, then generating black/white/gray images
                color_arr = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin, color_img =True) for y in X ]  
            else:
                color_arr = [convert_color_real_img(y, debug=0, type=type_bin) for y in  X ]   
        else:
            color_arr = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin) for y in X ]           
    ...
    fig.savefig(file_name_emb+'.png') ##True to see "not available area (unused area)"    
    plt.close('all')
  
def fillup_image(X, fig_size, file_name='test', size_p=1, cor_x=0,cor_y=0, 
        marker_p=',', type_bin='spb', setcolor="color", 
        max_v=0.0065536, min_v= 1e-07, num_bin = 10, dpi_v = 75, 
        alpha_v=0.5, colormap='',cmap_vmin=0.0, cmap_vmax=1.0, show = 0):
    ...
    if setcolor=="gray":
        colors = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin) for y in X ]   
        #print 'use gray'
        #print X.shape
    else:
        if colormap in ['','custom']:
            if (type_bin=="eqw" and num_bin == 10):  #if use color, eqw: 10 distinct color
                colors = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin, color_img =True) for y in X ]  
            else:
                colors = [convert_color_real_img(y, debug=0, type=type_bin) for y in  X ]
        else:
            #print 'use colormap'
            #colors = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin) for y in X ]   
            colors = [convert_bin(y, debug=0, type=type_bin, min_v = min_v, max_v = max_v, num_bin = num_bin) for y in X ] 
    ...
    if setcolor=="gray":                     
        ...
        if colormap in ['','custom']: #if use predefined color
