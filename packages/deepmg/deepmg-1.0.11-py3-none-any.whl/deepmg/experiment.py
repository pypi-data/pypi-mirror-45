...
def para_cmd ():
    ...
    parser.add_option('--ext_data',help="external data, default: ")
    ...
    parser.add_option('--model',help='model name for learning vgglike/model_lstm/resnet50/rf_model/gbc_model/svm_model/knn_model/none/)')
    ...
    parser.add_option('--rf_max_features',help='The number of max features in the forest (default:-2: auto, -1: None)')
    ...
    parser.add_option('--imp_fea',help='using important features for overlapped issues (default:none)')
    ...
    parser.add_option('-y','--type_bin',help='type of binnings: species-bins with log4(ab)/eqw/presence(pr) (default:ab)')
...
def validation_para(options):
    ...
    if (options.ext_data not in [''] and options.test_exte in ['n']):
        print  ('you entered --ext_data=' + str(options.ext_data) + ' and test_exte=' + str(options.test_exte))
        print  (utils.textcolor_display('--ext_data') + " is used if " + utils.textcolor_display('--test_exte')+ "='y'")
        valid = 0
    ...
    if not (options.scale_mode in ['mms','none','','qtf','qtnf','log2','ln','ptf']):
        print  ('you entered --scale_mode=' + str(options.scale_mode))
        print  (utils.textcolor_display('--scale_mode') + " only supports 'mms' (minmaxscaler),'none','','qtf'(quantiletransformer-uniform distribution),'qtnf'(quantiletransformer-normal),'log2','ln','ptf")
        valid = 0
    ...
    if not (options.type_bin in ['spb','pr','eqw']):
        print  ('you entered --type_bin=' + str(options.type_bin))
        print  (utils.textcolor_display('--type_bin') + " only supports 'spb','pr','eqw'")
        valid = 0
     

    if not (options.model in ['svm_model','rf_model','gbc_model','knn_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d']):
        print  ('you entered --model=' + str(options.model))
        print  (utils.textcolor_display('--model') + " only supports 'svm_model','rf_model','gbc_model','knn_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d'")
        valid = 0
...
def naming_folder (options, search_res = 'y'):
    ...
    if not(options.orderf_fill in ["none"]): #set name for features ordered randomly
        if options.type_emb in ['raw','fill','fills','zfill','zfills']:
            ...
            print  ("use of features ordered only supports on raw','fill','fills', 'zfill','zfills'")
            exit()
    ...
    if options.type_emb == 'raw':      #raw data  
        ...
        if options.shape_drawn == ',': #',' means pixel in the graph
            shape_draw='pix'
        pre_name = pre_name + '_' + str(options.type_emb) + '_' + shape_draw + '_r' + str(options.fig_size) + 'p'+ str(options.point_size) + options.type_bin + "m" + str(options.margin) +'a'+str(options.alpha_v) + options.setcolor +options.colormap  + 'bi' + str(options.num_bin)+'_'+str(options.min_v) + '_'+str(options.max_v)
        ...
        if options.type_emb in  ['fills','zfills']:            
            pre_name = pre_name+'_'+str(options.type_emb)  + str(shape_draw) + '_r' + str(options.fig_size) + 'p'+ str(options.point_size) + options.type_bin + "m" + str(options.margin) +'a'+str(options.alpha_v) + str(options.setcolor) + str(options.colormap)
        else: #others: tsne, lle, pca, lda ...            
            if options.imp_fea in ['rf']:  #if use RF to arrange important features to a front position
            ...
            if options.type_emb == 'lda':
                ...
                if options.label_visual == "": #naming for other manifolds
                    pre_name = pre_name+'_'+str(options.type_emb) +str(options.emb_data) + insert_named + '_p'+str(perplexity_x) + 'l'+str(learning_rate_x) + 'i'+str(n_iter_x) + shape_draw + '_r' + str(options.fig_size) + 'p'+ str(options.point_size) + options.type_bin + "m" + str(options.margin) +'a'+str(options.alpha_v) + str(options.setcolor) + str(options.colormap)
                else: 
                    pre_name = pre_name+'_'+str(options.type_emb) +str(options.emb_data) + str(options.label_visual)+ insert_named+ '_p'+str(perplexity_x) + 'l'+str(learning_rate_x) + 'i'+str(n_iter_x) + shape_draw + '_r' + str(options.fig_size) + 'p'+ str(options.point_size) + options.type_bin + "m" + str(options.margin) +'a'+str(options.alpha_v) + str(options.setcolor)  + str(options.colormap)
...
def create_fillup_folder (options,path_write_parentfolder_img,
    ...
    if not os.path.exists(path_write_parentfolder_img+'/'+prefix_img + '_' + str(len(labels)-1)+'.png') or options.recreate_img > 0 :
        ...
        vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = mean_spe, type_bin=options.type_bin,
                file_name= path_write_parentfolder_img+"/global"+prefix_img, fig_size = fig_v_size, 
        ...
        for i in range(0, len(labels)):
            ...
            vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = data[i], type_bin=options.type_bin,
                file_name= path_write_parentfolder_img+"/"+prefix_img+"_"+str(i), fig_size = fig_v_size, 
                min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor, 
                colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)
    ...
    if options.type_emb in ['fills','zfills'] and options.type_bin != 'pr': #fills: use fill-up with bins created from train-set with scale, should be used with type_bin='eqw'
        ...
        if (not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png")) or options.recreate_img > 0 :
            ...
            if options.orderf_fill == 'high':  
                ...
                vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y, X = [x for _,x in sorted(zip(mean_spe,mean_spe))], type_bin=options.type_bin,
                    file_name= path_write+"/global.png", fig_size = fig_v_size, 
                    min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                    size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor, 
                    colormap = options.colormap, cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)
                ...
                for i in range(0, len(train_y)):
                    ...
                    #print (train_high)
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,train_x[i]))]  , type_bin=options.type_bin,
                        file_name= path_write+"/train_"+str(i), fig_size = fig_v_size, 
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                        size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                        colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)
                ...
                for i in range(0, len(val_y)):
                    ...
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,val_x[i]))], type_bin=options.type_bin,
                        file_name= path_write+"/val_"+str(i), fig_size = fig_v_size, 
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                        size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                        colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)   
                ...
                for i in range(0, len(train_y)):
                    ...
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = train_x[i], type_bin=options.type_bin,
                        file_name= path_write+"/train_"+str(i), fig_size = fig_v_size, 
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                        size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                        colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)
                ...
                for i in range(0, len(val_y)):
                    ...
                    vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = val_x[i], type_bin=options.type_bin,
                        file_name= path_write+"/val_"+str(i), fig_size = fig_v_size, 
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                        size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                        colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)   
            ...
            if (not os.path.exists(path_write+"/testval_" + str(len(v_labels)-1) + ".png")) or options.recreate_img > 0 :
                if options.orderf_fill == 'high':  
                    ...
                    for i in range(0, len(v_labels)):
                        ...
                        vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = [x for _,x in sorted(zip(mean_spe,v_data[i]))], type_bin=options.type_bin,
                            file_name= path_write+"/testval_"+str(i), fig_size = fig_v_size, 
                            min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                            size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                            colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)   
                else:
                    for i in range(0, len(v_labels)):
                        #print  ('img fillup testval ' + str(i)            
                        vis_data.fillup_image(cor_x=cordi_x,cor_y=cordi_y,X = v_data[i], type_bin=options.type_bin,
                            file_name= path_write+"/testval_"+str(i), fig_size = fig_v_size, 
                            min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, alpha_v = options.alpha_v,
                            size_p =options.point_size, marker_p = options.shape_drawn, setcolor = options.setcolor , 
                            colormap = options.colormap,cmap_vmin = options.cmap_vmin,cmap_vmax = options.cmap_vmax)   
        ...
        if not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png") or options.recreate_img > 0 : 
            ...
            if options.type_emb =='tsne':
                ...
                som = MiniSom(options.fig_size,options.fig_size,input_x.shape[1],sigma=options.perlexity_neighbor,learning_rate=options.lr_visual)                
            ...
            if options.imp_fea in ['rf']: 
                ...
                vis_data.embed_image(X_embedded = X_embedded [indices_imp], X = mean_spe[indices_imp], file_name_emb = path_write+"/global",
                    marker=options.shape_drawn,size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                ...
                for i in range(0, len(train_x)):   
                    ...
                    vis_data.embed_image(X_embedded = X_embedded [indices_imp], X = train_x[i,indices_imp], file_name_emb = path_write+"/train_"+str(i),
                        marker=options.shape_drawn,size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                        margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                        min_v = min_v_temp, max_v= max_v_temp, num_bin = options.num_bin,
                        cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                ...
                for i in range(0, len(val_x)):                
                    vis_data.embed_image(X_embedded = X_embedded [indices_imp],X = val_x [i,indices_imp],file_name_emb = path_write+"/val_"+str(i),
                        marker=options.shape_drawn, size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                        margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, 
                        cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                ...
                vis_data.embed_image(X_embedded = X_embedded, X = mean_spe, file_name_emb = str(path_write) + "/global",
                    marker=options.shape_drawn,size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                    margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor,  colormap = options.colormap,
                    min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin, 
                    cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                ...
                #TRAIN: use X_embedded to create images; save labels
                for i in range(0, len(train_x)):                       
                    vis_data.embed_image(X_embedded = X_embedded, X = train_x [i,:],file_name_emb =path_write + "/train_"+str(i),
                        marker=options.shape_drawn,size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                        margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                        min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin,
                        cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
                ...
                #TEST: use X_embedded to create images; save labels               
                for i in range(0, len(val_x)):                
                    vis_data.embed_image(X_embedded = X_embedded,X = val_x [i,:],file_name_emb = path_write+"/val_"+str(i),
                        marker=options.shape_drawn, size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                        margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
                        min_v = min_v_temp, max_v=max_v_temp, num_bin = options.num_bin, 
                        cmap_vmin = options.cmap_vmin, cmap_vmax= options.cmap_vmax)
        ...
        if options.test_exte == 'y':
            if not os.path.exists(path_write+"/testval_" + str(len(v_labels)-1) + ".png") or options.recreate_img > 0 : 
                for i in range(0, len(v_data)):
                    ...
                    vis_data.embed_image(X_embedded = X_embedded,X = v_data [i,:],file_name_emb = path_write+"/testval_"+str(i),
                        marker=options.shape_drawn, size_p= options.point_size, fig_size = fig_v_size, type_bin= options.type_bin,
                        margin = options.margin, alpha_v=options.alpha_v, setcolor = options.setcolor, colormap = options.colormap,
...
def scaler_section (options,prefix_file_log,seed_value,index,train_x,val_x,v_data):
    ...
    '''
    if options.type_emb in ['fill','zfill']:
        print  ('options.type_emb=' + options.type_emb + ' is not supported!! for options.scale_mode=' +str(options.scale_mode) + '. Did you want to select fills/zfills?')
        exit()              

    print  ('before scale:' + str(np.max(train_x)) + '_' + str(np.min(train_x)) + '_' + str(np.max(val_x)) + '_'  + str(np.min(val_x))  )    

    if  options.scale_mode in ['log2','ln']:
        print('log2 and ln')
        ...
        if  options.scale_mode in ['minmaxscaler' , 'mms'] :       
            ...
            print('this scaler ' + options.scale_mode + ' is not available')   
            exit()        

        #use training set to learn and apply to validation set 
        train_x = scaler.fit_transform(train_x)
        val_x = scaler.transform(val_x)     
        if options.test_exte == 'y':      #use whole train+val to learn, then create for v_data
...
def run_kfold_deepmg(options,args):
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.run_time) :
        ...
        for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
            ...
            if options.scale_mode in ['mms','qtf','log2','ln','ptf','qtnf'] :
            ...
            if options.type_emb == "raw" or options.type_emb == "bin" : #reshape raw data into a sequence for ltsm model. 
                ...
                if options.type_emb == "bin" : 
                    ...
                    for i in range(0, len(train_x)):   
                        temp_train_x.append( [vis_data.convert_bin(value=y,  type=options.type_bin, min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin) for y in train_x[i] ])
                    for i in range(0, len(val_x)):   
                        temp_val_x.append( [vis_data.convert_bin(value=y,  type=options.type_bin, min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin) for y in val_x[i] ])
                    ...
                    if options.test_exte == 'y':     
                        for i in range(0, len(v_data)):   
                            temp_v_data.append( [vis_data.convert_bin(value=y, type=options.type_bin, min_v = min_v_temp, max_v = max_v_temp, num_bin = options.num_bin) for y in v_data[i] ])
                ...
                elif options.model in [ 'svm_model','rf_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
            ...
            if options.type_run  in ['vis','visual']:  
                ...
                if options.model in ['svm_model', 'rf_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    #print  (train_x.shape      )     
                    #print  ('run classic learning algorithms'  )
                    
                    if np.isnan(train_x.any()):
                        print  ('data contain NaN. Please check data again')
                        exit()
                    elif np.isfinite(train_x.all()):
                        print  ('data all are finite ')
                    else: 
                        print  ('data contain infitite value. Please check data again')
                        exit()
            ...
            if options.type_run in ['vis','visual']: #only visual, not learning
                ...
                if options.save_w == 'y' : #save weights        
                    ...
                    if options.model in [ 'svm_model','rf_model','gbc_model','knn_model']:
                        import pickle
...
def deepmg_visual(options,args):
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.run_time) :
        ...
        for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
           ...
           ##### scale mode, use for creating images/bins
            if options.scale_mode in ['ptf' , 'mms','qtf','log2','ln', 'qtnf'] :
                train_x,val_x,v_data = scaler_section (options,prefix_file_log,seed_value,index,train_x,val_x,v_data)              
            elif options.scale_mode in ['none','']:
                print  ('do not use scaler')
            else:
...
def run_holdout_deepmg(options,args, special_usecase = '', txt_time_pre =''):
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.whole_run_time) :
        ...
        for index in range(0,1):
            ...
            ##### scale mode, use for creating images/bins
            if options.scale_mode in ['minmaxscaler','quantiletransformer', 'mms','qtf','log2','ln','qtnf'] :
                train_x,val_x,v_data = scaler_section (options,prefix_file_log,seed_value,index,train_x,val_x,v_data)              
            elif options.scale_mode in ['none','']:
                print  ('do not use scaler')
            else:
