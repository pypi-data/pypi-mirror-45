...
from sklearn.metrics import roc_auc_score,accuracy_score,f1_score,precision_score,recall_score,matthews_corrcoef, confusion_matrix, log_loss, matthews_corrcoef
...
def para_cmd ():
    ...
    parser.add_option('--time_run',help='give the #runs (default:1)')
    #parser.add_option('--seed_ml',help='run seed for machine learning (default:n)')
    parser.add_option('--time_whole_run',help='give the #runs (default:1)')
...
def get_default_value (options):
    ...
    #        options.seed_ml = 'n'
    if not options.time_whole_run:  #if did not set from cmd, use default value
            options.time_whole_run = '1'
...
def para_config_file (options):
    ...
    #    else: #other cases, use default value
    #        options.seed_ml = 'n'
    if not options.time_whole_run:  #if did not set from cmd
        if config.has_option('Learning','time_whole_run'): #if avaiable from config file
            options.time_whole_run = config.get('Learning','time_whole_run')
        else: #other cases, use default value
            options.time_whole_run = '1'
...
def string_to_numeric(options):
    ...
    options.time_whole_run =  int(options.time_whole_run)
...
def write_para(options,configfile):
    ...
    Config.set('Learning','time_whole_run',options.time_whole_run)
    ...
    Config.set('model','rf_max_features',options.rf_max_features)
    Config.set('model','rf_max_depth',options.rf_max_depth)
...
def validation_para(options):
    ...
    if not (options.model in ['svm_model','rf_model','gbc_model','knn_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d']):
        ...
        print utils.textcolor_display('--model') + " only supports 'svm_model','rf_model','gbc_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d'"
        valid = 0
...
def name_log_final(arg, n_time_text):
    ...
    if arg.time_whole_run <> 1: # add #times of the experiment
        prefix = 'all'+str(arg.time_whole_run)+'_' + prefix
...
def run_holdout_deepmg(options,args, special_usecase = '', txt_time_pre =''):
    ...
    # ===================== BEGIN: run each run ================================================
    # ==========================================================================================
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.time_whole_run) :
        print "We're on seed %d" % (seed_value)    
        np.random.seed(seed_value) # for reproducibility    
        if options.type_run not in ['vis','visual']:   
            tf.set_random_seed(seed_value)
