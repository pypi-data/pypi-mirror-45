# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: fast.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf.internal import enum_type_wrapper
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='fast.proto',
  package='fast',
  syntax='proto3',
  serialized_options=None,
  serialized_pb=_b('\n\nfast.proto\x12\x04\x66\x61st\"\x93\x08\n\x07\x45lement\x12%\n\nsrcml_kind\x18\x01 \x01(\x0e\x32\x0f.fast.SrcmlKindH\x00\x12%\n\nsmali_kind\x18\x02 \x01(\x0e\x32\x0f.fast.SmaliKindH\x00\x12)\n\x0cpython3_kind\x18\x03 \x01(\x0e\x32\x11.fast.Python3KindH\x00\x12+\n\rsolidity_kind\x18\x04 \x01(\x0e\x32\x12.fast.SolidityKindH\x00\x12)\n\x0c\x63obol85_kind\x18\x05 \x01(\x0e\x32\x11.fast.Cobol85KindH\x00\x12\x0c\n\x04text\x18\x06 \x01(\x0c\x12\x0b\n\x03pos\x18\x07 \x01(\x05\x12\x0e\n\x06length\x18\x08 \x01(\x05\x12\x1c\n\x05\x63hild\x18\t \x03(\x0b\x32\r.fast.Element\x12\x0c\n\x04tail\x18\n \x01(\x0c\x12\"\n\x04unit\x18\x0b \x01(\x0b\x32\x12.fast.Element.UnitH\x01\x12(\n\x07literal\x18\x0c \x01(\x0b\x32\x15.fast.Element.LiteralH\x01\x12\x0c\n\x04line\x18\r \x01(\x05\x12\x0e\n\x06\x63olumn\x18\x0e \x01(\x05\x12\r\n\x05label\x18\x0f \x01(\x02\x12&\n\x06\x63hange\x18\x10 \x01(\x0e\x32\x16.fast.Element.DiffType\x12\x0c\n\x04keep\x18\x11 \x01(\x08\x12\n\n\x02id\x18\x12 \x01(\x05\x1a\x90\x02\n\x04Unit\x12\x10\n\x08\x66ilename\x18\x01 \x01(\t\x12\x10\n\x08revision\x18\x02 \x01(\t\x12\x31\n\x08language\x18\x03 \x01(\x0e\x32\x1f.fast.Element.Unit.LanguageType\x12\x0c\n\x04item\x18\x04 \x01(\x05\"\xa2\x01\n\x0cLanguageType\x12\t\n\x05\x44UMMY\x10\x00\x12\x07\n\x03\x41LL\x10\x01\x12\x06\n\x02OO\x10\x02\x12\x07\n\x03\x43XX\x10\x03\x12\x05\n\x01\x43\x10\x04\x12\x0c\n\x08\x43_FAMILY\x10\x05\x12\x08\n\x04JAVA\x10\x06\x12\n\n\x06\x43SHARP\x10\x07\x12\x0f\n\x0bOBJECTIVE_C\x10\x08\x12\t\n\x05SMALI\x10\t\x12\x0c\n\x08SOLIDITY\x10\n\x12\x0b\n\x07\x43OBOL85\x10\x0b\x12\x0b\n\x07PYTHON3\x10\x0c\x1a\xab\x01\n\x07Literal\x12/\n\x04type\x18\x01 \x01(\x0e\x32!.fast.Element.Literal.LiteralType\"o\n\x0bLiteralType\x12\x0e\n\ndummy_type\x10\x00\x12\x0f\n\x0bnumber_type\x10\x01\x12\r\n\tchar_type\x10\x02\x12\x0f\n\x0bstring_type\x10\x03\x12\x10\n\x0c\x62oolean_type\x10\x04\x12\r\n\tnull_type\x10\x05\"Q\n\x08\x44iffType\x12\x0b\n\x07MATCHED\x10\x00\x12\t\n\x05\x41\x44\x44\x45\x44\x10\x01\x12\x0b\n\x07\x44\x45LETED\x10\x02\x12\x10\n\x0c\x43HANGED_FROM\x10\x03\x12\x0e\n\nCHANGED_TO\x10\x04\x42\x06\n\x04typeB\x07\n\x05\x65xtra\"\xc4\x04\n\x05\x44\x65lta\x12\x0b\n\x03src\x18\x01 \x01(\t\x12\x0b\n\x03\x64st\x18\x02 \x01(\t\x12\x1e\n\x04\x64iff\x18\x03 \x03(\x0b\x32\x10.fast.Delta.Diff\x1a\x80\x04\n\x04\x44iff\x12(\n\x04type\x18\x01 \x01(\x0e\x32\x1a.fast.Delta.Diff.DeltaType\x12\'\n\x05match\x18\x02 \x01(\x0b\x32\x16.fast.Delta.Diff.MatchH\x00\x12#\n\x03\x61\x64\x64\x18\x03 \x01(\x0b\x32\x14.fast.Delta.Diff.AddH\x00\x12#\n\x03\x64\x65l\x18\x04 \x01(\x0b\x32\x14.fast.Delta.Diff.DelH\x00\x12%\n\x04move\x18\x05 \x01(\x0b\x32\x15.fast.Delta.Diff.MoveH\x00\x12)\n\x06update\x18\x06 \x01(\x0b\x32\x17.fast.Delta.Diff.UpdateH\x00\x1a!\n\x05Match\x12\x0b\n\x03src\x18\x01 \x01(\x05\x12\x0b\n\x03\x64st\x18\x02 \x01(\x05\x1a\x31\n\x03\x41\x64\x64\x12\x0b\n\x03src\x18\x01 \x01(\x05\x12\x0b\n\x03\x64st\x18\x02 \x01(\x05\x12\x10\n\x08position\x18\x03 \x01(\x05\x1a\x12\n\x03\x44\x65l\x12\x0b\n\x03src\x18\x01 \x01(\x05\x1a\x32\n\x04Move\x12\x0b\n\x03src\x18\x01 \x01(\x05\x12\x0b\n\x03\x64st\x18\x02 \x01(\x05\x12\x10\n\x08position\x18\x03 \x01(\x05\x1a\"\n\x06Update\x12\x0b\n\x03src\x18\x01 \x01(\x05\x12\x0b\n\x03\x64st\x18\x02 \x01(\x05\">\n\tDeltaType\x12\t\n\x05MATCH\x10\x00\x12\x07\n\x03\x41\x44\x44\x10\x01\x12\x07\n\x03\x44\x45L\x10\x02\x12\x08\n\x04MOVE\x10\x03\x12\n\n\x06UPDATE\x10\x04\x42\x07\n\x05\x64\x65lta\"\xa8\x03\n\x05Pairs\x12\x1e\n\x04pair\x18\x01 \x03(\x0b\x32\x10.fast.Pairs.Pair\x1a\xfe\x02\n\x04Pair\x12#\n\x04left\x18\x01 \x01(\x0b\x32\x15.fast.Pairs.Pair.Diff\x12$\n\x05right\x18\x02 \x01(\x0b\x32\x15.fast.Pairs.Pair.Diff\x12(\n\x04type\x18\x03 \x01(\x0e\x32\x1a.fast.Pairs.Pair.CloneType\x1a\xd7\x01\n\x04\x44iff\x12\x0f\n\x07project\x18\x01 \x01(\t\x12\x11\n\tleft_line\x18\x02 \x01(\x05\x12\x13\n\x0bleft_column\x18\x03 \x01(\x05\x12\x12\n\nright_line\x18\x04 \x01(\x05\x12\x14\n\x0cright_column\x18\x05 \x01(\x05\x12\x1f\n\x08old_code\x18\x06 \x01(\x0b\x32\r.fast.Element\x12\x1f\n\x08new_code\x18\x07 \x01(\x0b\x32\r.fast.Element\x12\x0c\n\x04hash\x18\x08 \x01(\t\x12\x1c\n\x06slices\x18\t \x01(\x0b\x32\x0c.fast.Slices\"\'\n\tCloneType\x12\t\n\x05MAYBE\x10\x00\x12\x07\n\x03YES\x10\x01\x12\x06\n\x02NO\x10\x02\"\xb6\x06\n\x03Log\x12 \n\x06\x63ommit\x18\x01 \x03(\x0b\x32\x10.fast.Log.Commit\x12 \n\x06\x61uthor\x18\x02 \x03(\x0b\x32\x10.fast.Log.Author\x1a\xb7\x05\n\x06\x43ommit\x12\n\n\x02id\x18\x01 \x01(\t\x12\x0b\n\x03msg\x18\x02 \x01(\x0c\x12\x13\n\x0b\x61uthor_name\x18\x03 \x01(\x0c\x12\x14\n\x0c\x61uthor_email\x18\x04 \x01(\x0c\x12\x13\n\x0b\x61uthor_time\x18\x05 \x01(\x03\x12\x16\n\x0e\x63ommitter_name\x18\x06 \x01(\x0c\x12\x17\n\x0f\x63ommitter_email\x18\x07 \x01(\x0c\x12\x16\n\x0e\x63ommitter_time\x18\x08 \x01(\x03\x12#\n\x04\x64iff\x18\t \x03(\x0b\x32\x15.fast.Log.Commit.Diff\x12\x1b\n\x05slice\x18\n \x01(\x0b\x32\x0c.fast.Slices\x1a\xc8\x03\n\x04\x44iff\x12\t\n\x01\x61\x18\x01 \x01(\t\x12\t\n\x01\x62\x18\x02 \x01(\t\x12\x0e\n\x06is_new\x18\x03 \x01(\x08\x12\x0f\n\x07is_code\x18\x04 \x01(\t\x12\x12\n\nindex_from\x18\x05 \x01(\t\x12\x10\n\x08index_to\x18\x06 \x01(\t\x12\x0c\n\x04mode\x18\x07 \x01(\t\x12(\n\x04hunk\x18\x08 \x03(\x0b\x32\x1a.fast.Log.Commit.Diff.Hunk\x1a\xaa\x02\n\x04Hunk\x12\x13\n\x0b\x66rom_lineno\x18\x01 \x01(\x05\x12\x13\n\x0b\x66rom_column\x18\x02 \x01(\x05\x12\x11\n\tto_lineno\x18\x03 \x01(\x05\x12\x11\n\tto_column\x18\x04 \x01(\x05\x12\x0f\n\x07\x63ontext\x18\x05 \x01(\t\x12\x1e\n\x07\x65lement\x18\x06 \x03(\x0b\x32\r.fast.Element\x12\x1a\n\x05graph\x18\x07 \x03(\x0b\x32\x0b.fast.Graph\x12/\n\x03mod\x18\x08 \x03(\x0b\x32\".fast.Log.Commit.Diff.Hunk.ModLine\x12\x1b\n\x05slice\x18\t \x01(\x0b\x32\x0c.fast.Slices\x1a\x37\n\x07ModLine\x12\x0c\n\x04line\x18\x01 \x01(\x0c\x12\x0e\n\x06is_add\x18\x02 \x01(\x08\x12\x0e\n\x06is_del\x18\x03 \x01(\x08\x1a\x31\n\x06\x41uthor\x12\n\n\x02id\x18\x01 \x01(\x05\x12\x0c\n\x04name\x18\x02 \x01(\x0c\x12\r\n\x05\x65mail\x18\x03 \x01(\t\"\xc9\x07\n\x06Slices\x12!\n\x05slice\x18\x01 \x03(\x0b\x32\x12.fast.Slices.Slice\x1a\x9b\x07\n\x05Slice\x12+\n\x04\x66ile\x18\x01 \x03(\x0b\x32\x1d.fast.Slices.Slice.SourceFile\x12\x0c\n\x04hash\x18\x02 \x01(\t\x1a\xa7\x06\n\nSourceFile\x12\x38\n\x08\x66unction\x18\x01 \x03(\x0b\x32&.fast.Slices.Slice.SourceFile.Function\x12\x0c\n\x04name\x18\x02 \x01(\t\x12+\n\x04type\x18\x03 \x01(\x0e\x32\x1d.fast.Slices.Slice.ChangeType\x1a\xa3\x05\n\x08\x46unction\x12\x41\n\x08variable\x18\x01 \x03(\x0b\x32/.fast.Slices.Slice.SourceFile.Function.Variable\x12\x0c\n\x04name\x18\x02 \x01(\t\x12+\n\x04type\x18\x03 \x01(\x0e\x32\x1d.fast.Slices.Slice.ChangeType\x1a\x98\x04\n\x08Variable\x12\x0c\n\x04name\x18\x01 \x01(\t\x12\x45\n\x03pos\x18\x02 \x01(\x0b\x32\x38.fast.Slices.Slice.SourceFile.Function.Variable.Position\x12+\n\x04type\x18\x03 \x01(\x0e\x32\x1d.fast.Slices.Slice.ChangeType\x12\x46\n\x04\x64\x65\x66n\x18\x04 \x03(\x0b\x32\x38.fast.Slices.Slice.SourceFile.Function.Variable.Position\x12\x45\n\x03use\x18\x05 \x03(\x0b\x32\x38.fast.Slices.Slice.SourceFile.Function.Variable.Position\x12\x0c\n\x04\x64var\x18\x06 \x03(\t\x12\r\n\x05\x61lias\x18\x07 \x03(\t\x12K\n\x05\x63\x66unc\x18\x08 \x03(\x0b\x32<.fast.Slices.Slice.SourceFile.Function.Variable.FunctionDecl\x1a\x61\n\x08Position\x12\x10\n\x08location\x18\x01 \x01(\t\x12+\n\x04type\x18\x02 \x01(\x0e\x32\x1d.fast.Slices.Slice.ChangeType\x12\x16\n\x0e\x64\x65lta_location\x18\x05 \x01(\t\x1a.\n\x0c\x46unctionDecl\x12\x0c\n\x04name\x18\x01 \x01(\t\x12\x10\n\x08location\x18\x02 \x01(\t\"-\n\nChangeType\x12\r\n\tUNCHANGED\x10\x00\x12\x07\n\x03\x41\x44\x44\x10\x01\x12\x07\n\x03\x44\x45L\x10\x02\"\xd6\x01\n\x04\x42ugs\x12\x12\n\nrepository\x18\x01 \x01(\t\x12\x1b\n\x03\x62ug\x18\x02 \x03(\x0b\x32\x0e.fast.Bugs.Bug\x1a\x9c\x01\n\x03\x42ug\x12\n\n\x02id\x18\x01 \x01(\x0c\x12\x10\n\x08opendate\x18\x02 \x01(\x0c\x12\x0f\n\x07\x66ixdate\x18\x03 \x01(\x0c\x12$\n\x07\x62uginfo\x18\x04 \x01(\x0b\x32\x13.fast.Bugs.Bug.Info\x12\x12\n\nfixed_file\x18\x05 \x03(\x0c\x1a,\n\x04Info\x12\x0f\n\x07summary\x18\x01 \x01(\x0c\x12\x13\n\x0b\x64\x65scription\x18\x02 \x01(\x0c\"\xbb\x06\n\x05Graph\x12\x1e\n\x04unit\x18\x01 \x03(\x0b\x32\x10.fast.Graph.Unit\x1a$\n\x04\x45\x64ge\x12\r\n\x05node1\x18\x01 \x01(\x05\x12\r\n\x05node2\x18\x02 \x01(\x05\x1a(\n\tNodeLabel\x12\x0c\n\x04node\x18\x01 \x01(\x05\x12\r\n\x05label\x18\x02 \x01(\t\x1a&\n\x08NodeType\x12\x0c\n\x04node\x18\x01 \x01(\x05\x12\x0c\n\x04type\x18\x02 \x01(\t\x1a\x93\x02\n\x0c\x43ontextEdges\x12#\n\tnextToken\x18\x01 \x03(\x0b\x32\x10.fast.Graph.Edge\x12\x1f\n\x05\x63hild\x18\x02 \x03(\x0b\x32\x10.fast.Graph.Edge\x12(\n\x0elastLexicalUse\x18\x03 \x03(\x0b\x32\x10.fast.Graph.Edge\x12!\n\x07lastUse\x18\x04 \x03(\x0b\x32\x10.fast.Graph.Edge\x12#\n\tlastWrite\x18\x05 \x03(\x0b\x32\x10.fast.Graph.Edge\x12#\n\treturnsTo\x18\x06 \x03(\x0b\x32\x10.fast.Graph.Edge\x12&\n\x0c\x63omputesFrom\x18\x07 \x03(\x0b\x32\x10.fast.Graph.Edge\x1a\x89\x01\n\x0c\x43ontextGraph\x12\'\n\x05\x65\x64ges\x18\x01 \x01(\x0b\x32\x18.fast.Graph.ContextEdges\x12(\n\tnodeLabel\x18\x02 \x03(\x0b\x32\x15.fast.Graph.NodeLabel\x12&\n\x08nodeType\x18\x03 \x03(\x0b\x32\x14.fast.Graph.NodeType\x1aQ\n\x0fSymbolCandidate\x12\x17\n\x0fsymbolDummyNode\x18\x01 \x01(\x05\x12\x12\n\nsymbolName\x18\x02 \x01(\t\x12\x11\n\tisCorrect\x18\x03 \x01(\x08\x1a\xa4\x01\n\x04Unit\x12\x10\n\x08\x66ilename\x18\x01 \x01(\t\x12\x14\n\x0cslotTokenIdx\x18\x02 \x01(\x05\x12\'\n\x05graph\x18\x03 \x01(\x0b\x32\x18.fast.Graph.ContextGraph\x12\x15\n\rSlotDummyNode\x18\x04 \x01(\x05\x12\x34\n\x0fsymbolCandidate\x18\x05 \x03(\x0b\x32\x1b.fast.Graph.SymbolCandidate\"\xe6\x01\n\x04\x44\x61ta\x12 \n\x07\x65lement\x18\x01 \x01(\x0b\x32\r.fast.ElementH\x00\x12\x18\n\x03log\x18\x02 \x01(\x0b\x32\t.fast.LogH\x00\x12\x1c\n\x05\x64\x65lta\x18\x03 \x01(\x0b\x32\x0b.fast.DeltaH\x00\x12\x1c\n\x05pairs\x18\x04 \x01(\x0b\x32\x0b.fast.PairsH\x00\x12\x1e\n\x06slices\x18\x05 \x01(\x0b\x32\x0c.fast.SlicesH\x00\x12\x1a\n\x04\x62ugs\x18\x06 \x01(\x0b\x32\n.fast.BugsH\x00\x12\x1c\n\x05graph\x18\x07 \x01(\x0b\x32\x0b.fast.GraphH\x00\x42\x0c\n\nRecordType*\x82v\n\x0b\x43obol85Kind\x12\r\n\tstartrule\x10\x00\x12\x13\n\x0f\x63ompilationunit\x10\x01\x12\x0f\n\x0bprogramunit\x10\x02\x12\x17\n\x13\x65ndprogramstatement\x10\x03\x12\x1a\n\x16identificationdivision\x10\x04\x12\x1e\n\x1aidentificationdivisionbody\x10\x05\x12\x16\n\x12programidparagraph\x10\x06\x12\x13\n\x0f\x61uthorparagraph\x10\x07\x12\x19\n\x15installationparagraph\x10\x08\x12\x18\n\x14\x64\x61tewrittenparagraph\x10\t\x12\x19\n\x15\x64\x61tecompiledparagraph\x10\n\x12\x15\n\x11securityparagraph\x10\x0b\x12\x14\n\x10remarksparagraph\x10\x0c\x12\x17\n\x13\x65nvironmentdivision\x10\r\x12\x1b\n\x17\x65nvironmentdivisionbody\x10\x0e\x12\x18\n\x14\x63onfigurationsection\x10\x0f\x12!\n\x1d\x63onfigurationsectionparagraph\x10\x10\x12\x1b\n\x17sourcecomputerparagraph\x10\x11\x12\x1b\n\x17objectcomputerparagraph\x10\x12\x12\x18\n\x14objectcomputerclause\x10\x13\x12\x14\n\x10memorysizeclause\x10\x14\x12\x12\n\x0e\x64isksizeclause\x10\x15\x12\x1b\n\x17\x63ollatingsequenceclause\x10\x16\x12\'\n#collatingsequenceclausealphanumeric\x10\x17\x12#\n\x1f\x63ollatingsequenceclausenational\x10\x18\x12\x16\n\x12segmentlimitclause\x10\x19\x12\x16\n\x12\x63haractersetclause\x10\x1a\x12\x19\n\x15specialnamesparagraph\x10\x1b\x12\x15\n\x11specialnameclause\x10\x1c\x12\x12\n\x0e\x61lphabetclause\x10\x1d\x12\x19\n\x15\x61lphabetclauseformat1\x10\x1e\x12\x14\n\x10\x61lphabetliterals\x10\x1f\x12\x13\n\x0f\x61lphabetthrough\x10 \x12\x10\n\x0c\x61lphabetalso\x10!\x12\x19\n\x15\x61lphabetclauseformat2\x10\"\x12\x11\n\rchannelclause\x10#\x12\x0f\n\x0b\x63lassclause\x10$\x12\x16\n\x12\x63lassclausethrough\x10%\x12\x13\n\x0f\x63lassclausefrom\x10&\x12\x11\n\rclassclauseto\x10\'\x12\x16\n\x12\x63urrencysignclause\x10(\x12\x16\n\x12\x64\x65\x63imalpointclause\x10)\x12\"\n\x1e\x64\x65\x66\x61ultcomputationalsignclause\x10*\x12\x1c\n\x18\x64\x65\x66\x61ultdisplaysignclause\x10+\x12\x1f\n\x1b\x65nvironmentswitchnameclause\x10,\x12\x31\n-environmentswitchnamespecialnamesstatusphrase\x10-\x12\r\n\todtclause\x10.\x12\x18\n\x14reservenetworkclause\x10/\x12\x1c\n\x18symboliccharactersclause\x10\x30\x12\x16\n\x12symboliccharacters\x10\x31\x12\x16\n\x12inputoutputsection\x10\x32\x12\x1f\n\x1binputoutputsectionparagraph\x10\x33\x12\x18\n\x14\x66ilecontrolparagraph\x10\x34\x12\x14\n\x10\x66ilecontrolentry\x10\x35\x12\x10\n\x0cselectclause\x10\x36\x12\x15\n\x11\x66ilecontrolclause\x10\x37\x12\x10\n\x0c\x61ssignclause\x10\x38\x12\x11\n\rreserveclause\x10\x39\x12\x16\n\x12organizationclause\x10:\x12\x1a\n\x16paddingcharacterclause\x10;\x12\x19\n\x15recorddelimiterclause\x10<\x12\x14\n\x10\x61\x63\x63\x65ssmodeclause\x10=\x12\x13\n\x0frecordkeyclause\x10>\x12\x1c\n\x18\x61lternaterecordkeyclause\x10?\x12\x12\n\x0epasswordclause\x10@\x12\x14\n\x10\x66ilestatusclause\x10\x41\x12\x15\n\x11relativekeyclause\x10\x42\x12\x16\n\x12iocontrolparagraph\x10\x43\x12\x13\n\x0fiocontrolclause\x10\x44\x12\x0f\n\x0brerunclause\x10\x45\x12\x15\n\x11reruneveryrecords\x10\x46\x12\x10\n\x0creruneveryof\x10G\x12\x13\n\x0freruneveryclock\x10H\x12\x0e\n\nsameclause\x10I\x12\x16\n\x12multiplefileclause\x10J\x12\x18\n\x14multiplefileposition\x10K\x12\x1b\n\x17\x63ommitmentcontrolclause\x10L\x12\x10\n\x0c\x64\x61tadivision\x10M\x12\x17\n\x13\x64\x61tadivisionsection\x10N\x12\x0f\n\x0b\x66ilesection\x10O\x12\x18\n\x14\x66iledescriptionentry\x10P\x12\x1e\n\x1a\x66iledescriptionentryclause\x10Q\x12\x12\n\x0e\x65xternalclause\x10R\x12\x10\n\x0cglobalclause\x10S\x12\x17\n\x13\x62lockcontainsclause\x10T\x12\x13\n\x0f\x62lockcontainsto\x10U\x12\x18\n\x14recordcontainsclause\x10V\x12\x1f\n\x1brecordcontainsclauseformat1\x10W\x12\x1f\n\x1brecordcontainsclauseformat2\x10X\x12\x1f\n\x1brecordcontainsclauseformat3\x10Y\x12\x14\n\x10recordcontainsto\x10Z\x12\x16\n\x12labelrecordsclause\x10[\x12\x11\n\rvalueofclause\x10\\\x12\r\n\tvaluepair\x10]\x12\x15\n\x11\x64\x61tarecordsclause\x10^\x12\x10\n\x0clinageclause\x10_\x12\x0c\n\x08linageat\x10`\x12\x13\n\x0flinagefootingat\x10\x61\x12\x14\n\x10linagelinesattop\x10\x62\x12\x17\n\x13linagelinesatbottom\x10\x63\x12\x17\n\x13recordingmodeclause\x10\x64\x12\x11\n\rmodestatement\x10\x65\x12\x11\n\rcodesetclause\x10\x66\x12\x10\n\x0creportclause\x10g\x12\x13\n\x0f\x64\x61tabasesection\x10h\x12\x18\n\x14\x64\x61tabasesectionentry\x10i\x12\x19\n\x15workingstoragesection\x10j\x12\x12\n\x0elinkagesection\x10k\x12\x18\n\x14\x63ommunicationsection\x10l\x12!\n\x1d\x63ommunicationdescriptionentry\x10m\x12(\n$communicationdescriptionentryformat1\x10n\x12(\n$communicationdescriptionentryformat2\x10o\x12(\n$communicationdescriptionentryformat3\x10p\x12\x1a\n\x16\x64\x65stinationcountclause\x10q\x12\x1a\n\x16\x64\x65stinationtableclause\x10r\x12\x10\n\x0c\x65ndkeyclause\x10s\x12\x12\n\x0e\x65rrorkeyclause\x10t\x12\x16\n\x12messagecountclause\x10u\x12\x15\n\x11messagedateclause\x10v\x12\x15\n\x11messagetimeclause\x10w\x12\x13\n\x0fstatuskeyclause\x10x\x12\x1d\n\x19symbolicdestinationclause\x10y\x12\x17\n\x13symbolicqueueclause\x10z\x12\x18\n\x14symbolicsourceclause\x10{\x12\x1a\n\x16symbolicterminalclause\x10|\x12\x1a\n\x16symbolicsubqueueclause\x10}\x12\x14\n\x10textlengthclause\x10~\x12\x17\n\x13localstoragesection\x10\x7f\x12\x12\n\rscreensection\x10\x80\x01\x12\x1b\n\x16screendescriptionentry\x10\x81\x01\x12!\n\x1cscreendescriptionblankclause\x10\x82\x01\x12 \n\x1bscreendescriptionbellclause\x10\x83\x01\x12!\n\x1cscreendescriptionblinkclause\x10\x84\x01\x12!\n\x1cscreendescriptioneraseclause\x10\x85\x01\x12!\n\x1cscreendescriptionlightclause\x10\x86\x01\x12 \n\x1bscreendescriptiongridclause\x10\x87\x01\x12(\n#screendescriptionreversevideoclause\x10\x88\x01\x12%\n screendescriptionunderlineclause\x10\x89\x01\x12 \n\x1bscreendescriptionsizeclause\x10\x8a\x01\x12 \n\x1bscreendescriptionlineclause\x10\x8b\x01\x12\"\n\x1dscreendescriptioncolumnclause\x10\x8c\x01\x12+\n&screendescriptionforegroundcolorclause\x10\x8d\x01\x12+\n&screendescriptionbackgroundcolorclause\x10\x8e\x01\x12#\n\x1escreendescriptioncontrolclause\x10\x8f\x01\x12!\n\x1cscreendescriptionvalueclause\x10\x90\x01\x12#\n\x1escreendescriptionpictureclause\x10\x91\x01\x12 \n\x1bscreendescriptionfromclause\x10\x92\x01\x12\x1e\n\x19screendescriptiontoclause\x10\x93\x01\x12!\n\x1cscreendescriptionusingclause\x10\x94\x01\x12!\n\x1cscreendescriptionusageclause\x10\x95\x01\x12)\n$screendescriptionblankwhenzeroclause\x10\x96\x01\x12%\n screendescriptionjustifiedclause\x10\x97\x01\x12 \n\x1bscreendescriptionsignclause\x10\x98\x01\x12 \n\x1bscreendescriptionautoclause\x10\x99\x01\x12\"\n\x1dscreendescriptionsecureclause\x10\x9a\x01\x12$\n\x1fscreendescriptionrequiredclause\x10\x9b\x01\x12\"\n\x1dscreendescriptionpromptclause\x10\x9c\x01\x12(\n#screendescriptionpromptoccursclause\x10\x9d\x01\x12 \n\x1bscreendescriptionfullclause\x10\x9e\x01\x12$\n\x1fscreendescriptionzerofillclause\x10\x9f\x01\x12\x12\n\rreportsection\x10\xa0\x01\x12\x16\n\x11reportdescription\x10\xa1\x01\x12\x1b\n\x16reportdescriptionentry\x10\xa2\x01\x12\"\n\x1dreportdescriptionglobalclause\x10\xa3\x01\x12%\n reportdescriptionpagelimitclause\x10\xa4\x01\x12#\n\x1ereportdescriptionheadingclause\x10\xa5\x01\x12\'\n\"reportdescriptionfirstdetailclause\x10\xa6\x01\x12&\n!reportdescriptionlastdetailclause\x10\xa7\x01\x12#\n\x1ereportdescriptionfootingclause\x10\xa8\x01\x12 \n\x1breportgroupdescriptionentry\x10\xa9\x01\x12\'\n\"reportgroupdescriptionentryformat1\x10\xaa\x01\x12\'\n\"reportgroupdescriptionentryformat2\x10\xab\x01\x12\'\n\"reportgroupdescriptionentryformat3\x10\xac\x01\x12#\n\x1ereportgroupblankwhenzeroclause\x10\xad\x01\x12\"\n\x1dreportgroupcolumnnumberclause\x10\xae\x01\x12\x1e\n\x19reportgroupindicateclause\x10\xaf\x01\x12\x1f\n\x1areportgroupjustifiedclause\x10\xb0\x01\x12 \n\x1breportgrouplinenumberclause\x10\xb1\x01\x12\"\n\x1dreportgrouplinenumbernextpage\x10\xb2\x01\x12\x1e\n\x19reportgrouplinenumberplus\x10\xb3\x01\x12\x1f\n\x1areportgroupnextgroupclause\x10\xb4\x01\x12\x1d\n\x18reportgroupnextgroupplus\x10\xb5\x01\x12!\n\x1creportgroupnextgroupnextpage\x10\xb6\x01\x12\x1d\n\x18reportgrouppictureclause\x10\xb7\x01\x12\x1b\n\x16reportgroupresetclause\x10\xb8\x01\x12\x1a\n\x15reportgroupsignclause\x10\xb9\x01\x12\x1c\n\x17reportgroupsourceclause\x10\xba\x01\x12\x19\n\x14reportgroupsumclause\x10\xbb\x01\x12\x1a\n\x15reportgrouptypeclause\x10\xbc\x01\x12!\n\x1creportgrouptypereportheading\x10\xbd\x01\x12\x1f\n\x1areportgrouptypepageheading\x10\xbe\x01\x12\"\n\x1dreportgrouptypecontrolheading\x10\xbf\x01\x12\x1a\n\x15reportgrouptypedetail\x10\xc0\x01\x12\"\n\x1dreportgrouptypecontrolfooting\x10\xc1\x01\x12\x1b\n\x16reportgroupusageclause\x10\xc2\x01\x12\x1f\n\x1areportgrouptypepagefooting\x10\xc3\x01\x12!\n\x1creportgrouptypereportfooting\x10\xc4\x01\x12\x1b\n\x16reportgroupvalueclause\x10\xc5\x01\x12\x1a\n\x15programlibrarysection\x10\xc6\x01\x12\x1c\n\x17librarydescriptionentry\x10\xc7\x01\x12#\n\x1elibrarydescriptionentryformat1\x10\xc8\x01\x12#\n\x1elibrarydescriptionentryformat2\x10\xc9\x01\x12\"\n\x1dlibraryattributeclauseformat1\x10\xca\x01\x12\"\n\x1dlibraryattributeclauseformat2\x10\xcb\x01\x12\x1d\n\x18libraryattributefunction\x10\xcc\x01\x12\x1e\n\x19libraryattributeparameter\x10\xcd\x01\x12\x1a\n\x15libraryattributetitle\x10\xce\x01\x12\'\n\"libraryentryprocedureclauseformat1\x10\xcf\x01\x12\'\n\"libraryentryprocedureclauseformat2\x10\xd0\x01\x12#\n\x1elibraryentryprocedureforclause\x10\xd1\x01\x12&\n!libraryentryproceduregivingclause\x10\xd2\x01\x12%\n libraryentryprocedureusingclause\x10\xd3\x01\x12#\n\x1elibraryentryprocedureusingname\x10\xd4\x01\x12$\n\x1flibraryentryprocedurewithclause\x10\xd5\x01\x12\"\n\x1dlibraryentryprocedurewithname\x10\xd6\x01\x12\x1a\n\x15libraryiscommonclause\x10\xd7\x01\x12\x1a\n\x15libraryisglobalclause\x10\xd8\x01\x12\x19\n\x14\x64\x61tadescriptionentry\x10\xd9\x01\x12 \n\x1b\x64\x61tadescriptionentryformat1\x10\xda\x01\x12 \n\x1b\x64\x61tadescriptionentryformat2\x10\xdb\x01\x12 \n\x1b\x64\x61tadescriptionentryformat3\x10\xdc\x01\x12 \n\x1b\x64\x61tadescriptionentryexecsql\x10\xdd\x01\x12\x16\n\x11\x64\x61taalignedclause\x10\xde\x01\x12\x1c\n\x17\x64\x61tablankwhenzeroclause\x10\xdf\x01\x12\x1d\n\x18\x64\x61tacommonownlocalclause\x10\xe0\x01\x12\x17\n\x12\x64\x61taexternalclause\x10\xe1\x01\x12\x15\n\x10\x64\x61taglobalclause\x10\xe2\x01\x12\x1c\n\x17\x64\x61taintegerstringclause\x10\xe3\x01\x12\x18\n\x13\x64\x61tajustifiedclause\x10\xe4\x01\x12\x15\n\x10\x64\x61taoccursclause\x10\xe5\x01\x12\x11\n\x0c\x64\x61taoccursto\x10\xe6\x01\x12\x13\n\x0e\x64\x61taoccurssort\x10\xe7\x01\x12\x16\n\x11\x64\x61tapictureclause\x10\xe8\x01\x12\x12\n\rpicturestring\x10\xe9\x01\x12\x11\n\x0cpicturechars\x10\xea\x01\x12\x17\n\x12picturecardinality\x10\xeb\x01\x12\x19\n\x14\x64\x61tareceivedbyclause\x10\xec\x01\x12\x19\n\x14\x64\x61tarecordareaclause\x10\xed\x01\x12\x18\n\x13\x64\x61taredefinesclause\x10\xee\x01\x12\x16\n\x11\x64\x61tarenamesclause\x10\xef\x01\x12\x13\n\x0e\x64\x61tasignclause\x10\xf0\x01\x12\x1b\n\x16\x64\x61tasynchronizedclause\x10\xf1\x01\x12\x1a\n\x15\x64\x61tathreadlocalclause\x10\xf2\x01\x12\x13\n\x0e\x64\x61tatypeclause\x10\xf3\x01\x12\x16\n\x11\x64\x61tatypedefclause\x10\xf4\x01\x12\x14\n\x0f\x64\x61tausageclause\x10\xf5\x01\x12\x14\n\x0f\x64\x61tausingclause\x10\xf6\x01\x12\x14\n\x0f\x64\x61tavalueclause\x10\xf7\x01\x12\x16\n\x11\x64\x61tavalueinterval\x10\xf8\x01\x12\x1a\n\x15\x64\x61tavalueintervalfrom\x10\xf9\x01\x12\x18\n\x13\x64\x61tavalueintervalto\x10\xfa\x01\x12\x1e\n\x19\x64\x61tawithlowerboundsclause\x10\xfb\x01\x12\x16\n\x11proceduredivision\x10\xfc\x01\x12!\n\x1cproceduredivisionusingclause\x10\xfd\x01\x12\"\n\x1dproceduredivisiongivingclause\x10\xfe\x01\x12$\n\x1fproceduredivisionusingparameter\x10\xff\x01\x12\'\n\"proceduredivisionbyreferencephrase\x10\x80\x02\x12!\n\x1cproceduredivisionbyreference\x10\x81\x02\x12#\n\x1eproceduredivisionbyvaluephrase\x10\x82\x02\x12\x1d\n\x18proceduredivisionbyvalue\x10\x83\x02\x12\x1a\n\x15proceduredeclaratives\x10\x84\x02\x12\x19\n\x14proceduredeclarative\x10\x85\x02\x12\x1b\n\x16proceduresectionheader\x10\x86\x02\x12\x1a\n\x15proceduredivisionbody\x10\x87\x02\x12\x15\n\x10proceduresection\x10\x88\x02\x12\x0f\n\nparagraphs\x10\x89\x02\x12\x0e\n\tparagraph\x10\x8a\x02\x12\r\n\x08sentence\x10\x8b\x02\x12\x0e\n\tstatement\x10\x8c\x02\x12\x14\n\x0f\x61\x63\x63\x65ptstatement\x10\x8d\x02\x12\x1c\n\x17\x61\x63\x63\x65ptfromdatestatement\x10\x8e\x02\x12 \n\x1b\x61\x63\x63\x65ptfrommnemonicstatement\x10\x8f\x02\x12!\n\x1c\x61\x63\x63\x65ptfromescapekeystatement\x10\x90\x02\x12 \n\x1b\x61\x63\x63\x65ptmessagecountstatement\x10\x91\x02\x12\x11\n\x0c\x61\x64\x64statement\x10\x92\x02\x12\x13\n\x0e\x61\x64\x64tostatement\x10\x93\x02\x12\x19\n\x14\x61\x64\x64togivingstatement\x10\x94\x02\x12\x1e\n\x19\x61\x64\x64\x63orrespondingstatement\x10\x95\x02\x12\x0c\n\x07\x61\x64\x64\x66rom\x10\x96\x02\x12\n\n\x05\x61\x64\x64to\x10\x97\x02\x12\x10\n\x0b\x61\x64\x64togiving\x10\x98\x02\x12\x0e\n\taddgiving\x10\x99\x02\x12\x10\n\x0b\x61lteredgoto\x10\x9a\x02\x12\x13\n\x0e\x61lterstatement\x10\x9b\x02\x12\x13\n\x0e\x61lterproceedto\x10\x9c\x02\x12\x12\n\rcallstatement\x10\x9d\x02\x12\x14\n\x0f\x63\x61llusingphrase\x10\x9e\x02\x12\x17\n\x12\x63\x61llusingparameter\x10\x9f\x02\x12\x1a\n\x15\x63\x61llbyreferencephrase\x10\xa0\x02\x12\x14\n\x0f\x63\x61llbyreference\x10\xa1\x02\x12\x16\n\x11\x63\x61llbyvaluephrase\x10\xa2\x02\x12\x10\n\x0b\x63\x61llbyvalue\x10\xa3\x02\x12\x18\n\x13\x63\x61llbycontentphrase\x10\xa4\x02\x12\x12\n\rcallbycontent\x10\xa5\x02\x12\x15\n\x10\x63\x61llgivingphrase\x10\xa6\x02\x12\x14\n\x0f\x63\x61ncelstatement\x10\xa7\x02\x12\x0f\n\ncancelcall\x10\xa8\x02\x12\x13\n\x0e\x63losestatement\x10\xa9\x02\x12\x0e\n\tclosefile\x10\xaa\x02\x12\x1b\n\x16\x63losereelunitstatement\x10\xab\x02\x12\x1b\n\x16\x63loserelativestatement\x10\xac\x02\x12\x1d\n\x18\x63loseportfileiostatement\x10\xad\x02\x12\x19\n\x14\x63loseportfileiousing\x10\xae\x02\x12)\n$closeportfileiousingclosedisposition\x10\xaf\x02\x12\'\n\"closeportfileiousingassociateddata\x10\xb0\x02\x12-\n(closeportfileiousingassociateddatalength\x10\xb1\x02\x12\x15\n\x10\x63omputestatement\x10\xb2\x02\x12\x11\n\x0c\x63omputestore\x10\xb3\x02\x12\x16\n\x11\x63ontinuestatement\x10\xb4\x02\x12\x14\n\x0f\x64\x65letestatement\x10\xb5\x02\x12\x15\n\x10\x64isablestatement\x10\xb6\x02\x12\x15\n\x10\x64isplaystatement\x10\xb7\x02\x12\x13\n\x0e\x64isplayoperand\x10\xb8\x02\x12\x0e\n\tdisplayat\x10\xb9\x02\x12\x10\n\x0b\x64isplayupon\x10\xba\x02\x12\x10\n\x0b\x64isplaywith\x10\xbb\x02\x12\x14\n\x0f\x64ividestatement\x10\xbc\x02\x12\x18\n\x13\x64ivideintostatement\x10\xbd\x02\x12\x1e\n\x19\x64ivideintogivingstatement\x10\xbe\x02\x12\x1c\n\x17\x64ividebygivingstatement\x10\xbf\x02\x12\x17\n\x12\x64ividegivingphrase\x10\xc0\x02\x12\x0f\n\ndivideinto\x10\xc1\x02\x12\x11\n\x0c\x64ividegiving\x10\xc2\x02\x12\x14\n\x0f\x64ivideremainder\x10\xc3\x02\x12\x14\n\x0f\x65nablestatement\x10\xc4\x02\x12\x13\n\x0e\x65ntrystatement\x10\xc5\x02\x12\x16\n\x11\x65valuatestatement\x10\xc6\x02\x12\x13\n\x0e\x65valuateselect\x10\xc7\x02\x12\x17\n\x12\x65valuatealsoselect\x10\xc8\x02\x12\x17\n\x12\x65valuatewhenphrase\x10\xc9\x02\x12\x11\n\x0c\x65valuatewhen\x10\xca\x02\x12\x16\n\x11\x65valuatecondition\x10\xcb\x02\x12\x14\n\x0f\x65valuatethrough\x10\xcc\x02\x12\x1a\n\x15\x65valuatealsocondition\x10\xcd\x02\x12\x16\n\x11\x65valuatewhenother\x10\xce\x02\x12\x12\n\revaluatevalue\x10\xcf\x02\x12\x16\n\x11\x65xeccicsstatement\x10\xd0\x02\x12\x15\n\x10\x65xecsqlstatement\x10\xd1\x02\x12\x18\n\x13\x65xecsqlimsstatement\x10\xd2\x02\x12\x15\n\x10\x65xhibitstatement\x10\xd3\x02\x12\x13\n\x0e\x65xhibitoperand\x10\xd4\x02\x12\x12\n\rexitstatement\x10\xd5\x02\x12\x16\n\x11generatestatement\x10\xd6\x02\x12\x14\n\x0fgobackstatement\x10\xd7\x02\x12\x12\n\rgotostatement\x10\xd8\x02\x12\x18\n\x13gotostatementsimple\x10\xd9\x02\x12\x1d\n\x18gotodependingonstatement\x10\xda\x02\x12\x10\n\x0bifstatement\x10\xdb\x02\x12\x0b\n\x06ifthen\x10\xdc\x02\x12\x0b\n\x06ifelse\x10\xdd\x02\x12\x18\n\x13initializestatement\x10\xde\x02\x12\x1e\n\x19initializereplacingphrase\x10\xdf\x02\x12\x1a\n\x15initializereplacingby\x10\xe0\x02\x12\x16\n\x11initiatestatement\x10\xe1\x02\x12\x15\n\x10inspectstatement\x10\xe2\x02\x12\x1a\n\x15inspecttallyingphrase\x10\xe3\x02\x12\x1b\n\x16inspectreplacingphrase\x10\xe4\x02\x12#\n\x1einspecttallyingreplacingphrase\x10\xe5\x02\x12\x1c\n\x17inspectconvertingphrase\x10\xe6\x02\x12\x0f\n\ninspectfor\x10\xe7\x02\x12\x16\n\x11inspectcharacters\x10\xe8\x02\x12\x1f\n\x1ainspectreplacingcharacters\x10\xe9\x02\x12\x17\n\x12inspectallleadings\x10\xea\x02\x12 \n\x1binspectreplacingallleadings\x10\xeb\x02\x12\x16\n\x11inspectallleading\x10\xec\x02\x12\x1f\n\x1ainspectreplacingallleading\x10\xed\x02\x12\x0e\n\tinspectby\x10\xee\x02\x12\x0e\n\tinspectto\x10\xef\x02\x12\x17\n\x12inspectbeforeafter\x10\xf0\x02\x12\x13\n\x0emergestatement\x10\xf1\x02\x12\x15\n\x10mergeonkeyclause\x10\xf2\x02\x12!\n\x1cmergecollatingsequencephrase\x10\xf3\x02\x12\x1f\n\x1amergecollatingalphanumeric\x10\xf4\x02\x12\x1b\n\x16mergecollatingnational\x10\xf5\x02\x12\x0f\n\nmergeusing\x10\xf6\x02\x12\x1f\n\x1amergeoutputprocedurephrase\x10\xf7\x02\x12\x17\n\x12mergeoutputthrough\x10\xf8\x02\x12\x16\n\x11mergegivingphrase\x10\xf9\x02\x12\x10\n\x0bmergegiving\x10\xfa\x02\x12\x12\n\rmovestatement\x10\xfb\x02\x12\x14\n\x0fmovetostatement\x10\xfc\x02\x12\x16\n\x11movetosendingarea\x10\xfd\x02\x12!\n\x1cmovecorrespondingtostatement\x10\xfe\x02\x12#\n\x1emovecorrespondingtosendingarea\x10\xff\x02\x12\x16\n\x11multiplystatement\x10\x80\x03\x12\x14\n\x0fmultiplyregular\x10\x81\x03\x12\x1b\n\x16multiplyregularoperand\x10\x82\x03\x12\x13\n\x0emultiplygiving\x10\x83\x03\x12\x1a\n\x15multiplygivingoperand\x10\x84\x03\x12\x19\n\x14multiplygivingresult\x10\x85\x03\x12\x12\n\ropenstatement\x10\x86\x03\x12\x17\n\x12openinputstatement\x10\x87\x03\x12\x0e\n\topeninput\x10\x88\x03\x12\x18\n\x13openoutputstatement\x10\x89\x03\x12\x0f\n\nopenoutput\x10\x8a\x03\x12\x14\n\x0fopeniostatement\x10\x8b\x03\x12\x18\n\x13openextendstatement\x10\x8c\x03\x12\x15\n\x10performstatement\x10\x8d\x03\x12\x1b\n\x16performinlinestatement\x10\x8e\x03\x12\x1e\n\x19performprocedurestatement\x10\x8f\x03\x12\x10\n\x0bperformtype\x10\x90\x03\x12\x11\n\x0cperformtimes\x10\x91\x03\x12\x11\n\x0cperformuntil\x10\x92\x03\x12\x13\n\x0eperformvarying\x10\x93\x03\x12\x19\n\x14performvaryingclause\x10\x94\x03\x12\x19\n\x14performvaryingphrase\x10\x95\x03\x12\x11\n\x0cperformafter\x10\x96\x03\x12\x10\n\x0bperformfrom\x10\x97\x03\x12\x0e\n\tperformby\x10\x98\x03\x12\x16\n\x11performtestclause\x10\x99\x03\x12\x13\n\x0epurgestatement\x10\x9a\x03\x12\x12\n\rreadstatement\x10\x9b\x03\x12\r\n\x08readinto\x10\x9c\x03\x12\r\n\x08readwith\x10\x9d\x03\x12\x0c\n\x07readkey\x10\x9e\x03\x12\x15\n\x10receivestatement\x10\x9f\x03\x12\x19\n\x14receivefromstatement\x10\xa0\x03\x12\x10\n\x0breceivefrom\x10\xa1\x03\x12\x19\n\x14receiveintostatement\x10\xa2\x03\x12\x12\n\rreceivenodata\x10\xa3\x03\x12\x14\n\x0freceivewithdata\x10\xa4\x03\x12\x12\n\rreceivebefore\x10\xa5\x03\x12\x10\n\x0breceivewith\x10\xa6\x03\x12\x12\n\rreceivethread\x10\xa7\x03\x12\x10\n\x0breceivesize\x10\xa8\x03\x12\x12\n\rreceivestatus\x10\xa9\x03\x12\x15\n\x10releasestatement\x10\xaa\x03\x12\x14\n\x0freturnstatement\x10\xab\x03\x12\x0f\n\nreturninto\x10\xac\x03\x12\x15\n\x10rewritestatement\x10\xad\x03\x12\x10\n\x0brewritefrom\x10\xae\x03\x12\x14\n\x0fsearchstatement\x10\xaf\x03\x12\x12\n\rsearchvarying\x10\xb0\x03\x12\x0f\n\nsearchwhen\x10\xb1\x03\x12\x12\n\rsendstatement\x10\xb2\x03\x12\x16\n\x11sendstatementsync\x10\xb3\x03\x12\x17\n\x12sendstatementasync\x10\xb4\x03\x12\x13\n\x0esendfromphrase\x10\xb5\x03\x12\x13\n\x0esendwithphrase\x10\xb6\x03\x12\x18\n\x13sendreplacingphrase\x10\xb7\x03\x12\x18\n\x13sendadvancingphrase\x10\xb8\x03\x12\x16\n\x11sendadvancingpage\x10\xb9\x03\x12\x17\n\x12sendadvancinglines\x10\xba\x03\x12\x1a\n\x15sendadvancingmnemonic\x10\xbb\x03\x12\x11\n\x0csetstatement\x10\xbc\x03\x12\x13\n\x0esettostatement\x10\xbd\x03\x12\x19\n\x14setupdownbystatement\x10\xbe\x03\x12\n\n\x05setto\x10\xbf\x03\x12\x0f\n\nsettovalue\x10\xc0\x03\x12\x0f\n\nsetbyvalue\x10\xc1\x03\x12\x12\n\rsortstatement\x10\xc2\x03\x12\x14\n\x0fsortonkeyclause\x10\xc3\x03\x12\x19\n\x14sortduplicatesphrase\x10\xc4\x03\x12 \n\x1bsortcollatingsequencephrase\x10\xc5\x03\x12\x1e\n\x19sortcollatingalphanumeric\x10\xc6\x03\x12\x1a\n\x15sortcollatingnational\x10\xc7\x03\x12\x1d\n\x18sortinputprocedurephrase\x10\xc8\x03\x12\x15\n\x10sortinputthrough\x10\xc9\x03\x12\x0e\n\tsortusing\x10\xca\x03\x12\x1e\n\x19sortoutputprocedurephrase\x10\xcb\x03\x12\x16\n\x11sortoutputthrough\x10\xcc\x03\x12\x15\n\x10sortgivingphrase\x10\xcd\x03\x12\x0f\n\nsortgiving\x10\xce\x03\x12\x13\n\x0estartstatement\x10\xcf\x03\x12\r\n\x08startkey\x10\xd0\x03\x12\x12\n\rstopstatement\x10\xd1\x03\x12\x14\n\x0fstringstatement\x10\xd2\x03\x12\x18\n\x13stringsendingphrase\x10\xd3\x03\x12\x12\n\rstringsending\x10\xd4\x03\x12\x1c\n\x17stringdelimitedbyphrase\x10\xd5\x03\x12\x14\n\x0fstringforphrase\x10\xd6\x03\x12\x15\n\x10stringintophrase\x10\xd7\x03\x12\x1c\n\x17stringwithpointerphrase\x10\xd8\x03\x12\x16\n\x11subtractstatement\x10\xd9\x03\x12\x1a\n\x15subtractfromstatement\x10\xda\x03\x12 \n\x1bsubtractfromgivingstatement\x10\xdb\x03\x12#\n\x1esubtractcorrespondingstatement\x10\xdc\x03\x12\x17\n\x12subtractsubtrahend\x10\xdd\x03\x12\x14\n\x0fsubtractminuend\x10\xde\x03\x12\x1a\n\x15subtractminuendgiving\x10\xdf\x03\x12\x13\n\x0esubtractgiving\x10\xe0\x03\x12!\n\x1csubtractminuendcorresponding\x10\xe1\x03\x12\x17\n\x12terminatestatement\x10\xe2\x03\x12\x16\n\x11unstringstatement\x10\xe3\x03\x12\x1a\n\x15unstringsendingphrase\x10\xe4\x03\x12\x1e\n\x19unstringdelimitedbyphrase\x10\xe5\x03\x12\x18\n\x13unstringorallphrase\x10\xe6\x03\x12\x17\n\x12unstringintophrase\x10\xe7\x03\x12\x11\n\x0cunstringinto\x10\xe8\x03\x12\x18\n\x13unstringdelimiterin\x10\xe9\x03\x12\x14\n\x0funstringcountin\x10\xea\x03\x12\x1e\n\x19unstringwithpointerphrase\x10\xeb\x03\x12\x1b\n\x16unstringtallyingphrase\x10\xec\x03\x12\x11\n\x0cusestatement\x10\xed\x03\x12\x13\n\x0euseafterclause\x10\xee\x03\x12\x0f\n\nuseafteron\x10\xef\x03\x12\x13\n\x0eusedebugclause\x10\xf0\x03\x12\x0f\n\nusedebugon\x10\xf1\x03\x12\x13\n\x0ewritestatement\x10\xf2\x03\x12\x14\n\x0fwritefromphrase\x10\xf3\x03\x12\x19\n\x14writeadvancingphrase\x10\xf4\x03\x12\x17\n\x12writeadvancingpage\x10\xf5\x03\x12\x18\n\x13writeadvancinglines\x10\xf6\x03\x12\x1b\n\x16writeadvancingmnemonic\x10\xf7\x03\x12\x1b\n\x16writeatendofpagephrase\x10\xf8\x03\x12\x1e\n\x19writenotatendofpagephrase\x10\xf9\x03\x12\x10\n\x0b\x61tendphrase\x10\xfa\x03\x12\x13\n\x0enotatendphrase\x10\xfb\x03\x12\x15\n\x10invalidkeyphrase\x10\xfc\x03\x12\x18\n\x13notinvalidkeyphrase\x10\xfd\x03\x12\x15\n\x10onoverflowphrase\x10\xfe\x03\x12\x18\n\x13notonoverflowphrase\x10\xff\x03\x12\x16\n\x11onsizeerrorphrase\x10\x80\x04\x12\x19\n\x14notonsizeerrorphrase\x10\x81\x04\x12\x16\n\x11onexceptionclause\x10\x82\x04\x12\x19\n\x14notonexceptionclause\x10\x83\x04\x12\x19\n\x14\x61rithmeticexpression\x10\x84\x04\x12\x0e\n\tplusminus\x10\x85\x04\x12\r\n\x08multdivs\x10\x86\x04\x12\x0c\n\x07multdiv\x10\x87\x04\x12\x0b\n\x06powers\x10\x88\x04\x12\n\n\x05power\x10\x89\x04\x12\n\n\x05\x62\x61sis\x10\x8a\x04\x12\x0e\n\tcondition\x10\x8b\x04\x12\x13\n\x0e\x61ndorcondition\x10\x8c\x04\x12\x18\n\x13\x63ombinablecondition\x10\x8d\x04\x12\x14\n\x0fsimplecondition\x10\x8e\x04\x12\x13\n\x0e\x63lasscondition\x10\x8f\x04\x12\x1b\n\x16\x63onditionnamereference\x10\x90\x04\x12$\n\x1f\x63onditionnamesubscriptreference\x10\x91\x04\x12\x16\n\x11relationcondition\x10\x92\x04\x12\x1a\n\x15relationsigncondition\x10\x93\x04\x12!\n\x1crelationarithmeticcomparison\x10\x94\x04\x12\x1f\n\x1arelationcombinedcomparison\x10\x95\x04\x12\x1e\n\x19relationcombinedcondition\x10\x96\x04\x12\x17\n\x12relationaloperator\x10\x97\x04\x12\x11\n\x0c\x61\x62\x62reviation\x10\x98\x04\x12\x0f\n\nidentifier\x10\x99\x04\x12\x0e\n\ttablecall\x10\x9a\x04\x12\x11\n\x0c\x66unctioncall\x10\x9b\x04\x12\x16\n\x11referencemodifier\x10\x9c\x04\x12\x16\n\x11\x63haracterposition\x10\x9d\x04\x12\x0b\n\x06length\x10\x9e\x04\x12\x0e\n\tsubscript\x10\x9f\x04\x12\r\n\x08\x61rgument\x10\xa0\x04\x12\x16\n\x11qualifieddataname\x10\xa1\x04\x12\x1d\n\x18qualifieddatanameformat1\x10\xa2\x04\x12\x1d\n\x18qualifieddatanameformat2\x10\xa3\x04\x12\x1d\n\x18qualifieddatanameformat3\x10\xa4\x04\x12\x1d\n\x18qualifieddatanameformat4\x10\xa5\x04\x12\x14\n\x0fqualifiedindata\x10\xa6\x04\x12\x0b\n\x06indata\x10\xa7\x04\x12\x0b\n\x06infile\x10\xa8\x04\x12\x0f\n\ninmnemonic\x10\xa9\x04\x12\x0e\n\tinsection\x10\xaa\x04\x12\x0e\n\tinlibrary\x10\xab\x04\x12\x0c\n\x07intable\x10\xac\x04\x12\x11\n\x0c\x61lphabetname\x10\xad\x04\x12\x13\n\x0e\x61ssignmentname\x10\xae\x04\x12\x0e\n\tbasisname\x10\xaf\x04\x12\x0b\n\x06\x63\x64name\x10\xb0\x04\x12\x0e\n\tclassname\x10\xb1\x04\x12\x11\n\x0c\x63omputername\x10\xb2\x04\x12\x12\n\rconditionname\x10\xb3\x04\x12\r\n\x08\x64\x61taname\x10\xb4\x04\x12\x11\n\x0c\x64\x61tadescname\x10\xb5\x04\x12\x14\n\x0f\x65nvironmentname\x10\xb6\x04\x12\r\n\x08\x66ilename\x10\xb7\x04\x12\x11\n\x0c\x66unctionname\x10\xb8\x04\x12\x0e\n\tindexname\x10\xb9\x04\x12\x11\n\x0clanguagename\x10\xba\x04\x12\x10\n\x0blibraryname\x10\xbb\x04\x12\x0e\n\tlocalname\x10\xbc\x04\x12\x11\n\x0cmnemonicname\x10\xbd\x04\x12\x12\n\rparagraphname\x10\xbe\x04\x12\x12\n\rprocedurename\x10\xbf\x04\x12\x10\n\x0bprogramname\x10\xc0\x04\x12\x0f\n\nrecordname\x10\xc1\x04\x12\x0f\n\nreportname\x10\xc2\x04\x12\x10\n\x0broutinename\x10\xc3\x04\x12\x0f\n\nscreenname\x10\xc4\x04\x12\x10\n\x0bsectionname\x10\xc5\x04\x12\x0f\n\nsystemname\x10\xc6\x04\x12\x16\n\x11symboliccharacter\x10\xc7\x04\x12\r\n\x08textname\x10\xc8\x04\x12\x0e\n\tcobolword\x10\xc9\x04\x12\x0c\n\x07literal\x10\xca\x04\x12\x13\n\x0e\x62ooleanliteral\x10\xcb\x04\x12\x13\n\x0enumericliteral\x10\xcc\x04\x12\x13\n\x0eintegerliteral\x10\xcd\x04\x12\x17\n\x12\x63icsdfhrespliteral\x10\xce\x04\x12\x18\n\x13\x63icsdfhvalueliteral\x10\xcf\x04\x12\x17\n\x12\x66igurativeconstant\x10\xd0\x04\x12\x14\n\x0fspecialregister\x10\xd1\x04\x12\x11\n\x0c\x63ommententry\x10\xd2\x04*\xd6\n\n\x0bPython3Kind\x12\x10\n\x0csingle_input\x10\x00\x12\x0e\n\nfile_input\x10\x01\x12\x0e\n\neval_input\x10\x02\x12\r\n\tdecorator\x10\x03\x12\x0e\n\ndecorators\x10\x04\x12\r\n\tdecorated\x10\x05\x12\x11\n\rasync_funcdef\x10\x06\x12\x0b\n\x07\x66uncdef\x10\x07\x12\x0e\n\nparameters\x10\x08\x12\x11\n\rtypedargslist\x10\t\x12\n\n\x06tfpdef\x10\n\x12\x0f\n\x0bvarargslist\x10\x0b\x12\n\n\x06vfpdef\x10\x0c\x12\x08\n\x04stmt\x10\r\x12\x0f\n\x0bsimple_stmt\x10\x0e\x12\x0e\n\nsmall_stmt\x10\x0f\x12\r\n\texpr_stmt\x10\x10\x12\r\n\tannassign\x10\x11\x12\x16\n\x12testlist_star_expr\x10\x12\x12\r\n\taugassign\x10\x13\x12\x0c\n\x08\x64\x65l_stmt\x10\x14\x12\r\n\tpass_stmt\x10\x15\x12\r\n\tflow_stmt\x10\x16\x12\x0e\n\nbreak_stmt\x10\x17\x12\x11\n\rcontinue_stmt\x10\x18\x12\x0f\n\x0breturn_stmt\x10\x19\x12\x0e\n\nyield_stmt\x10\x1a\x12\x0e\n\nraise_stmt\x10\x1b\x12\x0f\n\x0bimport_stmt\x10\x1c\x12\x0f\n\x0bimport_name\x10\x1d\x12\x0f\n\x0bimport_from\x10\x1e\x12\x12\n\x0eimport_as_name\x10\x1f\x12\x12\n\x0e\x64otted_as_name\x10 \x12\x13\n\x0fimport_as_names\x10!\x12\x13\n\x0f\x64otted_as_names\x10\"\x12\x0f\n\x0b\x64otted_name\x10#\x12\x0f\n\x0bglobal_stmt\x10$\x12\x11\n\rnonlocal_stmt\x10%\x12\x0f\n\x0b\x61ssert_stmt\x10&\x12\x11\n\rcompound_stmt\x10\'\x12\x0e\n\nasync_stmt\x10(\x12\x0b\n\x07if_stmt\x10)\x12\x0e\n\nwhile_stmt\x10*\x12\x0c\n\x08\x66or_stmt\x10+\x12\x0c\n\x08try_stmt\x10,\x12\r\n\twith_stmt\x10-\x12\r\n\twith_item\x10.\x12\x11\n\rexcept_clause\x10/\x12\t\n\x05suite\x10\x30\x12\x08\n\x04test\x10\x31\x12\x0f\n\x0btest_nocond\x10\x32\x12\x0b\n\x07lambdef\x10\x33\x12\x12\n\x0elambdef_nocond\x10\x34\x12\x0b\n\x07or_test\x10\x35\x12\x0c\n\x08\x61nd_test\x10\x36\x12\x0c\n\x08not_test\x10\x37\x12\x0e\n\ncomparison\x10\x38\x12\x0b\n\x07\x63omp_op\x10\x39\x12\r\n\tstar_expr\x10:\x12\x08\n\x04\x65xpr\x10;\x12\x0c\n\x08xor_expr\x10<\x12\x0c\n\x08\x61nd_expr\x10=\x12\x0e\n\nshift_expr\x10>\x12\x0e\n\narith_expr\x10?\x12\x08\n\x04term\x10@\x12\n\n\x06\x66\x61\x63tor\x10\x41\x12\x10\n\x0cpython_power\x10\x42\x12\r\n\tatom_expr\x10\x43\x12\x08\n\x04\x61tom\x10\x44\x12\x11\n\rtestlist_comp\x10\x45\x12\x0b\n\x07trailer\x10\x46\x12\x11\n\rsubscriptlist\x10G\x12\x14\n\x10python_subscript\x10H\x12\x0b\n\x07sliceop\x10I\x12\x0c\n\x08\x65xprlist\x10J\x12\x0c\n\x08testlist\x10K\x12\x12\n\x0e\x64ictorsetmaker\x10L\x12\x0c\n\x08\x63lassdef\x10M\x12\x0b\n\x07\x61rglist\x10N\x12\x13\n\x0fpython_argument\x10O\x12\r\n\tcomp_iter\x10P\x12\x0c\n\x08\x63omp_for\x10Q\x12\x0b\n\x07\x63omp_if\x10R\x12\x11\n\rencoding_decl\x10S\x12\x0e\n\nyield_expr\x10T\x12\r\n\tyield_arg\x10U*\xc1\x12\n\tSmaliKind\x12\x0e\n\nsmali_file\x10\x00\x12\x0e\n\nclass_spec\x10\x01\x12\x0e\n\nsuper_spec\x10\x02\x12\x13\n\x0fimplements_spec\x10\x03\x12\x0f\n\x0bsource_spec\x10\x04\x12\x0f\n\x0b\x61\x63\x63\x65ss_list\x10\x05\x12\t\n\x05\x66ield\x10\x06\x12\n\n\x06method\x10\x07\x12\x1d\n\x19statements_and_directives\x10\x08\x12\x17\n\x13ordered_method_item\x10\t\x12\x17\n\x13registers_directive\x10\n\x12\x14\n\x10param_list_or_id\x10\x0b\x12\x0f\n\x0bsimple_name\x10\x0c\x12\x0f\n\x0bmember_name\x10\r\x12\x14\n\x10method_prototype\x10\x0e\x12#\n\x1fparam_list_or_id_primitive_type\x10\x0f\x12\x0e\n\nparam_list\x10\x10\x12\x14\n\x10\x61rray_descriptor\x10\x11\x12\x13\n\x0ftype_descriptor\x10\x12\x12\x1b\n\x17nonvoid_type_descriptor\x10\x13\x12\x1d\n\x19reference_type_descriptor\x10\x14\x12\x13\n\x0finteger_literal\x10\x15\x12\x11\n\rfloat_literal\x10\x16\x12\x12\n\x0e\x64ouble_literal\x10\x17\x12\x11\n\rsmali_literal\x10\x18\x12\x1a\n\x16parsed_integer_literal\x10\x19\x12\x14\n\x10integral_literal\x10\x1a\x12\x17\n\x13\x66ixed_32bit_literal\x10\x1b\x12\x11\n\rfixed_literal\x10\x1c\x12\x11\n\rarray_literal\x10\x1d\x12\x16\n\x12\x61nnotation_element\x10\x1e\x12\x0e\n\nannotation\x10\x1f\x12\x11\n\rsubannotation\x10 \x12\x10\n\x0c\x65num_literal\x10!\x12\x1d\n\x19type_field_method_literal\x10\"\x12\x14\n\x10method_reference\x10#\x12\x13\n\x0f\x66ield_reference\x10$\x12\t\n\x05label\x10%\x12\r\n\tlabel_ref\x10&\x12\x11\n\rregister_list\x10\'\x12\x12\n\x0eregister_range\x10(\x12 \n\x1cverification_error_reference\x10)\x12\x13\n\x0f\x63\x61tch_directive\x10*\x12\x16\n\x12\x63\x61tchall_directive\x10+\x12\x17\n\x13parameter_directive\x10,\x12\x13\n\x0f\x64\x65\x62ug_directive\x10-\x12\x12\n\x0eline_directive\x10.\x12\x13\n\x0flocal_directive\x10/\x12\x17\n\x13\x65nd_local_directive\x10\x30\x12\x1b\n\x17restart_local_directive\x10\x31\x12\x16\n\x12prologue_directive\x10\x32\x12\x16\n\x12\x65pilogue_directive\x10\x33\x12\x14\n\x10source_directive\x10\x34\x12\x19\n\x15instruction_format12x\x10\x35\x12\x19\n\x15instruction_format22s\x10\x36\x12\x19\n\x15instruction_format31i\x10\x37\x12\x0f\n\x0binstruction\x10\x38\x12\x12\n\x0einsn_format10t\x10\x39\x12\x12\n\x0einsn_format10x\x10:\x12\x17\n\x13insn_format10x_odex\x10;\x12\x12\n\x0einsn_format11n\x10<\x12\x12\n\x0einsn_format11x\x10=\x12\x12\n\x0einsn_format12x\x10>\x12\x13\n\x0finsn_format20bc\x10?\x12\x12\n\x0einsn_format20t\x10@\x12\x18\n\x14insn_format21c_field\x10\x41\x12\x1d\n\x19insn_format21c_field_odex\x10\x42\x12\x19\n\x15insn_format21c_string\x10\x43\x12\x17\n\x13insn_format21c_type\x10\x44\x12\x13\n\x0finsn_format21ih\x10\x45\x12\x13\n\x0finsn_format21lh\x10\x46\x12\x12\n\x0einsn_format21s\x10G\x12\x12\n\x0einsn_format21t\x10H\x12\x12\n\x0einsn_format22b\x10I\x12\x18\n\x14insn_format22c_field\x10J\x12\x1d\n\x19insn_format22c_field_odex\x10K\x12\x17\n\x13insn_format22c_type\x10L\x12\x19\n\x15insn_format22cs_field\x10M\x12\x12\n\x0einsn_format22s\x10N\x12\x12\n\x0einsn_format22t\x10O\x12\x12\n\x0einsn_format22x\x10P\x12\x12\n\x0einsn_format23x\x10Q\x12\x12\n\x0einsn_format30t\x10R\x12\x12\n\x0einsn_format31c\x10S\x12\x12\n\x0einsn_format31i\x10T\x12\x12\n\x0einsn_format31t\x10U\x12\x12\n\x0einsn_format32x\x10V\x12\x19\n\x15insn_format35c_method\x10W\x12\x17\n\x13insn_format35c_type\x10X\x12\x1e\n\x1ainsn_format35c_method_odex\x10Y\x12\x1a\n\x16insn_format35mi_method\x10Z\x12\x1a\n\x16insn_format35ms_method\x10[\x12\x19\n\x15insn_format3rc_method\x10\\\x12\x1e\n\x1ainsn_format3rc_method_odex\x10]\x12\x17\n\x13insn_format3rc_type\x10^\x12\x1a\n\x16insn_format3rmi_method\x10_\x12\x1a\n\x16insn_format3rms_method\x10`\x12\x1a\n\x16insn_format45cc_method\x10\x61\x12\x1a\n\x16insn_format4rcc_method\x10\x62\x12\x12\n\x0einsn_format51l\x10\x63\x12\x1d\n\x19insn_array_data_directive\x10\x64\x12 \n\x1cinsn_packed_switch_directive\x10\x65\x12 \n\x1cinsn_sparse_switch_directive\x10\x66*\xd9\r\n\x0cSolidityKind\x12\x0e\n\nsourceunit\x10\x00\x12\x13\n\x0fpragmadirective\x10\x01\x12\x0e\n\npragmaname\x10\x02\x12\x0f\n\x0bpragmavalue\x10\x03\x12\x0b\n\x07version\x10\x04\x12\x13\n\x0fversionoperator\x10\x05\x12\x15\n\x11versionconstraint\x10\x06\x12\x15\n\x11importdeclaration\x10\x07\x12\x13\n\x0fimportdirective\x10\x08\x12\x16\n\x12\x63ontractdefinition\x10\t\x12\x18\n\x14inheritancespecifier\x10\n\x12\x10\n\x0c\x63ontractpart\x10\x0b\x12\x1c\n\x18statevariabledeclaration\x10\x0c\x12\x17\n\x13usingfordeclaration\x10\r\x12\x14\n\x10structdefinition\x10\x0e\x12\x16\n\x12modifierdefinition\x10\x0f\x12\x16\n\x12modifierinvocation\x10\x10\x12\x16\n\x12\x66unctiondefinition\x10\x11\x12\x14\n\x10returnparameters\x10\x12\x12\x10\n\x0cmodifierlist\x10\x13\x12\x13\n\x0f\x65ventdefinition\x10\x14\x12\r\n\tenumvalue\x10\x15\x12\x12\n\x0e\x65numdefinition\x10\x16\x12\x18\n\x14indexedparameterlist\x10\x17\x12\x14\n\x10indexedparameter\x10\x18\x12\x11\n\rparameterlist\x10\x19\x12\r\n\tparameter\x10\x1a\x12\x10\n\x0ctypenamelist\x10\x1b\x12\x14\n\x10unnamedparameter\x10\x1c\x12\x17\n\x13variabledeclaration\x10\x1d\x12\x0c\n\x08typename\x10\x1e\x12\x17\n\x13userdefinedtypename\x10\x1f\x12\x0b\n\x07mapping\x10 \x12\x14\n\x10\x66unctiontypename\x10!\x12\x13\n\x0fstoragelocation\x10\"\x12\x13\n\x0fstatemutability\x10#\x12\t\n\x05\x62lock\x10$\x12\x16\n\x12solidity_statement\x10%\x12\x17\n\x13\x65xpressionstatement\x10&\x12\x18\n\x14solidity_ifstatement\x10\'\x12\x12\n\x0ewhilestatement\x10(\x12\x13\n\x0fsimplestatement\x10)\x12\x10\n\x0c\x66orstatement\x10*\x12\x1b\n\x17inlineassemblystatement\x10+\x12\x14\n\x10\x64owhilestatement\x10,\x12\x1e\n\x1asolidity_continuestatement\x10-\x12\x12\n\x0e\x62reakstatement\x10.\x12\x1c\n\x18solidity_returnstatement\x10/\x12\x12\n\x0ethrowstatement\x10\x30\x12 \n\x1cvariabledeclarationstatement\x10\x31\x12\x12\n\x0eidentifierlist\x10\x32\x12\x16\n\x12\x65lementarytypename\x10\x33\x12\x0e\n\nexpression\x10\x34\x12\x15\n\x11primaryexpression\x10\x35\x12\x12\n\x0e\x65xpressionlist\x10\x36\x12\x11\n\rnamevaluelist\x10\x37\x12\r\n\tnamevalue\x10\x38\x12\x19\n\x15\x66unctioncallarguments\x10\x39\x12\x11\n\rassemblyblock\x10:\x12\x10\n\x0c\x61ssemblyitem\x10;\x12\x16\n\x12\x61ssemblyexpression\x10<\x12\x10\n\x0c\x61ssemblycall\x10=\x12\x1b\n\x17\x61ssemblylocaldefinition\x10>\x12\x16\n\x12\x61ssemblyassignment\x10?\x12\x1c\n\x18\x61ssemblyidentifierorlist\x10@\x12\x1a\n\x16\x61ssemblyidentifierlist\x10\x41\x12\x1b\n\x17\x61ssemblyrightassignment\x10\x42\x12\x13\n\x0flabeldefinition\x10\x43\x12\x12\n\x0e\x61ssemblyswitch\x10\x44\x12\x10\n\x0c\x61ssemblycase\x10\x45\x12\x1e\n\x1a\x61ssemblyfunctiondefinition\x10\x46\x12\x0f\n\x0b\x61ssemblyfor\x10G\x12\x13\n\x0f\x61ssemblyliteral\x10H\x12\x0f\n\x0bsubassembly\x10I\x12\x0c\n\x08\x64\x61tasize\x10J\x12\x10\n\x0clinkersymbol\x10K\x12\x13\n\x0ftupleexpression\x10L\x12 \n\x1c\x65lementarytypenameexpression\x10M\x12\x11\n\rnumberliteral\x10N\x12\x17\n\x13solidity_identifier\x10O*\xd8\x32\n\tSrcmlKind\x12\r\n\tUNIT_KIND\x10\x00\x12\x08\n\x04\x44\x45\x43L\x10\x01\x12\r\n\tDECL_STMT\x10\x02\x12\x08\n\x04INIT\x10\x03\x12\x08\n\x04\x45XPR\x10\x04\x12\r\n\tEXPR_STMT\x10\x05\x12\x0b\n\x07\x43OMMENT\x10\x06\x12\x08\n\x04\x43\x41LL\x10\x07\x12\x0b\n\x07\x43ONTROL\x10\x08\x12\x08\n\x04INCR\x10\t\x12\x08\n\x04NONE\x10\n\x12\x0c\n\x08VARIABLE\x10\x0b\x12\x0c\n\x08\x46UNCTION\x10\x0c\x12\x11\n\rFUNCTION_DECL\x10\r\x12\x0f\n\x0b\x43ONSTRUCTOR\x10\x0e\x12\x14\n\x10\x43ONSTRUCTOR_DECL\x10\x0f\x12\x0e\n\nDESTRUCTOR\x10\x10\x12\x13\n\x0f\x44\x45STRUCTOR_DECL\x10\x11\x12\t\n\x05MACRO\x10\x12\x12\x10\n\x0cSINGLE_MACRO\x10\x13\x12\x10\n\x0cNULLOPERATOR\x10\x14\x12\r\n\tENUM_DEFN\x10\x15\x12\r\n\tENUM_DECL\x10\x16\x12\x14\n\x10GLOBAL_ATTRIBUTE\x10\x17\x12\x15\n\x11PROPERTY_ACCESSOR\x10\x18\x12\x1a\n\x16PROPERTY_ACCESSOR_DECL\x10\x19\x12\x0e\n\nEXPRESSION\x10\x1a\x12\x0e\n\nCLASS_DEFN\x10\x1b\x12\x0e\n\nCLASS_DECL\x10\x1c\x12\x0e\n\nUNION_DEFN\x10\x1d\x12\x0e\n\nUNION_DECL\x10\x1e\x12\x0f\n\x0bSTRUCT_DEFN\x10\x1f\x12\x0f\n\x0bSTRUCT_DECL\x10 \x12\x12\n\x0eINTERFACE_DEFN\x10!\x12\x12\n\x0eINTERFACE_DECL\x10\"\x12\x11\n\rACCESS_REGION\x10#\x12\t\n\x05USING\x10$\x12\x15\n\x11OPERATOR_FUNCTION\x10%\x12\x1a\n\x16OPERATOR_FUNCTION_DECL\x10&\x12\t\n\x05\x45VENT\x10\'\x12\x0c\n\x08PROPERTY\x10(\x12\x13\n\x0f\x41NNOTATION_DEFN\x10)\x12\x13\n\x0fGLOBAL_TEMPLATE\x10*\x12\x08\n\x04UNIT\x10+\x12\x16\n\x12TART_ELEMENT_TOKEN\x10,\x12\x07\n\x03NOP\x10-\x12\n\n\x06STRING\x10.\x12\x08\n\x04\x43HAR\x10/\x12\x0b\n\x07LITERAL\x10\x30\x12\x0b\n\x07\x42OOLEAN\x10\x31\x12\t\n\x05NULL2\x10\x32\x12\x0b\n\x07\x43OMPLEX\x10\x33\x12\x0c\n\x08OPERATOR\x10\x34\x12\x0c\n\x08MODIFIER\x10\x35\x12\x08\n\x04NAME\x10\x36\x12\t\n\x05ONAME\x10\x37\x12\t\n\x05\x43NAME\x10\x38\x12\x08\n\x04TYPE\x10\x39\x12\x0c\n\x08TYPEPREV\x10:\x12\r\n\tCONDITION\x10;\x12\t\n\x05\x42LOCK\x10<\x12\x10\n\x0cPSEUDO_BLOCK\x10=\x12\t\n\x05INDEX\x10>\x12\x08\n\x04\x45NUM\x10?\x12\x14\n\x10\x45NUM_DECLARATION\x10@\x12\x10\n\x0cIF_STATEMENT\x10\x41\x12\x0b\n\x07TERNARY\x10\x42\x12\x08\n\x04THEN\x10\x43\x12\x08\n\x04\x45LSE\x10\x44\x12\n\n\x06\x45LSEIF\x10\x45\x12\x13\n\x0fWHILE_STATEMENT\x10\x46\x12\x10\n\x0c\x44O_STATEMENT\x10G\x12\x11\n\rFOR_STATEMENT\x10H\x12\x15\n\x11\x46OREACH_STATEMENT\x10I\x12\x0f\n\x0b\x46OR_CONTROL\x10J\x12\x16\n\x12\x46OR_INITIALIZATION\x10K\x12\x11\n\rFOR_CONDITION\x10L\x12\x11\n\rFOR_INCREMENT\x10M\x12\x14\n\x10\x46OR_LIKE_CONTROL\x10N\x12\x18\n\x14\x45XPRESSION_STATEMENT\x10O\x12\x11\n\rFUNCTION_CALL\x10Q\x12\x19\n\x15\x44\x45\x43LARATION_STATEMENT\x10R\x12\x0f\n\x0b\x44\x45\x43LARATION\x10S\x12\x1e\n\x1a\x44\x45\x43LARATION_INITIALIZATION\x10T\x12\x15\n\x11\x44\x45\x43LARATION_RANGE\x10U\x12\t\n\x05RANGE\x10V\x12\x12\n\x0eGOTO_STATEMENT\x10W\x12\x16\n\x12\x43ONTINUE_STATEMENT\x10X\x12\x13\n\x0f\x42REAK_STATEMENT\x10Y\x12\x13\n\x0fLABEL_STATEMENT\x10Z\x12\t\n\x05LABEL\x10[\x12\n\n\x06SWITCH\x10\\\x12\x08\n\x04\x43\x41SE\x10]\x12\x0b\n\x07\x44\x45\x46\x41ULT\x10^\x12\x17\n\x13\x46UNCTION_DEFINITION\x10_\x12\x18\n\x14\x46UNCTION_DECLARATION\x10`\x12\n\n\x06LAMBDA\x10\x61\x12\x13\n\x0f\x46UNCTION_LAMBDA\x10\x62\x12\x16\n\x12\x46UNCTION_SPECIFIER\x10\x63\x12\x14\n\x10RETURN_STATEMENT\x10\x64\x12\x12\n\x0ePARAMETER_LIST\x10\x65\x12\r\n\tPARAMETER\x10\x66\x12\x14\n\x10KRPARAMETER_LIST\x10g\x12\x0f\n\x0bKRPARAMETER\x10h\x12\x11\n\rARGUMENT_LIST\x10i\x12\x0c\n\x08\x41RGUMENT\x10j\x12\x19\n\x15PSEUDO_PARAMETER_LIST\x10k\x12\x1a\n\x16INDEXER_PARAMETER_LIST\x10l\x12\t\n\x05\x43LASS\x10m\x12\x15\n\x11\x43LASS_DECLARATION\x10n\x12\n\n\x06STRUCT\x10o\x12\x16\n\x12STRUCT_DECLARATION\x10p\x12\t\n\x05UNION\x10q\x12\x15\n\x11UNION_DECLARATION\x10r\x12\x13\n\x0f\x44\x45RIVATION_LIST\x10s\x12\x11\n\rPUBLIC_ACCESS\x10t\x12\x19\n\x15PUBLIC_ACCESS_DEFAULT\x10u\x12\x12\n\x0ePRIVATE_ACCESS\x10v\x12\x1a\n\x16PRIVATE_ACCESS_DEFAULT\x10w\x12\x14\n\x10PROTECTED_ACCESS\x10x\x12\x1c\n\x18PROTECTED_ACCESS_DEFAULT\x10y\x12\x14\n\x10MEMBER_INIT_LIST\x10z\x12\x1e\n\x1aMEMBER_INITIALIZATION_LIST\x10{\x12\x19\n\x15MEMBER_INITIALIZATION\x10|\x12\x1a\n\x16\x43ONSTRUCTOR_DEFINITION\x10}\x12\x1b\n\x17\x43ONSTRUCTOR_DECLARATION\x10~\x12\x19\n\x15\x44\x45STRUCTOR_DEFINITION\x10\x7f\x12\x1b\n\x16\x44\x45STRUCTOR_DECLARATION\x10\x80\x01\x12\x0b\n\x06\x46RIEND\x10\x81\x01\x12\x14\n\x0f\x43LASS_SPECIFIER\x10\x82\x01\x12\x0e\n\tTRY_BLOCK\x10\x83\x01\x12\x10\n\x0b\x43\x41TCH_BLOCK\x10\x84\x01\x12\x12\n\rFINALLY_BLOCK\x10\x85\x01\x12\x14\n\x0fTHROW_STATEMENT\x10\x86\x01\x12\x14\n\x0fTHROW_SPECIFIER\x10\x87\x01\x12\x19\n\x14THROW_SPECIFIER_JAVA\x10\x88\x01\x12\r\n\x08TEMPLATE\x10\x89\x01\x12\x15\n\x10GENERIC_ARGUMENT\x10\x8a\x01\x12\x1a\n\x15GENERIC_ARGUMENT_LIST\x10\x8b\x01\x12\x17\n\x12TEMPLATE_PARAMETER\x10\x8c\x01\x12\x1c\n\x17TEMPLATE_PARAMETER_LIST\x10\x8d\x01\x12\x16\n\x11GENERIC_PARAMETER\x10\x8e\x01\x12\x1b\n\x16GENERIC_PARAMETER_LIST\x10\x8f\x01\x12\x0c\n\x07TYPEDEF\x10\x90\x01\x12\x08\n\x03\x41SM\x10\x91\x01\x12\x0f\n\nMACRO_CALL\x10\x92\x01\x12\x10\n\x0bSIZEOF_CALL\x10\x93\x01\x12\x0b\n\x06\x45XTERN\x10\x94\x01\x12\x0e\n\tNAMESPACE\x10\x95\x01\x12\x14\n\x0fUSING_DIRECTIVE\x10\x96\x01\x12\x0e\n\tDIRECTIVE\x10\x97\x01\x12\x0b\n\x06\x41TOMIC\x10\x98\x01\x12\x1c\n\x17STATIC_ASSERT_STATEMENT\x10\x99\x01\x12\x16\n\x11GENERIC_SELECTION\x10\x9a\x01\x12\x15\n\x10GENERIC_SELECTOR\x10\x9b\x01\x12\x1d\n\x18GENERIC_ASSOCIATION_LIST\x10\x9c\x01\x12\x18\n\x13GENERIC_ASSOCIATION\x10\x9d\x01\x12\x0c\n\x07\x41LIGNAS\x10\x9e\x01\x12\r\n\x08\x44\x45\x43LTYPE\x10\x9f\x01\x12\x0c\n\x07\x43\x41PTURE\x10\xa0\x01\x12\x13\n\x0eLAMBDA_CAPTURE\x10\xa1\x01\x12\r\n\x08NOEXCEPT\x10\xa2\x01\x12\r\n\x08TYPENAME\x10\xa3\x01\x12\x0c\n\x07\x41LIGNOF\x10\xa4\x01\x12\x0b\n\x06TYPEID\x10\xa5\x01\x12\x10\n\x0bSIZEOF_PACK\x10\xa6\x01\x12\x0f\n\nENUM_CLASS\x10\xa7\x01\x12\x1b\n\x16\x45NUM_CLASS_DECLARATION\x10\xa8\x01\x12\x12\n\rREF_QUALIFIER\x10\xab\x01\x12\x12\n\rSIGNAL_ACCESS\x10\xac\x01\x12\x16\n\x11\x46OREVER_STATEMENT\x10\xad\x01\x12\x13\n\x0e\x45MIT_STATEMENT\x10\xae\x01\x12\x12\n\rCPP_DIRECTIVE\x10\xaf\x01\x12\x11\n\x0c\x43PP_FILENAME\x10\xb0\x01\x12\t\n\x04\x46ILE\x10\xb1\x01\x12\x0b\n\x06NUMBER\x10\xb2\x01\x12\x0f\n\nCPP_NUMBER\x10\xb3\x01\x12\x10\n\x0b\x43PP_LITERAL\x10\xb4\x01\x12\x13\n\x0e\x43PP_MACRO_DEFN\x10\xb5\x01\x12\x14\n\x0f\x43PP_MACRO_VALUE\x10\xb6\x01\x12\n\n\x05\x45RROR\x10\xb7\x01\x12\x0e\n\tCPP_ERROR\x10\xb8\x01\x12\x10\n\x0b\x43PP_WARNING\x10\xb9\x01\x12\x0f\n\nCPP_PRAGMA\x10\xba\x01\x12\x10\n\x0b\x43PP_INCLUDE\x10\xbb\x01\x12\x0f\n\nCPP_DEFINE\x10\xbc\x01\x12\x0e\n\tCPP_UNDEF\x10\xbd\x01\x12\r\n\x08\x43PP_LINE\x10\xbe\x01\x12\x0b\n\x06\x43PP_IF\x10\xbf\x01\x12\x0e\n\tCPP_IFDEF\x10\xc0\x01\x12\x0f\n\nCPP_IFNDEF\x10\xc1\x01\x12\r\n\x08\x43PP_THEN\x10\xc2\x01\x12\r\n\x08\x43PP_ELSE\x10\xc3\x01\x12\r\n\x08\x43PP_ELIF\x10\xc4\x01\x12\x0e\n\tCPP_EMPTY\x10\xc5\x01\x12\x0f\n\nCPP_REGION\x10\xc6\x01\x12\x12\n\rCPP_ENDREGION\x10\xc7\x01\x12\x0f\n\nUSING_STMT\x10\xc8\x01\x12\x0b\n\x06\x45SCAPE\x10\xc9\x01\x12\n\n\x05VALUE\x10\xca\x01\x12\x0f\n\nCPP_IMPORT\x10\xcb\x01\x12\x0e\n\tCPP_ENDIF\x10\xcc\x01\x12\x0b\n\x06MARKER\x10\xcd\x01\x12\x10\n\x0b\x45RROR_PARSE\x10\xce\x01\x12\x0f\n\nERROR_MODE\x10\xcf\x01\x12\x0f\n\nIMPLEMENTS\x10\xd0\x01\x12\x0c\n\x07\x45XTENDS\x10\xd1\x01\x12\x0b\n\x06IMPORT\x10\xd2\x01\x12\x0c\n\x07PACKAGE\x10\xd3\x01\x12\x15\n\x10\x41SSERT_STATEMENT\x10\xd4\x01\x12\x0e\n\tINTERFACE\x10\xd5\x01\x12\x1a\n\x15INTERFACE_DECLARATION\x10\xd6\x01\x12\x1b\n\x16SYNCHRONIZED_STATEMENT\x10\xd7\x01\x12\x0f\n\nANNOTATION\x10\xd8\x01\x12\x11\n\x0cSTATIC_BLOCK\x10\xda\x01\x12\x16\n\x11\x43HECKED_STATEMENT\x10\xdb\x01\x12\x18\n\x13UNCHECKED_STATEMENT\x10\xdc\x01\x12\x0e\n\tATTRIBUTE\x10\xdd\x01\x12\x0b\n\x06TARGET\x10\xde\x01\x12\x15\n\x10UNSAFE_STATEMENT\x10\xdf\x01\x12\x13\n\x0eLOCK_STATEMENT\x10\xe0\x01\x12\x14\n\x0f\x46IXED_STATEMENT\x10\xe1\x01\x12\x0b\n\x06TYPEOF\x10\xe2\x01\x12\x14\n\x0fUSING_STATEMENT\x10\xe3\x01\x12\x16\n\x11\x46UNCTION_DELEGATE\x10\xe4\x01\x12\x0f\n\nCONSTRAINT\x10\xe6\x01\x12\t\n\x04LINQ\x10\xe7\x01\x12\t\n\x04\x46ROM\x10\xe8\x01\x12\n\n\x05WHERE\x10\xe9\x01\x12\x0b\n\x06SELECT\x10\xea\x01\x12\x08\n\x03LET\x10\xeb\x01\x12\x0c\n\x07ORDERBY\x10\xec\x01\x12\t\n\x04JOIN\x10\xed\x01\x12\n\n\x05GROUP\x10\xee\x01\x12\x07\n\x02IN\x10\xef\x01\x12\x07\n\x02ON\x10\xf0\x01\x12\x0b\n\x06\x45QUALS\x10\xf1\x01\x12\x07\n\x02\x42Y\x10\xf2\x01\x12\t\n\x04INTO\x10\xf3\x01\x12\n\n\x05\x45MPTY\x10\xf4\x01\x12\x0f\n\nEMPTY_STMT\x10\xf5\x01\x12\r\n\x08RECEIVER\x10\xf6\x01\x12\x0c\n\x07MESSAGE\x10\xf7\x01\x12\r\n\x08SELECTOR\x10\xf8\x01\x12\x12\n\rPROTOCOL_LIST\x10\xf9\x01\x12\r\n\x08\x43\x41TEGORY\x10\xfa\x01\x12\r\n\x08PROTOCOL\x10\xfb\x01\x12\x15\n\x10REQUIRED_DEFAULT\x10\xfc\x01\x12\r\n\x08REQUIRED\x10\xfd\x01\x12\r\n\x08OPTIONAL\x10\xfe\x01\x12\x13\n\x0e\x41TTRIBUTE_LIST\x10\x80\x02\x12\x0f\n\nSYNTHESIZE\x10\x81\x02\x12\x0c\n\x07\x44YNAMIC\x10\x82\x02\x12\x0b\n\x06\x45NCODE\x10\x83\x02\x12\x14\n\x0f\x41UTORELEASEPOOL\x10\x84\x02\x12\x18\n\x13\x43OMPATIBILITY_ALIAS\x10\x85\x02\x12\x08\n\x03NIL\x10\x86\x02\x12\x14\n\x0f\x43LASS_INTERFACE\x10\x87\x02\x12\x19\n\x14\x43LASS_IMPLEMENTATION\x10\x88\x02\x12\x19\n\x14PROTOCOL_DECLARATION\x10\x89\x02\x12\t\n\x04\x43\x41ST\x10\x8a\x02\x12\x0f\n\nCONST_CAST\x10\x8b\x02\x12\x11\n\x0c\x44YNAMIC_CAST\x10\x8c\x02\x12\x15\n\x10REINTERPRET_CAST\x10\x8d\x02\x12\x10\n\x0bSTATIC_CAST\x10\x8e\x02\x12\r\n\x08POSITION\x10\x8f\x02\x12\x17\n\x12\x43UDA_ARGUMENT_LIST\x10\x90\x02\x12\x12\n\rOMP_DIRECTIVE\x10\x91\x02\x12\r\n\x08OMP_NAME\x10\x92\x02\x12\x0f\n\nOMP_CLAUSE\x10\x93\x02\x12\x16\n\x11OMP_ARGUMENT_LIST\x10\x94\x02\x12\x11\n\x0cOMP_ARGUMENT\x10\x95\x02\x12\x13\n\x0eOMP_EXPRESSION\x10\x96\x02\x12\x16\n\x11\x45ND_ELEMENT_TOKEN\x10\x97\x02\x12\t\n\x04MAIN\x10\x98\x02\x12\n\n\x05\x42REAK\x10\x99\x02\x12\r\n\x08\x43ONTINUE\x10\x9a\x02\x12\n\n\x05WHILE\x10\x9b\x02\x12\x07\n\x02\x44O\x10\x9c\x02\x12\x08\n\x03\x46OR\x10\x9d\x02\x12\x07\n\x02IF\x10\x9e\x02\x12\t\n\x04GOTO\x10\xa5\x02\x12\x13\n\x0eVISUAL_CXX_ASM\x10\xa7\x02\x12\x0b\n\x06SIZEOF\x10\xa8\x02\x12\t\n\x04\x41UTO\x10\xaa\x02\x12\r\n\x08REGISTER\x10\xab\x02\x12\r\n\x08RESTRICT\x10\xac\x02\x12\x0e\n\tIMAGINARY\x10\xb0\x02\x12\r\n\x08NORETURN\x10\xb1\x02\x12\x12\n\rSTATIC_ASSERT\x10\xb2\x02\x12\x0e\n\tCRESTRICT\x10\xb3\x02\x12\x0c\n\x07\x43XX_TRY\x10\xb4\x02\x12\x0e\n\tCXX_CATCH\x10\xb5\x02\x12\x0e\n\tCXX_CLASS\x10\xb6\x02\x12\x0e\n\tCONSTEXPR\x10\xb7\x02\x12\x11\n\x0cTHREAD_LOCAL\x10\xb9\x02\x12\x0c\n\x07NULLPTR\x10\xba\x02\x12\t\n\x04VOID\x10\xc6\x02\x12\x0b\n\x06RETURN\x10\xc7\x02\x12\x0c\n\x07INCLUDE\x10\xc8\x02\x12\x0b\n\x06\x44\x45\x46INE\x10\xc9\x02\x12\t\n\x04\x45LIF\x10\xca\x02\x12\n\n\x05\x45NDIF\x10\xcb\x02\x12\x0e\n\tERRORPREC\x10\xcc\x02\x12\x0c\n\x07WARNING\x10\xcd\x02\x12\n\n\x05IFDEF\x10\xce\x02\x12\x0b\n\x06IFNDEF\x10\xcf\x02\x12\t\n\x04LINE\x10\xd0\x02\x12\x0b\n\x06PRAGMA\x10\xd1\x02\x12\n\n\x05UNDEF\x10\xd2\x02\x12\x0b\n\x06INLINE\x10\xd3\x02\x12\x14\n\x0fMACRO_TYPE_NAME\x10\xd4\x02\x12\x0f\n\nMACRO_CASE\x10\xd5\x02\x12\x10\n\x0bMACRO_LABEL\x10\xd6\x02\x12\x0e\n\tSPECIFIER\x10\xd8\x02\x12\x08\n\x03TRY\x10\xd9\x02\x12\n\n\x05\x43\x41TCH\x10\xda\x02\x12\n\n\x05THROW\x10\xdb\x02\x12\x0b\n\x06THROWS\x10\xdc\x02\x12\x0b\n\x06PUBLIC\x10\xde\x02\x12\x0c\n\x07PRIVATE\x10\xdf\x02\x12\x0e\n\tPROTECTED\x10\xe0\x02\x12\x0c\n\x07VIRTUAL\x10\xe1\x02\x12\r\n\x08\x45XPLICIT\x10\xe4\x02\x12\x0c\n\x07\x46OREVER\x10\xe5\x02\x12\x0b\n\x06SIGNAL\x10\xe6\x02\x12\t\n\x04\x45MIT\x10\xe7\x02\x12\x08\n\x03NEW\x10\xeb\x02\x12\x0b\n\x06\x44\x45LETE\x10\xec\x02\x12\x0b\n\x06STATIC\x10\xed\x02\x12\n\n\x05\x43ONST\x10\xee\x02\x12\x0c\n\x07MUTABLE\x10\xef\x02\x12\r\n\x08VOLATILE\x10\xf0\x02\x12\x0e\n\tTRANSIENT\x10\xf1\x02\x12\x0c\n\x07\x46INALLY\x10\xf4\x02\x12\n\n\x05\x46INAL\x10\xf8\x02\x12\r\n\x08\x41\x42STRACT\x10\xf9\x02\x12\n\n\x05SUPER\x10\xfa\x02\x12\x11\n\x0cSYNCHRONIZED\x10\xfb\x02\x12\x0b\n\x06NATIVE\x10\xfc\x02\x12\r\n\x08STRICTFP\x10\xfd\x02\x12\x10\n\x0bNULLLITERAL\x10\xfe\x02\x12\x0b\n\x06\x41SSERT\x10\xff\x02\x12\x0c\n\x07\x46OREACH\x10\x80\x03\x12\x08\n\x03REF\x10\x81\x03\x12\x08\n\x03OUT\x10\x82\x03\x12\t\n\x04LOCK\x10\x84\x03\x12\x07\n\x02IS\x10\x85\x03\x12\r\n\x08INTERNAL\x10\x86\x03\x12\x0b\n\x06SEALED\x10\x87\x03\x12\r\n\x08OVERRIDE\x10\x88\x03\x12\r\n\x08IMPLICIT\x10\x89\x03\x12\x0f\n\nSTACKALLOC\x10\x8a\x03\x12\x07\n\x02\x41S\x10\x8b\x03\x12\r\n\x08\x44\x45LEGATE\x10\x8c\x03\x12\n\n\x05\x46IXED\x10\x8d\x03\x12\x0c\n\x07\x43HECKED\x10\x8e\x03\x12\x0e\n\tUNCHECKED\x10\x8f\x03\x12\x0b\n\x06REGION\x10\x90\x03\x12\x0e\n\tENDREGION\x10\x91\x03\x12\x0b\n\x06UNSAFE\x10\x92\x03\x12\r\n\x08READONLY\x10\x93\x03\x12\x08\n\x03GET\x10\x94\x03\x12\x08\n\x03SET\x10\x95\x03\x12\x08\n\x03\x41\x44\x44\x10\x96\x03\x12\x0b\n\x06REMOVE\x10\x97\x03\x12\n\n\x05YIELD\x10\x98\x03\x12\x0c\n\x07PARTIAL\x10\x99\x03\x12\n\n\x05\x41WAIT\x10\x9a\x03\x12\n\n\x05\x41SYNC\x10\x9c\x03\x12\t\n\x04THIS\x10\x9d\x03\x12\x0b\n\x06PARAMS\x10\x9e\x03\x12\n\n\x05\x41LIAS\x10\xa0\x03\x12\x0e\n\tASCENDING\x10\xa6\x03\x12\x0f\n\nDESCENDING\x10\xa7\x03\x12\x10\n\x0b\x41TINTERFACE\x10\xae\x03\x12\x15\n\x10\x41TIMPLEMENTATION\x10\xaf\x03\x12\n\n\x05\x41TEND\x10\xb0\x03\x12\x0f\n\nATPROTOCOL\x10\xb1\x03\x12\x0f\n\nATREQUIRED\x10\xb2\x03\x12\x0f\n\nATOPTIONAL\x10\xb3\x03\x12\x0c\n\x07\x41TCLASS\x10\xb9\x03\x12\t\n\x04WEAK\x10\xbb\x03\x12\x0b\n\x06STRONG\x10\xbc\x03\x12\x0c\n\x07OMP_OMP\x10\xc0\x03\x12\x12\n\rSPECIAL_CHARS\x10\xc1\x03\x12\x11\n\x0cSLICE_DEFINE\x10\xc2\x03\x12\x0e\n\tSLICE_USE\x10\xc3\x03\x62\x06proto3')
)

_COBOL85KIND = _descriptor.EnumDescriptor(
  name='Cobol85Kind',
  full_name='fast.Cobol85Kind',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='startrule', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='compilationunit', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='programunit', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='endprogramstatement', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='identificationdivision', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='identificationdivisionbody', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='programidparagraph', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='authorparagraph', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='installationparagraph', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datewrittenparagraph', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datecompiledparagraph', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='securityparagraph', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='remarksparagraph', index=12, number=12,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='environmentdivision', index=13, number=13,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='environmentdivisionbody', index=14, number=14,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='configurationsection', index=15, number=15,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='configurationsectionparagraph', index=16, number=16,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sourcecomputerparagraph', index=17, number=17,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='objectcomputerparagraph', index=18, number=18,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='objectcomputerclause', index=19, number=19,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='memorysizeclause', index=20, number=20,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='disksizeclause', index=21, number=21,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='collatingsequenceclause', index=22, number=22,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='collatingsequenceclausealphanumeric', index=23, number=23,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='collatingsequenceclausenational', index=24, number=24,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='segmentlimitclause', index=25, number=25,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='charactersetclause', index=26, number=26,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='specialnamesparagraph', index=27, number=27,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='specialnameclause', index=28, number=28,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetclause', index=29, number=29,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetclauseformat1', index=30, number=30,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetliterals', index=31, number=31,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetthrough', index=32, number=32,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetalso', index=33, number=33,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetclauseformat2', index=34, number=34,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='channelclause', index=35, number=35,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classclause', index=36, number=36,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classclausethrough', index=37, number=37,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classclausefrom', index=38, number=38,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classclauseto', index=39, number=39,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='currencysignclause', index=40, number=40,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='decimalpointclause', index=41, number=41,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='defaultcomputationalsignclause', index=42, number=42,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='defaultdisplaysignclause', index=43, number=43,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='environmentswitchnameclause', index=44, number=44,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='environmentswitchnamespecialnamesstatusphrase', index=45, number=45,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='odtclause', index=46, number=46,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reservenetworkclause', index=47, number=47,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symboliccharactersclause', index=48, number=48,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symboliccharacters', index=49, number=49,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inputoutputsection', index=50, number=50,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inputoutputsectionparagraph', index=51, number=51,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filecontrolparagraph', index=52, number=52,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filecontrolentry', index=53, number=53,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='selectclause', index=54, number=54,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filecontrolclause', index=55, number=55,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assignclause', index=56, number=56,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reserveclause', index=57, number=57,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='organizationclause', index=58, number=58,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='paddingcharacterclause', index=59, number=59,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recorddelimiterclause', index=60, number=60,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='accessmodeclause', index=61, number=61,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordkeyclause', index=62, number=62,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alternaterecordkeyclause', index=63, number=63,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='passwordclause', index=64, number=64,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filestatusclause', index=65, number=65,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relativekeyclause', index=66, number=66,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='iocontrolparagraph', index=67, number=67,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='iocontrolclause', index=68, number=68,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='rerunclause', index=69, number=69,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reruneveryrecords', index=70, number=70,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reruneveryof', index=71, number=71,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reruneveryclock', index=72, number=72,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sameclause', index=73, number=73,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplefileclause', index=74, number=74,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplefileposition', index=75, number=75,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='commitmentcontrolclause', index=76, number=76,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadivision', index=77, number=77,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadivisionsection', index=78, number=78,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filesection', index=79, number=79,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filedescriptionentry', index=80, number=80,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filedescriptionentryclause', index=81, number=81,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='externalclause', index=82, number=82,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='globalclause', index=83, number=83,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='blockcontainsclause', index=84, number=84,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='blockcontainsto', index=85, number=85,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordcontainsclause', index=86, number=86,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordcontainsclauseformat1', index=87, number=87,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordcontainsclauseformat2', index=88, number=88,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordcontainsclauseformat3', index=89, number=89,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordcontainsto', index=90, number=90,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='labelrecordsclause', index=91, number=91,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='valueofclause', index=92, number=92,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='valuepair', index=93, number=93,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datarecordsclause', index=94, number=94,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linageclause', index=95, number=95,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linageat', index=96, number=96,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linagefootingat', index=97, number=97,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linagelinesattop', index=98, number=98,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linagelinesatbottom', index=99, number=99,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordingmodeclause', index=100, number=100,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='modestatement', index=101, number=101,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='codesetclause', index=102, number=102,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportclause', index=103, number=103,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='databasesection', index=104, number=104,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='databasesectionentry', index=105, number=105,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='workingstoragesection', index=106, number=106,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linkagesection', index=107, number=107,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='communicationsection', index=108, number=108,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='communicationdescriptionentry', index=109, number=109,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='communicationdescriptionentryformat1', index=110, number=110,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='communicationdescriptionentryformat2', index=111, number=111,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='communicationdescriptionentryformat3', index=112, number=112,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='destinationcountclause', index=113, number=113,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='destinationtableclause', index=114, number=114,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='endkeyclause', index=115, number=115,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='errorkeyclause', index=116, number=116,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='messagecountclause', index=117, number=117,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='messagedateclause', index=118, number=118,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='messagetimeclause', index=119, number=119,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='statuskeyclause', index=120, number=120,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symbolicdestinationclause', index=121, number=121,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symbolicqueueclause', index=122, number=122,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symbolicsourceclause', index=123, number=123,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symbolicterminalclause', index=124, number=124,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symbolicsubqueueclause', index=125, number=125,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='textlengthclause', index=126, number=126,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='localstoragesection', index=127, number=127,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screensection', index=128, number=128,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionentry', index=129, number=129,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionblankclause', index=130, number=130,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionbellclause', index=131, number=131,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionblinkclause', index=132, number=132,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptioneraseclause', index=133, number=133,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionlightclause', index=134, number=134,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptiongridclause', index=135, number=135,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionreversevideoclause', index=136, number=136,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionunderlineclause', index=137, number=137,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionsizeclause', index=138, number=138,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionlineclause', index=139, number=139,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptioncolumnclause', index=140, number=140,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionforegroundcolorclause', index=141, number=141,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionbackgroundcolorclause', index=142, number=142,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptioncontrolclause', index=143, number=143,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionvalueclause', index=144, number=144,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionpictureclause', index=145, number=145,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionfromclause', index=146, number=146,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptiontoclause', index=147, number=147,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionusingclause', index=148, number=148,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionusageclause', index=149, number=149,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionblankwhenzeroclause', index=150, number=150,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionjustifiedclause', index=151, number=151,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionsignclause', index=152, number=152,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionautoclause', index=153, number=153,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionsecureclause', index=154, number=154,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionrequiredclause', index=155, number=155,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionpromptclause', index=156, number=156,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionpromptoccursclause', index=157, number=157,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionfullclause', index=158, number=158,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screendescriptionzerofillclause', index=159, number=159,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportsection', index=160, number=160,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescription', index=161, number=161,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionentry', index=162, number=162,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionglobalclause', index=163, number=163,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionpagelimitclause', index=164, number=164,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionheadingclause', index=165, number=165,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionfirstdetailclause', index=166, number=166,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionlastdetailclause', index=167, number=167,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportdescriptionfootingclause', index=168, number=168,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupdescriptionentry', index=169, number=169,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupdescriptionentryformat1', index=170, number=170,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupdescriptionentryformat2', index=171, number=171,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupdescriptionentryformat3', index=172, number=172,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupblankwhenzeroclause', index=173, number=173,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupcolumnnumberclause', index=174, number=174,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupindicateclause', index=175, number=175,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupjustifiedclause', index=176, number=176,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouplinenumberclause', index=177, number=177,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouplinenumbernextpage', index=178, number=178,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouplinenumberplus', index=179, number=179,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupnextgroupclause', index=180, number=180,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupnextgroupplus', index=181, number=181,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupnextgroupnextpage', index=182, number=182,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouppictureclause', index=183, number=183,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupresetclause', index=184, number=184,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupsignclause', index=185, number=185,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupsourceclause', index=186, number=186,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupsumclause', index=187, number=187,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypeclause', index=188, number=188,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypereportheading', index=189, number=189,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypepageheading', index=190, number=190,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypecontrolheading', index=191, number=191,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypedetail', index=192, number=192,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypecontrolfooting', index=193, number=193,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupusageclause', index=194, number=194,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypepagefooting', index=195, number=195,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgrouptypereportfooting', index=196, number=196,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportgroupvalueclause', index=197, number=197,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='programlibrarysection', index=198, number=198,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='librarydescriptionentry', index=199, number=199,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='librarydescriptionentryformat1', index=200, number=200,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='librarydescriptionentryformat2', index=201, number=201,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryattributeclauseformat1', index=202, number=202,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryattributeclauseformat2', index=203, number=203,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryattributefunction', index=204, number=204,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryattributeparameter', index=205, number=205,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryattributetitle', index=206, number=206,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedureclauseformat1', index=207, number=207,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedureclauseformat2', index=208, number=208,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedureforclause', index=209, number=209,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryproceduregivingclause', index=210, number=210,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedureusingclause', index=211, number=211,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedureusingname', index=212, number=212,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedurewithclause', index=213, number=213,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryentryprocedurewithname', index=214, number=214,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryiscommonclause', index=215, number=215,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryisglobalclause', index=216, number=216,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescriptionentry', index=217, number=217,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescriptionentryformat1', index=218, number=218,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescriptionentryformat2', index=219, number=219,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescriptionentryformat3', index=220, number=220,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescriptionentryexecsql', index=221, number=221,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataalignedclause', index=222, number=222,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datablankwhenzeroclause', index=223, number=223,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datacommonownlocalclause', index=224, number=224,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataexternalclause', index=225, number=225,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataglobalclause', index=226, number=226,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataintegerstringclause', index=227, number=227,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datajustifiedclause', index=228, number=228,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataoccursclause', index=229, number=229,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataoccursto', index=230, number=230,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataoccurssort', index=231, number=231,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datapictureclause', index=232, number=232,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='picturestring', index=233, number=233,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='picturechars', index=234, number=234,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='picturecardinality', index=235, number=235,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datareceivedbyclause', index=236, number=236,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datarecordareaclause', index=237, number=237,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataredefinesclause', index=238, number=238,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datarenamesclause', index=239, number=239,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datasignclause', index=240, number=240,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datasynchronizedclause', index=241, number=241,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datathreadlocalclause', index=242, number=242,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datatypeclause', index=243, number=243,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datatypedefclause', index=244, number=244,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datausageclause', index=245, number=245,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datausingclause', index=246, number=246,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datavalueclause', index=247, number=247,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datavalueinterval', index=248, number=248,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datavalueintervalfrom', index=249, number=249,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datavalueintervalto', index=250, number=250,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datawithlowerboundsclause', index=251, number=251,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivision', index=252, number=252,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionusingclause', index=253, number=253,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisiongivingclause', index=254, number=254,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionusingparameter', index=255, number=255,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionbyreferencephrase', index=256, number=256,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionbyreference', index=257, number=257,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionbyvaluephrase', index=258, number=258,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionbyvalue', index=259, number=259,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredeclaratives', index=260, number=260,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredeclarative', index=261, number=261,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduresectionheader', index=262, number=262,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduredivisionbody', index=263, number=263,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='proceduresection', index=264, number=264,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='paragraphs', index=265, number=265,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='paragraph', index=266, number=266,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sentence', index=267, number=267,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='statement', index=268, number=268,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='acceptstatement', index=269, number=269,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='acceptfromdatestatement', index=270, number=270,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='acceptfrommnemonicstatement', index=271, number=271,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='acceptfromescapekeystatement', index=272, number=272,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='acceptmessagecountstatement', index=273, number=273,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addstatement', index=274, number=274,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addtostatement', index=275, number=275,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addtogivingstatement', index=276, number=276,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addcorrespondingstatement', index=277, number=277,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addfrom', index=278, number=278,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addto', index=279, number=279,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addtogiving', index=280, number=280,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='addgiving', index=281, number=281,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alteredgoto', index=282, number=282,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alterstatement', index=283, number=283,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alterproceedto', index=284, number=284,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callstatement', index=285, number=285,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callusingphrase', index=286, number=286,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callusingparameter', index=287, number=287,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbyreferencephrase', index=288, number=288,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbyreference', index=289, number=289,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbyvaluephrase', index=290, number=290,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbyvalue', index=291, number=291,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbycontentphrase', index=292, number=292,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callbycontent', index=293, number=293,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='callgivingphrase', index=294, number=294,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cancelstatement', index=295, number=295,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cancelcall', index=296, number=296,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closestatement', index=297, number=297,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closefile', index=298, number=298,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closereelunitstatement', index=299, number=299,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closerelativestatement', index=300, number=300,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closeportfileiostatement', index=301, number=301,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closeportfileiousing', index=302, number=302,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closeportfileiousingclosedisposition', index=303, number=303,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closeportfileiousingassociateddata', index=304, number=304,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='closeportfileiousingassociateddatalength', index=305, number=305,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='computestatement', index=306, number=306,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='computestore', index=307, number=307,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='continuestatement', index=308, number=308,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='deletestatement', index=309, number=309,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='disablestatement', index=310, number=310,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='displaystatement', index=311, number=311,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='displayoperand', index=312, number=312,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='displayat', index=313, number=313,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='displayupon', index=314, number=314,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='displaywith', index=315, number=315,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dividestatement', index=316, number=316,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='divideintostatement', index=317, number=317,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='divideintogivingstatement', index=318, number=318,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dividebygivingstatement', index=319, number=319,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dividegivingphrase', index=320, number=320,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='divideinto', index=321, number=321,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dividegiving', index=322, number=322,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='divideremainder', index=323, number=323,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='enablestatement', index=324, number=324,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='entrystatement', index=325, number=325,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatestatement', index=326, number=326,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluateselect', index=327, number=327,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatealsoselect', index=328, number=328,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatewhenphrase', index=329, number=329,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatewhen', index=330, number=330,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatecondition', index=331, number=331,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatethrough', index=332, number=332,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatealsocondition', index=333, number=333,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatewhenother', index=334, number=334,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='evaluatevalue', index=335, number=335,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='execcicsstatement', index=336, number=336,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='execsqlstatement', index=337, number=337,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='execsqlimsstatement', index=338, number=338,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='exhibitstatement', index=339, number=339,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='exhibitoperand', index=340, number=340,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='exitstatement', index=341, number=341,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='generatestatement', index=342, number=342,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='gobackstatement', index=343, number=343,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='gotostatement', index=344, number=344,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='gotostatementsimple', index=345, number=345,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='gotodependingonstatement', index=346, number=346,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ifstatement', index=347, number=347,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ifthen', index=348, number=348,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ifelse', index=349, number=349,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='initializestatement', index=350, number=350,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='initializereplacingphrase', index=351, number=351,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='initializereplacingby', index=352, number=352,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='initiatestatement', index=353, number=353,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectstatement', index=354, number=354,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspecttallyingphrase', index=355, number=355,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectreplacingphrase', index=356, number=356,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspecttallyingreplacingphrase', index=357, number=357,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectconvertingphrase', index=358, number=358,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectfor', index=359, number=359,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectcharacters', index=360, number=360,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectreplacingcharacters', index=361, number=361,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectallleadings', index=362, number=362,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectreplacingallleadings', index=363, number=363,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectallleading', index=364, number=364,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectreplacingallleading', index=365, number=365,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectby', index=366, number=366,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectto', index=367, number=367,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inspectbeforeafter', index=368, number=368,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergestatement', index=369, number=369,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergeonkeyclause', index=370, number=370,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergecollatingsequencephrase', index=371, number=371,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergecollatingalphanumeric', index=372, number=372,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergecollatingnational', index=373, number=373,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergeusing', index=374, number=374,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergeoutputprocedurephrase', index=375, number=375,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergeoutputthrough', index=376, number=376,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergegivingphrase', index=377, number=377,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mergegiving', index=378, number=378,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='movestatement', index=379, number=379,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='movetostatement', index=380, number=380,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='movetosendingarea', index=381, number=381,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='movecorrespondingtostatement', index=382, number=382,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='movecorrespondingtosendingarea', index=383, number=383,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplystatement', index=384, number=384,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplyregular', index=385, number=385,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplyregularoperand', index=386, number=386,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplygiving', index=387, number=387,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplygivingoperand', index=388, number=388,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multiplygivingresult', index=389, number=389,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openstatement', index=390, number=390,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openinputstatement', index=391, number=391,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openinput', index=392, number=392,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openoutputstatement', index=393, number=393,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openoutput', index=394, number=394,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openiostatement', index=395, number=395,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='openextendstatement', index=396, number=396,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performstatement', index=397, number=397,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performinlinestatement', index=398, number=398,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performprocedurestatement', index=399, number=399,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performtype', index=400, number=400,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performtimes', index=401, number=401,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performuntil', index=402, number=402,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performvarying', index=403, number=403,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performvaryingclause', index=404, number=404,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performvaryingphrase', index=405, number=405,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performafter', index=406, number=406,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performfrom', index=407, number=407,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performby', index=408, number=408,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='performtestclause', index=409, number=409,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='purgestatement', index=410, number=410,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='readstatement', index=411, number=411,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='readinto', index=412, number=412,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='readwith', index=413, number=413,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='readkey', index=414, number=414,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivestatement', index=415, number=415,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivefromstatement', index=416, number=416,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivefrom', index=417, number=417,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receiveintostatement', index=418, number=418,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivenodata', index=419, number=419,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivewithdata', index=420, number=420,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivebefore', index=421, number=421,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivewith', index=422, number=422,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivethread', index=423, number=423,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivesize', index=424, number=424,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='receivestatus', index=425, number=425,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='releasestatement', index=426, number=426,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='returnstatement', index=427, number=427,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='returninto', index=428, number=428,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='rewritestatement', index=429, number=429,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='rewritefrom', index=430, number=430,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='searchstatement', index=431, number=431,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='searchvarying', index=432, number=432,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='searchwhen', index=433, number=433,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendstatement', index=434, number=434,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendstatementsync', index=435, number=435,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendstatementasync', index=436, number=436,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendfromphrase', index=437, number=437,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendwithphrase', index=438, number=438,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendreplacingphrase', index=439, number=439,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendadvancingphrase', index=440, number=440,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendadvancingpage', index=441, number=441,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendadvancinglines', index=442, number=442,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sendadvancingmnemonic', index=443, number=443,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='setstatement', index=444, number=444,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='settostatement', index=445, number=445,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='setupdownbystatement', index=446, number=446,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='setto', index=447, number=447,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='settovalue', index=448, number=448,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='setbyvalue', index=449, number=449,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortstatement', index=450, number=450,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortonkeyclause', index=451, number=451,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortduplicatesphrase', index=452, number=452,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortcollatingsequencephrase', index=453, number=453,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortcollatingalphanumeric', index=454, number=454,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortcollatingnational', index=455, number=455,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortinputprocedurephrase', index=456, number=456,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortinputthrough', index=457, number=457,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortusing', index=458, number=458,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortoutputprocedurephrase', index=459, number=459,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortoutputthrough', index=460, number=460,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortgivingphrase', index=461, number=461,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sortgiving', index=462, number=462,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='startstatement', index=463, number=463,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='startkey', index=464, number=464,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stopstatement', index=465, number=465,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringstatement', index=466, number=466,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringsendingphrase', index=467, number=467,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringsending', index=468, number=468,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringdelimitedbyphrase', index=469, number=469,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringforphrase', index=470, number=470,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringintophrase', index=471, number=471,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stringwithpointerphrase', index=472, number=472,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractstatement', index=473, number=473,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractfromstatement', index=474, number=474,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractfromgivingstatement', index=475, number=475,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractcorrespondingstatement', index=476, number=476,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractsubtrahend', index=477, number=477,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractminuend', index=478, number=478,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractminuendgiving', index=479, number=479,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractgiving', index=480, number=480,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subtractminuendcorresponding', index=481, number=481,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='terminatestatement', index=482, number=482,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringstatement', index=483, number=483,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringsendingphrase', index=484, number=484,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringdelimitedbyphrase', index=485, number=485,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringorallphrase', index=486, number=486,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringintophrase', index=487, number=487,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringinto', index=488, number=488,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringdelimiterin', index=489, number=489,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringcountin', index=490, number=490,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringwithpointerphrase', index=491, number=491,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unstringtallyingphrase', index=492, number=492,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='usestatement', index=493, number=493,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='useafterclause', index=494, number=494,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='useafteron', index=495, number=495,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='usedebugclause', index=496, number=496,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='usedebugon', index=497, number=497,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writestatement', index=498, number=498,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writefromphrase', index=499, number=499,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writeadvancingphrase', index=500, number=500,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writeadvancingpage', index=501, number=501,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writeadvancinglines', index=502, number=502,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writeadvancingmnemonic', index=503, number=503,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writeatendofpagephrase', index=504, number=504,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='writenotatendofpagephrase', index=505, number=505,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='atendphrase', index=506, number=506,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='notatendphrase', index=507, number=507,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='invalidkeyphrase', index=508, number=508,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='notinvalidkeyphrase', index=509, number=509,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='onoverflowphrase', index=510, number=510,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='notonoverflowphrase', index=511, number=511,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='onsizeerrorphrase', index=512, number=512,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='notonsizeerrorphrase', index=513, number=513,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='onexceptionclause', index=514, number=514,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='notonexceptionclause', index=515, number=515,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='arithmeticexpression', index=516, number=516,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='plusminus', index=517, number=517,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multdivs', index=518, number=518,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='multdiv', index=519, number=519,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='powers', index=520, number=520,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='power', index=521, number=521,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='basis', index=522, number=522,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='condition', index=523, number=523,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='andorcondition', index=524, number=524,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='combinablecondition', index=525, number=525,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='simplecondition', index=526, number=526,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classcondition', index=527, number=527,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='conditionnamereference', index=528, number=528,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='conditionnamesubscriptreference', index=529, number=529,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationcondition', index=530, number=530,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationsigncondition', index=531, number=531,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationarithmeticcomparison', index=532, number=532,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationcombinedcomparison', index=533, number=533,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationcombinedcondition', index=534, number=534,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='relationaloperator', index=535, number=535,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='abbreviation', index=536, number=536,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='identifier', index=537, number=537,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='tablecall', index=538, number=538,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='functioncall', index=539, number=539,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='referencemodifier', index=540, number=540,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='characterposition', index=541, number=541,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='length', index=542, number=542,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subscript', index=543, number=543,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='argument', index=544, number=544,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifieddataname', index=545, number=545,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifieddatanameformat1', index=546, number=546,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifieddatanameformat2', index=547, number=547,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifieddatanameformat3', index=548, number=548,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifieddatanameformat4', index=549, number=549,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='qualifiedindata', index=550, number=550,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='indata', index=551, number=551,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='infile', index=552, number=552,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inmnemonic', index=553, number=553,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insection', index=554, number=554,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inlibrary', index=555, number=555,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='intable', index=556, number=556,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='alphabetname', index=557, number=557,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assignmentname', index=558, number=558,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='basisname', index=559, number=559,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cdname', index=560, number=560,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classname', index=561, number=561,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='computername', index=562, number=562,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='conditionname', index=563, number=563,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dataname', index=564, number=564,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datadescname', index=565, number=565,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='environmentname', index=566, number=566,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='filename', index=567, number=567,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='functionname', index=568, number=568,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='indexname', index=569, number=569,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='languagename', index=570, number=570,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='libraryname', index=571, number=571,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='localname', index=572, number=572,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mnemonicname', index=573, number=573,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='paragraphname', index=574, number=574,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='procedurename', index=575, number=575,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='programname', index=576, number=576,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='recordname', index=577, number=577,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reportname', index=578, number=578,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='routinename', index=579, number=579,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='screenname', index=580, number=580,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sectionname', index=581, number=581,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='systemname', index=582, number=582,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='symboliccharacter', index=583, number=583,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='textname', index=584, number=584,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cobolword', index=585, number=585,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='literal', index=586, number=586,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='booleanliteral', index=587, number=587,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='numericliteral', index=588, number=588,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='integerliteral', index=589, number=589,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cicsdfhrespliteral', index=590, number=590,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='cicsdfhvalueliteral', index=591, number=591,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='figurativeconstant', index=592, number=592,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='specialregister', index=593, number=593,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='commententry', index=594, number=594,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=5154,
  serialized_end=20260,
)
_sym_db.RegisterEnumDescriptor(_COBOL85KIND)

Cobol85Kind = enum_type_wrapper.EnumTypeWrapper(_COBOL85KIND)
_PYTHON3KIND = _descriptor.EnumDescriptor(
  name='Python3Kind',
  full_name='fast.Python3Kind',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='single_input', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='file_input', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='eval_input', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='decorator', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='decorators', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='decorated', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='async_funcdef', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='funcdef', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='parameters', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='typedargslist', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='tfpdef', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='varargslist', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='vfpdef', index=12, number=12,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='stmt', index=13, number=13,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='simple_stmt', index=14, number=14,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='small_stmt', index=15, number=15,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='expr_stmt', index=16, number=16,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='annassign', index=17, number=17,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='testlist_star_expr', index=18, number=18,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='augassign', index=19, number=19,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='del_stmt', index=20, number=20,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='pass_stmt', index=21, number=21,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='flow_stmt', index=22, number=22,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='break_stmt', index=23, number=23,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='continue_stmt', index=24, number=24,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='return_stmt', index=25, number=25,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='yield_stmt', index=26, number=26,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='raise_stmt', index=27, number=27,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='import_stmt', index=28, number=28,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='import_name', index=29, number=29,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='import_from', index=30, number=30,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='import_as_name', index=31, number=31,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dotted_as_name', index=32, number=32,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='import_as_names', index=33, number=33,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dotted_as_names', index=34, number=34,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dotted_name', index=35, number=35,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='global_stmt', index=36, number=36,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='nonlocal_stmt', index=37, number=37,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assert_stmt', index=38, number=38,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='compound_stmt', index=39, number=39,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='async_stmt', index=40, number=40,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='if_stmt', index=41, number=41,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='while_stmt', index=42, number=42,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='for_stmt', index=43, number=43,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='try_stmt', index=44, number=44,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='with_stmt', index=45, number=45,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='with_item', index=46, number=46,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='except_clause', index=47, number=47,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='suite', index=48, number=48,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='test', index=49, number=49,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='test_nocond', index=50, number=50,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='lambdef', index=51, number=51,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='lambdef_nocond', index=52, number=52,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='or_test', index=53, number=53,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='and_test', index=54, number=54,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='not_test', index=55, number=55,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='comparison', index=56, number=56,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='comp_op', index=57, number=57,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='star_expr', index=58, number=58,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='expr', index=59, number=59,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='xor_expr', index=60, number=60,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='and_expr', index=61, number=61,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='shift_expr', index=62, number=62,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='arith_expr', index=63, number=63,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='term', index=64, number=64,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='factor', index=65, number=65,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='python_power', index=66, number=66,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='atom_expr', index=67, number=67,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='atom', index=68, number=68,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='testlist_comp', index=69, number=69,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='trailer', index=70, number=70,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subscriptlist', index=71, number=71,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='python_subscript', index=72, number=72,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='sliceop', index=73, number=73,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='exprlist', index=74, number=74,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='testlist', index=75, number=75,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dictorsetmaker', index=76, number=76,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='classdef', index=77, number=77,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='arglist', index=78, number=78,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='python_argument', index=79, number=79,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='comp_iter', index=80, number=80,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='comp_for', index=81, number=81,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='comp_if', index=82, number=82,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='encoding_decl', index=83, number=83,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='yield_expr', index=84, number=84,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='yield_arg', index=85, number=85,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=20263,
  serialized_end=21629,
)
_sym_db.RegisterEnumDescriptor(_PYTHON3KIND)

Python3Kind = enum_type_wrapper.EnumTypeWrapper(_PYTHON3KIND)
_SMALIKIND = _descriptor.EnumDescriptor(
  name='SmaliKind',
  full_name='fast.SmaliKind',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='smali_file', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='class_spec', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='super_spec', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='implements_spec', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='source_spec', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='access_list', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='field', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='method', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='statements_and_directives', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ordered_method_item', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='registers_directive', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='param_list_or_id', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='simple_name', index=12, number=12,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='member_name', index=13, number=13,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='method_prototype', index=14, number=14,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='param_list_or_id_primitive_type', index=15, number=15,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='param_list', index=16, number=16,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='array_descriptor', index=17, number=17,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='type_descriptor', index=18, number=18,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='nonvoid_type_descriptor', index=19, number=19,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='reference_type_descriptor', index=20, number=20,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='integer_literal', index=21, number=21,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='float_literal', index=22, number=22,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='double_literal', index=23, number=23,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='smali_literal', index=24, number=24,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='parsed_integer_literal', index=25, number=25,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='integral_literal', index=26, number=26,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='fixed_32bit_literal', index=27, number=27,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='fixed_literal', index=28, number=28,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='array_literal', index=29, number=29,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='annotation_element', index=30, number=30,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='annotation', index=31, number=31,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subannotation', index=32, number=32,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='enum_literal', index=33, number=33,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='type_field_method_literal', index=34, number=34,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='method_reference', index=35, number=35,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='field_reference', index=36, number=36,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='label', index=37, number=37,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='label_ref', index=38, number=38,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='register_list', index=39, number=39,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='register_range', index=40, number=40,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='verification_error_reference', index=41, number=41,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='catch_directive', index=42, number=42,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='catchall_directive', index=43, number=43,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='parameter_directive', index=44, number=44,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='debug_directive', index=45, number=45,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='line_directive', index=46, number=46,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='local_directive', index=47, number=47,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='end_local_directive', index=48, number=48,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='restart_local_directive', index=49, number=49,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='prologue_directive', index=50, number=50,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='epilogue_directive', index=51, number=51,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='source_directive', index=52, number=52,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='instruction_format12x', index=53, number=53,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='instruction_format22s', index=54, number=54,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='instruction_format31i', index=55, number=55,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='instruction', index=56, number=56,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format10t', index=57, number=57,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format10x', index=58, number=58,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format10x_odex', index=59, number=59,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format11n', index=60, number=60,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format11x', index=61, number=61,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format12x', index=62, number=62,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format20bc', index=63, number=63,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format20t', index=64, number=64,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21c_field', index=65, number=65,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21c_field_odex', index=66, number=66,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21c_string', index=67, number=67,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21c_type', index=68, number=68,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21ih', index=69, number=69,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21lh', index=70, number=70,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21s', index=71, number=71,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format21t', index=72, number=72,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22b', index=73, number=73,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22c_field', index=74, number=74,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22c_field_odex', index=75, number=75,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22c_type', index=76, number=76,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22cs_field', index=77, number=77,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22s', index=78, number=78,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22t', index=79, number=79,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format22x', index=80, number=80,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format23x', index=81, number=81,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format30t', index=82, number=82,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format31c', index=83, number=83,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format31i', index=84, number=84,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format31t', index=85, number=85,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format32x', index=86, number=86,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format35c_method', index=87, number=87,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format35c_type', index=88, number=88,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format35c_method_odex', index=89, number=89,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format35mi_method', index=90, number=90,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format35ms_method', index=91, number=91,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format3rc_method', index=92, number=92,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format3rc_method_odex', index=93, number=93,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format3rc_type', index=94, number=94,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format3rmi_method', index=95, number=95,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format3rms_method', index=96, number=96,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format45cc_method', index=97, number=97,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format4rcc_method', index=98, number=98,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_format51l', index=99, number=99,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_array_data_directive', index=100, number=100,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_packed_switch_directive', index=101, number=101,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='insn_sparse_switch_directive', index=102, number=102,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=21632,
  serialized_end=24001,
)
_sym_db.RegisterEnumDescriptor(_SMALIKIND)

SmaliKind = enum_type_wrapper.EnumTypeWrapper(_SMALIKIND)
_SOLIDITYKIND = _descriptor.EnumDescriptor(
  name='SolidityKind',
  full_name='fast.SolidityKind',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='sourceunit', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='pragmadirective', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='pragmaname', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='pragmavalue', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='version', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='versionoperator', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='versionconstraint', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='importdeclaration', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='importdirective', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='contractdefinition', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inheritancespecifier', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='contractpart', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='statevariabledeclaration', index=12, number=12,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='usingfordeclaration', index=13, number=13,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='structdefinition', index=14, number=14,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='modifierdefinition', index=15, number=15,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='modifierinvocation', index=16, number=16,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='functiondefinition', index=17, number=17,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='returnparameters', index=18, number=18,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='modifierlist', index=19, number=19,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='eventdefinition', index=20, number=20,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='enumvalue', index=21, number=21,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='enumdefinition', index=22, number=22,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='indexedparameterlist', index=23, number=23,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='indexedparameter', index=24, number=24,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='parameterlist', index=25, number=25,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='parameter', index=26, number=26,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='typenamelist', index=27, number=27,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='unnamedparameter', index=28, number=28,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='variabledeclaration', index=29, number=29,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='typename', index=30, number=30,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='userdefinedtypename', index=31, number=31,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='mapping', index=32, number=32,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='functiontypename', index=33, number=33,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='storagelocation', index=34, number=34,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='statemutability', index=35, number=35,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='block', index=36, number=36,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='solidity_statement', index=37, number=37,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='expressionstatement', index=38, number=38,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='solidity_ifstatement', index=39, number=39,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='whilestatement', index=40, number=40,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='simplestatement', index=41, number=41,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='forstatement', index=42, number=42,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='inlineassemblystatement', index=43, number=43,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='dowhilestatement', index=44, number=44,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='solidity_continuestatement', index=45, number=45,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='breakstatement', index=46, number=46,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='solidity_returnstatement', index=47, number=47,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='throwstatement', index=48, number=48,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='variabledeclarationstatement', index=49, number=49,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='identifierlist', index=50, number=50,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='elementarytypename', index=51, number=51,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='expression', index=52, number=52,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='primaryexpression', index=53, number=53,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='expressionlist', index=54, number=54,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='namevaluelist', index=55, number=55,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='namevalue', index=56, number=56,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='functioncallarguments', index=57, number=57,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyblock', index=58, number=58,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyitem', index=59, number=59,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyexpression', index=60, number=60,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblycall', index=61, number=61,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblylocaldefinition', index=62, number=62,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyassignment', index=63, number=63,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyidentifierorlist', index=64, number=64,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyidentifierlist', index=65, number=65,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyrightassignment', index=66, number=66,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='labeldefinition', index=67, number=67,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyswitch', index=68, number=68,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblycase', index=69, number=69,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyfunctiondefinition', index=70, number=70,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyfor', index=71, number=71,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='assemblyliteral', index=72, number=72,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='subassembly', index=73, number=73,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='datasize', index=74, number=74,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='linkersymbol', index=75, number=75,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='tupleexpression', index=76, number=76,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='elementarytypenameexpression', index=77, number=77,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='numberliteral', index=78, number=78,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='solidity_identifier', index=79, number=79,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=24004,
  serialized_end=25757,
)
_sym_db.RegisterEnumDescriptor(_SOLIDITYKIND)

SolidityKind = enum_type_wrapper.EnumTypeWrapper(_SOLIDITYKIND)
_SRCMLKIND = _descriptor.EnumDescriptor(
  name='SrcmlKind',
  full_name='fast.SrcmlKind',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='UNIT_KIND', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECL', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECL_STMT', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INIT', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXPR', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXPR_STMT', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='COMMENT', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CALL', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONTROL', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INCR', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NONE', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VARIABLE', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION', index=12, number=12,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_DECL', index=13, number=13,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTRUCTOR', index=14, number=14,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTRUCTOR_DECL', index=15, number=15,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DESTRUCTOR', index=16, number=16,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DESTRUCTOR_DECL', index=17, number=17,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MACRO', index=18, number=18,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SINGLE_MACRO', index=19, number=19,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NULLOPERATOR', index=20, number=20,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM_DEFN', index=21, number=21,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM_DECL', index=22, number=22,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GLOBAL_ATTRIBUTE', index=23, number=23,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROPERTY_ACCESSOR', index=24, number=24,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROPERTY_ACCESSOR_DECL', index=25, number=25,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXPRESSION', index=26, number=26,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_DEFN', index=27, number=27,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_DECL', index=28, number=28,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNION_DEFN', index=29, number=29,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNION_DECL', index=30, number=30,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRUCT_DEFN', index=31, number=31,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRUCT_DECL', index=32, number=32,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTERFACE_DEFN', index=33, number=33,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTERFACE_DECL', index=34, number=34,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ACCESS_REGION', index=35, number=35,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='USING', index=36, number=36,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OPERATOR_FUNCTION', index=37, number=37,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OPERATOR_FUNCTION_DECL', index=38, number=38,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EVENT', index=39, number=39,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROPERTY', index=40, number=40,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ANNOTATION_DEFN', index=41, number=41,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GLOBAL_TEMPLATE', index=42, number=42,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNIT', index=43, number=43,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TART_ELEMENT_TOKEN', index=44, number=44,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NOP', index=45, number=45,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRING', index=46, number=46,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CHAR', index=47, number=47,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LITERAL', index=48, number=48,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='BOOLEAN', index=49, number=49,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NULL2', index=50, number=50,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='COMPLEX', index=51, number=51,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OPERATOR', index=52, number=52,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MODIFIER', index=53, number=53,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NAME', index=54, number=54,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ONAME', index=55, number=55,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CNAME', index=56, number=56,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPE', index=57, number=57,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPEPREV', index=58, number=58,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONDITION', index=59, number=59,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='BLOCK', index=60, number=60,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PSEUDO_BLOCK', index=61, number=61,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INDEX', index=62, number=62,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM', index=63, number=63,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM_DECLARATION', index=64, number=64,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IF_STATEMENT', index=65, number=65,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TERNARY', index=66, number=66,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THEN', index=67, number=67,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ELSE', index=68, number=68,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ELSEIF', index=69, number=69,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='WHILE_STATEMENT', index=70, number=70,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DO_STATEMENT', index=71, number=71,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_STATEMENT', index=72, number=72,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOREACH_STATEMENT', index=73, number=73,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_CONTROL', index=74, number=74,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_INITIALIZATION', index=75, number=75,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_CONDITION', index=76, number=76,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_INCREMENT', index=77, number=77,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR_LIKE_CONTROL', index=78, number=78,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXPRESSION_STATEMENT', index=79, number=79,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_CALL', index=80, number=81,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECLARATION_STATEMENT', index=81, number=82,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECLARATION', index=82, number=83,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECLARATION_INITIALIZATION', index=83, number=84,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECLARATION_RANGE', index=84, number=85,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='RANGE', index=85, number=86,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GOTO_STATEMENT', index=86, number=87,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONTINUE_STATEMENT', index=87, number=88,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='BREAK_STATEMENT', index=88, number=89,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LABEL_STATEMENT', index=89, number=90,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LABEL', index=90, number=91,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SWITCH', index=91, number=92,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CASE', index=92, number=93,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DEFAULT', index=93, number=94,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_DEFINITION', index=94, number=95,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_DECLARATION', index=95, number=96,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LAMBDA', index=96, number=97,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_LAMBDA', index=97, number=98,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_SPECIFIER', index=98, number=99,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='RETURN_STATEMENT', index=99, number=100,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PARAMETER_LIST', index=100, number=101,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PARAMETER', index=101, number=102,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='KRPARAMETER_LIST', index=102, number=103,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='KRPARAMETER', index=103, number=104,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ARGUMENT_LIST', index=104, number=105,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ARGUMENT', index=105, number=106,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PSEUDO_PARAMETER_LIST', index=106, number=107,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INDEXER_PARAMETER_LIST', index=107, number=108,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS', index=108, number=109,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_DECLARATION', index=109, number=110,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRUCT', index=110, number=111,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRUCT_DECLARATION', index=111, number=112,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNION', index=112, number=113,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNION_DECLARATION', index=113, number=114,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DERIVATION_LIST', index=114, number=115,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PUBLIC_ACCESS', index=115, number=116,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PUBLIC_ACCESS_DEFAULT', index=116, number=117,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PRIVATE_ACCESS', index=117, number=118,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PRIVATE_ACCESS_DEFAULT', index=118, number=119,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTECTED_ACCESS', index=119, number=120,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTECTED_ACCESS_DEFAULT', index=120, number=121,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MEMBER_INIT_LIST', index=121, number=122,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MEMBER_INITIALIZATION_LIST', index=122, number=123,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MEMBER_INITIALIZATION', index=123, number=124,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTRUCTOR_DEFINITION', index=124, number=125,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTRUCTOR_DECLARATION', index=125, number=126,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DESTRUCTOR_DEFINITION', index=126, number=127,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DESTRUCTOR_DECLARATION', index=127, number=128,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FRIEND', index=128, number=129,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_SPECIFIER', index=129, number=130,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TRY_BLOCK', index=130, number=131,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CATCH_BLOCK', index=131, number=132,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FINALLY_BLOCK', index=132, number=133,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THROW_STATEMENT', index=133, number=134,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THROW_SPECIFIER', index=134, number=135,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THROW_SPECIFIER_JAVA', index=135, number=136,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TEMPLATE', index=136, number=137,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_ARGUMENT', index=137, number=138,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_ARGUMENT_LIST', index=138, number=139,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TEMPLATE_PARAMETER', index=139, number=140,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TEMPLATE_PARAMETER_LIST', index=140, number=141,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_PARAMETER', index=141, number=142,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_PARAMETER_LIST', index=142, number=143,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPEDEF', index=143, number=144,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ASM', index=144, number=145,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MACRO_CALL', index=145, number=146,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SIZEOF_CALL', index=146, number=147,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXTERN', index=147, number=148,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NAMESPACE', index=148, number=149,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='USING_DIRECTIVE', index=149, number=150,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DIRECTIVE', index=150, number=151,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATOMIC', index=151, number=152,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STATIC_ASSERT_STATEMENT', index=152, number=153,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_SELECTION', index=153, number=154,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_SELECTOR', index=154, number=155,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_ASSOCIATION_LIST', index=155, number=156,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GENERIC_ASSOCIATION', index=156, number=157,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ALIGNAS', index=157, number=158,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DECLTYPE', index=158, number=159,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CAPTURE', index=159, number=160,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LAMBDA_CAPTURE', index=160, number=161,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NOEXCEPT', index=161, number=162,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPENAME', index=162, number=163,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ALIGNOF', index=163, number=164,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPEID', index=164, number=165,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SIZEOF_PACK', index=165, number=166,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM_CLASS', index=166, number=167,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENUM_CLASS_DECLARATION', index=167, number=168,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REF_QUALIFIER', index=168, number=171,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SIGNAL_ACCESS', index=169, number=172,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOREVER_STATEMENT', index=170, number=173,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EMIT_STATEMENT', index=171, number=174,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_DIRECTIVE', index=172, number=175,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_FILENAME', index=173, number=176,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FILE', index=174, number=177,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NUMBER', index=175, number=178,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_NUMBER', index=176, number=179,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_LITERAL', index=177, number=180,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_MACRO_DEFN', index=178, number=181,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_MACRO_VALUE', index=179, number=182,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR', index=180, number=183,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_ERROR', index=181, number=184,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_WARNING', index=182, number=185,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_PRAGMA', index=183, number=186,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_INCLUDE', index=184, number=187,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_DEFINE', index=185, number=188,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_UNDEF', index=186, number=189,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_LINE', index=187, number=190,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_IF', index=188, number=191,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_IFDEF', index=189, number=192,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_IFNDEF', index=190, number=193,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_THEN', index=191, number=194,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_ELSE', index=192, number=195,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_ELIF', index=193, number=196,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_EMPTY', index=194, number=197,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_REGION', index=195, number=198,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_ENDREGION', index=196, number=199,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='USING_STMT', index=197, number=200,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ESCAPE', index=198, number=201,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VALUE', index=199, number=202,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_IMPORT', index=200, number=203,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CPP_ENDIF', index=201, number=204,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MARKER', index=202, number=205,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_PARSE', index=203, number=206,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERROR_MODE', index=204, number=207,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IMPLEMENTS', index=205, number=208,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXTENDS', index=206, number=209,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IMPORT', index=207, number=210,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PACKAGE', index=208, number=211,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ASSERT_STATEMENT', index=209, number=212,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTERFACE', index=210, number=213,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTERFACE_DECLARATION', index=211, number=214,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SYNCHRONIZED_STATEMENT', index=212, number=215,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ANNOTATION', index=213, number=216,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STATIC_BLOCK', index=214, number=218,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CHECKED_STATEMENT', index=215, number=219,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNCHECKED_STATEMENT', index=216, number=220,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATTRIBUTE', index=217, number=221,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TARGET', index=218, number=222,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNSAFE_STATEMENT', index=219, number=223,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LOCK_STATEMENT', index=220, number=224,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FIXED_STATEMENT', index=221, number=225,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TYPEOF', index=222, number=226,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='USING_STATEMENT', index=223, number=227,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FUNCTION_DELEGATE', index=224, number=228,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTRAINT', index=225, number=230,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LINQ', index=226, number=231,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FROM', index=227, number=232,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='WHERE', index=228, number=233,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SELECT', index=229, number=234,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LET', index=230, number=235,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ORDERBY', index=231, number=236,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='JOIN', index=232, number=237,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GROUP', index=233, number=238,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IN', index=234, number=239,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ON', index=235, number=240,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EQUALS', index=236, number=241,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='BY', index=237, number=242,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTO', index=238, number=243,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EMPTY', index=239, number=244,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EMPTY_STMT', index=240, number=245,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='RECEIVER', index=241, number=246,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MESSAGE', index=242, number=247,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SELECTOR', index=243, number=248,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTOCOL_LIST', index=244, number=249,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CATEGORY', index=245, number=250,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTOCOL', index=246, number=251,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REQUIRED_DEFAULT', index=247, number=252,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REQUIRED', index=248, number=253,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OPTIONAL', index=249, number=254,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATTRIBUTE_LIST', index=250, number=256,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SYNTHESIZE', index=251, number=257,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DYNAMIC', index=252, number=258,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENCODE', index=253, number=259,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='AUTORELEASEPOOL', index=254, number=260,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='COMPATIBILITY_ALIAS', index=255, number=261,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NIL', index=256, number=262,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_INTERFACE', index=257, number=263,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLASS_IMPLEMENTATION', index=258, number=264,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTOCOL_DECLARATION', index=259, number=265,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CAST', index=260, number=266,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONST_CAST', index=261, number=267,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DYNAMIC_CAST', index=262, number=268,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REINTERPRET_CAST', index=263, number=269,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STATIC_CAST', index=264, number=270,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='POSITION', index=265, number=271,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CUDA_ARGUMENT_LIST', index=266, number=272,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_DIRECTIVE', index=267, number=273,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_NAME', index=268, number=274,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_CLAUSE', index=269, number=275,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_ARGUMENT_LIST', index=270, number=276,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_ARGUMENT', index=271, number=277,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_EXPRESSION', index=272, number=278,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='END_ELEMENT_TOKEN', index=273, number=279,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MAIN', index=274, number=280,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='BREAK', index=275, number=281,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONTINUE', index=276, number=282,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='WHILE', index=277, number=283,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DO', index=278, number=284,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOR', index=279, number=285,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IF', index=280, number=286,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GOTO', index=281, number=293,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VISUAL_CXX_ASM', index=282, number=295,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SIZEOF', index=283, number=296,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='AUTO', index=284, number=298,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REGISTER', index=285, number=299,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='RESTRICT', index=286, number=300,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IMAGINARY', index=287, number=304,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NORETURN', index=288, number=305,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STATIC_ASSERT', index=289, number=306,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CRESTRICT', index=290, number=307,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CXX_TRY', index=291, number=308,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CXX_CATCH', index=292, number=309,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CXX_CLASS', index=293, number=310,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONSTEXPR', index=294, number=311,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THREAD_LOCAL', index=295, number=313,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NULLPTR', index=296, number=314,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VOID', index=297, number=326,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='RETURN', index=298, number=327,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INCLUDE', index=299, number=328,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DEFINE', index=300, number=329,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ELIF', index=301, number=330,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENDIF', index=302, number=331,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ERRORPREC', index=303, number=332,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='WARNING', index=304, number=333,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IFDEF', index=305, number=334,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IFNDEF', index=306, number=335,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LINE', index=307, number=336,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PRAGMA', index=308, number=337,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNDEF', index=309, number=338,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INLINE', index=310, number=339,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MACRO_TYPE_NAME', index=311, number=340,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MACRO_CASE', index=312, number=341,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MACRO_LABEL', index=313, number=342,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SPECIFIER', index=314, number=344,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TRY', index=315, number=345,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CATCH', index=316, number=346,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THROW', index=317, number=347,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THROWS', index=318, number=348,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PUBLIC', index=319, number=350,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PRIVATE', index=320, number=351,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PROTECTED', index=321, number=352,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VIRTUAL', index=322, number=353,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EXPLICIT', index=323, number=356,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOREVER', index=324, number=357,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SIGNAL', index=325, number=358,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='EMIT', index=326, number=359,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NEW', index=327, number=363,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DELETE', index=328, number=364,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STATIC', index=329, number=365,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CONST', index=330, number=366,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MUTABLE', index=331, number=367,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='VOLATILE', index=332, number=368,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='TRANSIENT', index=333, number=369,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FINALLY', index=334, number=372,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FINAL', index=335, number=376,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ABSTRACT', index=336, number=377,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SUPER', index=337, number=378,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SYNCHRONIZED', index=338, number=379,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NATIVE', index=339, number=380,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRICTFP', index=340, number=381,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NULLLITERAL', index=341, number=382,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ASSERT', index=342, number=383,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FOREACH', index=343, number=384,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REF', index=344, number=385,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OUT', index=345, number=386,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='LOCK', index=346, number=388,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IS', index=347, number=389,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='INTERNAL', index=348, number=390,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SEALED', index=349, number=391,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OVERRIDE', index=350, number=392,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='IMPLICIT', index=351, number=393,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STACKALLOC', index=352, number=394,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='AS', index=353, number=395,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DELEGATE', index=354, number=396,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='FIXED', index=355, number=397,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CHECKED', index=356, number=398,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNCHECKED', index=357, number=399,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REGION', index=358, number=400,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ENDREGION', index=359, number=401,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UNSAFE', index=360, number=402,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='READONLY', index=361, number=403,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GET', index=362, number=404,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SET', index=363, number=405,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ADD', index=364, number=406,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='REMOVE', index=365, number=407,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='YIELD', index=366, number=408,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PARTIAL', index=367, number=409,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='AWAIT', index=368, number=410,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ASYNC', index=369, number=412,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='THIS', index=370, number=413,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PARAMS', index=371, number=414,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ALIAS', index=372, number=416,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ASCENDING', index=373, number=422,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DESCENDING', index=374, number=423,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATINTERFACE', index=375, number=430,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATIMPLEMENTATION', index=376, number=431,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATEND', index=377, number=432,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATPROTOCOL', index=378, number=433,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATREQUIRED', index=379, number=434,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATOPTIONAL', index=380, number=435,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ATCLASS', index=381, number=441,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='WEAK', index=382, number=443,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='STRONG', index=383, number=444,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OMP_OMP', index=384, number=448,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SPECIAL_CHARS', index=385, number=449,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SLICE_DEFINE', index=386, number=450,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SLICE_USE', index=387, number=451,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=25760,
  serialized_end=32248,
)
_sym_db.RegisterEnumDescriptor(_SRCMLKIND)

SrcmlKind = enum_type_wrapper.EnumTypeWrapper(_SRCMLKIND)
startrule = 0
compilationunit = 1
programunit = 2
endprogramstatement = 3
identificationdivision = 4
identificationdivisionbody = 5
programidparagraph = 6
authorparagraph = 7
installationparagraph = 8
datewrittenparagraph = 9
datecompiledparagraph = 10
securityparagraph = 11
remarksparagraph = 12
environmentdivision = 13
environmentdivisionbody = 14
configurationsection = 15
configurationsectionparagraph = 16
sourcecomputerparagraph = 17
objectcomputerparagraph = 18
objectcomputerclause = 19
memorysizeclause = 20
disksizeclause = 21
collatingsequenceclause = 22
collatingsequenceclausealphanumeric = 23
collatingsequenceclausenational = 24
segmentlimitclause = 25
charactersetclause = 26
specialnamesparagraph = 27
specialnameclause = 28
alphabetclause = 29
alphabetclauseformat1 = 30
alphabetliterals = 31
alphabetthrough = 32
alphabetalso = 33
alphabetclauseformat2 = 34
channelclause = 35
classclause = 36
classclausethrough = 37
classclausefrom = 38
classclauseto = 39
currencysignclause = 40
decimalpointclause = 41
defaultcomputationalsignclause = 42
defaultdisplaysignclause = 43
environmentswitchnameclause = 44
environmentswitchnamespecialnamesstatusphrase = 45
odtclause = 46
reservenetworkclause = 47
symboliccharactersclause = 48
symboliccharacters = 49
inputoutputsection = 50
inputoutputsectionparagraph = 51
filecontrolparagraph = 52
filecontrolentry = 53
selectclause = 54
filecontrolclause = 55
assignclause = 56
reserveclause = 57
organizationclause = 58
paddingcharacterclause = 59
recorddelimiterclause = 60
accessmodeclause = 61
recordkeyclause = 62
alternaterecordkeyclause = 63
passwordclause = 64
filestatusclause = 65
relativekeyclause = 66
iocontrolparagraph = 67
iocontrolclause = 68
rerunclause = 69
reruneveryrecords = 70
reruneveryof = 71
reruneveryclock = 72
sameclause = 73
multiplefileclause = 74
multiplefileposition = 75
commitmentcontrolclause = 76
datadivision = 77
datadivisionsection = 78
filesection = 79
filedescriptionentry = 80
filedescriptionentryclause = 81
externalclause = 82
globalclause = 83
blockcontainsclause = 84
blockcontainsto = 85
recordcontainsclause = 86
recordcontainsclauseformat1 = 87
recordcontainsclauseformat2 = 88
recordcontainsclauseformat3 = 89
recordcontainsto = 90
labelrecordsclause = 91
valueofclause = 92
valuepair = 93
datarecordsclause = 94
linageclause = 95
linageat = 96
linagefootingat = 97
linagelinesattop = 98
linagelinesatbottom = 99
recordingmodeclause = 100
modestatement = 101
codesetclause = 102
reportclause = 103
databasesection = 104
databasesectionentry = 105
workingstoragesection = 106
linkagesection = 107
communicationsection = 108
communicationdescriptionentry = 109
communicationdescriptionentryformat1 = 110
communicationdescriptionentryformat2 = 111
communicationdescriptionentryformat3 = 112
destinationcountclause = 113
destinationtableclause = 114
endkeyclause = 115
errorkeyclause = 116
messagecountclause = 117
messagedateclause = 118
messagetimeclause = 119
statuskeyclause = 120
symbolicdestinationclause = 121
symbolicqueueclause = 122
symbolicsourceclause = 123
symbolicterminalclause = 124
symbolicsubqueueclause = 125
textlengthclause = 126
localstoragesection = 127
screensection = 128
screendescriptionentry = 129
screendescriptionblankclause = 130
screendescriptionbellclause = 131
screendescriptionblinkclause = 132
screendescriptioneraseclause = 133
screendescriptionlightclause = 134
screendescriptiongridclause = 135
screendescriptionreversevideoclause = 136
screendescriptionunderlineclause = 137
screendescriptionsizeclause = 138
screendescriptionlineclause = 139
screendescriptioncolumnclause = 140
screendescriptionforegroundcolorclause = 141
screendescriptionbackgroundcolorclause = 142
screendescriptioncontrolclause = 143
screendescriptionvalueclause = 144
screendescriptionpictureclause = 145
screendescriptionfromclause = 146
screendescriptiontoclause = 147
screendescriptionusingclause = 148
screendescriptionusageclause = 149
screendescriptionblankwhenzeroclause = 150
screendescriptionjustifiedclause = 151
screendescriptionsignclause = 152
screendescriptionautoclause = 153
screendescriptionsecureclause = 154
screendescriptionrequiredclause = 155
screendescriptionpromptclause = 156
screendescriptionpromptoccursclause = 157
screendescriptionfullclause = 158
screendescriptionzerofillclause = 159
reportsection = 160
reportdescription = 161
reportdescriptionentry = 162
reportdescriptionglobalclause = 163
reportdescriptionpagelimitclause = 164
reportdescriptionheadingclause = 165
reportdescriptionfirstdetailclause = 166
reportdescriptionlastdetailclause = 167
reportdescriptionfootingclause = 168
reportgroupdescriptionentry = 169
reportgroupdescriptionentryformat1 = 170
reportgroupdescriptionentryformat2 = 171
reportgroupdescriptionentryformat3 = 172
reportgroupblankwhenzeroclause = 173
reportgroupcolumnnumberclause = 174
reportgroupindicateclause = 175
reportgroupjustifiedclause = 176
reportgrouplinenumberclause = 177
reportgrouplinenumbernextpage = 178
reportgrouplinenumberplus = 179
reportgroupnextgroupclause = 180
reportgroupnextgroupplus = 181
reportgroupnextgroupnextpage = 182
reportgrouppictureclause = 183
reportgroupresetclause = 184
reportgroupsignclause = 185
reportgroupsourceclause = 186
reportgroupsumclause = 187
reportgrouptypeclause = 188
reportgrouptypereportheading = 189
reportgrouptypepageheading = 190
reportgrouptypecontrolheading = 191
reportgrouptypedetail = 192
reportgrouptypecontrolfooting = 193
reportgroupusageclause = 194
reportgrouptypepagefooting = 195
reportgrouptypereportfooting = 196
reportgroupvalueclause = 197
programlibrarysection = 198
librarydescriptionentry = 199
librarydescriptionentryformat1 = 200
librarydescriptionentryformat2 = 201
libraryattributeclauseformat1 = 202
libraryattributeclauseformat2 = 203
libraryattributefunction = 204
libraryattributeparameter = 205
libraryattributetitle = 206
libraryentryprocedureclauseformat1 = 207
libraryentryprocedureclauseformat2 = 208
libraryentryprocedureforclause = 209
libraryentryproceduregivingclause = 210
libraryentryprocedureusingclause = 211
libraryentryprocedureusingname = 212
libraryentryprocedurewithclause = 213
libraryentryprocedurewithname = 214
libraryiscommonclause = 215
libraryisglobalclause = 216
datadescriptionentry = 217
datadescriptionentryformat1 = 218
datadescriptionentryformat2 = 219
datadescriptionentryformat3 = 220
datadescriptionentryexecsql = 221
dataalignedclause = 222
datablankwhenzeroclause = 223
datacommonownlocalclause = 224
dataexternalclause = 225
dataglobalclause = 226
dataintegerstringclause = 227
datajustifiedclause = 228
dataoccursclause = 229
dataoccursto = 230
dataoccurssort = 231
datapictureclause = 232
picturestring = 233
picturechars = 234
picturecardinality = 235
datareceivedbyclause = 236
datarecordareaclause = 237
dataredefinesclause = 238
datarenamesclause = 239
datasignclause = 240
datasynchronizedclause = 241
datathreadlocalclause = 242
datatypeclause = 243
datatypedefclause = 244
datausageclause = 245
datausingclause = 246
datavalueclause = 247
datavalueinterval = 248
datavalueintervalfrom = 249
datavalueintervalto = 250
datawithlowerboundsclause = 251
proceduredivision = 252
proceduredivisionusingclause = 253
proceduredivisiongivingclause = 254
proceduredivisionusingparameter = 255
proceduredivisionbyreferencephrase = 256
proceduredivisionbyreference = 257
proceduredivisionbyvaluephrase = 258
proceduredivisionbyvalue = 259
proceduredeclaratives = 260
proceduredeclarative = 261
proceduresectionheader = 262
proceduredivisionbody = 263
proceduresection = 264
paragraphs = 265
paragraph = 266
sentence = 267
statement = 268
acceptstatement = 269
acceptfromdatestatement = 270
acceptfrommnemonicstatement = 271
acceptfromescapekeystatement = 272
acceptmessagecountstatement = 273
addstatement = 274
addtostatement = 275
addtogivingstatement = 276
addcorrespondingstatement = 277
addfrom = 278
addto = 279
addtogiving = 280
addgiving = 281
alteredgoto = 282
alterstatement = 283
alterproceedto = 284
callstatement = 285
callusingphrase = 286
callusingparameter = 287
callbyreferencephrase = 288
callbyreference = 289
callbyvaluephrase = 290
callbyvalue = 291
callbycontentphrase = 292
callbycontent = 293
callgivingphrase = 294
cancelstatement = 295
cancelcall = 296
closestatement = 297
closefile = 298
closereelunitstatement = 299
closerelativestatement = 300
closeportfileiostatement = 301
closeportfileiousing = 302
closeportfileiousingclosedisposition = 303
closeportfileiousingassociateddata = 304
closeportfileiousingassociateddatalength = 305
computestatement = 306
computestore = 307
continuestatement = 308
deletestatement = 309
disablestatement = 310
displaystatement = 311
displayoperand = 312
displayat = 313
displayupon = 314
displaywith = 315
dividestatement = 316
divideintostatement = 317
divideintogivingstatement = 318
dividebygivingstatement = 319
dividegivingphrase = 320
divideinto = 321
dividegiving = 322
divideremainder = 323
enablestatement = 324
entrystatement = 325
evaluatestatement = 326
evaluateselect = 327
evaluatealsoselect = 328
evaluatewhenphrase = 329
evaluatewhen = 330
evaluatecondition = 331
evaluatethrough = 332
evaluatealsocondition = 333
evaluatewhenother = 334
evaluatevalue = 335
execcicsstatement = 336
execsqlstatement = 337
execsqlimsstatement = 338
exhibitstatement = 339
exhibitoperand = 340
exitstatement = 341
generatestatement = 342
gobackstatement = 343
gotostatement = 344
gotostatementsimple = 345
gotodependingonstatement = 346
ifstatement = 347
ifthen = 348
ifelse = 349
initializestatement = 350
initializereplacingphrase = 351
initializereplacingby = 352
initiatestatement = 353
inspectstatement = 354
inspecttallyingphrase = 355
inspectreplacingphrase = 356
inspecttallyingreplacingphrase = 357
inspectconvertingphrase = 358
inspectfor = 359
inspectcharacters = 360
inspectreplacingcharacters = 361
inspectallleadings = 362
inspectreplacingallleadings = 363
inspectallleading = 364
inspectreplacingallleading = 365
inspectby = 366
inspectto = 367
inspectbeforeafter = 368
mergestatement = 369
mergeonkeyclause = 370
mergecollatingsequencephrase = 371
mergecollatingalphanumeric = 372
mergecollatingnational = 373
mergeusing = 374
mergeoutputprocedurephrase = 375
mergeoutputthrough = 376
mergegivingphrase = 377
mergegiving = 378
movestatement = 379
movetostatement = 380
movetosendingarea = 381
movecorrespondingtostatement = 382
movecorrespondingtosendingarea = 383
multiplystatement = 384
multiplyregular = 385
multiplyregularoperand = 386
multiplygiving = 387
multiplygivingoperand = 388
multiplygivingresult = 389
openstatement = 390
openinputstatement = 391
openinput = 392
openoutputstatement = 393
openoutput = 394
openiostatement = 395
openextendstatement = 396
performstatement = 397
performinlinestatement = 398
performprocedurestatement = 399
performtype = 400
performtimes = 401
performuntil = 402
performvarying = 403
performvaryingclause = 404
performvaryingphrase = 405
performafter = 406
performfrom = 407
performby = 408
performtestclause = 409
purgestatement = 410
readstatement = 411
readinto = 412
readwith = 413
readkey = 414
receivestatement = 415
receivefromstatement = 416
receivefrom = 417
receiveintostatement = 418
receivenodata = 419
receivewithdata = 420
receivebefore = 421
receivewith = 422
receivethread = 423
receivesize = 424
receivestatus = 425
releasestatement = 426
returnstatement = 427
returninto = 428
rewritestatement = 429
rewritefrom = 430
searchstatement = 431
searchvarying = 432
searchwhen = 433
sendstatement = 434
sendstatementsync = 435
sendstatementasync = 436
sendfromphrase = 437
sendwithphrase = 438
sendreplacingphrase = 439
sendadvancingphrase = 440
sendadvancingpage = 441
sendadvancinglines = 442
sendadvancingmnemonic = 443
setstatement = 444
settostatement = 445
setupdownbystatement = 446
setto = 447
settovalue = 448
setbyvalue = 449
sortstatement = 450
sortonkeyclause = 451
sortduplicatesphrase = 452
sortcollatingsequencephrase = 453
sortcollatingalphanumeric = 454
sortcollatingnational = 455
sortinputprocedurephrase = 456
sortinputthrough = 457
sortusing = 458
sortoutputprocedurephrase = 459
sortoutputthrough = 460
sortgivingphrase = 461
sortgiving = 462
startstatement = 463
startkey = 464
stopstatement = 465
stringstatement = 466
stringsendingphrase = 467
stringsending = 468
stringdelimitedbyphrase = 469
stringforphrase = 470
stringintophrase = 471
stringwithpointerphrase = 472
subtractstatement = 473
subtractfromstatement = 474
subtractfromgivingstatement = 475
subtractcorrespondingstatement = 476
subtractsubtrahend = 477
subtractminuend = 478
subtractminuendgiving = 479
subtractgiving = 480
subtractminuendcorresponding = 481
terminatestatement = 482
unstringstatement = 483
unstringsendingphrase = 484
unstringdelimitedbyphrase = 485
unstringorallphrase = 486
unstringintophrase = 487
unstringinto = 488
unstringdelimiterin = 489
unstringcountin = 490
unstringwithpointerphrase = 491
unstringtallyingphrase = 492
usestatement = 493
useafterclause = 494
useafteron = 495
usedebugclause = 496
usedebugon = 497
writestatement = 498
writefromphrase = 499
writeadvancingphrase = 500
writeadvancingpage = 501
writeadvancinglines = 502
writeadvancingmnemonic = 503
writeatendofpagephrase = 504
writenotatendofpagephrase = 505
atendphrase = 506
notatendphrase = 507
invalidkeyphrase = 508
notinvalidkeyphrase = 509
onoverflowphrase = 510
notonoverflowphrase = 511
onsizeerrorphrase = 512
notonsizeerrorphrase = 513
onexceptionclause = 514
notonexceptionclause = 515
arithmeticexpression = 516
plusminus = 517
multdivs = 518
multdiv = 519
powers = 520
power = 521
basis = 522
condition = 523
andorcondition = 524
combinablecondition = 525
simplecondition = 526
classcondition = 527
conditionnamereference = 528
conditionnamesubscriptreference = 529
relationcondition = 530
relationsigncondition = 531
relationarithmeticcomparison = 532
relationcombinedcomparison = 533
relationcombinedcondition = 534
relationaloperator = 535
abbreviation = 536
identifier = 537
tablecall = 538
functioncall = 539
referencemodifier = 540
characterposition = 541
length = 542
subscript = 543
argument = 544
qualifieddataname = 545
qualifieddatanameformat1 = 546
qualifieddatanameformat2 = 547
qualifieddatanameformat3 = 548
qualifieddatanameformat4 = 549
qualifiedindata = 550
indata = 551
infile = 552
inmnemonic = 553
insection = 554
inlibrary = 555
intable = 556
alphabetname = 557
assignmentname = 558
basisname = 559
cdname = 560
classname = 561
computername = 562
conditionname = 563
dataname = 564
datadescname = 565
environmentname = 566
filename = 567
functionname = 568
indexname = 569
languagename = 570
libraryname = 571
localname = 572
mnemonicname = 573
paragraphname = 574
procedurename = 575
programname = 576
recordname = 577
reportname = 578
routinename = 579
screenname = 580
sectionname = 581
systemname = 582
symboliccharacter = 583
textname = 584
cobolword = 585
literal = 586
booleanliteral = 587
numericliteral = 588
integerliteral = 589
cicsdfhrespliteral = 590
cicsdfhvalueliteral = 591
figurativeconstant = 592
specialregister = 593
commententry = 594
single_input = 0
file_input = 1
eval_input = 2
decorator = 3
decorators = 4
decorated = 5
async_funcdef = 6
funcdef = 7
parameters = 8
typedargslist = 9
tfpdef = 10
varargslist = 11
vfpdef = 12
stmt = 13
simple_stmt = 14
small_stmt = 15
expr_stmt = 16
annassign = 17
testlist_star_expr = 18
augassign = 19
del_stmt = 20
pass_stmt = 21
flow_stmt = 22
break_stmt = 23
continue_stmt = 24
return_stmt = 25
yield_stmt = 26
raise_stmt = 27
import_stmt = 28
import_name = 29
import_from = 30
import_as_name = 31
dotted_as_name = 32
import_as_names = 33
dotted_as_names = 34
dotted_name = 35
global_stmt = 36
nonlocal_stmt = 37
assert_stmt = 38
compound_stmt = 39
async_stmt = 40
if_stmt = 41
while_stmt = 42
for_stmt = 43
try_stmt = 44
with_stmt = 45
with_item = 46
except_clause = 47
suite = 48
test = 49
test_nocond = 50
lambdef = 51
lambdef_nocond = 52
or_test = 53
and_test = 54
not_test = 55
comparison = 56
comp_op = 57
star_expr = 58
expr = 59
xor_expr = 60
and_expr = 61
shift_expr = 62
arith_expr = 63
term = 64
factor = 65
python_power = 66
atom_expr = 67
atom = 68
testlist_comp = 69
trailer = 70
subscriptlist = 71
python_subscript = 72
sliceop = 73
exprlist = 74
testlist = 75
dictorsetmaker = 76
classdef = 77
arglist = 78
python_argument = 79
comp_iter = 80
comp_for = 81
comp_if = 82
encoding_decl = 83
yield_expr = 84
yield_arg = 85
smali_file = 0
class_spec = 1
super_spec = 2
implements_spec = 3
source_spec = 4
access_list = 5
field = 6
method = 7
statements_and_directives = 8
ordered_method_item = 9
registers_directive = 10
param_list_or_id = 11
simple_name = 12
member_name = 13
method_prototype = 14
param_list_or_id_primitive_type = 15
param_list = 16
array_descriptor = 17
type_descriptor = 18
nonvoid_type_descriptor = 19
reference_type_descriptor = 20
integer_literal = 21
float_literal = 22
double_literal = 23
smali_literal = 24
parsed_integer_literal = 25
integral_literal = 26
fixed_32bit_literal = 27
fixed_literal = 28
array_literal = 29
annotation_element = 30
annotation = 31
subannotation = 32
enum_literal = 33
type_field_method_literal = 34
method_reference = 35
field_reference = 36
label = 37
label_ref = 38
register_list = 39
register_range = 40
verification_error_reference = 41
catch_directive = 42
catchall_directive = 43
parameter_directive = 44
debug_directive = 45
line_directive = 46
local_directive = 47
end_local_directive = 48
restart_local_directive = 49
prologue_directive = 50
epilogue_directive = 51
source_directive = 52
instruction_format12x = 53
instruction_format22s = 54
instruction_format31i = 55
instruction = 56
insn_format10t = 57
insn_format10x = 58
insn_format10x_odex = 59
insn_format11n = 60
insn_format11x = 61
insn_format12x = 62
insn_format20bc = 63
insn_format20t = 64
insn_format21c_field = 65
insn_format21c_field_odex = 66
insn_format21c_string = 67
insn_format21c_type = 68
insn_format21ih = 69
insn_format21lh = 70
insn_format21s = 71
insn_format21t = 72
insn_format22b = 73
insn_format22c_field = 74
insn_format22c_field_odex = 75
insn_format22c_type = 76
insn_format22cs_field = 77
insn_format22s = 78
insn_format22t = 79
insn_format22x = 80
insn_format23x = 81
insn_format30t = 82
insn_format31c = 83
insn_format31i = 84
insn_format31t = 85
insn_format32x = 86
insn_format35c_method = 87
insn_format35c_type = 88
insn_format35c_method_odex = 89
insn_format35mi_method = 90
insn_format35ms_method = 91
insn_format3rc_method = 92
insn_format3rc_method_odex = 93
insn_format3rc_type = 94
insn_format3rmi_method = 95
insn_format3rms_method = 96
insn_format45cc_method = 97
insn_format4rcc_method = 98
insn_format51l = 99
insn_array_data_directive = 100
insn_packed_switch_directive = 101
insn_sparse_switch_directive = 102
sourceunit = 0
pragmadirective = 1
pragmaname = 2
pragmavalue = 3
version = 4
versionoperator = 5
versionconstraint = 6
importdeclaration = 7
importdirective = 8
contractdefinition = 9
inheritancespecifier = 10
contractpart = 11
statevariabledeclaration = 12
usingfordeclaration = 13
structdefinition = 14
modifierdefinition = 15
modifierinvocation = 16
functiondefinition = 17
returnparameters = 18
modifierlist = 19
eventdefinition = 20
enumvalue = 21
enumdefinition = 22
indexedparameterlist = 23
indexedparameter = 24
parameterlist = 25
parameter = 26
typenamelist = 27
unnamedparameter = 28
variabledeclaration = 29
typename = 30
userdefinedtypename = 31
mapping = 32
functiontypename = 33
storagelocation = 34
statemutability = 35
block = 36
solidity_statement = 37
expressionstatement = 38
solidity_ifstatement = 39
whilestatement = 40
simplestatement = 41
forstatement = 42
inlineassemblystatement = 43
dowhilestatement = 44
solidity_continuestatement = 45
breakstatement = 46
solidity_returnstatement = 47
throwstatement = 48
variabledeclarationstatement = 49
identifierlist = 50
elementarytypename = 51
expression = 52
primaryexpression = 53
expressionlist = 54
namevaluelist = 55
namevalue = 56
functioncallarguments = 57
assemblyblock = 58
assemblyitem = 59
assemblyexpression = 60
assemblycall = 61
assemblylocaldefinition = 62
assemblyassignment = 63
assemblyidentifierorlist = 64
assemblyidentifierlist = 65
assemblyrightassignment = 66
labeldefinition = 67
assemblyswitch = 68
assemblycase = 69
assemblyfunctiondefinition = 70
assemblyfor = 71
assemblyliteral = 72
subassembly = 73
datasize = 74
linkersymbol = 75
tupleexpression = 76
elementarytypenameexpression = 77
numberliteral = 78
solidity_identifier = 79
UNIT_KIND = 0
DECL = 1
DECL_STMT = 2
INIT = 3
EXPR = 4
EXPR_STMT = 5
COMMENT = 6
CALL = 7
CONTROL = 8
INCR = 9
NONE = 10
VARIABLE = 11
FUNCTION = 12
FUNCTION_DECL = 13
CONSTRUCTOR = 14
CONSTRUCTOR_DECL = 15
DESTRUCTOR = 16
DESTRUCTOR_DECL = 17
MACRO = 18
SINGLE_MACRO = 19
NULLOPERATOR = 20
ENUM_DEFN = 21
ENUM_DECL = 22
GLOBAL_ATTRIBUTE = 23
PROPERTY_ACCESSOR = 24
PROPERTY_ACCESSOR_DECL = 25
EXPRESSION = 26
CLASS_DEFN = 27
CLASS_DECL = 28
UNION_DEFN = 29
UNION_DECL = 30
STRUCT_DEFN = 31
STRUCT_DECL = 32
INTERFACE_DEFN = 33
INTERFACE_DECL = 34
ACCESS_REGION = 35
USING = 36
OPERATOR_FUNCTION = 37
OPERATOR_FUNCTION_DECL = 38
EVENT = 39
PROPERTY = 40
ANNOTATION_DEFN = 41
GLOBAL_TEMPLATE = 42
UNIT = 43
TART_ELEMENT_TOKEN = 44
NOP = 45
STRING = 46
CHAR = 47
LITERAL = 48
BOOLEAN = 49
NULL2 = 50
COMPLEX = 51
OPERATOR = 52
MODIFIER = 53
NAME = 54
ONAME = 55
CNAME = 56
TYPE = 57
TYPEPREV = 58
CONDITION = 59
BLOCK = 60
PSEUDO_BLOCK = 61
INDEX = 62
ENUM = 63
ENUM_DECLARATION = 64
IF_STATEMENT = 65
TERNARY = 66
THEN = 67
ELSE = 68
ELSEIF = 69
WHILE_STATEMENT = 70
DO_STATEMENT = 71
FOR_STATEMENT = 72
FOREACH_STATEMENT = 73
FOR_CONTROL = 74
FOR_INITIALIZATION = 75
FOR_CONDITION = 76
FOR_INCREMENT = 77
FOR_LIKE_CONTROL = 78
EXPRESSION_STATEMENT = 79
FUNCTION_CALL = 81
DECLARATION_STATEMENT = 82
DECLARATION = 83
DECLARATION_INITIALIZATION = 84
DECLARATION_RANGE = 85
RANGE = 86
GOTO_STATEMENT = 87
CONTINUE_STATEMENT = 88
BREAK_STATEMENT = 89
LABEL_STATEMENT = 90
LABEL = 91
SWITCH = 92
CASE = 93
DEFAULT = 94
FUNCTION_DEFINITION = 95
FUNCTION_DECLARATION = 96
LAMBDA = 97
FUNCTION_LAMBDA = 98
FUNCTION_SPECIFIER = 99
RETURN_STATEMENT = 100
PARAMETER_LIST = 101
PARAMETER = 102
KRPARAMETER_LIST = 103
KRPARAMETER = 104
ARGUMENT_LIST = 105
ARGUMENT = 106
PSEUDO_PARAMETER_LIST = 107
INDEXER_PARAMETER_LIST = 108
CLASS = 109
CLASS_DECLARATION = 110
STRUCT = 111
STRUCT_DECLARATION = 112
UNION = 113
UNION_DECLARATION = 114
DERIVATION_LIST = 115
PUBLIC_ACCESS = 116
PUBLIC_ACCESS_DEFAULT = 117
PRIVATE_ACCESS = 118
PRIVATE_ACCESS_DEFAULT = 119
PROTECTED_ACCESS = 120
PROTECTED_ACCESS_DEFAULT = 121
MEMBER_INIT_LIST = 122
MEMBER_INITIALIZATION_LIST = 123
MEMBER_INITIALIZATION = 124
CONSTRUCTOR_DEFINITION = 125
CONSTRUCTOR_DECLARATION = 126
DESTRUCTOR_DEFINITION = 127
DESTRUCTOR_DECLARATION = 128
FRIEND = 129
CLASS_SPECIFIER = 130
TRY_BLOCK = 131
CATCH_BLOCK = 132
FINALLY_BLOCK = 133
THROW_STATEMENT = 134
THROW_SPECIFIER = 135
THROW_SPECIFIER_JAVA = 136
TEMPLATE = 137
GENERIC_ARGUMENT = 138
GENERIC_ARGUMENT_LIST = 139
TEMPLATE_PARAMETER = 140
TEMPLATE_PARAMETER_LIST = 141
GENERIC_PARAMETER = 142
GENERIC_PARAMETER_LIST = 143
TYPEDEF = 144
ASM = 145
MACRO_CALL = 146
SIZEOF_CALL = 147
EXTERN = 148
NAMESPACE = 149
USING_DIRECTIVE = 150
DIRECTIVE = 151
ATOMIC = 152
STATIC_ASSERT_STATEMENT = 153
GENERIC_SELECTION = 154
GENERIC_SELECTOR = 155
GENERIC_ASSOCIATION_LIST = 156
GENERIC_ASSOCIATION = 157
ALIGNAS = 158
DECLTYPE = 159
CAPTURE = 160
LAMBDA_CAPTURE = 161
NOEXCEPT = 162
TYPENAME = 163
ALIGNOF = 164
TYPEID = 165
SIZEOF_PACK = 166
ENUM_CLASS = 167
ENUM_CLASS_DECLARATION = 168
REF_QUALIFIER = 171
SIGNAL_ACCESS = 172
FOREVER_STATEMENT = 173
EMIT_STATEMENT = 174
CPP_DIRECTIVE = 175
CPP_FILENAME = 176
FILE = 177
NUMBER = 178
CPP_NUMBER = 179
CPP_LITERAL = 180
CPP_MACRO_DEFN = 181
CPP_MACRO_VALUE = 182
ERROR = 183
CPP_ERROR = 184
CPP_WARNING = 185
CPP_PRAGMA = 186
CPP_INCLUDE = 187
CPP_DEFINE = 188
CPP_UNDEF = 189
CPP_LINE = 190
CPP_IF = 191
CPP_IFDEF = 192
CPP_IFNDEF = 193
CPP_THEN = 194
CPP_ELSE = 195
CPP_ELIF = 196
CPP_EMPTY = 197
CPP_REGION = 198
CPP_ENDREGION = 199
USING_STMT = 200
ESCAPE = 201
VALUE = 202
CPP_IMPORT = 203
CPP_ENDIF = 204
MARKER = 205
ERROR_PARSE = 206
ERROR_MODE = 207
IMPLEMENTS = 208
EXTENDS = 209
IMPORT = 210
PACKAGE = 211
ASSERT_STATEMENT = 212
INTERFACE = 213
INTERFACE_DECLARATION = 214
SYNCHRONIZED_STATEMENT = 215
ANNOTATION = 216
STATIC_BLOCK = 218
CHECKED_STATEMENT = 219
UNCHECKED_STATEMENT = 220
ATTRIBUTE = 221
TARGET = 222
UNSAFE_STATEMENT = 223
LOCK_STATEMENT = 224
FIXED_STATEMENT = 225
TYPEOF = 226
USING_STATEMENT = 227
FUNCTION_DELEGATE = 228
CONSTRAINT = 230
LINQ = 231
FROM = 232
WHERE = 233
SELECT = 234
LET = 235
ORDERBY = 236
JOIN = 237
GROUP = 238
IN = 239
ON = 240
EQUALS = 241
BY = 242
INTO = 243
EMPTY = 244
EMPTY_STMT = 245
RECEIVER = 246
MESSAGE = 247
SELECTOR = 248
PROTOCOL_LIST = 249
CATEGORY = 250
PROTOCOL = 251
REQUIRED_DEFAULT = 252
REQUIRED = 253
OPTIONAL = 254
ATTRIBUTE_LIST = 256
SYNTHESIZE = 257
DYNAMIC = 258
ENCODE = 259
AUTORELEASEPOOL = 260
COMPATIBILITY_ALIAS = 261
NIL = 262
CLASS_INTERFACE = 263
CLASS_IMPLEMENTATION = 264
PROTOCOL_DECLARATION = 265
CAST = 266
CONST_CAST = 267
DYNAMIC_CAST = 268
REINTERPRET_CAST = 269
STATIC_CAST = 270
POSITION = 271
CUDA_ARGUMENT_LIST = 272
OMP_DIRECTIVE = 273
OMP_NAME = 274
OMP_CLAUSE = 275
OMP_ARGUMENT_LIST = 276
OMP_ARGUMENT = 277
OMP_EXPRESSION = 278
END_ELEMENT_TOKEN = 279
MAIN = 280
BREAK = 281
CONTINUE = 282
WHILE = 283
DO = 284
FOR = 285
IF = 286
GOTO = 293
VISUAL_CXX_ASM = 295
SIZEOF = 296
AUTO = 298
REGISTER = 299
RESTRICT = 300
IMAGINARY = 304
NORETURN = 305
STATIC_ASSERT = 306
CRESTRICT = 307
CXX_TRY = 308
CXX_CATCH = 309
CXX_CLASS = 310
CONSTEXPR = 311
THREAD_LOCAL = 313
NULLPTR = 314
VOID = 326
RETURN = 327
INCLUDE = 328
DEFINE = 329
ELIF = 330
ENDIF = 331
ERRORPREC = 332
WARNING = 333
IFDEF = 334
IFNDEF = 335
LINE = 336
PRAGMA = 337
UNDEF = 338
INLINE = 339
MACRO_TYPE_NAME = 340
MACRO_CASE = 341
MACRO_LABEL = 342
SPECIFIER = 344
TRY = 345
CATCH = 346
THROW = 347
THROWS = 348
PUBLIC = 350
PRIVATE = 351
PROTECTED = 352
VIRTUAL = 353
EXPLICIT = 356
FOREVER = 357
SIGNAL = 358
EMIT = 359
NEW = 363
DELETE = 364
STATIC = 365
CONST = 366
MUTABLE = 367
VOLATILE = 368
TRANSIENT = 369
FINALLY = 372
FINAL = 376
ABSTRACT = 377
SUPER = 378
SYNCHRONIZED = 379
NATIVE = 380
STRICTFP = 381
NULLLITERAL = 382
ASSERT = 383
FOREACH = 384
REF = 385
OUT = 386
LOCK = 388
IS = 389
INTERNAL = 390
SEALED = 391
OVERRIDE = 392
IMPLICIT = 393
STACKALLOC = 394
AS = 395
DELEGATE = 396
FIXED = 397
CHECKED = 398
UNCHECKED = 399
REGION = 400
ENDREGION = 401
UNSAFE = 402
READONLY = 403
GET = 404
SET = 405
ADD = 406
REMOVE = 407
YIELD = 408
PARTIAL = 409
AWAIT = 410
ASYNC = 412
THIS = 413
PARAMS = 414
ALIAS = 416
ASCENDING = 422
DESCENDING = 423
ATINTERFACE = 430
ATIMPLEMENTATION = 431
ATEND = 432
ATPROTOCOL = 433
ATREQUIRED = 434
ATOPTIONAL = 435
ATCLASS = 441
WEAK = 443
STRONG = 444
OMP_OMP = 448
SPECIAL_CHARS = 449
SLICE_DEFINE = 450
SLICE_USE = 451


_ELEMENT_UNIT_LANGUAGETYPE = _descriptor.EnumDescriptor(
  name='LanguageType',
  full_name='fast.Element.Unit.LanguageType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='DUMMY', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ALL', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OO', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CXX', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='C', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='C_FAMILY', index=5, number=5,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='JAVA', index=6, number=6,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CSHARP', index=7, number=7,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='OBJECTIVE_C', index=8, number=8,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SMALI', index=9, number=9,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='SOLIDITY', index=10, number=10,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='COBOL85', index=11, number=11,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PYTHON3', index=12, number=12,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=628,
  serialized_end=790,
)
_sym_db.RegisterEnumDescriptor(_ELEMENT_UNIT_LANGUAGETYPE)

_ELEMENT_LITERAL_LITERALTYPE = _descriptor.EnumDescriptor(
  name='LiteralType',
  full_name='fast.Element.Literal.LiteralType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='dummy_type', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='number_type', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='char_type', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='string_type', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='boolean_type', index=4, number=4,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='null_type', index=5, number=5,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=853,
  serialized_end=964,
)
_sym_db.RegisterEnumDescriptor(_ELEMENT_LITERAL_LITERALTYPE)

_ELEMENT_DIFFTYPE = _descriptor.EnumDescriptor(
  name='DiffType',
  full_name='fast.Element.DiffType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='MATCHED', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ADDED', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DELETED', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CHANGED_FROM', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CHANGED_TO', index=4, number=4,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=966,
  serialized_end=1047,
)
_sym_db.RegisterEnumDescriptor(_ELEMENT_DIFFTYPE)

_DELTA_DIFF_DELTATYPE = _descriptor.EnumDescriptor(
  name='DeltaType',
  full_name='fast.Delta.Diff.DeltaType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='MATCH', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ADD', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DEL', index=2, number=2,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='MOVE', index=3, number=3,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='UPDATE', index=4, number=4,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=1576,
  serialized_end=1638,
)
_sym_db.RegisterEnumDescriptor(_DELTA_DIFF_DELTATYPE)

_PAIRS_PAIR_CLONETYPE = _descriptor.EnumDescriptor(
  name='CloneType',
  full_name='fast.Pairs.Pair.CloneType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='MAYBE', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='YES', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='NO', index=2, number=2,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=2035,
  serialized_end=2074,
)
_sym_db.RegisterEnumDescriptor(_PAIRS_PAIR_CLONETYPE)

_SLICES_SLICE_CHANGETYPE = _descriptor.EnumDescriptor(
  name='ChangeType',
  full_name='fast.Slices.Slice.ChangeType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='UNCHANGED', index=0, number=0,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ADD', index=1, number=1,
      serialized_options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='DEL', index=2, number=2,
      serialized_options=None,
      type=None),
  ],
  containing_type=None,
  serialized_options=None,
  serialized_start=3826,
  serialized_end=3871,
)
_sym_db.RegisterEnumDescriptor(_SLICES_SLICE_CHANGETYPE)


_ELEMENT_UNIT = _descriptor.Descriptor(
  name='Unit',
  full_name='fast.Element.Unit',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='filename', full_name='fast.Element.Unit.filename', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='revision', full_name='fast.Element.Unit.revision', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='language', full_name='fast.Element.Unit.language', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='item', full_name='fast.Element.Unit.item', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
    _ELEMENT_UNIT_LANGUAGETYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=518,
  serialized_end=790,
)

_ELEMENT_LITERAL = _descriptor.Descriptor(
  name='Literal',
  full_name='fast.Element.Literal',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Element.Literal.type', index=0,
      number=1, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
    _ELEMENT_LITERAL_LITERALTYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=793,
  serialized_end=964,
)

_ELEMENT = _descriptor.Descriptor(
  name='Element',
  full_name='fast.Element',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='srcml_kind', full_name='fast.Element.srcml_kind', index=0,
      number=1, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='smali_kind', full_name='fast.Element.smali_kind', index=1,
      number=2, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='python3_kind', full_name='fast.Element.python3_kind', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='solidity_kind', full_name='fast.Element.solidity_kind', index=3,
      number=4, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='cobol85_kind', full_name='fast.Element.cobol85_kind', index=4,
      number=5, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='text', full_name='fast.Element.text', index=5,
      number=6, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='pos', full_name='fast.Element.pos', index=6,
      number=7, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='length', full_name='fast.Element.length', index=7,
      number=8, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='child', full_name='fast.Element.child', index=8,
      number=9, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='tail', full_name='fast.Element.tail', index=9,
      number=10, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='unit', full_name='fast.Element.unit', index=10,
      number=11, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='literal', full_name='fast.Element.literal', index=11,
      number=12, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='line', full_name='fast.Element.line', index=12,
      number=13, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='column', full_name='fast.Element.column', index=13,
      number=14, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='label', full_name='fast.Element.label', index=14,
      number=15, type=2, cpp_type=6, label=1,
      has_default_value=False, default_value=float(0),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='change', full_name='fast.Element.change', index=15,
      number=16, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='keep', full_name='fast.Element.keep', index=16,
      number=17, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='id', full_name='fast.Element.id', index=17,
      number=18, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_ELEMENT_UNIT, _ELEMENT_LITERAL, ],
  enum_types=[
    _ELEMENT_DIFFTYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='type', full_name='fast.Element.type',
      index=0, containing_type=None, fields=[]),
    _descriptor.OneofDescriptor(
      name='extra', full_name='fast.Element.extra',
      index=1, containing_type=None, fields=[]),
  ],
  serialized_start=21,
  serialized_end=1064,
)


_DELTA_DIFF_MATCH = _descriptor.Descriptor(
  name='Match',
  full_name='fast.Delta.Diff.Match',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.Diff.Match.src', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dst', full_name='fast.Delta.Diff.Match.dst', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1382,
  serialized_end=1415,
)

_DELTA_DIFF_ADD = _descriptor.Descriptor(
  name='Add',
  full_name='fast.Delta.Diff.Add',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.Diff.Add.src', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dst', full_name='fast.Delta.Diff.Add.dst', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='position', full_name='fast.Delta.Diff.Add.position', index=2,
      number=3, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1417,
  serialized_end=1466,
)

_DELTA_DIFF_DEL = _descriptor.Descriptor(
  name='Del',
  full_name='fast.Delta.Diff.Del',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.Diff.Del.src', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1468,
  serialized_end=1486,
)

_DELTA_DIFF_MOVE = _descriptor.Descriptor(
  name='Move',
  full_name='fast.Delta.Diff.Move',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.Diff.Move.src', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dst', full_name='fast.Delta.Diff.Move.dst', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='position', full_name='fast.Delta.Diff.Move.position', index=2,
      number=3, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1488,
  serialized_end=1538,
)

_DELTA_DIFF_UPDATE = _descriptor.Descriptor(
  name='Update',
  full_name='fast.Delta.Diff.Update',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.Diff.Update.src', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dst', full_name='fast.Delta.Diff.Update.dst', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1540,
  serialized_end=1574,
)

_DELTA_DIFF = _descriptor.Descriptor(
  name='Diff',
  full_name='fast.Delta.Diff',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Delta.Diff.type', index=0,
      number=1, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='match', full_name='fast.Delta.Diff.match', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='add', full_name='fast.Delta.Diff.add', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='del', full_name='fast.Delta.Diff.del', index=3,
      number=4, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='move', full_name='fast.Delta.Diff.move', index=4,
      number=5, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='update', full_name='fast.Delta.Diff.update', index=5,
      number=6, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_DELTA_DIFF_MATCH, _DELTA_DIFF_ADD, _DELTA_DIFF_DEL, _DELTA_DIFF_MOVE, _DELTA_DIFF_UPDATE, ],
  enum_types=[
    _DELTA_DIFF_DELTATYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='delta', full_name='fast.Delta.Diff.delta',
      index=0, containing_type=None, fields=[]),
  ],
  serialized_start=1135,
  serialized_end=1647,
)

_DELTA = _descriptor.Descriptor(
  name='Delta',
  full_name='fast.Delta',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='src', full_name='fast.Delta.src', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dst', full_name='fast.Delta.dst', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='diff', full_name='fast.Delta.diff', index=2,
      number=3, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_DELTA_DIFF, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1067,
  serialized_end=1647,
)


_PAIRS_PAIR_DIFF = _descriptor.Descriptor(
  name='Diff',
  full_name='fast.Pairs.Pair.Diff',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='project', full_name='fast.Pairs.Pair.Diff.project', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='left_line', full_name='fast.Pairs.Pair.Diff.left_line', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='left_column', full_name='fast.Pairs.Pair.Diff.left_column', index=2,
      number=3, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='right_line', full_name='fast.Pairs.Pair.Diff.right_line', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='right_column', full_name='fast.Pairs.Pair.Diff.right_column', index=4,
      number=5, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='old_code', full_name='fast.Pairs.Pair.Diff.old_code', index=5,
      number=6, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='new_code', full_name='fast.Pairs.Pair.Diff.new_code', index=6,
      number=7, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='hash', full_name='fast.Pairs.Pair.Diff.hash', index=7,
      number=8, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='slices', full_name='fast.Pairs.Pair.Diff.slices', index=8,
      number=9, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1818,
  serialized_end=2033,
)

_PAIRS_PAIR = _descriptor.Descriptor(
  name='Pair',
  full_name='fast.Pairs.Pair',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='left', full_name='fast.Pairs.Pair.left', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='right', full_name='fast.Pairs.Pair.right', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Pairs.Pair.type', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_PAIRS_PAIR_DIFF, ],
  enum_types=[
    _PAIRS_PAIR_CLONETYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1692,
  serialized_end=2074,
)

_PAIRS = _descriptor.Descriptor(
  name='Pairs',
  full_name='fast.Pairs',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='pair', full_name='fast.Pairs.pair', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_PAIRS_PAIR, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1650,
  serialized_end=2074,
)


_LOG_COMMIT_DIFF_HUNK_MODLINE = _descriptor.Descriptor(
  name='ModLine',
  full_name='fast.Log.Commit.Diff.Hunk.ModLine',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='line', full_name='fast.Log.Commit.Diff.Hunk.ModLine.line', index=0,
      number=1, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='is_add', full_name='fast.Log.Commit.Diff.Hunk.ModLine.is_add', index=1,
      number=2, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='is_del', full_name='fast.Log.Commit.Diff.Hunk.ModLine.is_del', index=2,
      number=3, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2793,
  serialized_end=2848,
)

_LOG_COMMIT_DIFF_HUNK = _descriptor.Descriptor(
  name='Hunk',
  full_name='fast.Log.Commit.Diff.Hunk',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='from_lineno', full_name='fast.Log.Commit.Diff.Hunk.from_lineno', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='from_column', full_name='fast.Log.Commit.Diff.Hunk.from_column', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='to_lineno', full_name='fast.Log.Commit.Diff.Hunk.to_lineno', index=2,
      number=3, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='to_column', full_name='fast.Log.Commit.Diff.Hunk.to_column', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='context', full_name='fast.Log.Commit.Diff.Hunk.context', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='element', full_name='fast.Log.Commit.Diff.Hunk.element', index=5,
      number=6, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='graph', full_name='fast.Log.Commit.Diff.Hunk.graph', index=6,
      number=7, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='mod', full_name='fast.Log.Commit.Diff.Hunk.mod', index=7,
      number=8, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='slice', full_name='fast.Log.Commit.Diff.Hunk.slice', index=8,
      number=9, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_LOG_COMMIT_DIFF_HUNK_MODLINE, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2550,
  serialized_end=2848,
)

_LOG_COMMIT_DIFF = _descriptor.Descriptor(
  name='Diff',
  full_name='fast.Log.Commit.Diff',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='a', full_name='fast.Log.Commit.Diff.a', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='b', full_name='fast.Log.Commit.Diff.b', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='is_new', full_name='fast.Log.Commit.Diff.is_new', index=2,
      number=3, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='is_code', full_name='fast.Log.Commit.Diff.is_code', index=3,
      number=4, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='index_from', full_name='fast.Log.Commit.Diff.index_from', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='index_to', full_name='fast.Log.Commit.Diff.index_to', index=5,
      number=6, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='mode', full_name='fast.Log.Commit.Diff.mode', index=6,
      number=7, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='hunk', full_name='fast.Log.Commit.Diff.hunk', index=7,
      number=8, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_LOG_COMMIT_DIFF_HUNK, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2392,
  serialized_end=2848,
)

_LOG_COMMIT = _descriptor.Descriptor(
  name='Commit',
  full_name='fast.Log.Commit',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='fast.Log.Commit.id', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='msg', full_name='fast.Log.Commit.msg', index=1,
      number=2, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='author_name', full_name='fast.Log.Commit.author_name', index=2,
      number=3, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='author_email', full_name='fast.Log.Commit.author_email', index=3,
      number=4, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='author_time', full_name='fast.Log.Commit.author_time', index=4,
      number=5, type=3, cpp_type=2, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='committer_name', full_name='fast.Log.Commit.committer_name', index=5,
      number=6, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='committer_email', full_name='fast.Log.Commit.committer_email', index=6,
      number=7, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='committer_time', full_name='fast.Log.Commit.committer_time', index=7,
      number=8, type=3, cpp_type=2, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='diff', full_name='fast.Log.Commit.diff', index=8,
      number=9, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='slice', full_name='fast.Log.Commit.slice', index=9,
      number=10, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_LOG_COMMIT_DIFF, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2153,
  serialized_end=2848,
)

_LOG_AUTHOR = _descriptor.Descriptor(
  name='Author',
  full_name='fast.Log.Author',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='fast.Log.Author.id', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='name', full_name='fast.Log.Author.name', index=1,
      number=2, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='email', full_name='fast.Log.Author.email', index=2,
      number=3, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2850,
  serialized_end=2899,
)

_LOG = _descriptor.Descriptor(
  name='Log',
  full_name='fast.Log',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='commit', full_name='fast.Log.commit', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='author', full_name='fast.Log.author', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_LOG_COMMIT, _LOG_AUTHOR, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2077,
  serialized_end=2899,
)


_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION = _descriptor.Descriptor(
  name='Position',
  full_name='fast.Slices.Slice.SourceFile.Function.Variable.Position',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='location', full_name='fast.Slices.Slice.SourceFile.Function.Variable.Position.location', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Slices.Slice.SourceFile.Function.Variable.Position.type', index=1,
      number=2, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='delta_location', full_name='fast.Slices.Slice.SourceFile.Function.Variable.Position.delta_location', index=2,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3679,
  serialized_end=3776,
)

_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_FUNCTIONDECL = _descriptor.Descriptor(
  name='FunctionDecl',
  full_name='fast.Slices.Slice.SourceFile.Function.Variable.FunctionDecl',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='name', full_name='fast.Slices.Slice.SourceFile.Function.Variable.FunctionDecl.name', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='location', full_name='fast.Slices.Slice.SourceFile.Function.Variable.FunctionDecl.location', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3778,
  serialized_end=3824,
)

_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE = _descriptor.Descriptor(
  name='Variable',
  full_name='fast.Slices.Slice.SourceFile.Function.Variable',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='name', full_name='fast.Slices.Slice.SourceFile.Function.Variable.name', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='pos', full_name='fast.Slices.Slice.SourceFile.Function.Variable.pos', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Slices.Slice.SourceFile.Function.Variable.type', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='defn', full_name='fast.Slices.Slice.SourceFile.Function.Variable.defn', index=3,
      number=4, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='use', full_name='fast.Slices.Slice.SourceFile.Function.Variable.use', index=4,
      number=5, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dvar', full_name='fast.Slices.Slice.SourceFile.Function.Variable.dvar', index=5,
      number=6, type=9, cpp_type=9, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='alias', full_name='fast.Slices.Slice.SourceFile.Function.Variable.alias', index=6,
      number=7, type=9, cpp_type=9, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='cfunc', full_name='fast.Slices.Slice.SourceFile.Function.Variable.cfunc', index=7,
      number=8, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION, _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_FUNCTIONDECL, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3288,
  serialized_end=3824,
)

_SLICES_SLICE_SOURCEFILE_FUNCTION = _descriptor.Descriptor(
  name='Function',
  full_name='fast.Slices.Slice.SourceFile.Function',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='variable', full_name='fast.Slices.Slice.SourceFile.Function.variable', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='name', full_name='fast.Slices.Slice.SourceFile.Function.name', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Slices.Slice.SourceFile.Function.type', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3149,
  serialized_end=3824,
)

_SLICES_SLICE_SOURCEFILE = _descriptor.Descriptor(
  name='SourceFile',
  full_name='fast.Slices.Slice.SourceFile',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='function', full_name='fast.Slices.Slice.SourceFile.function', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='name', full_name='fast.Slices.Slice.SourceFile.name', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Slices.Slice.SourceFile.type', index=2,
      number=3, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_SLICES_SLICE_SOURCEFILE_FUNCTION, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3017,
  serialized_end=3824,
)

_SLICES_SLICE = _descriptor.Descriptor(
  name='Slice',
  full_name='fast.Slices.Slice',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='file', full_name='fast.Slices.Slice.file', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='hash', full_name='fast.Slices.Slice.hash', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_SLICES_SLICE_SOURCEFILE, ],
  enum_types=[
    _SLICES_SLICE_CHANGETYPE,
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2948,
  serialized_end=3871,
)

_SLICES = _descriptor.Descriptor(
  name='Slices',
  full_name='fast.Slices',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='slice', full_name='fast.Slices.slice', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_SLICES_SLICE, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=2902,
  serialized_end=3871,
)


_BUGS_BUG_INFO = _descriptor.Descriptor(
  name='Info',
  full_name='fast.Bugs.Bug.Info',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='summary', full_name='fast.Bugs.Bug.Info.summary', index=0,
      number=1, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='description', full_name='fast.Bugs.Bug.Info.description', index=1,
      number=2, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4044,
  serialized_end=4088,
)

_BUGS_BUG = _descriptor.Descriptor(
  name='Bug',
  full_name='fast.Bugs.Bug',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='fast.Bugs.Bug.id', index=0,
      number=1, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='opendate', full_name='fast.Bugs.Bug.opendate', index=1,
      number=2, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='fixdate', full_name='fast.Bugs.Bug.fixdate', index=2,
      number=3, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='buginfo', full_name='fast.Bugs.Bug.buginfo', index=3,
      number=4, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='fixed_file', full_name='fast.Bugs.Bug.fixed_file', index=4,
      number=5, type=12, cpp_type=9, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_BUGS_BUG_INFO, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3932,
  serialized_end=4088,
)

_BUGS = _descriptor.Descriptor(
  name='Bugs',
  full_name='fast.Bugs',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='repository', full_name='fast.Bugs.repository', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='bug', full_name='fast.Bugs.bug', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_BUGS_BUG, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=3874,
  serialized_end=4088,
)


_GRAPH_EDGE = _descriptor.Descriptor(
  name='Edge',
  full_name='fast.Graph.Edge',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='node1', full_name='fast.Graph.Edge.node1', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='node2', full_name='fast.Graph.Edge.node2', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4132,
  serialized_end=4168,
)

_GRAPH_NODELABEL = _descriptor.Descriptor(
  name='NodeLabel',
  full_name='fast.Graph.NodeLabel',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='node', full_name='fast.Graph.NodeLabel.node', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='label', full_name='fast.Graph.NodeLabel.label', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4170,
  serialized_end=4210,
)

_GRAPH_NODETYPE = _descriptor.Descriptor(
  name='NodeType',
  full_name='fast.Graph.NodeType',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='node', full_name='fast.Graph.NodeType.node', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='type', full_name='fast.Graph.NodeType.type', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4212,
  serialized_end=4250,
)

_GRAPH_CONTEXTEDGES = _descriptor.Descriptor(
  name='ContextEdges',
  full_name='fast.Graph.ContextEdges',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='nextToken', full_name='fast.Graph.ContextEdges.nextToken', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='child', full_name='fast.Graph.ContextEdges.child', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='lastLexicalUse', full_name='fast.Graph.ContextEdges.lastLexicalUse', index=2,
      number=3, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='lastUse', full_name='fast.Graph.ContextEdges.lastUse', index=3,
      number=4, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='lastWrite', full_name='fast.Graph.ContextEdges.lastWrite', index=4,
      number=5, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='returnsTo', full_name='fast.Graph.ContextEdges.returnsTo', index=5,
      number=6, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='computesFrom', full_name='fast.Graph.ContextEdges.computesFrom', index=6,
      number=7, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4253,
  serialized_end=4528,
)

_GRAPH_CONTEXTGRAPH = _descriptor.Descriptor(
  name='ContextGraph',
  full_name='fast.Graph.ContextGraph',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='edges', full_name='fast.Graph.ContextGraph.edges', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='nodeLabel', full_name='fast.Graph.ContextGraph.nodeLabel', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='nodeType', full_name='fast.Graph.ContextGraph.nodeType', index=2,
      number=3, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4531,
  serialized_end=4668,
)

_GRAPH_SYMBOLCANDIDATE = _descriptor.Descriptor(
  name='SymbolCandidate',
  full_name='fast.Graph.SymbolCandidate',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='symbolDummyNode', full_name='fast.Graph.SymbolCandidate.symbolDummyNode', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='symbolName', full_name='fast.Graph.SymbolCandidate.symbolName', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='isCorrect', full_name='fast.Graph.SymbolCandidate.isCorrect', index=2,
      number=3, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4670,
  serialized_end=4751,
)

_GRAPH_UNIT = _descriptor.Descriptor(
  name='Unit',
  full_name='fast.Graph.Unit',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='filename', full_name='fast.Graph.Unit.filename', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='slotTokenIdx', full_name='fast.Graph.Unit.slotTokenIdx', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='graph', full_name='fast.Graph.Unit.graph', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='SlotDummyNode', full_name='fast.Graph.Unit.SlotDummyNode', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='symbolCandidate', full_name='fast.Graph.Unit.symbolCandidate', index=4,
      number=5, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4754,
  serialized_end=4918,
)

_GRAPH = _descriptor.Descriptor(
  name='Graph',
  full_name='fast.Graph',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='unit', full_name='fast.Graph.unit', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_GRAPH_EDGE, _GRAPH_NODELABEL, _GRAPH_NODETYPE, _GRAPH_CONTEXTEDGES, _GRAPH_CONTEXTGRAPH, _GRAPH_SYMBOLCANDIDATE, _GRAPH_UNIT, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=4091,
  serialized_end=4918,
)


_DATA = _descriptor.Descriptor(
  name='Data',
  full_name='fast.Data',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='element', full_name='fast.Data.element', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='log', full_name='fast.Data.log', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='delta', full_name='fast.Data.delta', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='pairs', full_name='fast.Data.pairs', index=3,
      number=4, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='slices', full_name='fast.Data.slices', index=4,
      number=5, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='bugs', full_name='fast.Data.bugs', index=5,
      number=6, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='graph', full_name='fast.Data.graph', index=6,
      number=7, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='RecordType', full_name='fast.Data.RecordType',
      index=0, containing_type=None, fields=[]),
  ],
  serialized_start=4921,
  serialized_end=5151,
)

_ELEMENT_UNIT.fields_by_name['language'].enum_type = _ELEMENT_UNIT_LANGUAGETYPE
_ELEMENT_UNIT.containing_type = _ELEMENT
_ELEMENT_UNIT_LANGUAGETYPE.containing_type = _ELEMENT_UNIT
_ELEMENT_LITERAL.fields_by_name['type'].enum_type = _ELEMENT_LITERAL_LITERALTYPE
_ELEMENT_LITERAL.containing_type = _ELEMENT
_ELEMENT_LITERAL_LITERALTYPE.containing_type = _ELEMENT_LITERAL
_ELEMENT.fields_by_name['srcml_kind'].enum_type = _SRCMLKIND
_ELEMENT.fields_by_name['smali_kind'].enum_type = _SMALIKIND
_ELEMENT.fields_by_name['python3_kind'].enum_type = _PYTHON3KIND
_ELEMENT.fields_by_name['solidity_kind'].enum_type = _SOLIDITYKIND
_ELEMENT.fields_by_name['cobol85_kind'].enum_type = _COBOL85KIND
_ELEMENT.fields_by_name['child'].message_type = _ELEMENT
_ELEMENT.fields_by_name['unit'].message_type = _ELEMENT_UNIT
_ELEMENT.fields_by_name['literal'].message_type = _ELEMENT_LITERAL
_ELEMENT.fields_by_name['change'].enum_type = _ELEMENT_DIFFTYPE
_ELEMENT_DIFFTYPE.containing_type = _ELEMENT
_ELEMENT.oneofs_by_name['type'].fields.append(
  _ELEMENT.fields_by_name['srcml_kind'])
_ELEMENT.fields_by_name['srcml_kind'].containing_oneof = _ELEMENT.oneofs_by_name['type']
_ELEMENT.oneofs_by_name['type'].fields.append(
  _ELEMENT.fields_by_name['smali_kind'])
_ELEMENT.fields_by_name['smali_kind'].containing_oneof = _ELEMENT.oneofs_by_name['type']
_ELEMENT.oneofs_by_name['type'].fields.append(
  _ELEMENT.fields_by_name['python3_kind'])
_ELEMENT.fields_by_name['python3_kind'].containing_oneof = _ELEMENT.oneofs_by_name['type']
_ELEMENT.oneofs_by_name['type'].fields.append(
  _ELEMENT.fields_by_name['solidity_kind'])
_ELEMENT.fields_by_name['solidity_kind'].containing_oneof = _ELEMENT.oneofs_by_name['type']
_ELEMENT.oneofs_by_name['type'].fields.append(
  _ELEMENT.fields_by_name['cobol85_kind'])
_ELEMENT.fields_by_name['cobol85_kind'].containing_oneof = _ELEMENT.oneofs_by_name['type']
_ELEMENT.oneofs_by_name['extra'].fields.append(
  _ELEMENT.fields_by_name['unit'])
_ELEMENT.fields_by_name['unit'].containing_oneof = _ELEMENT.oneofs_by_name['extra']
_ELEMENT.oneofs_by_name['extra'].fields.append(
  _ELEMENT.fields_by_name['literal'])
_ELEMENT.fields_by_name['literal'].containing_oneof = _ELEMENT.oneofs_by_name['extra']
_DELTA_DIFF_MATCH.containing_type = _DELTA_DIFF
_DELTA_DIFF_ADD.containing_type = _DELTA_DIFF
_DELTA_DIFF_DEL.containing_type = _DELTA_DIFF
_DELTA_DIFF_MOVE.containing_type = _DELTA_DIFF
_DELTA_DIFF_UPDATE.containing_type = _DELTA_DIFF
_DELTA_DIFF.fields_by_name['type'].enum_type = _DELTA_DIFF_DELTATYPE
_DELTA_DIFF.fields_by_name['match'].message_type = _DELTA_DIFF_MATCH
_DELTA_DIFF.fields_by_name['add'].message_type = _DELTA_DIFF_ADD
_DELTA_DIFF.fields_by_name['del'].message_type = _DELTA_DIFF_DEL
_DELTA_DIFF.fields_by_name['move'].message_type = _DELTA_DIFF_MOVE
_DELTA_DIFF.fields_by_name['update'].message_type = _DELTA_DIFF_UPDATE
_DELTA_DIFF.containing_type = _DELTA
_DELTA_DIFF_DELTATYPE.containing_type = _DELTA_DIFF
_DELTA_DIFF.oneofs_by_name['delta'].fields.append(
  _DELTA_DIFF.fields_by_name['match'])
_DELTA_DIFF.fields_by_name['match'].containing_oneof = _DELTA_DIFF.oneofs_by_name['delta']
_DELTA_DIFF.oneofs_by_name['delta'].fields.append(
  _DELTA_DIFF.fields_by_name['add'])
_DELTA_DIFF.fields_by_name['add'].containing_oneof = _DELTA_DIFF.oneofs_by_name['delta']
_DELTA_DIFF.oneofs_by_name['delta'].fields.append(
  _DELTA_DIFF.fields_by_name['del'])
_DELTA_DIFF.fields_by_name['del'].containing_oneof = _DELTA_DIFF.oneofs_by_name['delta']
_DELTA_DIFF.oneofs_by_name['delta'].fields.append(
  _DELTA_DIFF.fields_by_name['move'])
_DELTA_DIFF.fields_by_name['move'].containing_oneof = _DELTA_DIFF.oneofs_by_name['delta']
_DELTA_DIFF.oneofs_by_name['delta'].fields.append(
  _DELTA_DIFF.fields_by_name['update'])
_DELTA_DIFF.fields_by_name['update'].containing_oneof = _DELTA_DIFF.oneofs_by_name['delta']
_DELTA.fields_by_name['diff'].message_type = _DELTA_DIFF
_PAIRS_PAIR_DIFF.fields_by_name['old_code'].message_type = _ELEMENT
_PAIRS_PAIR_DIFF.fields_by_name['new_code'].message_type = _ELEMENT
_PAIRS_PAIR_DIFF.fields_by_name['slices'].message_type = _SLICES
_PAIRS_PAIR_DIFF.containing_type = _PAIRS_PAIR
_PAIRS_PAIR.fields_by_name['left'].message_type = _PAIRS_PAIR_DIFF
_PAIRS_PAIR.fields_by_name['right'].message_type = _PAIRS_PAIR_DIFF
_PAIRS_PAIR.fields_by_name['type'].enum_type = _PAIRS_PAIR_CLONETYPE
_PAIRS_PAIR.containing_type = _PAIRS
_PAIRS_PAIR_CLONETYPE.containing_type = _PAIRS_PAIR
_PAIRS.fields_by_name['pair'].message_type = _PAIRS_PAIR
_LOG_COMMIT_DIFF_HUNK_MODLINE.containing_type = _LOG_COMMIT_DIFF_HUNK
_LOG_COMMIT_DIFF_HUNK.fields_by_name['element'].message_type = _ELEMENT
_LOG_COMMIT_DIFF_HUNK.fields_by_name['graph'].message_type = _GRAPH
_LOG_COMMIT_DIFF_HUNK.fields_by_name['mod'].message_type = _LOG_COMMIT_DIFF_HUNK_MODLINE
_LOG_COMMIT_DIFF_HUNK.fields_by_name['slice'].message_type = _SLICES
_LOG_COMMIT_DIFF_HUNK.containing_type = _LOG_COMMIT_DIFF
_LOG_COMMIT_DIFF.fields_by_name['hunk'].message_type = _LOG_COMMIT_DIFF_HUNK
_LOG_COMMIT_DIFF.containing_type = _LOG_COMMIT
_LOG_COMMIT.fields_by_name['diff'].message_type = _LOG_COMMIT_DIFF
_LOG_COMMIT.fields_by_name['slice'].message_type = _SLICES
_LOG_COMMIT.containing_type = _LOG
_LOG_AUTHOR.containing_type = _LOG
_LOG.fields_by_name['commit'].message_type = _LOG_COMMIT
_LOG.fields_by_name['author'].message_type = _LOG_AUTHOR
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION.fields_by_name['type'].enum_type = _SLICES_SLICE_CHANGETYPE
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION.containing_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_FUNCTIONDECL.containing_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.fields_by_name['pos'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.fields_by_name['type'].enum_type = _SLICES_SLICE_CHANGETYPE
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.fields_by_name['defn'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.fields_by_name['use'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.fields_by_name['cfunc'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_FUNCTIONDECL
_SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE.containing_type = _SLICES_SLICE_SOURCEFILE_FUNCTION
_SLICES_SLICE_SOURCEFILE_FUNCTION.fields_by_name['variable'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE
_SLICES_SLICE_SOURCEFILE_FUNCTION.fields_by_name['type'].enum_type = _SLICES_SLICE_CHANGETYPE
_SLICES_SLICE_SOURCEFILE_FUNCTION.containing_type = _SLICES_SLICE_SOURCEFILE
_SLICES_SLICE_SOURCEFILE.fields_by_name['function'].message_type = _SLICES_SLICE_SOURCEFILE_FUNCTION
_SLICES_SLICE_SOURCEFILE.fields_by_name['type'].enum_type = _SLICES_SLICE_CHANGETYPE
_SLICES_SLICE_SOURCEFILE.containing_type = _SLICES_SLICE
_SLICES_SLICE.fields_by_name['file'].message_type = _SLICES_SLICE_SOURCEFILE
_SLICES_SLICE.containing_type = _SLICES
_SLICES_SLICE_CHANGETYPE.containing_type = _SLICES_SLICE
_SLICES.fields_by_name['slice'].message_type = _SLICES_SLICE
_BUGS_BUG_INFO.containing_type = _BUGS_BUG
_BUGS_BUG.fields_by_name['buginfo'].message_type = _BUGS_BUG_INFO
_BUGS_BUG.containing_type = _BUGS
_BUGS.fields_by_name['bug'].message_type = _BUGS_BUG
_GRAPH_EDGE.containing_type = _GRAPH
_GRAPH_NODELABEL.containing_type = _GRAPH
_GRAPH_NODETYPE.containing_type = _GRAPH
_GRAPH_CONTEXTEDGES.fields_by_name['nextToken'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['child'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['lastLexicalUse'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['lastUse'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['lastWrite'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['returnsTo'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.fields_by_name['computesFrom'].message_type = _GRAPH_EDGE
_GRAPH_CONTEXTEDGES.containing_type = _GRAPH
_GRAPH_CONTEXTGRAPH.fields_by_name['edges'].message_type = _GRAPH_CONTEXTEDGES
_GRAPH_CONTEXTGRAPH.fields_by_name['nodeLabel'].message_type = _GRAPH_NODELABEL
_GRAPH_CONTEXTGRAPH.fields_by_name['nodeType'].message_type = _GRAPH_NODETYPE
_GRAPH_CONTEXTGRAPH.containing_type = _GRAPH
_GRAPH_SYMBOLCANDIDATE.containing_type = _GRAPH
_GRAPH_UNIT.fields_by_name['graph'].message_type = _GRAPH_CONTEXTGRAPH
_GRAPH_UNIT.fields_by_name['symbolCandidate'].message_type = _GRAPH_SYMBOLCANDIDATE
_GRAPH_UNIT.containing_type = _GRAPH
_GRAPH.fields_by_name['unit'].message_type = _GRAPH_UNIT
_DATA.fields_by_name['element'].message_type = _ELEMENT
_DATA.fields_by_name['log'].message_type = _LOG
_DATA.fields_by_name['delta'].message_type = _DELTA
_DATA.fields_by_name['pairs'].message_type = _PAIRS
_DATA.fields_by_name['slices'].message_type = _SLICES
_DATA.fields_by_name['bugs'].message_type = _BUGS
_DATA.fields_by_name['graph'].message_type = _GRAPH
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['element'])
_DATA.fields_by_name['element'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['log'])
_DATA.fields_by_name['log'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['delta'])
_DATA.fields_by_name['delta'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['pairs'])
_DATA.fields_by_name['pairs'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['slices'])
_DATA.fields_by_name['slices'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['bugs'])
_DATA.fields_by_name['bugs'].containing_oneof = _DATA.oneofs_by_name['RecordType']
_DATA.oneofs_by_name['RecordType'].fields.append(
  _DATA.fields_by_name['graph'])
_DATA.fields_by_name['graph'].containing_oneof = _DATA.oneofs_by_name['RecordType']
DESCRIPTOR.message_types_by_name['Element'] = _ELEMENT
DESCRIPTOR.message_types_by_name['Delta'] = _DELTA
DESCRIPTOR.message_types_by_name['Pairs'] = _PAIRS
DESCRIPTOR.message_types_by_name['Log'] = _LOG
DESCRIPTOR.message_types_by_name['Slices'] = _SLICES
DESCRIPTOR.message_types_by_name['Bugs'] = _BUGS
DESCRIPTOR.message_types_by_name['Graph'] = _GRAPH
DESCRIPTOR.message_types_by_name['Data'] = _DATA
DESCRIPTOR.enum_types_by_name['Cobol85Kind'] = _COBOL85KIND
DESCRIPTOR.enum_types_by_name['Python3Kind'] = _PYTHON3KIND
DESCRIPTOR.enum_types_by_name['SmaliKind'] = _SMALIKIND
DESCRIPTOR.enum_types_by_name['SolidityKind'] = _SOLIDITYKIND
DESCRIPTOR.enum_types_by_name['SrcmlKind'] = _SRCMLKIND
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

Element = _reflection.GeneratedProtocolMessageType('Element', (_message.Message,), dict(

  Unit = _reflection.GeneratedProtocolMessageType('Unit', (_message.Message,), dict(
    DESCRIPTOR = _ELEMENT_UNIT,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Element.Unit)
    ))
  ,

  Literal = _reflection.GeneratedProtocolMessageType('Literal', (_message.Message,), dict(
    DESCRIPTOR = _ELEMENT_LITERAL,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Element.Literal)
    ))
  ,
  DESCRIPTOR = _ELEMENT,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Element)
  ))
_sym_db.RegisterMessage(Element)
_sym_db.RegisterMessage(Element.Unit)
_sym_db.RegisterMessage(Element.Literal)

Delta = _reflection.GeneratedProtocolMessageType('Delta', (_message.Message,), dict(

  Diff = _reflection.GeneratedProtocolMessageType('Diff', (_message.Message,), dict(

    Match = _reflection.GeneratedProtocolMessageType('Match', (_message.Message,), dict(
      DESCRIPTOR = _DELTA_DIFF_MATCH,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Delta.Diff.Match)
      ))
    ,

    Add = _reflection.GeneratedProtocolMessageType('Add', (_message.Message,), dict(
      DESCRIPTOR = _DELTA_DIFF_ADD,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Delta.Diff.Add)
      ))
    ,

    Del = _reflection.GeneratedProtocolMessageType('Del', (_message.Message,), dict(
      DESCRIPTOR = _DELTA_DIFF_DEL,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Delta.Diff.Del)
      ))
    ,

    Move = _reflection.GeneratedProtocolMessageType('Move', (_message.Message,), dict(
      DESCRIPTOR = _DELTA_DIFF_MOVE,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Delta.Diff.Move)
      ))
    ,

    Update = _reflection.GeneratedProtocolMessageType('Update', (_message.Message,), dict(
      DESCRIPTOR = _DELTA_DIFF_UPDATE,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Delta.Diff.Update)
      ))
    ,
    DESCRIPTOR = _DELTA_DIFF,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Delta.Diff)
    ))
  ,
  DESCRIPTOR = _DELTA,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Delta)
  ))
_sym_db.RegisterMessage(Delta)
_sym_db.RegisterMessage(Delta.Diff)
_sym_db.RegisterMessage(Delta.Diff.Match)
_sym_db.RegisterMessage(Delta.Diff.Add)
_sym_db.RegisterMessage(Delta.Diff.Del)
_sym_db.RegisterMessage(Delta.Diff.Move)
_sym_db.RegisterMessage(Delta.Diff.Update)

Pairs = _reflection.GeneratedProtocolMessageType('Pairs', (_message.Message,), dict(

  Pair = _reflection.GeneratedProtocolMessageType('Pair', (_message.Message,), dict(

    Diff = _reflection.GeneratedProtocolMessageType('Diff', (_message.Message,), dict(
      DESCRIPTOR = _PAIRS_PAIR_DIFF,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Pairs.Pair.Diff)
      ))
    ,
    DESCRIPTOR = _PAIRS_PAIR,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Pairs.Pair)
    ))
  ,
  DESCRIPTOR = _PAIRS,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Pairs)
  ))
_sym_db.RegisterMessage(Pairs)
_sym_db.RegisterMessage(Pairs.Pair)
_sym_db.RegisterMessage(Pairs.Pair.Diff)

Log = _reflection.GeneratedProtocolMessageType('Log', (_message.Message,), dict(

  Commit = _reflection.GeneratedProtocolMessageType('Commit', (_message.Message,), dict(

    Diff = _reflection.GeneratedProtocolMessageType('Diff', (_message.Message,), dict(

      Hunk = _reflection.GeneratedProtocolMessageType('Hunk', (_message.Message,), dict(

        ModLine = _reflection.GeneratedProtocolMessageType('ModLine', (_message.Message,), dict(
          DESCRIPTOR = _LOG_COMMIT_DIFF_HUNK_MODLINE,
          __module__ = 'fast_pb2'
          # @@protoc_insertion_point(class_scope:fast.Log.Commit.Diff.Hunk.ModLine)
          ))
        ,
        DESCRIPTOR = _LOG_COMMIT_DIFF_HUNK,
        __module__ = 'fast_pb2'
        # @@protoc_insertion_point(class_scope:fast.Log.Commit.Diff.Hunk)
        ))
      ,
      DESCRIPTOR = _LOG_COMMIT_DIFF,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Log.Commit.Diff)
      ))
    ,
    DESCRIPTOR = _LOG_COMMIT,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Log.Commit)
    ))
  ,

  Author = _reflection.GeneratedProtocolMessageType('Author', (_message.Message,), dict(
    DESCRIPTOR = _LOG_AUTHOR,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Log.Author)
    ))
  ,
  DESCRIPTOR = _LOG,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Log)
  ))
_sym_db.RegisterMessage(Log)
_sym_db.RegisterMessage(Log.Commit)
_sym_db.RegisterMessage(Log.Commit.Diff)
_sym_db.RegisterMessage(Log.Commit.Diff.Hunk)
_sym_db.RegisterMessage(Log.Commit.Diff.Hunk.ModLine)
_sym_db.RegisterMessage(Log.Author)

Slices = _reflection.GeneratedProtocolMessageType('Slices', (_message.Message,), dict(

  Slice = _reflection.GeneratedProtocolMessageType('Slice', (_message.Message,), dict(

    SourceFile = _reflection.GeneratedProtocolMessageType('SourceFile', (_message.Message,), dict(

      Function = _reflection.GeneratedProtocolMessageType('Function', (_message.Message,), dict(

        Variable = _reflection.GeneratedProtocolMessageType('Variable', (_message.Message,), dict(

          Position = _reflection.GeneratedProtocolMessageType('Position', (_message.Message,), dict(
            DESCRIPTOR = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_POSITION,
            __module__ = 'fast_pb2'
            # @@protoc_insertion_point(class_scope:fast.Slices.Slice.SourceFile.Function.Variable.Position)
            ))
          ,

          FunctionDecl = _reflection.GeneratedProtocolMessageType('FunctionDecl', (_message.Message,), dict(
            DESCRIPTOR = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE_FUNCTIONDECL,
            __module__ = 'fast_pb2'
            # @@protoc_insertion_point(class_scope:fast.Slices.Slice.SourceFile.Function.Variable.FunctionDecl)
            ))
          ,
          DESCRIPTOR = _SLICES_SLICE_SOURCEFILE_FUNCTION_VARIABLE,
          __module__ = 'fast_pb2'
          # @@protoc_insertion_point(class_scope:fast.Slices.Slice.SourceFile.Function.Variable)
          ))
        ,
        DESCRIPTOR = _SLICES_SLICE_SOURCEFILE_FUNCTION,
        __module__ = 'fast_pb2'
        # @@protoc_insertion_point(class_scope:fast.Slices.Slice.SourceFile.Function)
        ))
      ,
      DESCRIPTOR = _SLICES_SLICE_SOURCEFILE,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Slices.Slice.SourceFile)
      ))
    ,
    DESCRIPTOR = _SLICES_SLICE,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Slices.Slice)
    ))
  ,
  DESCRIPTOR = _SLICES,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Slices)
  ))
_sym_db.RegisterMessage(Slices)
_sym_db.RegisterMessage(Slices.Slice)
_sym_db.RegisterMessage(Slices.Slice.SourceFile)
_sym_db.RegisterMessage(Slices.Slice.SourceFile.Function)
_sym_db.RegisterMessage(Slices.Slice.SourceFile.Function.Variable)
_sym_db.RegisterMessage(Slices.Slice.SourceFile.Function.Variable.Position)
_sym_db.RegisterMessage(Slices.Slice.SourceFile.Function.Variable.FunctionDecl)

Bugs = _reflection.GeneratedProtocolMessageType('Bugs', (_message.Message,), dict(

  Bug = _reflection.GeneratedProtocolMessageType('Bug', (_message.Message,), dict(

    Info = _reflection.GeneratedProtocolMessageType('Info', (_message.Message,), dict(
      DESCRIPTOR = _BUGS_BUG_INFO,
      __module__ = 'fast_pb2'
      # @@protoc_insertion_point(class_scope:fast.Bugs.Bug.Info)
      ))
    ,
    DESCRIPTOR = _BUGS_BUG,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Bugs.Bug)
    ))
  ,
  DESCRIPTOR = _BUGS,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Bugs)
  ))
_sym_db.RegisterMessage(Bugs)
_sym_db.RegisterMessage(Bugs.Bug)
_sym_db.RegisterMessage(Bugs.Bug.Info)

Graph = _reflection.GeneratedProtocolMessageType('Graph', (_message.Message,), dict(

  Edge = _reflection.GeneratedProtocolMessageType('Edge', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_EDGE,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.Edge)
    ))
  ,

  NodeLabel = _reflection.GeneratedProtocolMessageType('NodeLabel', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_NODELABEL,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.NodeLabel)
    ))
  ,

  NodeType = _reflection.GeneratedProtocolMessageType('NodeType', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_NODETYPE,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.NodeType)
    ))
  ,

  ContextEdges = _reflection.GeneratedProtocolMessageType('ContextEdges', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_CONTEXTEDGES,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.ContextEdges)
    ))
  ,

  ContextGraph = _reflection.GeneratedProtocolMessageType('ContextGraph', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_CONTEXTGRAPH,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.ContextGraph)
    ))
  ,

  SymbolCandidate = _reflection.GeneratedProtocolMessageType('SymbolCandidate', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_SYMBOLCANDIDATE,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.SymbolCandidate)
    ))
  ,

  Unit = _reflection.GeneratedProtocolMessageType('Unit', (_message.Message,), dict(
    DESCRIPTOR = _GRAPH_UNIT,
    __module__ = 'fast_pb2'
    # @@protoc_insertion_point(class_scope:fast.Graph.Unit)
    ))
  ,
  DESCRIPTOR = _GRAPH,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Graph)
  ))
_sym_db.RegisterMessage(Graph)
_sym_db.RegisterMessage(Graph.Edge)
_sym_db.RegisterMessage(Graph.NodeLabel)
_sym_db.RegisterMessage(Graph.NodeType)
_sym_db.RegisterMessage(Graph.ContextEdges)
_sym_db.RegisterMessage(Graph.ContextGraph)
_sym_db.RegisterMessage(Graph.SymbolCandidate)
_sym_db.RegisterMessage(Graph.Unit)

Data = _reflection.GeneratedProtocolMessageType('Data', (_message.Message,), dict(
  DESCRIPTOR = _DATA,
  __module__ = 'fast_pb2'
  # @@protoc_insertion_point(class_scope:fast.Data)
  ))
_sym_db.RegisterMessage(Data)


# @@protoc_insertion_point(module_scope)
