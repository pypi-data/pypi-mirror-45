...
def syslog(message, level=LEVEL['notice'], facility=FACILITY['daemon'],
	host='localhost', port=514, tag=None):
	...
	if not tag:
		tag = os.path.basename(inspect.stack()[1][1])
	timestamp = datetime.strftime(datetime.now(), '%b %d %H:%M:%S')
	sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	ip = socket.gethostbyname(socket.gethostname())
	data = '<%d>%s %s %s: %s' % (level + facility*8, timestamp, ip, tag, message)
	try:
		if not host:
			host = '127.0.0.1'
		if os.getenv('DEBUG'):
			print "HOST SYSLOG [syslog.py] [77] =", host
			print "PORT SYSLOG [syslog.py] [77] =", port
