...
""""
TODO:
- move working dir to OS temp dir
--> don't do that as it would make recovery of lost jobs after worker restart harder
- daemon setup
- setup action cache
- load actions and envs into cache - memcache? needs to be shared across worker subprocesses
"""
...
class NjinnWorker():
    ...
    def config_update(self):
        api_base = self.config['DEFAULT']['njinn_api']
        url = api_base + '/api/v1/workercom/config'

        headers = {"Authorization": self.config['DEFAULT']['secret']}
...
def main():
    ...
    registration_token = sys.argv[1] if len(sys.argv) > 1 else None
