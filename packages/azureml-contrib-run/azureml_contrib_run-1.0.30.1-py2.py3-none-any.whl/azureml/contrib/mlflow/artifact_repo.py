...
from .utils import artifact_uri_decomp, get_workspace_from_url, _EXP_NAME, _RUN_ID
...
class AzureMLflowArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri):
        ...
        parsed_path = artifact_uri_decomp(parsed_url.path)
        experiment_name = parsed_path[_EXP_NAME]
        run_id = parsed_path[_RUN_ID]
        self._run_id = run_id

        workspace = get_workspace_from_url(parsed_url)
        self.run_artifacts = RunArtifactsExtensionClient(workspace.service_context, experiment_name)
