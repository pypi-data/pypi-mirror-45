...
lang_to_scanner = {"en": Scanner(locale_en), "nl": Scanner(locale_nl)}
...
def datify(cleaned_bundle, past_boundary, future_boundary, now):
    ...
    if erdt is not None:
        ...
        start_date, _, _ = resolve_end_period(
            start_date, min_level, past_boundary, future_boundary, now
        )
    ...
    if not metadates:
        ...
        if "first" in words:
            ...
            start_date, _, date_changed = resolve_end_period(
                dt, min_dt_level, past_boundary, future_boundary, now
            )
...
def parse_date(
    ...
    past = past or rd()
    future = future or rd()
    locale = scanner or lang_to_locale[lang]
    ...
    for cleaned_bundle in cleaned_parts:
        ...
        start_date, end_date, levels, span = datify(
            cleaned_bundle, past_boundary, future_boundary, now
        )
