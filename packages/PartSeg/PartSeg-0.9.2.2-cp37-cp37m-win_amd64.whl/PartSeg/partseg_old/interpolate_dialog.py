...
class InterpolateDialog(QDialog):
    def __init__(self, spacing, *args, **kwargs):
        ...
        if len(start_value) == 3:
            self.z_spacing.setValue(start_value[2])
        ...
        layout = QGridLayout()
        layout.addWidget(QLabel("x scale"),0,0)
        layout.addWidget(self.x_spacing, 0, 1)
        layout.addWidget(QLabel("y scale"), 1, 0)
        layout.addWidget(self.y_spacing, 1, 1)
        if len(start_value) == 3:
            layout.addWidget(QLabel("z scale"), 2, 0)
            layout.addWidget(self.z_spacing, 2, 1)
        layout.addWidget(self.accept_button, 3, 0)
