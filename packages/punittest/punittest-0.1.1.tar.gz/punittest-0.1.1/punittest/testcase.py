...
def _assertion(func):
    """用来装饰PUnittest类中所有的AssertXxx方法，提供日志，
...
def _handler(rerun=Settings.CASE_FAIL_RERUN):
        ...
        def wrap(*args, **kwargs):
            ...
            while rerun_time > 0:
                try:
                    ...
                    rerun_time -= 1
                    if rerun_time <= 0:
                        # exc_type, exc_msg, exc_traceback = sys.exc_info()
                        logger.error('[TestResult]: Fail!')
                        # logger.error('[FailReason]: {0}: {1}'.format(exc_type.__name__, exc_msg))
                        raise e
                    ...
                    if rerun_time <= 0:
                        ...
                        # import traceback
                        # for t in traceback.format_exc().splitlines():
                        #     logger.error('[Traceback]: {0}'.format(t))
                        raise e
        return wrap
    return wrapper
...
class TestCaseHandler:
    ...
    def create_case_with_case_data(func):
        ...
        for index, (param, assert_) in enumerate(zip(params, asserts), 1):
            ...
            result[func_name] = _handler()(_feed_data(params=param, asserts=assert_)(func))
        return result
