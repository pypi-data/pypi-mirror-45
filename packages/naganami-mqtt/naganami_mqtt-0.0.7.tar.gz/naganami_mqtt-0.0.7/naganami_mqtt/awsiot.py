...
class AwsIotContoller(MqttController):
    ...
    def _set_subscribe_topics(self, client, userdata, flags, respons_code):
        ...
        client.subscribe('$aws/things/{0}/jobs/+/get/rejected'.format(self.name))
        client.subscribe('$aws/things/{0}/jobs/+/update/accepted'.format(self.name))
        client.subscribe('$aws/things/{0}/jobs/+/update/rejected'.format(self.name))
