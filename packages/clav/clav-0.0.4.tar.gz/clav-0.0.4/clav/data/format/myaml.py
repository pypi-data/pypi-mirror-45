...
# clav.text.yaml uses this module for its implementation of yaml.dumps().
# this module has been modified to:
#   - quote strings with {{ or }} in them (for ansible support)
#   - quote a string with " if it contains '
#   - improve readability of chunks
#
# Courtesy of https://github.com/mk-fg/pretty-yaml
...
class PrettyYAMLDumper(yaml.dumper.SafeDumper):
	...
	def represent_undefined(dumper, data):
		if (
			...
			hasattr(data, '_make') and
			hasattr(data, '_asdict')
		):
		...
		if node in self.anchors:
			if self.anchors[node] is None and not self.pyaml_force_embed:
				if not hint:
					...
					s = '_-_'.join(map(op.attrgetter('value'), hint))
					self.anchors[node] = self.pyaml_transliterate('{}'.format(s))
				# OLD - naz
				# self.anchors[node] = self.generate_anchor(node)\
...
class UnsafePrettyYAMLDumper(PrettyYAMLDumper):
	...
	def choose_scalar_style(self):
		...
		if self.event.style != 'plain':
			...
			if self.event.value[0] in '!&*-:?{}[],#|>@`':
				return '"' if "'" in self.event.value else "'"
			if any(_ in self.event.value for _ in (' ', '{{', '}}')):
				return '"' if "'" in self.event.value else "'"
			else:
				return None
...
UnsafePrettyYAMLDumper.add_representer(
	type, lambda s, t: s.represent_stringish(repr(t))
)
...
def dump(
	...
	**pyyaml_kws
	...
	if dst is bytes:
		...
		try:
			dst.write(b'') # tests if dst is unicode- or bytestream
		except:
			dst.write(buff.decode('utf-8'))
		else:
			dst.write(buff)
