...
from genomoncology.parse import DocType, is_not_skipped_call
...
@options.build_option
@options.pass_state
def aggregate_tsv(state, aggregate_key):
...
@options.build_option
@options.pass_state
def annotate_calls(state, fields, run_async, delete_if_exists):
    ...
    return [
        ...
        partition_all(state.batch_size),  # annotate calls is GET, hardcoded batch size REPLACE with char length batching
        create_function(
            state,
            sdk_function,
            fields=fields,
...
@options.build_option
@options.pass_state
def load_warehouse(state, workers):
...
def region_search(state):
    ...
    return [
        ...
        kms.create_sync_processor(state, kms.transcripts.process_transcripts),
        sinks.JsonlFileSink,
    ]
