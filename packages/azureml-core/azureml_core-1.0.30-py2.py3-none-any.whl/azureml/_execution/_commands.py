...
def cancel(project_object, run_config_object, run_id):
    ...
    if _is_local_target(run_config_object.target, custom_target_dict):
        _cancel_local(project_object, run_id)
    else:
        body = {"RunId": run_id,
                "Target": run_config_object.target,
        ...
        service_context = project_object.workspace.service_context
        uri = "{}/execution/v1.0{}/experiments/{}/cancel".format(service_context._get_execution_url(),
...
def _cancel_local(project_object, run_id):
    project_temp_dir = _get_project_temporary_directory(run_id)
    ...
    from azureml._restclient.run_client import RunClient
    run_client = RunClient(project_object.workspace.service_context,
                           project_object.history.name,
                           run_id)
...
def _read_run_configuration(path):
    ...
    with open(path, 'r') as file:
        return ruamel.yaml.load(file.read())
