...
from azureml._base_sdk_common.common import RUNCONFIGURATION_EXTENSION, AML_CONFIG_DIR,\
    AZUREML_DIR, COMPUTECONTEXT_EXTENSION, \
    get_project_files, to_camel_case, to_snake_case, get_run_config_dir_path, get_run_config_dir_name
...
class EnvironmentDefinition(Environment):
    """This is the new environment object - Keeping for backward compatability."""
...
class RunConfiguration(_AbstractRunConfigElement):
    ...
    def save(self, path=None, name=None, separate_environment_yaml=False):
        ...
        :param path: A user selected root directory for run configurations. Typically this is the Git Repository
            or the python project root directory. The configuration is saved to a sub dirctory named .azureml.
        :type path: str
        ...
        if not self.environment.python.conda_dependencies_file:
            self.environment.python.conda_dependencies_file = run_config_dir_name + "/"\
    ...
    def _save_with_default_comments(self, path, name, separate_environment_yaml=False):
        ...
        if separate_environment_yaml:
            ...
            full_file_path = os.path.join(path,
                                          run_config_dir_name, run_config_file_name)
            with open(full_file_path, 'w') as outfile:
                ruamel.yaml.round_trip_dump(commented_map_dict, outfile)

            full_file_path = os.path.join(path,
                                          run_config_dir_name, "environment.yml")
            with open(full_file_path, 'w') as outfile:
    ...
    def _get_runconfig_using_dict(cls, commented_map_or_dict, path=None, name=None):
        ...
        if cls._check_old_config(commented_map_or_dict):
            ...
            if inline_conda_dependencies:
                ...
                if run_config_object.environment.python.conda_dependencies_file is None:
                    run_config_object.environment.python.conda_dependencies_file = ".azureml/conda_dependencies.yml"
                conda_dependencies = CondaDependencies(
                    os.path.join(path,
