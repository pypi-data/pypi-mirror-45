...
class AksWebservice(Webservice):
    ...
    def _initialize(self, workspace, obj_dict):
        ...
        self.liveness_probe_requirements = \
            LivenessProbeRequirements.deserialize(obj_dict['livenessProbeRequirements'])
    ...
    def _validate_update(self, image, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas,
        ...
        if gpu_cores and gpu_cores <= 0:
            raise WebserviceException('Error, gpu_cores must be greater than zero.')
        ...
        if period_seconds and period_seconds <= 0:
            raise WebserviceException('Error, period_seconds must be greater than zero.')
        if timeout_seconds and timeout_seconds <= 0:
            raise WebserviceException('Error, timeout_seconds must be greater than zero.')
        if initial_delay_seconds and initial_delay_seconds <= 0:
            raise WebserviceException('Error, initial_delay_seconds must be greater than zero.')
        if success_threshold and success_threshold <= 0:
            raise WebserviceException('Error, success_threshold must be greater than zero.')
        if failure_threshold and failure_threshold <= 0:
            raise WebserviceException('Error, failure_threshold must be greater than zero.')
        ...
        if inference_config is None and models:
            raise WebserviceException('Error, both "models" and "inference_config" inputs must be provided in order '
                                      'to update any of the parameters in inference_config.')
...
class AksServiceDeploymentConfiguration(WebserviceDeploymentConfiguration):
    ...
    def __init__(self, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_refresh_seconds,
        ...
        :param gpu_cores: The number of gpu cores to allocate for this Webservice
        :type gpu_cores: int
        :param period_seconds: How often (in seconds) to perform the probe. Default to 10 seconds. Minimum value is 1.
        :type period_seconds: int
        :param initial_delay_seconds: Number of seconds after the container has started before liveness probe
            is initiated.
        :type initial_delay_seconds: int
        ...
        :param success_threshold: Minimum consecutive successes for the probe to be considered successful
            after having failed. Defaults to 1. Minimum value is 1.
        :type success_threshold: int
        :param failure_threshold: When a Pod starts and the probe fails, Kubernetes will try failureThreshold times
            before giving up. Defaults to 3. Minimum value is 1.
        :type failure_threshold: int
        :raises: WebserviceException
        """
    ...
    def validate_configuration(self):
        ...
        if self.gpu_cores and self.gpu_cores <= 0:
            raise WebserviceException('Invalid configuration, gpu_cores must be greater than zero.')
        if self.period_seconds and self.period_seconds <= 0:
            raise WebserviceException('Invalid configuration, period_seconds must be greater than zero.')
        if self.initial_delay_seconds and self.initial_delay_seconds <= 0:
            raise WebserviceException('Invalid configuration, initial_delay_seconds must be greater than zero.')
        if self.timeout_seconds and self.timeout_seconds <= 0:
            raise WebserviceException('Invalid configuration, timeout_seconds must be greater than zero.')
        if self.success_threshold and self.success_threshold <= 0:
            raise WebserviceException('Invalid configuration, success_threshold must be greater than zero.')
        if self.failure_threshold and self.failure_threshold <= 0:
            raise WebserviceException('Invalid configuration, failure_threshold must be greater than zero.')
