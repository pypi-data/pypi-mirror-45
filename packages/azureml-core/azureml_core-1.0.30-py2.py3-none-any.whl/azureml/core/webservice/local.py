    ...
    def decorator(func):
        def decorated(self, *args, **kwargs):
            if self._state not in states:
                raise WebserviceException('Cannot call {}() when service is {}.'.format(func.__name__, self._state))
...
class LocalWebservice(Webservice):
    ...
    def delete(self, delete_cache=True, delete_image=False, delete_volume=True):
        ...
        :param delete_volume: Delete the service's Docker volume.
        ...
        self._state = LocalWebservice.STATE_DELETED
    ...
    def deploy_to_cloud(self, name=None, deployment_config=None, deployment_target=None):
        ...
        :param deployment_target: A ComputeTarget to which to deploy the Webservice. As ACI has no associated
            ComputeTarget, leave this parameter as None to deploy to ACI.
    ...
    def get_logs(self, num_lines=5000, raw=False):
        ...
        :type num_lines: int
        :param raw: Return the raw Docker container output without attempting to format it.
        :type raw: bool
    ...
    def run(self, input_data):
        ...
        :raises: WebserviceException
        """
        if self._state == LocalWebservice.STATE_DEPLOYING:
    ...
    def serialize(self):
        ...
        :raises: NotImplementedError
        """
        raise NotImplementedError('Local web services cannot be serialized.')

    @_in_state([STATE_DEPLOYING, STATE_FAILED, STATE_RUNNING, STATE_UNKNOWN])
    def update(self, models=None, image_config=None, deployment_config=None, wait=False):
        """
        ...
        if image_config and not isinstance(image_config, ContainerImageConfig):
            raise WebserviceException('Error, provided image configuration must be of type ContainerImageConfig '
                                      'in order to update a local service.')
    ...
    def update_deployment_state(self, must_exist=False):
        ...
        if container:
            ...
            self._state = LocalWebservice.STATE_UNKNOWN
    ...
    def wait_for_deployment(self, show_output=False):
        ...
        try:
            ...
            self._state = LocalWebservice.STATE_RUNNING
            print('Local webservice is {} at {}'.format(self._state, self._base_url))
        except:
            self._state = LocalWebservice.STATE_FAILED
            raise
    ...
    def _collect_models(self):
        ...
        if not getattr(self, '_models_dir', None):
            self._models_dir = os.path.join(tempfile.gettempdir(),
                                            LocalWebservice.MODELS_DIR_TEMP_PREFIX + self._name)
    ...
    def _deploy(workspace, name, models, image_config, deployment_config, wait=False):
        ...
        :type models: list
        :param image_config:
        :type image_config: azureml.core.image.ContainerImageConfig
        ...
        if not isinstance(image_config, ContainerImageConfig):
            raise WebserviceException('Error, provided image configuration must be of type ContainerImageConfig '
                                      'in order to deploy a local service.')
    ...
    def _generate_base_urls(self):
        ...
        port = get_docker_port(self._docker_client, self._name, self._container)
    ...
    def _run_container(self, wait):
        ...
        container_name = LocalWebservice._construct_container_name(self._name)
        ...
        labels = {
            ...
            LocalWebservice.CONTAINER_LABEL_SERVICE_NAME: self._name,
        }
        ...
        self._state = LocalWebservice.STATE_DEPLOYING
    ...
    def _update_docker_context(self):
        ...
        try:
            response = requests.post(image_url, headers=headers, params=params, json=json_payload)
    ...
    def _update_volume(self):
        ...
        if getattr(self, '_volume', None) is None:
            self._volume_name = LocalWebservice._construct_volume_name(self._name)
