...
cdef object _rand_uint64_broadcast(object low, object high, object size,
    ...
    if np.issubdtype(dt, np.integer):
        ...
        if np.any(np.less_equal(high_arr, 0x0ULL)):
            raise ValueError('low >= high')
...
cdef object _rand_int64_broadcast(object low, object high, object size,
    ...
    if np.issubdtype(dt, np.integer):
        # Avoid object dtype path if already an integer
        if np.any(np.less_equal(high_arr, -0x8000000000000000LL)):
            raise ValueError('low >= high')
        high_m1 = high_arr - dt.type(1)
...
cdef object _rand_uint64(object low, object high, object size,
    ...
    out-of-range for uint64. The scalar path leaves integers as Python
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            return out_arr
    return _rand_uint64_broadcast(low_arr, high_arr, size, use_masked, state, lock)
...
cdef object _rand_uint32(object low, object high, object size,
    ...
    The internal integer generator produces values from the closed
    interval [low, high-1].  This requires some care since high can be
    out-of-range for uint32. The scalar path leaves integers as Python
    integers until the 1 has been subtracted to avoid needing to cast
    to a larger type.
    """
    cdef np.ndarray out_arr, low_arr, high_arr
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            return out_arr
    return _rand_uint32_broadcast(low_arr, high_arr, size, use_masked, state, lock)
...
cdef object _rand_uint16(object low, object high, object size,
    ...
    The internal integer generator produces values from the closed
    interval [low, high-1].  This requires some care since high can be
    out-of-range for uint16. The scalar path leaves integers as Python
    integers until the 1 has been subtracted to avoid needing to cast
    to a larger type.
    """
    cdef np.ndarray out_arr, low_arr, high_arr
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            return out_arr
    return _rand_uint16_broadcast(low_arr, high_arr, size, use_masked, state, lock)
...
cdef object _rand_uint8(object low, object high, object size,
    ...
    The internal integer generator produces values from the closed
    interval [low, high-1].  This requires some care since high can be
    out-of-range for uint8. The scalar path leaves integers as Python
    integers until the 1 has been subtracted to avoid needing to cast
    to a larger type.
    """
    cdef np.ndarray out_arr, low_arr, high_arr
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            return out_arr
    return _rand_uint8_broadcast(low_arr, high_arr, size, use_masked, state, lock)
...
cdef object _rand_bool(object low, object high, object size,
    ...
    The internal integer generator produces values from the closed
    interval [low, high-1].  This requires some care since high can be
    out-of-range for bool. The scalar path leaves integers as Python
    integers until the 1 has been subtracted to avoid needing to cast
    to a larger type.
    """
    cdef np.ndarray out_arr, low_arr, high_arr
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            return out_arr
    return _rand_bool_broadcast(low_arr, high_arr, size, use_masked, state, lock)
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            with lock, nogil:
                random_bounded_uint64_fill(state, off, rng, cnt, use_masked, out_data)
            return out_arr
    return _rand_int64_broadcast(low_arr, high_arr, size, use_masked, state, lock)
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            with lock, nogil:
                random_bounded_uint32_fill(state, off, rng, cnt, use_masked, out_data)
            return out_arr
    return _rand_int32_broadcast(low_arr, high_arr, size, use_masked, state, lock)
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            with lock, nogil:
                random_bounded_uint16_fill(state, off, rng, cnt, use_masked, out_data)
            return out_arr
    return _rand_int16_broadcast(low_arr, high_arr, size, use_masked, state, lock)
    ...
    if ((low_ndim == 0 or (low_ndim == 1 and low_arr.size == 1 and size is not None)) and
        ...
        if size is None:
            ...
            with lock, nogil:
                random_bounded_uint8_fill(state, off, rng, cnt, use_masked, out_data)
            return out_arr
    return _rand_int8_broadcast(low_arr, high_arr, size, use_masked, state, lock)
