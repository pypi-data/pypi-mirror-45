...
class ERRORS(IntEnum):
    ...
    TOKEN_DISPENSER_CLIENT_ERROR = 7
    TOKEN_DISPENSER_RETURN_ERROR = 8
...
class GPlaycli:
    ...
    def __init__(self, args=None, config_file=None):
        ...
        # token belongs to certain nation
        if args is not None and args.nation is not None:
            self.nation = str.upper(args.nation)
        else:
            self.nation = str.upper('cn')
        ...
        self.tokencachefile = os.path.expanduser(
            self.configparser.get("Cache", "token", fallback="token.cache")) + "/" + self.nation + "_" + self.device_codename
        self.api = None
        ...
        # if args are passed
            ...
            if self.verbose:
                ...
                formatter = MyFormatter(fmt='[%(asctime)s %(levelname)-8s %(message)s', datefmt='%Y-%m-%d %H:%M:%S.%f')
    ...
    def retrieve_token(self, force_new=False):
        ...
        if token is not None and not force_new:
            ...
            return token, gsfid
        logger.info("Retrieving token ..., token_url:%s,device_codename:%s,nation:%s", self.token_url, self.device_codename, self.nation)
        start = time.time()
        response = requests.get(self.token_url, params={"device_codename":self.device_codename, "nation":self.nation})
        roundtrip = time.time() - start
        logger.info("Retrieving token use time:%s s, status_code:%s, response.text:%s", roundtrip, response.status_code, response.text)
        if response.status_code != 200:
            logger.error('Token server request error')
            sys.exit(ERRORS.TOKEN_DISPENSER_CLIENT_ERROR)
        elif response.text == 'Auth error':
        ...
        elif response.text.startswith("Token login error"):
            logger.error('Token login error: %s', response.text)
            sys.exit(ERRORS.TOKEN_DISPENSER_SERVER_ERROR)
    ...
    def download(self, pkg_todownload):
        ...
        for detail, item in zip(details, pkg_todownload):
            ...
            # Download
            try:
                if len(detail['offer']) > 0 and detail['offer'][0]['checkoutFlowRequired']:
    ...
    @hooks.connected
    def home(self):
        homeapps = self.api.getHomeApps()
        logger.info("homeapps:%s", json.dumps(homeapps))
        return homeapps

    @hooks.connected
    def browse(self,cat=None, subCat=None):
        browseapps = self.api.browse(cat,subCat)
        logger.info("browseapps:%s", json.dumps(browseapps))
        return browseapps

    @hooks.connected
    def list(self, cat, ctr=None, nb_results=None, offset=None):
        listapps = self.api.list(cat, ctr, nb_results, offset)
        logger.info("listapps:%s", json.dumps(listapps))
        return listapps

    @hooks.connected
    def searchSuggest(self, query):
        suggestapps = self.api.searchSuggest(query)
        logger.info("suggestapps:%s", json.dumps(suggestapps))
        return suggestapps

    @hooks.connected
    def detail(self, pkg_todetail):
        failed_downloads = []
        # case where no filenames have been provided
        for index, pkg in enumerate(pkg_todetail):
            if isinstance(pkg, str):
                pkg_todetail[index] = [pkg, None]
            # remove whitespaces before and after package name
            pkg_todetail[index][0] = pkg_todetail[index][0].strip('\r\n ')

        # BulkDetails requires only one HTTP request
        # Get package info from store
        details = list()
        for pkg in pkg_todetail:
            try:
                detail = self.api.details(pkg[0])
                details.append(detail)
            except RequestError as request_error:
        ...
        if any([d is None for d in details]):
            logger.info("Token has expired while downloading. Retrieving a new one.")
            self.refresh_token()
            details = self.api.bulkDetails([pkg[0] for pkg in pkg_todetail])

        logger.info("detail info json:%s",json.dumps(details))
        for detail, item in zip(details, pkg_todetail):

            folder = os.path.exists(self.download_folder)
            if not folder:
            ...
            try:
                ...
                with open(filepath, "wb") as fbuffer:
                    fbuffer.write(str.encode(json.dumps(detail)))
            except IOError as exc:
                logger.error("Error while writing detail info %s : %s", packagename, exc)
                failed_downloads.append((item, exc))
        return details
    ...
    def connect(self):
        ...
        is installed.
        """
        self.api = GooglePlayAPI(locale=self.locale, timezone=self.timezone, device_codename=self.device_codename, proxies_config=self.proxyDict)
        error = None
        email = None
        password = None
        authsub_token = None
        ...
        with warnings.catch_warnings():
            ...
            except (ValueError, IndexError, LoginError, DecodeError, SystemError, RuntimeWarning):
    ...
    def set_proxyDict(self, proxyStr):
        if proxyStr is not None:
            self.proxyDict = {'http': 'socks5://' + proxyStr,'https': 'socks5://' + proxyStr}
        else:
            self.proxyDict = None
...
def main():
    ...
    parser.add_argument('-sg', '--suggest', action='store', dest='suggest_string', metavar="SEARCH",
                        type=str, help="Suggest the given string in Google Play Store")
    ...
    parser.add_argument('-i', '--info', action='store', dest='packages_to_detail',
                        metavar="AppID", nargs="+", type=str,
                        help="Detail the Apps that map given AppIDs")
    parser.add_argument('-sp', '--sock5 proxy', action='store', dest='proxyStr',
                        type=str, help="sockt proxy")
    parser.add_argument('-na', '--token nation', action='store', dest='nation',
                        type=str, help="token nation")
    parser.add_argument('-home', '--homeapps', action='store', dest='homeapps',
                        help="get homeapps")
    parser.add_argument('-browse', '--browseapps', action='store', dest='browseapps',
                        help="get browseapps")
    parser.add_argument('-list', '--listapps', action='store', dest='listapps',
                        help="get listapps")
    ...
    cli.set_proxyDict(args.proxyStr)
    ...
    if args.packages_to_detail is not None:
        if args.dest_folder is not None:
            cli.set_download_folder(args.dest_folder[0])
        cli.detail(args.packages_to_detail)

    if args.homeapps is not None:
        if args.dest_folder is not None:
            cli.set_download_folder(args.dest_folder[0])
        cli.home()

    if args.suggest_string is not None:
        if args.dest_folder is not None:
            cli.set_download_folder(args.dest_folder[0])
        cli.searchSuggest(args.suggest_string)

    if args.browseapps is not None:
        if args.dest_folder is not None:
            cli.set_download_folder(args.dest_folder[0])
        cli.browse("ART_AND_DESIGN");

    if args.listapps is not None:
        if args.dest_folder is not None:
            cli.set_download_folder(args.dest_folder[0])
        cli.list("ART_AND_DESIGN","apps_topselling_free");
