...
class S3DataHelper():
    ...
    def list_object(self, prefix):
        self.connect()
        response = self.s3read['s3'].meta.client.list_objects_v2(
            Bucket=self.s3read['s3bucket'],
            Prefix=prefix)
        if 'Contents' in response:
            return response['Contents']
        else:
            self.logger.warn('No contents in s3(Bucket=%s, prefix=%s)' %(self.s3read['s3bucket'], prefix))
            return []
    ...
    def get_keys(self, prefix):
        ...
        kwargs = {'Bucket': self.s3read['s3bucket'], 'Prefix': prefix}
        while True:
            resp = self.s3read['s3'].meta.client.list_objects_v2(**kwargs)
    ...
    def get_folder_prefixes(self, prefix):
        self.connect()

        keys = []
        kwargs = {'Bucket': self.s3read['s3bucket'], 'Prefix': prefix, 'Delimiter': '/'}
        ...
        if len(keys)==0:
            self.logger.warn('No folders in s3(Bucket=%s, prefix=%s)' %(self.s3read['s3bucket'], prefix))
    ...
    def upload_folder(self, s3folder, local_dir):
        ...
        for file in files:
            ...
            try: 
                ...
                #     self.s3write['s3bucket'],
                #     s3_path)
                # self.logger.info('put %s to s3 (bucket=%s, key=%s)' % (path, self.s3write['s3bucket'], s3_path))
