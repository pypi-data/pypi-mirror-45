...
class Command:
    ...
    def __execute(self, config):
        ...
        if self._input is not None:
            ...
            config['input'] = reader.encode('utf-8')
...
class CommandResultProcessor:
    ...
    def save_buffer(self, buffer, suffix):
        ...
        path = self.context.paths.save_task_result(self.command.task, fn, content=buffer)
        return path
    ...
    def __save_outputs(self):
        ...
        __save_output('stdout', self.result.stdout.decode('utf-8'))
        __save_output('stderr', self.result.stderr.decode('utf-8'))
