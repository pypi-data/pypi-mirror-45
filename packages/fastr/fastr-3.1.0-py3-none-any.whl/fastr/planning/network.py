...
from ..utils.compare import compare_execution_dir
        ...
        try:
            ...
            if len(validation_result) == 0:
                ...
                for line in validation_result:
                    log.info(line)
                log.info('Differences in execution directories:')
                for difference in compare_execution_dir(
                        os.path.join(temp_results_dir, '__fastr_run_tmp__'),
                        os.path.join(reference_data_dir, '__fastr_run_tmp__')
                        ):
                    log.info(difference)
