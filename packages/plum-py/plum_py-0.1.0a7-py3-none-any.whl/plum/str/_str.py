...
def _decode_str_bytes(memory, decoder, zipped_char_bytes, zero_termination=False):
    reset = True
...
def _add_str_rows_to_dump(dump, zipped_char_bytes, memory=None, string=None):
    nbytes = 0
    nchar = 0
...
def _add_str_memory_to_dump(dump, memory, decoder):
    zipped_char_bytes = []
    try:
        _decode_str_bytes(memory, decoder, zipped_char_bytes)
    finally:
        _add_str_rows_to_dump(dump, zipped_char_bytes, memory=memory)
...
class Str(str, Plum, metaclass=StrType):
    ...
    def __unpack__(cls, memory, offset, limit, dump, parent):
        ...
        if cls._zero_termination:
            ...
            try:
                ...
                if dump:
                    _add_str_rows_to_dump(dump, zipped_char_bytes, memory=chunk)
                raise
            ...
            if dump:
                dump.memory = b''
                _add_str_memory_to_dump(
                    dump, chunk, cls._codecs_info.incrementaldecoder(cls._errors))
    ...
    def __pack__(cls, memory, offset, value, dump):
        ...
        if dump:
            dump.cls = cls
            _add_str_value_to_dump(dump, value, cls._codecs_info.incrementalencoder(cls._errors))
            if cls._zero_termination:
                subdump = dump.add_level('--termination--')
                subdump.memory = b'\x00'

        chunk = value.encode(cls._codecs_info.name, cls._errors)
