...
namespace Fortuna {
    ...
    auto random_below(Integer size) -> Integer {
        if (size > 0) {
            auto const distribution = std::uniform_int_distribution<Integer> { 0, size - 1 };
            return storm(distribution);
        }
        else return analytic_continuation(random_below, size, 0);
    }
    ...
    auto front_geometric(Integer number) -> Integer {
        if (number > 0) {
            ...
            while (result < 0)
                result = plus_or_minus_linear(spread);
            return result;
        }
