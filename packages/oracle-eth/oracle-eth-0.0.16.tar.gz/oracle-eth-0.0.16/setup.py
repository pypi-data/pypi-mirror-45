...
shutil.copyfile(os.path.join(here, 'oracle.conf'), '/etc/oracle.conf')
shutil.copyfile(os.path.join(here, 'oracle_army.conf'), '/etc/oracle_army.conf')
shutil.copyfile(os.path.join(here, 'oracle_chain.conf'), '/etc/oracle_chain.conf')
...
setup(
    ...
    entry_points={'console_scripts': [
        ...
        'uploadchain_service = oracle.main:upload_chain',     # 上链的消费者程序
        'validattion_service = oracle.main:validation_chain', # 单进程方式验证数据是否已经上链
        'validattion_multiproc_service = oracle.main:validation_chain_multi_proc', # 多进程方式验证数据是否已经上链
    ]},
