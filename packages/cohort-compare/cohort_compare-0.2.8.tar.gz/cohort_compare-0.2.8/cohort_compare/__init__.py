...
def get_cohort_similarity_to_counties(cohort_avgs,compare_criteria,incl_criteria,this_dir=os.path.split(__file__)):
...
def map_similarirty(similarity_df,state_list,label_counties,plot_file_name,this_dir=os.path.split(__file__)[0]):
...
def compare_cohort_to_counties(comparison_mappings,cohort_df=pd.DataFrame({'A' : []}),cohort_averages={},incl_criteria=[],state_list=[],label_counties=False,plot_file_name="",return_similarity_scores=False,print_progress=False):
    ...
    similarity_df=get_cohort_similarity_to_counties(cohort_avgs,list(comparison_mappings.values()),incl_criteria,this_dir)
    if print_progress:
        print("Creating similarity map\n")
    map_similarirty(similarity_df,state_list,label_counties,plot_file_name,this_dir)
