...
#存入mongodb数据库
class save_mongo():
    def __init__(self, ip="127.0.0.1",fname='',dname="default",time=True):
        import pymongo
        if fname == '':
            if time: self.cname = 'default'+"_" + eamonn.time(2)
            else:    self.cname = 'default'
        else:
            if time:  self.cname = fname +  "_" + eamonn.time(2)
            else:     self.cname = fname
        self.conn_mgo = pymongo.MongoClient(ip, 27017)
        if time:
            self.db = self.conn_mgo[dname+  "_" + eamonn.time(2)]
        else:
