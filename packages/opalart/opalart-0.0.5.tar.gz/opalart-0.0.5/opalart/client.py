...
class GalleryClient(object):
    ...
    def _filter_posts(self, posts):
        ...
        """
        if self.as_json:
            posts = [ps['file_url'] for ps in posts if ps.get('file_url')]
        else:
            posts = [dict(ps.attrib)['file_url'] for ps in posts if ps.attrib.get('file_url')]
        return posts

    def set_tags_and_limit(self, tags, limit):
    ...
    async def getposts(self, tags, limit=None):
        """|coro|
        ...
        :rtype: list[str]
        """
        self.set_tags_and_limit(tags, limit)
        posts = await self._fetch_posts()
        return posts or None

    async def randpost(self, tags, limit=None):
        """|coro|
