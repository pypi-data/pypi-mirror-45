...
def main():
    if const.COMMAND_LINE_MODE:
        ...
        parser.add_argument("--count", type=int, help="Maximum document to crawl", metavar="c", default=100)
    ...
    for i in range(getConfig('GA', 'ga.max_generation', int)):
        if pop.relevant_count >= args.count:
            print "Document count is satisfied."
            break
        if pop.start is False:
            print "Fitness is convergent at: ", pop.avg_last_fitness
            break
    ...
    print "Predicted Relevant: " + pop.relevant_count
