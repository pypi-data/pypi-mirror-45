...
class Population:
    ...
    def calcFitnessBM25(self):
        ...
        for i, webpage in enumerate(self.individuals):
            webpage.fitness = scores[i] + getConfig('crawler', "webpage_fitness_bias", float)
        
        # normalize weight 0-1
        # max min normalization
        _max = max(self.individuals, key=operator.attrgetter('fitness')).fitness
        _min = min(self.individuals, key=operator.attrgetter('fitness')).fitness
        for webpage in self.individuals:
            webpage.fitness = (webpage.fitness - _min)/(_max-_min)
    ...
    def calcFitness(self):
        ...
        if(self.generation == 1):
            ...
            # tuple index, weight
            tfidf_scores = zip(feature_index, [sum(tfidf_matrix[:, x]).data.item()/corpus.__len__() for x in feature_index])
            ...
            for i, w in tfidf_scores:
                ...
                if (_scores >= getConfig('crawler', 'topic_keyword') and feature_names[i] not in self.query.split()):
                    weight_table.append((feature_names[i], _scores))
        ...
        for i, webpage in enumerate(self.individuals):
            if (webpage.fitness >= getConfig('crawler', "min_fitness_extract", float) and self.generation > 1):
                ...
                best_keyword = feature_names[_best_keyword1]
...
class Webpage:
    ...
    def save_db(self, output_prefix):
        con.insert_page(str(self.url), str(self.title), str(self.text), output_prefix)
