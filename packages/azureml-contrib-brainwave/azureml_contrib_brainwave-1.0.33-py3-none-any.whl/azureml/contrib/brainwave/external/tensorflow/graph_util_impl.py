...
# Based on https://raw.githubusercontent.com/tensorflow/tensorflow/2d3e25245ec4dc2b791212b65b17a7ff4051dfe3/tensorflow/python/framework/graph_util_impl.py
...
def _bfs_for_reachable_nodes(source_nodes, target_nodes, name_to_input_name, name_to_node):
  ...
  while next_to_visit:
    ...
    if n in source_nodes:
      # delete inputs to this node since it is a new input node
      del name_to_node[n].input[:]
      # stop doing bfs for this node
      continue
...
def extract_sub_graph(graph_def, source_nodes, dest_nodes):
  """Extract the subgraph that can reach any of the nodes in 'dest_nodes'.

  Args:
    graph_def: A graph_pb2.GraphDef proto.
    source_nodes: A list of strings specifying the source node names.
  ...
  if isinstance(source_nodes, six.string_types):
  ...
  _assert_nodes_are_present(name_to_node, source_nodes)
  _assert_nodes_are_present(name_to_node, dest_nodes)

  nodes_to_keep = _bfs_for_reachable_nodes(source_nodes, dest_nodes, name_to_input_name, name_to_node)
...
def convert_variables_to_constants(sess,
  ...
  Args:
    ...
    intput_node_names: List of name strings for the input nodes of the graph.
  ...
  inference_graph = extract_sub_graph(input_graph_def, input_node_names, output_node_names)
