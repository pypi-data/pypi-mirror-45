...
def preprocess_array(in_images, order='RGB', scaling_factor=1.0):
    ...
    :return: [?, 244, 244, 3] dim tensor of float32 pixel values of the image, cropped to 244x244, in the order
    specified.
    regularized for input to Resnet.
    """
    _decode = _decode_factory(order, scaling_factor)
    return tf.map_fn(_decode, in_images, tf.float32, name="map_normalize_jpeg")


def _decode_factory(order, scaling_factor):
...
def _preprocess_tensor(tensor, order, scaling_factor):
