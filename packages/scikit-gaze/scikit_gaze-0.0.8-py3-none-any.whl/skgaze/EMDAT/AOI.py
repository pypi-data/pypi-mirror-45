...
class AOI_Stat():
    ...
    def __init__(self,aoi,seg_fixation_data, starttime, endtime, active_aois):
        ...
        fixations = list(map(lambda i: fixation_data[i], fixation_indices))
        ...
        self.features['proportiontime'] = float(totaltimespent)/(float(endtime) - float(starttime))
        if numfixations > 0:
            self.features['longestfixation'] = max(map(lambda x: x.fixationduration,
            fixations))
            self.features['timetofirstfixation'] = fixations[0].timestamp - float(starttime)
            self.features['timetolastfixation'] = fixations[-1].timestamp - float(starttime)
