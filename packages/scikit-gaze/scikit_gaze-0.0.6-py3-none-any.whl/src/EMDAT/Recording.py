...
from src.EMDAT.data_structures import *
from src.EMDAT.Scene import *
...
def read_all_data(all_file):
    ...
    alld = map(Datapoint, all_file)
   
    return list(filter(lambda x: x.is_None!=True,alld))
...
def read_fixation_data(fixation_file, media_offset = (0,0)):
    ...
    return list(map(lambda x: Fixation(x, media_offset=media_offset), 
               fixation_file))
...
def read_aoilines(aoilines):
    ...
    last_aid = ''
    for aoi in aoilines:
        new_aoi = AOI(aoi.type_of_element,[(int(aoi.x),int(aoi.y)),(int(aoi.x)+int(aoi.width),int(aoi.y)),(int(aoi.x)+int(aoi.width),int(aoi.y)+int(aoi.height)),(int(aoi.x),int(aoi.y)+int(aoi.height))])
        aoilist.append(new_aoi)
...
def read_segs(segfile):
    ...
    scenes["problem"] = [("Segment",float(segfile.startTimestamp),float(segfile.lastTimestamp))]
