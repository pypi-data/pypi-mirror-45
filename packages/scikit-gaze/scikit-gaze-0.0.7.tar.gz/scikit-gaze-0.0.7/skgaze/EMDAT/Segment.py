...
import skgaze.EMDAT.geometry as geometry
from skgaze.EMDAT.AOI import *
...
class Segment():
    ...
    def __init__(self, segid, all_data, fixation_data, aois = None, prune_length = None):
        ...
        self.completion_time = int(all_data[-1].timestamp) - int(all_data[0].timestamp)
...
#            d.set_segid(segid)
        ...
        self.features['length'] = int(self.end)- int(self.start)
...
#            f.set_segid(segid)
        ...
        if self.numfixations > 0:
            ...
            self.features['meanfixationduration'] = mean(list(map(lambda x: float(x.fixationduration), fixation_data)))
            self.features['stddevfixationduration'] = stddev(list(map(lambda x: float(x.fixationduration), fixation_data)))
        ...
        while dindex < datalen:
            ...
            if not(d.is_valid):
                ...
                while not(d.is_valid) and (dindex < datalen-1):
                    dindex += 1
                    d = all_data[dindex]
                if int(d.timestamp) - int(gap_start) > max:
                    max = int(d.timestamp) - int(gap_start)
                if int(d.timestamp) - int(gap_start) > params.MAX_SEG_TIMEGAP:
...
#                print "###",d.event, d.data1
    ...
    def get_features(self, featurelist = None, aoifeaturelist = None, aoifeaturelabels = None):
        ...
        featvals = list(map(lambda x: self.features[x], featnames))
