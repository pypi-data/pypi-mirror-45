...
class Transaction:
    ...
    def exec(self, container_object):
        if self._args:
            getattr(container_object, self._method)(*self._args)
        elif self._kwargs:
            getattr(container_object, self._method)(**self._kwargs)
        else:
            getattr(container_object, self._method)()
...
class TxLog:
    ...
    def __init__(self, txlog_dir='./txlog_data', signature_checker=None, circular=True, min_age=None):
    ...
    def get_latest_uncommitted_tx(self):
        next_offset = self._get_committed_offset() + 1
        if self._get_tx_offset() < next_offset:
            return
