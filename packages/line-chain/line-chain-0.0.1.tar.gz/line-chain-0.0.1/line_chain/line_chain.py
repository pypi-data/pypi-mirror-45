...
from .configs import LineChainConfig, Line, Mode
...
class LineChain:
    ...
    def at(self, ratio: float) -> float:
        ...
        r = eval(f'self.mode_{current.mode.value}')(r)
    ...
    def __repr__(self):
        return '\n'.join([
            ...
            *[
                f' {item.ratio * 100: 6.1f}%, {item.mode.value} from {item.start} to {item.target},'
