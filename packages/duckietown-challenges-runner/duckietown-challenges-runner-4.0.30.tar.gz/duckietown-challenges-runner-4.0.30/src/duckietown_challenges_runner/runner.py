...
from duckietown_challenges.challenge_results import read_challenge_results, ChallengeResults, \
    NoResultsFound
from duckietown_challenges.cmd_submit_build import parse_complete_tag, get_complete_tag
from duckietown_challenges.constants import CHALLENGE_SOLUTION_OUTPUT_DIR, CHALLENGE_RESULTS_DIR, \
    ...
    DEFAULT_DTSERVER
from duckietown_challenges.rest_methods import dtserver_work_submission, dtserver_report_job
...
"""
    ...
    args = dict(do_upload=do_upload, do_pull=do_pull, more_features=more_features,
                ...
                tmpdir=tmpdir, token=token, debug_volumes=parsed.debug_volumes,
                impersonate=parsed.impersonate, copy_to_machine_cache=copy_to_machine_cache)
    ndots = 0
    ...
    if parsed.continuous:
        ...
        while True:
            ...
            try:
                ...
                sys.stderr.write('No work for me yet. Server answers in %.1f seconds.\n' % delta)
                ndots += 1
                if ndots == 5:
...
def go_(submission_id, do_pull, more_features, do_upload, delete, reset, evaluator_name,
        machine_id,
        tmpdir,
    ...
    from dt_shell import __version__ as version_dts

    evaluator_version = 'd-c:%s;d-c-r:%s;d-s:%s' % (version_dc, version_dcr, version_dts)
    process_id = evaluator_name

    timeout_server = 60
    res = dtserver_work_submission(token, submission_id, machine_id, process_id, evaluator_version,
                                   features=features, reset=reset, timeout=timeout_server,
    ...
    cr, uploaded = get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull,
                          copy_to_machine_cache=copy_to_machine_cache)

    msg = ('Reporting status %s for job %s submission %s.\n\n%s\n\n%s' %
           (cr.get_status(), job_id, submission_id,
            indent(cr.msg, ' msg |'), indent(json.dumps(cr.get_stats(), indent=4), 'stats |')))
...
def get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull,
           copy_to_machine_cache) -> Tuple[
    ...
    try:
        ...
        # solution_container = '%s/%s:%s@%s' % (organization, repository, tag, digest)
            ...
            solution_container = '%s/%s/%s:%s' % (registry, organization, repository, tag)
        ...
        try:
            ...
            with setup_logging(own_logs_dir):
                ...
                cr = run_single(wd=wd, aws_config=aws_config, steps2artefacts=steps2artefacts,
                                ...
                                project=project, do_pull=do_pull, debug_volumes=debug_volumes,
...
def run_single(*, wd, aws_config,
               steps2artefacts,
               challenge_parameters: EvaluationParameters,
               solution_container: str,
               challenge_name: str,
               challenge_step_name: str, submission_id: int,
               project: str,
               do_pull: bool,
               debug_volumes: Optional[str] = None,
               timeout_sec: float):
    ...
    config = get_config(wd='.', fd='./fifos', challenge_parameters_=challenge_parameters,
                        solution_container=solution_container,
    ...
    if valid_config:
        ...
        cr = run_one(wd, project, do_pull, services=services_names, monitor=tomonitor[0],
...
def write_logs_worker(service_name, container_id):
    ...
    for x in container.logs(stream=True, stdout=True, stderr=True,
                            timestamps=True):
        x = x.decode('utf-8')
        i = x.index(' ')
        timestamp = x[:i]
...
def run_one(wd, project, do_pull, monitor, services, timeout_sec: float):
    ...
    try:
        ...
        while True:
            ...
            for service_name, container_id in service2id.items():
                ...
                if container.status != 'running':
                    ...
                    if exit_code:
                        ...
                        if exit_code == 137:
                            msg += '\n\nError code 137 likely means an out-of-memory error. You likely did not give Docker enough RAM.'
            ...
            delta = time.time() - t0
            msg = ('Running for %d s (timeout: %d s). Services running: %s  exited:  %s' %
...
def get_config(wd, fd, challenge_parameters_,
               solution_container: str,
    ...
    extra_environment = dict(username=USERNAME, uid=UID,
                             USER=USERNAME, HOME=dir_home_guest)
...
def upload_files(wd, aws_config, ignore_patterns=('.DS_Store',), copy_to_machine_cache: bool = True):
