...
def go_(submission_id, do_pull, more_features, do_upload, delete, reset, evaluator_name, machine_id, tmpdir,
        token=None, debug_volumes: Optional[str] = None):
...
def get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull) -> Tuple[ChallengeResults, List]:
    # noinspection PyBroadException
    try:
...
def run_single(*, wd, aws_config,
    ...
    config = get_config(challenge_parameters_=challenge_parameters,
                        solution_container=solution_container,
...
def get_config(challenge_parameters_,
               solution_container: str,
...
def run_docker(cwd: str, project: str, cmd0: List[str], get_output=False) -> bytes:
    ...
    # with StdoutTee(fn1), StderrTee(fn2):
