...
from duckietown_challenges.rest_methods import get_challenge_description, dtserver_get_compatible_challenges
...
def runner_local_main_():
    ...
    compat = dtserver_get_compatible_challenges(token=token, impersonate=parsed.impersonate,
...
'''
    ...
    for i, challenge_step_name in enumerate(steps_ordered):
        ...
        project = 'project%s' % random.randint(1, 100000)
        cr = run_single(wd=wd, aws_config=aws_config, steps2artefacts=steps2artefacts,
                        ...
                        project=project, do_pull=do_pull, debug_volumes=parsed.debug_volumes,
                        submission_id=0,
