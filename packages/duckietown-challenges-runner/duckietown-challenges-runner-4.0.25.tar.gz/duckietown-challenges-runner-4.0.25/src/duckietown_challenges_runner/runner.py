...
def go_(submission_id, do_pull, more_features, do_upload, delete, reset, evaluator_name,
    ...
    if features['processor_free_percent'] < 50:
...
def get_cr(res, tmpdir, do_upload, evaluator_name, job_id, delete, debug_volumes, do_pull) -> Tuple[
        ...
        while True:
            ...
            for service_name, container_id in service2id.items():
                ...
                if container.status != 'running':
                    ...
                    exit_code = exit_code_for_container(container_id)
                    if exit_code:
                        msg = 'The container "%s" exited with code %s.' % (service_name, exit_code)
                        elogger.error(msg)
                        try:
...
def get_config(wd, fd, challenge_parameters_,
    ...
    volumes = [
        f'{wd}:/challenges',
        f'{fd}:/fifos',
        f'{dir_home_guest}:{dir_fake_home_host}'
    ]
