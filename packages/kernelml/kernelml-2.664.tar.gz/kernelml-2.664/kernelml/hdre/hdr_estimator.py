...
#             bins3 = [np.concatenate([[-np.inf],_bins_,[np.inf]]) for _bins_ in bins3]
        ...
        zcore = 2.0
        volume = 10 + self.num_dim+self.num_clusters+combo_len
        volatility = 1
        zscore = 1
        ...
        args = [dim_combos,pdf_combos,bin_combos,fftkernel,mins,maxs,param_to_dim,self.num_dim,self.num_clusters*combo_len,widths]
    ...
    def get_outliers(self,X,pad=1):
        ...
        for i,j in self.dim_combos:
            ...
            count+=1
        return np.all(mask==1,axis=1)
    
    

    def predict(self,X,pad=1,distance='chebyshev'):
        var = self.variance_
        w = self.centroids_
        mask = self.get_outliers(X,pad)
