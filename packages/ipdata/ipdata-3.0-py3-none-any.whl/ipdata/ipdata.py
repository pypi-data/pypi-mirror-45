...
class IPData:
    ...
    valid_fields = {'ip', 'is_eu', 'city', 'region', 'region_code', 'country_name', 'country_code', 'continent_name', 'continent_code', 'latitude', 'longitude', 'asn', 'organisation', 'postal', 'calling_code', 'flag', 'emoji_flag', 'emoji_unicode', 'carrier', 'languages', 'currency', 'time_zone', 'threat', 'count', 'status'}
    def __init__(self, api_key=None):
        if not api_key:
            raise APIKeyNotSet("Missing API Key")
        self.api_key = api_key
        self.headers = {'user-agent': 'ipdata-pypi'}
    def _validate_fields(self, select_field=None, fields=[]):
        if select_field and not select_field in self.valid_fields:
    ...
    def lookup(self, ip=None, select_field=None, fields=[]):
        query = ""
        ...
        if select_field and fields:
            raise IncompatibleParameters("The \"select_field\" and \"fields\" parameters cannot be used at the same time.")
        if select_field:
        ...
        return response
    def bulk_lookup(self, ips=[], fields=[]):
        query_params = {'api-key':self.api_key}
        if len(ips) < 2:
        ...
        if not status_code == 200:
            ...
            response['status'] = status_code
            return response
        response = {'response': response.json(), 'status': status_code}
        return response

# ToDo; Add Bulk Lookup Support
# Add basic tests
