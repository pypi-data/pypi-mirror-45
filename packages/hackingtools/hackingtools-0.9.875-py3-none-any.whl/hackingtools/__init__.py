...
def __createHtmlModalForm__(module_form):
    html = ''
    for m in module_form:
        if module_form[m]['__type__'] and module_form[m]['__id__'] and module_form[m]['__className__']:
            input_type = module_form[m]['__type__']
            input_id = module_form[m]['__id__']
            input_class = module_form[m]['__className__']
            input_placeholder = ''
            if module_form[m]['placeholder']:
                input_placeholder = module_form[m]['placeholder']
            input_value = ''
            if module_form[m]['value']:
                input_placeholder = module_form[m]['value']
            if input_type == 'file':
                html += "<label class=\"btn btn-default\">{placeholder}Browse file to crypt<span class=\"name-file\"></span><input type=\"file\" name=\"{id}filename\" class=\"{className}file-input\" hidden></label>".format(placeholder=input_placeholder, className=input_class, id=input_id)
            else:
                html += "<div class=\"form-group row\"><label for=\"{id}\" class=\"col-4 col-form-label\">{placeholder}Iterate count</label><div class=\"col-4\"><input class=\"{className}form-control\" type=\"{input_type}number\" value=\"{value}1\" name=\"{id}iteratecount\"></div></div>".format(id=input_id, placeholder=input_placeholder, className=input_class, input_type=input_type, value=input_value)
    return html
# Core method
...
def getModulesDjangoForms():
    return [__createHtmlModalForm__(Config.getConfig(parentKey='modules', key=mod, subkey='django_form')) for mod in getModulesNames()]
