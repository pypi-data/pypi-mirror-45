...
def __createHtmlModalForm__(mod):
    ...
    if form_url != '':
        ...
        html += form_url
        html += "' %}\" method=\"post\" enctype=\"multipart/form-data\">{% csrf_token %}"
        html += "<div class=\"modal-body\"><h5>{mod}</h5>".format(mod=mod)
    ...
    for m in module_form:
        if '__type__' in module_form[m] and '__id__' in module_form[m] and '__className__' in module_form[m]:
            ...
            if 'value' in module_form[m]:
                input_placeholder = module_form[m]['value']
            loading_text = ''
            ...
            if input_type == 'file':
                ...
                footer += "<button type=\"button\" class=\"{className}\" data-dismiss=\"modal\">{value}</button>".format(className=input_class, value=input_value)
            elif input_type == 'submit':
                footer += "<input type=\"submit\" class=\"{className}\" value=\"{value}\" id=\"{id}\"></button>".format(className=input_class, value=input_value, id=input_id)
                if loading_text:
                    footer += "<script>$('#{id}').on('click', function(){$('#{id}').attr('value', '{loading_text}');});</script>".format(id=input_id, loading_text=loading_text)
            else:
                html += "<div class=\"form-group row\"><label for=\"{id}\" class=\"col-4 col-form-label\">{placeholder}</label><div class=\"col-4\"><input class=\"{className}\" type=\"{input_type}\" value=\"{value}\" name=\"{id}\"></div></div>".format(id=input_id, placeholder=input_placeholder, className=input_class, input_type=input_type, value=input_value)
    footer += '</div>'
    html += footer
    if form_url != '':
        html += '</form>'
...
def getModulesDjangoForms():
    return [html for html in [__createHtmlModalForm__(mod) for mod in getModulesNames()] if html is not None]
