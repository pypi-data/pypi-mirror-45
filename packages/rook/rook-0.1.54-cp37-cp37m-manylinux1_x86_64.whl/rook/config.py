...
with open(os.path.join(os.path.dirname(__file__), 'version.json')) as f:
    try:
        ...
        VersionConfiguration.VERSION = j.get('version', VersionConfiguration.VERSION)
        VersionConfiguration.COMMIT = j.get('commit', VersionConfiguration.COMMIT)
    except Exception:
        pass
...
class AgentAddress(ConfigurationScheme):
    HOST = 'https://cloud.agent.rookout.com'
    PORT = 443
...
class AgentComConfiguration(ConfigurationScheme):
    ...
    WS_PING_TIMEOUT = 30
    WS_PING_INTERVAL = 10
    WS_RESET_BACKOFF_TIMEOUT = 3*60.0
