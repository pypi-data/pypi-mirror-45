...
from rook.config import AgentAddress, AgentComConfiguration
...
class _Singleton(object):
    ...
    def connect(self, token, host, port, tags=None, labels=None):
        ...
        if host.startswith('ws://') or host.startswith('wss://'):
            ...
            from rook.com.agent_com_ws import AgentCom
            from rook.com.command_handler import CommandHandler
            from rook.com.output_ws import Output

            labels = labels or {}
            tags = tags or []

            self._output = Output()
            self._aug_manager._output = self._output
            self._aug_manager._aug_factory._output = self._output
            self._agent_com = AgentCom(host, port, token, labels, tags)
            ...
            self._agent_com.wait_for_ready(AgentComConfiguration.GRPC_TIMEOUT)
        else:
