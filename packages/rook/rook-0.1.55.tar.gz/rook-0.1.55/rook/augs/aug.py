...
class Aug(object):
    ...
    def send_warning(self, error):
        if self._should_silence_warning(error):
    ...
    def _should_silence_warning(self, error):
        try:
            formatted_error_message = error.message % error.parameters
        except TypeError:
            formatted_error_message = error.message

        if formatted_error_message in self._warningCache or len(self._warningCache) >= 10:
            return True
        self._warningCache[formatted_error_message] = True
