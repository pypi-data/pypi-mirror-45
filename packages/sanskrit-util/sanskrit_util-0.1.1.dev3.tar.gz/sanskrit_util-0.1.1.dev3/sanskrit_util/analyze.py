...
class SimpleAnalyzer(Analyzer):
    ...
    def __init__(self, ctx):
        ...
        # self._form_cache = {}
        # for form in self.session.query(Form):
            # self._form_cache[form.name] = form
            
        # self._stem_cache = {}
        # for stem in self.session.query(Stem):
            # self._stem_cache[stem.name] = stem
    ...
    def _analyze_as_form(self, word):
        ...
        """
        logger.debug("Querying as form: %s", word)
        session = self.session
        results = session.query(Form).filter(Form.name == word).all()
        logger.debug("Found forms: %s", results)
        # results = [self._form_cache[word]] if word in self._form_cache else []
        return results
    ...
    def _analyze_as_stem(self, word):
        ...
        # stems = []
        # for stem in stem_endings_map.keys():
            # if stem in self._stem_cache:
                # stems.append(self._stem_cache[stem])

        logger.debug("Querying for stems: %s", stem_endings_map.keys())
