...
from mitos.mergefeature import featureappend, checklocalfeature, _copy_part
from mitos import mitfi
from mitos import mito
from mitos.mitofile import mitowriter
from mitos import protfeat
from mitos.rna.vienna import RNAplot
from mitos.sequence import sequence_info_fromfile, sequence_info_fromfilehandle
from mitos import trna
from mitos import sequin
...
if __name__ == '__main__':
    ...
    group.add_argument("-c", '--code', dest="code", action="store",
                       required=True, type=int, help="the genetic code")
...
#     group.add_argument( "--maxdeg", action = "store", type = "int", metavar = "NRNT", default = 2, help = "maximum number of missing nt for incomplete stop" )
    ...
    if args.refdir is None:
        args.refdir = os.environ.get('MITOS_REFDIR', None)
    if args.refdir is None:
        parser.error("no reference data directory given")
    if not os.path.isdir(args.refdir):
        parser.error("no such directory %s" % args.refdir)
    ...
    if args.alarab and not os.path.isdir(os.path.join(args.refdir, "featureProtHMM")):
        parser.error(
