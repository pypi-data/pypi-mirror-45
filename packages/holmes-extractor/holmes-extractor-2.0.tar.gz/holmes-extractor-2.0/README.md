...
```
git clone https://repo.xt.msg.team/hudsonr/holmes
```

then issue this operating system command from the `holmes` directory that forms the root
...
(`neuralcoref` and `numpy`) the installation may end with an error message on a
machine where these libraries have not been used in the past. If this occurs, simply re-issue
the command and everything should work on the second try.

Additionally, if you are working on Windows and have not used Python before,
...
![Figure 1](https://repo.xt.msg.team/hudsonr/holmes/src/master/docs/ontology_example.png)
