import random, re, gzip, time, sys, argparse, os, multiprocessing, math, pkg_resources
import numpy as np
...
class Chromosome:
    def __init__(self, chrom_id, chrom_sequence, recombination_rate=3e-8):
        assert type(recombination_rate) == float
        self.id = chrom_id
        self.seq = chrom_sequence
        self.len = len(self.seq)
        self.rec = recombination_rate
    def __str__(self):
...
def load_genome(fasta_f, chrom_selection=None, min_len=1e4):
        ...
        if line[0] == '>':
            if name is not None:
                ...
                if chrom_selection is None or name in chrom_selection:
                    # Only process if larger than the desired sequence length
                    if len(''.join(seq)) >= min_len:
    ...
    # Only process if within the list of selected chromosomes or list is None
    if chrom_selection is None or name in chrom_selection:
        # Only process if larger than the desired sequence length
        if len(''.join(seq)) >= min_len:
            genome_dict[name] = Chromosome(name, ''.join(seq))
...
def print_msp_params(msprime_simulate_args, log_file):
    ...
    Mutation rate : {}
    Number of populations : {}'''.format(msprime_simulate_args['mutation_rate'], len(pop_conf)), file=log_file)
    for i in range(len(pop_conf)):
        print('    Pop {} Ne : {}'.format(i+1, pop_conf[i].initial_size), file=log_file)
        print('    Pop {} sample size : {}'.format(i+1, pop_conf[i].sample_size//2), file=log_file)
    if len(pop_conf) > 1:
        print('    Migration matrix :',  file=log_file)
        mig_mat = msprime_simulate_args['migration_matrix']
        for row in mig_mat:
            print('        ', row,  file=log_file)

#
# Function to do msprime simulations for all chromosomes
def sim_chromosomes(out_dir, genome_dict, msprime_simulate_args, popmap_file, ploidy=2):
    # TODO: move all the directory creation to `simulate()`
    out_dir = out_dir.rstrip('/')
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    elif not os.path.isdir(out_dir):
        sys.exit('oops')
    # loop through all chromosome objects
    for chrom in sorted(genome_dict.keys()):
        ...
        with gzip.open(o_vcf_path, 'wt') as vcf_file:
            simulation.write_vcf(vcf_file, ploidy)
...
class LibraryOptions:
    def __init__(self,
        ...
        # Check inputs
        assert library_type.lower() in ['sdrad', 'ddrad']
        assert insert_mu < 1000
        assert insert_mu > read_len
        assert insert_min < insert_max
        assert insert_min > read_len
        ...
        self.ins_min = insert_min                   # insert size min for ddRAD
        self.ins_max = insert_max                   # insert size max for ddRAD
    ...
    read length : {} bp
    sequencing coverage : {} X'''.format(self.type,
            self.renz_1.name,
...
def def_rad_tag(chrom_sequence, chrom_id, cut_position, direction, library_opts=LibraryOptions()):
    ...
    # Process ddRAD tags
        ...
        minbp = library_opts.ins_min
        maxbp = library_opts.ins_max
        # process depending on direction
        if direction is 'positive':
            # Define boundaries for positive tag
            ...
            # double digest
            dd_cuts = double_digest(seq, enz_2, minbp, maxbp)
            if dd_cuts is None:
                rag_tag = None
            else:
                # create new tag
    ...
    for i, cut in enumerate(raw_cuts):
        ...
        for direction in ['negative', 'positive']:
            ...
            # Process ddRAD loci
                ...
                # Define ddRAD library parameters
                enz_2 = library_opts.renz_2
                min_bp = library_opts.ins_min
                max_bp = library_opts.ins_max
                # If ddcuts founds
                if rad_tag is not None:
                    # Define cut boundaries 
                    cutsite_first = rad_tag.cut
                    cutsite_last  = cutsite_first + len(enz.cutsite) - 1
                    # First check if they are close to the chromosome ends
                    if rad_tag.sta + 1 <= max_bp or rad_tag.end + 1 >= len(chrom_sequence):
                        if tag_dir is 'negative':
                            if rad_tag.sta <= max_bp:
                                # too close to chromosome end
                                status = 'rm_chrom_end'
                        elif tag_dir is 'positive':
                            if rad_tag.end + 1 >= len(chrom_sequence):
...
def create_allele(ref_locus_sequence, locus_variants, allele_i, sample_i, enzyme):
    # Check classes
    assert isinstance(enzyme, RestrictionEnzyme)
...
def extract_rad_alleles(loci_dict, rad_variants_dict, popmap_file, out_dir, library_opts=LibraryOptions(), ploidy = 2):
    # Check variables and classes
    assert type(loci_dict) == dict
        ...
        for chrom in sorted(loci_dict.keys()):
            for loc_id in sorted(loci_dict[chrom].keys()):
                ...
                for allele in range(ploidy):
                    ...
                    allele_seq, allele_cig, discard = create_allele(curr_locus.seq, locus_vars, allele, sam_i, enz)
...
def simulate(genome_dict,
    ...
    # TODO: Handle all paths here

    # Run msprime; generate variants and popmap
    sim_chromosomes(msprime_vcf_dir, genome_dict, msprime_simulate_args, popmap_file)
    if log_file is not None:
        print('\n{} - Simulating chromosomes with msprime...'.format(elapsed()), file=log_file)
        sys.stdout.flush()
    # Merge and process variants
    merge_vcf(genome_dict, msprime_vcf_dir, master_vcf_dir, mutation_opts)
    if log_file is not None:
        print('\n{} - Merging and processing variants...'.format(elapsed()), file=log_file)
        sys.stdout.flush()
    ...
    t = elapsed()
    # Report number of loci found
    if log_file is not None:
        n_loci = 0
        for chrom in sorted(genome_dict.keys()):
            n_loci += len(ref_loci_dict[chrom])
        print('\n{} - Simulating over {} reference RAD loci...'.format(t, n_loci), file=log_file)
    ...
    if log_file is not None:
        print('\n{} - Filtering RAD variants...'.format(elapsed()), file=log_file)
        sys.stdout.flush()
    # Extract RAD alleles
    extract_rad_alleles(ref_loci_dict, rad_variants, popmap_file, rad_alleles_dir, library_opts)
    if log_file is not None:
        print('\n{} - Extracting RAD alleles...'.format(elapsed()), file=log_file)
        sys.stdout.flush()
    # Sequence samples
    sequence_library(rad_reads_dir, popmap_file, rad_alleles_dir, library_opts, mutation_opts, pcr_opts)
    if log_file is not None:
        print('\n{} - Sequencing library...'.format(elapsed()), file=log_file)
        sys.stdout.flush()
