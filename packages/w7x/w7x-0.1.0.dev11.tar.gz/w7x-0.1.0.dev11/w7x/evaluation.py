...
memory = joblib.Memory(cachedir=os.path.join(tempfile.gettempdir(), 'joblib'))
...
class TracerEvaluator(object):
    ...
    def n_crits(self, variant='local'):
        if variant not in self._n_crits:
            if variant == 'local':
                n_crits = []
                for area, q_crit in zip(self.areas_local, self.design_loads_local):
    ...
    def heat_loads_local(self):
        return np.array([load_from_n_hits(
                         ...
                         if area != 0 else 0.
                         for n_hits, area in zip(self.n_hits_local, self.areas_local)
                         ])
...
class TileBasedTracerEvaluator(TracerEvaluator):
    ...
    def mesh_hits(self):
        ...
        if self._mesh_hits is None:
            # with tfields.lib.log.timeit('old'):
            #     mesh_hits = tfields.Tensors(self.mesh.in_faces(self.hits, delta=None))
            # with tfields.lib.log.timeit('new'):
            mesh_hits = tfields.Tensors(self.box_search_tree.in_faces(self.hits, delta=None))
    ...
    def project(self, tensor_field):
        mesh_hits = tfields.Tensors(self.box_search_tree.in_faces(tensor_field, delta=0.001))
        # print(mesh_hits.sum())
        # mesh_hits = tfields.Tensors(self.box_search_tree.in_faces(tensor_field, delta=1))
        print(mesh_hits.sum())
        print(mesh_hits)
