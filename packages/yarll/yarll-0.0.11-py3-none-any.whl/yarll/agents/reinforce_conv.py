...
class REINFORCE(Agent):
    ...
    def __init__(self, env, monitor_path: str, monitor: bool = False, video: bool = True, **usercfg) -> None:
        ...
        self.actions_taken = tf.compat.v1.placeholder(tf.float32, name="actions_taken")  # Discrete action
        self.advantage = tf.compat.v1.placeholder(tf.float32, name="advantage")  # Advantage
        self.build_network()
        self.make_trainer()

        if self.config["save_model"]:
            tf.compat.v1.add_to_collection("action", self.action)
            tf.compat.v1.add_to_collection("states", self.states)
            self.saver = FastSaver()
        summary_loss = tf.compat.v1.summary.scalar("model/loss", self.summary_loss)
        summary_entropy = tf.compat.v1.summary.scalar("model/entropy", self.entropy)
        self.summary_op = tf.compat.v1.summary.merge([summary_loss, summary_entropy])

        self.init_op = tf.compat.v1.global_variables_initializer()
        # Launch the graph.
        self.session = tf.compat.v1.Session()
        self.writer = tf.compat.v1.summary.FileWriter(os.path.join(self.monitor_path, "task0"), self.session.graph)
...
class DiscreteActionsModel(Model):
    def __init__(self):
        super(DiscreteActionsModel, self).__init__()
        self.d1 = Dense(self.config["n_hidden_units"], input=self.env.observation_space.shape, activation="tanh")
        self.d2 = Dense(self.env_runner.nA, activation="softmax")
...
class REINFORCEDiscrete(REINFORCE):
    ...
    def make_trainer(self):
        good_probabilities = tf.reduce_sum(input_tensor=tf.multiply(self.probs,
                                                       tf.one_hot(tf.cast(self.actions_taken, tf.int32),
                                                                  self.env_runner.nA)),
                                           axis=[1])
        eligibility = tf.math.log(good_probabilities) * self.advantage
        loss = -tf.reduce_sum(input_tensor=eligibility)
    ...
    def build_network(self):
        ...
        L1 = tf.contrib.layers.fully_connected(
            ...
            weights_initializer=tf.compat.v1.initializers.random_normal(),
        ...
        self.action = tf.squeeze(tf.random.categorical(logits=tf.math.log(self.probs), num_samples=1), name="action")
...
class REINFORCEDiscreteCNN(REINFORCEDiscrete):
    ...
    def build_network(self):
        ...
        self.L3 = tf.contrib.layers.fully_connected(
            ...
            weights_initializer=tf.compat.v1.initializers.random_normal(stddev=0.01),
            biases_initializer=tf.compat.v1.initializers.zeros())
        ...
        self.probs = tf.contrib.layers.fully_connected(
            ...
            num_outputs=self.env_runner.nA,
            activation_fn=tf.nn.softmax,
            weights_initializer=tf.compat.v1.initializers.truncated_normal(mean=0.0, stddev=0.02),
            biases_initializer=tf.compat.v1.initializers.zeros())
...
class REINFORCEDiscreteRNN(REINFORCEDiscrete):
    ...
    def build_network(self):
        ...
        n_states = tf.shape(input=self.states)[:1]
        states = tf.expand_dims(flatten(self.states), [0])

        enc_cell = tf.compat.v1.nn.rnn_cell.GRUCell(int(self.config["n_hidden_units"]))
        self.rnn_state_in = enc_cell.zero_state(1, tf.float32)
        L1, self.rnn_state_out = tf.compat.v1.nn.dynamic_rnn(cell=enc_cell,
                                                   inputs=states,
...
class REINFORCEDiscreteCNNRNN(REINFORCEDiscreteRNN):
    ...
    def build_network(self):
        ...
        self.states = tf.compat.v1.placeholder(tf.float32, [None] + shape, name="states")
        self.N = tf.compat.v1.placeholder(tf.int32, name="N")
        ...
        self.enc_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(int(self.config["n_hidden_units"]))
        self.rnn_state_in = self.enc_cell.zero_state(1, tf.float32)
        self.L3, self.rnn_state_out = tf.compat.v1.nn.dynamic_rnn(cell=self.enc_cell,
                                                        inputs=reshape,
                                                        initial_state=self.rnn_state_in,
                                                        dtype=tf.float32)
...
class REINFORCEContinuous(REINFORCE):
    ...
    def make_trainer(self):
        loss = tf.reduce_mean(input_tensor=-self.action_log_prob * self.advantage) - self.config["entropy_coef"] * self.entropy
        self.summary_loss = tf.reduce_mean(input_tensor=loss)
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.config["learning_rate"])
        self.train = optimizer.minimize(loss)

    def build_network_normal(self):

        self.actions_taken = tf.compat.v1.placeholder(
            tf.float32,
        ...
        self.mean = tf.debugging.check_numerics(self.mean, "mean")

        self.log_std = tf.compat.v1.get_variable(
            name="logstd",
            shape=list(self.env.action_space.shape),
            initializer=tf.compat.v1.initializers.zeros()
        , use_resource=False)
        std = tf.exp(self.log_std, name="std")
        std = tf.debugging.check_numerics(std, "std")

        self.action = self.mean + std * tf.random.normal(tf.shape(input=self.mean))
        self.action = tf.reshape(self.action, list(self.env.action_space.shape))

        neglogprob = 0.5 * tf.reduce_sum(input_tensor=tf.square((self.actions_taken - self.mean) / std), axis=-1) \
            + 0.5 * np.log(2.0 * np.pi) * tf.cast(tf.shape(input=self.actions_taken)[-1], dtype=tf.float32) \
            + tf.reduce_sum(input_tensor=self.log_std, axis=-1)
        self.action_log_prob = -neglogprob
        self.entropy = tf.reduce_sum(input_tensor=self.log_std + .5 * np.log(2.0 * np.pi * np.e), axis=-1)
    ...
    def build_network_rnn(self):
        ...
        enc_cell = tf.compat.v1.nn.rnn_cell.GRUCell(int(self.config["n_hidden_units"]))
        L1, _ = tf.compat.v1.nn.dynamic_rnn(cell=enc_cell, inputs=states,
