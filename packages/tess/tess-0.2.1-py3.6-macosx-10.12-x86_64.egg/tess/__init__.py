    ...
    def __init__(self, points, limits=1.0, periodic=False, radii=None, blocks=None):
        ...
        if radii is not None:
            ...
            for n,(x,y,z) in enumerate(points):
                placed = self._container.put(n, roundedoff(x,Lx,px), roundedoff(y,Ly,py), roundedoff(z,Lz,pz))
                if not placed:
                    raise ValueError("Could not place particle {} at position ({}, {}, {}).\n\
You may want to check that all points are within the box, and none are overlapping.".format(n, x, y, z))
