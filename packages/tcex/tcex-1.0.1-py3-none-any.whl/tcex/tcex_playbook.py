...
class TcExPlaybook(object):
    ...
    def create(self, key, value):
        ...
        if key is not None:
            key = key.strip()
            self.tcex.log.debug('create variable {}'.format(key))
        ...
        if key is not None:
            ...
            if self.out_variables_type.get(key_type) is not None:
                ...
                if value is not None:
                    ...
                    self.tcex.log.info(
                        'Variable {} has a none value and will not be written.'.format(key)
                    )
                ...
                if value is not None:
                    ...
                    self.tcex.log.info(
                        'Variable {} has a none value and will not be written.'.format(
                            v.get('variable')
                        )
                    )
            else:
    ...
    def db(self):
        ...
        if self._db is None:
            if self.tcex.default_args.tc_playbook_db_type == 'Redis':
                ...
                err = 'Invalid DB Type: ({})'.format(self.tcex.default_args.tc_playbook_db_type)
    ...
    def write_output(self):
        ...
        for data in list(self.output_data.values()):
    ...
    def create_string(self, key, value):
        ...
        if key is not None and value is not None:
            ...
            data = self.db.create(key.strip(), '{}'.format(json.dumps(value)))
        else:
            self.tcex.log.warning('The key or value field was None.')
        return data
