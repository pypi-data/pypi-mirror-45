...
class TcEx(object):
    ...
    def _resources(self, custom_indicators=False):
        ...
        if custom_indicators:
            ...
            # check for bad status code and response that is not JSON
            if not r.ok or 'application/json' not in r.headers.get('content-type', ''):
                warn = 'Custom Indicators are not supported ({}).'.format(r.text)
                self.log.warning(warn)
                return
            response = r.json()
            if response.get('status') != 'Success':
                warn = 'Bad Status: Custom Indicators are not supported ({}).'.format(r.text)
                self.log.warning(warn)
                return
            ...
            try:
                ...
                for entry in data:
                    ...
                    self._indicator_types.append('{}'.format(entry.get('name')))
    ...
    def data_filter(self, data):
        ...
        try:
            ...
            warn = 'Required Module is not installed ({}).'.format(e)
            self.log.warning(warn)
    ...
    def exit(self, code=None, msg=None):
        ...
        self.log.info('Exit Code: {}'.format(code))
        sys.exit(code)
