...
import cortecx.constrcution.cxsession as cxsession
...
class Word:
    ...
    def vector(self):
        try:
            return cxsession.cortecx_sess.word_embeddings[self.word]
        except KeyError:
            return np.zeros(300)
    ...
    def is_stop(self) -> bool:
        try:
            return cxsession.cortecx_sess.stop_words[self.word]
        except KeyError:
            return False
    ...
    def definition(self):
        try:
            return cxsession.cortecx_sess.word_dictionary[self.word]
        except KeyError:
    ...
    def synonyms(self):
        try:
            return cxsession.cortecx_sess.word_thesaurus[self.word]
        except KeyError:
            return KeyError('Word is not supported')
