...
class UserSection(models.Model):
    ...
    def next_section(self, status=None):
...
class UserPart(models.Model):
    ...
    def next_part(self, status=None):
        number = self.part.number
        part_list = UserSection.objects.filter(user_section=self.user_section, part__number__gt=number)
        if not status == None:
