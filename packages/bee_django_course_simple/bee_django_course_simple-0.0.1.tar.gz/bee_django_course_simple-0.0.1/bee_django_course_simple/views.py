...
class SectionDelete(DeleteView):
    model = Section
    success_url = reverse_lazy('bee_django_course_simple:section_list')
...
class QuestionDelete(DeleteView):
    model = Question
    success_url = reverse_lazy('bee_django_course_simple:part_list')
...
class CustomUserSectionDetailRedirect(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        if user_course_list.exists():
            ...
            user_section_list = user_course.usersection_set.all().order_by("status", "section__number")
            if user_course_list.exists():
...
class UserSectionUpdate(TemplateView):
    def post(self, request, *args, **kwargs):
        ...
        msg = ''
        if type == 'open':
            user_section.open_section()
            msg = '开启'
        elif type == 'pass':
            user_section.pass_section()
            next_section = user_section.next_section(status=0)
            if next_section:
                next_section.open_section()
            msg = '通过'
        elif type == 'close':
            user_section.close_section()
            msg = '关闭'
        return JsonResponse(data={
            'rc': 0,
            'message': msg + '成功'
