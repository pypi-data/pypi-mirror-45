...
def add_quyu_gg(quyu,conp,addr,dir,tag='all'):
    user,password,ip,db,schema=conp
    print("1、准备创建分区")
    sql="""
    ...
    if ex_tb in df["tablename"].values:
        ...
        est_cdc_gg(quyu,conp,addr)

    print("3、准备从RDBMS导出csv")
    if tag=='all':
        out_gg_all(quyu,dir,conp)
    else:
        out_gg_cdc(quyu,dir,conp)

    print("4、hawq中执行更新、插入语句")

    update_gg(quyu,conp)

def add_quyu_gghtml(quyu,conp,addr,dir,tag='all'):
    user,password,ip,db,schema=conp
    print("1、准备创建分区")
    sql="""
    SELECT  partitionname
    ...
    if ex_tb in df["tablename"].values:
        ...
        print('外部表%s还不存在'%quyu)
        est_cdc_gg_html(quyu,conp,addr)

    print("3、准备从RDBMS导出csv")
    if tag=='all':
        out_gghtml_all(quyu,dir,conp)
    else:
        out_gghtml_cdc(quyu,dir,conp)
...
def add_quyu(quyu,conp,addr='192.168.4.187:8111',dir='/data/lmf',tag='all'):
    add_quyu_gg(quyu,conp,addr,dir,tag)
    add_quyu_gghtml(quyu,conp,addr,dir,tag)
