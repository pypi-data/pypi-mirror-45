...
from worker.worker_runtime import RUNTIME_GPU_INFO, RUNTIME_SYSTEM_INFO
###############################################################################
...
def get_worker_info(worker_runtime, gpu_uuid):
    ...
    gpu_info = {}
    if gpu_uuid in worker_runtime.runtime[RUNTIME_GPU_INFO]:
        gpu_info = worker_runtime.runtime[RUNTIME_GPU_INFO][gpu_uuid]
    elif gpu_uuid == GPU_FAKE_UUID:
        gpu_info = GPU_FAKE_INFO
    else:
        return None
    gpu_info["sharing"] = worker_runtime.get_gpu_sharing()
...
def register_single_worker(toplevel_workdir,
    ...
    register_is_success, \
    worker_uuid, \
    worker_passcode = Worker.register(master_server, "gpu", secret)
...
def unregister(toplevel_workdir):
