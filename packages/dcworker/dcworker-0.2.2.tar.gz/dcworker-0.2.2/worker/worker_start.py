...
class Worker(object):
    ...
    def _download_dataset(self):
        ...
        self.worker_logger.log_info(
            "download_dataset",
            "Download infor is {}".format(json.dumps(download_info)))
        ...
        if JOB_CONTENT_TYPE_KNOWN_DATASETS in download_info:
            ...
            success = self._download_known_dataset(download_info[JOB_CONTENT_TYPE_KNOWN_DATASETS])

            if not success:
                return False

        if JOB_CONTENT_TYPE_CUSTOM_DATASET in download_info:
            success = self._download_custom_dataset(download_info[JOB_CONTENT_TYPE_CUSTOM_DATASET])
    ...
    def _download_known_dataset(self, known_datasets: List[str]) -> bool:
        ...
        for dataset_name in known_datasets:
            is_existed = self._wait_and_increment_dataset_reference(dataset_name, is_dataset_persisted=1)
            ...
            success = dcdataset.download_dataset_by_name(dataset_name=dataset_name, destination_dir=dataset_local_path)
            if not success:
                self.worker_logger.log_error("_download_known_dataset",
    ...
    def _download_custom_dataset(self, custom_dataset_info: Dict[str, str]) -> bool:
        ...
        if not isdir(dataset_local_path):
            ...
            is_existed = self._wait_and_increment_dataset_reference(dataset_name, is_dataset_persisted=0)
    ...
    def _wait_and_increment_dataset_reference(self, dataset_name: str, is_dataset_persisted: int) -> bool:
        """
