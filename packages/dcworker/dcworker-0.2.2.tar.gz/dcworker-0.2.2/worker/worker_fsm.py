...
class WorkerStagePreRun(WorkerStage):
    def run(self, state_machine):
        try:
            in_container = state_machine.runtime.worker_run_in_container()
            success = state_machine.worker.pre_run(in_container)
        except Exception as e:
...
class WorkerStageRun(WorkerStage):
    def run(self, state_machine):
        try:
            ...
            state_machine.worker.run(in_container, container_config)
        except Exception as e:
