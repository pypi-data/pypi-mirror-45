...
class AuthVisibility(Modifier):
    ...
    def modify(self, request, nodes, namespace, root_id, post_cut, breadcrumb):
        ...
        final = []
        for node in nodes:
            if (node.attr.get('visible_for_authenticated', True) and request.user.is_authenticated) or (node.attr.get('visible_for_anonymous', True) and not request.user.is_authenticated):  # noqa
                final.append(node)
            else:
