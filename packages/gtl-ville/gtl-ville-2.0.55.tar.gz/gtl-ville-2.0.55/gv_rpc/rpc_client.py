...
class RpcClient:
    ...
    async def _get_zones(self, zoneeids=None):
        lr = LocationsRequest()
        if zoneeids is not None:
            lr.eids.eids.extend(zoneeids)
        return await protobuf.decode_locations(await self.interface.get_zones(lr))
