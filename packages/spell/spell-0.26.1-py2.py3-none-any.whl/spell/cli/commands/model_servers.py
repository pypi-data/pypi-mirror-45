...
@click.argument("name")
@click.argument("path")
@click.option("--cluster-id", type=int, hidden=True)
def create(ctx, name, path, cluster_id):
    server_name, tag = get_name_and_tag(name)
    client = ctx.obj["client"]
    with api_client_exception_handler():
        model_server = client.new_model_server(server_name=server_name, tag=tag, path=path, cluster_id=cluster_id)
