...
import MuxTimeCharactCore as PyCharact
...
###############################################################################
    def ButUnselAllClick(self):
        for ck in self.GrChannels.findChildren(QtWidgets.QCheckBox):
            ck.setChecked(False)
        for ck in self.GrColumns.findChildren(QtWidgets.QCheckBox):
            ck.setChecked(False)
    ...
    def GetSelectedDigitals(self, DigGroup):
        Dig = []
        for dc in sorted(DigGroup.findChildren(QtWidgets.QCheckBox)):
            if dc.isChecked():
