...
class Config:
    ...
    def __attrs_post_init__(self):
        ...
        self.bind(']', 'next-block')
        self.bind('[', 'prev-block')
        self.bind('pagedown', 'next-block')
        self.bind('pageup', 'prev-block')

        self.bind('g g', 'jump-begin')
        self.bind('G', 'jump-end')
        self.bind('home', 'jump-begin')
        self.bind('end', 'jump-end')

        self.bind('tab', 'next-agenda')
        self.bind('s-tab', 'prev-agenda')

        self.bind('R', 'refresh-agenda')

        self.bind('a', 'add-task')
        self.bind('m', 'modify-task')
        self.bind('A', 'annotate')
        self.bind('D', 'denotate')
        self.bind('a-space', 'toggle-completed')
        self.bind('delete', 'delete-task')
        self.bind('u', 'undo')
        self.bind('S', 'synchronize')
    ...
    def set(self, key, value):
        '''Changes a setting'''
        setattr(self.settings, key.strip(), value.strip())
...
def _create_default_blocks(cfg):
    cfg.add_block(
        ...
        fmt='{description}<sr left=" "><warning>{priority}</warning></sr><sr left=" "><info>{tags}</info></sr>',  # pylint: disable=line-too-long
    ...
    cfg.add_block(
        ...
        sort='due,scheduled',
        fmt='{description}<sr left=" (sch: " right=")">{scheduled:%Y-%m-%d}</sr><sr left=" (due: " right=")">{due:%Y-%m-%d}</sr>')  # pylint: disable=line-too-long
