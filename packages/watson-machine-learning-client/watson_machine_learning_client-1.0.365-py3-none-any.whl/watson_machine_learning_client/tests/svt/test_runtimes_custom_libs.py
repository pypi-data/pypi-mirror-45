...
class TestRuntimeSpec(unittest.TestCase):
    ...
    def test_2_create_runtime(self):
        lib_meta = {
            self.client.runtimes.LibraryMetaNames.NAME: "libraries_custom",
        ...
        try:
            os.remove('libraries_custom-1.0.zip')
        except:
            pass

        filename = self.client.runtimes.download_library(TestRuntimeSpec.library_uid)

        self.assertTrue(filename == 'libraries_custom-1.0.zip')
        ...
        filename = self.client.repository.download(TestRuntimeSpec.library_uid)
        ...
        meta = {
            ...
            self.client.runtimes.ConfigurationMetaNames.LIBRARIES_DEFINITIONS: [
                LibraryDefinition("my_lib", "1.0", self.custom_library_path, description="t", platform={"name": "python", "versions": ["3.5"]}),
                LibraryDefinition("my_lib", "1.1", self.custom_library_path)
            ],
            self.client.runtimes.ConfigurationMetaNames.LIBRARIES_UIDS: [TestRuntimeSpec.library_uid]
        }
        runtime_details = self.client.runtimes.store(meta)
        TestRuntimeSpec.runtime_uid = self.client.runtimes.get_uid(runtime_details)
        runtime_url = self.client.runtimes.get_url(runtime_details)
