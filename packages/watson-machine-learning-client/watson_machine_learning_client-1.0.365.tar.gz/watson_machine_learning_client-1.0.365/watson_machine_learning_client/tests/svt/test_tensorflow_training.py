...
class TestWMLClientWithTensorflow(unittest.TestCase):
    ...
    def test_05_train_using_interactive_mode_s3(self):
        ...
        training_configuration_dict = {
            ...
            self.client.training.ConfigurationMetaNames.COMPUTE_CONFIGURATION: {"name": "p100"}
        }
    ...
    def test_08_save_trained_model_in_repository(self):
        ...
        saved_model_details = self.client.repository.store_model(TestWMLClientWithTensorflow.trained_model_uid, {self.client.repository.ModelMetaNames.NAME: "My supercool trained TF sample model"})
        ...
        response = requests.get(
            saved_model_details['entity']['latest_version']['url'],
            headers=self.client._get_headers()
        )
        self.assertTrue('tensorflow' == response.json()['entity']['framework']['name'])

    def test_08a_get_model_content(self):
        filename = "test_model.tar.gz"

        self.client.repository.download(TestWMLClientWithTensorflow.model_uid, filename=filename)
