# Adapted from pyzmq logging handlers:
# https://raw.githubusercontent.com/zeromq/pyzmq/master/zmq/log/handlers.py
#
# Copyright 2019 Caleb Hattingh
#
...
class PUSHHandler(logging.Handler):
    ...
    def __init__(self, interface_or_socket, context=None, ZMQ_SNDHWM=None):
        logging.Handler.__init__(self)
        if isinstance(interface_or_socket, zmq.Socket):
            self.socket = interface_or_socket
            self.ctx = self.socket.context
            ...
            if ZMQ_SNDHWM is not None:
                # The default is 1000
                self.socket.setsockopt(zmq.SND_HWM, ZMQ_SNDHWM)
            self.socket.bind(interface_or_socket)
