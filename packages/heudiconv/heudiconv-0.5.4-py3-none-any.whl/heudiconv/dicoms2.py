...
def group_dicoms_into_seqinfos(files, file_filter, dcmfilter, grouping):
    ...
    # usually the case - but interrupted scan may generate different
    # default behavior = throw error
    studyUID = None

    # heuristic contains `file_filter()` function
    if file_filter:
    ...
    for fidx, filename in enumerate(files):
        mw = dw.wrapper_from_file(filename, stop_before_pixels=True, force=True))
        ...
        file_studyUID = getattr(mw.dcm_data, 'StudyInstanceUID')
        if not file_studyUID:
            lgr.info("File {} is missing a StudyInstanceUID".format(filename))

        # Workaround for protocol name in private siemens csa header
        # protocol_name = getattr(mw.dcm_data, 'ProtocolName', '')
        # if not protocol_name.strip():

        protocol_name = getattr(mw.dcm_data, 'ProtocolName', '').strip()
        if not protocol_name:
            protocol_name = parse_private_csa_header(mw.dcm_data,
                                                     'ProtocolName',
                                                     'tProtocolName') or ""
