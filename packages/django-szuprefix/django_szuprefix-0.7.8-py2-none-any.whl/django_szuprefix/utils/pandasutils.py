...
def get_dataframe_from_table(table_name_or_sql, connection="default", coerce_float=True, just_sql=True):
    ...
    sql = sql.replace("%", "%%")
    con = isinstance(connection, (str, unicode)) and connection.startswith("hive://") and connection or db_sqlalchemy_str(connection)
...
def group_by(df, dimensions, measures=None, agg='count', interval=None):
    if isinstance(dimensions, (str, unicode)):
...
class AutoGrowTable(object):
    def __init__(self, connection, table_name, primary_key, insert_timestamp_field=None, update_timestamp_field=None, use_on_duplicate=False):
    ...
    def insert_or_update(self, df):
        ...
        if errors:
            log.error("pandas.AutoGrowTable %s.%s insert_or_update got %d errors: %s", self.connection.alias, self.table_name,
                      len(errors), errors)
        return errors
