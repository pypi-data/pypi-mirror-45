...
def create(name: str, func: callable, **kwargs) -> type:
    ...
    Args:
        ...
        func (``callable``):
            ...
            For example, in a :obj:`MessageHandler <DevLGram.MessageHandler>` the update type will be
            a :obj:`Message <DevLGram.Message>`; in a :obj:`CallbackQueryHandler <DevLGram.CallbackQueryHandler>` the
            update type will be a :obj:`CallbackQuery <DevLGram.CallbackQuery>`. Your function body can then access the
...
class Filters:
    """This class provides access to all library-defined Filters available in DevLGram.

    The Filters listed here are intended to be used with the :obj:`MessageHandler <DevLGram.MessageHandler>` only.
    ...
    """Filter messages that contain :obj:`Audio <DevLGram.Audio>` objects."""

    document = create("Document", lambda _, m: bool(m.document))
    """Filter messages that contain :obj:`Document <DevLGram.Document>` objects."""

    photo = create("Photo", lambda _, m: bool(m.photo))
    """Filter messages that contain :obj:`Photo <DevLGram.PhotoSize>` objects."""

    sticker = create("Sticker", lambda _, m: bool(m.sticker))
    """Filter messages that contain :obj:`Sticker <DevLGram.Sticker>` objects."""

    animation = create("Animation", lambda _, m: bool(m.animation))
    """Filter messages that contain :obj:`Animation <DevLGram.Animation>` objects."""

    game = create("Game", lambda _, m: bool(m.game))
    """Filter messages that contain :obj:`Game <DevLGram.Game>` objects."""

    video = create("Video", lambda _, m: bool(m.video))
    """Filter messages that contain :obj:`Video <DevLGram.Video>` objects."""
    ...
    """Filter messages that contain :obj:`Voice <DevLGram.Voice>` note objects."""

    video_note = create("VideoNote", lambda _, m: bool(m.video_note))
    """Filter messages that contain :obj:`VideoNote <DevLGram.VideoNote>` objects."""

    contact = create("Contact", lambda _, m: bool(m.contact))
    """Filter messages that contain :obj:`Contact <DevLGram.Contact>` objects."""

    location = create("Location", lambda _, m: bool(m.location))
    """Filter messages that contain :obj:`Location <DevLGram.Location>` objects."""

    venue = create("Venue", lambda _, m: bool(m.venue))
    """Filter messages that contain :obj:`Venue <DevLGram.Venue>` objects."""

    web_page = create("WebPage", lambda _, m: m.web_page)
    """Filter messages sent with a webpage preview."""

    poll = create("Poll", lambda _, m: m.poll)
    """Filter messages that contain :obj:`Poll <DevLGram.Poll>` objects."""
    ...
    def command(
        ...
        Args:
            commands (``str`` | ``list``):
                ...
                field of the :class:`Message <DevLGram.Message>`.
    ...
    def regex(pattern, flags: int = 0):
        ...
        Args:
            pattern (``str``):
                ...
                are stored in the *matches* field of the :class:`Message <DevLGram.Message>` itself.
