...
class ForwardMessages(BaseClient):
    async def forward_messages(
        ...
        Returns:
            ...
            :obj:`Messages <DevLGram.Message>` even if a list contains just one element, otherwise if
            *message_ids* was an integer, the single forwarded :obj:`Message <DevLGram.Message>`
            is returned.
        ...
        if as_copy:
            ...
            for chunk in [message_ids[i:i + 200] for i in range(0, len(message_ids), 200)]:
                messages = await self.get_messages(chat_id=from_chat_id, message_ids=chunk)  # type: DevLGram.Messages
