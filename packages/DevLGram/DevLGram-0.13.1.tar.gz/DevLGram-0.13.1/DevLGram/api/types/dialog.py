...
class Dialog(Object):
    ...
    Args:
        ...
        draft (optional): Either :obj:`DraftMessageEmpty <DevLGram.api.types.DraftMessageEmpty>` or :obj:`DraftMessage <DevLGram.api.types.DraftMessage>`
    """

    __slots__ = ["peer", "top_message", "read_inbox_max_id", "read_outbox_max_id", "unread_count", "unread_mentions_count", "notify_settings", "pinned", "unread_mark", "pts", "draft"]

    ID = 0xe4def5db
    QUALNAME = "Dialog"

    def __init__(self, *, peer, top_message: int, read_inbox_max_id: int, read_outbox_max_id: int, unread_count: int, unread_mentions_count: int, notify_settings, pinned: bool = None, unread_mark: bool = None, pts: int = None, draft=None):
    ...
    def read(b: BytesIO, *args) -> "Dialog":
        ...
        return Dialog(peer=peer, top_message=top_message, read_inbox_max_id=read_inbox_max_id, read_outbox_max_id=read_outbox_max_id, unread_count=unread_count, unread_mentions_count=unread_mentions_count, notify_settings=notify_settings, pinned=pinned, unread_mark=unread_mark, pts=pts, draft=draft)
