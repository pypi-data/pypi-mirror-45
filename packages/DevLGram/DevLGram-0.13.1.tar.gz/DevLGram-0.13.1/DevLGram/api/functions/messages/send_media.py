...
class SendMedia(Object):
    ...
    __slots__ = ["peer", "media", "message", "random_id", "silent", "background", "clear_draft", "reply_to_msg_id", "reply_markup", "entities"]

    ID = 0xb8d1262b
    QUALNAME = "messages.SendMedia"

    def __init__(self, *, peer, media, message: str, random_id: int, silent: bool = None, background: bool = None, clear_draft: bool = None, reply_to_msg_id: int = None, reply_markup=None, entities: list = None):
    ...
    def read(b: BytesIO, *args) -> "SendMedia":
        ...
        return SendMedia(peer=peer, media=media, message=message, random_id=random_id, silent=silent, background=background, clear_draft=clear_draft, reply_to_msg_id=reply_to_msg_id, reply_markup=reply_markup, entities=entities)
