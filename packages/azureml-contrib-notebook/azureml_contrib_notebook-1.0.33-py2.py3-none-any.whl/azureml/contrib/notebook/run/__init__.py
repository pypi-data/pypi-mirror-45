...
def _set_created_from_content(self, type, path):
    ...
    if type == 'Notebook':
        ...
        if artifact_name in artifact.artifacts:
            ...
            self._internal_run_dto = self._client.patch_run(create_run_dto)
            return artifact
        else:
...
def _get_created_from_content(self):
    ...
    if (self._internal_run_dto.created_from is not None and
        self._internal_run_dto.created_from.type == "Notebook" and
        self._internal_run_dto.created_from.location_type == "ArtifactId" and
            self._internal_run_dto.created_from.location is not None):
            _, uri = self._client.artifacts.get_file_by_artifact_id(
                artifact_id=self._internal_run_dto.created_from.location)
            return uri
    else:
...
def _experiment_submit_notebook_decrator(original_submit):
...
def _experiment_start_logging_decrator(original_star_logging):
...
Experiment.submit = _experiment_submit_notebook_decrator(Experiment.submit)
Experiment.start_logging = _experiment_start_logging_decrator(Experiment.start_logging)
