...
class Visualize(object):
    ...
    def __init__(self, feature_vector=None, model=None, label=None):
        """
                model: model result or knowledpack
        ...
        if self.label is None:
            print(
                "Label column is not specified for this knowledpack, specify as label=XXX")
    ...
    def label(self, value):
        if value is None and self.knowledgepack:
            self._label = self.knowledgepack.query_summary.get(
                'label_column', None)
        else:
            self._label = value
    ...
    def feature_vector(self, value):
        self._feature_vector = value[[
            i for i in sorted(value.columns.tolist()) if ('gen_' == i[:4])]+[self.label]]

    def plot_features(self, features_list=None,
                      plot_style='boxplot', figsize=(20, 5),
                      features_per_row=4):
        """
        ...
        for index in range(num_plots):
            df_list = [reframe(self.feature_vector, feature, self.label) for feature in
                       features_list[index*features_per_row:(index+1)*features_per_row]]

            if plot_style == 'violinplot':
                violinplot(x="Features", y="Sensor Values", hue=self.label,
                           data=concat(df_list), palette="muted", ax=axarr[index])
            elif plot_style == 'boxplot':
                boxplot(x="Features", y="Sensor Values", hue=self.label,
                        data=concat(df_list), palette="muted", ax=axarr[index])
            else:
            ...
            axarr[index].set_xticklabels(
                features_list[index *
                              features_per_row:(index+1)*features_per_row],
                fontsize=16)
        ...
        axarr[0].legend(loc='center left',
                        bbox_to_anchor=(1, 0.85), fontsize=16)
        axarr[0].set_title("Feature Vector Distribution", fontsize=16)
        plt.show()

    def neuron_feature_map(self,
                           feature1,
                           features_list,
                           label=None,
                           norm=None,
                           ...
                           vector_alpha=0.3,
                           neuron_alpha=0.2,
                           color_list=['m', 'k', 'b', 'lime', 'r', 'c', 'y', 'brown', 'tan', 'g', 'pink', 'gray']):
        """
            ...
            Args:
                ...
                norm(int): takes from knowledpack, to overide 0=L1, 1=LSUP
        ...
        for feature in features_list:
            self.columns_index[0] = self.feature_vector.columns.tolist().index(
                feature)

            if self.vector_labels == ['all']:
                self.vector_labels = np.unique(
                    self.feature_vector[self.label]).tolist()
            ...
            if neuron_figure and self.knowledgepack:
                if norm is None:
                    ...
                    norm = norm
                self._plot_neurons(
                    self.knowledgepack.neuron_array, feature, norm)
    ...
    def _plot_neurons(self, neurons, versus_feature, norm):
        ...
        for neuron in neurons:
            ...
            if neuron['Category'] < 30000:  # if it is not degenerated neuron
                cat_label = self.knowledgepack.class_map[str(
                    neuron['Category'])]
                clr = self.color_list[self.label_indx_list.index(cat_label)]
                if norm == 0:
                    dim = len(neuron['Vector'])
                    new_aif = aif * np.sqrt(2)/dim
                    self.neuron_scatter.add_patch(patches.Rectangle((x_val-(aif*2/dim), y_val),
                                                                    2*new_aif,
                                                                    2*new_aif,
                                                                    color=clr,
                    ...
                    self.neuron_scatter.add_patch(patches.Rectangle((x_val-aif, y_val-aif),
                                                                    2*aif,
                                                                    2*aif,
                                                                    color=clr,
                                                                    alpha=self.neuron_alpha,
    ...
    def _plot_hist(self, Xs, Ys, clr):
        ...
        self.axHisty.hist(Ys, bins=bins, color=clr,
                          alpha=self.vector_alpha, orientation='horizontal')
    ...
    def _plot_feature_vector(self, versus_feature):
        for label_indx in self.vector_labels:
            temp_label_data = self.feature_vector[self.feature_vector[self.label] == label_indx].index.tolist(
            )
            temp = self.feature_vector.loc[temp_label_data[0]].tolist()
            cat_label = temp[-1]
            clr = self.color_list[self.label_indx_list.index(cat_label)]
            Xs = self.feature_vector.loc[temp_label_data,
                                         versus_feature].tolist()
            Ys = self.feature_vector.loc[temp_label_data,
                                         self.pivot_feature].tolist()
    ...
    def _set_initial_figure(self):
        ...
        if self.knowledgepack:
            self.label_indx_list = np.unique([self.knowledgepack.class_map[str(i)] for i in
                                              np.sort(list(map(int, list(self.knowledgepack.class_map.keys()))))] +
                                             np.unique(self.feature_vector[self.label]).tolist()).tolist()
        else:
            self.label_indx_list = list(
                np.unique(self.feature_vector[self.label]))
    ...
    def _set_final_figure_settings(self, versus_feature):
        ...
        self.neuron_scatter.set_xticklabels(
            range(0, 256, 10), rotation=0, fontsize=8)

        self.neuron_scatter.set_yticks(range(0, 256, 10))
        self.neuron_scatter.set_yticklabels(
            range(0, 256, 10), rotation=0, fontsize=8)

        if self.hist_figure:
            self.axHistx.set_xticks(range(0, 256, 10))
            self.axHistx.yaxis.set_tick_params(labelsize=8)
            self.axHistx.set_title(
                self.pivot_feature + ' vs ' + versus_feature)
