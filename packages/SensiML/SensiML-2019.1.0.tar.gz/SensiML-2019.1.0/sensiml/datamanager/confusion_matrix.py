...
PREC = "precision"
F1 = "f1_score"
HAS_KEYS = {"f1_score": True, "accuracy": False, "sensitivity": True, "specificity": False,
            "positive_predictive_rate": True, "precision": True}
EXPECTED_KEYS = ["ConfusionMatrix", "f1_score", "accuracy", "sensitivity",
                 "positive_predictive_rate", "precision", "specificity"]
NEEDED_PROPS = ["confusion_matrix", "y_true", "y_pred", "f1_score", "accuracy",
                "sensitivity", "positive_predictive_rate", "precision", "specificity"]
...
class ConfusionMatrix(object):
    ...
    def __init__(self, metrics_dict):
        ...
        if not isinstance(metrics_dict, dict):
            raise ConfusionMatrixException(("Expected a dictionary, instead got %s" % str(type(metrics_dict))))
        keys_missing = []
        for key in EXPECTED_KEYS:
            if key not in metrics_dict.keys():
                keys_missing.append(key)
        if len(keys_missing) >= 1:
            raise ConfusionMatrixException(("Not all keys present in metrics dict, missing keys %s" % str(keys_missing)))
        ...
        self._y_true = metrics_dict["y_true"]
        self._y_pred = metrics_dict["y_pred"]
    ...
    def __str__(self):
        ...
        for i, r in cm.iterrows():
            row = [decimal_cell_format.format(a) for a in r] + [decimal_cell_format.format(r.sum() - r['UNC']),
                                                                decimal_cell_format.format(
                                                                    100 * cm.ix[i, i] / float(r.sum() - r['UNC']))]
        ...
        pct_row_format = "{:>" + cell_width_str + "}" + ("{:>" + cell_width_str + "." + decimal_points_str + "f}") * (
        len(cm)) + \
                         (
                         "{:>" + cell_width_str + "}") * 3 + "{:>" + cell_width_str + "." + decimal_points_str + "f}" + "\n"
        pp_row = 100 * np.array(cm).diagonal().astype(float) / cm.sum()[:-2].astype(float)
        pp_row = pp_row.tolist()



        pp_row += ['', '', 'Acc(%)',
                   100. * sum(np.array(cm).diagonal()) / (sum(cm.sum()) - sum(cm['UNC'])) ]
    ...
    def se_by_class_name(self, class_name):
        ...
        return self._sensitivity[class_name] if class_name in self._class_titles else NaN
    ...
    def pp_by_class_name(self, class_name):
        ...
        return self._positive_predictive_rate[class_name] if class_name in self._class_titles else NaN
    ...
    def _build_data_frame(self):
        ...
        self._class_titles = [x for x in self._cm.keys() if x not in STATIC_COLUMNS]
        self._class_titles.sort()
        row_index = 0
        df_cols = ["ClassIn"] + self._class_titles + ["UNK", "UNC", "Support", "Sens(%)"]
...
class ConfusionMatrixList(ConfusionMatrix):
        ...
        if isinstance(confusion_matrices, dict):
            for key, value in confusion_matrices.items():
                if not isinstance(value, ConfusionMatrix):
                    raise ConfusionMatrixException(("Expected a ConfusionMatrix, instead got %s" % str(type(value))))
        else:
            raise ConfusionMatrixException(("Expected a dictionary, instead got %s" % str(type(confusion_matrices))))
    ...
    def sort_by_metric(self, metric):
        ...
        if metric not in EXPECTED_KEYS[1:len(EXPECTED_KEYS)]:
            raise ConfusionMatrixException(("Metric %s is not a supported metric for sorting, use one of the following %s" \
    ...
    def _average_child_metrics(self):
        ...
        for key in ret.keys():
            if key == NEEDED_PROPS[1]:
                ...
                ret[key] = y_pred
            elif key != NEEDED_PROPS[0]:  # create a list for each statistical measure, and append each CM's value
    ...
    def _inspect_confusion_matrix(self):
        ...
        class_titles = [x for x in to_inspect.confusion_matrix.keys() if x not in STATIC_COLUMNS]
        ...
        for i, conf in self._confusion_matrices.items():
            if is_first:
                ...
                if not has_keys:
                    ...
                    if self._get_truth(getattr(conf, prop)[AVG], opr, getattr(ret[1], prop)[AVG]):
                        ret = (i, conf)
        return ret
