...
class GetBuffer(Proto):
    def __init__(self, buffer):
        ...
        self.values(
            self.cmd_num,
            66,
            binascii.unhexlify(buffer.hash),
            buffer.id_onblock
        )
...
class SendBuffer(Proto):
    def __init__(self, _buffer):
        ...
        buffer_str = str(len(_buffer)) + 's'


        self.structure = struct.Struct('=%s %s' % (F_HEADER, buffer_str))
        self.create_buffer()

        buff = binascii.hexlify(_buffer)

        self.values = (
            self.cmd_num,
            len(_buffer),
            binascii.unhexlify(buff)
        )

        self.pack()

class SendTransactionArray(Proto):
    def __init__(self, tr_buffer):
        self.cmd_num = CMD_NUMS['CommitTransactionArray']
        self.structure = struct.Struct('=%s %s' % (F_HEADER, F_TRANSACTION*len(tr_buffer)))
        self.create_buffer()
        self.pack(tr_buffer)

    def pack(self, tr_buffer):
        self.values = ()
        self.values += (
            self.cmd_num,
            188 * len(tr_buffer)
