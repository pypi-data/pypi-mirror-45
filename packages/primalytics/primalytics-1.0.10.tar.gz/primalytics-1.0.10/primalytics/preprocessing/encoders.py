	...
	def transform(self, data, array_indices=None):
		...
		if is_df or is_series:
			...
			for variable in original_columns:
				if variable in self.categoricals:
					n_new_columns = len(list(self.hashing_dict[variable].values())[0])
					hashing_columns = [variable + '_H' + str(i) for i in range(n_new_columns)]
					new_columns += hashing_columns

					if self.verbose:
						print('Transforming', variable)
					if is_series:
						transformed = transformed.append(pandas.Series(self.hashing_dict[variable][data[variable]], index=hashing_columns))
					else:
						transformed = transformed.join(pandas.DataFrame(list(data[variable].apply(lambda x: self.hashing_dict[variable][x])), index=data.index, columns=hashing_columns))
				else:
					new_columns.append(variable)

			transformed = transformed[new_columns]
			transformed = transformed[new_columns]
