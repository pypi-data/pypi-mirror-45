		...
		if is_df or is_series:
			...
			for variable in original_columns:
				if variable in self._categoricals:
					...
					if is_series:
						transformed = transformed.append(pandas.Series(self._hashing_dict[variable][data[variable]], index=hashing_columns))
					else:
						transformed = transformed.join(pandas.DataFrame(list(data[variable].apply(lambda x: self._hashing_dict[variable][x])), index=data.index, columns=hashing_columns))
				else:
