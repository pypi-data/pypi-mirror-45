...
def create_permissions_for_config(config: ComposeProjectConfig):
    ...
    service_permissions = ["view", "up", "stop", "start", "remove", "restart", "scale", "update", "rollback"]
    container_permissions = ["stop", "start", "remove", "restart", "rename"]
...
def validate_and_resolve_config() -> [str, str]:
    try:
...
def execute_compose_command(project_name: str, args: List[str], debug: bool = True) -> str:
    try:
        arguments = ["docker-compose", "--log-level", "ERROR"]
