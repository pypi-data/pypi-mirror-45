...
@DatasetReader.register("iberlef_ner_input_reader")
class IberLEFNERDatasetReader(DatasetReader):
    """
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        with open(file_path, "r", encoding="utf-8") as data_file:
            ...
            for is_divider, lines in itertools.groupby(data_file, _is_divider):
                ...
                if not is_divider:
                    # fields = [line.strip() for line in lines if not line.startswith('#')]
                    # unzipping trick returns tuples, but our Fields need lists
                    # tokens_ = [list(field) for field in zip(*fields)]
                    tokens_ = [line.strip() for line in lines]
