...
class MetaArray(object):
    ...
    def _readData1(self, fd, meta, mmap=False):
        ...
        for ax in meta['info']:
            if 'values_len' in ax:
                ax['values'] = np.fromstring(fd.read_df(ax['values_len']), dtype=ax['values_type'])
        ...
        if mmap:
            ...
            subarr = np.fromstring(fd.read_df(), dtype=meta['type'])
    ...
    def _readData2(self, fd, meta, mmap=False, subset=None):
        ...
        if dynAxis is None:
            ...
            if meta['type'] == 'object':
                ...
                subarr = pickle.loads(fd.read_df())
            else:
                if mmap:
                    subarr = np.memmap(fd, dtype=meta['type'], mode='r', shape=meta['shape'])
                else:
        ...
        ## One axis is dynamic, read in a frame at a time
            ...
            while True:
                ...
                if meta['type'] == 'object':
                    data = pickle.loads(fd.read_df(inf['len']))
                else:
                    data = np.fromstring(fd.read_df(inf['len']), dtype=meta['type'])
    ...
    def _readHDF5Remote(self, fileName):
        ...
        if proc == None:
            ...
            MetaArray._h5py_metaarray = proc._import('validol.pyqtgraph.metaarray')
