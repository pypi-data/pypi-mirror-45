...
class ContrailFirewallRule(contrail.ContrailResource):
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if prop_diff.get(self.MATCH_TAG_TYPES) is not None:
            obj_1 = vnc_api.FirewallRuleMatchTagsTypeIdList()
            if prop_diff.get(self.MATCH_TAG_TYPES, {}).get(self.MATCH_TAG_TYPES_TAG_TYPE) is not None:
                for index_1 in range(len(prop_diff.get(self.MATCH_TAG_TYPES, {}).get(self.MATCH_TAG_TYPES_TAG_TYPE))):
                    obj_1.add_tag_type(prop_diff.get(self.MATCH_TAG_TYPES, {}).get(self.MATCH_TAG_TYPES_TAG_TYPE)[index_1])
            obj_0.set_match_tag_types(obj_1)
