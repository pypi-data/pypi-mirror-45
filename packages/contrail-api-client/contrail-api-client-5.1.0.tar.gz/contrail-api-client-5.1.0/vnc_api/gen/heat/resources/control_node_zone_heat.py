...
class ContrailControlNodeZone(contrail.ContrailResource):
    PROPERTIES = (
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.ControlNodeZone(name=self.properties[self.NAME],
        ...
        try:
            obj_uuid = super(ContrailControlNodeZone, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().control_node_zone_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        try:
            self.vnc_lib().control_node_zone_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().control_node_zone_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('control_node_zone %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().control_node_zone_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::ControlNodeZone': ContrailControlNodeZone,
    }
