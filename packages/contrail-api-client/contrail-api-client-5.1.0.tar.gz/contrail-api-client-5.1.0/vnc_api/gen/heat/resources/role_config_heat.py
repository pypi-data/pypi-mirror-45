...
class ContrailRoleConfig(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        ROLE_CONFIG_CONFIG: properties.Schema(
            properties.Schema.STRING,
            _('ROLE_CONFIG_CONFIG.'),
            update_allowed=True,
            required=False,
        ),
        ...
        NODE_PROFILE: properties.Schema(
            properties.Schema.STRING,
            _('NODE_PROFILE.'),
            update_allowed=True,
            required=False,
    ...
    attributes_schema = {
        ...
        ROLE_CONFIG_CONFIG: attributes.Schema(
            _('ROLE_CONFIG_CONFIG.'),
        ),
        ...
        NODE_PROFILE: attributes.Schema(
            _('NODE_PROFILE.'),
        ),
    }
    ...
    def handle_create(self):
        parent_obj = None
        if parent_obj is None and self.properties.get(self.NODE_PROFILE) and self.properties.get(self.NODE_PROFILE) != 'config-root':
            try:
                parent_obj = self.vnc_lib().node_profile_read(fq_name_str=self.properties.get(self.NODE_PROFILE))
            except vnc_api.NoIdError:
                parent_obj = self.vnc_lib().node_profile_read(id=str(uuid.UUID(self.properties.get(self.NODE_PROFILE))))
            except:
                parent_obj = None

        if parent_obj is None and self.properties.get(self.NODE_PROFILE) != 'config-root':
            raise Exception('Error: parent is not specified in template!')

        obj_0 = vnc_api.RoleConfig(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.ROLE_CONFIG_CONFIG) is not None:
            obj_0.set_role_config_config(self.properties.get(self.ROLE_CONFIG_CONFIG))
        ...
        try:
            obj_uuid = super(ContrailRoleConfig, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().role_config_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.ROLE_CONFIG_CONFIG) is not None:
            obj_0.set_role_config_config(prop_diff.get(self.ROLE_CONFIG_CONFIG))
        ...
        try:
            self.vnc_lib().role_config_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().role_config_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('role_config %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().role_config_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::RoleConfig': ContrailRoleConfig,
    }
