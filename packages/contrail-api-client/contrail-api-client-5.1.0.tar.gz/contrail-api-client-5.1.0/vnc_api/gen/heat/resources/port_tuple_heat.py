...
class ContrailPortTuple(contrail.ContrailResource):
    ...
    def handle_create(self):
        parent_obj = None
        if parent_obj is None and self.properties.get(self.SERVICE_INSTANCE) and self.properties.get(self.SERVICE_INSTANCE) != 'config-root':
            try:
                parent_obj = self.vnc_lib().service_instance_read(fq_name_str=self.properties.get(self.SERVICE_INSTANCE))
            except vnc_api.NoIdError:
                parent_obj = self.vnc_lib().service_instance_read(id=str(uuid.UUID(self.properties.get(self.SERVICE_INSTANCE))))
            except:
                parent_obj = None

        if parent_obj is None and self.properties.get(self.SERVICE_INSTANCE) != 'config-root':
