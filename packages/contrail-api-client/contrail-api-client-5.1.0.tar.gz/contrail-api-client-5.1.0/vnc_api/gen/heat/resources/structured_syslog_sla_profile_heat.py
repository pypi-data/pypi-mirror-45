...
class ContrailStructuredSyslogSlaProfile(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        STRUCTURED_SYSLOG_SLA_PARAMS: properties.Schema(
            properties.Schema.STRING,
            _('STRUCTURED_SYSLOG_SLA_PARAMS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        STRUCTURED_SYSLOG_SLA_PARAMS: attributes.Schema(
            _('STRUCTURED_SYSLOG_SLA_PARAMS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.StructuredSyslogSlaProfile(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.STRUCTURED_SYSLOG_SLA_PARAMS) is not None:
            obj_0.set_structured_syslog_sla_params(self.properties.get(self.STRUCTURED_SYSLOG_SLA_PARAMS))
        ...
        try:
            obj_uuid = super(ContrailStructuredSyslogSlaProfile, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().structured_syslog_sla_profile_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.STRUCTURED_SYSLOG_SLA_PARAMS) is not None:
            obj_0.set_structured_syslog_sla_params(prop_diff.get(self.STRUCTURED_SYSLOG_SLA_PARAMS))
        ...
        try:
            self.vnc_lib().structured_syslog_sla_profile_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().structured_syslog_sla_profile_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('structured_syslog_sla_profile %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().structured_syslog_sla_profile_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::StructuredSyslogSlaProfile': ContrailStructuredSyslogSlaProfile,
    }
