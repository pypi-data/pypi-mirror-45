...
class ContrailConfigDatabaseNode(contrail.ContrailResource):
    PROPERTIES = (
        NAME, FQ_NAME, DISPLAY_NAME, CONFIG_DATABASE_NODE_IP_ADDRESS, PERMS2, PERMS2_OWNER, PERMS2_OWNER_ACCESS, PERMS2_GLOBAL_ACCESS, PERMS2_SHARE, PERMS2_SHARE_TENANT, PERMS2_SHARE_TENANT_ACCESS, ANNOTATIONS, ANNOTATIONS_KEY_VALUE_PAIR, ANNOTATIONS_KEY_VALUE_PAIR_KEY, ANNOTATIONS_KEY_VALUE_PAIR_VALUE, TAG_REFS, GLOBAL_SYSTEM_CONFIG
    ) = (
        'name', 'fq_name', 'display_name', 'config_database_node_ip_address', 'perms2', 'perms2_owner', 'perms2_owner_access', 'perms2_global_access', 'perms2_share', 'perms2_share_tenant', 'perms2_share_tenant_access', 'annotations', 'annotations_key_value_pair', 'annotations_key_value_pair_key', 'annotations_key_value_pair_value', 'tag_refs', 'global_system_config'
    )
    ...
    properties_schema = {
        ...
        CONFIG_DATABASE_NODE_IP_ADDRESS: properties.Schema(
            properties.Schema.STRING,
            _('CONFIG_DATABASE_NODE_IP_ADDRESS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        CONFIG_DATABASE_NODE_IP_ADDRESS: attributes.Schema(
            _('CONFIG_DATABASE_NODE_IP_ADDRESS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.ConfigDatabaseNode(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.DISPLAY_NAME) is not None:
            obj_0.set_display_name(self.properties.get(self.DISPLAY_NAME))
        if self.properties.get(self.CONFIG_DATABASE_NODE_IP_ADDRESS) is not None:
            obj_0.set_config_database_node_ip_address(self.properties.get(self.CONFIG_DATABASE_NODE_IP_ADDRESS))
        ...
        try:
            obj_uuid = super(ContrailConfigDatabaseNode, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().config_database_node_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        if prop_diff.get(self.CONFIG_DATABASE_NODE_IP_ADDRESS) is not None:
            obj_0.set_config_database_node_ip_address(prop_diff.get(self.CONFIG_DATABASE_NODE_IP_ADDRESS))
        ...
        try:
            self.vnc_lib().config_database_node_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().config_database_node_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('config_database_node %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().config_database_node_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::ConfigDatabaseNode': ContrailConfigDatabaseNode,
    }
