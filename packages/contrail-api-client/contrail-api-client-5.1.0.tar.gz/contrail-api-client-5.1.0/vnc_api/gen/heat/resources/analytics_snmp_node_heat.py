...
class ContrailAnalyticsSnmpNode(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        ANALYTICS_SNMP_NODE_IP_ADDRESS: properties.Schema(
            properties.Schema.STRING,
            _('ANALYTICS_SNMP_NODE_IP_ADDRESS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        ANALYTICS_SNMP_NODE_IP_ADDRESS: attributes.Schema(
            _('ANALYTICS_SNMP_NODE_IP_ADDRESS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.AnalyticsSnmpNode(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.ANALYTICS_SNMP_NODE_IP_ADDRESS) is not None:
            obj_0.set_analytics_snmp_node_ip_address(self.properties.get(self.ANALYTICS_SNMP_NODE_IP_ADDRESS))
        ...
        try:
            obj_uuid = super(ContrailAnalyticsSnmpNode, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().analytics_snmp_node_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.ANALYTICS_SNMP_NODE_IP_ADDRESS) is not None:
            obj_0.set_analytics_snmp_node_ip_address(prop_diff.get(self.ANALYTICS_SNMP_NODE_IP_ADDRESS))
        ...
        try:
            self.vnc_lib().analytics_snmp_node_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().analytics_snmp_node_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('analytics_snmp_node %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().analytics_snmp_node_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::AnalyticsSnmpNode': ContrailAnalyticsSnmpNode,
    }
