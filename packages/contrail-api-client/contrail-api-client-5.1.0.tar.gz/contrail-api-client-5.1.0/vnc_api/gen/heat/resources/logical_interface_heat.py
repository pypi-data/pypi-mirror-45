    ...
    def handle_create(self):
        ...
        if parent_obj is None and self.properties.get(self.PHYSICAL_ROUTER) and self.properties.get(self.PHYSICAL_ROUTER) != 'config-root':
            try:
                ...
                parent_obj = None
        if parent_obj is None and self.properties.get(self.PHYSICAL_INTERFACE) and self.properties.get(self.PHYSICAL_INTERFACE) != 'config-root':
            try:
                parent_obj = self.vnc_lib().physical_interface_read(fq_name_str=self.properties.get(self.PHYSICAL_INTERFACE))
            except vnc_api.NoIdError:
                parent_obj = self.vnc_lib().physical_interface_read(id=str(uuid.UUID(self.properties.get(self.PHYSICAL_INTERFACE))))
            except:
                parent_obj = None

        if parent_obj is None and self.properties.get(self.PHYSICAL_INTERFACE) != 'config-root':
