...
from suanpan import asyncio, g, interfaces, objects, runtime
...
class Handler(Component):
    ...
    def initCurrent(self, message):
        self.current = objects.Context.froms(  # pylint: disable=attribute-defined-outside-init
            message=message
        )
        return self.current
    ...
    def context(self, message):
        yield objects.Context.froms(message=message)
...
class Stream(interfaces.HasBaseServices, interfaces.HasLogger, interfaces.HasDevMode):
    ...
    def initCurrent(self, args):  # pylint: disable=unused-argument
        ...
        return objects.Context.froms(message=message, handler=handler)
