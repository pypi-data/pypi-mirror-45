...
class StepMania:
    def __init__(self, file_name):
        try:
            read.is_smfile(file_name)
        except ValueError as e:
            raise ValueError(str(e))
        except OSError as e:
            raise OSError(str(e))
        if not read.is_smfile(file_name):
    ...
    def SetSamplelength(self, length):
        ...
        self._file.seek(0)
        list1 = self._file.readlines()
        list1[15] = '#SAMPLELENGTH:' + str(length) + ';'
        self._file.truncate()
        self._file.writelines(list1)
...
def AGBPM(musicfile_str):
    """
    Automatic generation of BPM values
    """
    yy, sr = librosa.load(musicfile_str)
