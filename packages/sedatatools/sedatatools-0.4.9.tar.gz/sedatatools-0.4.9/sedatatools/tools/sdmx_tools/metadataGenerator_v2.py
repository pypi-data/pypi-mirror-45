...
def getTableMetadataFromDb(server, dbname, user, password, projectYear):
    ...
    conn = pymssql.connect(host = server, database = dbname, user = 'sewebsite3', password = 'XXXXXXX')
    cursor = conn.cursor()
    ...
    for ext in extension:
        for el in tableNames: # TODO this will create problem if there's no extension in file name, it will brea!!!
            if ext in el[0]:
...
def getVariableDescriptionsFromFile(variableDescriptionLocation):
    """
    Open file and read variable names and its description. Files shoul be in format: variable_id, description
...
def getGeotype(geoLevelInfo): # number of geotypes
    ...
    # # create first level to hold all other nesting TODO check if it is really necessary
...
def verifyConfig(configPath):
    ...
    """
    #TODO do this properly later on
    errors = []
    warnings = []
    if not os.path.isfile(configPath):
        print("Config file doesn't exist on selected location!")
        return False
