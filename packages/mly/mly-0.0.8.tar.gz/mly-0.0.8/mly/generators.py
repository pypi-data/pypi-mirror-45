...
def load_inj(dataset,name,detector):    
    inj=[]
    f_inj = open(null_path+'/'+dataset+'/'+detector+'/'+name,'r') 
...
def load_noise(fs,date_file,detector,name,ind='all'): 
    noise=[]
    with open(null_path+'/'+str(fs)+'/'+date_file+'/'+detector+'/'
              +name,'r') as f:
...
# This function creates a list of indeses as begining for every instantiation of # noise generated. This makes the lag method happen.
...
################################################################################
#                                                                              #
# parameters       (list) A list with three elemntets: [ source_cbcs, noise_ty #
...
def data_generator_cbc(parameters        
    ...
    # match filter one. There was a mis-match which I coulndnt resolve how to
        ...
        for i in range(0,size):
            ...
            if 'H' in detectors:
                ...
                if length==inj_len:                                       
                    ...
                    disp = np.random.randint(
                        -int(fs*(length-inj_len)/2),int(fs*(length-inj_len)/2))
                
                # Due to offset the injection file will be different
                if disp>=0: 
                    injH=injH[disp:]                              
                if disp<0: 
                    injH=injH[0:-disp]  
            ...
            if 'L' in detectors:
                ...
                # Zero-padding the injection 
                injL=np.hstack((np.zeros(int(fs*(t-inj_len)/2)), injL
                                ,np.zeros(int(fs*(t-inj_len)/2+disp))))  
                # Making the noise a TimeSeries
                ...
                # we multiply by two and we get rid of the DC value 
                # and everything above fs/2.
                injL_fft_0N=2*np.abs(injL_fft_0[1:int(t*fs/2)+1]) 
            ...
            if 'V' in detectors:
                ...
                # Zero-padding the injection 
                injV=np.hstack((np.zeros(int(fs*(t-inj_len)/2)), injV
                                ,np.zeros(int(fs*(t-inj_len)/2+disp))))  
                # Making the noise a TimeSeries
                ...
                # we multiply by two and we get rid of the DC value 
                # and everything above fs/2.
                injV_fft_0N=2*np.abs(injV_fft_0[1:int(t*fs/2)+1]) 
        ...
        for i in range(0,size):
            ...
            if 'H' in detectors:
                ...
                injH=np.hstack((np.zeros(int(fs*(t-inj_len)/2))
                                ,injH ,np.zeros(int(fs*(t-inj_len)/2+disp))))   
                # Making the noise a TimeSeries
                H_back=TimeSeries(XH,sample_rate=fs)
                # Calculating the ASD so tha we can use it for whitening later
                asdH=H_back.asd(1,0.5)                                    
                # Calculating the one sided fft of the template, we multiply by
                # two and we get rid of the DC value and everything above fs/2.
                injH_fft_0=np.fft.fft(injH)                       
                injH_fft_0N=2*np.abs(injH_fft_0[1:int(t*fs/2)+1]) 
            ...
            if 'L' in detectors:
                ...
                injL=np.hstack((np.zeros(int(fs*(t-inj_len)/2))
                                ,injL ,np.zeros(int(fs*(t-inj_len)/2+disp))))   
                # Making the noise a TimeSeries
                L_back=TimeSeries(XL,sample_rate=fs)
                # Calculating the ASD so tha we can use it for whitening later
                asdL=L_back.asd(1,0.5)                                    
            ...
            if 'V' in detectors:
                ...
                injV=np.hstack((np.zeros(int(fs*(t-inj_len)/2))
                                ,injV ,np.zeros(int(fs*(t-inj_len)/2+disp))))   
                # Making the noise a TimeSeries
                V_back=TimeSeries(XV,sample_rate=fs)
                # Calculating the ASD so tha we can use it for whitening later
                asdV=V_back.asd(1,0.5)                                    
...
def auto_gen(set_type
    ...
    if set_type[0]=='cbc':
        ...
        if set_type[3] in dirlist( null_path+'/injections/cbcs'):
        ...
        for i in range(len(d['segment'])):
            ...
            with open(path+dir_name+'/'+'gen_'+d['name'][i]+'_'
                ...
                if set_type[0]=='cbc':
                    ...
                    comand=(
                    'data_generator_cbc(parameters=[\''
                     +injection_source+'\',\''+noise_type+'\','
                     +str(d['set'][i])+']'+
                               ',length='+str(length)+           
                               ',fs='+str(fs)+              
                               ',size='+str(d['size'][i])+             
