...
from nevolution_risk.v5.logic.continent import Continent
from nevolution_risk.v5.logic.continent_loader import ContinentLoader
from nevolution_risk.v5.logic.graphlogic import GraphLoader, RiskGraph
from nevolution_risk.v5.logic.node import Node
from nevolution_risk.v5.logic.player import Player
...
class Graph(object):
    ...
    def __init__(self, seed=None, player_positions=None, player_count=4):
        ...
        self.card_fac = Card_factory(self.seed)

        self.players = []
        self.players.append(Player('player_one', 0, green, action_len, self.card_fac))
        self.players.append(Player("player_two", 1, deepskyblue, action_len, self.card_fac))
        self.players.append(Player("player_three", 2, darkviolet, action_len, self.card_fac))
        self.players.append(Player("player_four", 3, orange, action_len, self.card_fac))
    ...
    def fight_simulation(self, attack_size, def_troop):
        """
        Simulate an attack with its troops loss. The variables n,m determines the number of dices and are
        depending of attack_size and def_troop.
        ...
        attacker_dice = 0
        for random in range(0, n):
            attacker_dice += self.dice_throw()

        defender_dice = 0
        for random in range(0, m):
            defender_dice += self.dice_throw()
        ...
        print(attack_casualties, def_casualties)
