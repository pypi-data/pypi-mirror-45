...
class StarDist(object):
    ...
    def train(self, X, Y, validation_data, epochs=None, steps_per_epoch=None):
        ...
        data_val = StarDistData(X_val, Y_val, same_patches=False, **data_kwargs)
        n_data_val = len(data_val)
        n_val = n_data_val
        n_take = int(np.ceil(n_val / self.config.train_batch_size))
        ids = tuple(np.random.choice(n_data_val, size=n_take, replace=(n_take > n_data_val)))
        Xv, Mv, Pv, Dv = [None]*len(ids), [None]*len(ids), [None]*len(ids), [None]*len(ids)
        ...
        for cb in self.callbacks:
            if isinstance(cb,CARETensorBoard):
                cb.output_slices = ([slice(None)],[slice(None), slice(None), slice(None), slice(0,3)])
