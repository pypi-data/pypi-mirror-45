...
def roi_bytes(x,y,pos=None):
    ...
    n_coords = len(x)
    N = 64 + n_coords*2*2 #  *nbytes*(x,y)
    H = [0]*N
    H[ 0: 4] = map(ord,'Iout') # magic start
    H[ 4: 6] = _int16(227) # version
    H[ 6: 8] = _int16(0) # roi type (0 = polygon)
    H[ 8:10] = _int16(top) # bbox top
    H[10:12] = _int16(left) # bbox left
    H[12:14] = _int16(bottom) # bbox bottom
    H[14:16] = _int16(right) # bbox right
    H[16:18] = _uint16(n_coords) # number of coordinates
    if pos is not None:
        H[56:60] = _int(pos) # position (C,Z,T)

    for i,(_x,_y) in enumerate(zip(x,y)):
        xs, ys = 64+(2*i), 64+(n_coords*2)+(2*i)
        H[xs:xs+2] = _int16(_x - left)
        H[ys:ys+2] = _int16(_y - top)
