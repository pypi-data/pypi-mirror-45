...
class Table(_TreelabObject):
    ...
    def get_row(self, row_id: str):
        ...
        return Row(row_id=row_id, table=self, table_data=self.dataframe)
    ...
    def get_rows(self, row_ids: List[str]):
        """
        Get rows by row_ids
        :param row_ids:
        :return:
        ...
        return Rows(row_ids=row_ids, table_data=self.dataframe)
    ...
    def get_column_by_id(self, col_id: str):
        ...
        return Column(col_id=col_id, table=self, table_data=self.dataframe)
    ...
    def get_columns_by_name(self, field_name: str):
        ...
        :return:
        """
        col_id = [k for k, v in self.columns_info.items() if v.get('name') == field_name]
        return self.get_column_by_id(col_id[0])
    ...
    def get_columns(self, col_ids: List[str]):
        """
        Get columns by col_ids
        :param col_ids:
        :return:
        """

        return Columns(col_ids=col_ids, table_data=self.dataframe)
    ...
    def init_data(self):
        ...
        content = [[cell['value']['text'] for cell in row['cells']] for row in raw_data['tableData']['rows']]

        self.dataframe = pd.DataFrame(data=content, index=indexs, columns=colunms_id)
...
class Row(_TreelabObject):
    ...
    def __init__(self, row_id: str = None, table: Table = None, table_data: pd.DataFrame = None):
        super().__init__()
    ...
    @property
    def data(self):
        return self.table_data.loc[self.row_id]

    def _get_event_id(self, event: EventPayload):
        return event.rowId


class Rows(_TreelabObject):
    __repr_fields__ = {'_id'}

    def __init__(self, row_ids: List[str] = None, table_data: pd.DataFrame = None):
        super().__init__()
    ...
    @property
    def data(self):
        return self.table_data.loc[self.row_ids]
...
class Column(_TreelabObject):
    ...
    def __init__(self, col_id: str = None, field_name: str = '', order: int = 0, foreign_table_id: str = '',
                 table: Table = None, field_type: FieldType = FieldType.TEXT, table_data: pd.DataFrame = None):
        super().__init__()
    ...
    @property
    def data(self):
        return self.table_data[self.col_id]

    def _get_event_id(self, event: EventPayload):
        return event.columnId


class Columns(_TreelabObject):
    __repr_fields__ = {'_id'}

    def __init__(self, col_ids: List[str] = None, table_data: pd.DataFrame = None):
        super().__init__()
    ...
    @property
    def data(self):
        return self.table_data[self.col_ids]
