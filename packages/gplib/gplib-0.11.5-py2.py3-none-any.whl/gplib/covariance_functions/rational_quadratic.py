...
class RationalQuadratic(StationaryFunction):
    ...
    """
    def __init__(self, ov2=1, alpha=2.0, ls=None, ard_dims=None):
        hyperparams = [
            OptimizableParameter(
                'output_variance', LogParameterTransformation,
                default_value=ov2,
                min_value=np.exp(-20), max_value=np.exp(20)
            ),
            OptimizableParameter(
                'alpha', LogParameterTransformation,
                default_value=alpha,
                min_value=np.exp(-15), max_value=np.exp(5)
            )
        ]

        super(RationalQuadratic, self).__init__(hyperparams, ls, ard_dims)
