...
class MultiStart(FittingMethod):
    ...
    def fit(self, model, folds, verbose=False):
        ...
        def measurement_wrapper(mod_params):
            ...
            # Evaluate border if out of bounds
            lower_b, upper_b = zip(*model.get_param_bounds(trans=True))
            dist_b = \
                np.clip(
                    mod_params - np.array(upper_b),
                    a_min=-1e-5,
                    a_max=None
                ) + \
                np.clip(
                    mod_params - np.array(lower_b),
                    a_min=None,
                    a_max=1e-5
                )
            mod_params = mod_params - dist_b
            ...
            value += 1e3 * np.max(np.abs(dist_b))
