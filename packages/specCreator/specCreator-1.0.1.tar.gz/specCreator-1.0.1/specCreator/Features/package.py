...
class Package(object):
    ...
    def packageFramework(self, debug):
        ...
        ResultModel.instance().resultDict["dependency"] = self.processDependency(contentDict)
    ...
    def startPackage(self, subSpecName, debug):
        ...
        if not haveSourceFile:
            self.formatter.format_warning("没有源码, 不能打 " + debug)
            return "没有源码，打包失败"
        # 打包之前需要先更新repo源。
        ...
        if returnCode > 0 or "** BUILD FAILED **" in content:
            self.result.returnError("打包失败，请搜索尖括号内 <error generated> 查看具体信息 ")
        ...
        if haveSourceFile:
            ...
            self.shell.excommand_until_done("cp -R " + binaryFilePath + " " + productPath)
            # 创建头文件软链
