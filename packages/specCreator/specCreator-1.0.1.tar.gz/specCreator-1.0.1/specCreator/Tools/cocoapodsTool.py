...
class CocoapodsTool(object):
    ...
    def updateRepoList(self, repoList):
        for repoUrl in repoList:
            if not self.arguments.updateMasterSource and repoUrl == masterRepo:
                continue
    ...
    def pushRepo(self, podName, version, repoUrl):
        ...
        repoPath = self.repoPathByURL(repoDict, repoUrl)
        if not repoPath:
            self.addRepo(repoUrl)
            repoDict = self.repoDict()
            repoPath = self.repoPathByURL(repoDict, repoUrl)
        if not repoPath:
            self.result.returnError("找不到repoPath")
        repoName = self.repoNameFromURL(repoUrl)
    ...
    def repoPathByURL(self, repoDict, repoURL):
    ...
    def cleanRepoEnv(self, repoList=[]):
        ...
        for repoName in repoList:
            if not self.arguments.updateMasterSource and repoName == "master":
                continue
            self.cleanRepo(repoName)
    ...
    def create_tmp_podspec(self, subSpecName):
        ...
        repoSource = self.arguments.repoSourceList[0]
