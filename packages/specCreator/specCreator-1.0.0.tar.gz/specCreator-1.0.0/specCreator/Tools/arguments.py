...
class Arguments(object):
    ...
    def _initArguments(self):
        ...
        self.moduleSources = ""
        # 是否更新master源，因为更新比较慢，默认不更新。
        self.updateMasterSource = False
        # 依赖的json串，
        self.dependencyJSON = ""
        ...
        # 打包前是否检查
        self.check = False
        # 如果是subspec打包，则指定拿几个subspec打包，不指定则全部打
        self.subspecs = ""
        # 要不要打framework静态库
        ...
        try:
            opts, args = getopt.getopt(sys.argv[2:], "",
                                       ["help", "user=", "projectPath=", "subPackage=", "podName=", "branch=",
                                        ...
                                        "LFSURL=","moduleSources=", "updateMasterSource", "useSSH"])
        ...
        for cmd, arg in opts:
            ...
            if cmd == "--debug":
                ...
                if self.moduleSources:
                    self.moduleSources += "," + masterRepo
                    self.moduleSourceList = self.moduleSources.strip(" ").split(",")
                else:
                    self.moduleSources = masterRepo
                    self.moduleSourceList = [masterRepo]
            elif cmd == "--updateMasterSource":
                self.updateMasterSource = boolValue(arg)
            elif cmd == "--useSSH":
    ...
    def argumentDict(self):
        return {
            ...
            "branch":self.branch,"version": self.version,"commitId":self.commitId,
            "subPackage":self.subPackage,"moduleSources":self.moduleSources,"updateMasterSource":self.updateMasterSource,
            "dependencyJSON": self.dependencyJSON,"useSSH":self.useSSH,"check":self.check,
            "subspecs": self.subspecs, "framework":self.framework, "debugPackage":self.debugPackage,
