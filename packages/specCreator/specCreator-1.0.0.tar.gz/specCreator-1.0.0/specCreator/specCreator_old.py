...
def create_tmp_podspec(user, podName, subSpecName, version, commitId, dependencyJSON):
    ...
    repoSource = "http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs" #TODO 这里整改
...
def clean_tmp_podspec(projectPath, sourceList):
    userPath = os.path.expanduser('~')
    for source in sourceList:
        if not isURL(source):
            formatPrint("提供的moduleSources里含有不合法的URL：" + source, " *")
            returnError()
        repoName = repoNameFromURL(source)
        repoPath = userPath + "/.cocoapods/repos/" + repoName
        if not fileExist(repoPath):
            excommandUntilDone("pod repo add " + repoName + " " + source)
            excommandUntilDone("pod repo update " + repoName)
        os.chdir(repoPath)
        excommandUntilDone("git add .")
        excommandUntilDone("git reset --hard")
        os.chdir(projectPath)
        excommandUntilDone("pod repo update " + source)
...
def autoPackage(user, projectPath, podName, branch, version, subPackage=False, commitId="", dependencyJSON="",
                moduleSources="", updateMasterSource=False, framework=True, archive=False, check=True, debugPackage=False,
                releasePackage=True, autoPush=False, autoPushRepo=False,repoSources="", resultPath="",
                whiteListPath="", subspecs="", CAFPath="", LFSURL=""):
    if not projectPath or not podName or not version:
        formatPrint("缺少必备的参数，请检查", "- ")
        returnError()
    resultDict = {}
    formatPrint("正在清理现场","- ")
    moduleSourceList = moduleSources.strip(" ").split(",")
    cleanEnv(projectPath, moduleSourceList)
    ...
    if autoPushRepo:
        if not repoSource:
            formatPrint("自动发布请指定发布源参数：--repoSources","* ")
            returnError()
        formatPrint("正在自动发布","- ")
        # TODO 发布多个地方。
...
def getarguments(type=""):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[2:], "facdrp",
                                   ["help", "gerritUser=", "projectPath=", "subPackage=", "podName=", "branch=",
                                    ...
                                    "repoSources=", "resultPath=", "whiteListPath=", "subspecs=", "CAFPath=", "LFSURL=",
                                    "moduleSources=", "updateMasterSource"])
    ...
    for cmd, arg in opts:
        ...
        if cmd in ("--projectPath"):
            ...
            moduleSources = str(arg)
        elif cmd == "--updateMasterSource":
            updateMasterSource = boolValue(arg)
        elif cmd == "--framework":
    ...
    for arg in args:
        print arg
    if type == "autoPackage":
        return user, projectPath, podName, branch, version, subPackage, commitId, dependencyJSON, moduleSources, \
               updateMasterSource, framework, archive, check, debugPackage, releasePackage, autoPush, autoPushRepo,\
               repoSources, resultPath, whiteListPath, subspecs, CAFPath, LFSURL
    elif type == "init":
...
def checkopts():
    ...
    if arg == "env":
        ...
        user, projectPath, podName, branch, version, subPackage, commitId, dependencyJSON, moduleSources, \
        updateMasterSource, framework, archive, check, debugPackage, releasePackage, autoPush, autoPushRepo,\
        repoSource, resultPath, whiteListPath, subspecs, CAFPath, LFSURL = getarguments(arg)
        os.chdir(projectPath)
        autoPackage(user, projectPath, podName, branch, vesrsion, subPackage, commitId, dependencyJSON, moduleSources,
                    updateMasterSource, framework, archive, check,debugPackage, releasePackage, autoPush, autoPushRepo,
                    repoSources, resultPath, whiteListPath, subspecs, CAFPath, LFSURL)
    elif arg == "init":
...
def cleanEnv(projectPath, moduleSourceList=[]):
    # type: () -> object
    # 清除本工程的所有改动
    excommandUntilDone("git add . ; git reset --hard")
    # 清除预发布的repo源的改动。
    clean_tmp_podspec(projectPath, moduleSourceList)
