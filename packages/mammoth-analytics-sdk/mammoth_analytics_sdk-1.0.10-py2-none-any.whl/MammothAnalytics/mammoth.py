...
class MammothConnector(object):
    ...
    def wait_till_file_processing_get_ds(self, file_id):
        ...
        self._wait_for_ds_status(final_ds_id, 'processing', check_for_equality=False)
        if append_to_ds_id:
            self._wait_for_ds_status(append_to_ds_id, 'ready')
        ds = self.get_dataset(final_ds_id)
        log.error("ds details: {0}".format(ds))
        return ds

    @handleError
    def _wait_for_ds_status(self, ds_id, status, check_for_equality=True):
