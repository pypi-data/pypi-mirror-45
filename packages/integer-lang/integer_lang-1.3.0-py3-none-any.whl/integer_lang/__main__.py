from integer_lang.standalone import Lark_StandAlone as Parser
from integer_lang.basenum import _str_to_base_num as toInt
import sys
import re
...
class Interpreter:
	...
	def function_definition (self, node):
		...
		if (fname in reservedWords):
			raise NameError("Cannot overwrite built-in function " + fname)
	...
	def function_call (self, node):
		...
		fname = str(node.children[0]).upper()
		if (fname == "EVAL"):
			#because this can apparently be an issue
			return self.eval_call(node)
		elif (fname in self.function_stack):
...
def main():
	...
	try:
		...
		print('Error: {0}'.format(err).split('\n')[0])
		sys.exit()
