...
def _class_averaged_score(score_func,
    ...
    if is_class_averaging and \
        ...
        if len(dropped_classes) > 0:
            dropped_msg_fmt = "For {}, the following classes were ignored since they are not in the test set: {}"
            dropped_msg = dropped_msg_fmt.format(metric_name, dropped_classes)
...
def compute_mean_cv_scores(scores, metrics, logger=None):
...
def compute_metrics_forecasting(y_pred: np.ndarray,
    ...
    results = {}
    for name in metrics:
        if name == constants.Metric.ForecastResiduals:
            try:
                metric_class = Metric.get_metric_class(name)
...
class ForecastMetric(RegressionMetric):
    ...
    def __init__(self, y_pred, y_true, horizons, logger=None):
        """Initialize the forecast metric class.

        :param y_pred: The predicted labels for computing the metric.
        :param y_true: The true labels for computing the metric.
        :param horizons: The horizons associated to each y.
        """
        self.horizons = horizons
        super(ForecastMetric, self).__init__(y_pred, y_true, logger=logger)
    ...
    * The confusion values (TP, FP, TN, FN) are computed with the
    ...
    def _binarize_y_test(self, class_labels):
        ...
        if y_test_bin.shape[1] == 1:
            y_test_bin = np.concatenate((y_test_bin, 1 - y_test_bin), axis=1)
        return y_test_bin
...
class ResidualsMetric(RegressionMetric, HasVariance):
    ...
    def aggregate(scores):
        ...
        agg_stddev = ResidualsMetric._total_variance(res_counts, means, variances)
        data_agg = {
...
class ForecastResidualsMetric(ForecastMetric, HasVariance):
    ...
    def compute(self, sample_weights=None, bin_info=None,
        ...
        grouped_residuals = dict()          # type: Dict[int, List[float]]
        self._data = {}
        for idx, h in enumerate(self.horizons):
            if h in grouped_residuals:
                grouped_residuals[h].append(residuals[idx])
            else:
                grouped_residuals[h] = [residuals[idx]]
        for h in grouped_residuals:
            self._data[h] = {}
            partial_residuals = grouped_residuals[h]
    ...
    def aggregate(scores):
        ...
        score_data = [score[Metric.DATA] for score in scores]
        grouped_data = {}                   # type: Dict[int, List[Dict[str, Any]]]
        horizons = set()
        for cv in score_data:
            for key in cv.keys():
                horizons.add(key)
                if key in grouped_data.keys():
                    grouped_data[key].append({Metric.DATA: cv[key]})
                else:
                    grouped_data[key] = [{Metric.DATA: cv[key]}]

        data = {}
        for horizon in horizons:
            data[horizon] = ResidualsMetric.aggregate(grouped_data[horizon])[Metric.DATA]
