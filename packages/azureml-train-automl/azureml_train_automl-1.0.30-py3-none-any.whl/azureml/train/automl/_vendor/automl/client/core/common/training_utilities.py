...
from typing import Any, cast, Iterable, Optional, List, Dict, Callable
import datetime
import numpy as np
import pandas as pd
import scipy
...
from .exceptions import DataException, ConfigException
from .featurization import TimeSeriesTransformer
...
def _check_timeseries_input_and_get_tsdf(
        ...
        for data_tuple in tsdf.groupby_grain():
            ...
            try:
                ...
                if not is_validation_data or tsdf_grain.shape[0] > 1:
                    ...
                    data_points = len(
                        pd.date_range(
                            start=tsdf_grain.time_index[0],
                            end=tsdf_grain.time_index[-1],
                            freq=tsdf_grain.infer_freq()))
                ...
                if is_validation_data:
                    if data_points < automl_settings.max_horizon:
                        print(
                            "WARNING: Validatation set has less data points {} than max_horizon {} for grain [{}]. "
                        ...
                        print(
                            "WARNING: Validatation set has more data points {} than max_horizon {} for grain [{}]. "
    ...
    if not is_validation_data:
        data_points = len(pd.date_range(start=tsdf.time_index[0], end=tsdf.time_index[-1], freq=tsdf.infer_freq()))
