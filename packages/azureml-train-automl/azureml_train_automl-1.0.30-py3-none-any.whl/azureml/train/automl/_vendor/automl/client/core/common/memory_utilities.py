...
def get_all_ram(logger: Optional[logging.Logger] = None) -> int:
    ...
    try:
        ...
        if logger is not None:
            logger.error("Failed to get total memory on platform '{}': {}".format(sys.platform, e))
        raise

    raise Exception("{}: Unsupported platorm. Cannot determine the total memory on the machine."
                    .format(sys.platform))
...
def get_available_physical_memory(logger: Optional[logging.Logger] = None) -> int:
    ...
    try:
        ...
        if logger is not None:
            logger.error("Failed to get available memory on platform '{}': {}".format(sys.platform, e))
        raise

    raise Exception("{}: Unsupported platorm. Unable to findout available memory.".format(sys.platform))
