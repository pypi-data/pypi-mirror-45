    ...
    def sweep(self, column: Union[str, List[str]], featurize_separately: bool = False, *args: Any, **kwargs: Any) \
        ...
        try:
            ...
            if isinstance(column, list):
                X_train_sample = X_train.iloc[:, column]
                X_valid_sample = X_valid.iloc[:, column]
            else:
            ...
            self._logger.info(
                "Binary feature sweeper: {sweeper}, Baseline score: {bscore}, "
            ...
            self._logger.warning("Binary feature sweeper: {sweeper} failed due to exception {ex}".format(sweeper=self,
                                                                                                         ex=ex))
            logging_utilities.log_traceback(ex, self._logger)
        finally:
