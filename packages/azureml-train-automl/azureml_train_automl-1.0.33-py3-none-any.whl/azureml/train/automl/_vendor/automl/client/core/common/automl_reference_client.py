...
from .limit_function_call_exceptions import TimeoutException
...
class AutoMLReferenceClient:
    ...
    def _job_setup(self, job: AutoMLJob, raw_data_context: RawDataContext) -> TransformedDataContext:
        ...
        training_utilities.validate_data_splits(X=transformed_data_context.X,
                                                ...
                                                primary_metric=job.settings.primary_metric,
                                                task_type=job.settings.task_type)
