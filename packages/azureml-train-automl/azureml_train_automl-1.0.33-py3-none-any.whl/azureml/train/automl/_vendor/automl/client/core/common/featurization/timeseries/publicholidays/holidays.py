...
class Holidays:
    ...
    HOLIDAYS_DF = pd.DataFrame(columns=['Date', 'Name', 'Country', 'CountryCode', 'IsPaidTimeOff'])

    @classmethod
    def __load_holidays_data(cls, _path: str, _type: int) -> None:
        with lzma.open(_path, "rb") as fr:
            cls.HOLIDAYS_DF = pickle.loads(fr.read())
            cls.HOLIDAYS_DF.drop(columns=['index'], inplace=True)

    def __init__(self, type: int = 1):
        """Init the Holidays class to load the holiday data."""
        self.holiday_dataPath = pkg_resources \
            .resource_filename('automl.client.core.common',
                               os.path.join('featurization', 'timeseries', 'publicholidays',
                                            'data', 'holidays.7z'))
        Holidays.__load_holidays_data(self.holiday_dataPath, type)
        ...
        mappings_cr = Series(Holidays.HOLIDAYS_DF.CountryCode.values, index=Holidays.HOLIDAYS_DF.Country)
        mappings_cc = Series(Holidays.HOLIDAYS_DF.Country.values, index=Holidays.HOLIDAYS_DF.CountryCode)
    ...
    def adding_holidayname_by_window(self, _country: str, _countryCode: str, _name: str,
        ...
        :param _country: country of current data.
        ...
        for i in range(1, window + 1):
            ...
            if np.datetime64(newDate) not in data['Date'].values:
                ...
                data = data.append({'Name': name, 'Date': newDate, 'Country': _country,
    ...
    def adding_holidayname_by_window_row_by_row(self, data: pd.DataFrame, target_data: pd.DataFrame) -> pd.DataFrame:
        ...
        country_lst = Series(data.CountryCode.values, index=data.Country).to_dict().keys()
        for item in country_lst:
            temp_target = target_data[target_data.Country == item]
            temp_country_df = data[data.Country == item]
            ...
            target_data_origin = pd.concat([target_data_origin, temp_target], ignore_index=True).drop_duplicates()
    ...
    def update_holidays_by_adding_window(self) -> None:
        ...
        window_dataPath = pkg_resources \
            .resource_filename('automl.client.core.common',
                               os.path.join('featurization', 'timeseries', 'publicholidays',
                                            'data', 'holiday_effect_window.csv'))
        with lzma.open(holiday_origin_dataPath, "rb") as f:
            content = f.read()
        df_origin = pd.read_csv(BytesIO(content),
                                parse_dates=['date'],
                                date_parser=lambda x: pd.datetime.strptime(x, '%m/%d/%Y'))
        df_origin = df_origin.rename(columns={'normalizeHolidayName': 'Name', 'countryOrRegion': 'Country',
                                              'countryCode': 'CountryCode', 'date': 'Date',
                                              'isPaidTimeOff': 'IsPaidTimeOff'})
        holiday_window = pd.read_csv(window_dataPath)
        df_com = df_origin.set_index(['holidayName', 'Country']) \
            .join(holiday_window.set_index(['holidayName', 'Country']), how='left')
        ...
        country_lst = Series(_holidays.CountryCode.values, index=_holidays.Country).to_dict().keys()

        current_df = pd.DataFrame(columns=['Date', 'Country', 'gapFormer', 'gapNext'])
        for item in country_lst:
            temp_dts = list(dict.fromkeys(_holidays[_holidays.Country == item]['Date'].tolist(), None).keys())
            temp_dts.sort()
            i = 0
            while i < len(temp_dts):
                ...
                current_df = current_df.append({'Date': temp_dts[i], 'Country': item,
        ...
        _holidays = _holidays.set_index(['Date', 'Country']) \
            .join(current_df.set_index(['Date', 'Country']), how='left')
        ...
        df = pd.read_csv(StringIO(csv_content),
                         parse_dates=['Date'],
                         date_parser=lambda x: pd.datetime.strptime(x, '%m/%d/%Y'))
        with lzma.open(self.holiday_dataPath, "wb") as f:
        ...
        if date_col_name in Holidays.HOLIDAYS_DF.columns:
            if country_code:
                if self.holidays_dates_country_code != country_code:
                    self.holidays_dates = dict.fromkeys(Holidays.HOLIDAYS_DF[Holidays.HOLIDAYS_DF.CountryCode ==
                ...
                if self.holidays_dates_country != country_or_region:
                    self.holidays_dates = dict.fromkeys(Holidays.HOLIDAYS_DF[Holidays.HOLIDAYS_DF.Country ==
    ...
    def is_holiday_by_country_or_region(self, target_date: date, country_or_region: str = "United States") -> bool:
        ...
        :param target_date: The date which needs to be check.
        :param country_or_region: Indicate which country's holiday infomation will be used for the check.
        :return: Whether the target_date is a holiday or not. True or False.
        """
    ...
    def get_holidays_in_range(self, start_date: date, end_date: date, country_code: str = "US") -> pd.DataFrame:
        ...
        rs_date.drop(columns=['Country', 'CountryCode'], inplace=True)
    ...
    def get_holidays_in_range_by_country_or_region(self, start_date: date, end_date: date,
        ...
        if country_or_region is not None:
            rsdf_country = Holidays.HOLIDAYS_DF[Holidays.HOLIDAYS_DF.Country == country_or_region]
        else:
            return rs
        rs_date = rsdf_country[(rsdf_country.Date >= start_date) & (rsdf_country.Date <= end_date)]
        rs_date.drop(columns=['Country', 'CountryCode'], inplace=True)
