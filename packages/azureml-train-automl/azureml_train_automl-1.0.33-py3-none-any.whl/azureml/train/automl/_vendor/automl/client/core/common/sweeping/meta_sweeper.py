...
from automl.client.core.common.configuration.sampler_config import SamplerConfig
...
class MetaSweeper:
    ...
    def _recover_sweeping_from_checkpointfile(self, checkpoint_file: str) -> List[Tuple[Any, Pipeline]]:
        ...
        try:
            ...
            logging_utilities.log_traceback(
                ex,
                self._logger,
                override_error_msg="Failed to recover feature sweeping state from checkpoint file due to {}".format(ex)
            )
        return result
    ...
    def _sweep_internal(sweepers: List[AbstractSweeper], enabled_sweeper_configs: List[SweeperConfig],
        ...
        logger.info("Beginning Feature Sweeping...")
