...
class WidgetVersionBehavior:
    ...
    def create(self, widget, port, editor_port, widget_file_path, docker_image,\
        ...
        if docker_image is not None and docker_image != '':
            ...
            widget_file_path = cu.save_image(docker_image)

        if from_version != '' and from_version is not None:
            try:
                from_version_id = int(from_version)
                version = self.util.cli_request('GET', self.util.build_url('{app}/iot/v1/containers/{widget_id}/versions/{from_version_id}',\
                    {'widget_id': widget_id, 'from_version_id': from_version_id} ))['payload']['data']
            except:
                version = self.util.get_version(self.object_type, widget_id, from_version)

            if version is None:
                raise click.BadParameter(click.style("Widget version not found", fg='red'), param_hint="--version-number")
            ...
            if widget_file_path is None or widget_file_path == '':
                widget_file_path = input("Widget File Path: ").replace(" ", "")
                if widget_file_path is None or widget_file_path == '':
                    raise click.ClickException(click.style("If you don't create from an old version then you must provide --widget-file-path or --docker-image", fg='red'))

            if port is None or port == '':
                port = int(input("Port: "))
                if port is None or port == '':
                    raise click.ClickException(click.style("If you don't create from an old version then you must provide a --port for your widget", fg='red'))
        ...
        if widget_file_path is not None and widget_file_path != '':
            if not self.json:
                click.echo("Uploading image to Lumavate")
                click.echo('Image Size: {}'.format(self.util.get_size(widget_file_path)))

            version['ephemeralKey'] = self.util.upload_ephemeral(widget_file_path, 'application/gz')
        ...
        resp = self.util.cli_request('POST',
            self.util.build_url('{app}/iot/v1/containers/{widget_id}/versions', {'widget_id': widget_id}), data=json.dumps(version))

        if self.json:
            click.echo(json.dumps(resp))
            return

        # TO DO: Add progress bar after create for loading/validating widget
        progress = 0
        status_id = None
        error = None
        ...
        if 'results' in resp['payload']['data'].keys():
            status_id = resp['payload']['data']['results']['statusId']
        if status_id:
            with click.progressbar(length=100, label='Creating widget') as bar:
                while progress < 100 and error is None:
                    status_resp = self.util.cli_request('GET', self.util.build_url('{app}/iot/v1/statuses/{status_id}', {'status_id': status_id}))
                    progress = status_resp['payload']['data']['overallPercent']
                    error = status_resp['payload']['data']['errorMessage']
