...
class Util:
    def __init__(self, context=None, profile=None):
        self.context = pyke.auth.load_cache() if not context else context
        if profile is None:
            self.profile = self.context.get('profileName')
        else:
            self.profile = profile

    def login_token(self, renew=False):
        if renew:
            self.context['accessToken'] = self.login()['access_token']
            return self.context['accessToken']

        if self.context is not None:
            return self.context.get('accessToken')

    def login_without_context(self, env, config_data=None):
        if config_data is None:
            config_data = pyke.auth.load_config()
        else:
            config_data = config_data

        env = config_data['envs'][env]
        ...
        req_data = pyke.auth.sanatize_data(data)

        token_url = env['token'].replace(" ", "")
        resp = self.cli_request('POST', 'https://' + token_url+ '/oauth/token', json=req_data, external=True)
    ...
    def login(self):
        data = {
          'grant_type': 'client_credentials',
          'audience': self.context['audience'].replace(" ", ""),
          'client_id': self.context['clientId'].replace(" ", ""),
          'client_secret': self.context['clientSecret'].replace(" ", "")
        }

        res = self.cli_request('POST', 'https://{}/oauth/token'.format(self.context['token']), json=data, external=True)
        ...
        if self.context.get('orgId') is not None:
            url = '{}/auth/v1/session'.format(self.context.get('app'))
            data = {'companyId': self.context.get('orgId')}
            r = requests.post(url, json=data, headers={'Authorization': 'Bearer ' + resp['access_token']})
            if r.status_code != 200:
                raise click.BadParameter(click.style("Not authorized for company id {}".format(self.context.get('orgId')), fg='red'))

        pyke.auth.set_cache_token(resp.get('access_token'))
        pyke.auth.set_profile_token(self.context.get('profileName'), resp.get('access_token'))
    ...
    def load_users(self):
        resp = self.cli_request('GET', self.build_url('{app}/auth/v1/users'))
    ...
    def print_table(self, data, format, titles={}, handlers={}):
        ...
        click.echo(format.format(**headers))
    ...
    def check_response(self, resp):
        if resp.status_code == 500:
            ...
            message = resp.json()['payload']['error']
            if 'apiField' in resp.json()['payload']:
                message += ' (' + resp.json()['payload']['apiField'] + ')'
    ...
    def build_url(self, format, extra_data = {}):
        data = pyke.auth.sanatize_data(self.context)
    ...
    def cli_request(self, method, url, data=None, json=None, headers=None, params=None, external=False):
        ...
        url = url.replace(" ", "")
        if data is not None:
            resp = self.check_response(req(url, data=data, **req_args))
        else:
            resp = self.check_response(req(url, **req_args))

        if resp.status_code != 200 and external is False:
    ...
    def list_companies(self, env, print_table=True):
        conf = pyke.auth.load_config()
        ...
        headers = {
            'Authorization': 'Bearer ' + self.login_without_context(env),
            'Content-Type': 'application/json'
        }

        app_url = conf['envs'][env]['app'].replace(" ", "")
        res = requests.get('{}/auth/v1/me/companies?sort=name'.format(app_url), headers=headers)
        ...
        if print_table is True:
            self.print_table(resp, '{id} {name} {instanceType} {isTest}', handlers=handlers, titles=titles)
    ...
    def upload_ephemeral(self, filename, content_type):
        resp = self.cli_request('GET', self.build_url('{app}/iot/v1/files/ephemeral?contentType=' + urllib.parse.quote(content_type)))
        if '/' in filename:
    ...
    def get_current_access_records(self, object, object_id):
        ...
        resp = self.cli_request('GET', self.build_url('{app}/iot/v1/' + path))
    ...
    def lookup_object_id(self, object_type, id, abort_if_null=True):
        if object_type == 'component-set':
            ...
            resp = self.cli_request('GET',
                self.build_url('{app}/iot/v1/component-sets?{filter_type}={object_filter}',
            ...
            resp = self.cli_request('GET',
                self.build_url('{app}/iot/v1/containers?{filter_type}={object_filter}',
                    extra_data={'filter_type': filter_type, 'object_filter': object_filter}))
    ...
    def get_versions_from_mask(self, object_type, parent_id, version):
        if object_type == 'component-set':
            versions = self.cli_request('GET', self.build_url('{app}/iot/v1/component-sets/{parent_id}/versions?pagesize=500',\
                {'parent_id': parent_id}))['payload']['data']
        else:
            versions = self.cli_request('GET', self.build_url('{app}/iot/v1/containers/{parent_id}/versions?pagesize=500',\
                {'parent_id': parent_id}))['payload']['data']
    ...
    def get_version_from_version_number(self, object_type, parent_id, version_number):
        ...
        if object_type == 'component-set':
            version = self.cli_request('GET', self.build_url('{app}/iot/v1/component-sets/{parent_id}/versions?major={major}&minor={minor}&patch={patch}',\
                { 'parent_id': parent_id, 'major': major, 'minor': minor, 'patch': patch }))['payload']['data']
        else:
            version = self.cli_request('GET', self.build_url('{app}/iot/v1/containers/{parent_id}/versions?major={major}&minor={minor}&patch={patch}',\
                { 'parent_id': parent_id, 'major': major, 'minor': minor, 'patch': patch }))['payload']['data']
    ...
    def resolve_default_icon_path(self):
        ...
        if platform.system() == 'Windows':
            ...
            return '{}\\images\\default-icon.svg'.format(icon_path)
        else:
            icon_path = pwd.rsplit('/', 1)[0]
            return '{}/images/default-icon.svg'.format(icon_path)
    ...
    def show_progress(self, status_id, label=None):
      ...
      with click.progressbar(length=100, label=label) as bar:
        while progress < 100 and error is None:
          status_resp = self.cli_request('GET', self.build_url('{app}/iot/v1/statuses/{status_id}', {'status_id': status_id}))
