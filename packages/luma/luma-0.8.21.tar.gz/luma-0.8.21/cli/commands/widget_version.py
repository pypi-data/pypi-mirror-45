...
@click.option('--json/--table', default=False)
@pyke.auth.profile_context_required
def create(widget, profile, port, editor_port, widget_file_path,\
  ...
  behavior.WidgetVersionBehavior(profile=profile, json=json).create(widget, port,\
      editor_port, widget_file_path, docker_image, from_version, version, version_bump, env, label, format)
