...
@click.option('--editor-port', type=int, help='The port to connect to the your microservice')
...
@click.option('--json/--table', default=False, help='Return raw json from the platform.')
@pyke.auth.profile_context_required
def create(microservice, profile, port, editor_port, microservice_file_path,\
    ...
    behavior.MicroserviceVersionBehavior(profile=profile, json=json).create(microservice, port,\
        editor_port, microservice_file_path, docker_image, from_version, version, version_bump, env, label, format)
