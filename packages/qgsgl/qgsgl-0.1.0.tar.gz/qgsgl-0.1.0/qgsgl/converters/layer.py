from qgis.core import QgsSingleSymbolRenderer, QgsCategorizedSymbolRenderer, \
    QgsGraduatedSymbolRenderer, QgsFillSymbolLayer, QgsLineSymbolLayer, \
    QgsSimpleMarkerSymbolLayerBase, QgsProject
from ..glyphs import default_library
from ..layer import CircleLayer, FillLayer, LabelLayer, LineLayer, SymbolLayer
...
class LayerConverter:
    ...
    def __init__(self, qgis_layer, source_id, source_layer_id=None, **kwargs):
        ...
        self._visible = kwargs.get('visible', None)
        if self._visible is None:
            self._visible = QgsProject.instance().layerTreeRoot().findLayer(
                self._qgis_layer.id()).itemVisibilityChecked()
    ...
    def _get_layer_types(self, symbol_layer):
        ...
        if isinstance(symbol_layer, QgsFillSymbolLayer):
            # TODO: Is this the right way to check whether there is a stroke?
            if symbol_layer.strokeColor():
                return [self.LINE, self.FILL]
            else:
                return [self.FILL]
        # TODO: Check whether this is a supported type of symbol layer.
        return [self.SYMBOL]

    def _get_gl_layer(self, layer_type, symbol_layer, id, **kwargs):
        if layer_type == self.CIRCLE:
            return self._circle_class(symbol_layer, **kwargs)
        if layer_type == self.FILL:
            return self._fill_class(symbol_layer, **kwargs)
        if layer_type == self.LINE:
            return self._line_class(symbol_layer, **kwargs)
        return self._symbol_class(symbol_layer, image_id=id, **kwargs)
    ...
    def _convert_symbol(self, symbol, **options):
        ...
        for symbol_layer in symbol.symbolLayers():
            for layer_type in self._get_layer_types(symbol_layer):
                id = self._get_id()
                gl_layer = self._get_gl_layer(
                    layer_type, symbol_layer, id,
                    opacity=symbol.opacity())
                layer_json = self._get_layer_json(gl_layer, id, **options)
                layers.append(layer_json)
                if layer_type == self.SYMBOL:
                    self.images.append(SpriteImage(symbol_layer, id))
