...
class Kolibri(object):
    ...
    def __init__(self, root_dir, options=None):
        """
        ...
        self.build_static_page_assets_dir = os.path.join(
            self.build_static_dir, "_page-assets")
        ...
        self.config.setdefault("site_env", {}) 

        self.build_config = utils.dictdot(self.config.get("build", {}))
        self.build_config.setdefault("layout", DEFAULT_LAYOUT)
        self.layout = self.build_config.get("layout")

        site_env = self.config.get("build.env")
        if options and options.get("env") is not None: 
            site_env = options.get("env")

        self.site_config = utils.dictdot(self.config.get("site", {}))
        if site_env: 
            if site_env in self.config["site_env"]: 
                self.site_config = utils.merge_dicts(self.site_config, self.config.get('site_env.%s' % site_env))
            else: 
                raise ValueError("Can't build. ENV '%s' is specified, but didn't match" % site_env)
                ...
                for o in sfc_o:
                    if (sfc_c.get(o)):
                        ...
                        with open(_ff, "w") as f:
                            f.write(sfc_c.get(o))
                        if o == "script":
                            script_props = sfc_c["script_props"]
                            content += "{% block __PAGE_COMPONENT_SCRIPT__ %}"
                            content += "{{ Kolibri.script_tag(\"%s\", props='%s') }}" % (_sff, script_props)
                        elif o == "style":
                            content += "{% block __PAGE_COMPONENT_STYLE__ %}"
                            content += "{{ Kolibri.stylesheet_tag(\"%s\") }}" % _sff
                        content += "{% endblock %}"
