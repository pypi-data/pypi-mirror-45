...
class SpellCheck:
    ...
    def __init__(self, path: str, from_azure: bool = False):
        self.set_embedding(path, from_azure)
        self.data = None
        self.file_name = 'output_spell_check.csv'
    
    def set_embedding(self, path: str, from_azure: bool):
        if from_azure:
            self.set_embedding_from_azure(path)
            self.embedding = Word2Vec.load('embedding.model')
        else:
            self.embedding = Word2Vec.load(path)
            
    def set_embedding_from_azure(self, config_file_path: str):
    ...
    def calculate_limits(self, ind: int, sentence_len: int):
    ...
    def find_correct_word(self, ind: int, left_limit: int, right_limit: int, sentence_lst: list):
        left_words = sentence_lst[left_limit:ind][::-1]
        right_words = sentence_lst[ind+1:right_limit]
        similar_words = self.embedding.predict_output_word(left_words + right_words, topn = 10)
    ...
    def spell_check_sentence(self, sentence: str):
        ...
        mask_wrong = [False if word in self.embedding.wv.vocab or word.isdigit() else True for word in sentence_lst]
        
        if True in mask_wrong:
            sentence_len = len(sentence_lst)
            for ind, value in enumerate(mask_wrong):
                if value:
                    left_limit, right_limit = self.calculate_limits(ind,sentence_len)    
                    if False in mask_wrong[left_limit:right_limit]:    
                        most_similar_word, similarity_score = self.find_correct_word(ind, left_limit, right_limit, sentence_lst)    
                        if similarity_score > self.threshold:
                            sentence_lst[ind] = most_similar_word
                        
            return ' '.join(sentence_lst)
    
    def spell_check(self, window_limit: int = 5, threshold: float = 0.9, save_result: bool = False):
        ...
        if self.data is not None:
            try:
                ...
                print('Erro spell checking: {}'.format(e))
            else:
                result = pd.DataFrame({'Original': self.data, 'Spell Checked': spell_checked_sentences, 'Corrected': corrected})
                if save_result: 
                    print('Saving spell checked file as {}'.format(self.file_name))
                    result.to_csv(self.file_name, sep = ';', encoding = 'utf-8', index=False)
                return result
            finally:
