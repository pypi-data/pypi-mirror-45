...
def print_combination(combination, prefix="", postfix=""):
    """
    Prints the `combination` string on its own line. This string is preceded by the
    optionally provided `prefix` string, and followed by the optionally provided
    `postfix` string.
    """
    if len(prefix) > 0:
        print(prefix, end=" ")
...
def main(file_path, prefix="", postfix="", max_args=None):
    ...
    for combination in line_combinations:
        print_combination(combination, prefix, postfix)
