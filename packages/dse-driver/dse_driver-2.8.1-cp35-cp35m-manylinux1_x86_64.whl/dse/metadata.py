...
class KeyspaceMetadata(object):
    ...
    for clusters running pre-6.7 versions of DSE.
...
def get_schema_parser(connection, server_version, timeout):
    ...
    if server_major_version >= 4:
        server_dse_version_split = list(int(x) for x in server_version.split('.')[3])
        if server_dse_version_split[0] >= 6 and server_dse_version_split[1] >= 7:
