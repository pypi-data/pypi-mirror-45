...
from .thingamabob import Thingamabob, ThingamabobSchema


@pytest.fixture
def schema():
    return ThingamabobSchema()


def test_post(app):  # noqa
    assert app


def test_post(app, client, schema):  # noqa
    with client:
        obj = Thingamabob(id=42)
        resp = client.post('thingamabob/', json=schema.dump(obj).data)
...
def test_get(app, client, schema):  # noqa
    with client:
        resp = client.get('thingamabob/?id=42')
        rv = schema.load(resp.json).data
        assert rv
        assert rv.id == 42
