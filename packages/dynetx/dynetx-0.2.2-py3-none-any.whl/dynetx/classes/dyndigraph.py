...
class DynDiGraph(nx.DiGraph):
    ...
    **Edges:**
    ...
    def degree_iter(self, nbunch=None, t=None):
        ...
        if nbunch is None:
            nodes_nbrs = ((n, succs, self._pred[n]) for n, succs in list(self._succ.items()))
        else:
        ...
        if t is None:
            ...
            for n, succ, pred in nodes_nbrs:
                edges_succ = len([v for v in list(succ.keys()) if self.__presence_test(n, v, t)])
                edges_pred = len([v for v in list(pred.keys()) if self.__presence_test(v, n, t)])
    ...
    def interactions_iter(self, nbunch=None, t=None):
        ...
        if nbunch is None:
            nodes_nbrs_succ = list(self._succ.items())
        else:
    ...
    def in_interactions_iter(self, nbunch=None, t=None):
        ...
        """
        if nbunch is None:
            nodes_nbrs_pred = list(self._pred.items())
        else:
    ...
    def in_degree_iter(self, nbunch=None, t=None):
        ...
        """
        if nbunch is None:
            nodes_nbrs = list(self._pred.items())
        else:
        ...
        if t is None:
            ...
            for n, nbrs in nodes_nbrs:
                edges_t = len([v for v in list(nbrs.keys()) if self.__presence_test(v, n, t)])
                if edges_t > 0:
                    yield (n, edges_t)
                else:
                    yield (n, 0)
    ...
    def out_degree_iter(self, nbunch=None, t=None):
        ...
        """
        if nbunch is None:
            nodes_nbrs = list(self._succ.items())
        else:
