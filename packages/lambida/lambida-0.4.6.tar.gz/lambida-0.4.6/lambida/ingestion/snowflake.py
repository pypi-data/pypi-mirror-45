...
class SnowflakeJsonInsert(object):
    ...
    def insert(self, message):
        cursor_list = self.ctx.execute_string("insert into %s (select PARSE_JSON('%s'))" % (self.table, message))
        self.log.info('Ingested Message into Snowflake: {}'.format(cursor_list))
