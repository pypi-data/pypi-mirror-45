...
def show(train_data, save = False,root = ''):
    '''
    ...
    '''
    samples = 4
    idxs = np.random.choice(len(train_data), samples, replace=False)
    print(idxs)
...
def datagenerator(data_dir,patch_size = (128,128),stride = (32,32), train_data_num = float('inf'),download=True,datasets = "Hess_VTI",aug_times=0,scales = [1],verbose=True,jump=1,agc=True):
    ...
    Args:
        ...
        datasets : the name of the datasets will be download,if download = True
    ...
    if download:
        if datasets: 
            ...
            print("Please input the name of the dataset to download ")
...
#    torch.set_default_dtype(torch.float32) 
    root = '/home/wj/Northwest_AI/Read_segy/data/Train'
    train_data  = datagenerator(data_dir = root,patch_size = (64,64),stride = (32,32),train_data_num =float("inf"),download=False,datasets = "Hess_VTI",aug_times=0,scales = [1],verbose=False,jump=80,agc=True)
...
#    DDataset = DownsamplingDataset(xs,0.7)
    show(DDataset,save=True,root = root) # show and save the 4 samples data