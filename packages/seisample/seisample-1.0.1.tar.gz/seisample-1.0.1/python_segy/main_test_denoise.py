...
def parse_args():
    ...
    parser.add_argument('--set_names', default=['Zhuozhou'], help='directory of test dataset')
    ...
    parser.add_argument('--model_dir', default=os.path.join('models_denoise', 'DnCNN_sigma25'), help='directory of the model')
        ...
        for im in os.listdir(os.path.join(args.set_dir, set_cur)):
            if im.endswith(".segy") or im.endswith(".sgy"):
                ...
                if args.save_result:
                    ...
                    show(np.hstack((y, x_))) 
                    save_result(x_, path=os.path.join(args.result_dir, set_cur, name+'_dncnn'+'.png'))  # save the denoised image
                psnrs.append(psnr_x_)
        psnr_avg = np.mean(psnrs)
        psnrs.append(psnr_avg)
        if args.save_result:
            save_result(psnrs, path=os.path.join(args.result_dir, set_cur, 'results.txt'))
        log('Datset: {0:10s} \n  PSNR = {1:2.2f}dB'.format(set_cur, psnr_avg))
