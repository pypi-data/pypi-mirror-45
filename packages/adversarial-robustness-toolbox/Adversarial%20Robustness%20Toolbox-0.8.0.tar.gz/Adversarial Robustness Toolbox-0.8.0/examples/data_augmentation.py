...
def build_model(input_shape=(32, 32, 3), nb_classes=10):
    ...
    conv2d_1 = Conv2D(6, (5, 5), padding='valid', kernel_regularizer=l2(0.0001), activation='relu',
                      kernel_initializer='he_normal', input_shape=input_shape)(img_input)
...
pgd = ProjectedGradientDescent(classifier, eps=8, eps_step=2, max_iter=10, random_init=True)
...
print('Accuracy on original FGM adversarial samples: %.2f%%' %
...
x_test_pgd = pgd.generate(x_test)
labels_pgd = np.argmax(classifier.predict(x_test_pgd), axis=1)
print('Accuracy on new FGM adversarial samples: %.2f%%' % (np.sum(labels_pgd == labels_true) / x_test.shape[0] * 100))
