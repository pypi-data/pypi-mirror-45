...
from Pyautomators import parser
from Pyautomators.model_core import FileLocation
from Pyautomators.textutil import ensure_stream_with_encoder
# LAZY: from Pyautomators.step_registry import setup_step_decorators
import logging
...
class FeatureScenarioLocationCollector(object):
    ...
    USE CASE:
        Pyautomators feature/foo.feature:10
        Pyautomators @selected_features.txt
        Pyautomators @rerun_failed_scenarios.txt
...
def collect_feature_locations(paths, strict=True):
    ...
    for path in paths:
        if os.path.isdir(path):
            ...
            # -- USE: Pyautomators @list_of_features.txt
...
def load_step_modules(step_paths):
    ...
    from Pyautomators import matchers
    from Pyautomators.step_registry import setup_step_decorators
    step_globals = {
...
def print_undefined_step_snippets(undefined_steps, stream=None, colored=True):
    ...
    if colored:
        ...
        from Pyautomators.formatter.ansi_escapes import escapes
