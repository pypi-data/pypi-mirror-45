...
from Pyautomators.model_core import FileLocation
from Pyautomators.reporter.junit import JUnitReporter
from Pyautomators.reporter.summary import SummaryReporter
from Pyautomators.tag_expression import TagExpression
from Pyautomators.formatter.base import StreamOpener
from Pyautomators.formatter import _registry as _format_registry
from Pyautomators.userdata import UserData, parse_user_define
from Pyautomators._types import Unknown
from Pyautomators.textutil import select_best_encoding, to_texts
...
options = [
    ...
    (("-en", "--environment_file"),
     dict( dest="environment_file", metavar="FILE",
          help="Please indicate your enviroment.py file")),
...
def read_configuration(path):
    ...
    for fixed, keywords in options:
        ...
        if dest in "tags_help lang_list lang_help version".split():
            continue
        if not config.has_option("Pyautomators", dest):
            continue
        action = keywords.get("action", "store")
        if action == "store":
            use_raw_value = dest in raw_value_options
            result[dest] = config.get("Pyautomators", dest, raw=use_raw_value)
        elif action in ("store_true", "store_false"):
            result[dest] = config.getboolean("Pyautomators", dest)
        elif action == "append":
            if dest == "userdata_defines":
                continue    # -- SKIP-CONFIGFILE: Command-line only option.
            result[dest] = \
                [s.strip() for s in config.get("Pyautomators", dest).splitlines()]
        else:
    ...
    special_config_section_map = {
        "Pyautomators.formatters": "more_formatters",
        "Pyautomators.userdata":   "userdata",
    }
...
def config_filenames():
    ...
    for path in reversed(paths):
        for filename in reversed(
                ("Pyautomators.ini", ".Pyautomatorsrc", "setup.cfg", "tox.ini")):
...
def setup_parser():
    ...
    description = """\
    Run a number of feature tests with Pyautomators."""
    more = """
    EXAMPLES:
    Pyautomators features/
    Pyautomators features/one.feature features/two.feature
    Pyautomators features/one.feature:10
    Pyautomators @features.txt
    """
...
class Configuration(object):
    """Configuration object for Pyautomators and Pyautomators runners."""
    ...
    def __init__(self, command_args=None, load_config=True, verbose=None,
                 **kwargs):
        """
        Constructs a Pyautomators configuration object.
    ...
    def setup_logging(self, level=None, configfile=None, **kwargs):
        ...
        if configfile:
            ...
            if("filename" in kwargs.keys()):
                logging.basicConfig(format=format_, datefmt=datefmt, finename=kwargs["filename"])
            else:    
