# -*- coding: UTF-8 -*-
"""
This module provides Runner class to run Pyautomators feature files (or model elements).
"""
...
from Pyautomators._types import ExceptionUtil
from Pyautomators.capture import CaptureController
from Pyautomators.configuration import ConfigError
from Pyautomators.formatter._registry import make_formatters
from Pyautomators.runner_util import \
    collect_feature_locations, parse_features, \
    exec_file, load_step_modules, PathManager
from Pyautomators.step_registry import registry as the_step_registry
...
class ContextMaskWarning(UserWarning):
    ...
    *Pyautomators* overwites the value.

    If the variable was originally set by *Pyautomators* then this will be raised if
...
class Context(object):
    ...
    Certain names are used by *Pyautomators*; be wary of using them yourself as
    *Pyautomators* may overwrite the value you set. These names are:

    .. attribute:: feature

      This is set when we start testing a new feature and holds a
      :class:`~Pyautomators.model.Feature`. It will not be present outside of a
    ...
    .. attribute:: scenario
      ...
      :class:`~Pyautomators.model.Scenario`. It will not be present outside of the
      scope of a scenario.

    .. attribute:: tags

      The current set of active tags (as a Python set containing instances of
      :class:`~Pyautomators.model.Tag` which are basically just glorified strings)
    ...
    .. attribute:: table
      ...
      This is set at the step level and holds any :class:`~Pyautomators.model.Table`
    ...
    .. attribute:: config
      ...
      The configuration of *Pyautomators* as determined by configuration files and
    ...
    .. attribute:: active_outline
      ...
      :class:`~Pyautomators.model.Row` that is active for the current scenario. It is
    ...
    .. attribute:: log_capture
      ...
      logging as an instance of :class:`~Pyautomators.log_capture.LoggingCapture`.
    ...
    indeed by *Pyautomators* to overwite a user-set variable, then a
    :class:`Pyautomators.runner.ContextMaskWarning` warning will be raised.
    ...
    .. _`configuration file section names`: Pyautomators.html#configuration-files
    """
    ...
    def _use_with_Pyautomators_mode(self):
    ...
    def _emit_warning(self, attr, params):
        ...
        if self._mode is self.BEHAVE and self._origin[attr] is not self.BEHAVE:
            msg = "Pyautomators runner is masking context attribute '%(attr)s' " \
            ...
            if self._origin[attr] is not self.USER:
                msg = "user code is masking context attribute '%(attr)s' " \
                      "originally set by Pyautomators"
    ...
    def execute_steps(self, steps_text):
        ...
        with self._use_with_Pyautomators_mode():
            for step in steps:
...
class ModelRunner(object):
    """
    Test runner for a Pyautomators model (features).
...
class Runner(ModelRunner):
    """
    Standard test runner for Pyautomators:
