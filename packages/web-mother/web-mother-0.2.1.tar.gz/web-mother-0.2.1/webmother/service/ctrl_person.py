...
def add(oid, indicator, employment_json, *auth_args):
    ...
    idt = Employment().verify(*auth_args).operable('{}/*'.format(o.get('node')), 'employ.hire')
    # END
...
def update(oid, uid, employment_json, *auth_args):
    ...
    # 授权检查
    idt = Employment().verify(*auth_args).operable(o.get('node'), 'employ.hire')
    # END
...
def remove(oid, uid, *auth_args):
    ...
    # 授权检查
    Employment().verify(*auth_args).operable(o.get('node'), 'employ.fire')
    # END
...
def query(oid, *auth_args):
    ...
    cursor = db.org2user.find({'org': bson_id(oid)}, {'org': 0})
