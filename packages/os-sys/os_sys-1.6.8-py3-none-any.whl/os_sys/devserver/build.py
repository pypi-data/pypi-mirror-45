...
def run_local(path='.', port=9999):
    ...
    for path in paths:
        try:
            _path = str(''.join(path.split('.')[:-2])).replace(base_dir, '', count=1)
            if base_dir == '':
                data = open(path.replace('.', os.path.abspath(), count=1)).read()
            else:
                data = open(path.read())
                _raw.append(eval("""(%s, %s)""" % (_path, data)))
        except Exception:
            pass
    end = tuple(_raw)
    config(*end, port=port)
