...
from server.apps.registry import Apps
from server.conf import UserSettingsHolder, settings
from server.core import mail
from server.core.exceptions import ImproperlyConfigured
from server.core.signals import request_started
from server.db import DEFAULT_DB_ALIAS, connections, reset_queries
from server.db.models.options import Options
from server.template import Template
from server.test.signals import setting_changed, template_rendered
from server.urls import get_script_prefix, set_script_prefix
from server.utils.translation import deactivate
...
def setup_test_environment(debug=None):
    ...
    settings.EMAIL_BACKEND = 'server.core.mail.backends.locmem.EmailBackend'
...
class override_settings(TestContextDecorator):
    ...
    def decorate_class(self, cls):
        from server.test import SimpleTestCase
...
class override_system_checks(TestContextDecorator):
    ...
    def __init__(self, new_checks, deployment_checks=None):
        from server.core.checks.registry import registry
...
def require_jinja2(test_func):
    ...
    test_func = override_settings(TEMPLATES=[{
        'BACKEND': 'server.template.backends.server.DjangoTemplates',
        'APP_DIRS': True,
    }, {
        'BACKEND': 'server.template.backends.jinja2.Jinja2',
        'APP_DIRS': True,
...
class LoggingCaptureMixin:
    """
    Capture the output from the 'server' logger and store it on the class's
