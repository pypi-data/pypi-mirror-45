...
from server.core.files.temp import NamedTemporaryFile
from server.core.management.base import BaseCommand, CommandError
from server.core.management.utils import (
    find_command, handle_extensions, popen_wrapper,
)
from server.utils.encoding import DEFAULT_LOCALE_ENCODING
from server.utils.functional import cached_property
from server.utils.jslex import prepare_js_for_gettext
from server.utils.text import get_text_list
from server.utils.translation import templatize
...
class Command(BaseCommand):
    help = (
        ...
        "file in the conf/locale (in the server tree) or locale (for projects and "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--domain', '-d', default='server', dest='domain',
            help='The domain of the message files (default: "server").',
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help='The file extension(s) to examine (default: "html,txt,py", or "js" '
                 'if the domain is "serverjs"). Separate multiple extensions with '
    ...
    def copy_plural_forms(self, msgs, locale):
        ...
        """
        server_dir = os.path.normpath(os.path.join(os.path.dirname(server.__file__)))
        if self.domain == 'serverjs':
            domains = ('serverjs', 'server')
        else:
            domains = ('server',)
        for domain in domains:
            server_po = os.path.join(server_dir, 'conf', 'locale', locale, 'LC_MESSAGES', '%s.po' % domain)
            if os.path.exists(server_po):
                with open(server_po, 'r', encoding='utf-8') as fp:
