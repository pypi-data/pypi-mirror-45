...
from server.db.migrations.operations.base import Operation
from server.db.migrations.utils import COMPILED_REGEX_TYPE, RegexObject
from server.utils import datetime_safe
from server.utils.functional import LazyObject, Promise
from server.utils.timezone import utc
from server.utils.version import get_docs_version
...
class DatetimeSerializer(BaseSerializer):
    def serialize(self):
        ...
        if self.value.tzinfo is not None:
            imports.append("from server.utils.timezone import utc")
...
class DeconstructableSerializer(BaseSerializer):
    ...
    def _serialize_path(path):
        ...
        if module == "server.db.models":
            imports = {"from server.db import models"}
            name = "models.%s" % name
        else:
...
class OperationSerializer(BaseSerializer):
    def serialize(self):
        from server.db.migrations.writer import OperationWriter
...
class SettingsReferenceSerializer(BaseSerializer):
    def serialize(self):
        return "settings.%s" % self.value.setting_name, {"from server.conf import settings"}
