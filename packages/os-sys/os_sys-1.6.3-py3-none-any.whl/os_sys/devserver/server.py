            ...
            if '<?php' in response and '?>' in response:
                ...
                for p in code:
                    ...
                    if "is not recognized as an internal or external command" in a:
                        ...
                        php_done = False
                        if name != plat:
                            why = 'WARNING: php runner only 100% works on windows not on %s' % plat
                        else:
                            why = 'WARNING: internal error php can\'t be loaded'
                    if php_done:
                        response = response.replace('<?php'+to_do+'?>', a, 1)
                    else:
                        response = response.replace('<?php'+to_do+'?>', '<script>alert("WARNING: %s");</script>' % why, 1)
                        php_done = True
                    print('php:', a)
            header = 'HTTP/1.1 200 OK\n'
