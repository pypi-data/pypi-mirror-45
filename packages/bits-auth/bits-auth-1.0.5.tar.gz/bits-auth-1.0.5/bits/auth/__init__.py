...
class Auth(object):
    ...
    def aws(self):
        ...
        return AWS(
            auth=self,
            credentials_file='etc/aws_credentials',
            root_account='374741154730',
            verbose=self.verbose,
        )
    ...
    def bitsdbapi(self):
        ...
        return BitsdbApi(
            ...
            host=self.settings['bitsdbapi']['host'],
            name=self.settings['bitsdbapi']['name'],
    ...
    def disclosure(self):
        ...
        from bitsapiclient.services.disclosure import Disclosure
        return Disclosure(
    ...
    def quay(self):
        """Connect to Quay API."""
        from bitsapiclient.services.quay import Quay
        return Quay(
    ...
    def shoretel(self):
        ...
        return Shoretel(
            self.settings['mysql_servers']['shoretel']['db_host'],
            self.settings['mysql_servers']['shoretel']['db_port'],
            self.settings['mysql_servers']['shoretel']['db_user'],
            self.settings['mysql_servers']['shoretel']['db_pass'],
            self.settings['mysql_servers']['shoretel']['db']
        )
