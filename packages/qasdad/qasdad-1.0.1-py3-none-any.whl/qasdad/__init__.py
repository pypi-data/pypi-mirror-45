...
class Column:
	...
	memName = None
	##\deprecated
	##data is deprecated and was a list that holds the value of the physical property (with a unit)
	...
	data = None
	##The preffered unit of the physical property. preferedUnit does not have to be a SI-Unit
	preferedUnit = None
	##rawData is a list hat holds the value of the physical property (without a unit) rawData*preferedUnit is the value of the physical property
	rawData = None
	##rawDelta is  a list that holds the measuring inaccuracy (without a unit) rawDelta*preferedUnit is the measuring inaccuracy
	#delta may be None
	#If delta is not None the lengths of data and delta have to be equivalent
	rawDelta = None
	##\deprecated
	##delta is deprecated was a list that holds the measuring inaccuracy (with a unit)
	#delta may be None
	#If delta is not None the lengths of data and delta have to be equivalent
	delta = None
	...
	def dataForPlot(self):
		startTime3 = millis()
		ret = np.zeros(len(self.rawData))
		factor = subsSIUnitsOne(self.preferedUnit)
		for i in range(len(self.rawData)):#TODO PERFORMANCE improves are possible here
			ret[i] = self.rawData[i]*factor
		if PROFILE:
			print(str(millis()-startTime3) + " ms for dataForPlot")
		return ret
	##\return unit of the physical property
	def unit(self):
		return checkGetUnit(self.preferedUnit)
		exit(0)
	#Constructor:
	...
	#\param preferedUnit prefered unit of the physical property. Does not have to be a SI unit
	...
	def __init__(self, memName, nameType, preferedUnit, rawData, specialTexMathName=None, specialTexTextName=None, rawDelta=None):#TODO nameType soll ein optionales argument sein, wenn texMathName und texTextName gegeben sind
		rawData = list(rawData)
		if not isinstance(rawData, list): #TODO PERFORMANCE Isn't numpy.array better than a list
...
class PlotLine:
	...
	def __init__(self, ax, logFile, xColumn, yColumn, xerr, yerr, **kwargs):
		...
		self.xcol = Column(xColumn.memName, xColumn.nameType, xColumn.preferedUnit, xColumn.rawData, xColumn.specialTexMathName, xColumn.specialTexTextName, xColumn.rawDelta)#We need a deep copy of xColumn, xColumn.rawData and xColumn.rawDelta, because we want to remove all nan's from xColumn.rawData
		self.ycol = Column(yColumn.memName, yColumn.nameType, yColumn.preferedUnit, yColumn.rawData, yColumn.specialTexMathName, yColumn.specialTexTextName, yColumn.rawDelta)#Aus mir unerklärlichen Gründen ist das eine deep Copy von yColumn.data, also self.ycol.yColumn[0] = 123 verändern nicht yColumn
		...
		if self.xcol.unit() != 1:
			logFile.write("ax.set_xlabel(\"" + self.xcol.textName() + " [$" + anythingExceptNumbersToTex(self.xcol.unit()) + "$]" + "\", usetex="+str(not _NO_TEX_PLOT_)+")\n")
			self.ax.set_xlabel(self.xcol.textName() + " [$" + anythingExceptNumbersToTex(self.xcol.unit()) + "$]", usetex=(not _NO_TEX_PLOT_))
		else:
			logFile.write("ax.set_xlabel(\"" + self.xcol.textName() + "\", usetex="+str(not _NO_TEX_PLOT_)+")\n")
			self.ax.set_xlabel(self.xcol.textName() , usetex=(not _NO_TEX_PLOT_))
		if self.ycol.unit() != 1:
			logFile.write("ax.set_ylabel(\"" + self.ycol.textName() + " [$" + anythingExceptNumbersToTex(self.ycol.unit()) + "$]" + "\", usetex="+str(not _NO_TEX_PLOT_)+")\n")
			self.ax.set_ylabel(self.ycol.textName() + " [$" + anythingExceptNumbersToTex(self.ycol.unit()) + "$]", usetex=(not _NO_TEX_PLOT_)) #, fontsize=40
		else:
			logFile.write("ax.set_ylabel(\"" + self.ycol.textName() + "\", usetex="+str(not _NO_TEX_PLOT_)+")\n")
			self.ax.set_ylabel(self.ycol.textName() , usetex=(not _NO_TEX_PLOT_))
...
def readFile(filepath):
	...
	for i in range(0,len(matrix[0])):
		...
		ret.add(Column(memNames[i],nameType[i],units[i],dat,rawDelta=rawDelta))
	if PROFILE:
...
def calcColumn(table, equation, resultMemName=None, resultNameType="eq", resultMathName=None, resultTextName=None, examplePath=None):
	...
	eqcp = equation
	for a in table:
		eqcp = eqcp.subs(table[a].memName, table[a].preferedUnit)
	unit = checkGetUnit(eqcp)
	ret = Column(resultMemName, resultNameType, unit, [], specialTexMathName=resultMathName, specialTexTextName=resultTextName)
	exampleRow = length-1
...
def calcNfft(tCol,yCol,memName, memNamef): #TODO check wheter tCol has equal steps
	...
	fty = Column(memName, "eq", tCol.preferedUnit*yCol.preferedUnit, np.abs(ft), rawDelta=None)
	ftx = Column(memNamef, "eq", 1/tCol.preferedUnit, np.arange(0,len(ft))/np.array(tCol.rawData[-1]), rawDelta=None)
	return [fty, ftx]
