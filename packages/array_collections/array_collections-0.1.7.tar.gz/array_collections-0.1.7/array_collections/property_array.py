...
from free_properties import FreeProperty
from free_properties.free_property import inplace_magic_names
...
def has_property(array):
    return (0 in array.shape or 
            isinstance(ndarray.__getitem__(array, (0,)*array.ndim),
                       FreeProperty))
...
class property_array(ndarray):
    ...
    **Examples**
    ...
    def __getitem__(self, key):
        ...
        if isinstance(value, FreeProperty):
            value = value.value
        return value
    ...
    def __setitem__(self, key, value):
        if isinstance(key, (int, integer)):
            ...
            value = np.broadcast_to(value, items.shape)
            for i, v in zip(items.view(ndarray).flatten(), value.flatten()):
                i.value = v 
    
    def __array_wrap__(self, result):
        if self is not result:
            result.__class__ = array
        return result
    ...
    def __repr__(self):
        ...
        if not has_property(view):
            self.__class__ = array
            return repr(self)
        elif len(self) == 0:
            return array.__repr__(self)
        else:
            return array.__repr__(self.astype(type(self.item(0).value)))

def wrap_ifunc(iname):
    name = iname[:2] + iname[3:] # Remove 'i'
    if hasattr(property_array, name):
        ifunc = getattr(property_array, iname)
        func = getattr(property_array, name)
        def wrapper(self, *args, **kwargs):
            self[:] = func(self, *args, **kwargs)
            return self
        wrapper.__name__ = iname
        wrapper.__doc__ = ifunc.__name__
        return wrapper
