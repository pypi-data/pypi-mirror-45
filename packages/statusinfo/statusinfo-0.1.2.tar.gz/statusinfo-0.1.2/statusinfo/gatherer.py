...
class Gatherer:
    ...
    @staticmethod
    @gather(ICON_MEMORY)
    def gather_memory(args):
        filename = '/proc/meminfo'

        with open(filename) as memu_file:
            meminfo = memu_file.read()
    ...
    def gather_synchronization(args):
        path = '~/.local/share'
        path = Path(path)
        path = path.expanduser()
