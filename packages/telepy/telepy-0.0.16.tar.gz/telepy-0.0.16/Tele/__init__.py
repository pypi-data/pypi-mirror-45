...
def get_updates(offset=None, limit=None,
    ...
    try:
        return _call_api('getUpdates', params=params)['result']
    except KeyError:
...
def send_media_group(chat_id, media, disable_notification=None,
    ...
    return _call_api('sendMediaGroup', json=_remove_none(params))
...
def edit_message_media(chat_id=None, message_id=None, inline_message_id=None,
    ...
    return _call_api('editMessageMedia', params=_remove_none(params))
...
def stop_poll(chat_id=None, message_id=None, reply_markup=None):
    params = {
        'chat_id': chat_id,
        'message_id': message_id,
        'reply_markup': reply_markup,
...
def _num_ln(keys, num):
    ...
    for d in range(num_of_lines):
        lst1 = []
        for e in range(cor, num + cor):
            lst1.append(keys[e])
        lst.append(lst1)
        cor += num
    lst.append(keys[len_k - (len_k % num): len_k])
    return lst
...
def message_reply(update, text=None, file=None, photo=None,
    ...
    return _call_api('sendMessage', json=_remove_none(params))
...
def _call_api(function, **kwargs):
    """Make api call"""
    try:
        response = requests.post(_API + function, **kwargs).json()
        if not response['ok']:
            _error(response)
        return response
    except Exception as exc:
...
def bot(filters=None, not_on=None, chat_id=None,
        chat_type=None, from_id=None, on_error=None):
    """
    :param filters: (:obj:`str`, :obj:`tuple`, optional)
     message object 'https://core.telegram.org/bots/api#message'
     or https://core.telegram.org/bots/api#callbackquery etc..
    :param not_on: (:obj:`str`, :obj:`tuple`, optional)
     negative message object 'https://core.telegram.org/bots/api#message'
     or https://core.telegram.org/bots/api#callbackquery etc..
    ...
    def decorator(func):
        if on_error:
            ...
            _funcs_list[func] = {'filter': filters,
                                 'not_on': not_on,
                                 'chat_id': chat_id,
                                 'chat_type': chat_type,
                                 'from_id': from_id,
...
def _filtering(up):
    def _filter_from_id(f):
        if 'edited_message' in up:
            if f['from_id'] != up['edited_message']['from']['id']:
                return False
        elif f['from_id'] != up['from']['id']:
            return False
        return True

    def _filter_chat_id(f):
        if 'message' in up:
            if f['chat_id'] != up['message']['chat']['id']:
                return False
        elif 'edited_message' in up:
            if f['chat_id'] != up['edited_message']['chat']['id']:
                return False
        elif f['chat_id'] != up['chat']['id']:
            return False
        return True

    def _filter_chat_type(f):
        if 'message' in up:
            if f['chat_type'] != up['message']['chat']['type']:
                return False
        elif 'edited_message' in up:
            if f['chat_type'] != up['message']['chat']['type']:
                return False
        elif f['chat_type'] != up['chat']['type']:
            return False
        return True

    def _filter_not_on(f):
    ...
    for func in _funcs_list:
        ...
        if 'poll' not in up:
            if f['from_id']:
                if not _filter_from_id(f):
                    continue

            if f['chat_id']:
                if not _filter_chat_id(f):
                    continue

            if f['chat_type']:
                if not _filter_chat_type(f):
                    continue

        if f['not_on']:
            if not _filter_not_on(f):
                continue
...
def bot_run(offset_=None, multi=None, timeout=10):
    ...
    if offset_:
        try:
            get_updates(offset=get_updates(-1)[-1]['update_id'] - 1)
        except IndexError:
            pass
    offset = None
    ...
    if multi:
        while True:
            updates = getUpdates(timeout=timeout, offset=offset)
            if updates:
                for up in updates:
                    if 'message' in up:
                        up = up['message']
                    elif 'callback_query' in up:
                        up = up['callback_query']
...
"""
    ...
    :param disable_web_page_preview: (:obj:`bool`, optional)  Disables link previews 
     for links in this message
    :param disable_notification: (:obj:`bool`, optional)  Sends the message silently. 
     Users will receive a notification with no sound.
    :param reply_to_message_id: (:obj:`int`, optional)  If the message is a reply, 
     ID of the original message
    :param reply_markup: (:obj:`InlineKeyboardMarkup`, :obj:`ReplyKeyboardMarkup`, 
     :obj:`ReplyKeyboardRemove`, :obj:`ForceReply`, optional)  Additional interface options. 
...
"""
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param from_chat_id: (:obj:`int`) Unique identifier for the chat where the original 
     message was sent (or channel username in the format @channelusername)
    :param message_id: (:obj:`int`) Message identifier in the chat specified in from_chat_id

    :param disable_notification: (:obj:`bool`, optional)  Sends the message silently. 
     Users will receive a notification with no sound. 
    :param update: (:obj:`update`, optional) getUpdates results. instead of entering 
     from_chat_id and message_id 
"""
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) Photo to send. Pass a file_id as String to send a photo 
    ...
    :param caption: (:obj:`str`, optional)  Photo caption (may also be used when 
     resending photos by file_id), 0-1024 characters
    :param reply_to_message_id: (:obj:`int`, optional)  If the message is a reply, 
     ID of the original message 
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) Audio file to send. Pass a file_id as String to send 
     an audio file that exists on the Telegram servers (recommended), pass an HTTP URL 
     as a String for Telegram to get an audio file from the Internet, or upload a new 
...
"""
    ...
    :param caption: (:obj:`str`, optional) Document caption (may also be used when 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) path file | file_id | url. Video to send.Pass a file_id 
     as String to send a video that exists on the Telegram servers (recommended), pass 
     an HTTP URL as a String for Telegram to get a video from the Internet, or upload 
    ...
    :param thumb: (:obj:`str`, optional) Thumbnail of the file sent. path file | 
     file_id | url 
    :param caption: (:obj:`str`, optional) Video caption (may also be used when resending 
     videos by file_id), 0-1024 characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
    :param supports_streaming: (:obj:`bool`, optional) Pass True, if the uploaded 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) Animation to send. Pass a file_id as String to send 
     an animation that exists on the Telegram servers (recommended), pass an HTTP URL 
     as a String for Telegram to get an animation from the Internet, or upload a new animation 
     using multipart/form-data
    :param duration: (:obj:`int`, optional)  Duration of sent animation in seconds

    :param width: (:obj:`int`, optional) Animation width
    :param height: (:obj:`int`, optional) Animation height
    :param thumb: (:obj:`str`, optional) Thumbnail of the file sent. path file | 
     file_id | url 
    :param caption: (:obj:`str`, optional) Animation caption (may also be used when 
...
"""
    ...
    :param caption: (:obj:`str`, optional) Voice message caption, 0-1024 characters

    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
    :param duration: (:obj:`int`, optional) Duration of the voice message in seconds
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) Video note to send. Pass a file_id as String to send 
     a video note that exists on the Telegram servers (recommended) or upload a new video 
     using multipart/form-data
    :param duration: (:obj:`str`, optional) Duration of sent video in seconds
    :param length: (:obj:`int`, optional) Video width and height, i.e. diameter of 
     the video message
    :param thumb: (:obj:`str`, optional) Thumbnail of the file sent. path file | 
     file_id | url 
    :param disable_notification: (:obj:`bool`, optional)  Sends the message silently. 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param media: (:obj:`Array of InputMediaPhoto` :obj:`InputMediaVideo`) A JSON-serialized 
...
"""
    ...
    :param live_period: (:obj:`int`, optional) Period in seconds for which the location 
...
"""
    ...
    :param inline_message_id: (:obj:`str`, optional) Required if chat_id and message_id 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param message_id: (:obj:`int`) Required if inline_message_id is not specified. 
...
"""
    ...
    :param foursquare_id: (:obj:`str`, optional) Foursquare identifier of the venue

    :param foursquare_type: (:obj:`str`, optional) Foursquare type of the venue, 
     if known. (For example, “arts_entertainment/default”, “arts_entertainment/aquarium” 
     or “food/icecream”.)
    :param disable_notification: (:obj:`bool`, optional)  Sends the message silently.      
    :param reply_to_message_id: (:obj:`int`, optional)  If the message is a reply, 
     ID of the original message 
...
"""
    ...
    :param vcard: (:obj:`str`, optional) Additional data about the contact in the 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param action: (:obj:`str`) Type of action to broadcast. Choose one, depending 
     on what the user is about to receive: typing for text messages, upload_photo for 
     photos, record_video or upload_video for videos, record_audio or upload_audio for 
...
"""
    ...
    :param options: (:obj:`Array of String`) List of answer options, 2-10 strings 1-100 characters each
    :param disable_notification: (:obj:`bool`, optional)  Sends the message silently. 
     Users will receive a notification with no sound. 
    :param reply_to_message_id: (:obj:`int`, optional)  If the message is a reply, 
     ID of the original message 
...
"""
    ...
    :param offset: (:obj:`int`, optional) Sequential number of the first photo to 
     be returned. By default, all photos are returned.
    :param limit: (:obj:`int`, optional) Limits the number of photos to be retrieved. 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     ...
     username of the target channel (in the format @channelusername) 
    :param user_id: (:obj:`int`)  Unique identifier of the target user 
    :param until_date: (:obj:`int`, optional) Date when the user will be unbanned, 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     ...
     username of the target channel (in the format @channelusername) 
    :param user_id: (:obj:`int`)  Unique identifier of the target user 
    :param until_date: (:obj:`int`, optional) Date when restrictions will be lifted 
     for the user, unix time. If user is restricted for more than 366 days or less than 
     30 seconds from the current time, they are considered to be restricted forever
    :param can_send_messages: (:obj:`bool`, optional) Pass True, if the user can 
     send text messages, contacts, locations and venues
    :param can_send_media_messages: (:obj:`bool`, optional) Pass True, if the user 
     can send audios, documents, photos, videos, video notes and voice notes, implies 
     can_send_messages
    :param can_send_other_messages: (:obj:`bool`, optional) Pass True, if the user 
     can send animations, games, stickers and use inline bots, implies can_send_media_messages

    :param can_add_web_page_previews: (:obj:`bool`, optional)  Pass True, if the 
     user may add web page previews to their messages, implies can_send_media_messages
...
"""
    ...
    :param chat_id: (:obj:`int`)
     ...
     username of the target channel (in the format @channelusername) 
    :param user_id: (:obj:`int`)  Unique identifier of the target user 
    :param can_change_info: (:obj:`bool`, optional) Pass True, if the administrator 
     can change chat title, photo and other settings
    :param can_post_messages: (:obj:`bool`, optional) Pass True, if the administrator 
     can create channel posts, channels only 
    :param can_edit_messages: (:obj:`bool`, optional) Pass True, if the administrator 
     can edit messages of other users and can pin messages, channels only
    :param can_delete_messages: (:obj:`bool`, optional) Pass True, if the administrator      
    :param can_invite_users: (:obj:`bool`, optional) Pass True, if the administrator 
     can invite new users to the chat
    :param can_restrict_members: (:obj:`bool`, optional) Pass True, if the administrator 
     can restrict, ban or unban chat members
    :param can_pin_messages: (:obj:`bool`, optional) Pass True, if the administrator 
     can pin messages, supergroups only
    :param can_promote_members: (:obj:`bool`, optional) Pass True, if the administrator 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param photo: (:obj:`str`) path file | file_id | url .New chat photo, uploaded 
     using multipart/form-data
"""
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param description: (:obj:`str`, optional) New chat description, 0-255 characters
...
"""
    ...
    :param text: (:obj:`str`, optional) Text of the notification. If not specified, 
     nothing will be shown to the user, 0-200 characters 
    :param show_alert: (:obj:`bool`, optional) If true, an alert will be shown by 
     the client instead of a notification at the top of the chat screen. Defaults to false.
    :param url: (:obj:`str`, optional) URL that will be opened by the user's client. 
     If you have created a Game and accepted the conditions via @Botfather, specify the 
     URL that opens your game – note that this will only work if the query comes from 
     a callback_game button. Otherwise, you may use links like t.me/your_bot?start=XXXX 
     that open your bot with a parameter.
    :param cache_time: (:obj:`int`, optional) The maximum amount of time in seconds 
     that the result of the callback query may be cached client-side. Telegram apps will 
...
"""
    ...
    :param inline_message_id: (:obj:`str`, optional) Required if chat_id and message_id 
     are not specified. Identifier of the inline message
    :param media: (:obj:`InputMedia`) A JSON-serialized object for a new media content 
     of the message
    :param reply_markup: (:obj:`InlineKeyboardMarkup`, :obj:`ReplyKeyboardMarkup`, 
     :obj:`ReplyKeyboardRemove`, :obj:`ForceReply`, optional)  Additional interface options. 
     A JSON-serialized object for an inline keyboard, custom reply keyboard, instructions 
...
"""
    ...
    :param chat_id: (:obj:`int`)
     Unique identifier for the target chat or
     username of the target channel (in the format @channelusername) 
    :param file: (:obj:`str`) Sticker to send. Pass a file_id as String to send a 
     file that exists on the Telegram servers (recommended), pass an HTTP URL as a String 
     for Telegram to get a .webp file from the Internet, or upload a new one using multipart/form-data. 
...
"""
    ...
    :param user_id: (:obj:`int`)  Unique identifier of the target user 
    :param file: (:obj:`str`) Png image with the sticker, must be up to 512 kilobytes 
     in size, dimensions must not exceed 512px, and either width or height must be exactly 
     512px. 
"""
...
"""
    ...
    :param user_id: (:obj:`int`)  Unique identifier of the target user 
    :param name: (:obj:`str`) Short name of sticker set, to be used in t.me/addstickers/ 
     URLs (e.g., animals). Can contain only english letters, digits and underscores. Must 
     begin with a letter, can't contain consecutive underscores and must end in “_by_<bot 
     username>”. <bot_username> is case insensitive. 1-64 characters.
    :param title: (:obj:`str`) Sticker set title, 1-64 characters
    :param file: (:obj:`str`) Png image with the sticker, must be up to 512 kilobytes 
     in size, dimensions must not exceed 512px, and either width or height must be exactly 
     512px. Pass a file_id as a String to send a file that already exists on the Telegram 
     servers, pass an HTTP URL as a String for Telegram to get a file from the Internet, 
    ...
    :param contains_masks: (:obj:`bool`, optional) Pass True, if a set of mask stickers 
     should be created
    :param mask_position: (:obj:`MaskPosition`, optional) A JSON-serialized object 
     for position where the mask should be placed on faces
"""
...
"""
    ...
    :param update: (:obj:`update`, optional) getUpdates results. instead of entering 
     from_chat_id and message_id 
    :param results: (:obj:`InlineQueryResult`) A JSON-serialized array of results 
     for the inline query
    :param cache_time: (:obj:`int`, optional) The maximum amount of time in seconds 
     that the result of the inline query may be cached on the server. Defaults to 300.

    :param is_personal: (:obj:`bool`, optional) Pass True, if results may be cached 
     on the server side only for the user that sent the query. By default, results may 
     be returned to any user who sends the same query
    :param next_offset: (:obj:`str`, optional) Pass the offset that a client should 
     send in the next query with the same text to receive more results. Pass an empty 
     string if there are no more results or if you don‘t support pagination. Offset length 
     can’t exceed 64 bytes.
    :param switch_pm_text: (:obj:`str`, optional) If passed, clients will display 
     a button with specified text that switches the user to a private chat with the bot 
     and sends the bot a start message with the parameter switch_pm_parameter
    :param switch_pm_parameter: (:obj:`str`, optional) Deep-linking parameter for 
     the /start message sent to the bot when user presses the switch button. 1-64 characters, 
     only A-Z, a-z, 0-9, _ and - are allowed. Example: An inline bot that sends YouTube 
     videos can ask the user to connect the bot to their YouTube account to adapt search 
     results accordingly. To do this, it displays a ‘Connect your YouTube account’ button 
     above the results, or even before showing any. The user presses the button, switches 
     to a private chat with the bot and, in doing so, passes a start parameter that instructs 
     the bot to return an oauth link. Once done, the bot can offer a switch_inline button 
     so that the user can easily return to the chat where they wanted to use the bot's 
     inline capabilities.
     InlineQueryResult
"""
...
"""
    ...
    :param input_message_content: (:obj:`input_message_content`)  Unique identifier 
     for this result, 1-64 Bytes
    :param reply_markup: (:obj:`InlineKeyboardMarkup`, :obj:`ReplyKeyboardMarkup`, 
     :obj:`ReplyKeyboardRemove`, :obj:`ForceReply`, optional)  Additional interface options. 
     A JSON-serialized object for an inline keyboard, custom reply keyboard, instructions 
     to remove reply keyboard or to force a reply from the user. 
    :param url: (:obj:`str`) Optional. URL of the result
    :param hide_url: (:obj:`bool`) Optional. Pass True, if you don't want the URL 
     to be shown in the message
    :param description: (:obj:`str`) Optional. Short description of the result
    :param thumb_url: (:obj:`str`) Optional. Url of the thumbnail for the result
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param photo_url: (:obj:`str`) A valid URL of the photo. Photo must be in jpeg 
    ...
    :param description: (:obj:`str`) Optional. Short description of the result
    :param caption: (:obj:`str`)  Optional. Caption of the photo to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
    :param reply_markup: (:obj:`InlineKeyboardMarkup`, :obj:`ReplyKeyboardMarkup`, 
     ...
     A JSON-serialized object for an inline keyboard, custom reply keyboard, instructions 
     to remove reply keyboard or to force a reply from the user. 
    :param input_message_content: (:obj:`input_message_content`, optional)  Unique 
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param gif_url: (:obj:`str`) A valid URL for the GIF file. File size must not 
     exceed 1MB
    ...
    :param thumb_url: (:obj:`str`) URL of the static thumbnail for the result (jpeg 
     or gif)
    :param title: (:obj:`str`) Optional. Title for the result
    :param caption: (:obj:`str`) Optional. Caption of the GIF file to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param mpeg4_url: (:obj:`str`) A valid URL for the MP4 file. File size must not 
     exceed 1MB
    ...
    :param thumb_url: (:obj:`str`) URL of the static thumbnail (jpeg or gif) for 
     the result
    :param title: (:obj:`str`) Optional. Title for the result
    :param caption: (:obj:`str`)  Optional. Caption of the MPEG-4 file to be sent, 
     0-1024 characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param video_url: (:obj:`str`)	A valid URL for the embedded video player or video 
     file
    :param mime_type: (:obj:`str`) Mime type of the content of video url, “text/html” 
     or “video/mp4”
    :param thumb_url: (:obj:`str`) 	URL of the thumbnail (jpeg only) for the video

    :param title: (:obj:`str`) Title for the result
    :param caption: (:obj:`str`) Optional. Caption of the video to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param title: (:obj:`str`) Title for the result
    :param caption: (:obj:`str`) Optional. Caption of the document to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
    :param document_url: (:obj:`str`) A valid URL for the file
    :param mime_type: (:obj:`str`) Mime type of the content of the file, either “application/pdf” 
     or “application/zip”
    :param description: (:obj:`str`) Optional. Short description of the result
    :param reply_markup: (:obj:`InlineKeyboardMarkup`, :obj:`ReplyKeyboardMarkup`, 
     :obj:`ReplyKeyboardRemove`, :obj:`ForceReply`, optional)  Additional interface options. 
    ...
    :param input_message_content: (:obj:`input_message_content`, optional)  Unique 
     identifier for this result, 1-64 Bytes
    :param thumb_url: (:obj:`str`) Optional. URL of the thumbnail (jpeg only) for 
     the file
    :param thumb_width: (:obj:`int`) Optional. Thumbnail width
    :param thumb_height: (:obj:`int`) Optional. Thumbnail height
"""
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param video_file_id: (:obj:`str`) A valid file identifier for the video file
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param voice_file_id: (:obj:`str`) A valid file identifier for the voice message
...
"""
    ...
    :param id: (:obj:`str`)  Unique identifier for this result, 1-64 Bytes
    :param audio_file_id: (:obj:`str`) A valid file identifier for the audio file
...
"""
    ...
    :param message_text: (:obj:`str`) Text of the message to be sent, 1-4096 characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
    :param disable_web_page_preview : (:obj:`bool`)  Optional. Disables link previews 
     for links in the sent message 
"""
...
"""
    ...
    :param result_id: (:obj:`str`) The unique identifier for the result that was 
     chosen
    :param from_user:  (:obj:`User`) The user that chose the result
    :param location: (:obj:`Location`) Optional. Sender location, only for bots that 
     require user location
    :param inline_message_id: (:obj:`str`) Optional. Identifier of the sent inline 
     message. Available only if there is an inline keyboard attached to the message. Will 
     be also received in callback queries and can be used to edit the message.
    :param query: (:obj:`str`) The query that was used to obtain the result
"""
InputMediaPhoto = input_media_photo
"""

    :param media: (:obj:`str`) File to send. Pass a file_id to send a file that exists 
     on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file 
...
"""
    ...
    :param thumb: (:obj:`str`, :obj:`InputFile`) Optional. Thumbnail of the file 
     sent; can be ignored if thumbnail generation for the file is supported server-side. 
     The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail‘s 
     width and height should not exceed 90. Ignored if the file is not uploaded using 
...
"""
    ...
    :param thumb: (:obj:`str`, :obj:`InputFile`) Optional. Thumbnail of the file 
     ...
     file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded 
     using multipart/form-data under <file_attach_name>.
    :param caption: (:obj:`str`) Optional. Caption of the animation to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
...
"""
    ...
    :param thumb: (:obj:`str`, :obj:`InputFile`) Optional. Thumbnail of the file 
     ...
     file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded 
     using multipart/form-data under <file_attach_name>.
    :param caption: (:obj:`str`) Optional. Caption of the audio to be sent, 0-1024 
     characters
    :param parse_mode: (:obj:`str`, optional)  Send Markdown or HTML, if you want 
     Telegram apps to show bold, italic, fixed-width text or inline URLs in your bot's 
     message. 
...
"""
    ...
    :param k: Array of KeyboardButton see examples here:
     https://gitlab.com/2411eliko/tele/blob/master/Examples/keyboard_and_inlinekeyboard.py 
