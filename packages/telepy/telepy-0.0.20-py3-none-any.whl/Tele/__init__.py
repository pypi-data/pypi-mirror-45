...
def get_updates(offset=None, limit=None,
    ...
    try:
        return _call_api('getUpdates', params=_remove_none(params))['result']
    except KeyError:
...
def bot(filters=None, not_on=None, command=None,
        chat_id=None, chat_type=None, from_id=None,
        regex=None, on_error=None):
    """
    ...
    def decorator(func):
        if on_error:
            ...
            _funcs_list[func] = dict(filters=filters, not_on=not_on,
                                     command=command, chat_id=chat_id,
                                     chat_type=chat_type, from_id=from_id,
                                     regex=regex, on_error=on_error
                                     )
...
def _filter_from_id(fil, from_id):
    if type(fil) in (list, tuple):
        for f in fil:
            if f == from_id:
                return True
...
def _filter_chat_id(fil, chat_id):
    if type(fil) in (list, tuple):
        for f in fil:
            if f == chat_id:
                return True
...
def _filter_chat_type(fil, chat_type):
    if type(fil) in (list, tuple):
        for f in fil:
            if f == chat_type:
                return True
    ...
    for func in _funcs_list:
        ...
        if 'from' in up:
            if f['from_id']:
                ...
                if not _filter_from_id(f['from_id'], up['from']['id']):
                    continue

            if 'chat' in up:
                if f['chat_id']:
                    if not _filter_chat_id(f['chat_id'], up['chat']['id']):
                        continue

                if f['chat_type']:
                    if not _filter_chat_type(f['chat_type'], up['chat']['type']):
                        continue
        if f['not_on']:
            if _filter_key(f['not_on'], filter_keys):
                continue

        if f['filters']:
            if not _filter_key(f['filters'], filter_keys):
                continue
...
def bot_run(offset_=None, multi=None, limit=None,
    ...
    if offset_:
        try:
            ...
            get_updates(offset=get_updates(offset=-1,
                                           timeout=0)[-1]['update_id']+1)
        except IndexError:
            pass
    ...
    if multi:
        while True:
            updates = getUpdates(offset=offset, limit=limit, timeout=timeout,
