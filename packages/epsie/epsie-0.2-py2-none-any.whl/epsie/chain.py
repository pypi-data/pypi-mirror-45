...
class Chain(object):
    ...
    def __init__(self, parameters, model, proposals, betas=1., swap_interval=1,
        ...
        # combine the proposals into a joint proposal, one for each temperature
        self.proposal_dists = []
        for tk in range(len(self.betas)):
            tprops = []
            for p in proposals:
                p = copy.copy(p)
                p.temperature_index = tk
                tprops.append(p)
            self.proposal_dists.append(JointProposal(*tprops, brng=brng))
        self._iteration = 0
        self._lastclear = 0
        self._scratchlen = 0
    ...
    def brng(self):
        ...
        return self.proposal_dists[0].brng
    ...
    def random_state(self):
        ...
        return self.proposal_dists[0].random_state
    ...
    def state(self):
        ...
        state['proposal_dist'] = self.proposal_dists[0].state
    ...
    def set_state(self, state):
        ...
        for ii, p in enumerate(self.proposal_dists):
            self.proposal_dists[ii].set_state(state['proposal_dist'])
    ...
    def step(self):
        ...
        # of the chain:
        [p.update(self) for p in self.proposal_dists]
    ...
    def _singletemp_step(self, current_pos, current_stats, current_blob,
                         tidx):
        """Steps a single temperature."""
        beta = self.betas[tidx]
        proposal_dist = self.proposal_dists[tidx]
        proposal = proposal_dist.jump(current_pos)
        ...
        if logp == -numpy.inf:
            ...
            if not proposal_dist.symmetric:
                logar += proposal_dist.logpdf(current_pos, proposal) - \
                         proposal_dist.logpdf(proposal, current_pos)
            ar = numpy.exp(logar)
