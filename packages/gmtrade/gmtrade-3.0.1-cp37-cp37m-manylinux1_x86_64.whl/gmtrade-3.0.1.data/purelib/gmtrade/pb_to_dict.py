...
from gmtrade.utils import DictLikeObject, utc_datetime2beijing_datetime
...
def protobuf_to_dict(pb, type_callable_map=TYPE_CALLABLE_MAP, use_enum_labels=False,
                     including_default_value_fields=True, is_utc_time=True, dcls=None):
    if dcls is None:
        dcls = DictLikeObject
    ...
    for field, value in pb.ListFields():
        ...
        if isinstance(value, Timestamp):
            ...
            if is_utc_time:
                result_dict[field.name] = utc_datetime2beijing_datetime(result_dict[field.name])
    ...
    if including_default_value_fields:
        for field in pb.DESCRIPTOR.fields:
            ...
            if ((field.label != FieldDescriptor.LABEL_REPEATED and
                 field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE) or field.containing_oneof):
                # 处理google.protobuf.Timestamp的类型在缺少值的情况
                if field.message_type.full_name == 'google.protobuf.Timestamp' and field.name not in result_dict:
            ...
            if field.cpp_type != FieldDescriptor.CPPTYPE_MESSAGE and hasattr(field, 'default_value'):
