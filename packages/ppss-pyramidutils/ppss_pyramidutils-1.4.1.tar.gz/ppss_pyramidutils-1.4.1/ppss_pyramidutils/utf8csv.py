...
class UnicodeWriter:
    ...
    def writerow(self, row):
        try:
            ...
            self.l.error("step:"+unicode(r)+ " " + e.message + repr(e)+ "--" + repr(row)    )
            pass
...
class Importer(object):
    ...
    def __init__(self, fn, mapping=None, delimiter=",", headertransform=None):
        ...
        if PY2:
            ...
            self.csvreader = csv.reader(open(fn,'rb'), dialect=csv.excel,delimiter=delimiter)
        #self.csvreader = csv.reader(f,delimiter=";")
        h = self.csvreader.next()
        if headertransform:
        ...
        for row in self.csvreader:
            try:
                thisrow = {}
                for key,val in self.titlepos.iteritems():
                    if mapping :
                        l.debug(mapping)
