...
class TestExportFileMigration(AiidaTestCase):
    ...
    def setUp(self):
        ...
        self.node_count = 25
        self.struct_count = 2
        self.known_struct_label = ''
        self.known_cell = [[4, 0, 0], [0, 4, 0], [0, 0, 4]]
        self.known_kinds = [
            {
                'name': 'Ba',
                'mass': 137.327,
    ...
    def test_no_node_export(self, temp_dir):
        ...
        self.assertEqual(orm.QueryBuilder().append(orm.Node).count(),
                         node_count)
        self.assertEqual(orm.QueryBuilder().append(orm.Computer).count(),
                         computer_count)

        # Check unique identifiers
        computers = orm.QueryBuilder().append(
            orm.Computer, project=['uuid']).all()[0][0]
        users = orm.QueryBuilder().append(
            orm.User, project=['email']).all()[0][0]
    ...
    def test_wrong_versions(self):
        ...
        wrong_versions.append(str(float(newest_version) + 0.1))
    ...
    def test_migrate_newest_version(self):
        ...
        with self.assertRaises(SystemExit) as exception:
            ...
            self.assertIn(
                'Critical: Your export file is already at the newest export version {}'
                .format(metadata['export_version']),
                exception.exception,
                msg=
                "Expected a critical statement that the export version '{}' is the newest export version '{}', "
                "instead got {}".format(metadata['export_version'],
                                        newest_version, exception.exception))
            self.assertIsNone(
                new_version,
                msg="migrate_recursively should not return anything, "
                "hence the 'return' should be None, but instead it is {}".
    ...
    def test_v02_to_newest(self, temp_dir):
        ...
        builder = orm.QueryBuilder().append(
            orm.StructureData, project=['attributes.kinds'])
    ...
    def test_v03_to_newest(self, temp_dir):
        ...
        input_file = get_archive_file('export_v0.3.aiida')
        output_file = os.path.join(temp_dir, 'output_file.aiida')
