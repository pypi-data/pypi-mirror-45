...
class TestMigrateV03toV04(AiidaTestCase):
    ...
    def test_migrate_v3_to_v4_complete(self):
        ...
        for change in new_node_attrs:
            ...
            for node in data['export_data']['Node'].values():
                self.assertIn(
                    change,
                    node,
                    msg="'{}' not found for {}".format(change, node))
            # metadata.json
            self.assertIn(
                change,
        ...
        for node in data['export_data']['Node'].values():
            self.assertIn(
                node['node_type'],
                legal_node_types,
                msg="{} is not a legal node_type. Legal node types: {}".format(
                    node['node_type'], legal_node_types))
            self.assertIn(
                node['process_type'],
                legal_process_types,
                msg="{} is not a legal process_type. Legal process types: {}".
                format(node['process_type'], legal_node_types))
        ...
        for link in illegal_links:
            self.assertNotIn(
                link,
                data['links_uuid'],
                msg="{} should not be in the migrated export file".format(
                    link))
        ...
        for attr in updated_attrs:
            ...
            for group in data['export_data']['Group'].values():
                self.assertIn(
                    attr,
                    group,
                    msg="{} not found in Group {}".format(attr, group))
                self.assertIn(
                    group['type_string'],
                    legal_group_type,
                    msg="{} is not a legal Group type_string".format(
                        group['type_string']))
            # metadata.json
            self.assertIn(
                attr,
                metadata['all_fields_info']['Group'],
                msg="{} not found in metadata.json".format(attr))
            ...
            for node_id in calcjob_nodes:
                for attr in mandatory_updated_calcjob_attrs:
                    self.assertIn(
                        ...
                        data[field][node_id],
                        msg=
                        "Updated attribute name '{}' not found in {} for node_id: {}"
                        .format(attr, field, node_id))
            ...
            for node_id in trajectorydata_nodes:
                for attr in updated_attrs:
                    self.assertIn(
                        ...
                        data[field][node_id],
                        msg=
                        "Updated attribute name '{}' not found in {} for TrajecteoryData node_id: {}"
                        .format(attr, field, node_id))
        ...
        for attr in removed_attrs:
            ...
            for computer in data['export_data']['Computer'].values():
                self.assertNotIn(
                    attr,
                    computer,
                    msg="'{}' should have been removed from Computer {}".
                    format(attr, computer['name']))
            # metadata.json
            self.assertNotIn(
                attr,
                metadata['all_fields_info']['Computer'],
                msg=
                "'{}' should have been removed from Computer in metadata.json".
                format(attr))
        ...
        for entity in new_entities:
            for field in fields:
                self.assertIn(
                    entity,
                    metadata[field],
                    msg="{} not found in {} in metadata.json".format(
                        entity, field))
        ...
        for field in new_fields:
            self.assertIn(
                field,
                list(data.keys()),
                msg="New field '{}' not found in data.json".format(field))
            self.assertEqual(
                len(data[field]),
                attrs_count,
                msg=
                "New field '{}' found to have only {} entries, but should have had {} entries"
                .format(field, len(data[field]), attrs_count))
    ...
    def test_illegal_create_links(self):
        ...
        with SandboxFolder(sandbox_in_repo=False) as folder:
            ...
            violations = []
            for link in data['links_uuid']:
                if link['input'] in workfunc_uuids and link[
                        'type'] == 'createlink':
            ...
            self.assertEqual(
                ...
                known_illegal_links,
                msg=
                "{} illegal create links were expected, instead {} was/were found"
                .format(known_illegal_links, len(violations)))
        ...
        self.assertEqual(
            len(violations),
            0,
            msg="0 illegal links were expected, instead {} was/were found".
