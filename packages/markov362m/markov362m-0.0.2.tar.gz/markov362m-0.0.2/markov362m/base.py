...
class MarkovChain(nx.DiGraph):
    ...
    def add_state(self, state, label=None, color=DEFAULT_COLOR,
        ...
        Args:
            ...
            other_arguments: Do not use (used for plotting, etc.)
        """
        ...
        self.add_node(state, label=label, position=position, **kwargs)
    ...
    def add_transition(self, state1, state2, probability = 1,
        ...
        args:
            ...
            other arguments: used for internals such as plotting. Do not use.
        """
    ...
    def set_name(self, state, name):
        ...
        Args:
            state1 (str): the initial state
        """
        self.relabel_nodes({state: name}, copy=False)
    ...
    def P_to_dict(self, vct):
        ...
        """
        return( { self.I_to_S[i] : np.float32(vct[i]) 
    ...
    def T_to_dict(self, vct):
        ...
        Returns:
            dict: a dictionary from states to values
        """
        return( { self.T_to_S[i] : np.float32(vct[i][0]) 
    ...
    def C_to_dict(self, vct):
        ...
        Returns:
            dict: a dictionary from states to values
        """
        return( { self.C_to_S[i] : np.float32(vct[i][0]) 
            for i in range(len(vct)) } )
