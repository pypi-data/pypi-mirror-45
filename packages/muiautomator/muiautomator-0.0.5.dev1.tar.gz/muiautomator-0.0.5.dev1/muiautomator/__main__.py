...
from muiautomator.version import __apk_version__, __atx_agent_version__

appdir = os.path.join(os.path.expanduser("~"), '.muiautomator')
logger.debug("use cache directory: %s", appdir)

GITHUB_BASEURL = "https://github.com/openatx"
MYGITHUB_BASEURL = "https://github.com/u1028498345"
...
class Initer():
    ...
    def atx_agent_url(self):
        ...
        if not name:
            raise Exception(
                ...
                % self.abis)
        return MYGITHUB_BASEURL + '/agent/releases/download/%s/%s' % (
    ...
    def server_url(self):
        files = {
            'armeabi-v7a': 'server_{v}_linux_armv7.tar.gz',
            'arm64-v8a': 'server_{v}_linux_armv7.tar.gz',
            'armeabi': 'server_{v}_linux_armv6.tar.gz',
            'x86': 'server_{v}_linux_386.tar.gz',
        }
        name = ""
        for abi in self.abis:
            name = files.get(abi)
    ...
    def sh_url(self):
        ...
        return ''.join([
            MYGITHUB_BASEURL,
            "/agent/releases/download/%s/atx.sh" % __atx_agent_version__
        ])
    ...
    def install(self, server_addr=None):
        # 安装 uiautomator.apk uiautomator-test.apk minicap minitouch server atx.sh
        ...
        # 安装server
        logger.info("Install server")
        path = self.push_url(self.server_url, tgz=True, extract_name="server")
        self.shell(path)
        # 上传atx.sh
        logger.info("push atx.sh")
        self.push_url(self.sh_url)
