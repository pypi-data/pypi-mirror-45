...
from muiautomator.exceptions import (NullPointerExceptionError,
                                     UiObjectNotFoundError)
from muiautomator.utils import Exists, U, check_alive, hooks_wrap, intersect
...
def _failprompt(fn):
    def _inner(self, *args, **kwargs):
        ...
        from muiautomator import messagebox
        try:
            return fn(self, *args, **kwargs)
...
class Session(object):
    ...
    def __repr__(self):
        if self._pid and self._pkg_name:
            return "<muiautomator.Session pid:%d pkgname:%s>" % (
