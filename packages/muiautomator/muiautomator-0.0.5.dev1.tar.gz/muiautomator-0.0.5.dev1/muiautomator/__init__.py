...
from muiautomator.version import __atx_agent_version__
from muiautomator import adbutils
import requests
...
from muiautomator.exceptions import (
    UiaError,
...
from muiautomator.session import (  # noqa: F401
...
def connect_usb(serial=None):
    ...
    if not d.agent_alive:
        ...
        warnings.warn("backend muiautomator is not alive, start again ...",
                      RuntimeWarning)
        d.reset_uiautomator()
    return d
...
class UIAutomatorServer(object):
    ...
    def __str__(self):
        return 'muiautomator object for %s:%d' % (self._host, self._port)
    ...
    def jsonrpc_retry_call(self, *args,
        ...
        try:
            ...
            warnings.warn(
                "muiautomator is not reponding, restart muiautomator automatically",
                RuntimeWarning,
                stacklevel=1)
            # for XiaoMi, want to recover muiautomator must start app:com.github.uiautomator
            ...
            if args[1] != 'dumpWindowHierarchy':  # args[1] method
                warnings.warn(
                    "muiautomator raise exception %s, and run code again" % e,
                    RuntimeWarning,
                    stacklevel=1)
            time.sleep(1)
            return self.jsonrpc_call(*args, **kwargs)
    ...
    def reset_uiautomator(self):
        ...
        # wait until muiautomator service working
        ...
        while time.time() < deadline:
            ...
            time.sleep(1)
        raise RuntimeError(
            "Uiautomator started failed. Find solutions in https://github.com/openatx/muiautomator/wiki/Common-issues"
        )
        ...
        if attr.startswith('ext_'):
            ...
            if plugin_name not in self.__plugins:
                if plugin_name == 'xpath':
                    import muiautomator.ext.xpath as xpath
                    xpath.init()
                else:
                    raise ValueError(
