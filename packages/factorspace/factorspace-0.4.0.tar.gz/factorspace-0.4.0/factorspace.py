...
#裁剪规则集
def slim_rules(rules_set,rule_weights,slim=0.):

    #削减规则
    rules_set=np.delete(rules_set,np.where(np.array(rule_weights)<=slim),axis=0).tolist()
    rule_weights=np.delete(rule_weights,np.where(np.array(rule_weights)<=slim)).tolist()
...
def factor_analy(train_data,train_label,none_value=-1,train_times=10000,used_factors=False,zero_decide_random=False,rule_weight=False,strict=False,delete_rule_set=[]):
        ...
        if last_factor==-1:
            ...
            for k in range(len(factor_classes)):
                ...
                if len(np.unique(label[factor_classes[k]]))==1:
                    ...
                    #构建推理规则集
                    if (rule in delete_rule_set)==False:
                        rules_set.append(rule.copy())
...
#自适应规则删除算法
def adapt_rules_delete(rules_set,delete_set,adapt_data,adapt_label,none_value=-1):
...
#自适应规则删除算法
def adapt_factor_analy_delete(train_data,train_label,test_data,test_label,rules_set=[],delete_set=[],train_batch=100,test_batch=100,replace=False,repeat=10000,slim=False,exit_train_acc=1,exit_test_acc=1.,delete_rules_set=False,accuracy_info=False,none_value=-1):
    ...
    for i in range(repeat):
        ...
        rules_set+=factor_analy(data,label,delete_rule_set=delete_set,none_value=none_value)
        rules_set,delete_set=adapt_rules_delete(rules_set,delete_set,ddata,dlabel,none_value=none_value)
