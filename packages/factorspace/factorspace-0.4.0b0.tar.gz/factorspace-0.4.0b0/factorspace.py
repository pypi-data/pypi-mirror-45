...
#数据去重
def uniqu_data(train_data,train_label):
...
#自适应规则算法
def adapt_rules(rules_set,delete_set,test_data,test_label,none_value=-1):

    #构建单因素删除集
    for i in range(len(test_data)):

        #定义临时变量
        np_data=np.asarray(test_data[i])    
        count=len(rules_set)
        delete_index=[]
...
#自适应规则算法
def adapt_factor_analy(train_data,train_label,test_data,test_label,rules_set=[],delete_set=[],train_batch=100,test_batch=100,replace=False,repeat=10000,exit_acc=1.,delete_rules_set=False,none_value=-1):
    ...
    for i in range(repeat):
        ...
        rules_set,delete_set=adapt_rules(rules_set,delete_set,tdata,tlabel,none_value=none_value)
        acc=accuracy(rules_set,tdata,tlabel,none_value=none_value)
