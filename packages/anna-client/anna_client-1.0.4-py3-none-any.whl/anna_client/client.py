...
class Client:
	...
	def query(self, params=None):
		endpoint = self.host + '/v1.0/get'
		params = self.validate(params)
		return self.handle_response(requests.get(endpoint, params=params, headers=self.get_headers()))

	def push(self, params):
		endpoint = self.host + '/v1.0/push'
	...
	def get_tasks(self, site):
		endpoint = self.host + '/v1.0/tasks'
		params = self.validate(site)
		return self.handle_response(requests.get(endpoint, params=params, headers=self.get_headers()))

	def update(self, jobs):
		endpoint = self.host + '/v1.0/update'
		if len(jobs) > 0:
			requests.post(endpoint, json=jobs, headers=self.get_headers())

	def request_job(self):
		endpoint = self.host + '/v1.0/job'
