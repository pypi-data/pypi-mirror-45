...
def upload(host, username, password, project_path, remote_path='.'):
    with SSHClient() as ssh:
        ssh.set_missing_host_key_policy(AutoAddPolicy())
        ssh.connect(hostname=host, username=username, password=password)
        with SFTPClient.from_transport(ssh.get_transport()) as sftp:
            _transferring(ssh, sftp, Path(project_path), remote_path)
    return 'Successful!'
...
def put():
    ...
    if len(params) == 4: host, username, password, remote_path = params
        ...
        remote_path = '.'
    else: return 'Error: please check the command (propor <host> <username> <password> <remote_path=".">).'
