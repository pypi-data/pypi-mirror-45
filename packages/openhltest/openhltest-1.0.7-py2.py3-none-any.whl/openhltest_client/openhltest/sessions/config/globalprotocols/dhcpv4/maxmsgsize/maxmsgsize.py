...
class MaxMsgSize(Base):
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.maxmsgsize.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `max-msg-size` resource
