...
class OutstandingSessionCount(Base):
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.outstandingsessioncount.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `outstanding-session-count` resource
