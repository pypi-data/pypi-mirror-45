...
class Config(Base):
	...
	def PortControl(self, input):
		...
		Returns:
			({"detail": "string", "stack-trace": "string", "errata": {"stack-trace": "string", "name": "string", "detail": "string"}, "name": "string"})
	...
	def Load(self, input):
		...
		"""
		with open(input['file-name'], 'r') as fid:
			input['file-name'] = fid.read()
	...
	def Save(self, input):
		...
		with open(input['file-name'], 'w') as fid:
			fid.write(output['content'])
		return output
