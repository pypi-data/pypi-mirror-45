...
class DfBit(Base):
	...
	def __init__(self, parent):
		super(DfBit, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.dfbit.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `df-bit` resource
