...
class Identification(Base):
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv4.identification.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `identification` resource
