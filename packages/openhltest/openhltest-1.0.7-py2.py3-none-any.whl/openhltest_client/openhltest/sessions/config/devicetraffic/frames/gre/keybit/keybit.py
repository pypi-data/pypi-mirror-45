...
class KeyBit(Base):
	...
	def __init__(self, parent):
		super(KeyBit, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.gre.keybit.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `key-bit` resource
