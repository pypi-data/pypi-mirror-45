...
class Sequence_number(Base):
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.icmptimestamprequest.sequence_number.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `sequence_number` resource
