...
class EnableCustomOptionAssignForRelayAgents(Base):
	"""Enable custom option assignments on relay agent mode.
	"""
	YANG_NAME = 'enable-custom-option-assign-for-relay-agents'
	YANG_KEYWORD = 'container'
	YANG_KEY = None
	YANG_PROPERTY_MAP = {"Single": "single", "PatternType": "pattern-type", "PatternFormat": "pattern-format", "ValueList": "value-list"}

	def __init__(self, parent):
		super(EnableCustomOptionAssignForRelayAgents, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.globalprotocols.dhcpv4.enablecustomoptionassignforrelayagents.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `enable-custom-option-assign-for-relay-agents` resource
