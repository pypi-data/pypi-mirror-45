...
class DestinationAddress(Base):
	...
	def __init__(self, parent):
		super(DestinationAddress, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.ipv6.destinationaddress.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `destination-address` resource
