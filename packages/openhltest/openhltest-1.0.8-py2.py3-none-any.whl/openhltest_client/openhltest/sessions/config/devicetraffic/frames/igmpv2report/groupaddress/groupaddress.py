...
class GroupAddress(Base):
	...
	def __init__(self, parent):
		super(GroupAddress, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.groupaddress.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `group-address` resource
