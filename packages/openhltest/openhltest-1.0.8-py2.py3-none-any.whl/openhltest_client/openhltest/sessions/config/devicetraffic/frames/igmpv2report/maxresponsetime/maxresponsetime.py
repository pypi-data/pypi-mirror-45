...
class MaxResponseTime(Base):
	...
	def Increment(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.increment.increment.Increment)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.decrement.decrement.Decrement)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.random.random.Random)
		"""
		from openhltest_client.openhltest.sessions.config.devicetraffic.frames.igmpv2report.maxresponsetime.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `max-response-time` resource
