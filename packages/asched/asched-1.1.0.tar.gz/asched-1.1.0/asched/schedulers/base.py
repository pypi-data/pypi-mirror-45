...
from asched_src.asched import SchedulerAlreadyRunningError, SchedulerNotRunningError
from asched_src.asched.executors.base import MaxInstancesReachedError, BaseExecutor
from asched_src.asched.executors.pool import ThreadPoolExecutor
from asched_src.asched.jobstores import ConflictingIdError, JobLookupError, BaseJobStore
from asched_src.asched.jobstores import MemoryJobStore
from asched_src.asched import Job
from asched_src.asched.triggers import BaseTrigger
from asched_src.asched import (
    asbool, asint, astimezone, maybe_ref, timedelta_seconds, undefined, TIMEOUT_MAX)
from asched_src.asched.events import (
...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    :ivar int state: current running state of the scheduler (one of the following constants from
        ``asched.schedulers.base``: ``STATE_STOPPED``, ``STATE_RUNNING``, ``STATE_PAUSED``)

    .. seealso:: :ref:`scheduler-config`
    """

    _trigger_plugins = dict((ep.name, ep) for ep in iter_entry_points('asched.triggers'))
    _trigger_classes = {}
    _executor_plugins = dict((ep.name, ep) for ep in iter_entry_points('asched.executors'))
    _executor_classes = {}
    _jobstore_plugins = dict((ep.name, ep) for ep in iter_entry_points('asched.jobstores'))
    _jobstore_classes = {}
    ...
    def configure(self, gconfig={}, prefix='asched.', **options):
        """
    ...
    def add_executor(self, executor, alias='default', **executor_opts):
        ...
        :param str|unicode|asched.executors.base.BaseExecutor executor: either an executor
    ...
    def add_jobstore(self, jobstore, alias='default', **jobstore_opts):
        ...
        :param str|unicode|asched.jobstores.base.BaseJobStore jobstore: job store to be added
    ...
    def add_job(self, func, trigger=None, args=None, kwargs=None, id=None, name=None,
        ...
        :param str|asched.triggers.base.BaseTrigger trigger: trigger that determines when
            ``func`` is called
    ...
    def _configure(self, config):
        # Set general options
        self._logger = maybe_ref(config.pop('logger', None)) or getLogger('asched.scheduler')
