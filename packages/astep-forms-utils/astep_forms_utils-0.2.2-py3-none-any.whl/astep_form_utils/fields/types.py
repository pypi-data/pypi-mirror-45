...
from typing import Union, Any, Dict, Type, List
...
class FileField(Field):
    ...
    def validate(self, value: FileStorage) -> str:
        if isinstance(value, FileStorage):
            return value.stream.read().decode("UTF-8")
...
class EnumField(Field):
    ...
    def __init__(self, name: str, options: Type[Enum], label=None):
        """
...
class SubFieldsEnumField(EnumField, SubfieldsPromise):
    ...
    def __init__(
        ...
        :param label: label as shown in UI
        """
        super().__init__(name, options, label)
