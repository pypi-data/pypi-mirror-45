...
class SelectEdit(QObject, vmi.Menu, vmi.Mouse):
    ...
    def apply(self):
        if self._Image is not None:
            ...
            face = vmi.mkFace(self.curveShape.wire())
            look = self.view.camera('look')
    ...
    def rebuild(self):
        ...
        if len(self.nodes) > 2:
            if self.curveType == 0:
                ...
                self.curveShape.clone(vmi.mkWire(vmi.mkBSplineCurveK(pts=self.nodes, closed=True)))
        elif len(self.nodes) > 1:
            if self.curveType == 0:
                self.curveShape.clone(vmi.mkWire(vmi.mkSegments(pts=self.nodes, closed=False)))
            elif self.curveType == 1:
                self.curveShape.clone(vmi.mkWire(vmi.mkBSplineCurveK(pts=self.nodes, closed=False)))
        else:
