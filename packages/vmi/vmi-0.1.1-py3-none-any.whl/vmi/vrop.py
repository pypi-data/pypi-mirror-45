...
class PolyActor(QObject, vmi.Menu, vmi.Mouse):
    def __init__(self, view: vmi.View, name=None):
        ...
        self._RGB, self._Alpha = [0.9, 0.9, 0.9], 1.0
        self.color(rgb=self._RGB, alpha=self._Alpha)
    ...
    def color(self, rgb=None, alpha=None):
        ...
        if rgb is not None:
            self._RGB = rgb
            self._Property.SetColor(rgb)
        if alpha is not None:
            self._Alpha = alpha
            self._Property.SetOpacity(alpha)
        return {'rgb': list(self._Property.GetColor()),
                'a': self._Property.GetOpacity()}
    ...
    def pickable(self, arg=None):
        ...
        if arg is True:
            self._Pickable = True
            self._Prop.SetPickable(1)
        elif arg is False:
            self._Pickable = False
            self._Prop.SetPickable(0)
...
class ImageSlice(QObject, vmi.Menu, vmi.Mouse):
    ...
    def dn(self, n3=None):
        ...
        n3, dth3 = [abs(_) for _ in vmi.vtNormalized(n3)], self.dth3()
    ...
    def lthn(self, n3=None):
        if n3 is None:
            n3 = self._BindSlicePlane._SlicePlane.GetNormal()
        n3, lth3 = [abs(_) for _ in vmi.vtNormalized(n3)], self.lth3()
