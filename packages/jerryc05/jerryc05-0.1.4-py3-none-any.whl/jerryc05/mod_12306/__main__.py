...
def main(args=L):
	F='0';E='\\';C=args
	if not C:C=[]
	if G(C)<3:raise W(f'Missing argument: Expected "<d_city> <a_city> <date>" but found {C}.')
	A=C[2]
	...
	X,Y=query_city(C[0],C[1]);import urllib.request
	with urllib.request.urlopen(f"https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date={A}&leftTicketDTO.from_station={X[1]}&leftTicketDTO.to_station={Y[1]}&purpose_codes=ADULT")as Z:
		import json;H('+-------+-------+-------+-------+-------+-------+-------+---------+---------+------+------+\n| TRAIN | START |  END  | TOTAL |  VIP  |  1ST  |  2ND  |  SOFT-  |  HARD-  | HARD | NONE |\n|  NO.  | TIME: | TIME: | TIME: | CLASS | CLASS | CLASS | SLEEPER | SLEEPER | SEAT | SEAT |\n+-------+-------+-------+-------+-------+-------+-------+---------+---------+------+------+');import jerryc05.mod_12306.mod_parser;D=jerryc05.mod_12306.mod_parser.ticket_count;T=jerryc05.mod_12306.mod_parser.colored_text;U=Z.read()
		if b'<'in U[:4]:raise W(f"Invalid date, {A} may be some date in the past.")
		L=json.loads(U)['data']['result']
		if not L:L='|||-----|||||-----|-----|-----|||||||||||||||||||||||',
		for a in L:
			B=a.split('|');b=B[3];c=B[8];d=B[9];e=B[10];M=D(B[32]);N=D(B[31]);I=D(B[30]);O=D(B[23]);P=D(B[28]);Q=D(B[29]);R=D(B[26]);S=f"| {b:5} | {c:5} | {d:5} | {e:^5} | {M:^5} | {N:^5} | {I:^5} | {O:^7} | {P:^7} | {Q:^5}| {R:^5}|"
