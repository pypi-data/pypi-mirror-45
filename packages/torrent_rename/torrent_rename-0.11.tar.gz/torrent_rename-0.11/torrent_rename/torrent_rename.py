...
class torrent_rename(object):
	...
	def getName(self, torrent_file, overwrite="auto"):
		...
		if output:
			# name = re.findall()
			name = re.findall('(name\d+:)(.*?)(\d+:)', output)
			...
			dst = os.path.join(os.path.dirname(os.path.abspath(torrent_file)), name)  + ".torrent"
			count = 1
			overwrite_string = ""
			if os.path.isfile(dst):
				if not os.stat(dst).st_size == os.stat(os.path.abspath(torrent_file)).st_size:
					if overwrite == "auto":
						if len(re.findall("_\d+\.torrent", os.path.basename(dst))) > 0:
							count = int(re.findall("_\d+", os.path.basename(dst)[1:])) + 1
						dst = os.path.join(os.path.dirname(os.path.abspath(torrent_file)), name)  + "_" + str(count) + ".torrent"
	...
	def usage(self, print_help=False):
		...
		if len(sys.argv) == 1:
			...
			self.rename(args.PATH, args.overwrite)
