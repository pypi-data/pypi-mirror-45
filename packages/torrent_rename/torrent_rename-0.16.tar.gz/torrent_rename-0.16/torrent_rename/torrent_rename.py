				...
				if not os.stat(dst).st_size == os.stat(os.path.abspath(torrent_file)).st_size:
					if overwrite == "auto":
						...
						if overwrite:
							os.remove(dst)
							overwrite_string = " " + make_colors("[OVERWRITE]", 'lightwhite', 'lightred')
					print (make_colors("RENAME", 'lightwhite', 'lightblue') + "%s:"%(overwrite_string), str(name) + ".torrent")
					os.rename(os.path.abspath(torrent_file), dst)
				else:
					print (make_colors("RENAME", 'lightwhite', 'lightblue') + ": " + make_colors("[PASS]", 'lightwhite', 'lightmagenta') + " " +  str(name) + ".torrent")
			else:
				print (make_colors("RENAME", 'lightwhite', 'lightblue') + "%s:"%(overwrite_string), str(name) + ".torrent")
				os.rename(os.path.abspath(torrent_file), dst)
			print ("-"*max_width)
