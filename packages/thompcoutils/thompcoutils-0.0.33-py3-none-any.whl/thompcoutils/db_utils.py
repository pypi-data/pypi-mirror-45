...
class DbUtils:
    def __init__(self, db_type, host, username, password, schema, port, sqlite_file=None):
        ...
        if db_type == "sqlite":
            ...
            self._connect_mysql(db_type, username, password, schema, host, port)
        elif db_type == "odbc":
    ...
    def _connect_mysql(self, db_type, username, password, database, host, port):
        port_str = ""
        if port is not None:
            port_str = ":" + str(port)
        uri = db_type + "://" + username + ":" + password + "@" + host + port_str + "/" + database
        self._connect_uri(uri)
