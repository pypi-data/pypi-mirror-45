...
from . import Rule,get_connect_config
import _gen
class DBHelper:
    ...
    def _db_connect(self):
        ...
        try:
            ...
            _gen.Gen.log("数据库类型=>",self.dbtype)

            _gen.Gen.log("数据库名=>",self.dbname)
            _gen.Gen.log("数据库地址(仅mysql|db2)=>",self.host,self.port)
            _gen.Gen.log("数据库账号=>",self.user,self.pwd)
            ...
            _gen.Gen.log("连接成功,获得=>%s"%self.conn)
    ...
    def db_execute(self, sql,error="ignore-"):
        ...
        try:
            ...
            _gen.Gen.log("出现异常=>"+str(e))
            raise RuntimeError("sql执行结果报错.")
        finally:

            cursor.close()
            if isinstance(res,int):
                _gen.Gen.log("执行结果=>%s(生效行数)"%(res,))
            else:
                _gen.Gen.log("执行结果=>%s"%(res,))  
            _gen.Gen.log("\n\n")
