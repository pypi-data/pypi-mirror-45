...
class TestCase(unittest.TestCase):
    ...
    def httpShouldReturn(self, body=None, callback=None, scheme='https', host='test.wikipedia.org', path='/w/',
        ...
        url = '{scheme}://{host}{path}{script}.php'.format(
            scheme=scheme, host=host, path=path, script=script)
    ...
    def stdSetup(self):
        ...
        site = mwklient.Site('test.wikipedia.org')
        responses.reset()
        return site
...
class TestClient(TestCase):
    ...
    def test_user_agent_is_sent(self):
        ...
        site = mwklient.Site('test.wikipedia.org',
                             clients_useragent='MyFabulousClient')
    ...
    def test_custom_headers_are_sent(self):
        ...
        site = mwklient.Site('test.wikipedia.org', custom_headers={
                             'X-Wikimedia-Debug': 'host=mw1099.eqiad.wmnet; log'})

        assert 'host=mw1099.eqiad.wmnet; log' in responses.calls[
            0].request.headers['X-Wikimedia-Debug']
    ...
    def test_httpauth_raise_error_on_invalid_type(self):
        ...
        with pytest.raises(RuntimeError):
            site = mwklient.Site('test.wikipedia.org', httpauth=1)
    ...
    def test_api_disabled(self):
        ...
        with pytest.raises(mwklient.errors.APIDisabledError):
            site = mwklient.Site('test.wikipedia.org')
    ...
    def test_min_version(self):
        ...
        with pytest.raises(mwklient.errors.MediaWikiVersionError):
            site = mwklient.Site('test.wikipedia.org')
    ...
    def test_api_error_response(self):
        ...
        with pytest.raises(mwklient.errors.APIError) as excinfo:
...
class TestLogin(TestCase):
    ...
    @mock.patch('mwklient.client.Site.site_init')
    @mock.patch('mwklient.client.Site.raw_api')
...
class TestClientApiMethods(TestCase):
    ...
    def setUp(self):
        self.api = mock.patch('mwklient.client.Site.api').start()
        self.api.return_value = self.metaResponse()
        self.site = mwklient.Site('test.wikipedia.org')
    ...
    def test_revisions(self):
        ...
        revisions = [rev for rev in self.site.revisions(
            [689697696, 689816909], prop='content')]
        ...
        assert revisions[0]['timestamp'] == time.strptime(
            '2015-11-08T21:52:46Z', '%Y-%m-%dT%H:%M:%SZ')
...
class TestClientUploadArgs(TestCase):
    ...
    def setUp(self):
        self.raw_call = mock.patch('mwklient.client.Site.raw_call').start()
    ...
    def configure(self, rights=['read', 'upload']):
        ...
        self.raw_call.side_effect = [
            ...
            # 2nd response:
            json.dumps(
                {'query': {'tokens': {'csrftoken': self.vars['token']}}}),
    ...
    def test_upload_args(self):
        ...
        self.site.upload(file=StringIO(
            'test'), filename=self.vars['fname'], comment=self.vars['comment'])
    ...
    def test_upload_ambigitious_args(self):
        ...
        with pytest.raises(TypeError):
            self.site.upload(
                filename='Test', file=StringIO('test'), filekey='abc')

    def test_upload_missing_upload_permission(self):
        self.configure(rights=['read'])

        with pytest.raises(mwklient.errors.InsufficientPermission):
...
class TestClientGetTokens(TestCase):
    ...
    def test_token_old_system_without_specifying_title(self):
        ...
        self.raw_call.return_value = self.makePageResponse(
            edittoken='sometoken', title='Test')
        self.site.get_token('edit')
    ...
    def test_token_old_system_with_specifying_title(self):
        ...
        self.raw_call.return_value = self.makePageResponse(
            edittoken='sometoken', title='Some page')
