...
import mwklient.page
import mwklient.image
...
class List():
    ...
    def set_iter(self, data):
        ...
        if self.result_member not in data['query']:
            self._iter = iter(six.moves.range(0))
        elif isinstance(data['query'][self.result_member], list):
...
class GeneratorList(List):
    ...
    def __init__(self, site, list_name, prefix, *args, **kwargs):
        ...
        pref = self.prefix + 'limit'
        self.args['g' + pref] = self.args[pref]
        del self.args[pref]
        ...
        self.page_class = mwklient.page.Page
    ...
    def __next__(self):
        ...
        if info['ns'] == 6:
            return mwklient.image.Image(self.site, u'', info)
        return mwklient.page.Page(self.site, u'', info)
    ...
    def load_chunk(self):
        ...
        self.args['iiprop'] = iiprop
        return super(GeneratorList, self).load_chunk()


class Category(mwklient.page.Page, GeneratorList):

    def __init__(self, site, name, info=None, namespace=None):
        mwklient.page.Page.__init__(self, site, name, info)
        kwargs = {}
    ...
    def __repr__(self):
        return "<Category object '%s' for %s>" % (self.name.encode('utf-8'),
                                                  self.site)

    def members(self, prop='ids|title', namespace=None, sort='sortkey',
                direc='asc', start=None, end=None, generator=True):
        prefix = self.get_prefix('cm', generator)
        kwargs = dict(self.generate_kwargs(
            prefix, prop=prop, namespace=namespace, sort=sort, direc=direc,
            start=start, end=end, title=self.name))
        return self.get_list(generator)(self.site, 'categorymembers', 'cm',
                                        **kwargs)
...
class PageList(GeneratorList):
    ...
    def get(self, name, info=()):
        ...
        cls = {
            14: Category,
            6: mwklient.image.Image,
        }.get(namespace, mwklient.page.Page)
    ...
    def guess_namespace(self, name):
        ...
        """
        for namespace in self.site.namespaces:
            if namespace == 0:
                continue
            if name.startswith(u'%s:' % self.site.namespaces[namespace].
                               replace(' ', '_')):
                return namespace
            if namespace in self.site.default_namespaces:
                if name.startswith(u'%s:' % self.site.
                                   default_namespaces[namespace].
                                   replace(' ', '_')):
                    return namespace
        return 0
