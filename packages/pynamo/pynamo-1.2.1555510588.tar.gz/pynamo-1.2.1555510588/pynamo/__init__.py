...
class Pynamo(object):
    ...
    def put_item(self, table_name, item, overwrite=False, **kwargs):
        ...
        if not overwrite:
            ...
            if keys_response['status'] != 200:
                return self.__return(self.__client_error(keys_response['error_msg'], response, table_name))
    ...
    def table_key_values(self, table_name):
        ...
        keys_response = self.table_keys(table_name)
        if keys_response['status'] != 200:
            return self.__return(self.__client_error(keys_response['error_msg'], keys_response, table_name))
        keys = keys_response['data']
    ...
    def copy_table(self, source_table, target_table,
        ...
        try:
            ...
            source_data = self.scan(source_table)['data']
