...
def main():
    ...
    with tf.Session() as sess:
        ...
        with open(html_file, 'a') as f:
            ...
            print("<h3> Classification probablities (softmax values) <pre>", f)
            print(softmax_values_data, f)
            plt.figure(figsize=(6,3))
            plot_value_array(0, predictions, correct_labels)
            plt.savefig(png_file, bbox_inches='tight')
            print("<br/> <img src='%s'/> <br/>" % png_file, f)
            # print("Num target : " + str(len(correct_labels)))
            print("</pre> <h3> Predicted Class <pre>", f)
            # print(correct_labels[0], f)
            print(predictions[0], f)
            print("</pre>", f)
        f.close()
