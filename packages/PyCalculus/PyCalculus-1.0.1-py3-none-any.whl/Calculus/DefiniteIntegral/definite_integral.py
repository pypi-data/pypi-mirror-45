...
def eval_single_partition_concurrently(mutex_obj, sum_obj, math_func, from_x, to_x):
    while True:
        if mutex_obj[0]:
...
def summarize_multiple_partitions_concurrently(math_func, from_x, to_x, div_num):
    ...
    thread = None
    for i in range(div_num):
        thread = Thread(target=eval_single_partition_concurrently, args=(mutex_obj, sum_obj, math_func, float(from_x) + i*dx, float(from_x) + ((i+1)*dx)))
