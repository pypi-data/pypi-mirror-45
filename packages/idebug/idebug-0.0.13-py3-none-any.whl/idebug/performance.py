...
class Loop:
    ...
    def report(self, addi_info='...'):
        if self.count <= self.len:
            ...
            print(f"\n{'*'*60}\n {self.__class__} ({self.count}/{self.len}) | {title}")
            for tpl in tpls:
                timeexp, unit = inumber.convert_timeunit(tpl[1])
                print(f" {tpl[0]} : {timeexp}")
            self.count += 1
...
class Runtime:
    ...
    def report(self):
        runtime = (datetime.now() - self.start_dt).total_seconds()
        timeexp, unit = inumber.convert_timeunit(runtime)
        print(f"\n{'*'*60}\n {self.__class__} | {self.funcname} : {timeexp}")
