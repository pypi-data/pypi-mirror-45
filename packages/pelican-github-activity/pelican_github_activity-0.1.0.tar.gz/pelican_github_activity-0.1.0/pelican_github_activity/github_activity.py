...
class GitHubActivity():
    ...
    def __init__(self, generator):
        ...
        self.ignore_repos = generator.settings.get('GITHUB_REPO_IGNORE', [])

    def _valid_link(self, link: str) -> bool:
        return not any(repo.lower() in link.lower() for repo in self.ignore_repos)
    ...
    def fetch(self):
        ...
        for activity in self.activities['entries']:
            if self._valid_link(activity['link']):
                # for k in activity.keys():
                #     # print('KEY: ', k, 'VALUE: ', activity[k])
                entries.append(
