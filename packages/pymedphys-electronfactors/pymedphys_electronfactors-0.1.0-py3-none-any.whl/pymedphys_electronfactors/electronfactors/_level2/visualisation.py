...
def model_and_display(x, y):
    (
        width, length, circle_centre, _, _, _
...
def plot_model(width_data, length_data, factor_data):
    ...
    vmin = np.nanmin(np.concatenate(
        [model_factor.ravel(), factor_data.ravel()]))
    vmax = np.nanmax(np.concatenate(
        [model_factor.ravel(), factor_data.ravel()]))
    # vrange = vmax - vmin
