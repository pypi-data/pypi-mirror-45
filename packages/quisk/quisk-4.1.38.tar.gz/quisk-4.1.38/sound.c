...
{  // called in an infinite loop by the main program
	...
	if (is_DGT) {		// Discard previous samples and use digital samples
...
{  /* Call with keyword arguments ONLY; change local parameters */
	static char * kwlist[] = {"dc_remove_bw", NULL} ;

	if (!PyArg_ParseTupleAndKeywords (args, keywds, "|i", kwlist, &dc_remove_bw))
		return NULL;
	Py_INCREF (Py_None);
	return Py_None;
}
