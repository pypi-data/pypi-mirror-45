...
class MMLParser:
    ...
    def parse(self,channel,seq):
        ...
        while pos < len(seq):
            ...
            if m.group(2):  # note
                note = self.NOTE_MAP[m.group(3)]
                num = note + 12*self.octave[channel]
                acc = m.group(4)
                if acc:
                    if acc in '+#':
                        num += 1
                ...
                self.octave[channel] += 1
            elif piece[0] in 'PR': # rest
                if m.group(8):
                    divide = int(m.group(8))
                else:
                    divide = self.divide[channel]
                length = 60*4/self.tempo/divide
                if m.group(9):  # dot
                    length *= 1.5
                if prev_note_num is not None: # tied to the previous rest
                    if prev_note_num != 0:
                        raise Exception("Tied notes cannot be different")
                    length += prev_length;
                # check for possible following rest tie
                if seq[pos:pos+1] == '&':
                    prev_note_num = 0
                    prev_length = length
                    pos += 1
    ...
    def play(self,*seqs):
        ...
        for ts,etype,i,note in self.mix(*seqs):
            ...
            if etype == self.EVT_NOTE_ON and note > 0:
                self.callback(i,etype,note,self.vel[i])
            elif etype == self.EVT_NOTE_OFF:
                self.callback(i,etype,note,0)
