...
class Mold(six.with_metaclass(_mold.MoldMeta, object)):
    ...
    def dispatch_repr(self, dep_list):
        ...
        if hasattr(self, "_cls_constructor_spec"):
            params = self._cls_constructor_spec.cls_arguments(self)
            items_reprs = [(kw, list(_py_lang.dispatch_repr(value, dep_list))) for kw, value in params]
...
def reproduction(single_object):
    ...
    """
    return "".join(_py_lang.dispatch_repr(single_object, None, top_level=True))


def serialize(output_file_path, *objects, **kw_objects):
    """
    Dumps objects into a python file.
    Objects are ordered alphabetically to make it able to be handled by VCSs.
    """
    all_objects = itertools.chain(objects, sorted(kw_objects.items()))
...
def serialize_in_order(output_file_path, *objects):
    ...
    """
    content = _py_lang.build_py_file_content(*objects)
