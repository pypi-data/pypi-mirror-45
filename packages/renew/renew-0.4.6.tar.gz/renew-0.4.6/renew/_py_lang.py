...
def dispatch_repr(object_, dep_list=None, top_level=False, is_last_item=None):
    if hasattr(object_, "make_py_reference"):
        for element in object_.dispatch_repr(dep_list):
            yield element

    elif isinstance(object_, list):
        items_reprs = [(None, list(dispatch_repr(item, dep_list, is_last_item=is_last))) for item, is_last in
                       mark_last(object_)]
        ...
        items_reprs = [(None, list(dispatch_repr(item, dep_list, is_last_item=is_last))) for item, is_last in
                       mark_last(object_.items())]
        for element in make_a_markup("OrderedDict([", items_reprs, "])"):
            yield element

    elif isinstance(object_, dict):
        items_reprs = [(kw, list(dispatch_repr(value, dep_list, is_last_item=is_last)))
        ...
        for line, is_last_part in mark_last(list(_split_long_string(object_))):
            yield indent + repr(line) + ("\n" if not is_last_part or is_last_item else ",\n")
        if top_level:
            yield ")"
