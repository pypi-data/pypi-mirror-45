...
class Viewer2D(QtWidgets.QWidget):
    ...
    def __init__(self,parent=None,scaling_options=edict(scaled_xaxis=edict(label="",units=None,offset=0,scaling=1),scaled_yaxis=edict(label="",units=None,offset=0,scaling=1))):
    ...
    def set_scaling_axes(self,scaling_options=None):
        ...
        scaling_options=edict(scaled_xaxis=edict(label="",units=None,offset=0,scaling=1),scaled_yaxis=edict(label="",units=None,offset=0,scaling=1))
        """
    ...
    def setImage(self,data_red=None,data_green=None,data_blue=None):
        try:
            ...
            if green_flag:
                self.data_to_export['data2D'][self.title+'_image_green']=OrderedDict(data=data_green,x_axis=self.x_axis_scaled,y_axis=self.y_axis_scaled)
            if blue_flag:
                self.data_to_export['data2D'][self.title+'_image_blue']=OrderedDict(data=data_blue,x_axis=self.x_axis_scaled,y_axis=self.y_axis_scaled)
...
if __name__ == '__main__':
    ...
    prog = Viewer2D(Form);
    prog.set_scaling_axes(scaling_options=edict(scaled_xaxis=edict(label="eV",units=None,offset=20,scaling=2),scaled_yaxis=edict(label="time",units='s',offset=-10,scaling=0.1)))
    Nx=100;
    Ny=200
