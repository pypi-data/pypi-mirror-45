...
from .callbacks import LRFinder, CyclicLR, SGDRScheduler
...
class ModelContainer:
    ...
    def get_model(self, params):
        ...
        if 'tracks' in self.data['train'] and boolify(self.data['train']['tracks']):
            ...
            # Apply track network
            tns_branches['tracks'] = self.scalar_net(tns_branches['tracks'])
    ...
    def train(self):
        ...
        if not self.params['data_generator']['use']:
            ...
            try:
                ...
                if query_yes_no('Evaluate model?'):
                    ...
                    print('Exiting without predicting on test set.')
                    sys.exit()
        ...
        # Use DataGenerator instead
            ...
            datagen_train = DataGenerator(set_name='train',
                                          data_snippet=self.data,
                                          load_params=drop_key(self.params['data_generator'], ['use', 'max_queue_size', 'n_workers', 'n_points']),
    ...
    def _get_callbacks(self):
        ...
        modelcheckpoint = ks.callbacks.ModelCheckpoint(
                            filepath=self.dirs['saved_models'] +
                            'model.{epoch:04d}-{val_loss:.4f}.hdf5',
    ...
    def _load_best_model(self):
        ...
        if len(pathnames) == 0:
            ...
            self.model = ks.models.load_model(best_path)
            if self.verbose:
                if len(pathnames) > 1:
                    print(f'Loaded best model for evaluation from {best_path}.')
                else:
                    print(f'Loaded initial model from {best_path}.')
