...
class Kernel_Install_Inside_Jupyter:
    ...
    def __init__(self, kernel_class,spec):
        self.kernel_module = kernel_class.__module__
        self.kernel_spec   = spec
        self.kernel_name   = spec.get('display_name')
...
class Kernel_Install:
    ...
    def __init__(self, kernel_name, kernel_class, kernel_spec, jupyter_kernel):
        ...
        self.install_code   = """   
                                   ...
                                   spec = {2}
                                   Kernel_Install_Inside_Jupyter({1},spec).install()                                   
                              """.format(self.kernel_module, self.kernel_class, json.dumps(self.kernel_spec))
        self.uninstall_code = """
                                   from {0} import {1}
                                   from osbot_jupyter.api.Kernel_Install import Kernel_Install_Inside_Jupyter                    
