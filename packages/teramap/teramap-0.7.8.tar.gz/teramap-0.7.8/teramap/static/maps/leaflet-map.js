...
// Some commonly used leaflet utils
if (!L.LatLng.toWKT) {
...
function leaflet_map(map_id, options) {
    ...
    if (options.fullscreenControl) {
        controlOptions = L.extend({postion: 'topleft'}, options.fullscreenControl);
        map.fullscreenControl = L.control.fullscreen(controlOptions).addTo(map);
    }
    if (options.scaleControl) {
        controlOptions = L.extend({imperial: false}, options.scaleControl);
        map.scale = L.control.scale(controlOptions).addTo(map);
    }
    ...
    if (options.minimap) {
        controlOptions = L.extend({
            minimapLayer: L.tileLayer('https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png')
        }, options.minimap);
        new MiniMap(options['minimapLayer'], controlOptions).addTo(map);
    }
