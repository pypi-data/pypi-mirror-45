...
class Film:
    ...
    frame = property('', set_frame, '', "Lit les frames à partir d'un seul\
 fichier. Ces frames sont séparées par un séparateur, par défaut #")
    ...
    frames = property('', set_frames, '', "Lit les frames à partir de \
fichiers dans répertoire. Chaque frame est dans un fichier propre. Les \
fichiers sont lus dans le répertoire dans le sens alphnumérique croissant \
de leurs noms.")
    ...
    Retourne
        ...
        - 1 si il y a 1 fichier par frame (cas de frames)
        - le nombre de frames dans le fichier si il s'agit d'un fichier unique
        """)
    ...
    fixée, par défaut 40 par l'attribut sous_titres_largeur.
    ...
    sous_titres_reverse = property(get_sous_titres_reverse,
                                   ...
                                   """
    Retourne ou fixe l'affichage des sous-titres est reverse ou non.
    ...
    # public methods
    def projection(self, nb_frames=0, x=2, y=4, trace=0, delai=0.1):
        """Lance l'instance film en lisant les frames.

        - nb_frames: nombre de frames à exécuter. 0 = non déterminé
        - x, y: les coordonnées haut gauche de départ du film
          (y compris le titre, si self.affiche_titre = True)
        ...
        no_frame = 0
        while no_frame <= nb_frames or nb_frames == 0:
            try:
                for f in self._frames:
                    ...
                    sleep(delai)
                    if no_frame > nb_frames and nb_frames != 0:
                        break
                    if self.reverse:
                        x -= self.deplacement
                    else:
