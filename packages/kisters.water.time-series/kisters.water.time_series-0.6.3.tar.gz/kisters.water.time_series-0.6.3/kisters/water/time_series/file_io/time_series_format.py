...
from typing import Iterable, Mapping, MutableMapping, Any, Union
...
class TimeSeriesReader(ABC):
    ...
    def __extract_path(self, file):
        ...
        for ext in list(self.format.extensions):
            if file.lower().endswith("." + ext.lower()):
                path = file[:-(len(ext) + 1)]
        if path.startswith(self.format.root_dir + "/"):
    ...
    def _get_metadata(self, file: str) -> Iterable[Mapping[str, Any]]:
        ...
        if os.path.isfile(meta_file):
            ...
            if (list(self.format.extensions)[0].upper() in json_list[0]
               and json_list[0]['tsPath'] == self.__extract_path(file)):
                    return json_list
        return self._extract_metadata(file)

    def read(self, file: str) -> Iterable[TimeSeries]:
...
class TimeSeriesWriter(ABC):
    ...
    @abstractmethod
    def write(self, file: str, data_list: Union[Iterable[DataFrame], Iterable[TimeSeries]],
    ...
    def write_metadata(cls, file: str, meta_list: Iterable[Mapping[str, Any]]):
        with open(file + ".metadata", "w") as o:
            json.dump(
                meta_list, o, default=_json_serial, indent=4, sort_keys=True)
