...
from typing import Any, MutableMapping, TYPE_CHECKING, Optional, Mapping
...
class TSOTimeSeries(TimeSeriesItemMixin, TimeSeriesAttributesMixin, TimeSeriesCutRangeMixin, TimeSeries):
    ...
    def _load_data_frame(self, start: datetime=None, end: datetime=None, params: Mapping[str, Any]=None) -> pd.DataFrame:
        url = (self.__TSO._base_url + 'organizations/' + self.__data['organization']
               + '/timeSeries/' + str(self.id) + '/data/')
        params = {}
    ...
    def _raw_metadata(self):
        ...
        r = self.__TSO._session.get(
            self.__TSO._base_url + 'organizations/' + self.__data['organization'] +
    ...
    def __refresh(self):
        self.__data = self.__TSO._filter_by_id_list(
            self.__TSO._session.get(
                self.__TSO._base_url + "organizations/" + self.__TSO._organization + "/timeSeries/").json(),
            [str(self.id)])[0]

    def write_data_frame(self, data_frame: pd.DataFrame, **kwargs):
        headers = {'content-type': 'application/json'}
        newdata = []
        data_frame.fillna("null", inplace=True)
        for row in data_frame.iterrows():
            ...
            newdata.append(list)
        r = self.__TSO._session.put(self.__TSO._base_url + 'organizations/' + self.__data['organization'] +
                                    "/timeSeries/" + str(self.id) + "/data", json=newdata, headers=headers)
    ...
    def delete_time_series(self):
        r = self.__TSO._session.delete(self.__TSO._base_url + 'organizations/' + self.__data['organization'] +
                                       "/timeSeries/" + self.id)
        return r
