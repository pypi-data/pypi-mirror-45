...
class MemoryTimeSeries(TimeSeriesCutRangeMixin, TimeSeriesAttributesMixin, TimeSeriesItemMixin):
    def __init__(self, metadata: MutableMapping[str, Any], data: pd.DataFrame, comments: pd.DataFrame=None):
        """
    ...
    @classmethod
    def __validate_instance(cls, metadata: Mapping[str, Any], data: pd.DataFrame, comments: pd.DataFrame) -> str:
    ...
    def write_data_frame(self, data_frame: pd.DataFrame, start: datetime=None, end: datetime=None):
        ...
        if self.__data is None:
            ...
            self.__data = pd.concat([self.__data, data_frame[~data_frame.index.isin(self.__data.index)]], sort=True)
