...
class Firmware():
    ...
    def open(self, url):
        ...
        with urlopen(self.url) as res:
            ...
            # 세번째 블록의 데이터가 [01 02 03 04 05 06 07 08 09 0a 0b] 인지 확인하여
            # 맞는 경우 2번째 블럭(CC2541)을, 아니면 1번째 블럭(STM32F4)을 펌웨어 헤더로 사용
            flagCheck3rdBlock = True
            for i in range(11):
                if self.resource[32 + i] != i:
                    flagCheck3rdBlock = False
                    break
            
            if flagCheck3rdBlock == True:
                self.rawHeader  = self.resource[16:32]
                self.header     = FirmwareHeader.parse(self.rawHeader)
                self.header.length = self.length
                self.imageType  = ImageType(self.resource[15])
            else:
                self.rawHeader  = self.resource[0:16]
                self.header     = FirmwareHeader.parse(self.rawHeader)
                self.imageType  = ImageType.RawImageSingle
...
class Updater:
    ...
    def _eventInformation(self, information):
        ...
        if      isinstance(information, Information):
        ...
        elif    isinstance(information, InformationCC2541):
            self.modeUpdate         = information.modeUpdate
            self.flagUpdated        = True 
            self.modelNumber        = information.modelNumber
            self.deviceType         = DeviceType(((self.modelNumber.value >> 8) & 0xFF))
            self.imageType          = information.imageType
            ...
            if information.modeUpdate == ModeUpdate.Complete:
                ...
                print(Fore.YELLOW + "* Connected Device : {0}".format(self.deviceType) + Style.RESET_ALL)
                print("  Model Number : {0}".format(information.modelNumber))
                print("    Image Type : {0}".format(information.imageType))
                print("       Version : {0}.{1}.{2} ({3} / 0x{3:08X})".format(information.version.major, information.version.minor, information.version.build, information.version.v))
                print("  Release Date : {0}.{1}.{2}".format(information.year, information.month, information.day))
                print("   Mode Update : {0}\n".format(information.modeUpdate))
    ...
    def _eventUpdateLocation(self, updateLocation):
    ...
    def _update(self):
        ...
        firmware = []
        firmware.append(Firmware("https://s3.ap-northeast-2.amazonaws.com/byrobot/fw_drone_7_drone_p1_latest.eb"))
        firmware.append(Firmware("https://s3.ap-northeast-2.amazonaws.com/byrobot/fw_drone_7_bleserver_p1_latest.eb"))
        #firmware.append(Firmware("file:///firmware/fw_drone_7_drone_p1_latest.eb"))
        #firmware.append(Firmware("file:///firmware/fw_drone_7_bleserver_p1_a_latest.eb"))
        #firmware.append(Firmware("file:///firmware/fw_drone_7_bleserver_p1_b_latest.eb"))
        ...
        # 이벤트 핸들링 함수 등록
        drone.setEventHandler(DataType.Information, self._eventInformation)
        drone.setEventHandler(DataType.UpdateLocation, self._eventUpdateLocation)
        ...
        while self._flagThreadRun:
            for fw in firmware:
                ...
                if self.modelNumber == fw.header.modelNumber:
                    ...
                    # 펌웨어 이미지 확인
                    flagImageTypePass = False

                    if      self.imageType == ImageType.ImageSingle and fw.imageType == ImageType.RawImageSingle:
                        flagImageTypePass = True
                        
                    elif    self.imageType == ImageType.ImageA and fw.imageType == ImageType.RawImageB:
                        flagImageTypePass = True
                        
                    elif    self.imageType == ImageType.ImageB and fw.imageType == ImageType.RawImageA:
                        flagImageTypePass = True

                    if flagImageTypePass == True:
