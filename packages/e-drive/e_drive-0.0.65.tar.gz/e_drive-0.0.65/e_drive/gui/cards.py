...
class CardReaderAndList(App):
    ...
    def open(self):
        ...
        self.drone.setEventHandler(DataType.RawCardFunctionList, self.eventRawCardFunctionList)
    ...
    def my_callback(self, dt):
        ...
        if      index == 0:
            ...
            self.drone.sendRequest(DeviceType.Drone, DataType.RawCardFunctionList)
    ...
    def eventRawCardList(self, rawCardList):
        ...
        for i in range(0, 40):
            ...
            if      i == rawCardList.index and i == rawCardList.size:
                self.labelNumberCard[i].text = "[color=ffff33][b]" + "{0}".format(i) + "[/b][/color]"
            elif    i == rawCardList.index:
                self.labelNumberCard[i].text = "[color=ff3333][b]" + "{0}".format(i) + "[/b][/color]"
            elif    i == rawCardList.size:
                self.labelNumberCard[i].text = "[color=33ff33][b]" + "{0}".format(i) + "[/b][/color]"
            else:
                self.labelNumberCard[i].text = "[color=555555]" + "{0}".format(i) + "[/color]"
    ...
    def eventRawCardFunctionList(self, rawCardFunctionList):

        for i in range(0, 40):
            
            if      i == rawCardFunctionList.index and i == rawCardFunctionList.size:
                self.labelNumberFunction[i].text = "[color=ffff33][b]" + "{0}".format(i) + "[/b][/color]"
            elif    i == rawCardFunctionList.index:
                self.labelNumberFunction[i].text = "[color=ff3333][b]" + "{0}".format(i) + "[/b][/color]"
            elif    i == rawCardFunctionList.size:
                self.labelNumberFunction[i].text = "[color=33ff33][b]" + "{0}".format(i) + "[/b][/color]"
            else:
                self.labelNumberFunction[i].text = "[color=555555]" + "{0}".format(i) + "[/color]"

            self.widgetCardFunctionListTop[i].canvas.clear()
            self.widgetCardFunctionListBottom[i].canvas.clear()

            cardTop = (rawCardFunctionList.card[i] >> 4) & 0x0F
            cardBottom = (rawCardFunctionList.card[i]) & 0x0F
            colorTop = Color(0, 0, 0)
            colorBottom = Color(0, 0, 0)
            ...
            with self.widgetCardFunctionListTop[i].canvas:
                Color(colorTop.r, colorTop.g, colorTop.b)
                Rectangle(pos=(self.widgetCardFunctionListTop[i].x, self.widgetCardFunctionListTop[i].y), size=(self.widgetCardFunctionListTop[i].width, self.widgetCardFunctionListTop[i].height))

            with self.widgetCardFunctionListBottom[i].canvas:
                Color(colorBottom.r, colorBottom.g, colorBottom.b)
                Rectangle(pos=(self.widgetCardFunctionListTop[i].x, self.widgetCardFunctionListBottom[i].y), size=(self.widgetCardFunctionListTop[i].width, self.widgetCardFunctionListTop[i].height))


    def build(self):

        self.labelNumberCard = []
        self.widgetCardListTop = []
        self.widgetCardFunctionListBottom = []

        self.labelNumberFunction = []
        self.widgetCardFunctionListTop = []
        self.widgetCardFunctionListBottom = []
        ...
        l2TopAbove = BoxLayout(size_hint=(1, None), height=60)
        l3TopCardListArray = []
        for i in range(0, 40):
            self.labelNumberCard.append(Label(text='{0}'.format(i+1), markup=True))
            ...
            l3TopCardListArray[i].add_widget(self.labelNumberCard[i])
            l3TopCardListArray[i].add_widget(self.widgetCardListTop[i])
            l3TopCardListArray[i].add_widget(self.widgetCardListBottom[i])
            l2TopAbove.add_widget(l3TopCardListArray[i])
        
        l2TopBelow = BoxLayout(size_hint=(1, None), height=60)
        l3TopCardFunctionListArray = []
        for i in range(0, 40):
            self.labelNumberFunction.append(Label(text='{0}'.format(i+1), markup=True))
            self.widgetCardFunctionListTop.append(Widget())
            self.widgetCardFunctionListBottom.append(Widget())
            l3TopCardFunctionListArray.append(BoxLayout(orientation='vertical'))
            l3TopCardFunctionListArray[i].add_widget(self.labelNumberFunction[i])
            l3TopCardFunctionListArray[i].add_widget(self.widgetCardFunctionListTop[i])
            l3TopCardFunctionListArray[i].add_widget(self.widgetCardFunctionListBottom[i])
            l2TopBelow.add_widget(l3TopCardFunctionListArray[i])
        ...
        l1Base.add_widget(l2TopAbove)
        l1Base.add_widget(l2TopBelow)
