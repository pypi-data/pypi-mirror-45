...
from ambition_edc.pdf_reports import AmbitionCrfPdfReport
...
class AEReport(AmbitionCrfPdfReport):
        ...
        for followup in followups:
            ...
            for obj in qs:
                ...
                t = Table(
                    [
                        [
                            Paragraph(f"{AeFollowup._meta.verbose_name}: {index}", s),
                ...
                story.append(t)
            index -= 1
        qs = AeInitial.history.filter(id=self.ae_initial.id).order_by("-history_date")
        for obj in qs:
            username = (
                obj.user_created if obj.history_type == "+" else obj.user_modified
            )
            t = Table(
                [
                    [
                        Paragraph(AeInitial._meta.verbose_name, s),
                        Paragraph(username, s),
                        Paragraph(obj.modified.strftime("%Y-%m-%d %H:%M"), s),
    ...
    def _draw_followup_story(self, story, obj, index, total):
        ...
        self.draw_narrative(
            story, title="Description summary of AE outcome:", text=obj.relevant_history
        )
