...
def load(dataDirectory, dataModel, keepRealisations=False):
    ...
    lmax_custom = {"BR": 13, "SV": 16}
    fname = os.path.join(dataDirectory, "CHAOS-6-x4_spline-coefficients.dat")

    # Try opening the file
    try:
        f = open(fname)
    except IOError:
        print('CHAOS-6 file was not found at %s' % fname)
        return -1

    # If it went well, print the header
    print(f.readline())
    ...
    lmax_b = lmax_custom["BR"]
    lmax_sv = lmax_custom["SV"]
    dataModel.addMeasure("BR lmax={}".format(lmax_b), "MF", lmax_b, "nT", g, times=times)
    ...
    dataModel.addMeasure("BR", "MF", lmax, "nT", g, times=times)
