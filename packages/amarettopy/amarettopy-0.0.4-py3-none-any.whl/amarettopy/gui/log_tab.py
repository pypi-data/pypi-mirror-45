    ...
    def refresh(self):
        ...
        while i < n :
            ...
            for rec in recs:
                ...
                code     = rec["code"]
                payload  = rec["payload"]
                if level == "FATAL" and group == "system" and subGroup in ["error", "assert"]:
                    hd = struct.unpack('<6sH', payload)
                    filename = hd[0].decode('utf-8')
                    fileline = hd[1]
                    ext = "(.c)" if code == 0 else "(.h)"
                    payload = "file:" + filename + ext + ", line:" + str(fileline) + ', payload = ' + ''.join([r'\x{:02x}'.format(x) for x in payload])
