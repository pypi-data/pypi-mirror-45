...
def toDegree(rawPosition):
    ...
    """
    return rawPosition * 360.0 / float(0x10000)
...
class InvalidComandPayloadSizeError(MCPTransportError):
    """不正なペイロードサイズ例外"""
    pass
...
class AmarettoPy:
    ...
    def __init__(self,baud=os.environ.get('AMARETTO_BAUD',115200),port=os.environ.get('AMARETTO_PORT',"/dev/ttyUSB0"),timeout_ms=3000):
        """
    ...
    def open(self, baud=os.environ.get('AMARETTO_BAUD',115200),port=os.environ.get('AMARETTO_PORT',"/dev/ttyUSB0")):
        """
        シリアルポートをオープンする
    ...
    def _nack2exception(self, payload):
        ...
        if err == MCP_INVALID_COMMAND_PAYLOAD:
            ...
            return InvalidComandPayloadSizeError
    ...
    def _rpc_log_raw(self, msg, ackfmt):
        ...
        if ackType == MSG_TYPE_NACK:
            ...
            for i in range(readSize):
                ...
                (g,sg,c) = log_group2str(group, subGroup, code)
                logRecord = {'index':logIndex, 'level':log_level2str(logLevel), 'group':g, 'subGroup':sg, 'code':c, 'payload':payload_data}
                # print(logRecord)
    ...
    def get_raw_position(self, devId):
        """
        位置センサの生値を取得する [360/65536 * 度] 16bit符号なし整数
        """
        return self._rpc(MSG_TYPE_GET_PARAM_CMD, devId, payload=struct.pack('<B', PARAM_ID_RAW_POSITION), ackfmt='H')[0]

    def get_recorded_calibration_data(self, devId):
        """
        記録された最新の補正値を取得する
        """
        return self._rpc(MSG_TYPE_GET_PARAM_CMD, devId, payload=struct.pack('<B', PARAM_ID_RECORDED_CALIB), ackfmt='H')[0]
    ...
    def calibration(self, devId):
        """
        キャリブレーションを開始する
        """
        return self._rpc(MSG_TYPE_CALIBRATION_CMD, devId, ackfmt='')
    ...
    def print_log(self, devId, startIndex = 0, readSize = -1):
        ...
        if len(data['logRecords']) > 0:
            ...
            for record in data['logRecords']:
                record['payload'] = str(['0x{:02x}'.format(c) for c in record['payload']])
