...
def split(data, label_field, text_fileds, train_frac=0.8, dev=False, clust_func=None, vec_func=None):
    """
    ...
    text_fileds = text_fileds if isinstance(text_fileds,list) else [text_fileds]
    vecs = []
    for text_field in text_fileds:
    ...
    clust_out_size = int(test_size*0.1)
...
class Text2Vec:
    ...
    def __init__(self,name='wiki.pl.vec',cache=None,url='https://dl.fbaipublicfiles.com/fasttext/vectors-wiki/wiki.pl.vec',mode='sum'):
        ...
        self.vec = FastText(name=name, cache=cache, url=url)
