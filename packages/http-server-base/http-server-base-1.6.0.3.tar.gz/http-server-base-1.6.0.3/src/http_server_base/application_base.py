import re
from typing import Tuple, List, Type, Set, Callable, Union, Any, Iterable, Optional, Dict
...
from tornado.routing import ReversibleRouter, ReversibleRuleRouter, PathMatches, Rule
from tornado.web import RequestHandler, Application

from . import IResponder, IRouter, IApplication
from . import HandlerType, HandlerListType, HandlerController
from . import BasicResponder
from . import Logged_RequestHandler, Empty_RequestHandler, HealthCheck_RequestHandler
...
class ApplicationBase(IApplication):
    """
    ...
    __router: Optional[IRouter] = None
    __settings: Dict[str, Any] = None
    
    __self_addr: str = None
    __listen_port: int = None
    __protocol: str = None
    __host_params: Dict[str, Any] = None
    ...
    (
        ...
        **settings,
        ...
        _settings = camel_case_switcher.dict_processor.dict_keys_camel_case_to_underscope(ConfigLoader.get_from_config(config_prefix, config_name=config_name, default=lambda: dict()), recursive=True)
        if (config_priority):
        ...
        self.__host_params = self.initialize_host(_settings)
        if (self.custom_router is None):
            _h = self.handlers
        else:
            if (isinstance(self.custom_router, IRouter)):
                self.__router = self.custom_router
            elif (isinstance(self.custom_router, type) and issubclass(self.custom_router, IRouter)):
                self.__router = self.custom_router(self, settings.get('optimize_tree', False))
            else:
        ...
        self.attach_controllers(**self.__host_params)
    ...
    def add_host_handlers(self, *host_handlers: HandlerType, host: str = None, settings: dict = None, protocol = None, listen_port_part = None, **kwargs):
        ...
        if (protocol is None):
            protocol = self.__host_params['protocol']
        if (listen_port_part is None):
            listen_port_part = self.__host_params['listen_port_part']
        if (host is None):
    ...
    def add_router_handlers(self, *handlers: HandlerType, **kwargs):
        ...
        for h in handlers:
            self.__router.add_handler(*h, **kwargs)
    
    def add_handlers(self, host_pattern: Union[str, RegExpType] = None, host_handlers: List[Union[HandlerType, Rule]] = None, **kwargs):
