...
class BuilderService:
    ...
    def __collect_trained_model_metrics(self, trained_model_data):
        ...
        classifier_id = str(trained_model_data.identifier)

        app_globals.metrics_registrar.gauge(name='build_duration', documentation='model duration time metric',
                                            labels=[CLASSIFIER_ID]).labels(classifier_id=classifier_id).set(
        ...
        if GENERAL in trained_model_data.stats:
            ...
            if ACCURACY in general_model_stats:
                app_globals.metrics_registrar.gauge(name=ACCURACY, documentation='model accuracy metric',
                                                    labels=[CLASSIFIER_ID]).labels(classifier_id=classifier_id).set(
                    general_model_stats[ACCURACY])
            if ERROR_RATE in general_model_stats:
                app_globals.metrics_registrar.gauge(name=ERROR_RATE, documentation='model error rate metric',
                                                    labels=[CLASSIFIER_ID]).labels(classifier_id=classifier_id).set(
            ...
            if TEST_SET_SIZE in general_model_stats:
                app_globals.metrics_registrar.gauge(name=TEST_SET_SIZE, documentation='model test set size metric',
                                                    labels=[CLASSIFIER_ID]).labels(classifier_id=classifier_id).set(
                    general_model_stats[TEST_SET_SIZE])
            if DATASET_SIZE in general_model_stats:
                app_globals.metrics_registrar.gauge(name=DATASET_SIZE, documentation='model dataset size metric',
                                                    labels=[CLASSIFIER_ID]).labels(classifier_id=classifier_id).set(
