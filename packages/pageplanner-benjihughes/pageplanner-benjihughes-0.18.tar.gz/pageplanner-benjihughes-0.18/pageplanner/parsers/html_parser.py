...
class HTMLParser:
    ...
    def create_document_from_html(self, html, focus, variants=None, rank=None):
        ...
        html = BeautifulSoup(html.raw_html)
        ...
        focus_total = 0
        variant_total = 0
        word_count = self.get_word_count(html)

        # Iterate over each selector in the config.
        for selector in self.elem_config.keys():
            element_data = self.get_element_data(html, selector, focus, variants)
