	...
	if(local):
		...
		if os.path.exists(workflow):
			...
			try:
				...
				return wf.Workflow(workflow, out_workflow, workflow, True)
...
def displayGraph(workflow, local=False):
	if (not local) :
		...
		name, suffix = workflow.link.split(".cwl") 
		graphFile = name + ".svg"
		dotCommand = 'dot -Tsvg > ' + graphFile
		p = subprocess.Popen(["cwltool", "--print-dot", workflow.link], stderr=PIPE, stdout=PIPE)
		dot = subprocess.check_output(dotCommand, stdin=p.stdout, shell=True)
