...
class data_ndarray(np.ndarray):
    ...
    def setfiletype(self,filetype):
        self.filetype=filetype
def read(filename,filetype=None,read_grid=True,**kargs):
    ...
    if ext=='txt':
        ...
        if 'pytecio' not in globals():
            from . import pytecio 
            globals()['pytecio'] = pytecio
        data = pytecio.read(filename)
    elif ext in ['g','q','xyz','fmt']:
        return plot3d.read(filename)
    else:
        data=class_read(filename,read_grid=read_grid)
    return data
class write:
    def __init__(self,filename,data,filetype=None):
