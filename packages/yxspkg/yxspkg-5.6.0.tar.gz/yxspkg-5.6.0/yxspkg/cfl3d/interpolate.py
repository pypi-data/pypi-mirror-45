...
def get_lines_common(one2oneData):
    ...
    def adjust_lines_order(lines):
        ...
        if ijk1[axis1][0] > ijk1[axis1][1]:
            ijk1[axis1] = ijk1[axis1][1],ijk2[axis1][0]
...
def get_point_pattern(lines_common,shapes):
    ...
    t = [kk for i in lines_common for kk in zip(*[split_2_points(j,shapes[j[0]-1]) for j in i])]
    point_list = [set(t[0]) ]
    block_set = set([i[0] for i in t[0]])
    other_list = []
    for ps in t[1:]:
        for p in ps:
            if p[0] not in block_set:
                other_list.append(ps)
                continue
            for i in point_list:
                if p in i:
                    i.update(ps)
                ...
                is_union(point_list,ps)
            block_set.update([bid[0] for bid in ps])
    length0 = -1
    while True:
        length = len(other_list)
        if length == length0:
            block_set.add(other_list[0][0][0])
        else:
            length0 = length
        if length is 0:
            break
