...
from azureml.dataprep.api.engineapi.enginerequests import get_requests_channel
...
_interactive_spark = None


def _ensure_interactive_spark():
    def handle_spark_collect(request, writer):
        try:
    ...
    def handle_spark_run(request, writer):
        try:
            module = _import_loader(request['path'])
    ...
    global _interactive_spark
    if _interactive_spark is None:
        _interactive_spark = get_requests_channel()
        _interactive_spark.register_handler('spark_collect', handle_spark_collect)
        _interactive_spark.register_handler('spark_run', handle_spark_run)
