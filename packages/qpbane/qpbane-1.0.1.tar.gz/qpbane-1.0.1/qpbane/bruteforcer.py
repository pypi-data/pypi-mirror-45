...
def ftpanon(ip,timeout=5):
  try:
    ...
    pass
  return False
def ssh(ip,username='',password='',p=22,timeout=5):
 try:
  s = SSHClient()
...
def mysql(u,username='root',password=''):
 try:
  ...
  pass
 return False
def hydra(u,proto="telnet",p=23,wl=[],logs=True,returning=False,mapping=False,timeout=5,ehlo=False,helo=True,ttls=False,proxy=None,proxies=None):
 '''
   this function is similar to hydra tool to bruteforce attacks on different ports.

   proto: (set by default to: telnet) set the chosen protocol (ftp, ssh, telnet, smtp and mysql) and don't forget to set the port.
'''
 o=''
 if proto=="ftp":
  s=ftp
