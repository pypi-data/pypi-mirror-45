    ...
    def get_zermelo_appointments(self, endpoint, fields):
        ...
        while start_epoch < now_epoch:
            try:
                ...
                print(start_epoch, end_epoch)
                url = '{0}{1}?access_token={2}&fields={3}&start={4}&end={5}&includeHidden=True&cancelled={6}&valid=True'.format(self.url, endpoint, self.access_token, fields, start_epoch, end_epoch, self.get_cancelled_lessons)
                data = requests.get(url).json()['response']['data']
