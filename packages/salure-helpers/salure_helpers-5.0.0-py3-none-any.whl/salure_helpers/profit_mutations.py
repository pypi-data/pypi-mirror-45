...
class ProfitMutations:
    def __init__(self, profit_env, profit_token, input_dir, output_dir):
        ...
        if not os.path.exists(self.output_dir + 'archive'):
            os.makedirs(self.output_dir + 'archive')
        if not os.path.exists(self.output_dir + 'profit'):
            os.makedirs(self.output_dir + 'profit')
        if not os.path.exists(self.output_dir + 'comparison_data'):
            os.makedirs(self.output_dir + 'comparison_data')
    ...
    def get_changed_data(self, filename_old: str, filename_new:str, check_columns: list, unique_key: str):
        """
        ...
        try:
            if os.path.isfile('{dir}{filename}.msgpack'.format(dir=self.input_dir + 'profit/', filename=filename_old)):
                df_old = pd.read_msgpack('{dir}{filename}.msgpack'.format(dir=self.input_dir + 'profit/', filename=filename_old))
            else:
                raise Exception('Old datafile doesn\'t exist')
            if os.path.isfile('{dir}{filename}.msgpack'.format(dir=self.input_dir + 'comparison_data/', filename=filename_new)):
                df_new = pd.read_msgpack('{dir}{filename}.msgpack'.format(dir=self.input_dir + 'comparison_data/', filename=filename_new))
            else:
    ...
    def process_dataset(self, check_columns: list, sheetname: str, filename_old: str, filename_new: str, unique_key: str, changed_or_edited: str, dateformat=None, export_columns=None, column_value_mapping=None, column_mapping=None):
        """
        ...
        """
        try:
            df_mutations = self.get_changed_data(filename_old, filename_new, unique_key=unique_key, check_columns=check_columns)
    ...
    def process_new_employees(self, unique_key: str, filename_old: str, filename_new: str):
        """
        This function is the process trigger to process new employees
        ----------
        :return: nothing
        """
        df_mutations = self.get_changed_data(filename_old, filename_new, check_columns=[unique_key], unique_key=unique_key)
