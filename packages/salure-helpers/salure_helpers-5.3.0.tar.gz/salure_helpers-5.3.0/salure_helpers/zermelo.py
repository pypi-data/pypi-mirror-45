...
class Zermelo():
    ...
    def __init__(self, customer, token, storage_location, initial_zermelo_extract=False, get_cancelled_lessons=False):
        """
        ...
        self.get_cancelled_lessons = get_cancelled_lessons
    ...
    def run_all_extracts(self):
        ...
        self.get_zermelo_cancelled_appointments(endpoint='appointments', fields=['id', 'start', 'end', 'type', 'remark', 'valid', 'cancelled', 'modified',
                                                                       'moved', 'changeDescription', 'branch', 'branchOfSchool', 'created', 'lastModified',
                                                                       'hidden', 'appointmentInstance', 'new', 'teachers', 'students', 'subjects', 'groups',
                                                                       'locations', 'locationsOfBranch', 'groupsInDepartments'])
    ...
    def get_zermelo_cancelled_appointments(self, endpoint, fields):
        start = time.time()
        fields = ','.join(fields)
        ...
        while start_epoch < now_epoch:
            try:
                if (start_epoch +  (3600 * 24 * 2)) > now_epoch:
                    ...
                    end_epoch = int(start_epoch +  (3600 * 24 * 2))
                print(start_epoch, end_epoch)
                url = '{0}{1}?access_token={2}&fields={3}&start={4}&end={5}&includeHidden=True&cancelled=True&valid=True'.format(self.url, endpoint, self.access_token, fields, start_epoch, end_epoch)
                data = requests.get(url).json()['response']['data']
        ...
        df.drop(columns=['students', 'teachers', 'subjects', 'groups', 'locations', 'locationsOfBranch', 'groupsInDepartments'], inplace=True)
        df.index.name = '{0}_id'.format(endpoint)
        file = '{0}{1}.csv'.format(self.storage_location, 'cancelled_appointments')
        df.to_csv(file, '|')
        print('Done in {} seconds'.format(time.time() - start))
