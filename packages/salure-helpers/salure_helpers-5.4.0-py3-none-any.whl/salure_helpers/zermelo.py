...
class Zermelo():
    ...
    def __init__(self, customer, token, storage_location, initial_zermelo_extract=False, cancelled_appointsments='&cancelled=false'):
        """
        ...
        self.cancelled_appointsments = cancelled_appointsments
    ...
    def get_zermelo_appointments(self, endpoint, fields):
        ...
        while start_epoch < now_epoch:
            try:
                ...
                print(start_epoch, end_epoch)
                url = '{0}{1}?access_token={2}&fields={3}&start={4}&end={5}&includeHidden=True{6}&valid=True'.format(self.url, endpoint, self.access_token, fields, start_epoch, end_epoch, self.cancelled_appointsments)
                data = requests.get(url).json()['response']['data']
