...
class Parameters:
    ...
    def parse_command_line(self):
        ...
        parser.add_argument("--version", "-v", action='version', version='%(prog)s 1.2.11',
    ...
    def load_config(self, path_to_config):
        try:
            ...
            sys.exit('Unable to parse the specified config file.')
    ...
    def load_palette(self):
        try:
            ...
            sys.exit('Unable to load palette. Please check the palette path in the config file')
    ...
    def load_triplet_code(self):
        try:
            ...
            sys.exit('Unable to load triplet code. Please check the triplet code path in the config file')
    ...
    def load_font(self):
        try:
            ...
            sys.exit('Unable to load font. Please check the font paths in the config file.')
    ...
    def create_pdf(self):
        try:
            ...
            sys.exit('Unable to create the pdf file. There might be a problem with the \'reportlab\' python package.')
...
class Nt_seq_loader(Loader):
    ...
    def extract_data(self):
        ...
        self.sequence = sequnce_obj.extract()
        self.reverse = 0
        self.coordinates = self.global_parameters['coordinates']
...
class Transcript_struct_data_preparation(Data_preparation):
    ...
    def create_dict(self):
        ...
        adjustable_parameters = []
        for i in self.parameters.gtf['for_transcript']:
            dictonary = dict(coordinates=coordinates, exons=i['exons'], CDS=i['CDS'], strand=i['strand'],
                             name=i['name_of_transcript'], start=i['start_of_transcript'], end=i['end_of_transcript'], transcript_name = i['transcript_name'])
            adjustable_parameters.append(dictonary)
...
class Transcript_struct_loader(Loader):
    ...
    def create_track(self):
                    ...
                    if j < k:
                        ...
                        for i in self.config['transcript_labels']:
                            ...
                            if self.name2 in i:
                                self.name2 = i[1]
                                find2 = 1
                        if find1 == 0 and self.config['transcript_label'] == 'none':
                            self.name1 = ''
                        if find2 == 0 and self.config['transcript_label'] == 'none':
                            self.name2 = ''

                        if self.config['transcript_label'] == 'name':
                            self.name1 = self.adjustable_parameters[j]['transcript_name']
                            self.name2 = self.adjustable_parameters[k]['transcript_name']

                        if self.config['transcript_label'] == 'both':
                        ...
                        if self.config['transcript_label'] == 'none':
                            end_string1 = 0
                            start_string2 = 1
                        ...
                        if start_string2 + width_of_string2 > (self.config['page_width']*cm - 3*self.config['margin']*cm):
                            start_string2 = self.config['page_width']*cm - 5*self.config['margin']*cm - width_of_string2
        ...
        for k in range(len(self.adjustable_parameters)):
            ...
            input_data = dict(exons=exons, transcript_labels = transcript_labels, max_line = max_line, CDS=CDS, coordinates=coordinates, new_line = new_line, transcript=i,
                              delete=delete,
...
class Bedgraph_data_preparation(Data_preparation):
    ...
    def create_dict(self):
        ...
        for i in range(len(self.parameters.config['bedgraph'])):
            ...
            if self.parameters.config['bedgraph_upper_limit'] == 'auto':
                upper_limit = 'd'
            else:
    ...
    def extract_data(self):
        ...
        for i in range(len(self.adjustable_parameters)):
            ...
            with open(self.path_to_bedgraph) as bedgraph:
                ...
                if self.found == 0:
                    if 'chr' in self.global_parameters['scaffold']:
                        self.global_parameters['scaffold'] = self.global_parameters['scaffold'][3::]
                    else:
                        self.global_parameters['scaffold'] = 'chr' + self.global_parameters['scaffold']
                for line in bedgraph:
                    line = line.strip()
                    line = line.split('\t')

                    if line[0] == self.global_parameters['scaffold'] and (
                            int(line[1]) >= self.global_parameters['start']) and (
                            int(line[2]) <= self.global_parameters['end']):
                        pos_of_start.append(int(line[1]) - self.global_parameters['start'])
                        length_of_interval.append(int(line[2]) - int(line[1]))
                        coverage_on_interval.append((line[3]))
    ...
    def extract_data(self):
        ...
        for i in range(len(self.adjustable_parameters)):
            ...
            with open(self.path_to_bedgraph) as bedgraph:
                ...
                for line in bedgraph:
                    ...
                    if line[0] == self.global_parameters['scaffold'] and ((int(self.global_parameters['end']) >= int(line[1]) >= int(self.global_parameters['start'])) or (self.global_parameters['start'] <= int(line[2]) <= self.global_parameters['end'])):
                        if int(line[1]) < self.global_parameters['start']:
                            line[1] = self.global_parameters['start']
                        if int(line[2]) > self.global_parameters['end']:
                            line[2] = self.global_parameters['end']
                ...
                if self.found == 0:
                    if 'chr' in self.global_parameters['scaffold']:
                       ...
                       pass
                       # self.global_parameters1['scaffold'] = 'chr' + self.global_parameters['scaffold']
                for line in bedgraph:
                    line = line.strip()
                    line = line.split('\t')
        ...
        for i in range(len(self.adjustable_parameters)):
            ...
            with open(self.path_to_bedgraph) as bedgraph:
                ...
                for i in range(self.global_parameters['end'] - self.global_parameters['start']):
                    users_region.append(start)
                    start +=1
               # print(self.path_to_bedgraph)
                #print(self.global_parameters['scaffold'])
                #bedgraph_user = self.global_parameters['scaffold'] +  '\t' + str(self.global_parameters['start']) + '\t' + str(self.global_parameters['end']) + '\t' + '0'
               # a = pybedtools.BedTool(bedgraph_user, from_string =  True)
               # b = pybedtools.BedTool(bedgraph.read(), from_string= True)
                #print(a.intersect(b))

                #a = pybedtools.example_bedtool(bedgraph_user)
                #b = pybedtools.example_bedtool('./'+self.path_to_bedgraph)
                #c = b.intersect(bedgraph_user)
                #print(c)
                ...
                for line in bedgraph:
                    ...
                    if line[0] == self.global_parameters['scaffold'] and ((int(self.global_parameters['end']) >= int(line[1]) >= int(self.global_parameters['start'])) or (self.global_parameters['start'] <= int(line[2]) <= self.global_parameters['end'])):
                        ...
                        bed_region = []
                        for i in range(end - start):
                            bed_region.append(start1)
                            start1 += 1
                        if len(set(users_region) & set(bed_region)) != 0 :
                        '''
                        if int(line[1]) < self.global_parameters['start']:
                                line[1] = self.global_parameters['start']
                        if int(line[2]) > self.global_parameters['end']:
...
class Image:
    ...
    def highlighting_the_frame(self):
        try:
            ...
            sys.exit(
                'Unable to highlight the requested frame. Please check the entered data and the installation of the required modules (see the program manual).')
    ...
    def save(self):
        try:
            ...
            sys.exit('Unable to save the pdf file. There might be a problem with the \'reportlab\' python package.')
...
class Vgrid_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Vgrid track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Title_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Image label track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Axis_tics_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Axis tics track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Nt_seq_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Nucleotide sequence track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Transcript_struct_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Transcript structure track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Bedgraph_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Bedgraph track. Please check the entered data and the installation of the required modules (see the program manual).')
...
class Aa_seq_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                "Unable to create an Aminoacid sequence track. Please check the entered data and the installation of the required modules (see the program manual). It can be problem with generating or loading fasta sequence file. Please check if 'bedtools' software is installed.")
...
class Genomic_intervals_tracks_maker(Tracks_maker):
    ...
    def create_tracks(self):
        try:
            ...
            sys.exit(
                'Unable to create a Genomic intervals track. Please check the entered data and the installation of the required modules (see the program manual).')
