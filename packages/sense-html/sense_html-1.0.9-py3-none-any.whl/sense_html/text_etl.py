...
def remove_general(text):
    ...
    keywords = ['证券简称', '公告编号', '虚假记载', '重大遗漏', '【活动报名】',
                '免责声明', '仅代表', '如有侵权', '采编', '证券之星', '查看更多', '未经许可，请勿转载']
    for word in keywords:
        if word in text:
            return True
    return False
...
def remove_handler(text):
    lines = ['责任编辑：', '返回搜狐，查看更多', '网易科技讯', '关注同花顺财经（ths518），获取更多机会']
    for line in lines:
...
def remove_src(text):
    ...
    """
    text = str(text).strip()
    keywords = ['讯：', '讯:', '讯 ', '讯，', '消息 ', '消息:', '消息：', '报道：', '报道:', '报道 ', '消息，', '日电 ']
    for word in keywords:
        if text.find(word) != -1:
...
def get_text(html):
    try:
        if '<p>' in html:
            ...
            p_node_list = response.xpath("//p | //span")
            if len(p_node_list) <= 1:
                return re.sub('<.*?>', '', html)
            text = ''
            for p_node in p_node_list:
                line = ''.join([text.strip() for text in p_node.xpath("string(.)").split('\n')])
                text += line
    ...
    try:
        for i in range(len(context_list)):
            ...
            if is_short_line(context_list[i], threshold):
                continue
            if has_end_punc(context_list[i]):
                continue
