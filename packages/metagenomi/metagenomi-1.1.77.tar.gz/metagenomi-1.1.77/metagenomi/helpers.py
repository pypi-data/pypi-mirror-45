...
def in_db(mgid, db):
    response = db.table.query(
                        KeyConditionExpression=Key('mgid').eq(mgid)
                        )
...
def to_datetime(d):
    if d == 'not applicable' or d == 'None' or d is None or d == 'NA' or d == 'missing':
        return 'None'
    ...
    try:
        ...
        # Sometimes there are lists of dates
        if ',' in d:
            d = d.split(',')
            d = d[0]
        ...
        try:
            d = datetime.datetime.strptime(d, input_format)
            return str(d.strftime(TIME_FMT))
...
def to_latlon(latlon, country=None):
    ...
    # print(f'Converting latlon. Input = {latlon}')
