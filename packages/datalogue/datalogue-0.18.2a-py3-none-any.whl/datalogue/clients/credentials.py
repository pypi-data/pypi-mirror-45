...
class _CredentialsClient:
    """
    Client to interact with the Storage objects
    """
    ...
    def list(self, store_type: Optional[str] = None, page: int = 1, item_per_page: int = 25) -> Union[
        DtlError, List[Datastore]]:
        """
        Lists available credentials that user has Use Permission

        :param store_type: type of the storage (S3, GCS, Jdbc, etc)
        :param page: page to be retrieved
        :param item_per_page: number of items to be put in a page
        :return: Returns a List of all the available storages or an error message as a string
        """

        res = self.http_client.make_authed_request(
            self.service_uri +
            f"/credentials?page={page}&size={item_per_page}{'' if store_type is None else f'&type={store_type}'}",
            HttpMethod.GET)
...
class _AdminCredentialsClient:
    ...
    def update(self, credentials_id: UUID, credentials_definition: Credentials,
        ...
        TODO rework the api so that you can only update the name without the Storage themselves
    ...
    def list(self, store_type: Optional[str] = None, page: int = 1, item_per_page: int = 25) \
            -> Union[DtlError, List[CredentialsReference]]:
        """
        Lists available storages that Organization has
        ...
        res = self.http_client.make_authed_request(
            self.service_uri +
            f"/credentials?page={page}&size={item_per_page}{'' if store_type is None else f'&type={store_type}'}", HttpMethod.GET)
