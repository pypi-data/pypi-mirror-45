...
class DefaultLearner(object):
    ...
    def __init__(self, model, outputs, optimizer, loss_name="loss"):
        ...
        self._update_opt = optimizer.minimize(outputs[loss_name])
    ...
    def epoch_fit_loop(self, inputs, outputs):
        """
        ...
        try:
            while True:
                ...
                batch_outs = self.learn_step(inputs, outputs)
                # # accumulate outputs
    ...
    def learn_step(self, inputs, outputs):
        try:
    ...
    def fit(self, inputs):
        ...
        for epoch_i in range(int(self.model.config["FLOW.N_EPOCHS"])):
            ...
            # epoch begin trigger
            accumulators = self.epoch_fit_loop(inputs, self.outputs)
