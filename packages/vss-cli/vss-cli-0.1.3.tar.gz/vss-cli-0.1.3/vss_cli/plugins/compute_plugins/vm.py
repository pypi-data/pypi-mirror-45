...
def compute_vm_set_nic_up(
    ...
    if network:
        ...
        net = ctx.get_network_by_name_or_moref(
            network
        )
        attr = 'network'
        value = net[0]['moref']
...
disks_opt = click.option(
    ...
    type=click.INT, multiple=True, required=False
)
...
high_io_opt = click.option(
    '--high-io', '-h',
    help='VM will be created with a '
