...
class IonDevice(devices.Device):
    u"""A device with qubits placed on a line.
    ...
    def __init__(self,
                 ...
                 oneq_gates_duration,
                 qubits):
        u"""Initializes the description of an ion trap device.
        ...
        if isinstance(scheduled_operation.operation.gate, ops.XXPowGate):
            for other in schedule.operations_happening_at_same_time_as(
                ...
                if self._check_if_XXPow_operation_interacts(
                    ...
                    raise ValueError(
                        u'Simultaneous two-qubit '
                        u'operations on same qubit: {} vs {}.'.format(
                            scheduled_operation, other))

    def validate_circuit(self, circuit):
        super(IonDevice, self).validate_circuit(circuit)
    ...
    def can_add_operation_into_moment(self,
        ...
        if not super(IonDevice, self).can_add_operation_into_moment(operation, moment):
            return False
    ...
    def __repr__(self):
        return (u'IonDevice(measurement_duration={!r}, '
                u'twoq_gates_duration={!r}, '
                u'oneq_gates_duration={!r} '
...
IonDevice = value.value_equality(IonDevice)
