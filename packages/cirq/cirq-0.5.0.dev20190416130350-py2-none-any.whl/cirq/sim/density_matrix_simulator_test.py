...
def test_invalid_dtype():
    with pytest.raises(ValueError, match=u'complex'):
        cirq.DensityMatrixSimulator(dtype=np.int32)


@pytest.mark.parametrize(u'dtype', [np.complex64, np.complex128])
...
def test_run_mixture(dtype):
    ...
    np.testing.assert_equal(result.measurements[u'1'], [[0]] * 100)
    # Test that we get at least one of each result. Probability of this test
    # failing is 2 ** (-99).
    q0_measurements = set(x[0] for x in result.measurements[u'0'].tolist())
    assert q0_measurements == set([0, 1])
...
def test_run_repetitions_measure_at_end(dtype):
    q0, q1 = cirq.LineQubit.range(2)
    simulator = cirq.DensityMatrixSimulator(dtype=dtype)
    with mock.patch.object(simulator, u'_base_iterator',
...
def test_run_param_resolver(dtype):
    ...
    for b0 in [0, 1]:
        for b1 in [0, 1]:
            circuit = cirq.Circuit.from_ops((cirq.X**sympy.Symbol(u'b0'))(q0),
                                            (cirq.X**sympy.Symbol(u'b1'))(q1),
                                            cirq.measure(q0),
                                            cirq.measure(q1))
            param_resolver = {u'b0': b0, u'b1': b1}
...
def test_run_correlations(dtype):
    ...
    for _ in xrange(10):
        result = simulator.run(circuit)
        bits = result.measurements[u'0,1'][0]
...
def test_run_ignore_displays(dtype):
    ...
    result = simulator.run(circuit)
    assert result.measurements[u'0'] == [[True]]
    ...
    for b0 in [0, 1]:
        for b1 in [0, 1]:
            circuit = cirq.Circuit.from_ops((cirq.X**sympy.Symbol(u'b0'))(q0),
                                            ...
                                            cirq.measure(q0),
                                            cirq.measure(q1))
            params = [cirq.ParamResolver({u'b0': b0, u'b1': b1}),
                      cirq.ParamResolver({u'b0': b1, u'b1': b0})]
...
def test_simulate_bit_flips(dtype):
    ...
    for b0 in [0, 1]:
        for b1 in [0, 1]:
            ...
            np.testing.assert_equal(result.measurements, {u'0': [b0], u'1': [b1]})
...
def test_simulate_param_resolver(dtype):
    ...
    for b0 in [0, 1]:
        for b1 in [0, 1]:
            circuit = cirq.Circuit.from_ops((cirq.X**sympy.Symbol(u'b0'))(q0),
                                            (cirq.X**sympy.Symbol(u'b1'))(q1))
            resolver = cirq.ParamResolver({u'b0': b0, u'b1': b1})
...
def test_simulate_moment_steps_intermediate_measurement(dtype):
    ...
    for i, step in enumerate(simulator.simulate_moment_steps(circuit)):
        if i == 1:
            result = int(step.measurements[u'0'][0])
...
def test_density_matrix_simulator_state_repr():
    ...
    assert (repr(cirq.DensityMatrixSimulatorState(
        density_matrix=np.ones((2, 2)) * 0.5, qubit_map={q0: 0}))
            == u"cirq.DensityMatrixSimulatorState(density_matrix="
               u"np.array([[0.5, 0.5], [0.5, 0.5]]), "
               u"qubit_map={cirq.LineQubit(0): 0})")
...
def test_density_matrix_trial_result_repr():
    ...
    assert (repr(cirq.DensityMatrixTrialResult(
        ...
        measurements={u'm': np.array([[1]])},
        final_simulator_state=final_simulator_state)) ==
            u"cirq.DensityMatrixTrialResult("
            u"params=cirq.ParamResolver({'s': 1}), "
            u"measurements={'m': array([[1]])}, "
            u"final_simulator_state=cirq.DensityMatrixSimulatorState("
                u"density_matrix=np.array([[0.5, 0.5], [0.5, 0.5]]), "
                u"qubit_map={cirq.LineQubit(0): 0}))"u"")
...
def test_compute_displays(dtype):
    ...
    np.testing.assert_allclose(result.display_values[u'z3'], 1, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'z0z1'], -1, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'z0x1'], 0, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'z1x2'], -1, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'x0z1'], 0, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'x3'], -1, atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'approx_z1x2'], -1,
                               atol=1e-7)
...
def test_compute_samples_displays(dtype):
    ...
    assert u'x3' not in result.display_values
    np.testing.assert_allclose(result.display_values[u'approx_z1x2'], -1,
                               atol=1e-7)
    np.testing.assert_allclose(result.display_values[u'approx_z1x3'], 1,
                               atol=1e-7)
