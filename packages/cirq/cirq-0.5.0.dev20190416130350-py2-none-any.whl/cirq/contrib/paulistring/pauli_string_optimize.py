...
def pauli_string_optimized_circuit(circuit,
                                   move_cliffords = True,
                                   atol = 1e-8
                                   ):
...
def assert_no_multi_qubit_pauli_strings(circuit):
    for op in circuit.all_operations():
        if isinstance(op, PauliStringGateOperation):
            assert len(op.pauli_string) == 1, (
                   u'Multi qubit Pauli string left over')


def merge_equal_strings(string_dag):
...
def remove_negligible_strings(string_dag,
                              atol=1e-8):
    for node in tuple(string_dag.nodes()):
