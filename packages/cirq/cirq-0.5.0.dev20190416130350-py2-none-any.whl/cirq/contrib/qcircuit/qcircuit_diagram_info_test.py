...
def test_get_qcircuit_diagram_info():
    qubits = cirq.NamedQubit(u'x'), cirq.NamedQubit(u'y')
    ...
    name = ur'{\text{SWAP}^{0.5}}'
    expected_info = cirq.CircuitDiagramInfo(
            (ur'\multigate{1}' + name, ur'\ghost' + name),
            exponent=0.5,
            connected=False)
    ...
    gate = cirq.SWAP
    op = gate(*qubits)
    qubit_map = dict((q, i) for q, i in izip(qubits, (4, 3)))
    args = cirq.CircuitDiagramInfoArgs(
            known_qubits=qubits,
            known_qubit_count=None,
            use_unicode_characters=True,
    ...
    actual_info = ccq.get_qcircuit_diagram_info(op, args)
    expected_info = cirq.CircuitDiagramInfo(
            (ur'\ghost{\text{SWAP}}', ur'\multigate{1}{\text{SWAP}}'),
            connected=False)
    assert actual_info == expected_info

    qubit_map = dict((q, i) for q, i in izip(qubits, (2, 5)))
    args = cirq.CircuitDiagramInfoArgs(
            known_qubits=qubits,
            known_qubit_count=None,
            use_unicode_characters=True,
            ...
            qubit_map=qubit_map)
    actual_info = ccq.get_qcircuit_diagram_info(op, args)
    expected_info = cirq.CircuitDiagramInfo((ur'\gate{\text{swap}}',) * 2)
    assert actual_info == expected_info
