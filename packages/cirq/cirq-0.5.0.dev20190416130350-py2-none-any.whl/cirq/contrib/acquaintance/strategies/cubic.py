...
def skip_and_wrap_around(items):
    n_items = len(items)
    positions = dict((p, i) for i, p in enumerate(itertools.chain(
        xrange(0, n_items, 2), reversed(xrange(1, n_items, 2)))))
    return tuple(items[positions[i]] for i in xrange(n_items))
...
def cubic_acquaintance_strategy(
    ...
    u"""Acquaints every triple of qubits.
    ...
    moments = []
    index_order = tuple(xrange(n_qubits))
    max_separation = max(((n_qubits - 1) // 2) + 1, 2)
    for separation in xrange(1, max_separation):
        stepped_indices_concatenated = tuple(itertools.chain(*(
                xrange(offset, n_qubits, separation)
                for offset in xrange(separation))))
        new_index_order = skip_and_wrap_around(stepped_indices_concatenated)
        permutation = dict((i, new_index_order.index(j))
