...
class AcquaintanceDevice(devices.Device):
    __metaclass__ = abc.ABCMeta
    u"""A device that contains only acquaintance and permutation gates.
    ...
    def validate_operation(self, operation):
        if not (isinstance(operation, ops.GateOperation) and
            ...
            raise ValueError(
                    u'not (isinstance({0!r}, {1!r}) and '
                          u'ininstance({0!r}.gate, {2!r})'.format(
...
def get_acquaintance_size(obj):
    ...
    if isinstance(obj.gate, SwapNetworkGate):
        ...
        if (obj.gate.acquaintance_size - 1) in obj.gate.part_lens:
            return obj.gate.acquaintance_size
    sizer = getattr(obj.gate, u'_acquaintance_size_', None)
    return 0 if sizer is None else sizer(len(obj.qubits))

class _UnconstrainedAcquaintanceDevice(AcquaintanceDevice):
    u"An acquaintance device with no constraints other than of the gate types."

    def __repr__(self):
        return u'UnconstrainedAcquaintanceDevice'  # coverage: ignore
