...
u"""Protocol and methods for quantum channels."""
...
class SupportsChannel(Protocol):
    u"""An object that may be describable as a quantum channel."""

    def _channel_(self):
        ur"""A list of matrices describing the quantum channel.
    ...
    def _has_channel_(self):
        u"""Whether this value has a channel representation.
...
def channel(val,
    ...
    ur"""Returns a list of matrices describing the channel for the given value.
    ...
    Raises:
        TypeError: `val` doesn't have a _channel_ or _unitary_ method (or that
            ...
            specified.
    """
    channel_getter = getattr(val, u'_channel_', None)
    channel_result = (
    ...
    if (channel_getter is None and unitary_getter is None
        ...
        raise TypeError(u"object of type '{}' has no _channel_ or _mixture_ or "
                        u"_unitary_ method.".format(type(val)))
    raise TypeError(u"object of type '{}' does have a _channel_, _mixture_ or "
                u"_unitary_ method, but it returned NotImplemented."
                .format(type(val)))


def has_channel(val):
    u"""Returns whether the value has a channel representation.
    ...
    Returns:
        ...
        exists.
    """
    channel_getter = getattr(val, u'_has_channel_', None)
