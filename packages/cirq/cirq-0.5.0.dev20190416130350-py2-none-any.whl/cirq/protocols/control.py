...
def control(controllee,
            control_qubits = None,
            default = RaiseTypeErrorIfNotProvided):
    u"""Returns a Controlled version of the given value, if defined.
    ...
    if control_qubits is None:
        control_qubits = []
    controller = getattr(controllee, u'controlled_by', None)
    ...
    if controller is None:
        raise TypeError(u"object of type '{}' has no controlled_by "
                        u"method.".format(type(controllee)))
    raise TypeError(u"object of type '{}' does have a controlled_by method, "
                    u"but it returned NotImplemented.".format(type(controllee)))
