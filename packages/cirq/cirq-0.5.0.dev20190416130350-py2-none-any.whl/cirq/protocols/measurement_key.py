...
u"""Protocol for object that have measurement keys."""
...
class SupportsMeasurementKey(Protocol):
    ur"""An object that is a measurement and has a measurement key.
    ...
    def _measurement_key_(self):
        u"""Return the key that will be used to identify this measurement.
...
def measurement_key(
    ...
    u"""Get the measurement key for the given value.
    ...
    """
    getter = getattr(val, u'_measurement_key_', None)
    ...
    if getter is None:
        raise TypeError(
                u"object of type '{}' has no _measurement_key_ method."
                    .format(type(val)))

    raise TypeError(u"object of type '{}' does have a _measurement_key_ method, "
                    u"but it returned NotImplemented.".format(type(val)))


def is_measurement(val):
    u"""Returns whether or not the given value is a measurement.
