...
class BucketPriorityQueue(Generic[TItem]):
    u"""A priority queue for when priorities are integers over a small range.
    ...
    def __init__(self,
                 entries = (), **_3to2kwargs):
        if 'drop_duplicate_entries' in _3to2kwargs: drop_duplicate_entries = _3to2kwargs['drop_duplicate_entries']; del _3to2kwargs['drop_duplicate_entries']
        else: drop_duplicate_entries = False
        u"""Initializes a new priority queue.
    ...
    @property
    def drop_duplicate_entries(self):
        return self._drop_set is not None

    def __nonzero__(self):
        u"""Returns whether or not the priority queue contains any items."""
        return bool(self._len)

    def __len__(self):
        u"""Returns how many items are in the priority queue."""
        return self._len

    def __iter__(self):
        u"""Iterates the (priority, item) entries in the queue."""
        for i, bucket in enumerate(self._buckets):
            for item in bucket:
                yield i + self._offset, item

    def enqueue(self, priority, item):
        u"""Adds an entry to the priority queue.
        ...
        if i < 0:
            self._buckets[:0] = [[] for _ in xrange(-i)]
            self._offset = priority
            i = 0
    ...
    def dequeue(self):
        u"""Removes and returns an item from the priority queue.
        ...
        """
        if self._len == 0:
            raise ValueError(u'BucketPriorityQueue is empty.')
    ...
    def __unicode__(self):
        ...
        return u'BucketPriorityQueue {' + _indent(lines) + u'\n}'

    def __repr__(self):
        return u'{}(entries={!r}, drop_duplicate_entries={!r})'.format(
            u'cirq.circuits._bucket_priority_queue.BucketPriorityQueue',
            list(self),
...
def _indent(lines):
    paragraph = u''.join(u'\n' + unicode(line) for line in lines)
    return paragraph.replace(u'\n', u'\n    ')
