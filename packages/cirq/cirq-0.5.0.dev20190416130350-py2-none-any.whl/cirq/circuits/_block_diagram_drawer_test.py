...
def _assert_same_diagram(actual, expected):
    assert actual == expected, (
        u"Diagram differs from the desired diagram.\n"
        u'\n'
        u'Actual diagram:\n'
        u'{}\n'
        u'\n'
...
def _curve_pieces_diagram(chars):
...
def test_mixed_block_curve():
    ...
    for a, b, c, d in itertools.product(xrange(3), repeat=4):
        x = (a * 3 + b) * 2
        y = (c * 3 + d) * 2
...
def test_lines_meet_content():
    ...
    b.content = u'long text\nwith multiple lines'
    b.left = u'>'
    b.right = u'<'
    b.top = u'v'
    b.bottom = u'^'

    _assert_same_diagram(d.render(), u"""
long text<<<<<<<<<<
...
with multiple lines"""[1:])
    ...
    _assert_same_diagram(d.render(min_block_height=5), u"""
         v
         v
>>>>>long text<<<<<
with multiple lines
    ...
    _assert_same_diagram(d.render(min_block_height=4), u"""
         v
>>>>>long text<<<<<
with multiple lines
         ^"""[1:])

    _assert_same_diagram(d.render(min_block_height=20, min_block_width=40), u"""
                   v
                   v
                   v
                   v
...
>>>>>>>>>>>>>>>long text<<<<<<<<<<<<<<<<
    ...
    _assert_same_diagram(d.render(min_block_height=21, min_block_width=41), u"""
                    v
                    v
                    v
                    v
...
def test_content_stretches_other_blocks():
    ...
    d.mutable_block(0, 0).content = u'long text\nwith multiple lines'
...
def test_lines_stretch_content():
    d = BlockDiagramDrawer()
    d.mutable_block(0, 0).left = u'l'
    d.mutable_block(2, 4).right = u'r'
    d.mutable_block(11, 15).bottom = u'b'
    d.mutable_block(16, 17).top = u't'
    d.mutable_block(19, 20).center = u'c'
    d.mutable_block(21, 23).content = u'C'
    _assert_same_diagram(d.render(), u"""
