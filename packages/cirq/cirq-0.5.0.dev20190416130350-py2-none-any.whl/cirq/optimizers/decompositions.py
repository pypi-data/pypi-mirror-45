...
def is_negligible_turn(turns, tolerance):
...
def single_qubit_matrix_to_pauli_rotations(
    ...
    u"""Implements a single-qubit operation with few rotations.
...
def single_qubit_matrix_to_gates(
    ...
    u"""Implements a single-qubit operation with few gates.
...
def single_qubit_op_to_framed_phase_form(
        mat):
    u"""Decomposes a 2x2 unitary M into U^-1 * diag(1, r) * U * diag(g, g).
...
def single_qubit_matrix_to_phased_x_z(
    ...
    u"""Implements a single-qubit operation with a PhasedX and Z gate.
