...
def assert_optimizes(before, expected):
    ...
    assert actual == expected, u'ACTUAL {} : EXPECTED {}'.format(actual,
                                                                expected)

def assert_optimization_not_broken(circuit):
    u"""Check that the unitary matrix for the input circuit is the same (up to
...
def test_ignores_czs_separated_by_parameterized():
    ...
    assert_optimizes(
        before=cirq.Circuit([
            ...
            cirq.Moment([cirq.Z(a)**sympy.Symbol(u'boo')]),
            cirq.Moment([cirq.CZ(a, b)]),
        ]),
        expected=cirq.Circuit([
            cirq.Moment([cirq.CZ(a, b)]),
