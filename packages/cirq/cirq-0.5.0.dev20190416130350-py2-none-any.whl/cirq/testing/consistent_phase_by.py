...
def assert_phase_by_is_consistent_with_unitary(val):
    u"""Uses `val._unitary_` to check `val._phase_by_`'s behavior."""
    ...
    for t in [0.125, -0.25, 1]:
        ...
        for i in xrange(qubit_count):
            ...
            lin_alg_utils.assert_allclose_up_to_global_phase(
                ...
                expected,
                atol=1e-8,
                err_msg=u'Phased unitary was incorrect for index #{}'.format(i))
