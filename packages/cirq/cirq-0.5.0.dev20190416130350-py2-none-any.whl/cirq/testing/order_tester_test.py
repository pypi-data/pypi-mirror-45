...
class MockValue(object):
    ...
    def __repr__(self):
        return u'MockValue(val={!r}, ...)'.format(self.val)
...
def test_add_ordering_group_correct():
    ...
    ot.add_ascending_equivalence_group(float(u'inf'), float(u'inf'))
...
def test_propagates_internal_errors():
    ...
    ot = cirq.testing.OrderTester()
    with pytest.raises(ValueError, match=u'oh no'):
        ot.add_ascending(UnorderableClass())


def test_add_ascending_equivalence_group():
    ot = cirq.testing.OrderTester()
    with pytest.raises(AssertionError, match=u'Expected X=1 to equal Y=3'):
    ...
    with pytest.raises(AssertionError,
                       match=u'Expected X=4 to be less than Y=3'):
...
def test_fails_to_return_not_implemented_vs_unknown():
    ...
    def make_impls(bad_index, bad_result):
        def make_impl(i, op):
            def impl(x, y):
    ...
    ot = cirq.testing.OrderTester()
    for k in xrange(len(CMP_OPS)):
        for b in [False, True]:
            item = MockValue(0, *make_impls(bad_index=k, bad_result=b))
            with pytest.raises(AssertionError, match=u'return NotImplemented'):
...
def test_fails_on_inconsistent_hashes():
    class ModifiedHash(tuple):
        def __hash__(self):
            return super(ModifiedHash, self).__hash__() ^ 1
