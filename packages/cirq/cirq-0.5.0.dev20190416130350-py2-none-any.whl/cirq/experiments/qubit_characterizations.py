...
Cliffords = NamedTuple(u'Cliffords',
                       [(u'c1_in_xy', List[List[ops.Gate]]),
                        (u'c1_in_xz', List[List[ops.Gate]]),
                        (u's1', List[List[ops.Gate]]),
                        (u's1_x', List[List[ops.Gate]]),
                        (u's1_y', List[List[ops.Gate]])])


class RabiResult(object):
    u"""Results from a Rabi oscillation experiment."""

    def __init__(self, rabi_angles,
    ...
    @property
    def data(self):
        u"""Returns a sequence of tuple pairs with the first item being a Rabi
        angle and the second item being the corresponding excited state
        probability.
        """
        return [(angle, prob) for angle, prob in izip(self._rabi_angles,
                                                     self._excited_state_probs)]

    def plot(self, **plot_kwargs):
        u"""Plots excited state probability vs the Rabi angle (angle of rotation
        around the x-axis).
        ...
        """
        fig = plt.figure()
        plt.plot(self._rabi_angles, self._excited_state_probs, u'ro-',
                 figure=fig, **plot_kwargs)
        plt.xlabel(ur"Rabi Angle (Radian)", figure=fig)
        plt.ylabel(u'Excited State Probability', figure=fig)
        fig.show()


class RandomizedBenchMarkResult(object):
    u"""Results from a randomized benchmarking experiment."""

    def __init__(self, num_cliffords,
    ...
    @property
    def data(self):
        u"""Returns a sequence of tuple pairs with the first item being a
        number of Cliffords and the second item being the corresponding average
        ground state probability.
        """
        return [(num, prob) for num, prob in izip(self._num_cfds_seq,
                                                 self._gnd_state_probs)]

    def plot(self, **plot_kwargs):
        u"""Plots the average ground state probability vs the number of
        ...
        """
        fig = plt.figure()
        plt.plot(self._num_cfds_seq, self._gnd_state_probs, u'ro-',
                 figure=fig, **plot_kwargs)
        plt.xlabel(ur"Number of Cliffords", figure=fig)
        plt.ylabel(u'Ground State Probability', figure=fig)
        fig.show()


class TomographyResult(object):
    u"""Results from a state tomography experiment."""
    ...
    @property
    def data(self):
        u"""Returns an n^2 by n^2 complex matrix representing the density
        matrix of the n-qubit system.
        """
        return self._density_matrix

    def plot(self):
        u"""Plots the real and imaginary parts of the density matrix as two
        3D bar plots.
        """
...
def rabi_oscillations(sampler, qubit,
                      max_angle = 2 * np.pi, **_3to2kwargs):
    if 'num_points' in _3to2kwargs: num_points = _3to2kwargs['num_points']; del _3to2kwargs['num_points']
    else: num_points =  200
    if 'repetitions' in _3to2kwargs: repetitions = _3to2kwargs['repetitions']; del _3to2kwargs['repetitions']
    else: repetitions =  1000
    u"""Runs a Rabi oscillation experiment.
    ...
    """
    theta = sympy.Symbol(u'theta')
    circuit = circuits.Circuit.from_ops(ops.X(qubit) ** theta)
    circuit.append(ops.measure(qubit, key=u'z'))
    sweep = study.Linspace(key=u'theta', start=0.0, stop=max_angle / np.pi,
                           length=num_points)
    ...
    for i in xrange(num_points):
        excited_state_probs[i] = np.mean(results[i].measurements[u'z'])

    return RabiResult(angles, excited_state_probs)


def single_qubit_randomized_benchmarking(sampler,
                                         qubit,
                                         use_xy_basis = True, **_3to2kwargs
                                         ):
    if 'repetitions' in _3to2kwargs: repetitions = _3to2kwargs['repetitions']; del _3to2kwargs['repetitions']
    else: repetitions =  1000
    if 'num_circuits' in _3to2kwargs: num_circuits = _3to2kwargs['num_circuits']; del _3to2kwargs['num_circuits']
    else: num_circuits =  20
    if 'num_clifford_range' in _3to2kwargs: num_clifford_range = _3to2kwargs['num_clifford_range']; del _3to2kwargs['num_clifford_range']
    else: num_clifford_range =  xrange(10, 100, 10)
    u"""Clifford-based randomized benchmarking (RB) of a single qubit.
...
def two_qubit_randomized_benchmarking(sampler,
    ...
    if 'num_clifford_range' in _3to2kwargs: num_clifford_range = _3to2kwargs['num_clifford_range']; del _3to2kwargs['num_clifford_range']
    else: num_clifford_range =  xrange(5, 50, 5)
    u"""Clifford-based randomized benchmarking (RB) of two qubits.
    ...
    for num_cfds in num_clifford_range:
        ...
        for _ in xrange(num_circuits):
            ...
            circuit.append(ops.measure(first_qubit, second_qubit, key=u'z'))
            results = sampler.run(circuit, repetitions=repetitions)
            gnds = [(not r[0] and not r[1]) for r in results.measurements[u'z']]
...
def single_qubit_state_tomography(sampler,
                                  qubit,
                                  circuit,
                                  repetitions = 1000):
    u"""Single-qubit state tomography.
...
def two_qubit_state_tomography(sampler,
                               first_qubit,
                               second_qubit,
                               circuit,
                               repetitions = 1000):
    ur"""Two-qubit state tomography.
    ...
    def _measurement(two_qubit_circuit):
        ...
        results_hist = results.histogram(key=u'z')
...
def _two_qubit_clifford_matrices(q_0,
                                 q_1,
                                 cliffords
                                 ):
    mats = []
    ...
    for i in xrange(clifford_group_size):
...
def _random_single_q_clifford(qubit, num_cfds,
                              cfds,
                              cfd_matrices):
...
def _random_two_q_clifford(q_0, q_1,
...
def _find_inv_matrix(mat, mat_sequence):
    mat_prod = np.einsum(u'ij,...jk->...ik', mat, mat_sequence)
    diag_sums = list(np.absolute(np.einsum(u'...ii->...', mat_prod)))
    idx = diag_sums.index(max(diag_sums))
    return idx


def _matrix_bar_plot(mat, z_label, fig,
                     plt_position, kets = None,
                     title = None):
    num_rows, num_cols = mat.shape
    indices = np.meshgrid(xrange(num_cols), xrange(num_rows))
    ...
    ax1 = fig.add_subplot(plt_position, projection=u'3d')  # type: Axes3D

    dz = mat.flatten()
    ax1.bar3d(x_indices, y_indices, z_indices, dx, dy, dz, color=u'#ff0080',
              alpha=1.0)
...
def _plot_density_matrix(mat):
    a, _ = mat.shape
    num_qubits = int(np.sqrt(a))
    state_labels = [[0, 1]] * num_qubits
    kets = []
    for label in itertools.product(*state_labels):
        kets.append(u'|' + unicode(list(label))[1:-1] + u'>')
    mat_re = np.real(mat)
    mat_im = np.imag(mat)
    fig = plt.figure(figsize=(12.0, 5.0))
    _matrix_bar_plot(mat_re, ur'Real($\rho$)', fig, 121, kets,
                     u'Density Matrix (Real Part)')
    _matrix_bar_plot(mat_im, ur'Imaginary($\rho$)', fig, 122, kets,
                     u'Density Matrix (Imaginary Part)')
    return fig


def _gate_seq_to_mats(gate_seq):
...
def _two_qubit_clifford(q_0, q_1,
                        idx,
                        cliffords):
    u"""Generates a two-qubit Clifford gate.
...
def _single_qubit_gates(gate_seq,
                        qubit):
