...
def test_from_proto_bad_dict():
    with pytest.raises(ValueError):
        cirq.GridQubit.from_proto_dict({u'row': 1})
    with pytest.raises(ValueError):
        cirq.GridQubit.from_proto_dict({u'col': 1})
    with pytest.raises(ValueError):
        cirq.GridQubit.from_proto_dict({})
    with pytest.raises(ValueError):
        cirq.GridQubit.from_proto_dict({u'nothing': 1})
