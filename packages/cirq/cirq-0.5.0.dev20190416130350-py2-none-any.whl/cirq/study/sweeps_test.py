...
def test_product_duplicate_keys():
    with pytest.raises(ValueError):
        _ = cirq.Linspace(u'a', 0, 9, 10) * cirq.Linspace(u'a', 0, 10, 11)
...
def test_linspace():
    sweep = cirq.Linspace(u'a', 0.34, 9.16, 7)
    assert len(sweep) == 7
...
def test_linspace_one_point():
    sweep = cirq.Linspace(u'a', 0.34, 9.16, 1)
    assert len(sweep) == 1
    params = list(sweep.param_tuples())
    assert len(params) == 1
    assert params[0] == ((u'a', 0.34),)
...
def test_zip():
    sweep = cirq.Points(u'a', [1, 2, 3]) + cirq.Points(u'b', [4, 5, 6, 7])
    assert len(sweep) == 3
...
def test_equality():
    ...
    et.make_equality_group(lambda: cirq.Linspace(u'a', 0, 10, 11))
    et.make_equality_group(lambda: cirq.Linspace(u'b', 0, 10, 11))
    et.make_equality_group(lambda: cirq.Points(u'a', range(11)))
    et.make_equality_group(lambda: cirq.Points(u'b', range(11)))

    # Product and Zip sweeps can also be equated.
    et.make_equality_group(
        lambda: cirq.Linspace(u'a', 0, 5, 6) * cirq.Linspace(u'b', 10, 15, 6))
    et.make_equality_group(
        lambda: cirq.Linspace(u'a', 0, 5, 6) + cirq.Linspace(u'b', 10, 15, 6))
    et.make_equality_group(
        lambda: cirq.Points(u'a', [1, 2]) *
                     (cirq.Linspace(u'b', 0, 5, 6) +
                      cirq.Linspace(u'c', 10, 15, 6)))


def test_repr():
    assert repr(cirq.study.sweeps.Product(cirq.UnitSweep)) == \
        u'cirq.study.sweeps.Product(cirq.UnitSweep)'
    assert repr(cirq.study.sweeps.Zip(cirq.UnitSweep)) == \
        u'cirq.study.sweeps.Zip(cirq.UnitSweep)'
