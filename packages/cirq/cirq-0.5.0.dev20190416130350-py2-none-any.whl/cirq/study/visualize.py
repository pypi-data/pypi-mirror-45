...
u"""Tool to visualize the results of a study."""
...
def plot_state_histogram(result):
    u"""Plot the state histogram from a single result with repetitions.
    ...
    for meas in measurement_by_result:
        ...
        state_ind = int(u''.join([unicode(x) for x in [int(x) for x in meas]]), 2)
        values[state_ind] += 1

    plot_labels = [bin(x)[2:].zfill(num_qubits) for x in xrange(states)]
    plt.bar(np.arange(states), values, tick_label=plot_labels)
    plt.xlabel(u'qubit state')
    plt.ylabel(u'result count')
    plt.show()
