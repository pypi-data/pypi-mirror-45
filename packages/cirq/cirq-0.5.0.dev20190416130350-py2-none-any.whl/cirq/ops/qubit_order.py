...
TInternalQubit = TypeVar(u'TInternalQubit')
TExternalQubit = TypeVar(u'TExternalQubit')


class QubitOrder(object):
    u"""Defines the kronecker product order of qubits."""

    def __init__(self, explicit_func
                 ):
        self._explicit_func = explicit_func

    DEFAULT = None  # type: QubitOrder
    u"""A basis that orders qubits in the same way that calling `sorted` does.
    ...
    @staticmethod
    def explicit(fixed_qubits,
                 fallback=None):
        u"""A basis that contains exactly the given qubits in the given order.
        ...
        if len(set(result)) < len(result):
            raise ValueError(
                u'Qubits appear in fixed_order twice: {}.'.format(result))
        ...
        def func(qubits):
            ...
            if not fallback:
                raise ValueError(
                    u'Unexpected extra qubits: {}.'.format(remaining))
    ...
    @staticmethod
    def sorted_by(key):
        u"""A basis that orders qubits ascending based on a key function.
    ...
    def order_for(self, qubits
                  ):
        u"""Returns a qubit tuple ordered corresponding to the basis.
    ...
    def as_qubit_order(val
        ...
        u"""Converts a value into a basis.
        ...
        if isinstance(val, QubitOrder):
            return val
        raise ValueError(
            u"Don't know how to interpret <{}> as a Basis.".format(val))

    def map(self,
            internalize,
            externalize
            ):
        u"""Transforms the Basis so that it applies to wrapped qubits.
