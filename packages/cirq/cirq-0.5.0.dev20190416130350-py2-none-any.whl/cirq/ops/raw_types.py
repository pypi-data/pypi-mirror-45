...
class Qid(object):
    ...
    u"""Identifies a quantum object such as a qubit, qudit, resonator, etc.
...
class Gate(object):
    ...
    def validate_args(self, qubits):
        ...
        if len(qubits) != self.num_qubits():
            raise ValueError(
                u'Wrong number of qubits for <{!r}>. '
                u'Expected {} qubits but got <{!r}>.'.format(
                    self,
                    self.num_qubits(),
                    qubits))
        ...
        if any([not isinstance(qubit, Qid)
                for qubit in qubits]):
            raise ValueError(
                    u'Gate was called with type different than Qid.')
    ...
    def controlled_by(self, *control_qubits):
        u"""Returns a controlled version of this gate.
...
class Operation(object):
    ...
    def controlled_by(self, *control_qubits):
        u"""Returns a controlled version of this operation.
        ...
        if control_qubits is None or len(control_qubits) is 0:
            raise ValueError(
                u"Can't get controlled operation without control qubit. Op: {}"
                .format(repr(self)))
        else:
            return ControlledOperation(control_qubits, self)

class _InverseCompositeGate(Gate):
    u"""The inverse of a composite gate."""
    ...
    def __repr__(self):
        return u'({!r}**-1)'.format(self._original)

_InverseCompositeGate = value.value_equality(_InverseCompositeGate)
