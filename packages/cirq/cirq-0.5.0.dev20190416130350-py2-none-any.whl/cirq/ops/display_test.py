...
def test_pauli_string_expectation_value_mixed_state_linearity():
    ...
    pauli_string_expectation = cirq.pauli_string_expectation(
            cirq.PauliString(dict((q, np.random.choice(paulis)) for q in qubits)))
...
@pytest.mark.parametrize(u'paulis', [
    (cirq.Z, cirq.Z),
    (cirq.Z, cirq.X),
    (cirq.X, cirq.X),
    (cirq.X, cirq.Y),
...
@pytest.mark.parametrize(u'measurements, value', [
    (np.array([[0, 0, 0],
               [0, 0, 0]]),
     1),
    (np.array([[0, 0, 0],
...
def test_pauli_string_expectation_helper():
    ...
    assert (cirq.pauli_string_expectation(pauli_string, key=u'a')
            == cirq.pauli_string_expectation(pauli_string, key=u'a'))
    assert (cirq.pauli_string_expectation(pauli_string, 5, key=u'a')
            == cirq.pauli_string_expectation(pauli_string, 5, key=u'a'))
