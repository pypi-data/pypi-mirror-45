...
def large_circuit():
    np.random.seed(0)
    qubits = [cirq.GridQubit(0, i) for i in xrange(10)]
    sqrt_x = cirq.PhasedXPowGate(exponent=0.5, phase_exponent=0.0)
    circuit = cirq.Circuit(device=cirq.google.Foxtail)
    for _ in xrange(11):
        circuit.append(
            [sqrt_x(qubit) for qubit in qubits if np.random.random() < 0.5])
        circuit.append([cirq.CZ(qubits[i], qubits[i + 1]) for i in xrange(9)])
    circuit.append([cirq.measure(*qubits, key=u'meas')])
    return circuit
...
@pytest.mark.parametrize((u'scheduler', u'use_processes'),
                         izip(SCHEDULERS, (True, False)))
...
@pytest.mark.parametrize(u'scheduler', SCHEDULERS)
def test_run(scheduler):
    np.random.seed(0)
    circuit = basic_circuit()
    circuit.append(
        [cirq.measure(Q1, key=u'a'), cirq.measure(Q2, key=u'b')],
         strategy=InsertStrategy.NEW_THEN_INLINE)

    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    assert result.measurements[u'a'].dtype == bool
    assert result.measurements[u'b'].dtype == bool
...
@pytest.mark.skipif(not hasattr(np, u'float128'),
                    reason=u"system doesn't have np.float128")
@pytest.mark.parametrize(u'scheduler', SCHEDULERS)
...
def test_run_no_sharing_few_qubits(scheduler):
    ...
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[False]])
    np.testing.assert_equal(result.measurements[u'b'], [[True]])
...
def test_simulate_moment_steps():
    ...
    for step in simulator.simulate_moment_steps(circuit):
        results.append(step)
    expected = [{}, {}, {}, {}, {u'a': [False], u'b': [False]}]
    assert len(results) == len(expected)
    assert all(a.measurements == b for a, b in izip(results, expected))
...
def test_simulate_moment_steps_sample():
    ...
    circuit = cirq.Circuit.from_ops(cirq.X(Q1),
                                    cirq.measure(Q1, key=u'a'),
                                    cirq.measure(Q2, key=u'b'),
                                    device=test_device)
    simulator = cg.XmonSimulator()
...
def compute_gate(circuit, resolver, num_qubits=1):
    simulator = cg.XmonSimulator()
    gate = []
    for initial_state in xrange(1 << num_qubits):
...
def test_param_resolver_exp_w_axis_half_turns():
    exp_w = cirq.PhasedXPowGate(
        exponent=1.0, phase_exponent=sympy.Symbol(u'a'))
    circuit = cirq.Circuit(device=test_device)
    circuit.append(exp_w(Q1))
    resolver = cirq.ParamResolver({u'a': 0.5})
    result = compute_gate(circuit, resolver)
...
def test_param_resolver_exp_w_multiple_params():
    exp_w = cirq.PhasedXPowGate(
        exponent=sympy.Symbol(u'a'),
        phase_exponent=sympy.Symbol(u'b'))
    circuit = cirq.Circuit(device=test_device)
    circuit.append(exp_w(Q1))
    resolver = cirq.ParamResolver({u'a': -0.5, u'b': 0.5})
    result = compute_gate(circuit, resolver)
    amp = 1.0 / math.sqrt(2)
    np.testing.assert_almost_equal(result,
                                   np.array([[amp, amp],
...
def test_param_resolver_exp_11_half_turns():
    circuit = cirq.Circuit(device=test_device)
    circuit.append(cirq.CZ(Q1, Q2)**sympy.Symbol(u'a'))
    resolver = cirq.ParamResolver({u'a': 0.5})
...
def test_param_resolver_param_dict():
    ...
    assert result.params.param_dict == {u'a': 0.5}


def test_run_circuit_sweep():
    circuit = cirq.Circuit.from_ops(
        cirq.X(Q1)**sympy.Symbol(u'a'),
        cirq.measure(Q1, key=u'm'),
        device=test_device,
    )

    sweep = cirq.Linspace(u'a', 0, 10, 11)
    simulator = cg.XmonSimulator()

    for i, result in enumerate(
            simulator.run_sweep(circuit, sweep, repetitions=1)):
        assert result.params[u'a'] == i
        np.testing.assert_equal(result.measurements[u'm'], [[i % 2 != 0]])
...
def test_run_circuit_sweeps():
    ...
    sweep = cirq.Linspace(u'a', 0, 5, 6)
    sweep2 = cirq.Linspace(u'a', 6, 10, 5)
    simulator = cg.XmonSimulator()
...
def test_composite_gates(scheduler):
    ...
    circuit.append([cirq.measure(Q1, Q2, key=u'a')])

    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[True, True]])


class UnsupportedGate(cirq.SingleQubitGate):

    def __repr__(self):
        return u"UnsupportedGate"
...
def test_unsupported_gate_defense_in_depth(scheduler):
    ...
    simulator = cg.XmonSimulator()
    with pytest.raises(ValueError, match=u"UnsupportedGate"):
        _ = run(simulator, circuit, scheduler)

    with pytest.raises(ValueError, match=u"using an XmonDevice"):
...
def test_measurement_qubit_order(scheduler):
    ...
    circuit.append([cirq.measure(Q1, Q3, Q2, key=u'')])
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u''], [[True, False, True]])


@pytest.mark.parametrize(u'scheduler', SCHEDULERS)
def test_inverted_measurement(scheduler):
    circuit = cirq.Circuit.from_ops(
        cirq.measure(Q1, key=u'a', invert_mask=(False,)),
        cirq.X(Q1),
        cirq.measure(Q1, key=u'b', invert_mask=(False,)),
        cirq.measure(Q1, key=u'c', invert_mask=(True,)),
        cirq.X(Q1),
        cirq.measure(Q1, key=u'd', invert_mask=(True,)),
        device=test_device,
    )
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements,
                            {u'a': [[False]], u'b': [[True]], u'c': [[False]],
                             u'd': [[True]]})
...
def test_inverted_measurement_multiple_qubits(scheduler):
    circuit = cirq.Circuit.from_ops(
        cirq.measure(Q1, Q2, key=u'a', invert_mask=(False, True)),
        cirq.measure(Q1, Q2, key=u'b', invert_mask=(True, False)),
        cirq.measure(Q2, Q1, key=u'c', invert_mask=(True, False)),
        device=test_device,
    )
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[False, True]])
    np.testing.assert_equal(result.measurements[u'b'], [[True, False]])
    np.testing.assert_equal(result.measurements[u'c'], [[True, False]])
...
def test_measurement_multiple_measurements(scheduler):
    ...
    circuit.append([cirq.measure(Q1, Q2, key=u'a')])
    circuit.append(cirq.X(Q1))
    circuit.append([cirq.measure(Q1, Q2, key=u'b')])
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[True, False]])
    np.testing.assert_equal(result.measurements[u'b'], [[False, False]])
...
def test_measurement_multiple_measurements_qubit_order(scheduler):
    ...
    circuit.append(cirq.X(Q1))
    circuit.append([cirq.measure(Q1, Q2, key=u'a')])
    circuit.append([cirq.measure(Q2, Q1, key=u'b')])
    simulator = cg.XmonSimulator()
    result = run(simulator, circuit, scheduler)
    np.testing.assert_equal(result.measurements[u'a'], [[True, False]])
    np.testing.assert_equal(result.measurements[u'b'], [[False, True]])


@pytest.mark.parametrize(u'scheduler', SCHEDULERS)
def test_measurement_keys_repeat(scheduler):
    circuit = cirq.Circuit(device=test_device)
    circuit.append([cirq.measure(Q1, key=u'a'), cirq.X.on(Q1), cirq.X.on(Q2),
                    cirq.measure(Q2, key=u'a')])
    simulator = cg.XmonSimulator()
    with pytest.raises(ValueError, message=u'Repeated Measurement key a'):
...
def test_handedness_of_basic_gates():
    circuit = cirq.Circuit.from_ops(
        ...
        cirq.Z(Q1)**-0.5,
        cirq.Y(Q1)**0.5,
        cirq.measure(Q1, key=u''),
        device=test_device,
    )
    result = cg.XmonSimulator().run(circuit)
    np.testing.assert_equal(result.measurements[u''], [[True]])
...
def bit_flip_circuit(flip0, flip1):
    ...
    m1, m2 = cirq.measure(Q1, key=u'q1'), cirq.measure(Q2, key=u'q2')
    circuit = cirq.Circuit(device=test_device)
...
def test_circuit_repetitions():
    ...
    np.testing.assert_equal(result.measurements[u'q1'], [[True]] * 10)
    np.testing.assert_equal(result.measurements[u'q2'], [[True]] * 10)
...
def test_circuit_parameters():
    sim = cg.XmonSimulator()
    circuit = bit_flip_circuit(sympy.Symbol(u'a'), sympy.Symbol(u'b'))

    resolvers = [cirq.ParamResolver({u'a': b1, u'b': b2})
                 for b1 in xrange(2) for b2 in xrange(2)]

    all_trials = sim.run_sweep(circuit, params=resolvers, repetitions=1)
    assert len(all_trials) == 4
    for result in all_trials:
        assert result.repetitions == 1
        expect_a = result.params[u'a'] == 1
        expect_b = result.params[u'b'] == 1
        np.testing.assert_equal(result.measurements[u'q1'], [[expect_a]])
        np.testing.assert_equal(result.measurements[u'q2'], [[expect_b]])
    # All parameters explored.
    assert (set(itertools.product([0, 1], [0, 1]))
            == set((r.params[u'a'], r.params[u'b']) for r in all_trials))
...
def test_circuit_param_and_reps():
    ...
    for result in all_trials:
        ...
        expect_a = result.params[u'a'] == 1
        expect_b = result.params[u'b'] == 1
        np.testing.assert_equal(result.measurements[u'q1'], [[expect_a]] * 3)
        np.testing.assert_equal(result.measurements[u'q2'], [[expect_b]] * 3)
    # All parameters explored.
    assert (set(itertools.product([0, 1], [0, 1]))
            == set((r.params[u'a'], r.params[u'b']) for r in all_trials))
...
def test_simulator_simulate_trial_result_str():
    ...
    assert unicode(result) == u"measurements: a=1 b=1 c=0\noutput vector: -1|110⟩"
...
def test_simulator_implied_measurement_key():
    ...
    circuit = cirq.Circuit.from_ops(
        cirq.X(q),
        cirq.measure(q),
        cirq.measure(q, key=u'other'),
        device=test_device,
    )
    result = cirq.google.XmonSimulator().run(circuit, repetitions=5)
    assert unicode(result) == u"(0, 0)=11111\nother=11111"
