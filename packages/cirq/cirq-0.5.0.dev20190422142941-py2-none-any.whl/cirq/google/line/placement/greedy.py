...
class GreedySequenceSearch(object):
    u"""Base class for greedy search heuristics.
    ...
    def __init__(self,
                 device,
                 start):
        u"""Greedy sequence search constructor.
        ...
        if start not in device.qubits:
            raise ValueError(u'Starting qubit must be a qubit on the chip')
    ...
    def get_or_search(self):
        u"""Starts the search or gives previously calculated sequence.
    ...
    @abc.abstractmethod
    def _choose_next_qubit(self, qubit,
                           used):
        u"""Selects next qubit on the linear sequence.
    ...
    def _find_sequence(self):
        u"""Looks for a sequence starting at a given qubit.
    ...
    def _sequence_search(self, start,
                         current):
        u"""Search for the continuous linear sequence from the given qubit.
    ...
    def _expand_sequence(self, seq):
        u"""Tries to expand given sequence with more qubits.
    ...
    def _find_path_between(self, p, q,
                           used):
        u"""Searches for continuous sequence between two qubits.
    ...
    def _neighbors_of_excluding(self, qubit, used
                                ):
        return [n for n in self._c_adj[qubit] if n not in used]


class _PickFewestNeighbors(GreedySequenceSearch):
    u"""Minimal qubit connectivity greedy heuristic for linear sequence.
    ...
    def _choose_next_qubit(self, qubit,
        ...
        return min(
            ...
            key=lambda n:
                ...
                float(u'inf'))  # Avoid dead ends with no neighbors at all.


class _PickLargestArea(GreedySequenceSearch):
    u"""Largest area greedy heuristic for linear sequence.
    ...
    def _collect_unused(self, start,
                        used):
        u"""Lists all the qubits that are reachable from given qubit.
...
class GreedySequenceSearchStrategy(place_strategy.LinePlacementStrategy):
    u"""Greedy search method for linear sequence of qubits on a chip.
    """

    def __init__(self, algorithm = u'best'):
        u"""Initializes greedy sequence search strategy.
    ...
    def place_line(self,
        ...
        if algos is None:
            raise ValueError(
                u"Unknown greedy search algorithm %s" % self.algorithm)
