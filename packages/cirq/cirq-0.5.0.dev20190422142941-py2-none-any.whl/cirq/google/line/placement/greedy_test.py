...
def test_get_or_search_calls_find_sequence_once():
    ...
    with mock.patch.object(search, u'_find_sequence') as find_sequence:
        sequence = [q00, q01]
...
def test_find_sequence_assembles_head_and_tail():
    ...
    with mock.patch.object(search, u'_sequence_search') as sequence_search:
        head = [q01, q00]
        tail = [q01, q02]
...
def test_find_sequence_calls_expand_sequence():
    ...
    with mock.patch.object(
            search, u'_sequence_search') as sequence_search, mock.patch.object(
        search, u'_expand_sequence') as expand_sequence:
        head = [q01, q00]
        tail = [q01, q02]
        sequence_search.side_effect = [tail, head]
...
def test_search_sequence_calls_choose_next_qubit():
    ...
    with mock.patch.object(search, u'_choose_next_qubit') as choose_next_qubit:
        choose_next_qubit.return_value = None
        search._sequence_search(q01, [])
        choose_next_qubit.assert_called_once_with(q01, set([q01]))

    with mock.patch.object(search, u'_choose_next_qubit') as choose_next_qubit:
        choose_next_qubit.return_value = None
        search._sequence_search(q01, [q00])
        choose_next_qubit.assert_called_once_with(q01, set([q00, q01]))
...
def test_find_path_between_finds_path():
    ...
    start = q00
    search = greedy.GreedySequenceSearch(_create_device(qubits), start)
    assert search._find_path_between(q00, q01, set([q00, q01])) == [q10, q11]
...
def test_find_path_between_does_not_find_path():
    ...
    start = q00
    search = greedy.GreedySequenceSearch(_create_device(qubits), start)
    assert search._find_path_between(q00, q01, set([q00, q01])) is None
...
def test_minimal_sequence_search_chooses_minimal():
    ...
    #     |
    #     +
    assert search._choose_next_qubit(q10, set([q10])) == q20
    assert search._choose_next_qubit(q20, set([q10, q20])) == q21
...
def test_minimal_sequence_search_does_not_use_used():
    ...
    #
    #     +
    assert search._choose_next_qubit(q10, set([q00, q10])) == q20
...
def test_minimal_sequence_search_returns_none_for_single_node():
    ...
    assert search._choose_next_qubit(q00, set([q00])) is None
...
def test_minimal_sequence_search_returns_none_when_blocked():
    ...
    search = greedy._PickFewestNeighbors(
        _create_device(qubits), q10)
    assert search._choose_next_qubit(q10, set([q00, q10])) is None
...
def test_minimal_sequence_search_traverses_grid():
    ...
    #                             ║
    #                             (2,4)
    assert search._choose_next_qubit(q02, set([q02])) == q03
    assert search._choose_next_qubit(q03, set([q02, q03])) == q04
    assert search._choose_next_qubit(q04, set([q02, q03, q04])) == q14
    assert search._choose_next_qubit(q14, set([q02, q03, q04, q14])) == q24
    assert search._choose_next_qubit(q24, set([q02, q03, q04, q14, q24])) is None
    ...
    #                             ║
    #                             START
    assert search._choose_next_qubit(q24, set([q24])) == q14
    assert search._choose_next_qubit(q14, set([q24, q14])) == q04
    assert search._choose_next_qubit(q04, set([q24, q14, q04])) == q03
    assert search._choose_next_qubit(q03, set([q24, q14, q04, q03])) == q02
    assert search._choose_next_qubit(q02, set([q24, q14, q04, q03, q02])) == q01
    assert search._choose_next_qubit(q01, set([q24, q14, q04, q03, q02, q01])
                                     ) in [q00, q11]
    assert search._choose_next_qubit(q00, set([q24, q14, q04, q03, q02, q01,
                                           q00])) is None
    assert search._choose_next_qubit(q11, set([q24, q14, q04, q03, q02, q01,
                                           q11])) is None
    ...
    device = _create_device(qubits)
    method = greedy.GreedySequenceSearchStrategy(u'minimal_connectivity')
...
def test_largest_sequence_search_does_not_use_used():
    ...
    #
    #     +
    assert search._choose_next_qubit(q10, set([q10, q20])) == q00
...
def test_largest_sequence_search_traverses_grid():
    ...
    #         |
    #         +
    assert search._choose_next_qubit(q20, set([q20])) == q30
    assert search._choose_next_qubit(q30, set([q20, q30])) == q40
    assert search._choose_next_qubit(q40, set([q20, q30, q40])) == q41
    assert search._choose_next_qubit(q41, set([q20, q30, q40, q41])) == q42
    assert search._choose_next_qubit(q42, set([q20, q30, q40, q41, q42])) is None

    method = greedy.GreedySequenceSearchStrategy(u'largest_area')
...
def test_largest_collect_unused_collects_all_for_empty():
    ...
    assert search._collect_unused(start, set([start])) == set(qubits)
...
def test_largest_collect_unused_collects():
    ...
    search = greedy._PickLargestArea(
        _create_device(qubits), start)
    assert search._collect_unused(start, set([q00, q01])) == set([q01, q02, q12])
...
def test_largest_collect_stops_on_used():
    ...
    search = greedy._PickLargestArea(
        _create_device(qubits), start)
    assert search._collect_unused(start, set([start, q04])) == set([
        q00, q01, q11, q02, q03])
...
def test_greedy_search_method_fails_when_unknown():
    ...
    method = greedy.GreedySequenceSearchStrategy(u'fail')
    with pytest.raises(ValueError):
        method.place_line(_create_device(qubits), length)


@mock.patch(u'cirq.google.line.placement.greedy._PickLargestArea')
@mock.patch(u'cirq.google.line.placement.greedy._PickFewestNeighbors')
