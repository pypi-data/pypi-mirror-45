...
class AnnealSequenceSearch(object):
    u"""Simulated annealing search heuristic.
    """
    ...
    def search(
            self,
            trace_func = None):
        u"""Issues new linear sequence search.
        ...
        def search_trace(state, temp,
                         cost, probability, accepted):
            if trace_func:
                trace_seqs, _ = state
    ...
    def _quadratic_sum_cost(self, state):
        u"""Cost function that sums squares of lengths of sequences.
    ...
    def _force_edges_active_move(self, state):
        u"""Move function which repeats _force_edge_active_move a few times.
        ...
        Returns:
          ...
          original state.
        """
        for _ in xrange(self._rand.randint(1, 4)):
            state = self._force_edge_active_move(state)
        return state

    def _force_edge_active_move(self, state):
        u"""Move which forces a random edge to appear on some sequence.
    ...
    def _force_edge_active(self, seqs, edge,
                           sample_bool
                           ):
        u"""Move which forces given edge to appear on some sequence.
    ...
    def _create_initial_solution(self):
        u"""Creates initial solution based on the chip description.
        ...
        def extract_sequences():
            u"""Creates list of sequcenes for initial state.
        ...
        def assemble_edges():
            u"""Creates list of edges for initial state.
    ...
    def _normalize_edge(self, edge):
        u"""Gives unique representative of the edge.
    ...
    def _choose_random_edge(self, edges):
        u"""Picks random edge from the set of edges.
...
class AnnealSequenceSearchStrategy(place_strategy.LinePlacementStrategy):
    u"""Linearized sequence search using simulated annealing method.
...
def index_2d(seqs, target):
    u"""Finds the first index of a target item within a list of lists.
    ...
    for i in xrange(len(seqs)):
        for j in xrange(len(seqs[i])):
            if seqs[i][j] == target:
                return i, j
    raise ValueError(u'Item not present.')
