...
class XmonDevice(devices.Device):
    u"""A device with qubits placed in a grid. Neighboring qubits can interact.
    """
    ...
    def __init__(self,
                 ...
                 exp_11_duration,
                 qubits):
        u"""Initializes the description of an xmon device.
    ...
    def duration_of(self, operation):
        ...
        raise ValueError(u'Unsupported gate type: {!r}'.format(operation))

    def validate_gate(self, gate):
        u"""Raises an error if the given gate isn't allowed.
        ...
        if not isinstance(gate, (ops.CZPowGate,
                                 ...
                                 ops.MeasurementGate,
                                 ops.ZPowGate)):
            raise ValueError(u'Unsupported gate type: {!r}'.format(gate))
    ...
    def validate_operation(self, operation):
        ...
        if (len(operation.qubits) == 2
            ...
            if not cast(GridQubit, p).is_adjacent(q):
                raise ValueError(
                    u'Non-local interaction: {!r}.'.format(operation))
        ...
        if isinstance(scheduled_operation.operation.gate, ops.CZPowGate):
            for other in schedule.operations_happening_at_same_time_as(
                ...
                if self._check_if_exp11_operation_interacts(
                    ...
                    raise ValueError(
                        u'Adjacent Exp11 operations: {} vs {}.'.format(
                            scheduled_operation, other))

    def validate_circuit(self, circuit):
        super(XmonDevice, self).validate_circuit(circuit)
        _verify_unique_measurement_keys(circuit.all_operations())

    def validate_moment(self, moment):
        super(XmonDevice, self).validate_moment(moment)
            ...
            if (isinstance(op, ops.GateOperation) and
                ...
                for other in moment.operations:
                    if (other is not op and
                        ...
                        raise ValueError(
                            u'Adjacent Exp11 operations: {}.'.format(moment))
    ...
    def can_add_operation_into_moment(self,
        ...
        if not super(XmonDevice, self).can_add_operation_into_moment(operation, moment):
            return False
    ...
    def at(self, row, col):
        u"""Returns the qubit at the given position, if there is one, else None.
        """
        q = GridQubit(row, col)
        return q if q in self.qubits else None

    def row(self, row):
        u"""Returns the qubits in the given row, in ascending order."""
        return sorted(q for q in self.qubits if q.row == row)

    def col(self, col):
        u"""Returns the qubits in the given column, in ascending order."""
        return sorted(q for q in self.qubits if q.col == col)

    def __repr__(self):
        return (u'XmonDevice(measurement_duration={!r}, '
                u'exp_w_duration={!r}, '
                u'exp_11_duration={!r} '
...
XmonDevice = value.value_equality(XmonDevice)

def _verify_unique_measurement_keys(operations):
    seen = set()  # type: Set[str]
    for op in operations:
        if protocols.is_measurement(op):
            key = protocols.measurement_key(op)
            if key in seen:
                raise ValueError(u'Measurement key {} repeated'.format(key))
            seen.add(key)
