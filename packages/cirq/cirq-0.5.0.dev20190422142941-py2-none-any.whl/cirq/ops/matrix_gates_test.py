...
def test_single_qubit_approx_eq():
    ...
    assert not cirq.approx_eq(i, u'', atol=1e-9)
...
def test_str_executes():
    assert u'1' in unicode(cirq.SingleQubitMatrixGate(np.eye(2)))
    assert u'0' in unicode(cirq.TwoQubitMatrixGate(np.eye(4)))
