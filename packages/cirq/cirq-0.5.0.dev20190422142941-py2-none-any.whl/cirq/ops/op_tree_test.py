...
def test_flatten_op_tree():
    operations = [
        cirq.GateOperation(cirq.SingleQubitGate(), [cirq.NamedQubit(unicode(i))])
        for i in xrange(10)
    ]
...
def test_freeze_op_tree():
    ...
    assert (
            cirq.freeze_op_tree(
            (operations[0], (operations[i] for i in xrange(1, 5)),
             operations[5:])) ==
...
def test_transform_bad_tree():
    ...
    with pytest.raises(TypeError):
        _ = list(cirq.flatten_op_tree(cirq.transform_op_tree([
            cirq.GateOperation(cirq.Gate(), [cirq.NamedQubit(u'q')]), (4,)
        ])))


def test_transform_leaves():
    gs = [cirq.SingleQubitGate() for _ in xrange(10)]
    operations = [
        cirq.GateOperation(gs[i], [cirq.NamedQubit(unicode(i))])
        for i in xrange(10)
    ]
    expected = [
        cirq.GateOperation(gs[i], [cirq.NamedQubit(unicode(i) + u'a')])
        for i in xrange(10)
    ]
    ...
    def move_left(op):
        return cirq.GateOperation(
            op.gate,
            [cirq.NamedQubit(cast(cirq.NamedQubit, q).name + u'a')
             for q in op.qubits])
