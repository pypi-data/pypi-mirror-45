...
u"""A recursive type describing trees of operations, and utility methods for it.
"""
...
u"""The recursive type consumed by circuit builder methods.
...
def flatten_op_tree(root,
                    preserve_moments = False
                    ):
    u"""Performs an in-order iteration of the operations (leaves) in an OP_TREE.
    ...
    raise TypeError(u'Not a collections.Iterable or an Operation: {} {}'.format(
        type(root), root))


def transform_op_tree(
        root,
        op_transformation=lambda e: e,
        iter_transformation=lambda e: e,
        preserve_moments = False
):
    u"""Maps transformation functions onto the nodes of an OP_TREE.
    ...
    raise TypeError(
        u'Not a collections.Iterable or an Operation: {}'.format(root))


def freeze_op_tree(root):
    u"""Replaces all iterables in the OP_TREE with tuples.
