...
def generate_supremacy_circuit_google_v2(qubits,
                                         cz_depth,
                                         seed):
    u"""
    ...
    for moment_index in xrange(2, cz_depth+1):
        layer_index = _add_cz_layer(layer_index, circuit)
...
def generate_supremacy_circuit_google_v2_grid(n_rows, n_cols,
                                              cz_depth, seed
                                              ):
    u"""
    Generates Google Random Circuits v2 as in github.com/sboixo/GRCS cz_v2.
    ...
    (as in the QASM mapping)
    """
    qubits = [devices.GridQubit(i, j) for i in xrange(n_rows)
              for j in xrange(n_cols)]
    return generate_supremacy_circuit_google_v2(qubits, cz_depth, seed)


def generate_supremacy_circuit_google_v2_bristlecone(n_rows,
                                                     cz_depth, seed
                                                     ):
    u"""
    ...
    def get_qubits(n_rows):
        def count_neighbors(qubits, qubit):
            u"""Counts the qubits that the given qubit can interact with."""
            possibles = [
...
def _choice(rand_gen, sequence):
    u"""Choose a random element from a non-empty sequence.
...
def _add_cz_layer(layer_index, circuit):
    cz_layer = None
    while not cz_layer:
...
def _make_cz_layer(qubits, layer_index
                   ):
    u"""
