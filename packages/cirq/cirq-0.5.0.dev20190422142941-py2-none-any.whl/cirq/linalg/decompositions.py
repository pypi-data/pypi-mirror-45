...
u"""Utility methods for breaking matrices into useful pieces."""
...
def deconstruct_single_qubit_matrix_into_angles(
        mat):
    u"""Breaks down a 2x2 unitary into more useful ZYZ angle parameters.
...
def _group_similar(items,
                   comparer):
    u"""Combines similar items into groups.
...
def _perp_eigendecompose(matrix,
                         rtol = 1e-5,
                         atol = 1e-8,
                         ):
    u"""An eigendecomposition that ensures eigenvectors are perpendicular.
    ...
    vecs = [cols[:, i] for i in xrange(len(cols))]
...
def map_eigenvalues(
    ...
    u"""Applies a function to the eigenvalues of a matrix.
    ...
    for piece, val in izip(pieces, out_vals):
...
def kron_factor_4x4_to_2x2s(
    ...
    u"""Splits a 4x4 matrix U = kron(A, B) into A, B, and a global factor.
    ...
    a, b = max(
        ((i, j) for i in xrange(4) for j in xrange(4)),
...
def so4_to_magic_su2s(
    ...
    if 'check_preconditions' in _3to2kwargs: check_preconditions = _3to2kwargs['check_preconditions']; del _3to2kwargs['check_preconditions']
    else: check_preconditions =  True
    if 'atol' in _3to2kwargs: atol = _3to2kwargs['atol']; del _3to2kwargs['atol']
    else: atol =  1e-8
    if 'rtol' in _3to2kwargs: rtol = _3to2kwargs['rtol']; del _3to2kwargs['rtol']
    else: rtol =  1e-5
    u"""Finds 2x2 special-unitaries A, B where mat = Mag.H @ kron(A, B) @ Mag.
    ...
    if check_preconditions:
        if mat.shape != (4, 4) or not predicates.is_special_orthogonal(
            ...
            raise ValueError(u'mat must be 4x4 special orthogonal.')
...
class KakDecomposition(object):
    u"""A convenient description of an arbitrary two-qubit operation.
    ...
    def __init__(self, **_3to2kwargs):
        single_qubit_operations_after = _3to2kwargs['single_qubit_operations_after']; del _3to2kwargs['single_qubit_operations_after']
        interaction_coefficients = _3to2kwargs['interaction_coefficients']; del _3to2kwargs['interaction_coefficients']
        single_qubit_operations_before = _3to2kwargs['single_qubit_operations_before']; del _3to2kwargs['single_qubit_operations_before']
        global_phase = _3to2kwargs['global_phase']; del _3to2kwargs['global_phase']
        u"""Initializes a decomposition for a two-qubit operation U.
    ...
    def __repr__(self):
        return (
            u'cirq.KakDecomposition(\n'
            u'    interaction_coefficients={!r},\n'
            u'    single_qubit_operations_before=(\n'
            u'        {},\n'
            u'        {},\n'
            u'    ),\n'
            u'    single_qubit_operations_after=(\n'
            u'        {},\n'
            u'        {},\n'
            u'    ),\n'
            u'    global_phase={!r})'
    ...
    def _unitary_(self):
        u"""Returns the decomposition's two-qubit unitary matrix.
...
KakDecomposition = value.value_equality(KakDecomposition)

def kak_canonicalize_vector(x, y, z):
    u"""Canonicalizes an XX/YY/ZZ interaction by swap/negate/shift-ing axes.
...
def kak_decomposition(
        ...
        rtol = 1e-5,
        atol = 1e-8):
    u"""Decomposes a 2-qubit unitary into 1-qubit ops and XX/YY/ZZ interactions.
