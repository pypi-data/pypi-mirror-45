...
class Circuit(object):
    ...
    def from_ops(*operations, **_3to2kwargs
        ...
        else: strategy =  InsertStrategy.EARLIEST
    ...
    def __pow__(self, exponent):
        u"""A circuit raised to a power, only valid for exponent -1, the inverse.
    ...
    def __repr__(self):
        if not self._moments and self._device == devices.UnconstrainedDevice:
            return u'cirq.Circuit()'

        if not self._moments:
            return u'cirq.Circuit(device={!r})'.format(self._device)

        moment_str = _list_repr_with_indented_item_lines(self._moments)
        if self._device == devices.UnconstrainedDevice:
            return u'cirq.Circuit(moments={})'.format(moment_str)

        return u'cirq.Circuit(moments={}, device={!r})'.format(moment_str,
                                                              self._device)
    ...
    def reachable_frontier_from(
            self,
            start_frontier, **_3to2kwargs
    ):
        if 'is_blocker' in _3to2kwargs: is_blocker = _3to2kwargs['is_blocker']; del _3to2kwargs['is_blocker']
        else: is_blocker =  lambda op: False
        u"""Determines how far can be reached into a circuit under certain rules.
    ...
    def findall_operations_between(self,
                                   start_frontier,
                                   end_frontier,
                                   omit_crossing_operations = False
                                   ):
        u"""Finds operations between the two given frontiers.
        ...
        for q in sorted(involved_qubits):
            for i in xrange(start_frontier.get(q, 0),
    ...
    def are_all_measurements_terminal(self):
        u"""Whether all measurement gates are at the end of the circuit."""
        return self.are_all_matches_terminal(protocols.is_measurement)

    def are_all_matches_terminal(self,
            predicate):
        u"""Check whether all of the ops that satisfy a predicate are terminal.
    ...
    def insert(
        ...
        u""" Inserts operations into the circuit.
    ...
    def _insert_operations(self,
        ...
        if len(operations) != len(insertion_indices):
            raise ValueError(u'operations and insertion_indices must have the'
                             u'same length.')
        self._moments += [
            ops.Moment() for _ in xrange(1 + max(insertion_indices) - len(self))
        ]
    ...
    def to_text_diagram(
            self, **_3to2kwargs):
        if 'qubit_order' in _3to2kwargs: qubit_order = _3to2kwargs['qubit_order']; del _3to2kwargs['qubit_order']
        else: qubit_order =  ops.QubitOrder.DEFAULT
        if 'precision' in _3to2kwargs: precision = _3to2kwargs['precision']; del _3to2kwargs['precision']
        else: precision =  3
        if 'transpose' in _3to2kwargs: transpose = _3to2kwargs['transpose']; del _3to2kwargs['transpose']
        else: transpose =  False
        if 'use_unicode_characters' in _3to2kwargs: use_unicode_characters = _3to2kwargs['use_unicode_characters']; del _3to2kwargs['use_unicode_characters']
        else: use_unicode_characters =  True
    ...
    def to_text_diagram_drawer(
        ...
        if 'get_circuit_diagram_info' in _3to2kwargs: get_circuit_diagram_info = _3to2kwargs['get_circuit_diagram_info']; del _3to2kwargs['get_circuit_diagram_info']
        else: get_circuit_diagram_info = None
        if 'qubit_order' in _3to2kwargs: qubit_order = _3to2kwargs['qubit_order']; del _3to2kwargs['qubit_order']
        else: qubit_order =  ops.QubitOrder.DEFAULT
        if 'precision' in _3to2kwargs: precision = _3to2kwargs['precision']; del _3to2kwargs['precision']
        else: precision =  3
        ...
        if 'qubit_namer' in _3to2kwargs: qubit_namer = _3to2kwargs['qubit_namer']; del _3to2kwargs['qubit_namer']
        else: qubit_namer =  None
        if 'use_unicode_characters' in _3to2kwargs: use_unicode_characters = _3to2kwargs['use_unicode_characters']; del _3to2kwargs['use_unicode_characters']
        else: use_unicode_characters =  True
        ...
        if qubit_namer is None:
            qubit_namer = lambda q: unicode(q) + (u'' if transpose else u': ')
    ...
    def _to_qasm_output(
        ...
        u"""Returns a QASM object equivalent to the circuit.
        ...
        """
        if header is None:
            header = u'Generated from Cirq v{}'.format(
    ...
    def to_qasm(self,
        ...
        Args:
            ...
            qubit_order: Determines how qubits are ordered in the QASM
                register.
        """
        return unicode(self._to_qasm_output(header, precision, qubit_order))
...
def _get_operation_circuit_diagram_info_with_fallback(
    ...
    symbols = (name,) + tuple(u'#{}'.format(i + 1)
                              for i in xrange(1, len(op.qubits)))

    return protocols.CircuitDiagramInfo(wire_symbols=symbols)


def _is_exposed_formula(text):
    return re.match(u'[a-zA-Z_][a-zA-Z0-9_]*$', text) is None
        ...
        if exponent is not None:
            if info.connected:
                ...
                for index in indices:
                    out_diagram.write(x, index, u'^' + exponent)
        if x > max_x:
            max_x = x
...
def _draw_moment_groups_in_diagram(moment_groups,
                                   use_unicode_characters,
                                   out_diagram):
...
def _apply_unitary_circuit(circuit,
    ...
    def on_stuck(bad_op):
        return TypeError(
            u'Operation without a known matrix or decomposition: {!r}'.format(
                bad_op))
...
def _decompose_measurement_inversions(op):
    ...
    if gate:
        return [ops.X(q) for q, b in izip(op.qubits, gate.invert_mask) if b]
    return NotImplemented
...
@overload
def _group_until_different(items,
                           key,
                           ):
    pass
...
def _group_until_different(items,
                           key,
                           value=lambda e: e):
    u"""Groups runs of items that are identical according to a keying function.
