...
def convert_and_separate_circuit(circuit,
                                 leave_cliffords = True,
                                 atol = 1e-8,
                                 ):
    u"""Converts any circuit into two circuits where (circuit_left+circuit_right)
...
def regular_half(circuit):
    u"""Return only the Clifford part of a circuit.  See
...
def pauli_string_half(circuit):
    u"""Return only the non-Clifford part of a circuit.  See
    convert_and_separate_circuit().
...
def _pull_non_clifford_before(circuit):
    def _iter_ops_range_reversed(moment_end):
        for i in reversed(xrange(moment_end)):
            moment = circuit[i]
