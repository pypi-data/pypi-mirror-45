...
def clifford_optimized_circuit(circuit,
                               atol = 1e-8
                               ):
    ...
    def find_merge_point(start_i,
        ...
        num_passed_over = 0
        for i in xrange(start_i+1, len(all_ops)):
            op = all_ops[i]
    ...
    def try_merge_clifford(cliff_op, start_i):
        ...
        for pauli, quarter_turns in reversed(
            ...
            assert len(merge_op.pauli_string) == 1, u'PauliString length != 1'

            qubit, pauli = iter(merge_op.pauli_string.items()).next()
            quarter_turns = round(merge_op.half_turns * 2)
            if merge_op.pauli_string.coefficient not in [1, -1]:
                raise NotImplementedError(u"TODO: handle all coefficients.")
    ...
    def try_merge_cz(cz_op, start_i):
        u"""Returns the number of operations removed at or before start_i."""
        for i in reversed(xrange(start_i)):
            op = all_ops[i]
