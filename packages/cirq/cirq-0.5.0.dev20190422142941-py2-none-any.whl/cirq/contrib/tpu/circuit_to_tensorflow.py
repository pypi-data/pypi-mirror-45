...
ComputeFuncAndFeedDict = NamedTuple(
    u'ComputeFuncAndFeedDict',
    [
        (u'compute', Callable[[], tf.Tensor]),
        (u'feed_dict', Dict[tf.Tensor, Any])
    ])
...
def circuit_to_tensorflow_runnable(
    ...
    u"""Returns a compute function and feed_dict for a `cirq.Circuit`'s output.
    ...
    if not circuit.are_all_measurements_terminal():
        raise ValueError(u'not circuit.are_all_measurements_terminal()')
...
_TransformsThenCzs = namedtuple(
    u'_GroupTransformThenInteract',
    [u'group_matrices', u'cz_indices'])


class _QubitGrouping(object):
    u"""Decides how to combine qubits into groups to maximize performance."""
    ...
    def all_in_same_group(self, *qubits):
        return len(set(self.loc(q)[0] for q in qubits)) <= 1

    def decompose_keep_func(self, op):
        # Keep CZs.
...
def _circuit_as_layers(circuit,
                       grouping):
    u"""Transforms a circuit into a series of GroupMatrix+CZ layers.
    ...
    Returns:
        ...
        between groups.
    """
    frontier = dict((q, 0) for q in circuit.all_qubits())
    ...
    while True:
        ...
        for g in grouping.groups:
            ...
            start_frontier = dict((q, frontier[q]) for q in g)
    ...
    assert frontier == dict((q, len(circuit)) for q in circuit.all_qubits())

    return layers


def _deref(tensor, index):
    u"""Equivalent to `tensor[index, ...]`.
...
def _multi_deref(tensors, index):
    u"""Equivalent to `[t[index, ...] for t in tensors]`.
...
TItem = TypeVar(u'TItem')


def _deref_helper(func,
                  index,
                  min_index,
                  max_index):
...
class _TensorCircuit(object):
    def __init__(self,
        ...
        self.secret_group_matrices = [
            tf.placeholder(
                name=u'group_matrices_{}'.format(i),
                dtype=tf.complex64,
        ...
        for i in xrange(len(self.grouping.groups)):
    ...
    def _pick_initial_state_func(
        ...
        if isinstance(state, int):
            secret_index = tf.placeholder(
                name=u'initial_computational_basis_state',
                dtype=tf.int32,
                shape=())
        ...
        if isinstance(state, np.ndarray):
            secret_vector = tf.placeholder(name=u'initial_state_vector',
                                           dtype=tf.complex64,
                                           shape=state.shape)
            self.feed_dict[secret_vector] = state
            return lambda: secret_vector

        raise ValueError(u'Unsupported state type: {}'.format(type(state)))
    ...
    def _after_group_matrices(self,
        ...
        for i in xrange(len(group_matrices)):
            inner_size = s[i]
    ...
    def _after_czs(self, v, pairs):
        iota = tf.range(self.grouping.system_size())
        t = tf.constant(0, dtype=tf.int32)
        for k in xrange(pairs.shape[0]):
            i = pairs[k, 0]
            j = pairs[k, 1]
    ...
    def _after_layer(self, v, layer_index):
