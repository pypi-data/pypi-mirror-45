...
def assert_links_to(circuit, expected, **kwargs):
    actual = circuit_to_quirk_url(circuit, **kwargs)
    actual = actual.replace(u'\n', u'').replace(u' ', u'').strip()
...
def test_various_unknown_gate_types():
    ...
    circuit = cirq.Circuit.from_ops(
        ...
        cirq.PhasedXPowGate(exponent=1, phase_exponent=sympy.Symbol(u'r'))(a),
...
def test_ccz():
    ...
    # Symbolic exponent.
    circuit = cirq.Circuit.from_ops(
        cirq.CCZ(a, b, c)**sympy.Symbol(u't'))
    assert_links_to(circuit, u"""
