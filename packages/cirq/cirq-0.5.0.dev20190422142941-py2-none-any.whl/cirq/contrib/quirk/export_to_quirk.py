...
def _try_convert_to_quirk_gate(op,
    ...
    if prefer_unknown_gate_to_failure:
        return UNKNOWN_GATE
    raise TypeError(u'Unrecognized operation: {!r}'.format(op))


def _to_quirk_cols(op,
                   prefer_unknown_gate_to_failure
                   ):
...
def circuit_to_quirk_url(circuit,
                         prefer_unknown_gate_to_failure=False,
                         escape_url=True):
    u"""Returns a Quirk URL for the given circuit.
    ...
    circuit_json = json.JSONEncoder(ensure_ascii=False,
                                    separators=(u',', u':'),
                                    sort_keys=True).encode({u'cols': cols})
    if escape_url:
        suffix = urllib.quote(circuit_json)
    else:
        suffix = circuit_json
    return u'http://algassert.com/quirk#circuit={}'.format(suffix)
