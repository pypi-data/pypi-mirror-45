...
class ExecutionStrategy(object):
    ...
    u"""Tells StrategyExecutor how to execute an acquaintance strategy.
    ...
    def device(self):
        u"""The device for which the executed acquaintance strategy should be
        valid.
        """
    ...
    def initial_mapping(self):
        u"""The initial mapping of logical indices to qubits."""
    ...
    def get_operations(self,
        ...
        u"""Gets the logical operations to apply to qubits."""
...
class StrategyExecutor(circuits.PointOptimizer):
    u"""Executes an acquaintance strategy."""
    ...
    def __call__(self, strategy):
        if not is_acquaintance_strategy(strategy):
            raise TypeError(u'not is_acquaintance_strategy(strategy)')
        expose_acquaintance_gates(strategy)
        strategy.device = self.execution_strategy.device
        super(StrategyExecutor, self).optimize_circuit(strategy)
    ...
    def optimization_at(self,
        ...
        raise TypeError(u'Can only execute a strategy consisting of gates that '
                         u'are instances of AcquaintanceOpportunityGate or '
                         u'PermutationGate.')


class AcquaintanceOperation(ops.GateOperation):
    u"""Represents an a acquaintance opportunity between a particular set of
    ...
    def __init__(self,
        ...
        if len(logical_indices) != len(qubits):
            raise ValueError(u'len(logical_indices) != len(qubits)')
        super(AcquaintanceOperation, self).__init__(AcquaintanceOpportunityGate(num_qubits=len(qubits)),
                         qubits)
    ...
    def _circuit_diagram_info_(self,
        ...
        wire_symbols = tuple(u'({})'.format(i) for i in self.logical_indices)
        return protocols.CircuitDiagramInfo(wire_symbols=wire_symbols)


class GreedyExecutionStrategy(ExecutionStrategy):
    u"""A greedy execution strategy.
    ...
    def __init__(self,
        ...
        if len(set(len(indices) for indices in gates)) > 1:
            raise NotImplementedError(
                    u'Can only implement greedy strategy if all gates '
                    u'are of the same arity.')
    ...
    def get_operations(self,
        ...
        if index_set in self.index_set_to_gates:
            ...
            index_to_qubit = dict(izip(indices, qubits))
            for gate_indices, gate in sorted(gates.items()):
                yield gate(*[index_to_qubit[i] for i in gate_indices])


    @staticmethod
    def canonicalize_gates(gates
        ):
        u"""Canonicalizes a set of gates by the qubits they act on.
        ...
        return dict((canonical_indices, dict(list(gates.items())))
