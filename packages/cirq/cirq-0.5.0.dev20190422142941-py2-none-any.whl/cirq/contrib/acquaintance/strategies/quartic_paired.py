...
def qubit_pairs_to_qubit_order(
    ...
    u"""Takes a sequence of qubit pairs and returns a sequence in which every
    ...
    if set(len(qubit_pair) for qubit_pair in qubit_pairs) != set((2,)):
        raise ValueError(
            u'set(len(qubit_pair) for qubit_pair in qubit_pairs) != '
            u'set((2,))')
    n_pairs = len(qubit_pairs)
    qubits = [] # type: List[ops.Qid]
    for i in xrange(0, 2 * (n_pairs // 2), 2):
...
def quartic_paired_acquaintance_strategy(
    ...
    u"""Acquaintance strategy for pairs of pairs.
    ...
    for i in reversed(xrange(0, n_qubits, 2)):
        moment = ops.Moment([acquaint(*qubits[j: j + 4])
                             for j in xrange(i % 4, n_qubits - 3, 4)])
