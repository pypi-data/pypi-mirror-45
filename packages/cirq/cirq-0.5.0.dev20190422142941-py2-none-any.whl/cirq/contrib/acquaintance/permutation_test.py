...
def test_swap_permutation_gate():
    no_decomp = lambda op: (isinstance(op, cirq.GateOperation) and
                            op.gate == cirq.SWAP)
    a, b = cirq.NamedQubit(u'a'), cirq.NamedQubit(u'b')
...
def test_validate_permutation_errors():
    ...
    with pytest.raises(IndexError,
            message=u'key and value sets must be the same.'):
        validate_permutation({0: 2, 1: 3})

    with pytest.raises(IndexError,
            message=u'keys of the permutation must be non-negative.'):
        validate_permutation({-1: 0, 0: -1})

    with pytest.raises(IndexError, message=u'key is out of bounds.'):
...
def test_linear_permutation_gate(n_elements, n_permuted):
    ...
    mapping = dict(izip(qubits, elements))
...
def random_equal_permutations(n_perms, n_items, prob):
    indices_to_permute = [i for i in xrange(n_items) if random.random() <= prob]
    permuted_indices = random.sample(
            indices_to_permute, len(indices_to_permute))
    base_permutation = dict(izip(indices_to_permute, permuted_indices))
    fixed_indices = [i for i in xrange(n_items) if i not in base_permutation]
    permutations = []
    for _ in xrange(n_perms):
        permutation = base_permutation.copy()
        permutation.update(
                dict((i, i) for i in fixed_indices if random.random() <= prob))
...
def random_permutation_equality_groups(
    ...
    for _ in xrange(n_groups):
        ...
        perm = perms[0]
        fingerprint = tuple(perm.get(i, i) for i in xrange(n_items))
        if fingerprint not in fingerprints:
            yield perms
            fingerprints.add(fingerprint)


@pytest.mark.parametrize(u'permutation_sets',
...
@pytest.mark.parametrize(u'num_qubits,permutation', [(2, {
    0: 1,
    1: 0
}), (3, {
    0: 0,
...
@pytest.mark.parametrize(u'num_qubits,permutation,inverse', [(2, {
    0: 1,
    1: 0
}, {
    0: 1,
