...
def random_part_lens(max_n_parts, max_part_size):
    return tuple(random.randint(1, max_part_size)
            for _ in xrange(random.randint(1, max_n_parts)))

@pytest.mark.parametrize(u'left_part_lens,right_part_lens', [
    tuple(random_part_lens(7, 2)
          for _ in (u'left', u'right'))
    for _ in xrange(5)])
def test_shift_swap_network_gate_acquaintance_opps(
    ...
    parts = dict((side, []) for side in sides)
    for side, part_lens in izip(sides, (left_part_lens, right_part_lens)):
        for part_len in part_lens:
            parts[side].append(set(xrange(i, i + part_len)))
            i += part_len

    expected_opps = set(
            frozenset(left_part | right_part) for left_part, right_part in
            itertools.product(parts[u'left'], parts[u'right']))
    assert actual_opps == expected_opps

circuit_diagrams = {
    (u'undecomposed', (1,) * 3, (1,) * 3): u"""
0: ───(0, 0, 0)↦(1, 0, 0)───
      │
1: ───(0, 1, 0)↦(1, 1, 0)───
      │
...
def test_shift_swap_network_gate_diagrams(
    ...
    diagram = circuit_diagrams[u'undecomposed', left_part_lens, right_part_lens]
    cirq.testing.assert_has_diagram(circuit, diagram)

    cca.expose_acquaintance_gates(circuit)
    diagram = circuit_diagrams[u'decomposed', left_part_lens, right_part_lens]
    cirq.testing.assert_has_diagram(circuit, diagram)
