...
class SupportsQasm(Protocol):
    u"""An object that can be turned into QASM code.
...
class SupportsQasmWithArgsAndQubits(Protocol):
    u"""An object that can be turned into QASM code if it knows its qubits.
...
def qasm(val, **_3to2kwargs
    ...
    u"""Returns QASM code for the given value, if possible.
    ...
    """
    method = getattr(val, u'_qasm_', None)
    ...
    if method is None:
        raise TypeError(u"object of type '{}' "
                        u"has no _qasm_ method.".format(type(val)))
    raise TypeError(u"object of type '{}' does have a _qasm_ method, "
