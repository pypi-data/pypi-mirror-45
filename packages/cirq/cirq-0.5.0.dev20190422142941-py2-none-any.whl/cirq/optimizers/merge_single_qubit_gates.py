...
class MergeSingleQubitGates(circuits.PointOptimizer):
    u"""Optimizes runs of adjacent unitary 1-qubit operations."""

    def __init__(self, **_3to2kwargs):
        if 'synthesizer' in _3to2kwargs: synthesizer = _3to2kwargs['synthesizer']; del _3to2kwargs['synthesizer']
        else: synthesizer =  None
        if 'rewriter' in _3to2kwargs: rewriter = _3to2kwargs['rewriter']; del _3to2kwargs['rewriter']
        else: rewriter =  None
        u"""
        Args:
            ...
            synthesizer: A special kind of rewriter that operates purely on
                ...
                to make this matrix".
        """
        super(MergeSingleQubitGates, self).__init__()
        if rewriter is not None and synthesizer is not None:
            raise ValueError(u"Can't specify both rewriter and synthesizer.")
...
def merge_single_qubit_gates_into_phased_x_z(
        circuit,
        atol = 1e-8):
    u"""Canonicalizes runs of single-qubit rotations in a circuit.
