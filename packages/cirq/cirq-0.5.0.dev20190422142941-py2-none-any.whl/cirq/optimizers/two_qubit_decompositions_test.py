...
@pytest.mark.parametrize(u'rad,expected', (lambda err, largeErr: [
    (np.pi/4, True),
    (np.pi/4 + err, True),
...
def test_is_trivial_angle(rad, expected):
    ...
    assert out == expected, u'rad = {}'.format(rad)
...
@pytest.mark.parametrize(u'max_partial_cz_depth,max_full_cz_depth,effect', [
    (0, 0, np.eye(4)),
    (0, 0, np.array([
        [0, 0, 0, 1],
        [0, 0, 1, 0],
    ...
    (1, 2, _random_single_partial_cz_effect()) for _ in xrange(10)
] + [
    (2, 2, _random_double_full_cz_effect()) for _ in xrange(10)
] + [
    (2, 3, _random_double_partial_cz_effect()) for _ in xrange(10)
] + [
    (3, 3, cirq.testing.random_unitary(4)) for _ in xrange(10)
])
