...
class MergeInteractions(circuits.PointOptimizer):
    u"""Combines series of adjacent one and two-qubit gates operating on a pair
    of qubits."""

    def __init__(self,
                 tolerance = 1e-8,
                 allow_partial_czs = True,
                 post_clean_up = lambda op_list: op_list):
        super(MergeInteractions, self).__init__(post_clean_up=post_clean_up)
    ...
    def _op_to_matrix(self,
        ...
        u"""Determines the effect of an operation on the given qubits.
    ...
    def _scan_two_qubit_ops_into_matrix(
        ...
        u"""Accumulates operations affecting the given pair of qubits.
        ...
        while index is not None:
            operations = list(set(circuit.operation_at(q, index) for q in qubits))
            op_data = [
    ...
    @staticmethod
    def _flip_kron_order(mat4x4):
        u"""Given M = sum(kron(a_i, b_i)), returns M' = sum(kron(b_i, a_i))."""
