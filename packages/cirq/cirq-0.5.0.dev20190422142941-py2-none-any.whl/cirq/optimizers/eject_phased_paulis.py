...
u"""Pushes 180 degree rotations around axes in the XY plane later in the circuit.
"""
...
class EjectPhasedPaulis(object):
    u"""Pushes X, Y, and PhasedX gates towards the end of the circuit.
...
def _absorb_z_into_w(moment_index,
                     op,
                     state):
    u"""Absorbs a Z^t gate into a W(a) flip.
...
def _dump_into_measurement(moment_index,
                           op,
                           state):
...
def _potential_cross_whole_w(moment_index,
                             op,
                             tolerance,
                             state):
    u"""Grabs or cancels a held W gate against an existing W gate.
...
def _potential_cross_partial_w(moment_index,
                               op,
                               state):
    u"""Cross the held W over a partial W gate.
...
def _single_cross_over_cz(moment_index,
                          op,
                          qubit_with_w,
                          state):
    u"""Crosses exactly one W flip over a partial CZ.
...
def _double_cross_over_cz(op,
                          state):
    u"""Crosses two W flips over a partial CZ.
...
def _try_get_known_cz_half_turns(op):
...
def _try_get_known_phased_pauli(op
                                ):
...
def _try_get_known_z_half_turns(op):
    if (not isinstance(op, ops.GateOperation) or
