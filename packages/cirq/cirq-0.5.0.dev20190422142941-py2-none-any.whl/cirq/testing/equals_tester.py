...
u"""A utility class for testing equality methods.
...
class EqualsTester(object):
    u"""Tests equality against user-provided disjoint equivalence groups."""
    ...
    def _eq_check(v1, v2):
        ...
        assert eq != ne, (u"__eq__ is inconsistent with __ne__ "
                          u"between {!r} and {!r}".format(v1, v2))
        return eq

    def _verify_equality_group(self, *group_items):
        u"""Verifies that a group is an equivalence group.
        ...
        for v1, v2 in itertools.product(group_items, group_items):
            ...
            assert same or v1 is not v2, u"{!r} isn't equal to itself!".format(
                v1)
            assert same, (
                u"{!r} and {!r} can't be in the same equality group. "
                u"They're not equal.".format(v1, v2))
        ...
        for other_group in self._groups:
            for v1, v2 in itertools.product(group_items, other_group):
                assert not EqualsTester._eq_check(v1, v2), (
                    u"{!r} and {!r} can't be in different equality groups. "
                    u"They're equal.".format(v1, v2))
        ...
        if len(set(hashes)) > 1:
            examples = ((v1, h1, v2, h2)
                        for v1, h1 in izip(group_items, hashes)
                        for v2, h2 in izip(group_items, hashes)
                        if h1 != h2)
            example = examples.next()
            raise AssertionError(
                u'Items in the same group produced different hashes. '
                u'Example: hash({!r}) is {!r} but hash({!r}) is {!r}.'.format(
                    *example))

    def add_equality_group(self, *group_items):
        u"""Tries to add a disjoint equivalence group to the equality tester.
    ...
    def make_equality_group(self, *factories):
        u"""Tries to add a disjoint equivalence group to the equality tester.
        ...
        """
        self.add_equality_group(*(f() for f in factories for _ in xrange(2)))


class _ClassUnknownToSubjects(object):
    u"""Equality methods should be able to deal with the unexpected."""
