...
def random_circuit(qubits,
                   n_moments,
                   op_density,
                   gate_domain = None
                   ):
    u"""Generates a random circuit.
    ...
    if not 0 < op_density < 1:
        raise ValueError(u'op_density must be in (0, 1).')
    if gate_domain is None:
        gate_domain = DEFAULT_GATE_DOMAIN
    if not gate_domain:
        raise ValueError(u'gate_domain must be non-empty')
    max_arity = max(gate_domain.values())

    if isinstance(qubits, int):
        qubits = tuple(ops.NamedQubit(unicode(i)) for i in xrange(qubits))
    n_qubits = len(qubits)
    if n_qubits < 1:
        raise ValueError(u'At least one qubit must be specified.')

    moments = [] # type: List[ops.Moment]
    for _ in xrange(n_moments):
        operations = []
