...
def test_add_equality_group_correct():
    ...
    eq.add_equality_group({u'b': 3, u'a': 2}, {u'a': 2, u'b': 3})

    eq.add_equality_group(u'unrelated')


def test_assert_make_equality_group():
    eq = EqualsTester()

    with pytest.raises(AssertionError, match=u"can't be in the same"):
    ...
    with pytest.raises(AssertionError, match=u"can't be in different"):
...
def test_add_equality_group_bad_hash():
    ...
    eq = EqualsTester()
    eq.add_equality_group(KeyHash(u'a', 5), KeyHash(u'a', 5))
    eq.add_equality_group(KeyHash(u'b', 5))
    with pytest.raises(AssertionError, match=u'produced different hashes'):
        eq.add_equality_group(KeyHash(u'c', 2), KeyHash(u'c', 3))


def test_add_equality_group_exception_hash():
    class FailHash(object):
        def __hash__(self):
            raise ValueError(u'injected failure')

    eq = EqualsTester()
    with pytest.raises(ValueError, match=u'injected failure'):
...
def test_fails_when_forgot_type_check():
    ...
    with pytest.raises(AttributeError, match=u"has no attribute 'x'"):
...
def test_fails_hash_is_default_and_inconsistent():
    ...
    with pytest.raises(AssertionError, match=u'different hashes'):
...
def test_fails_when_ne_is_inconsistent():
    ...
    with pytest.raises(AssertionError, match=u'inconsistent'):
...
def test_fails_when_not_reflexive():
    ...
    with pytest.raises(AssertionError, match=u'equal to itself'):
