...
u"""Defines which types are Sweepable."""
...
def to_resolvers(sweepable):
    u"""Convert a Sweepable to a list of ParamResolvers."""
    if isinstance(sweepable, ParamResolver):
        ...
        return list(iterable) if isinstance(iter(iterable).next(),
                                            ParamResolver) else sum(
            [list(s) for s in iterable], [])
    raise TypeError(u'Unexpected Sweepable type.')