...
def test_value_of():
    ...
    assert r.value_of(u'x') == sympy.Symbol(u'x')
    assert r.value_of(u'a') == 0.5
...
def test_equals():
    ...
    et.make_equality_group(lambda: cirq.ParamResolver({u'a': 0.0}))
...
def test_repr():
    ...
    cirq.testing.assert_equivalent_repr(cirq.ParamResolver({u'a': 2.0}))
