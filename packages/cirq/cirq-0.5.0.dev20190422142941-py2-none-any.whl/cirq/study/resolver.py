...
u"""Resolves ParameterValues to assigned values."""
...
ParamResolverOrSimilarType = Union[u'cirq.ParamResolver', Dict[unicode, float], None]


class ParamResolver(object):
    u"""Resolves sympy.Symbols to actual values.
    ...
    def __new__(cls, param_dict = None):
        if isinstance(param_dict, ParamResolver):
            return param_dict
        return super(ParamResolver, cls).__new__(cls)

    def __init__(self, param_dict = None):
        if hasattr(self, u'_param_hash'):
            return  # Already initialized. Got wrapped as part of the __new__.

        self.param_dict = cast(Dict[unicode, float],
    ...
    def value_of(
        ...
        u"""Attempt to resolve a Symbol or name or float to its assigned value.
        ...
        if isinstance(value, sympy.Basic):
            if sys.version_info.major < 3:
                ...
                d = dict((k.encode(), v) for k, v in self.param_dict.items())
                v = value.subs(d)
            else:
    ...
    def __repr__(self):
        return u'cirq.ParamResolver({})'.format(repr(self.param_dict))
