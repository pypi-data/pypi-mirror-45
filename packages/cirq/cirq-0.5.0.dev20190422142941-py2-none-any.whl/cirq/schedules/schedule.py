...
class Schedule(object):
    u"""A quantum program with operations happening at specific times.
    ...
    def query(self, **_3to2kwargs):
        if 'include_op_end_times' in _3to2kwargs: include_op_end_times = _3to2kwargs['include_op_end_times']; del _3to2kwargs['include_op_end_times']
        else: include_op_end_times = False
        if 'include_query_end_time' in _3to2kwargs: include_query_end_time = _3to2kwargs['include_query_end_time']; del _3to2kwargs['include_query_end_time']
        else: include_query_end_time = False
        if 'qubits' in _3to2kwargs: qubits = _3to2kwargs['qubits']; del _3to2kwargs['qubits']
        else: qubits =  None
        if 'duration' in _3to2kwargs: duration = _3to2kwargs['duration']; del _3to2kwargs['duration']
        else: duration =  Duration()
        time = _3to2kwargs['time']; del _3to2kwargs['time']
        u"""Finds operations by time and qubit.
    ...
    def __getitem__(self, item):
        u"""Finds operations overlapping a given time or time slice.
        ...
        if isinstance(item, slice):
            if item.step:
                raise ValueError(u'Step not supported.')
    ...
    def operations_happening_at_same_time_as(
        ...
        u"""Finds operations happening at the same time as the given operation.
    ...
    def include(self, scheduled_operation):
        u"""Adds a scheduled operation to the schedule.
        ...
        if collisions:
            raise ValueError(u'Operation {} has collisions: {}'.format(
    ...
    def exclude(self, scheduled_operation):
        u"""Omits a scheduled operation from the schedule, if present.
    ...
    def to_circuit(self):
        u"""Convert the schedule to a circuit.
