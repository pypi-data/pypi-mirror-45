...
def test_init():
    ...
    assert d.qubits == set([q00, q01, q10])
...
def test_repr():
    ...
    assert repr(d) == (u"XmonDevice("
                       u"measurement_duration=cirq.Duration(picos=1000), "
                       u"exp_w_duration=cirq.Duration(picos=2000), "
                       u"exp_11_duration=cirq.Duration(picos=3000) "
                       u"qubits=[cirq.GridQubit(0, 0), cirq.GridQubit(0, 1), "
                       u"cirq.GridQubit(1, 0), "
                       u"cirq.GridQubit(1, 1)])")
...
def test_validate_circuit_repeat_measurement_keys():
    ...
    circuit.append([cirq.measure(cirq.GridQubit(0, 0), key=u'a'),
                    cirq.measure(cirq.GridQubit(0, 1), key=u'a')])

    with pytest.raises(ValueError, message=u'Measurement key a repeated'):
...
def test_validate_schedule_repeat_measurement_keys():
    ...
    s = cirq.Schedule(d, [
        cirq.ScheduledOperation.op_at_on(
            cirq.measure(cirq.GridQubit(0, 0), key=u'a'), cirq.Timestamp(), d),
        cirq.ScheduledOperation.op_at_on(
            cirq.measure(cirq.GridQubit(0, 1), key=u'a'), cirq.Timestamp(), d),
    ])
