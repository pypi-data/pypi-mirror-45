...
def _accept(random_sample, cost_diff,
            temp):
    u"""Calculates probability and draws if solution should be accepted.
...
def anneal_minimize(
        ...
        repeat = 100,
        trace_func = None):
    u"""Minimize solution using Simulated Annealing meta-heuristic.
    ...
    if not 0.0 < cooling_factor < 1.0:
        raise ValueError(u"Cooling factor must be within (0, 1) range")
