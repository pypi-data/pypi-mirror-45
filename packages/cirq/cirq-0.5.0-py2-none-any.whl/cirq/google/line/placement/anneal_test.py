...
# WITHOUT WARRANTIES OR CONDITIONS OF mock.ANY KIND, either express or implied.
...
@mock.patch(u'cirq.google.line.placement.optimization.anneal_minimize')
...
def test_search_converts_trace_func(anneal_minimize):
    ...
    wrapper_func = anneal_minimize.call_args[1][u'trace_func']
...
def test_force_edges_active_move_calls_force_edge_active_move():
    ...
    search = AnnealSequenceSearch(
        _create_device([q00, q01, q10, q11]),
        seed=0xF00D0005)
    with mock.patch.object(search, u'_force_edge_active_move') \
...
def test_force_edge_active_move_calls_force_edge_active():
    ...
    with mock.patch.object(search, u'_force_edge_active') as force_edge_active:
...
def test_force_edge_active_creates_valid_solution_different_sequnces():
    q00, q10, q20, q30 = [GridQubit(x, 0) for x in xrange(4)]
    q01, q11, q21, q31 = [GridQubit(x, 1) for x in xrange(4)]
...
def test_force_edge_active_creates_valid_solution_single_sequence():
    ...
    assert search._force_edge_active(
        ...
        lambda: samples.next()) == [[q30], [q31, q21, q20, q10, q00, q01, q11]]
...
def test_create_initial_solution_creates_valid_solution():
    ...
    q00, q01, q02 = [GridQubit(0, x) for x in xrange(3)]
    q10, q11, q12 = [GridQubit(1, x) for x in xrange(3)]
...
def test_choose_random_edge_chooses():
    q00, q11, q22 = [GridQubit(x, x) for x in xrange(3)]
    ...
    assert search._choose_random_edge(set([e0])) == e0
    assert search._choose_random_edge(set([e0, e1, e2])) in [e0, e1, e2]


def _verify_valid_state(qubits, state):
    seqs, edges = state
...
def test_index_2d():
    ...
    assert index_2d([[], [u'a']], u'a') == (1, 0)
    assert index_2d([[u'a', u'a']], u'a') == (0, 0)
    assert index_2d([[u'a'], [u'a']], u'a') == (0, 0)
    assert index_2d([[u'a', u'a'], [u'a']], u'a') == (0, 0)
