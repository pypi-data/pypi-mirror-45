...
u"""Global level manager of shared numpy arrays."""
...
class SharedMemManager(object):
    u"""Manager of global shared numpy arrays.
    ...
    def _create_array(self, arr):
        u"""Returns the handle of a RawArray created from the given numpy array.
        ...
        if not isinstance(arr, np.ndarray):
            raise ValueError(u'Array is not a numpy ndarray.')
        try:
            c_arr = np.ctypeslib.as_ctypes(arr)
        except (KeyError, NotImplementedError):
            raise ValueError(
                u'Array has unsupported dtype {}.'.format(arr.dtype))
    ...
    def _get_next_free(self):
        ...
        while self._arrays[self._current] is not None:
            ...
            if loop_count == len(self._arrays):
                raise RuntimeError(
                    u'Cannot find free space to allocate new array.')

    def _free_array(self, handle):
        u"""Frees the memory for the array with the given handle.
    ...
    def _get_array(self, handle):
        u"""Returns the array with the given handle.
        ...
        with warnings.catch_warnings():
            warnings.simplefilter(u'ignore', RuntimeWarning)
    ...
    @staticmethod
    def get_instance():
        u"""Get the SharedMemManager instance."""
