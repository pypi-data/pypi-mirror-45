...
class ConvertToCzAndSingleGates(circuits.PointOptimizer):
    u"""Attempts to convert strange multi-qubit gates into CZ and single qubit
    gates.
    ...
    def __init__(self,
        ...
        """
        super(ConvertToCzAndSingleGates, self).__init__()
    ...
    def _decompose_two_qubit_unitaries(self, op):
        # Known matrix?
        if len(op.qubits) == 2:
    ...
    def _on_stuck_raise(self, op):
        raise TypeError(u"Don't know how to work with {!r}. "
                        u"It isn't composite or an operation with a "
                        u"known unitary effect on 1 or 2 qubits.".format(op))
