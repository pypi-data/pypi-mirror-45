...
def two_qubit_matrix_to_operations(q0,
                                   q1,
                                   mat,
                                   allow_partial_czs,
                                   atol = 1e-8,
    ...
    u"""Decomposes a two-qubit operation into Z/XY/CZ gates.
...
def _xx_yy_zz_interaction_via_full_czs(q0,
                                       q1,
                                       x,
                                       y,
                                       z):
...
def _cleanup_operations(operations):
...
def _kak_decomposition_to_operations(q0,
                                     q1,
                                     kak,
                                     allow_partial_czs,
                                     atol = 1e-8
                                     ):
    u"""Assumes that the decomposition is canonical."""
...
def _is_trivial_angle(rad, atol):
    u"""Tests if a circuit for an operator exp(i*rad*XX) (or YY, or ZZ) can
...
def _parity_interaction(q0,
                        q1,
                        rads,
                        atol,
                        gate = None):
    u"""Yields a ZZ interaction framed by the given operation."""
    if abs(rads) < atol:
        return
...
def _do_single_on(u, q, atol=1e-8):
    for gate in decompositions.single_qubit_matrix_to_gates(u, atol):
        yield gate(q)


def _non_local_part(q0,
                    q1,
                    interaction_coefficients,
                    allow_partial_czs,
                    atol = 1e-8):
    u"""Yields non-local operation of KAK decomposition."""
