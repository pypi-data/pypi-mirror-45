...
class ReplaceWithXGates(PointOptimizer):
    u"""Replaces a block of operations with X gates.
    ...
    def optimization_at(self, circuit, index, op):
        ...
        while not done:
            ...
            if n is None:
                break
            next_ops = set(circuit.operation_at(q, n) for q in op.qubits)
            next_ops = [e for e in next_ops if e]
            next_ops = sorted(next_ops, key=lambda e: unicode(e.qubits))
...
def test_repr():
    ...
    ], new_operations=[])) == u'cirq.PointOptimizationSummary(0, (), ())'
