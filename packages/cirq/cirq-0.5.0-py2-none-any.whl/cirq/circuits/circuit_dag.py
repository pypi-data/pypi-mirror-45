...
class Unique(Generic[T]):
    u"""A wrapper for a value that doesn't compare equal to other instances.
    ...
    def __repr__(self):
        return u'cirq.Unique({}, {!r})'.format(id(self), self.val)
...
Unique = functools.total_ordering(Unique)

def _disjoint_qubits(op1, op2):
    u"""Returns true only if the operations have qubits in common."""
    return not set(op1.qubits) & set(op2.qubits)


class CircuitDag(networkx.DiGraph):
    u"""A representation of a Circuit as a directed acyclic graph.
    ...
    def __init__(self,
                 can_reorder = _disjoint_qubits,
                 incoming_graph_data = None,
                 device = devices.UnconstrainedDevice
                 ):
        u"""Initializes a CircuitDag.
        ...
        """
        super(CircuitDag, self).__init__(incoming_graph_data)
    ...
    def from_ops(*operations, **_3to2kwargs
        ...
        if 'can_reorder' in _3to2kwargs: can_reorder = _3to2kwargs['can_reorder']; del _3to2kwargs['can_reorder']
        else: can_reorder =  _disjoint_qubits
    ...
    def __eq__(self, other):
        ...
        def node_match(attr1, attr2):
            return attr1[u'val'] == attr2[u'val']
    ...
    def ordered_nodes(self):
        ...
        def get_root_node(some_node
            ...
            while pred[some_node]:
                some_node = iter(pred[some_node]).next()
            return some_node

        def get_first_node():
            return get_root_node(iter(g.nodes()).next())

        def get_next_node(succ
                          ):
            if succ:
                return get_root_node(iter(succ).next())
            else:
