...
u"""Defines the OptimizationPass type."""
...
class PointOptimizationSummary(object):
    u"""A description of a local optimization to perform."""
    ...
    def __repr__(self):
        return u'cirq.PointOptimizationSummary({!r}, {!r}, {!r})'.format(
            self.clear_span,
            self.clear_qubits,
            self.new_operations)


class PointOptimizer(object):
    u"""Makes circuit improvements focused on a specific location."""
    ...
    def optimization_at(self,
        ...
        u"""Describes how to change operations near the given location.
        ...
        while i < len(circuit):  # Note: circuit may mutate as we go.
            for op in circuit[i].operations:
                ...
                circuit.clear_operations_touching(
                    opt.clear_qubits,
                    [e for e in xrange(i, i + opt.clear_span)])
