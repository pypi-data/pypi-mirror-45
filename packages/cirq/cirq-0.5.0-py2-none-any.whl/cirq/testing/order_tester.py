...
u"""A utility class for testing ordering methods.
...
class OrderTester(object):
    u"""Tests ordering against user-provided disjoint ordered groups or items."""

    def __init__(self):
        self._groups = []
        self._eq_tester = EqualsTester()

    def _verify_ordering_one_sided(self, a, b, sign):
        u"""Checks that (a vs b) == (0 vs sign)."""
        for cmp_name, cmp_func in _NAMED_COMPARISON_OPERATORS:
            expected = cmp_func(0, sign)
            actual = cmp_func(a, b)
            assert expected == actual, (
                u"Ordering constraint violated. Expected X={} to {} Y={}, "
                u"but X {} Y returned {}".format(
                    a,
                    [u'be more than', u'equal', u'be less than'][sign + 1],
                    b,
                    cmp_name,
                    actual))

    def _verify_ordering(self, a, b, sign):
        u"""Checks that (a vs b) == (0 vs sign) and (b vs a) == (sign vs 0)."""
        self._verify_ordering_one_sided(a, b, sign)
        self._verify_ordering_one_sided(b, a, -sign)

    def _verify_not_implemented_vs_unknown(self, item):
        try:
            ...
            raise AssertionError(
                u"Objects should return NotImplemented when compared to an "
                u"unknown value, i.e. comparison methods should start with\n"
                u"\n"
                u"    if not isinstance(other, type(self)):\n"
                u"        return NotImplemented\n"
                u"\n"
                u"That rule is being violated by this value: {!r}".format(
                    item))

    def add_ascending(self, *items):
        u"""Tries to add a sequence of ascending items to the order tester.
    ...
    def add_ascending_equivalence_group(self, *group_items):
        u"""Tries to add an ascending equivalence group to the order tester.
...
class _EqualToEverything(object):
    ...
    def __repr__(self):
        return u'_EqualToEverything'


class _SmallerThanEverythingElse(object):
    def __eq__(self, other):
    ...
    def __repr__(self):
        return u'SmallerThanEverythingElse'


class _LargerThanEverythingElse(object):
    def __eq__(self, other):
    ...
    def __repr__(self):
        return u'LargerThanEverythingElse()'
