...
def _assert_not_mirror(gate):
    ...
    assert right_handed, u'Mirrors'


def _assert_no_collision(gate):
    trans_x = gate.transform(cirq.X)
    trans_y = gate.transform(cirq.Y)
    trans_z = gate.transform(cirq.Z)
    assert trans_x.to != trans_y.to, u'Collision'
    assert trans_y.to != trans_z.to, u'Collision'
    assert trans_z.to != trans_x.to, u'Collision'
...
@pytest.mark.parametrize(u'pauli,flip_x,flip_z',
...
@pytest.mark.parametrize(u'trans_x,trans_z', _all_rotation_pairs())
...
@pytest.mark.parametrize(u'trans1,trans2,from1',
    ((trans1, trans2, from1)
...
def test_init_from_double_map_vs_kwargs(trans1, trans2, from1):
    ...
    from1_str, from2_str = (unicode(frm).lower()+u'_to' for frm in (from1, from2))
...
@pytest.mark.parametrize(u'pauli,sqrt,expected', (
...
@pytest.mark.parametrize(u'gate', _all_clifford_gates())
...
@pytest.mark.parametrize(u'gate,rep', (
    (cirq.SingleQubitCliffordGate.I,
     u'cirq.SingleQubitCliffordGate(X:+X, Y:+Y, Z:+Z)'),
    (cirq.SingleQubitCliffordGate.H,
     u'cirq.SingleQubitCliffordGate(X:+Z, Y:-Y, Z:+X)'),
    (cirq.SingleQubitCliffordGate.X,
     u'cirq.SingleQubitCliffordGate(X:+X, Y:-Y, Z:-Z)'),
    (cirq.SingleQubitCliffordGate.X_sqrt,
     u'cirq.SingleQubitCliffordGate(X:+X, Y:+Z, Z:-Y)')))
def test_repr(gate, rep):
    assert repr(gate) == rep


@pytest.mark.parametrize(u'gate,trans_y', (
...
@pytest.mark.parametrize(u'gate,gate_equiv', (
...
@pytest.mark.parametrize(u'gate,other',
...
@pytest.mark.parametrize(u'gate,pauli,half_turns',
    itertools.product(_all_clifford_gates(),
                      _paulis,
                      (0.1, 0.25, 0.5, -0.5)))
...
@pytest.mark.parametrize(u'gate,sym,exp', (
    (cirq.SingleQubitCliffordGate.I,       u'I', 1),
    (cirq.SingleQubitCliffordGate.H,       u'H', 1),
    (cirq.SingleQubitCliffordGate.X,       u'X', 1),
    (cirq.SingleQubitCliffordGate.X_sqrt,  u'X', 0.5),
    (cirq.SingleQubitCliffordGate.X_nsqrt, u'X', -0.5),
    (
