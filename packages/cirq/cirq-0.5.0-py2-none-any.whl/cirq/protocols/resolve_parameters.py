...
class SupportsParameterization(Protocol):
    u"""An object that can be parameterized by Symbols and resolved
    via a ParamResolver"""

    def _is_parameterized_(self):
        u"""Whether the gate is parameterized by any Symbols that require
...
def is_parameterized(val):
    u"""Returns whether the object is parameterized with any Symbols.
    ...
    getter = getattr(val, u'_is_parameterized_', None)
...
def resolve_parameters(
        val,
        param_resolver):
    u"""Resolves symbol parameters in the effect using the param resolver.
    ...
    getter = getattr(val, u'_resolve_parameters_', None)
