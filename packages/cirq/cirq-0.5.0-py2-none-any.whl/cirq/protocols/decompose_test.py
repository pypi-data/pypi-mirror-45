...
class DecomposeNotImplemented(object):
    def _decompose_(self, **kwargs):
        return NotImplemented


class DecomposeNone(object):
    def _decompose_(self, **kwargs):
        return None


class DecomposeGiven(object):
...
class DecomposeWithQubitsGiven(object):
...
class DecomposeGenerated(object):
    def _decompose_(self):
...
def test_decompose_once():
    ...
    with pytest.raises(TypeError, match=u'no _decompose_ method'):
    ...
    op = cirq.X(cirq.NamedQubit(u'q'))
    assert cirq.decompose_once(DecomposeGiven(op)) == [op]
    assert cirq.decompose_once(DecomposeGiven([[[op]], []])) == [op]
    assert cirq.decompose_once(DecomposeGiven(op for _ in xrange(2))) == [op, op]
    assert type(cirq.decompose_once(DecomposeGiven(op for _ in xrange(2)))
                ) == list
...
def test_decompose_on_stuck_raise():
    ...
    with pytest.raises(ValueError, match=u"but can't be decomposed"):
    ...
    with pytest.raises(TypeError, match=u'test'):
        _ = cirq.decompose(
            no_method, keep=lambda _: False, on_stuck_raise=TypeError(u'test'))
    with pytest.raises(NotImplementedError, match=u'op cirq.CZ.on'):
        _ = cirq.decompose(
            cirq.CZ(a, b),
            keep=lambda _: False,
            on_stuck_raise=lambda op: NotImplementedError(u'op {!r}'.format(
                op)))


    # There's a nice warning if you specify `on_stuck_raise` but not `keep`.
    with pytest.raises(ValueError, match=u'on_stuck_raise'):
        assert cirq.decompose([], on_stuck_raise=None)
    with pytest.raises(ValueError, match=u'on_stuck_raise'):
        assert cirq.decompose([], on_stuck_raise=TypeError(u'x'))
