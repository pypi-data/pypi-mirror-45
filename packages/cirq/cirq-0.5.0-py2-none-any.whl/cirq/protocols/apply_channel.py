...
u"""A protocol for implementing high performance channel evolutions."""
...
class ApplyChannelArgs(object):
    ur"""Arguments for efficiently performing a channel.
...
class SupportsApplyChannel(Protocol):
    u"""An object that can efficiently implement a channel."""

    def _apply_channel_(self, args
    ):
        u"""Efficiently applies a channel.
...
def apply_channel(val,
    ...
    u"""High performance evolution under a channel evolution.
    ...
    func = getattr(val, u'_apply_channel_', None)
    if func is not None:
        result = func(args)
        if result is not NotImplemented and result is not None:
            def err_str(buf_num_str):
                return (
                    u"Object of type '{}' returned a result object equal to "
                    u"auxiliary_buffer{}. This type violates the contract "
                    u"that appears in apply_unitary's documentation.".format(
                        type(val), buf_num_str))
            assert result is not args.auxiliary_buffer0, err_str(u'0')
            assert result is not args.auxiliary_buffer1, err_str(u'1')
            return result
    ...
    if default is not RaiseTypeErrorIfNotProvided:
        return default
    raise TypeError(
            u"object of type '{}' has no _apply_channel_, _apply_unitary_, "
            u"_unitary_, or _channel_ methods (or they returned None or "
            u"NotImplemented).".format(type(val)))


def _apply_unitary(val, args):
    u"""Attempt to use `apply_unitary` and return the result.
...
def _apply_krauss(krauss,
        args):
    u"""Directly apply the kraus operators to the target tensor."""
    # Initialize output.
    args.out_buffer[:] = 0
...
def _apply_krauss_single_qubit(krauss,
        args):
    u"""Use slicing to apply single qubit channel."""
...
def _apply_krauss_multi_qubit(krauss,
        args):
    u"""Use numpy's einsum to apply a multi-qubit channel."""
    for krauss_op in krauss:
