...
@pytest.mark.parametrize(u'controllee', (
    NoMethod(),
    object(),
...
def test_controlled_by_error():
    ...
    with pytest.raises(TypeError, match=u"no controlled_by method"):
        _ = cirq.protocols.control(NoMethod(), [p])


@pytest.mark.parametrize(u'controllee,control_qubits,out', (
...
def test_op_tree_control():
    ...
    expected= [
        cirq.ControlledOperation(
            controls, cirq.GateOperation(gs[i], [cirq.NamedQubit(unicode(i))]))
        for i in xrange(10)
    ]
