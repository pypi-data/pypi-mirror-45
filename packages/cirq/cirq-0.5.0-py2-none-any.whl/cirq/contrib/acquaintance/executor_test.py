...
class ExampleGate(cirq.Gate):
    ...
    def __init__(self, wire_symbols):
...
def test_executor_explicit():
    ...
    gates = {(i, j): ExampleGate([unicode(k) for k in ij])
             for ij in combinations(xrange(num_qubits), 2)
             for i, j in (ij, ij[::-1])}
    initial_mapping = dict((q, i) for i, q in enumerate(sorted(qubits)))
    ...
    with pytest.raises(NotImplementedError):
        bad_gates = {(0,): ExampleGate([u'0']), (0, 1): ExampleGate([u'0', u'1'])}
...
class DiagonalGate(cirq.Gate):
    ...
    def __init__(self, num_qubits, diagonal):
        dimension = 2 ** num_qubits
        if (diagonal.shape != (dimension,) or not
            np.allclose(
                np.absolute(diagonal), np.ones(dimension))):
            raise ValueError(u'Diagonal must be an (2**num_qubits)-dimensional '
                    u'vector with unit-norm entries.')
    ...
    def _circuit_diagram_info_(self, args):
        ...
        return (u'Diag',) * qubit_count

    def num_qubits(self):
        return self._num_qubits

    @staticmethod
    def random(num_qubits):
        dimension = 2 ** num_qubits
...
def random_diagonal_gates(num_qubits,
                 acquaintance_size
                 ):

    return dict((Q, DiagonalGate.random(acquaintance_size))
             for Q in
             combinations(cirq.LineQubit.range(num_qubits), acquaintance_size))


@pytest.mark.parametrize(u'num_qubits, acquaintance_size, gates',
    [(num_qubits, acquaintance_size,
      random_diagonal_gates(num_qubits, acquaintance_size))
      for acquaintance_size, num_qubits in
      ([(2, n) for n in xrange(2, 9)] +
       [(3, n) for n in xrange(3, 9)] +
       [(4, n) for n in (4, 7)] +
       [(5, n) for n in (5, 6)])
      for _ in xrange(2)
      ])
def test_executor_random(num_qubits,
                         acquaintance_size,
                         gates):
    qubits = cirq.LineQubit.range(num_qubits)
    ...
    initial_mapping = dict((q, q) for q in qubits)
    final_mapping = cca.GreedyExecutionStrategy(gates, initial_mapping)(circuit)
    permutation = dict((q.x, qq.x) for q, qq in final_mapping.items())
...
def test_acquaintance_operation():
    ...
    for logical_indices in (logical_qubits, int_indices):
        ...
        wire_symbols = tuple(u'({})'.format(i) for i in logical_indices)
