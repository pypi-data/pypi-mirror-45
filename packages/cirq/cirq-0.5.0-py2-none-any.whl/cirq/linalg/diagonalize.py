...
u"""Utility methods for diagonalizing matrices."""
...
def diagonalize_real_symmetric_matrix(
    ...
    u"""Returns an orthogonal matrix that diagonalizes the given matrix.
    ...
    if np.any(np.imag(matrix) != 0) or not predicates.is_hermitian(matrix):
        raise ValueError(u'Input must be real and symmetric.')
...
def _contiguous_groups(
    ...
    u"""Splits range(length) into approximate equivalence classes.
...
def diagonalize_real_symmetric_and_sorted_diagonal_matrices(
    ...
    u"""Returns an orthogonal matrix that diagonalizes both given matrices.
    ...
    if check_preconditions:
        if (np.any(np.imag(symmetric_matrix)) or
                not predicates.is_hermitian(symmetric_matrix,
                                            rtol=rtol,
                                            atol=atol)):
            raise ValueError(u'symmetric_matrix must be real symmetric.')
        if (not predicates.is_diagonal(diagonal_matrix, atol=atol) or
            ...
            raise ValueError(
                u'diagonal_matrix must be real diagonal descending.')
        if not predicates.commutes(diagonal_matrix,
                                   symmetric_matrix,
                                   rtol=rtol,
                                   atol=atol):
            raise ValueError(u'Given matrices must commute.')
...
def bidiagonalize_real_matrix_pair_with_symmetric_products(
    ...
    u"""Finds orthogonal matrices that diagonalize both mat1 and mat2.
    ...
    if check_preconditions:
        if np.any(np.imag(mat1) != 0):
            raise ValueError(u'mat1 must be real.')
        if np.any(np.imag(mat2) != 0):
            raise ValueError(u'mat2 must be real.')
        if not predicates.is_hermitian(mat1.dot(mat2.T), rtol=rtol, atol=atol):
            raise ValueError(u'mat1 @ mat2.T must be symmetric.')
        if not predicates.is_hermitian(mat1.T.dot(mat2), rtol=rtol, atol=atol):
            raise ValueError(u'mat1.T @ mat2 must be symmetric.')
...
def bidiagonalize_unitary_with_special_orthogonals(
    ...
    u"""Finds orthogonal matrices L, R such that L @ matrix @ R is diagonal.
    ...
    if check_preconditions:
        if not predicates.is_unitary(mat, rtol=rtol, atol=atol):
            raise ValueError(u'matrix must be unitary.')
