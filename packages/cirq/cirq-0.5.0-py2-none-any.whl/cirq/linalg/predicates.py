...
u"""Utility methods for checking properties of matrices."""
...
def is_diagonal(matrix, **_3to2kwargs):
    if 'atol' in _3to2kwargs: atol = _3to2kwargs['atol']; del _3to2kwargs['atol']
    else: atol =  1e-8
    u"""Determines if a matrix is a approximately diagonal.
    ...
    for i in xrange(min(matrix.shape)):
        matrix[i, i] = 0
...
def is_hermitian(
    ...
    u"""Determines if a matrix is approximately Hermitian.
...
def is_orthogonal(
    ...
    u"""Determines if a matrix is approximately orthogonal.
...
def is_special_orthogonal(
    ...
    u"""Determines if a matrix is approximately special orthogonal.
...
def is_unitary(
    ...
    u"""Determines if a matrix is approximately unitary.
...
def is_special_unitary(
    ...
    u"""Determines if a matrix is approximately unitary with unit determinant.
...
def commutes(
    ...
    u"""Determines if two matrices approximately commute.
...
def allclose_up_to_global_phase(
    ...
    else: equal_nan =  False
    if 'atol' in _3to2kwargs: atol = _3to2kwargs['atol']; del _3to2kwargs['atol']
    else: atol =  1.e-8
    if 'rtol' in _3to2kwargs: rtol = _3to2kwargs['rtol']; del _3to2kwargs['rtol']
    else: rtol =  1.e-5
    u"""Determines if a ~= b * exp(i t) for some t.
...
def slice_for_qubits_equal_to(target_qubit_axes,
                              little_endian_qureg_value, **_3to2kwargs
                              ):
    if 'num_qubits' in _3to2kwargs: num_qubits = _3to2kwargs['num_qubits']; del _3to2kwargs['num_qubits']
    else: num_qubits =  None
    u"""Returns an index corresponding to a desired subset of an np.ndarray.
