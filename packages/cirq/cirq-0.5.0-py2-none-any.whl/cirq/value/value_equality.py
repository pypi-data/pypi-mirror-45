...
u"""Defines `@cirq.value_equality`, for easy __eq__/__hash__ methods."""
...
class _SupportsValueEquality(Protocol):
    u"""An object decorated with the value equality decorator."""

    def _value_equality_values_(self):
        u"""Returns a value or values that define the identity of this object.
    ...
    def _value_equality_approximate_values_(self):
        u"""Returns value or values used for approximate equality.
    ...
    def _value_equality_values_cls_(self):
        u"""Automatically implemented by the `cirq.value_equality` decorator.
...
def _value_equality_eq(self,
                       other):
    cls_self = self._value_equality_values_cls_()
    get_cls_other = getattr(other, u'_value_equality_values_cls_', None)
...
def _value_equality_approx_eq(self,
                              other,
                              atol):
...
@overload
def value_equality(cls, **_3to2kwargs):
    if 'approximate' in _3to2kwargs: approximate = _3to2kwargs['approximate']; del _3to2kwargs['approximate']
    else: approximate =  False
    if 'manual_cls' in _3to2kwargs: manual_cls = _3to2kwargs['manual_cls']; del _3to2kwargs['manual_cls']
    else: manual_cls =  False
    if 'distinct_child_types' in _3to2kwargs: distinct_child_types = _3to2kwargs['distinct_child_types']; del _3to2kwargs['distinct_child_types']
    else: distinct_child_types =  False
    if 'unhashable' in _3to2kwargs: unhashable = _3to2kwargs['unhashable']; del _3to2kwargs['unhashable']
    else: unhashable =  False
    pass


@overload
def value_equality( **_3to2kwargs):
    if 'approximate' in _3to2kwargs: approximate = _3to2kwargs['approximate']; del _3to2kwargs['approximate']
    else: approximate =  False
    if 'manual_cls' in _3to2kwargs: manual_cls = _3to2kwargs['manual_cls']; del _3to2kwargs['manual_cls']
    else: manual_cls =  False
...
def value_equality(cls = None, **_3to2kwargs
                  ):
    if 'approximate' in _3to2kwargs: approximate = _3to2kwargs['approximate']; del _3to2kwargs['approximate']
    else: approximate =  False
    if 'manual_cls' in _3to2kwargs: manual_cls = _3to2kwargs['manual_cls']; del _3to2kwargs['manual_cls']
    ...
    u"""Implements __eq__/__ne__/__hash__ via a _value_equality_values_ method.
    ...
    if distinct_child_types and manual_cls:
        raise ValueError(u"'distinct_child_types' is "
                         u"incompatible with 'manual_cls")

    values_getter = getattr(cls, u'_value_equality_values_', None)
    if values_getter is None:
        raise TypeError(u'The @cirq.value_equality decorator requires a '
                        u'_value_equality_values_ method to be defined.')

    if distinct_child_types:
        setattr(cls, u'_value_equality_values_cls_', lambda self: type(self))
    elif manual_cls:
        cls_getter = getattr(cls, u'_value_equality_values_cls_', None)
        if cls_getter is None:
            raise TypeError(u'The @cirq.value_equality decorator requires a '
                            u'_value_equality_values_cls_ method to be defined '
                            u'when "manual_cls" is set.')
    else:
        setattr(cls, u'_value_equality_values_cls_', lambda self: cls)
    setattr(cls, u'__hash__', None if unhashable else _value_equality_hash)
    setattr(cls, u'__eq__', _value_equality_eq)
    setattr(cls, u'__ne__', _value_equality_ne)

    if approximate:
        if not hasattr(cls, u'_value_equality_approximate_values_'):
            setattr(cls, u'_value_equality_approximate_values_', values_getter)
        setattr(cls, u'_approx_eq_', _value_equality_approx_eq)
