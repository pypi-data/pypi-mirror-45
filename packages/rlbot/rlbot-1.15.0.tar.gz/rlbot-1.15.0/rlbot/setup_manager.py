...
class SetupManager:
    ...
    def connect_to_game(self):
        ...
        if not process_configuration.is_process_running(ROCKET_LEAGUE_PROCESS_INFO['program'],
            ...
            try:
                ...
                webbrowser.open(f"steam://rungameid/{ROCKET_LEAGUE_PROCESS_INFO['gameid']}//-rlbot")
            except webbrowser.Error:
                self.logger.info(
                    "Unable to launch Rocket League automatically. Please launch Rocket League manually using the -rlbot option to continue.")
    ...
    def get_game_mode(self):
        if self.start_match_configuration is not None:
            return self.start_match_configuration.game_mode
        return 0
    ...
    def ensure_rlbot_gateway_started(self):
        ...
        if self.rlbot_gateway_process is None or not psutil.pid_exists(self.rlbot_gateway_process.pid):
            # TODO: Handle this stuff internally in RLBot.exe. If we switch arena type within
            # a session, then we're in trouble.
            game_mode = self.get_game_mode()
            if game_mode == 1:  # hoops
                prediction_util.copy_pitch_data_to_temp('hoops')
            elif game_mode == 2:  # dropshot
