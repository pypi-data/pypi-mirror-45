...
def create_key(request, data):
    ...
    print("%s_%s_%s" % (subscription, sha256, request['message']['messageId']))
...
def handle_pubsub_retry(request, data, project_id, topic_name):
    key = create_key(request, data)
    counter = get_count(key)
    return handle_error(data, counter, project_id, topic_name)