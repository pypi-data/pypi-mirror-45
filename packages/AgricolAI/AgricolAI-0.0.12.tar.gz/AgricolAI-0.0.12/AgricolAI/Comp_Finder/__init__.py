...
class Comp_Finder():
    def __init__(self, niter=100, kfold=5, PLS_filter=.1, pid=0):
        self.niter = niter
        self.kfold = kfold
        self.PLS_filter = PLS_filter
        self.pid = pid
        print("James Model(niter=%d, kfold=%d, PLS_filter=%.2f, #ofGPU=%d)"%(self.niter, self.kfold, self.PLS_filter, self.pid))
...
class Selector():
    ...
    def select_by_NN(self, top, one_time=False):
        ...
        model_1st = Model(inputs = model.input, outputs = model.get_layer("AE").output)
