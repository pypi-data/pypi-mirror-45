...
def getRobotConfiguration(input_ = "pepper", options_ = ""):
    node_name = input_
    path = getRIZEpath()
    print (path + "/static/robots/" + node_name + ".json")
    json_node = read_json(path + "/static/robots/" + node_name + ".json")
...
def getNodeConfiguration(input_ = "pepper", options_ = ""):
    node_name = input_
    path = getRIZEpath()
    print (path + "/static/devices/" + node_name + ".json")
    json_node = read_json(path + "/static/devices/" + node_name + ".json")
    dict_node = json2dict(json_node)
    print (json_node)
    return True, dict_node
...
def onLoadRobots(input_ = "", options_ = ""):
    lista = []
    path = getRIZEpath()
    files = nep.getFiles(path + "/static/robots")
    for f in files:
        json_value = read_json(path + "/static/robots/" + f)
...
def onSaveRobots(input_ = "", options_ = ""):
    ...
    path = getRIZEpath()
    path_projects = path + "/static/robots"
...
def onLoadNode(input_ = "", options_ = ""):
    lista = []
    path = getRIZEpath()
    files = nep.getFiles(path + "/static/devices")
    for f in files:
        json_value = read_json(path + "/static/devices/" + f)
        dict_value = json2dict(json_value)
        lista.append(dict_value)
    dict_nodes = {"values":lista}
    return dict_nodes
...
def onSaveNode(input_ = "", options_ = ""):
    ...
    path = getRIZEpath()
    path_projects = path + "/static/devices"
...
def onLoadSettings(input_ = "", options_ = ""):
    path = getRIZEpath()
    json_value = read_json(path + "/static/rize/configuration.json")
    dict_value = json2dict(json_value)
    return dict_value
...
def onSaveSettings(input_ = "", options_ = ""):
    path = getRIZEpath()
    path_projects = path + "/static/rize"
