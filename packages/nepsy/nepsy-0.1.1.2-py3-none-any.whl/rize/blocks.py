...
class block_program():
        def __init__(self):
            ...
            block_cognition = kapuccino.fsm_bt(self.blackboard_model, "ZMQ")
            thread.start_new_thread( block_cognition.managment)
            ...
            while True:
                ...
                if s:
                    ...
                    if msg["action"] == "action":
                        print (" ---- NEW ACTION ----")
                        block_cognition.runAction(msg["input"])
                        time.sleep(.5)
                    elif msg["action"] == "start":
                        print (" ---- NEW PROGRAM ----")
                        block_cognition.loadProgram(rize.getRIZEpath() +  "/static/projects/" +  str(msg["input"]))
                        time.sleep(.5)
                        block_cognition.stop = False
                    elif msg["action"] == "stop":
                        print (" ---- STOP PROGRAM ---- ")
                        block_cognition.stop = True
                    elif msg["action"] == "close":
                        block_cognition.stop = True
                        block_cognition.on = False
                        print (" ---- CLOSING... ---- ")
                        time.sleep(1)
                        break
                time.sleep(1)
...
class block_execution():
        ...
        def onBuildBlockProgram(self,input_="", options_=""):
                ...
                project_name = input_
                path_projects = rize.getRIZEpath() + "/" + "static" + "/" + "projects"
...
def onLoadBlocksPrograms(input_ = "", options_ = ""):
        ...
        list_projects = os.listdir(path + "/static/projects")
        print ("Projects")
        print (list_projects)
