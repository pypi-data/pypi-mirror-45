...
class ThunderBag():
    ...
    def bound_setter(self, bounds=None):
        if not bounds:
            ...
            bounds = peak_finding.make_bounds(first_thunder.tightness, first_thunder.no_peaks, first_thunder.bounds,
                                              first_thunder.peak_widths, first_thunder.peak_centres,
                                              first_thunder.peak_amps)
        for thund in self.thunder_bag.values():  # set these first values for all of them
    ...
    def get_fit_stats(self):
        ...
        for key, thund in self.thunder_bag.keys():
            chisq = thund.peaks.chisqr
            reduced_chi_sq = thund.peaks.redchi
