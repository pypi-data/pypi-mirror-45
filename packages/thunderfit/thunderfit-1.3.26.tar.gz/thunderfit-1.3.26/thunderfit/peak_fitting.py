...
def fit_peaks(x_data, y_data, peak_types, peak_centres, peak_amps, peak_widths, bounds):
    ...
    if not peaks.success:
        ...
        #print('peaks failed to fit, removing bounds and trying again')
        #for peak in model_specs:
        #    peak['bounds'] = None
        #model, peak_params = generate_model(model_specs)
        #peaks = model.fit(y_data, peak_params, x=x_data)
        #if not peaks.success:
        #    print("removing bounds didn't work, setting peak params to none")
        #    peak_params = {key: nan for key in peak_params}  # make sure this is properly handled later!
        #peaks.success = False # we will access this later to save the plots with bounds removed
