...
def main():
    ...
    if arguments.get('clip_data', False) or arguments.get('bg_first_only', False) or arguments.get(
        ...
        bag.choose_spectrum(arguments) # choose which spectrum to base everything off of
    ...
    if arguments.get('bounds_first_only', False):
        bag = bag.bound_setter()
    else:
        bounds = {'amps': False, 'centers': False, 'widths': False}
        bag = bag.bound_setter(bag, first, bounds)
    ...
    ###### plot map scan
    map_scan_tools.plot_map_scan(bag, bag.fit_params, dirname)
    ...
    utili.save_fit_report(bag.stats, path=dirname, filename=f"{file_name}_report.json")
    utili.save_fit_report(bag.fit_params, path=dirname, filename=f"{file_name}_peak_info.json")
