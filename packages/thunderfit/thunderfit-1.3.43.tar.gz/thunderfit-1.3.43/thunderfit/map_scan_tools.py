...
from numpy import unique, round, array, nanmin, nanmax, nan, empty
...
def map_scan_plot(coordinates, values):
    ...
    for i in range(no_fits):
        ...
        nanmat = empty((xx.max() - xx.min() + 1, yy.max() - yy.min() + 1,))
        nanmat[:] = nan
        data_ = coo_matrix((Z, (xx, yy))).toarray(out=nanmat) # out=nanmat should mean that any missing values are nan
        f = plt.figure()
        ax = plt.gca()
