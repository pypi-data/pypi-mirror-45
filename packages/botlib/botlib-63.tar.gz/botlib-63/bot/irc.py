...
class IRC(bot.base.Bot):
    ...
    def handle_privmsg(self, event):
        ...
        if event.txt.startswith("\001DCC"):
            ...
            txt = "\001VERSION %s %s - %s\001" % ("BOTLIB", bot.base.__version__, bot.base.__txt__)
    ...
    def start(self):
        last = kernel.last(obj.utils.get_type(self.cfg))
        if last:
            self.cfg.update(last)
        prev = Cfg(self.cfg)
        self.cfg.upgrade(obj.base.cfg)
        if prev != self.cfg:
            self.cfg.save()
        ...
        while 1:
            ...
            time.sleep(nr * 3.0)
            nr += 1
        if not obj.base.cfg.resume and not self.state.error:
...
class DCC(bot.base.Bot):
    ...
    def connect(self, event):
        ...
        s.connect((addr, port))
        s.send(bytes('Welcome to %s %s !!\n' % (obj.base.cfg.name, event.nick), "utf-8"))
        s.setblocking(True)
