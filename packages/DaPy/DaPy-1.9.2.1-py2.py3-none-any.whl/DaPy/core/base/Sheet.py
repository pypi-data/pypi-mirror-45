...
class BaseSheet(object):
    ...
    def __init__(self, obj=None, columns=None, nan=nan):
        ...
        elif obj in (None, [], dict(), OrderedDict()):
        ...
        for row in self:
            if where(row) is True:
                ...
                if len(yes_row) == limit:
                    break
        return SeriesSet(yes_row, column, self._nan)
...
class SeriesSet(BaseSheet):
    ...
    def corr(self, method='pearson'):
        ...
        for i, (title, sequence) in enumerate(self.items()):
            corr_line = []
    ...
    def apply(self, func, col='all', inplace=False, axis=0):
        ...
        if axis == 0:
            ...
            new_val = [func(row) for row in self[map_columns]]
