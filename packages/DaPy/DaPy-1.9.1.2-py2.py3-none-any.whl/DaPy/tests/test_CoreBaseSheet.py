...
class Test1_OperaData(TestCase):
    def setUp(self):
        self.frame = Frame(TABLE_DATA, TABLE_COL)
        self.col = SeriesSet(TABLE_DATA, TABLE_COL)
    ...
    def test_append(self):
        ...
        _test_append_row(self.col)
        _test_append_col(self.col)
    ...
    def test_insert(self):
        def _test_insert_row(sheet):
            ...
            self.assertEqual(tuple(sheet.shape), (5, 5))
            self.assertEqual(sheet.missing, [0, 0, 2, 0, 4])
            self.assertEqual(sheet.columns, ['A', 'B', 'C', 'D', 'C_4', 'C_5'])
        ...
        def _test_insert_col(sheet):
            ...
            self.assertEqual(tuple(sheet.shape), (5, 6))
            self.assertEqual(sheet.missing, [2, 0, 0, 0, 1, 0])
        ...
        _test_insert_row(self.col)
        _test_insert_col(self.col)
