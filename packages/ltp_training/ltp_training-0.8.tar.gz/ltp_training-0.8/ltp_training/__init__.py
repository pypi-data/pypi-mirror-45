...
def getResultsPY(team_name, username, password):
    try:
        ...
        predictions = queryRedshiftToPandas(query, username, password) #see the function definition at the end of the code
        ...
        predictions=predictions[['teamname', 'error']] #drop unimportant columns
        predictions=predictions.groupby(['teamname']).mean().reset_index() #group by to get the average accuracy per team

        team_prediction=predictions[predictions.teamname==team_name] #select the specific team's result
...
def queryRedshiftToPandas(query, username, password):
    ...
    engine = get_engine_redshift(username, password) 
...
getResultsPY('xxxx','nunopedro','oH3mofoZBd0V')