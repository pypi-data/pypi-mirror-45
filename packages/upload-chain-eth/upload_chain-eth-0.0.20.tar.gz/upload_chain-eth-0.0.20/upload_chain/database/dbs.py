...
from upload_chain.database.modles import (
    Person, PersonDc, Soldiers, Keys, Contract, Events, Army, Organization, db, Account, QueueFlow
)
from upload_chain.logger import logger
from upload_chain.exceptions import ContractError
...
def query_keys(machine):
    ...
    :return: Key对象
    """
    key = Keys.select().where(Keys.machine == machine)
    if len(key) == 1:
        # 如果有数据，就直接使用这条数据
        return key[0]
    if len(key) == 0:
