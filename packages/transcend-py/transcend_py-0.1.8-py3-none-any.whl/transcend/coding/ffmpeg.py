...
from transcend.coding.cli import *
from transcend.coding import faststart
from transcend.coding.exceptions import FastStartError
...
class Ffmpeg(Runnable):
    ...
    '''
    __slots__ = 'input', '_opts', '_with_schedtool'
    BINPATH = '/home/ubuntu/bin'
    BIN = 'ffmpeg'
    OPT = [
        Flag('y', 'overwrite'),
    ...
    def __init__(self, input=None, with_schedtool=True, *opt, **opts):
    ...
    def _process_stdout(self, stdout_queue, stdout, STDOUT):
        try:
    ...
    def _exec_pipe(self, *outputs, stdout=None, stderr=None,
        ...
        proc = subprocess.Popen(self.get_cmd(*outputs),
        ...
        while proc.poll() is None:
            ...
            STDOUT = self._process_stdout(stdout_queue, stdout, STDOUT)
            STDERR = self._process_stdout(stderr_queue, stderr, STDERR)
