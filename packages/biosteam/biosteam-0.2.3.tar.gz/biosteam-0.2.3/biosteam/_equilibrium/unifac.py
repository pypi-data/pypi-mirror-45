...
class UNIFAC_subgroup:
    ...
    #          +f'main_group_id = {self.main_group_id}\n'
    #          +f'main_group = {self.main_group}\n'
    #          +f'R = {self.R}\n'
    #          +f'Q = {self.Q}')
...
def calc_set_up(subgroups, chemgroups):
    ...
    for groups in chemgroups:
        ...
        for group, count in groups.items():
            ...
            if group in group_counts: group_counts[group] += count
            else: group_counts[group] = count
        rs.append(ri)
        qs.append(qi)
...
# For Streams
def DORTMUND(species, xs, T, cached={}):
...
def DORTMUND_psi(T, subgroup1, subgroup2, subgroup_data, interaction_data):
    main1 = subgroup_data[subgroup1].main_group_id
    main2 = subgroup_data[subgroup2].main_group_id
    try:
...
def UNIFAC_Coeffictients(xs, T, subgroups, interactions,
    ...
    UNIFAC_psis = {k: {m: (psi_function(T, m, k, subgroups, interactions))
