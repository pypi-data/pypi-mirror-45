...
def collect_roots(root):
    ...
    if isdir(root):
        ...
        try:
            ...
            for r in chain.from_iterable(imap(collect_roots, deps)):
                yield r
...
def main():
    ...
    parser.set_defaults(
        ...
        quiet=os.environ.get('QUIET_ENV') or sys.stderr.isatty())
    ...
    if args.platform:
        ...
        sys.stderr.write('warning: platform not set\n')
    ...
    host_os = '{0}-{1}'.format(os.uname()[4],
                               LbPlatformUtils.host_os().split('-')[1])
    ...
    env = dict((name, value) for name, value in env.items()
