...
async def parse(error, method, name, data):
...
def create(client, objects, model, authorize, fail, parse = parse):
    ...
    def _parse(method, path, data):

        return parse(fail, method, path.name, data)

    handle_ = handle.Handle(seed_, authorize, fail, parse)
