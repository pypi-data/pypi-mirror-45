...
class Request(object):
    ...
    def parameter_string(self):
        param_hash = self.merge_two_dicts(self.query_dict, self.auth_dict or {})
        ...
        params_dict = OrderedDict(sorted(params_dict.items()))
        params_list = []
    ...
    def string_to_sign(self):
        return bytes("\n".join([self.method, self.path, self.parameter_string()]), 'utf-8')

    def signature(self, token):
        return hmac.new(token.secret, self.string_to_sign(), hashlib.sha256).hexdigest()
