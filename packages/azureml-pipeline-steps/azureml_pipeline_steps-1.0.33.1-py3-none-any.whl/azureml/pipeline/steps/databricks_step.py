...
class DatabricksStep(PipelineStep):
    ...
    :param inputs: List of input connections for data consumed by this step. Fetch this inside the notebook
                    using dbutils.widgets.get("input_name"). Can be DataReference or PipelineData.
    ...
    :param python_script_name: Name of a python script (relative to source_directory). If the script takes inputs
        and outputs, those will be passed to the script as parameters. In additional, the following parameters will
        be available within the script:
        ...
        AZUREML_SCRIPT_DIRECTORY_NAME: Directory path structure in DBFS where your source_directory has been copied.
