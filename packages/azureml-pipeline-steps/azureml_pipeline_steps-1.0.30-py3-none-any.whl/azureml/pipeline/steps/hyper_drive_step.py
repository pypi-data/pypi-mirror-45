...
class HyperDriveStep(PipelineStep):
    ...
    def __init__(self, name, hyperdrive_run_config,
        ...
        if hyperdrive_run_config is None:
            raise ValueError('hyperdrive_run_config is required')
        from azureml.train.hyperdrive import HyperDriveConfig
        if not isinstance(hyperdrive_run_config, HyperDriveConfig):
            raise ValueError('Unexpected hyperdrive_run_config type: {}'.format(type(hyperdrive_run_config)))
        ...
        estimator_run_config = hyperdrive_run_config._estimator.run_config
    ...
    def create_node(self, graph, default_datastore, context):
        ...
        """
        self._params['HyperDriveRunConfig'] = self._get_hyperdrive_run_config(context._workspace,
        ...
        def _get_param_def(param_name):
            is_metadata_param = param_name in ('HyperDriveRunConfig', 'MetricsOutputName')
    ...
    def _get_hyperdrive_run_config(self, workspace, experiment_name):
        from azureml.train.hyperdrive import _search

        telemetry_values = _search._get_telemetry_values(self._hyperdrive_run_config, workspace)
        ...
        hyperdrive_dto = _search._create_experiment_dto(self._hyperdrive_run_config, workspace,
                                                        experiment_name, telemetry_values)

        return json.dumps(hyperdrive_dto.to_dict())
