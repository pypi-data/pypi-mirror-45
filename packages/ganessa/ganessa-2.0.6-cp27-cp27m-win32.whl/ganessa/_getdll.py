...
def _add_dir_to_path(ndir, dll):
    ...
    if status:
        if ndir.lower() not in unistr(environ['path']).lower().split(';'):
            if debuglevel > 2: print(environ['path'])
