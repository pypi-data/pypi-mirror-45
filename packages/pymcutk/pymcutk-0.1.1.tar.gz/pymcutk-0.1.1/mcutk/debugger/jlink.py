...
class JLINK(DebuggerBase):
    ...
    def flash(self, filepath, addr=0):
        ...
        if filepath.endswith(".hex"):
            ...
            # if "0x" in str(self._board.start_address):
            #     self._board.start_address = self._board.start_address.replace("0x", "")
            # self._board.start_address = int(self._board.start_address)
    ...
    def get_gdbserver(self, devicename=None, jlinkscript=None, **kwargs):
    ...
    def gdb_init_template():
        """
        Jlink GDB server default gdb commands.
        Return a string.
