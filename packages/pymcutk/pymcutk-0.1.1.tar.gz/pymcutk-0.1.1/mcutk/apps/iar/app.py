        ...
        if exitcode == 0:
            ...
            if logfile:
                with open(logfile, "r") as fobj:
                    content = fobj.read()
                s = re.compile(r'Total number of warnings:\s*[1-9]', re.I).search(content)
                result = "warning" if s else "pass"
...
def _cspy_programming(iar_root, prjdir, usbid, target, debugfile, timeout):
    ...
    ret = run_command(cspybat_command, timeout=timeout)
    logging.info('iar cspybat.exe exit code: %s', ret)
    return ret


def _update_xcl_file(iar_root, prjdir, target, glob_pattern):
    """Replace iar_root and project path in xcl file."""
    str_root_pattern = r"(C:.Program.Files.*IAR Systems\\Embedded Workbench[\w\W]*\\arm\\)|(C:.PROGRA~2.IARSYS~1.EMBEDD~.*(\/)|(\\)arm\/)|(C:.PROGRA~2.IAR.*(\/)|(\\)arm\/)"
...
def get_all_instance():
    ...
    try:
        ...
        print("Error: Not found IAR instance in your system!")
        return version_pool
