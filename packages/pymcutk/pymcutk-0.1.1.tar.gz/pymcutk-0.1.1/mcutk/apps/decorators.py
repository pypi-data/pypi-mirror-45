...
def build(func):
    ...
    def wraper(*args, **kwargs):
        ...
        returncode = run_command(cmdline, shell=True, stdout=False, timeout=timeout)[0]
        ...
        if _toolchain.name in ("mcux", "lpcx", "ds5") and workspace:
            output_abs = os.path.join(workspace, output)
        else:
            # Assume output file is located project root
        ...
        if not os.path.exists(output_abs):
            logging.warning("Warning: output path is not exists: [%s]", output_abs)
            return br
        ...
        for ext in valid_extension:
            ...
            logging.warning("Warning: unable to find output in directory: [%s]", output_abs)
