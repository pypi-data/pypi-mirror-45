...
class APP(IDEBase):
    """ARMGCC interface impleantation.
    This class inherit from IDEBase, and it has the ability to build project,
    and parse the build result.

    Parameters:
        path :  armgcc tool path.The GNU Arm Embedded toolchains are integrated and validated packages featuring the Arm Embedded GCC compiler,
                libraries and other GNU tools necessary for bare-metal software development on devices based on the Arm Cortex-M and Cortex-R processors.
                The toolchains are available for cross-compilation on Microsoft Windows, Linux and Mac OS X host operating systems.
    ...
    Create an instance:
        (1) search in windows registry:
            ...
            >>> armgcc = APP.get_latest()
            >>> print armgcc.version

        (2) from api
            >>> armgcc = APP(armgcc_path, cmake_path, make_path, version="5.4 2016q3")
            >>> print armgcc.version
    ...
    def build_project(self, project, target, logfile, **kwargs):
        """Return a command line string for armgcc.

        Arguments:
            project {armgcc.Project} -- armgcc project object
            target {string} -- target name
            logfile {string} -- log file path
            cmake_path {string} -- cmake tool path.
            make_path {string}  --  make tool path.
    ...
    @staticmethod
    def get_latest():
        """Search and return a latest armgcc instance in system
