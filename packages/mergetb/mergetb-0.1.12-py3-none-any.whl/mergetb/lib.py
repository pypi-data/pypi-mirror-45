...
from mergetb.term import green, gray, red, blue, cyan, neutral
...
def materialize_attach(pid, eid, rid, mid, key):
    """Attach the given key to the materialization connection.

    :param pid: project id
    :param eid: experiment id
    :param rid: realiztaion id
    :param mid: materialization id
    :param key: public key as string
    """
    return do_put_get(
        (
            'projects/'+pid+'/experiments/'+eid+'/realizations/'+rid+
            '/materializations/'+mid+'/attach'
        ),
        {'key': key}
    )

def materialize_detach(pid, eid, rid, mid, key):
...
def connection_create(pid, eid, name, wgcid):
    """Create a materialziation connection to the given materialziation on the container specififed.

    :param pid: project id
    :param eid: experiment id
    :param name: container name/service/containerdId
    :param wgdic: The ID of the wureguard enclave to connect to
    """
    return do_put(
        'projects/'+pid+'/experiments/'+eid+'/xdc/'+name+'/connection/'+wgcid,
        {},
    )
