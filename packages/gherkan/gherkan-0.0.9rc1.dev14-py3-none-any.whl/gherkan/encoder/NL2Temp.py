...
class NLTempMatcher():
    def __init__(self, lang, nmm : NLPModelManager):
    ...
    def get_pn(self, phrase):
        if self.lang == "en":
            ...
            action_lemma = self.lemmatize("".join(phrase.lower().rsplit(actor)))
                ...
                action_lemma = re.sub(r'\b{}\b'.format(actor), "", phrase, re.IGNORECASE)
        ...
        if actor.lower() == self.subj_of_interest:  # if no specification, use the one defined in Feature
            ...
            for key in self.program_dic[actor.lower()]:
                if self.finetune.strip_extra_spaces(action_lemma.lower()) == self.program_dic[actor.lower()][key]:
                    actor_l = actor.split()
