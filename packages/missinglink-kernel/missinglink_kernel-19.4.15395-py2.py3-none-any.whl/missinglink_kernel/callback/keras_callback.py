...
class KerasCallback(BaseCallback, ModelHashInterface, GradCAMInterface, VisualBackPropInterface):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        ml_prefixed_metrics_dict = {self.__prefix_metric(name, is_test=False): value for name, value in metric_data.items()}
    ...
    @contextmanager
    def test(self, model=None, callback=None, generate_confusion_matrix=True):
    ...
    def __wrap_test_results(self, phase, steps, caller):
    ...
    def __ml_test_loop_wrap(self, phase, is_method_bounded, call_test_loop):
        def __ml_test_loop(ins_index, batch_size_index, steps_index):
            def wrap(*args, **kwargs):
                ...
                return self.__wrap_test_results(phase, temp_steps, lambda: call_test_loop(*args, **kwargs))
    ...
    def _patch_test_loop(self, phase, call_test_loop=None):
        training_model = self._get_training_model()
