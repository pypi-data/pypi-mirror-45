...
class VanillaProject(BaseCallback):
    ...
    def experiment(
        ...
        with VanillaExperiment(self, metrics) as experiment:
            try:
                yield experiment
...
class VanillaExperiment(object):
    def __init__(self, project, every_n_secs=None, every_n_iter=None):
    ...
    def test(self, steps=None):
        ...
        with self._context_validator.context(Context.TEST):
            ...
            self._on_test_end(self._phase.labels)
    ...
    def _on_test_begin(self, steps):
        ...
        # noinspection PyProtectedMember
        self._project._test_begin(steps, weights_hash)

    def _on_test_end(self, labels):
        # noinspection PyProtectedMember
        self._project._test_iteration_end(
            self._phase.y_test, self._phase.y_pred, None, is_finished=True, metric_data=self.get_average_metrics(), class_mapping=self._project._convert_class_mapping(labels))
