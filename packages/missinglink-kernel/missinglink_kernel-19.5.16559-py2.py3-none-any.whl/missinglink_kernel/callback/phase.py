...
class Phase(object):
    ...
    def bind_data_generator(self, query, data_callback, volume_id=None, cache_directory=None, batch_size=32,
                            use_threads=None, processes=-1, shuffle=True, cache_limit=None):

        self._queries.add_query(query, volume_id)

        return MLDataGeneratorFactory(query=query, data_callback=data_callback, volume_id=volume_id,
                                      cache_directory=cache_directory, batch_size=batch_size,
                                      use_threads=use_threads, processes=processes, shuffle=shuffle,
                                      cache_limit=cache_limit).create()
...
class PhaseTest(Phase):
    ...
    def add_test_data(self, y_test, y_pred):
    ...
    def bind_data_generator(self, query, data_callback, volume_id=None, cache_directory=None, batch_size=32,
        ...
        return super(PhaseTest, self).bind_data_generator(query, data_callback, volume_id=volume_id,
                                                          cache_directory=cache_directory, batch_size=batch_size,
                                                          use_threads=use_threads, processes=processes, shuffle=shuffle,
